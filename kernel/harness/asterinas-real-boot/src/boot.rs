...
fn run() -> Result<bool, Box<dyn Error>> {
    ...
    // If the ISO digest already failed, do not boot: emit honest failure directly.
    if !iso_verified {
...
fn finalize(
    ...
    eprintln!(
        "[boot] boot_serial_entry_selected={} (console=ttyS0 present in captured log)",
        outcome.boot_serial_console_selected
    );
    eprintln!(
        "[boot] status={}",
