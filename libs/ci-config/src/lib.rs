//! # ci-config-kernel — the ci CONFORMANCE-FLOOR config kernel
//!
//! Pure, I/O-free parse + CLOSED-schema validation of the portable ci policy into
...
//!
//! ## CLOSED schema
//! Every struct is `#[serde(deny_unknown_fields)]`: an unknown key in `ci.toml` is a
...
/// (merge-base frozen-reference trees still carry `ci.toml` until this rename is in `dev`).
pub const CONFIG_FILENAME_LEGACY: &str = "ci.toml";
...
/// The published `$id` URL for the closed `ci.toml` schema (ADR-0533 §Decision item 5).
pub const SCHEMA_ID: &str = "https://ci.dev/schema/ci-config/v2";
...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::Parse(m) => write!(f, "ci config parse error: {m}"),
            ConfigError::Bundled(m) => write!(f, "ci bundled-default error: {m}"),
        }
    }
}
...
///
/// `Oyatie` is the `#[default]`, so a profile-less `ci.toml` (and the compiled-in
...
impl Profile {
    ...
    /// key when it is the default, so today's `ci.toml` (which carries no `profile` key)
...
/// The full, validated ci policy. Parsed from `ci.toml`, or materialized from the
...
impl OyaCiConfig {
    ...
    /// Parse + closed-schema-validate an `ci.toml`. The top-level `profile` (or `extends`)
...
fn default_allowed_roles() -> Vec<String> {
    ...
    // (13 → 12). Kept byte-for-byte aligned with `ci.toml [naming]` and
    // `governance_predictable_naming_kernel::ALLOWED_ROLES`.
    [
        "kernel",
        "domain",
        "usecase",
...
fn default_doctrinal_carve_outs() -> Vec<String> {
    vec![
        "tooling-agent-read".to_owned(),
        "ci-gate-contract".to_owned(),
    ]
}
...
fn default_vocab_carve_outs() -> Vec<VocabCarveOut> {
    let rows: &[(VocabCarveOutKind, &str, &[&str], &str)] = &[
        ...
        (
            ...
            "libs/ci-config/",
            &[],
            "the config-era deny-list SSOT (forbidden-stem table + bundled disposition) — naming a stem here is the deny-list, not residue (same rationale as check-brand-residue)",
        ),
        (
        ...
        (
            ...
            &[],
            "integration test fixtures that reference live repo contracts/openapi/foundry/ paths and fixture data strings — structural references to real contract paths, not brand residue; moved from oya/developer-sdk/crates/dev-cli/tests/ where it was already baselined",
        ),
        (
            VocabCarveOutKind::PathExact,
...
impl EnforcementConfig {
    ...
    /// `governance` is a brand literal) and NO `governance_lanes` (ADR-0533 item 1 — "no
...
/// hardcoded `.ci/faces/`-class literal at the producer's `main.rs`, so the faces location
...
mod tests {
    ...
    fn bundled_default_matches_todays_naming_consts() {
        ...
        assert_eq!(cfg.naming.check_family_prefix, "check-");
    ...
    fn bundled_default_matches_todays_vocab_consts() {
        ...
        // 9 carve-out rules, including the line-level palantir exemption + the ci-config
...
"#;
    ...
    fn output_and_cross_artifact_sections_default_under_oyatie() {
        ...
        let toml =
            "[output]\nfaces_dir = \".ci/faces\"\n[cross_artifact]\nsources = [\"x.json\"]\n";
        let cfg = OyaCiConfig::from_toml_str(toml).expect("parses");
        assert_eq!(cfg.output.faces_dir, ".ci/faces");
