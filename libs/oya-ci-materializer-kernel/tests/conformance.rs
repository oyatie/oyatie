...
use oya_ci_materializer_kernel::{
    ControlPlane, FindingCode, MaterializeScope, OutputSink,
    evaluate, plan,
};
...
fn cp1_plan_determinism() {
    ...
    let scope_a = MaterializeScope::Consume { target_paths: BTreeSet::new() };
    let scope_b = MaterializeScope::Consume { target_paths: BTreeSet::new() };
...
fn cp1_plan_determinism_synthetic() {
    ...
    let plan_a = plan(&manifest, MaterializeScope::Consume { target_paths: BTreeSet::new() }).unwrap();
    let plan_b = plan(&manifest, MaterializeScope::Consume { target_paths: BTreeSet::new() }).unwrap();
    assert_eq!(plan_a, plan_b, "CP-1: synthetic fixture plan must be deterministic");
}
...
fn cp2_topological_order_from_input_contract() {
    let manifest = minimal_two_artifact_manifest();
    let result = plan(&manifest, MaterializeScope::Consume { target_paths: BTreeSet::new() }).unwrap();
    ...
    assert!(emitter_pos < producer_pos, "CP-2: emitter must precede producer");
}

/// CP-2 extended: in the synthetic fixture, ts-schema precedes api-client because
/// api-client's input_contract references "emit-graphql-types" (ts-schema's operation_id).
#[test]
...
fn cp3_canary_catches_nondeterminism() {
    ...
    let pass_b = vec![
        ("emitter".to_owned(), "emitter-bytes".to_owned()),
        ("producer".to_owned(), "producer-bytes-v2-DIFFERENT".to_owned()),
    ];
    let findings = evaluate(&pass_a, &pass_b, &[], &manifest);
    assert!(!findings.is_green(), "CP-3: nondeterministic producer must yield RED");
    assert!(
        findings.findings.iter().any(|f| f.code == FindingCode::GeneratedArtifactNondeterministic),
    ...
    assert!(evaluate(&pass_a2, &pass_b2, &[], &manifest).is_green(), "CP-3: deterministic -> GREEN");
}
...
fn cp4_single_build_invariant() {
    let manifest = minimal_two_artifact_manifest();
    let result = plan(&manifest, MaterializeScope::DeterminismCanary {
        target_paths: BTreeSet::new(),
    }).unwrap();

    for step in &result.steps {
        assert_eq!(step.multiplicity, 2,
            "CP-4: de-commit step '{}' must have multiplicity=2 in canary scope", step.artifact_id);
        assert_eq!(step.output, OutputSink::TwoCapturedBuffers,
            "CP-4: de-commit step '{}' must use TwoCapturedBuffers", step.artifact_id);
    }
    ...
    let committed_plan = plan(&committed_manifest, MaterializeScope::DeterminismCanary {
        target_paths: BTreeSet::new(),
    }).unwrap();
    let step = committed_plan.steps.iter().find(|s| s.artifact_id == "committed-face").unwrap();
    assert_eq!(step.multiplicity, 1, "CP-4: committed-class step must have multiplicity=1");
}
...
fn cp5_anti_forgery_full_path_keying() {
    ...
    assert!(!decommit.contains(&"ci/scm-facts.generated.json"),
        "CP-5: canonical path must not be de-committed by the evil row");
    assert!(decommit.contains(&"evil/scm-facts.generated.json"),
    ...
    assert!(
        findings.findings.iter().any(|f| f.artifact_id == "canonical"
            && f.code == FindingCode::GeneratedArtifactStale),
...
fn cp6_repo_agnosticism_synthetic_fixture() {
    let manifest = synthetic_manifest();
    let result = plan(&manifest, MaterializeScope::Consume {
        target_paths: BTreeSet::new(),
    }).expect("CP-6: engine must plan the synthetic fixture without error");

    // 5 artifacts in the fixture (4 not-tracked + 1 committed).
    assert_eq!(result.steps.len(), 5, "CP-6: plan must cover all 5 synthetic artifacts");

    // 2 node-codegen steps.
    let node_count = result.steps.iter().filter(|s| s.runner_id == "node-codegen").count();
    assert_eq!(node_count, 2, "CP-6: plan must include 2 node-codegen steps");
    ...
    let actual: BTreeSet<&str> = result.steps.iter().map(|s| s.generator_target.as_str()).collect();
    assert_eq!(actual, expected, "CP-6: plan targets must be exactly what the fixture declares");
}
            ...
            for (lineno, line) in content.lines().enumerate() {
                if line.contains(lit) {
                    ...
                    violations.push(format!(
                        ...
                        entry.display(), lineno + 1, lit, line.trim()
                    ));
                }
            }
        }
        ...
        for pattern in &banned {
            for (lineno, line) in content.lines().enumerate() {
                if line.contains(pattern) {
                    ...
                    violations.push(format!(
                        "{}:{}: banned symbol {:?}: {}",
                        entry.display(), lineno + 1, pattern, trimmed
                    ));
                }
            }
        }
...
fn mf1_evaluate_accepts_separate_manifest_parameter() {
    ...
    assert!(evaluate(&pass_a, &pass_b, &[], &candidate).is_green(),
    ...
    assert!(!evaluate(&pass_a, &pass_b, &committed, &mergebase).is_green(),
