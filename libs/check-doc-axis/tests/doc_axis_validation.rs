...
/// Set OYATIE_TODAY so date-sensitive tests are reproducible.
...
fn set_today(date: &str) {
    unsafe {
        std::env::set_var("OYATIE_TODAY", date);
    }
}
...
fn rule4_manifest_crate_missing_from_catalog_blocks() {
  ...
  "name": "my-service",
  "bounded_contexts": [
    { "name": "core", "crates": ["check-doc-axis"] }
  ]
}"#;
    write_file(root, "microservices/my-service/manifest.json", manifest);
    // Do NOT create registry/catalog/check-doc-axis.yaml.
    let result = validate(root, false);
...
fn rule4_manifest_with_matching_catalog_passes() {
  ...
  "bounded_contexts": [
    ...
    write_file(
        root,
        "registry/catalog/check-doc-axis.yaml",
        "context: governance\nrole: gate\n",
    );
    let result = validate(root, false);
