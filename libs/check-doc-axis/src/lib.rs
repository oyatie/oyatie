...
use governance_adr_shape_kernel::has_canonical_amended_date;
...
///
/// The reference date is read from the `OYATIE_TODAY` environment variable
/// (format `YYYY-MM-DD`) so tests can override it without `SystemTime`.
fn parse_ymd_to_days_ago(ymd: &str) -> Option<i64> {
    let today_str = std::env::var("OYATIE_TODAY").unwrap_or_else(|_| today_ymd_from_system());
...
mod tests {
    ...
    #[test]
    fn extract_crates_from_manifest_finds_crates() {
        let manifest = r#"{
  "bounded_contexts": [
    {
      "name": "core",
      "crates": ["check-doc-axis", "check-no-grouping"]
    }
  ]
}"#;
        let crates = extract_crates_from_manifest(manifest);
        assert!(crates.contains(&"check-doc-axis".to_string()));
        assert!(crates.contains(&"check-no-grouping".to_string()));
    }
