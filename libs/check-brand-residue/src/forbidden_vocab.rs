...
//! engine. The `cloud-ci-accounting-registry-app` producer calls [`census_findings`]
...
//! - the catalog spec (`registry/catalog/check-brand-residue.yaml`) — same reason;
...
pub const CARVE_OUT_RULES: &[CarveOutRule] = &[
    CarveOutRule {
        ...
        value: "libs/check-brand-residue/",
        exempt_stems: &[],
    ...
    CarveOutRule {
        ...
        exempt_stems: &[],
        reason: "the frozen cloud-/oya- rename baseline ENUMERATES the identifiers still to be renamed; some of those identifiers contain a forbidden stem, so listing them is the census, not residue (same rationale as check-brand-residue's own source). A file that names the debt is not the debt.",
    },
    CarveOutRule {
        kind: CarveOutKind::PathExact,
    ...
    CarveOutRule {
        kind: CarveOutKind::PathPrefix,
        value: "libs/ci-config/",
        exempt_stems: &[],
        reason: "the config-era deny-list SSOT (forbidden-stem table + bundled disposition) — naming a stem here is the deny-list, not residue (same rationale as check-brand-residue)",
    },
    CarveOutRule {
        kind: CarveOutKind::PathExact,
        value: "ci.toml",
    ...
    CarveOutRule {
        kind: CarveOutKind::PathExact,
        value: "registry/catalog/check-brand-residue.yaml",
        exempt_stems: &[],
    ...
    CarveOutRule {
        ...
        exempt_stems: &[],
        reason: "integration test fixtures that reference live repo contracts/openapi/foundry/ paths and fixture data strings — these are structural references to real contract paths, not brand residue; the file was moved from oya/developer-sdk/crates/dev-cli/tests/ where it was already baselined",
    },
    CarveOutRule {
        kind: CarveOutKind::PathExact,
...
/// from `ci.toml`'s `[vocab]` section. [`VocabPolicy::bundled_default`] reproduces the
...
mod tests {
    ...
    fn strict_zero_has_no_path_class_carve_outs() {
        ...
        for path in [
            ...
            "libs/check-brand-residue/src/forbidden_vocab.rs",
        ] {
            assert!(
    ...
    fn generated_faces_are_carved_out() {
        assert!(is_path_carved_out(
            "cloud/cloud-ci/gates/cloud-ci-accounting-registry-app/gate-baseline.generated.json"
        ));
    }
