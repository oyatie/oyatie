//! SQLx Postgres adapter for the owned `data` SQL port.
//!
...
//! `shared-postgres-command-kernel::SET_LOCAL_TENANT_SQL`) inside the
...
//! database, mirroring the `shared-postgres-command-adapter-sqlx`
//! live-probe pattern.
...
use shared_postgres_command_kernel::{PostgresPoolConfig, SET_LOCAL_TENANT_SQL};
...
pub const LIVE_DATA_POSTGRES_ENABLE_ENV: &str = "OYATIE_DATA_LIVE_POSTGRES";
/// Admin (schema-owning) connection URL for the live harness.
pub const LIVE_DATA_POSTGRES_ADMIN_URL_ENV: &str = "OYATIE_DATA_POSTGRES_ADMIN_URL";
/// Application-role (RLS-subject) connection URL for the live harness.
pub const LIVE_DATA_POSTGRES_APP_URL_ENV: &str = "OYATIE_DATA_POSTGRES_APP_URL";
...
const LIVE_RLS_SCHEMA: &str = "data_live_rls";
const LIVE_RLS_DROP_SCHEMA_SQL: &str = "DROP SCHEMA IF EXISTS data_live_rls CASCADE";
const LIVE_RLS_CREATE_SCHEMA_SQL: &str = "CREATE SCHEMA data_live_rls";
const LIVE_RLS_CREATE_TABLE_SQL: &str = "CREATE TABLE data_live_rls.tenant_rows (tenant_id text NOT NULL, row_id text NOT NULL, payload text NOT NULL, PRIMARY KEY (tenant_id, row_id))";
const LIVE_RLS_ENABLE_SQL: &str =
    "ALTER TABLE data_live_rls.tenant_rows ENABLE ROW LEVEL SECURITY";
const LIVE_RLS_FORCE_SQL: &str =
    "ALTER TABLE data_live_rls.tenant_rows FORCE ROW LEVEL SECURITY";
const LIVE_RLS_CREATE_POLICY_SQL: &str = "CREATE POLICY tenant_isolation ON data_live_rls.tenant_rows USING (tenant_id = current_setting('oyatie.tenant_id', true)) WITH CHECK (tenant_id = current_setting('oyatie.tenant_id', true))";
const LIVE_RLS_GRANT_USAGE_SQL: &str = "GRANT USAGE ON SCHEMA data_live_rls TO PUBLIC";
const LIVE_RLS_GRANT_TABLE_SQL: &str =
    "GRANT SELECT, INSERT, UPDATE, DELETE ON data_live_rls.tenant_rows TO PUBLIC";
const LIVE_RLS_INSERT_SQL: &str =
    "INSERT INTO data_live_rls.tenant_rows (tenant_id, row_id, payload) VALUES ($1, $2, $3)";
const LIVE_RLS_SELECT_ROWS_SQL: &str =
    "SELECT tenant_id, row_id, payload FROM data_live_rls.tenant_rows ORDER BY row_id";
...
/// `current_role_bypassrls` in data-outbox-adapter-postgres; `rolsuper`
...
mod tests {
    ...
    /// Postgres via OYATIE_DATA_LIVE_POSTGRES + admin/app URLs.
    #[tokio::test]
