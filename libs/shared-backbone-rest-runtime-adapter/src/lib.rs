...
fn principal_header(request: &HttpRequest) -> Result<String, JsonWriteBindingError> {
    required_header(request, &["x-principal-ref"])
}
...
fn policy_header(request: &HttpRequest) -> Result<String, JsonWriteBindingError> {
    required_header(request, &["x-policy-decision-ref"])
}

fn request_id_header(request: &HttpRequest) -> Result<String, JsonWriteBindingError> {
    required_header(request, &["x-request-id", "x-request-id"])
}
...
fn messenger_context(
    ...
    Ok(messenger_rest::MessengerRestContext {
        ...
        context_kind: messenger_context_kind(&required_header(request, &["x-context-kind"])?)?,
...
fn mail_context(
    ...
    Ok(mail_rest::MailRestContext {
        ...
        context_kind: mail_context_kind(&required_header(request, &["x-context-kind"])?)?,
        principal_ref: principal_header(request)?,
        idempotency_key: idempotency_header(request)?,
        policy_decision_ref: policy_header(request)?,
        request_id: request_id_header(request)?,
...
fn social_context(
    ...
    Ok(social_rest::SocialRestContext {
        scope_org_id: scope_header(request)?,
        context_kind: social_context_kind(&required_header(request, &["x-context-kind"])?)?,
        principal_ref: principal_header(request)?,
        idempotency_key: idempotency_header(request)?,
        policy_decision_ref: policy_header(request)?,
        request_id: request_id_header(request)?,
...
fn tenant_sql_context(request: &HttpRequest) -> Result<TenantSqlContext, JsonWriteBindingError> {
    TenantSqlContext::new(
        ...
        required_header(request, &["x-home-cell"])?,
        required_header(request, &["x-shard-key"])?,
        required_header(request, &["x-jurisdiction-code"])?,
    )
    .map_err(|error| JsonWriteBindingError::SqlPlan {
        reason: format!("{error:?}"),
    })
...
fn backbone_authz_request(
    ...
    BackboneAuthzRequest {
        ...
        tenant_ref: header_value(request, "x-scope-ref").map(str::to_string),
        context_kind: header_value(request, "x-context-kind").map(str::to_string),
        caller_principal_ref: header_value(request, "x-principal-ref").map(str::to_string),
        caller_policy_decision_ref: header_value(request, "x-policy-decision-ref")
            .map(str::to_string),
...
fn backbone_install_verified_authz_headers(
    ...
    remove_header_case_insensitive(&mut request.headers, "x-principal-ref");
    remove_header_case_insensitive(&mut request.headers, "x-policy-decision-ref");
    request.headers.insert(
        "x-principal-ref".to_string(),
        principal.principal_ref.clone(),
    );
    request.headers.insert(
        "x-policy-decision-ref".to_string(),
...
mod tests {
    ...
    fn common_headers(idempotency_key: &str, request_id: &str) -> BTreeMap<String, String> {
        BTreeMap::from([
            ("x-scope-ref".to_string(), "tenant:t".to_string()),
            ("x-context-kind".to_string(), "professional".to_string()),
            ("x-principal-ref".to_string(), "user:u".to_string()),
    ...
    fn community_sql_json_request(method: HttpMethod, path: &str, body: &str) -> HttpRequest {
        ...
        request.headers.extend([
            ("x-home-cell".to_string(), "cell-a".to_string()),
            ("x-shard-key".to_string(), "tenant:t#cell-a".to_string()),
            ("x-jurisdiction-code".to_string(), "US".to_string()),
        ]);
        request
    }
    ...
    fn community_sql_write_plan_json_routes_execute_recorded_sql_batches() {
        ...
        vote_request
            .headers
            .insert("x-principal-ref".to_string(), "user:voter".to_string());
    ...
    fn authz_middleware_passes_pbac_context_and_replaces_caller_authz_headers() {
        ...
        req.headers
            .insert("authorization".to_string(), "Bearer secret".to_string());
        req.headers
            .insert("x-principal-ref".to_string(), "forged:user".to_string());
        req.headers.insert(
            "x-policy-decision-ref".to_string(),
