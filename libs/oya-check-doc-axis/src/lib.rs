...
//!    is older than 14 days must carry a `superseded_by: ADR-NNNN`
//!    frontmatter field linking to a real existing ADR file. Readable archive
//!    storage is not a compliant disposition.
//!
...
fn check_shadow_ideas(
    ...
    for entry in entries.flatten() {
        ...
        // real ADR. Readable archive storage is not a compliant disposition.
        let contents = match fs::read_to_string(&path) {
            Ok(c) => c,
            Err(_) => continue,
        };
        ...
        if let Some(adr_id) = find_superseded_by(&contents) {
            ...
            findings.push(DocAxisFinding {
                ...
                rule_violated: DocAxisRule::ShadowIdea,
                suggested_fix: format!(
                    "Idea-pager is {age_days} days old (limit: {IDEA_PROMOTION_DAYS}). Promote to an ADR and add `superseded_by: ADR-NNNN`."
                ),
                blocking: true,
            });
        }
