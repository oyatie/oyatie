//! `check-dependency-seam` — ADR-0092 D13 dependency seam lane.
//!
...
impl DependencySeamConfig {
    pub fn for_repo(repo_root: impl Into<PathBuf>) -> Self {
        ...
        Self {
            ...
            fixture_root: repo_root.join("crates/check-dependency-seam/tests/fixtures"),
            repo_root,
...
fn read_workspace_members(repo_root: &Path) -> Result<Vec<WorkspaceMember>, String> {
    ...
    let member_dirs = workspace_members_kernel::resolve_member_dirs(repo_root)
...
mod tests {
    ...
    fn fixture_repo(slug: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("dependency-seam-{slug}-{nanos}"));
