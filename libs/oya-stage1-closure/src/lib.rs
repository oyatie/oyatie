...
const MATERIALIZER_BINDING_FIELDS: [&str; 4] = [
...
fn validate_immutable_successor(
    ...
    for field in MATERIALIZER_BINDING_FIELDS {
        validate_artifact_binding(
            successor.get(field).unwrap_or(&Value::Null),
            &format!("immutable_successor.{field}"),
            findings,
        );
    }
    if frozen {
...
fn validate_nullable_successor_facts_ref(facts_ref: &Value, findings: &mut BTreeSet<String>) {
    ...
    {
        findings.insert(
            "immutable_successor.facts_ref.blob_oid must be a Git object identifier".to_owned(),
        );
    }
    if facts_ref
        .get("sha256")
...
fn validate_protected_parent_facts(
    ...
    let successor = epoch.get("immutable_successor").unwrap_or(&Value::Null);
    for field in MATERIALIZER_BINDING_FIELDS {
        if facts.get(field) != successor.get(field) {
            findings.insert(format!(
                "immutable_successor.{field} must exactly equal protected facts {field}"
            ));
        }
    }
    if facts.get("immutable_successor_bundle") != successor.get("facts_ref") {
        findings.insert(
            "immutable_successor.facts_ref must exactly equal protected facts immutable_successor_bundle"
                .to_owned(),
        );
    }
