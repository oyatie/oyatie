...
* `Connector` trait — the contract every `oya-connector-<vendor>-adapter`
  satisfies.
...
* **Ontology.** Adapters publish projections so the Ontology µservice can
  ...
  `oya-check-ontology-projection-coverage` gate enforces coverage.
* **Rate-limit honesty.** Adapters publish budgets; callers schedule.

## Tests

`cargo test -p oya-shared-connector-kernel` — 16 unit tests covering id
