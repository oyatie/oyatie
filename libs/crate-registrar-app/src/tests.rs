...
impl TmpDir {
    fn new(tag: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!("registrar-writers-{tag}-{pid}-{n}"));
...
fn capability_fresh_apply_upserts_into_meta_dir_group() {
    ...
    let next =
        capability_mapping::compute(&current, "libs/crate-registrar-app", "build/").unwrap();
    // The new dir is present, in the build/ group, sorted.
    assert!(next.contains("libs/crate-registrar-app"));
...
fn capability_matches_capability_keyed_group() {
    let current = registry_fixture();
    let next =
        capability_mapping::compute(&current, "libs/data-pagination-kernel", "data").unwrap();
    let value: Value = serde_json::from_str(&next).unwrap();
...
fn capability_reapply_is_byte_identical_noop() {
    ...
    let once =
        capability_mapping::compute(&current, "libs/crate-registrar-app", "build/").unwrap();
    let twice =
        capability_mapping::compute(&once, "libs/crate-registrar-app", "build/").unwrap();
    assert_eq!(
        once, twice,
...
fn capability_already_present_is_byte_identical_noop() {
    let current = registry_fixture();
    // libs/crate-registrar-kernel is ALREADY in the build/ group — re-mapping it must produce
    // bytes identical to the canonical re-serialization of the input.
    let next =
        capability_mapping::compute(&current, "libs/crate-registrar-kernel", "build/").unwrap();
    assert_eq!(
        next, current,
...
fn capability_unknown_slug_rejected() {
    let current = registry_fixture();
    let err = capability_mapping::compute(&current, "libs/whatever-kernel", "totally-made-up")
        .unwrap_err();
    assert_eq!(
        err,
...
fn capability_apply_roundtrip_and_idempotent() {
    ...
    let wrote =
        capability_mapping::apply(&tmp.path, "libs/crate-registrar-app", "build/").unwrap();
    assert!(wrote, "first apply writes the file");
    let after_first = tmp.read(capability_mapping::REGISTRY_PATH);
    assert!(after_first.contains("libs/crate-registrar-app"));
...
fn capability_write_preserves_hand_authored_key_order() {
    ...
    let next = capability_mapping::compute(&current, "libs/crate-registrar-app", "build/")
        .expect("compute");
    ...
    assert!(
        next.contains("libs/crate-registrar-app"),
...
fn adr_creates_block_when_absent() {
    let paths = vec![
        "libs/crate-registrar-app/src/lib.rs".to_owned(),
        "libs/crate-registrar-app/Cargo.toml".to_owned(),
        "libs/crate-registrar-app/BUCK".to_owned(),
    ];
...
fn adr_upserts_into_existing_block_sorted_and_deduped() {
    ...
    let current = "# ADR-0568\n\n## Governed surfaces\n\n```\nlibs/crate-registrar-app/BUCK\nlibs/crate-registrar-app/Cargo.toml\n```\n\n## Consequences\n\nbody\n";
    let paths = vec![
        "libs/crate-registrar-app/Cargo.toml".to_owned(), // duplicate
        "libs/crate-registrar-app/OWNERS".to_owned(),
        "libs/crate-registrar-app/src/lib.rs".to_owned(),
    ];
    ...
    let expected = [
        "libs/crate-registrar-app/BUCK",
        "libs/crate-registrar-app/Cargo.toml",
        "libs/crate-registrar-app/OWNERS",
        "libs/crate-registrar-app/src/lib.rs",
    ];
    let mut last = 0;
...
fn adr_verbatim_paths_are_literal_no_globs() {
    ...
    let paths = vec![
        "libs/crate-registrar-app/src/lib.rs".to_owned(),
        "libs/crate-registrar-app/src/tests.rs".to_owned(),
    ];
    let next = adr_governed_paths::compute(ADR_NO_BLOCK, &paths).unwrap();
    assert!(next.contains("libs/crate-registrar-app/src/lib.rs"));
    assert!(next.contains("libs/crate-registrar-app/src/tests.rs"));
...
fn adr_brace_glob_path_rejected() {
    let paths = vec!["libs/crate-registrar-app/src/{lib,tests}.rs".to_owned()];
    let err = adr_governed_paths::compute(ADR_NO_BLOCK, &paths).unwrap_err();
    assert_eq!(
        err,
        WriterError::BraceGlobInGovernedPath(
            "libs/crate-registrar-app/src/{lib,tests}.rs".to_owned()
        ),
...
fn adr_heading_with_no_fence_followed_by_foreign_code_block_is_not_hijacked() {
    ...
    let current = "# ADR-0568\n\n## Governed surfaces\n\nNo paths yet.\n\n## Consequences\n\n```rust\nlet hijacked = true;\nfn foreign() {}\n```\n\nMore prose.\n";
    let paths = vec!["libs/crate-registrar-app/src/lib.rs".to_owned()];
    let next = adr_governed_paths::compute(current, &paths).unwrap();
    ...
    assert!(
        gov_block.contains("libs/crate-registrar-app/src/lib.rs"),
...
fn adr_info_string_fence_existing_path_is_preserved() {
    ...
    let current =
        "# ADR-0568\n\n## Governed surfaces\n\n```text\nlibs/crate-registrar-app/BUCK\n```\n";
    let paths = vec!["libs/crate-registrar-app/Cargo.toml".to_owned()];
    let next = adr_governed_paths::compute(current, &paths).unwrap();
    assert!(
        next.contains("libs/crate-registrar-app/BUCK"),
        "the existing path under an info-string fence is preserved:\n{next}"
    );
    assert!(next.contains("libs/crate-registrar-app/Cargo.toml"));
...
fn member_glob_covered_dir_is_byte_identical_noop() {
    // `libs/crate-registrar-app` is covered by `libs/oya-*` → return the manifest verbatim.
    let next =
        workspace_member_glob::compute(MANIFEST_FIXTURE, "libs/crate-registrar-app").unwrap();
    assert_eq!(
...
fn member_glob_covered_by_bare_leaf_glob_noop() {
    // A bare-`*` leaf glob (`cloud/cloud-ci/gates/*`) covers a non-prefixed leaf.
    let next =
...
fn member_glob_reapply_is_byte_identical_noop() {
    let once = workspace_member_glob::compute(MANIFEST_FIXTURE, "libs/foo-kernel").unwrap();
    let twice = workspace_member_glob::compute(&once, "libs/foo-kernel").unwrap();
    assert_eq!(
        once, twice,
...
fn member_glob_uncovered_dir_is_fail_closed() {
    // No members glob covers `apps/thing-app` → the writer REFUSES (never synthesizes a glob).
    let err = workspace_member_glob::compute(MANIFEST_FIXTURE, "apps/thing-app").unwrap_err();
    assert_eq!(
        err,
        WriterError::WorkspaceMemberUncovered("apps/thing-app".to_owned()),
...
#[test]
fn member_glob_prefix_narrowing_rejects_non_leaf() {
    // `libs/oya-*` does NOT cover a non-prefixed leaf — the writer must not pretend it does
...
fn member_glob_unparseable_toml_is_fail_closed() {
    let err =
        workspace_member_glob::compute("this is = = not toml", "libs/foo-kernel").unwrap_err();
    assert!(
        matches!(err, WriterError::WorkspaceManifest(_)),
        "got {err:?}"
    );
...
fn member_glob_trailing_slash_dir_is_normalized() {
    ...
    let next = workspace_member_glob::compute(MANIFEST_FIXTURE, "libs/foo-kernel/").unwrap();
    assert_eq!(next, MANIFEST_FIXTURE);
}
...
fn member_glob_apply_covered_is_noop_no_write() {
    ...
    let wrote = workspace_member_glob::apply(&tmp.path, "libs/crate-registrar-app").unwrap();
    ...
    let wrote_again =
        workspace_member_glob::apply(&tmp.path, "libs/crate-registrar-app").unwrap();
    assert!(!wrote_again, "re-apply is a no-op (no write)");
    assert_eq!(
        tmp.read(workspace_member_glob::MANIFEST_PATH),
        MANIFEST_FIXTURE
...
fn member_glob_apply_uncovered_is_fail_closed_no_write() {
    ...
    let err = workspace_member_glob::apply(&tmp.path, "apps/thing-app").unwrap_err();
    assert_eq!(
        err,
        WriterError::WorkspaceMemberUncovered("apps/thing-app".to_owned())
    );
