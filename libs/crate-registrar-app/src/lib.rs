//! # crate-registrar-app (ADR-0568, G011 slice 2)
//!
//! The I/O / adapter half of `register_crate`: the thin WRITERS that APPLY the kernel's typed
//! [`Edit`](crate_registrar_kernel::Edit)s to the born-accounting SSOTs on disk. The pure
//! [`crate-registrar-kernel`](crate_registrar_kernel) computes an ordered
//! [`RegistrationPlan`](crate_registrar_kernel::RegistrationPlan); this crate turns the NEW
...
//!
//! ## The four writers
//! - [`capability_mapping`] — [`Edit::CapabilityMapping`](crate_registrar_kernel::Edit::CapabilityMapping):
...
//! - [`adr_governed_paths`] — [`Edit::AdrGovernedPathAppend`](crate_registrar_kernel::Edit::AdrGovernedPathAppend):
...
//! - [`catalog_yaml`] — [`Edit::CatalogYaml`](crate_registrar_kernel::Edit::CatalogYaml):
...
//! - [`workspace_member_glob`] — [`Edit::WorkspaceMemberGlob`](crate_registrar_kernel::Edit::WorkspaceMemberGlob):
...
//!   `workspace-members-kernel` (the single source of member-glob semantics), so `compute`
...
pub mod workspace_member_glob {
    ...
    ///   glob/ADR decision).
    ///
    /// Coverage is resolved purely through `workspace-members-kernel`
