...
impl OperationLedgerEntry {
    ...
    pub fn validate(&self) -> Result<(), ContractShapeError> {
        ...
        for (field, value) in [
            ("tenant_account_project", self.tenant_account_project.as_str()),
...
impl Operation {
    ...
    pub fn pending(
        ...
        if metadata.state.is_terminal() {
            return Err(ContractShapeError::InvalidOperationState {
                message: format!("pending operation cannot carry terminal state {:?}", metadata.state),
            });
        }
        Ok(Self {
            name,
    ...
    pub fn validate(&self) -> Result<(), ContractShapeError> {
        ...
        match (&self.result, self.metadata.state) {
            ...
            (Some(OperationResult::Response(_)), state) => Err(
                ContractShapeError::InvalidOperationState {
...
mod tests {
    ...
    fn operation_constructors_enforce_done_result_coupling() {
        ...
        .is_err());
        assert!(Operation::pending("operations/", operation_ledger("", OperationState::Running))
            .is_err());
