...
fn assert_operation_ledger_matches<F: ConformanceFixture>(
    ...
    for (field, actual, expected) in [
        ...
        ("region_cell", ledger.region_cell.as_str(), fixture.region_cell()),
...
/// immutable.
pub fn check_operation_ledger_semantics<F: ConformanceFixture>(
    fixture: &F,
) -> Result<(), ConformanceViolation> {
    ...
    match provider.delete(&second_name, &delete_key) {
        Err(ProviderError::IdempotencyKeyReuse { .. }) => {}
        other => {
            return Err(violation(
                CHECK,
    ...
    match provider.operation_ledger_entry("operations/never-issued") {
        Err(ProviderError::NotFound { .. }) => Ok(()),
        other => Err(violation(
            CHECK,
            format!("reading an unknown operation ledger row must fail with not_found, got {other:?}"),
        )),
    }
}
...
pub fn run_all_checks<F: ConformanceFixture>(fixture: &F) -> Vec<ConformanceViolation> {
    [
        ...
        check_operation_ledger_semantics(fixture),
    ]
    .into_iter()
