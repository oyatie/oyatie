...
/// value the producer can source from `ci.toml`'s `[naming]` section. The bundled
...
/// (`ci-config-kernel`) maps its `[naming]` section onto it at the producer boundary.
...
///     (`tooling-agent-read` per ADR-0053, `ci-gate-contract` per
...
/// expand without one).
///
/// Entries as of ADR-0528 REMED-001: `tooling-agent-read`, `ci-gate-contract`.
pub const DOCTRINAL_CARVE_OUTS: [&str; 2] = ["tooling-agent-read", "ci-gate-contract"];
...
/// True iff the crate name follows the `check-<feature>` pattern (bundled-default
...
mod tests {
    ...
    fn undeclared_context_flagged() {
        let r = check(&[row("intelligence-account-kernel", Some("kernel"), None)]).unwrap();
        assert!(
            r.violations
                .iter()
    ...
    fn check_family_recognized_no_declared_role_required() {
        // Per ADR-0105: check-<feature> is self-layering.
        let r = check(&[row("check-brand-residue", None, Some("foundry"))]).unwrap();
        assert!(
            !r.violations
                .iter()
                .any(|v| v.kind == NamingViolationKind::UndeclaredRole),
    ...
    fn backend_qualified_adapter_layer_inferred_correctly() {
        // Per ADR-0105: intelligence-account-adapter-inmemory has layer
        // `adapter`, not `inmemory`. Declared role `adapter` should match.
        let r = check(&[row(
            "intelligence-account-adapter-inmemory",
            Some("adapter"),
            Some("foundry"),
        )])
        .unwrap();
    ...
    fn usecase_layer_accepted() {
        let r = check(&[row("dsr-usecase", Some("usecase"), Some("dsr"))]).unwrap();
        assert!(r.violations.is_empty(), "{:?}", r.violations);
    }

    #[test]
    fn api_layer_accepted() {
        let r = check(&[row("cloud-compute-vm-api", Some("api"), Some("cloud"))]).unwrap();
        assert!(r.violations.is_empty(), "{:?}", r.violations);
    }
    ...
    fn is_check_family_helper() {
        assert!(is_check_family("check-brand-residue"));
        assert!(is_check_family("check-x"));
        assert!(!is_check_family("check-"));
        assert!(!is_check_family("checker"));
        assert!(!is_check_family("governance-check-x"));
    }
    ...
    fn is_backend_qualified_adapter_helper() {
        ...
        assert!(is_backend_qualified_adapter(
            "cloud-billing-adapter-aws"
        ));
        assert!(is_backend_qualified_adapter("x-adapter-postgres"));
        assert!(!is_backend_qualified_adapter(
            "intelligence-account-adapter"
        )); // no backend
        assert!(!is_backend_qualified_adapter(
            "intelligence-account-domain"
        ));
        assert!(!is_backend_qualified_adapter("adapter-foo")); // 'adapter' not penultimate-after-something
    }

    #[test]
    fn is_doctrinal_carve_out_helper() {
        // Closed entries per ADR-0107 and ADR-0528.
        assert!(is_doctrinal_carve_out("tooling-agent-read"));
        assert!(is_doctrinal_carve_out("ci-gate-contract"));
        // Random non-carve-out names are not exempt.
        assert!(!is_doctrinal_carve_out("governance-portfolio-citation"));
        assert!(!is_doctrinal_carve_out("adapter-substitution-test"));
    ...
    fn doctrinal_carve_out_passes_without_role() {
        ...
        let r = check(&[row("tooling-agent-read", None, Some("tooling"))]).unwrap();
        assert!(
            !r.violations
                .iter()
                .any(|v| v.kind == NamingViolationKind::UndeclaredRole),
        ...
        // `contract` layer role.
        let r = check(&[row("ci-gate-contract", None, Some("cloud-ci"))]).unwrap();
        assert!(
            !r.violations
                .iter()
                .any(|v| v.kind == NamingViolationKind::UndeclaredRole),
