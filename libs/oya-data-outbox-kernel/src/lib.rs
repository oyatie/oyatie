//! Transactional outbox + CDC change-stream shapes on the owned `oya-data`
//! SQL port.
//!
...
//! messenger-local `oya-shared-transactional-outbox-kernel` seam this
...
pub const INSERT_OUTBOX_EVENT_SQL: &str = "INSERT INTO oya_data_outbox.outbox_events (tenant_id, event_id, event_kind, aggregate_id, schema_version, idempotency_key, payload) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (tenant_id, idempotency_key) DO NOTHING";
