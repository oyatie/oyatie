...
//!
//! - Crate `governance-adapter-with-no-importer-kernel` —
...
//! - Dev-CLI `governance-adapter-with-no-importer-app` —
...
/// check ignores port-declaring siblings (e.g. `intelligence-adapter-anthropic-api-kernel`).
...
///
/// Excludes port-declaring siblings like `intelligence-adapter-anthropic-api-kernel`
/// (last segment = `kernel`, not adapter) and the fitness kernel itself
/// (`governance-adapter-with-no-importer-kernel` — last segment
/// `kernel`).
...
///
/// Examples:
/// - `intelligence-account-adapter-inmemory` -> `intelligence-account`
/// - `cloud-billing-adapter-aws` -> `cloud-billing`
/// - `intelligence-claude-account-adapter` -> `intelligence-claude-account`
...
mod tests {
    ...
    fn accepts_adapter_with_matching_importer() {
        let report = check(&[
            krate("intelligence-claude-account-adapter"),
            krate("intelligence-claude-account-importer-supervisor"),
        ]);
    ...
    fn flags_adapter_with_no_importer() {
        let report = check(&[krate("cloud-billing-adapter-aws")]);
        assert!(!report.is_clean());
        assert_eq!(report.violations.len(), 1);
        let violation = &report.violations[0];
        assert_eq!(violation.adapter_crate, "cloud-billing-adapter-aws");
        assert_eq!(
            violation.expected_importer_pattern,
            "cloud-billing-importer*"
        );
    ...
    fn variant_suffixed_adapter_matches_base_importer() {
        let report = check(&[
            krate("intelligence-account-adapter-inmemory"),
            krate("intelligence-account-importer-supervisor"),
        ]);
    ...
    fn unrelated_importer_does_not_satisfy() {
        let report = check(&[
            krate("cloud-billing-adapter-aws"),
            krate("cloud-marketplace-importer-supervisor"),
        ]);
        assert!(!report.is_clean());
        assert_eq!(report.violations.len(), 1);
    }
    ...
    fn ignores_adapter_kernel_crates() {
        ...
        let report = check(&[krate("intelligence-adapter-anthropic-api-kernel")]);
        assert!(report.is_clean());
    ...
    fn detects_violation_against_temp_crate_layout() {
        ...
        for name in [
            "cloud-billing-adapter-aws",           // orphan: no importer
            "intelligence-claude-account-adapter", // matched by importer below
            "intelligence-claude-account-importer-supervisor",
