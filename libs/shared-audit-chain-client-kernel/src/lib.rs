...
//!
//! `shared-audit-chain-client-kernel` follows BNF v4.1:
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AuditChainClientError::SkeletonNotYetImplemented(method) => write!(
                f,
                "shared-audit-chain-client-kernel: {method} is skeleton-only \
            ...
            AuditChainClientError::EmptyField(name) => {
                write!(
                    f,
                    "shared-audit-chain-client-kernel: required field {name:?} is empty"
                )
            }
        }
    }
