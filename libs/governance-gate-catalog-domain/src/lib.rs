...
//! - Crate id `governance-gate-catalog-domain` — `oya-` brand prefix
...
//!   `governance-predictable-naming-kernel::ALLOWED_ROLES`, post-ADR-0565).
//! - Library identifier `governance_gate_catalog_domain` —
...
pub const AGGREGATED_VALIDATE_LANES: &[&str] = &[
    ...
    // (impl: dev-cli/src/changeset_state_gates.rs).
...
/// and the `pr-review` context. Keep this list aligned with the live dev
...
pub const AGGREGATED_NON_GATE_COMMANDS: &[&str] = &[
    ...
    "cargo run -q -p governance-purpose-audit-app",
...
/// (`check-documentation-system`, `check-quality-lane`) previously did
