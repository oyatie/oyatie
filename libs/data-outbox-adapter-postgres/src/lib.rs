//! Postgres CDC change-stream adapter (via sqlx) for the owned `data`
//! outbox port.
//!
...
//! `data_outbox.outbox_events` — behind the same trait, so consumers
//! never observe the engine swap.
//!
//! This adapter absorbs ALL engine impedance: the tenant RLS scope runs the
//! parameterized `set_config` (reusing `shared-postgres-command-kernel::
...
//! policy set (run AS the `data_outbox_runtime` role, NOT PUBLIC, asserted
...
pub const POLL_CHANGES_SQL: &str = "SELECT tenant_id, event_id, event_kind, aggregate_id, payload, commit_wall_nanos, commit_logical \
     FROM data_outbox.outbox_events \
     WHERE tenant_id = $1 \
...
pub const LIVE_OUTBOX_POSTGRES_ENABLE_ENV: &str = "OYATIE_OUTBOX_LIVE_POSTGRES";
/// Admin (schema-owning) connection URL for the live harness.
pub const LIVE_OUTBOX_POSTGRES_ADMIN_URL_ENV: &str = "OYATIE_OUTBOX_POSTGRES_ADMIN_URL";
/// Application-role (RLS-subject) connection URL for the live harness.
pub const LIVE_OUTBOX_POSTGRES_APP_URL_ENV: &str = "OYATIE_OUTBOX_POSTGRES_APP_URL";
...
pub const RUNTIME_ROLE: &str = "data_outbox_runtime";
...
const LIVE_FIXTURE_DROP_SQL: &str = "DROP SCHEMA IF EXISTS data_outbox CASCADE";
const LIVE_FIXTURE_INSERT_SQL: &str = "INSERT INTO data_outbox.outbox_events (tenant_id, event_id, event_kind, aggregate_id, schema_version, idempotency_key, payload) VALUES ($1, $2, 'tenant.provisioned', 'aggregates/x', '1', $3, '\\x00') ON CONFLICT (tenant_id, idempotency_key) DO NOTHING";
...
pub async fn run_live_cdc_cross_tenant_probe() -> Result<Option<LiveCdcProbeReport>, DataSqlError> {
    ...
    // migrations create `data_outbox_runtime` (NOBYPASSRLS), the table, the
    ...
    // so the app pool can assume `data_outbox_runtime` on every connection
...
mod tests {
    ...
    fn poll_sql_filters_by_tenant() {
        ...
        assert!(POLL_CHANGES_SQL.contains("FROM data_outbox.outbox_events"));
    }
    ...
    fn migration_statement_split_keeps_the_dollar_block_intact() {
        ...
        // through `shared_postgres_command_kernel::split_migration_statements`
        ...
        let do_block = statements
            .iter()
            .find(|s| s.contains("CREATE ROLE data_outbox_runtime"))
        ...
        assert!(
            statements
                .iter()
                .any(|s| s.contains("GRANT USAGE ON SCHEMA data_outbox"))
        );
    }
    ...
    fn migration_targets_runtime_role_not_public() {
        ...
        assert!(table_migration.contains("TO data_outbox_runtime"));
        assert!(!table_migration.contains("TO PUBLIC"));
        assert!(table_migration.contains(
            "GRANT SELECT, INSERT ON data_outbox.outbox_events TO data_outbox_runtime"
        ));
    }
    ...
    /// OYATIE_OUTBOX_LIVE_POSTGRES + admin/app URLs. Proves RLS cross-tenant
    /// denial against the PRODUCTION policy set (run AS data_outbox_runtime,
