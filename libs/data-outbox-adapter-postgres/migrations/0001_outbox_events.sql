-- data outbox events: Postgres/RLS durable schema for the CDC
...
-- (data_outbox.outbox_events) and the table the transitional
...
CREATE SCHEMA IF NOT EXISTS data_outbox;

CREATE TABLE IF NOT EXISTS data_outbox.outbox_events (
...
ALTER TABLE data_outbox.outbox_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE data_outbox.outbox_events FORCE ROW LEVEL SECURITY;
CREATE POLICY outbox_events_tenant_isolation ON data_outbox.outbox_events AS PERMISSIVE FOR ALL TO data_outbox_runtime USING (tenant_id = current_setting('oyatie.tenant_id', true)) WITH CHECK (tenant_id = current_setting('oyatie.tenant_id', true));
CREATE POLICY outbox_events_require_tenant_guc ON data_outbox.outbox_events AS RESTRICTIVE FOR ALL TO data_outbox_runtime USING (current_setting('oyatie.tenant_id', true) IS NOT NULL AND current_setting('oyatie.tenant_id', true) <> '') WITH CHECK (current_setting('oyatie.tenant_id', true) IS NOT NULL AND current_setting('oyatie.tenant_id', true) <> '');
...
CREATE INDEX IF NOT EXISTS outbox_events_commit_order
    ON data_outbox.outbox_events (tenant_id, commit_logical);

COMMENT ON TABLE data_outbox.outbox_events IS 'data transactional-outbox / CDC change-stream events; tenant-scoped under oyatie.tenant_id, ordered by the monotone commit_logical stream position (commit_wall_nanos is informational only).';
