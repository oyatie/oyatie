-- data outbox runtime role contract (applied BEFORE 0001_outbox_events.sql).
--
-- The RLS policies in 0001 scope `TO data_outbox_runtime` (NOT PUBLIC), so
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'data_outbox_runtime') THEN
        CREATE ROLE data_outbox_runtime NOLOGIN NOBYPASSRLS;
    END IF;
END
$$;
CREATE SCHEMA IF NOT EXISTS data_outbox;
GRANT USAGE ON SCHEMA data_outbox TO data_outbox_runtime;
