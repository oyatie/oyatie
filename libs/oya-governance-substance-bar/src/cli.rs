...
use oya_governance_substance_bar::{enforce_substance_bar, format_text_report};
...
fn run(args: impl IntoIterator<Item = String>) -> Result<ExitCode> {
    ...
    let outcome = enforce_substance_bar(&args.root)?;
...
mod tests {
    ...
    fn violations_fail_closed_without_strict() {
        let root = temp_root("substance-cli-fail-closed");
        fs::create_dir_all(root.join("docs")).expect("create docs dir");
        fs::write(
            root.join("docs/short.md"),
            "---\ndoc_class: Standard\n---\n# Short\n",
        )
        .expect("write undersized doc");
