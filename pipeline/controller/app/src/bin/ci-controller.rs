//! ci-controller binary entry point.
//!
//! Reads configuration from environment variables:
//!
//! | Variable                               | Default                                                                  | Description                                    |
//! |----------------------------------------|--------------------------------------------------------------------------|------------------------------------------------|
//! | `OYATIE_CI_CONTROLLER_LISTEN_ADDR`        | `0.0.0.0:8081`                                                           | Bind address for healthz/metrics/gate-run      |
//! | `OYATIE_CI_CONTROLLER_NAMESPACE`          | `ci`                                                                 | Namespace to watch Jobs in + spawn Jobs into   |
//! | `OYATIE_CI_CONTROLLER_REPO_OWNER`         | `jason931225`                                                            | GitHub repo owner (forge of record)            |
//! | `OYATIE_CI_CONTROLLER_REPO_NAME`          | `oyatie`                                                                 | GitHub repo name (forge of record)             |
//! | `GITHUB_CI_TOKEN`                      | (required)                                                               | GitHub token for status posting (controller-only) |
//! | `OYATIE_CI_GATE_RUN_BEARER`               | (required)                                                               | Fail-closed bearer for POST /gate-run (keystone-1) |
//! | `OYATIE_CI_CONTROLLER_GRACE_CYCLES`       | `12`                                                                     | Waiting-pod-reason grace threshold             |
//! | `OYATIE_CI_GATE_IMAGE`                    | `registry.registry.svc.cluster.local:5000/rust-ci:dev`               | Rust-CI image for gate runner Pods             |
//! | `OYATIE_CI_GATE_FORGE_CLONE_URL`          | `https://github.com/jason931225/oyatie.git`                              | Git clone URL for gate Job init container      |
//! | `OYATIE_CI_GATE_ACTIVE_DEADLINE_SECS`     | `3600`                                                                   | Gate Job active deadline (seconds)             |
//! | `OYATIE_CI_GATE_TTL_AFTER_FINISHED_SECS`  | `86400`                                                                  | Gate Job TTL after finished for GC (seconds)   |
//! | `OYATIE_CI_GATE_RUNNER_SA`                | `ci-gate-runner`                                                     | Low-privilege SA for gate runner Pods          |
//! | `OYATIE_CI_STATUS_API_BASE_URL`             | unset                                                                    | Optional base URL for `/gate-runs/<run_id>` debug links |

use ci_controller_app::{
...
use ci_controller_kernel::CommitStatusPoster;
use std::sync::Arc;
use tracing::info;
...
async fn main() {
    ...
    let listen_addr = env_or("OYATIE_CI_CONTROLLER_LISTEN_ADDR", "0.0.0.0:8081");
    let namespace = env_or("OYATIE_CI_CONTROLLER_NAMESPACE", "ci");
    ...
    let repo_owner = env_or("OYATIE_CI_CONTROLLER_REPO_OWNER", "jason931225");
    let repo_name = env_or("OYATIE_CI_CONTROLLER_REPO_NAME", "oyatie");
    let grace_cycles: u32 = env_or("OYATIE_CI_CONTROLLER_GRACE_CYCLES", "12")
        .parse()
        .unwrap_or(12);

    // Gate Job spawn configuration (for POST /gate-run — plank role).
    let gate_image = env_or(
        "OYATIE_CI_GATE_IMAGE",
        "registry.registry.svc.cluster.local:5000/rust-ci:dev",
    );
    let gate_forge_clone_url = env_or(
        "OYATIE_CI_GATE_FORGE_CLONE_URL",
        "https://github.com/jason931225/oyatie.git",
    );
    let gate_active_deadline_secs: i64 = env_or("OYATIE_CI_GATE_ACTIVE_DEADLINE_SECS", "3600")
        .parse()
        .unwrap_or(3600);
    let gate_ttl_after_finished_secs: i32 = env_or("OYATIE_CI_GATE_TTL_AFTER_FINISHED_SECS", "86400")
        .parse()
        .unwrap_or(86400);
    let gate_runner_sa = env_or("OYATIE_CI_GATE_RUNNER_SA", "ci-gate-runner");
    let status_api_base_url = env_optional("OYATIE_CI_STATUS_API_BASE_URL");
    ...
    info!(
        forge = "github",
        "using GitHub commit-status producer (presubmit)"
    );
    ...
    let gate_run_bearer = env_required("OYATIE_CI_GATE_RUN_BEARER");
    ...
    info!(listen_addr = %listen_addr, namespace = %namespace, "ci-controller starting");
