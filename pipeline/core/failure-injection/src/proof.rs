...
pub enum Fault {
    ...
    ConsumerMissing,
    RevisionMismatch,
    /// The deleted path's own digest is withheld from the view.
    UnattestedDeletion,
    ViewUnavailable,
}
...
pub const FAULTS: [Fault; 5] = [
    Fault::ConsumerFailed, Fault::ConsumerMissing, Fault::RevisionMismatch,
    Fault::UnattestedDeletion, Fault::ViewUnavailable,
];
...
impl Fault {
    pub fn name(self) -> &'static str {
        match self {
            ...
            Self::UnattestedDeletion => "unattested deletion",
...
///
/// Only [`Fault::UnattestedDeletion`] varies with `deletion`; the other faults
/// perturb candidate-wide state and are still proved once per deletion.
pub fn inject(candidate: &Candidate, deletion: &str, fault: Fault) -> Candidate {
    ...
    match fault {
        ...
        Fault::UnattestedDeletion => injected.view = withhold(candidate.view.as_ref(), deletion),
...
pub enum ProofGap {
    ...
    /// Refused, but for a class other than the one injected.
    WrongClass {
        deletion: String,
        fault: Fault,
        got: Vec<Refusal>,
