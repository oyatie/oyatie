...
fn admit(base: &str, head: &str) -> ExitCode {
    ...
    // The repository is named rather than inherited from the working
    // directory: a gatherer that reads "wherever the process happens to be"
    // cannot be tested against a repository it did not start in.
    let gathered = verify::facts(std::path::Path::new("."), base, head, &signers);
    ...
    if violations.is_empty() {
        // Say what was admitted. A silent pass cannot be told from a range
        // that collapsed to nothing, and "no output" is what a gate looks
        // like when it has stopped judging anything at all.
        println!(
            "commit signing admission: {} commit(s) in {base}..{head} signed by a trusted key",
            facts.len()
        );
