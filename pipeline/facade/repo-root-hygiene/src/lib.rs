...
pub const VIOLATION_CODES: [&str; 7] = [
    ...
    // Frozen integer ceilings were retired: git is the audit log. Re-introducing
    // `corpus_budget` (counts, reviewed_raises, or class censuses) is born-blocking.
...
pub fn evaluate_keyed(policy: &Value, observed: &Value) -> BTreeSet<Finding> {
    ...
    findings.extend(evaluate_corpus_budget(policy));
...
/// Frozen corpus counts were retired. Git is the audit log; a hand-maintained
/// integer ceiling is not a hyperscaler control. Presence of the block is
/// born-blocking so it cannot sneak back as DATA.
pub fn evaluate_corpus_budget(policy: &Value) -> BTreeSet<Finding> {
    let mut findings = BTreeSet::new();
    if policy.get("corpus_budget").is_some() {
        findings.insert(Finding::new(
            "corpus_budget_malformed",
            "<corpus_budget>",
            "corpus_budget is retired; delete the block. Git history is the audit log; do not freeze path counts in JSON.",
        ));
    }
    findings
}
...
"#;
    ...
    #[test]
    fn corpus_budget_absent_is_green() {
        let findings = evaluate_corpus_budget(&json!({}));
        assert!(
            findings.is_empty(),
            "retired corpus_budget must not be required; got {findings:#?}"
        );
    }

    #[test]
    fn corpus_budget_block_is_born_blocking() {
        let findings = evaluate_corpus_budget(&json!({ "corpus_budget": { "counts": {} } }));
        assert!(
            findings.iter().any(|f| f.code == "corpus_budget_malformed"),
            "reintroducing corpus_budget must fail; got {findings:#?}"
        );
    }
