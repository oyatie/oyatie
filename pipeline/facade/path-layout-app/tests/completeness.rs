...
#[cfg(unix)]
#[test]
fn every_live_changed_crate_path_must_be_a_regular_git_blob() {
    use std::os::unix::fs::symlink;
    ...
    .expect("create cross-owner source symlink");
    let head = commit(&root, "symlinked module");

    let rejected = admit(&root, &base, &head);
    assert!(!rejected.status.success());
    let error = String::from_utf8_lossy(&rejected.stderr);
    assert!(error.contains("network/core/route/src/blob.rs"), "{error}");
    assert!(error.contains("regular Git blob"), "{error}");
    let _ = std::fs::remove_dir_all(root);
}
...
#[test]
fn repository_cargo_runner_substitution_is_forbidden() {
    let root = fixture();
    let base = commit(&root, "base");
    write(
        &root,
    ...
    let head = commit(&root, "cargo runner substitution");

    let rejected = admit(&root, &base, &head);
    assert!(!rejected.status.success());
    let error = String::from_utf8_lossy(&rejected.stderr);
    assert!(
        error.contains("target runner configuration is forbidden"),
        "{error}"
    );
    let _ = std::fs::remove_dir_all(root);
}
