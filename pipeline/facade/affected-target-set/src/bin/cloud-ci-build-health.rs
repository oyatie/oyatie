...
/// the repository whose `presubmit` runs produced it, and a settable API host would be a way
/// to point provenance validation at a server the candidate controls.
const GITHUB_API: &str = "https://api.github.com";
/// GitHub rejects API requests without a User-Agent.
const HTTP_USER_AGENT: &str = "cloud-ci-build-health";
...
///
/// The merge-base of a PR IS a `dev` commit, and its push-to-dev `presubmit` run already
...
///
/// WHAT IT IS WORTH, measured rather than asserted. Across the last 60 `presubmit` PR runs on
...
/// `cloud-ci-run-terminal-state` consumer. `curl` was rejected despite being present: it would
...
fn reuse_trusted_baselines_inner(
    ...
    let runs = api.get_text(&format!(
        "repos/{repo}/actions/workflows/presubmit.yml/runs?event=push&branch=dev&status=completed&head_sha={merge_base}&per_page=20"
    ))?;
...
fn main() -> ExitCode {
    ...
    let args = match parse_args(std::env::args()) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "{LOG}: usage: cloud-ci-build-health --baseline-report <merge-base.json> \
...
mod trusted_baseline_tests {
    ...
    fn run_entry(id: u64, sha: &str, event: &str, branch: &str, conclusion: &str) -> String {
        format!(
            r#"{{"id":{id},"run_attempt":{RUN_ATTEMPT},"head_sha":"{sha}","event":"{event}","head_branch":"{branch}","status":"completed","conclusion":"{conclusion}","path":".github/workflows/presubmit.yml"}}"#
        )
    }
    ...
    fn wrong_sha_event_branch_status_or_foreign_workflow_is_unavailable() {
        let payload = runs(&[
            ...
            format!(
                r#"{{"id":14,"run_attempt":1,"head_sha":"{SHA}","event":"push","head_branch":"dev","status":"in_progress","conclusion":null,"path":".github/workflows/presubmit.yml"}}"#
            ),
            format!(
    ...
    fn duplicate_or_malformed_runs_are_refused() {
        ...
        let malformed = format!(
            r#"{{"workflow_runs":[{{"id":14,"head_sha":"{SHA}","event":"push","head_branch":"dev","status":"completed","conclusion":"success","path":".github/workflows/presubmit.yml"}}]}}"#
        );
        assert!(is_refused(
    ...
    fn downloaded_archive_digest_is_stable() {
        let path = std::env::temp_dir().join("build-health-digest-test");
    ...
    fn an_unspawnable_program_is_a_capability_fault_not_a_refusal() {
        let dest = std::env::temp_dir().join("build-health-capability-fault-probe");
        let err = capture_to_file(
            "no-such-program-on-any-runner-image",
            &["--version"],
            &dest,
        )
        .unwrap_err();
        ...
        let control = std::env::temp_dir().join("build-health-capability-fault-control");
    ...
    fn every_outcome_is_reported_where_a_machine_can_read_it() {
        let dir = std::env::temp_dir().join("build-health-reuse-outcome-report");
