...
//!
//! Wired into the binding `presubmit` affected-set admission path so the required
...
const HTTP_USER_AGENT: &str = "cloud-ci-hub-exclusivity";
...
const LIVE_OPEN_PRS_ENV: &str = "OYATIE_CI_HUB_EXCLUSIVITY_LIVE_OPEN_PRS";
...
fn main() -> ExitCode {
    let args = match parse_args(std::env::args()) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "{LOG}: usage: cloud-ci-hub-exclusivity [--repo-root <path>] [--policy <pack.json>] \
