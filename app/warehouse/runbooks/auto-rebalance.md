---
doc_class: Runbook
doc_id: RUNBOOK-WAREHOUSE-AUTO_REBALANCE
...
# Runbook: Warehouse Auto Rebalance

## Operator Contract
- Runbook id: warehouse-auto-rebalance.
- Primary service namespace: `warehouse`.
...
- Owning team: axis-warehouse + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-warehouse-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=warehouse; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n warehouse rollout status deploy/warehouse --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service warehouse --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service warehouse --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service warehouse --cell $CELL --window 30m`.
