...
- **Source (read-only):** `oya/warehouse/**` on `origin/dev` until shrink-only delete on `integ/oya`.
...
2. Shrink-only burn of `oya/warehouse/**` on `integ/oya` (NOT this rail) after verify.
3. Hub retargets (`specs/**`, capability-registry) on tip-free `integ/specs`.

## Out of envelope

- `oya/warehouse/**` deletes — `integ/oya` shrink-only only.
