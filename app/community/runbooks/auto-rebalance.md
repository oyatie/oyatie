---
doc_class: Runbook
doc_id: RUNBOOK-COMMUNITY-AUTO_REBALANCE
...
# Runbook: Community Auto Rebalance

## Operator Contract
- Runbook id: community-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-community-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=community; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n community rollout status deploy/community --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service community --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service community --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service community --cell $CELL --window 30m`.
