...
## Trigger Conditions
- Page on alert `VerifiedAnonymousDeanonymizationIncidentCritical` when `community_verified_anonymous_deanonymization_incident_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `VerifiedAnonymousDeanonymizationIncidentSloBurn` when `community_verified_anonymous_deanonymization_incident_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open sev2 if `community_verified_anonymous_deanonymization_total` exceeds the threshold documented in `app/community/slos/audit-chain-seal-latency.openslo.yaml`.
- Open sev2 if `community_verified_anonymous_deanonymization_incident_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `cargo run -p dev-cli -- gate validate community-verified-anonymous-deanonymization-incident --production-snapshot` exits non-zero against the latest production evidence bundle.
- Primary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/verified-anonymous-deanonymization-incident?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=101` backed by `app/community/dashboards/moderation-queue-depth.json`.
- Secondary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/verified-anonymous-deanonymization-incident?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=202` backed by `app/community/dashboards/post-throughput.json`.
...
- Service-specific metric `community_verified_anonymous_deanonymization_total` is red while `community_verified_anonymous_deanonymization_incident_audit_emit_total{status="sealed"}` is flat.
...
- Metric pattern: `community_verified_anonymous_deanonymization_incident_error_ratio` rises before `community_verified_anonymous_deanonymization_incident_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `community_verified_anonymous_deanonymization_incident_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `community_verified_anonymous_deanonymization_incident_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific pattern: `community_verified_anonymous_deanonymization_total` rises while `community_verified_anonymous_deanonymization_incident_dependency_error_ratio` is flat; inspect local state before escalating Cloudflare Trust and Safety.
- Service-specific pattern: `community_verified_anonymous_deanonymization_incident_dependency_error_ratio` rises while `community_verified_anonymous_deanonymization_total` is flat; inspect vendor or adjacent-service dependency health before local rollback.
...
- Failure mode 7: regional partition; keep prod-us-east-1 as evidence leader and reject cross-region mutation until `community_verified_anonymous_deanonymization_incident_state_hash_match == 1`.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=community_verified_anonymous_deanonymization_incident_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=community_verified_anonymous_deanonymization_incident_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=community_verified_anonymous_deanonymization_incident_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=community_verified_anonymous_deanonymization_total{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p dev-cli -- gate validate community-verified-anonymous-deanonymization-incident --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p community-moderation-queue-domain verified_anonymous_deanonymization_incident -- --nocapture`.
17. Check API contract smoke: `curl -s https://community.internal.oyatie.dev/v1/community/verified-anonymous-deanonymization-incident/incident-handoff -H "x-tenant: $TENANT"`.
...
25. Run targeted SQL state query: `psql $OYATIE_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from community_verified_anonymous_deanonymization_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric community_verified_anonymous_deanonymization_incident_error_ratio --window 30m --threshold 0.02`.
...
2. Does community_verified_anonymous_deanonymization_incident_queue_depth grow while community_verified_anonymous_deanonymization_incident_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths app/community/runbooks/verified-anonymous-deanonymization-incident.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://community.internal.oyatie.dev/v1/community/verified-anonymous-deanonymization-incident/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: confirmed CommunityThread correctness risk.
  - Required action: keep `community-verified-anonymous-deanonymization-incident-circuit-breaker` open until `community_verified_anonymous_deanonymization_incident_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: dependency saturation or replay backlog.
  - Required action: keep `community-verified-anonymous-deanonymization-incident-circuit-breaker` open until `community_verified_anonymous_deanonymization_incident_error_ratio` is below 0.01 for 3 evaluator windows.
...
## Resolution Steps
1. Identify code owner path: `rg "verified_anonymous_deanonymization_incident|VerifiedAnonymousDeanonymizationIncidentCritical|community.verified_anonymous_deanonymization_incident.incident_state" crates app/community -g "!app/community/runbooks/**"`.
2. Patch domain invariant: `edit community-moderation-queue-domain where verified_anonymous_deanonymization_incident state transition is validated`.
...
6. Add regression test: `cargo test -p community-moderation-queue-domain verified_anonymous_deanonymization_incident_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate community-verified-anonymous-deanonymization-incident --fixture incident-verified-anonymous-deanonymization-incident.json`.
8. Add SLO assertion: `update app/community/slos/audit-chain-seal-latency.openslo.yaml with alert VerifiedAnonymousDeanonymizationIncidentCritical when this was a missing alert`.
9. Add dashboard panel: `update app/community/dashboards/moderation-queue-depth.json with community_verified_anonymous_deanonymization_incident_error_ratio, community_verified_anonymous_deanonymization_incident_lag_seconds, and community_verified_anonymous_deanonymization_total`.
...
14. Watch burn rate: `oya ops watch --metric community_verified_anonymous_deanonymization_incident_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `community-moderation-queue-domain`: inspect for `verified_anonymous_deanonymization_incident` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `community-post-store-worker`: inspect for `verified_anonymous_deanonymization_incident` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `community-voting-engine-domain`: inspect for `verified_anonymous_deanonymization_incident` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `community-search-index-worker`: inspect for `verified_anonymous_deanonymization_incident` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `app/community/dashboards/moderation-queue-depth.json`: verify panel coverage for `community_verified_anonymous_deanonymization_incident_error_ratio`, `community_verified_anonymous_deanonymization_incident_lag_seconds`, and `community_verified_anonymous_deanonymization_total`.
...
- `community_verified_anonymous_deanonymization_incident_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `community_verified_anonymous_deanonymization_incident_lag_seconds < 120` for all production cells.
- `community_verified_anonymous_deanonymization_incident_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `community_verified_anonymous_deanonymization_total` is below the threshold documented in `app/community/slos/audit-chain-seal-latency.openslo.yaml`.
...
- Capacity math: if `community_verified_anonymous_deanonymization_incident_queue_depth` is 5000 and the worker drains 25 items/second, the best-case drain is 200 seconds before retries; page earlier when drain time exceeds 300 seconds.
...
- Rollback checkpoint 2: before opening `community-verified-anonymous-deanonymization-incident-circuit-breaker`, capture `community_verified_anonymous_deanonymization_incident_request_rate` and `community_verified_anonymous_deanonymization_incident_success_ratio` from Mimir.
...
## Handoff Notes
- Do not hand off with only the alert name; include `community_verified_anonymous_deanonymization_incident_error_ratio`, `community_verified_anonymous_deanonymization_incident_lag_seconds`, `community_verified_anonymous_deanonymization_incident_queue_depth`, `community_verified_anonymous_deanonymization_total`, current breaker state, and audit seal status.
