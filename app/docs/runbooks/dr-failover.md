# docs DR failover runbook

## Purpose

Recover the docs service inside the declared ADR-0343 DR envelope without changing tenant version pins or weakening Cedar admission.

## Recovery order

1. Freeze new mutating ingress for docs at api-gateway while read-only routes stay available when data freshness permits.
...
3. freeze export workers, replay CRDT operation journals, and promote the document-store replica before reopening collaboration sessions.
