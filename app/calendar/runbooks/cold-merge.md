---
doc_class: Runbook
doc_id: RUNBOOK-CALENDAR-COLD_MERGE
...
# Runbook: Calendar Cold Merge

## Operator Contract
- Runbook id: calendar-cold-merge.
...
- ADR-0347 enforced_by lanes: `governance-lane-prefix-vocabulary`; `governance-lane-prefix-vocabulary`; `governance-rename-inventory-presence`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-calendar-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=calendar; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n calendar rollout status deploy/calendar --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service calendar --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query sharding_cold_merge_candidate_count --service calendar --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_cold_merge_quiet_hours --service calendar --cell $CELL --window 30m`.
