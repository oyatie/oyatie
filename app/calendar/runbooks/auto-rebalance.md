---
doc_class: Runbook
doc_id: RUNBOOK-CALENDAR-AUTO_REBALANCE
...
# Runbook: Calendar Auto Rebalance

## Operator Contract
- Runbook id: calendar-auto-rebalance.
...
- ADR-0347 enforced_by lanes: `governance-lane-prefix-vocabulary`; `governance-rename-inventory-presence`.
...
- ADR-0349 purpose wording: GitHub Actions (presubmit) and ArgoCD are the two canonical self-hostable CI/CD substrates for the Oyatie corpus.
- ADR-0349 enforced_by lanes: `governance-github-actions-parity`; `governance-argocd-application-cosign-verified`; `governance-argocd-tenant-namespace-isolation`; `governance-owned-ci-declarative-only`; `governance-deploy-audit-chain-emit`.
...
- Trigger 7: GitHub Actions or ci parity drift blocks the release train for the sharding automation lane.
- Trigger 8: ArgoCD reports a pending sync tied to this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-calendar-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=calendar; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n calendar rollout status deploy/calendar --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service calendar --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service calendar --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service calendar --cell $CELL --window 30m`.
...
10. Verify GitHub Actions/ci parity evidence exists for this service before declaring the runbook complete.
...
6. If GitHub Actions/ci parity is unknown, keep the change in report-only state and run local `oya verify --ci-required` before push.
...
- Evidence 7: ci run id or GitHub Actions run id proving CI/CD parity for this service.
...
6. Confirm GitHub Actions and ci parity evidence is attached per ADR-0349.
