...
- Event surface: automation events for this microservice include local verifier outcomes, governance lane rename evidence, sharding automation decisions, and ci/ArgoCD CI/CD transitions.
...
- Cedar decision id, audit-chain event id, verifier run id, ci run id, ArgoCD sync id, cosign verification result, and governance lane id.
...
- ADR-0347: every `governance-*` CI lane prefix in the Oyatie corpus RENAMES to `governance-*` in a single bulk-rename pull request (Wave 15-ZB). Enforced by `governance-lane-prefix-vocabulary` and `governance-rename-inventory-presence`.
...
- ADR-0349 / ADR-0515: GitHub Actions under `presubmit` is the live CI authority; ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `governance-github-actions-parity`, `governance-argocd-application-cosign-verified`, `governance-argocd-tenant-namespace-isolation`, `governance-owned-ci-declarative-only`, and `governance-deploy-audit-chain-emit`.
...
- Access control: service principals, ci principals, ArgoCD principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
### Residual Risk
- Residual risk is medium until Wave 15-ZA/ZB/ZD/ZE implementation PRs land the verifier mirror, governance rename, sharding automation bodies, and ci/ArgoCD substrate files.
