...
# Data Protection Impact Assessment: calendar
...
- Event surface: automation events for this microservice include local verifier outcomes, governance lane rename evidence, sharding automation decisions, and oya-ci/ArgoCD CI/CD transitions.
...
- Cedar decision id, audit-chain event id, verifier run id, oya-ci run id, ArgoCD sync id, cosign verification result, and governance lane id.
...
- ADR-0347: every `oya-governance-*` CI lane prefix in the Oyatie corpus RENAMES to `oya-governance-*` in a single bulk-rename pull request (Wave 15-ZB). Enforced by `oya-governance-lane-prefix-vocabulary` and `oya-governance-rename-inventory-presence`.
...
- ADR-0349 / ADR-0515: GitHub Actions under `oya-ci-required` is the live CI authority; ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `oya-governance-github-actions-parity`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-governance-owned-ci-declarative-only`, and `oya-governance-deploy-audit-chain-emit`.
...
- Access control: service principals, oya-ci principals, ArgoCD principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
### Residual Risk
- Residual risk is medium until Wave 15-ZA/ZB/ZD/ZE implementation PRs land the verifier mirror, governance rename, sharding automation bodies, and oya-ci/ArgoCD substrate files.
