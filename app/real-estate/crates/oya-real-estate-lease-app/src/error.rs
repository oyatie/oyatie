...
pub enum ServiceErrorKind { Configuration, InvariantViolation, Authorization, Validation, Conflict, NotFound, AdapterUnavailable, ContractStub }
...
impl ServiceError {
    ...
    pub fn contract_stub(surface: &'static str) -> Self {
        Self::with_field(ServiceErrorKind::ContractStub, surface, "handler is intentionally scaffolded until implementation packet lands")
    }
}
...
impl From<std::io::Error> for ServiceError {
    fn from(error: std::io::Error) -> Self { Self::configuration(error.to_string()) }
}

impl From<toml::de::Error> for ServiceError {
    fn from(error: toml::de::Error) -> Self { Self::configuration(error.to_string()) }
}
