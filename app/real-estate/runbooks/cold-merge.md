---
doc_class: Runbook
doc_id: RUNBOOK-REAL_ESTATE-COLD_MERGE
...
# Runbook: Real Estate Cold Merge

## Operator Contract
- Runbook id: real-estate-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-real-estate-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=real-estate; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n real-estate rollout status deploy/real-estate --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service real-estate --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service real-estate --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service real-estate --cell $CELL --window 30m`.
