...
impl ChangeFeed for SqliteStore {
    ...
    fn changes(&self, consumer: Consumer, account: &str, limit: usize) -> Result<FeedRead, Error> {
        ...
        let tx = db
            ...
            .map_err(storage)?;
        let (tenant, revision, floor) = tail(&tx, account)?;
        // Absent cursor ⇒ start at the tail.
        let since = cursor(&tx, consumer, account)?.unwrap_or(revision);
        if since < floor {
            // Poisoned here, in the store, with the cursor read: no scheduler
            // can forget to, and the key stops occupying a tenant slot.
            tx.execute(
                "INSERT INTO feed_dirty(consumer,tenant,account,poison) VALUES(?1,?2,?3,'cursor-below-floor') ON CONFLICT(consumer,account) DO UPDATE SET poison=excluded.poison WHERE poison IS NULL",
                params![consumer.name(), tenant, account],
            )
            .map_err(storage)?;
        ...
        let page = history::page(&tx, account, since, limit)?;
        Ok(FeedRead::Changes {
            cursor: Cursor(since),
            page,
        })
