...
mod upstream {
    include!(env!("STALWART_IMAP_CONDSTORE_SOURCE"));
}
#[tokio::test]
async fn upstream_imap_condstore() {
    for (source, expected) in [
        (
            include_bytes!(env!("STALWART_IMAP_CONDSTORE_SOURCE")).as_slice(),
            "084ec1542edcb312b44cbfe6fd17742081cc6c077c36558cfdd3bc0aa86ad241",
        ),
        (
    ...
    upstream::test(&mut primary, &mut check).await;
    primary.close().await;
    check.close().await;
    let outcomes = outcomes.borrow();
    let passed = outcomes.iter().filter(|v| v["passed"] == true).count();
    println!("{}",serde_json::to_string_pretty(&serde_json::json!({"upstream_revision":"474dd0229cb20cf513036619781ed97bd8073c3f","suite":"tests/src/imap/condstore.rs","transport":"duplex IMAP wire","wire_predicates":outcomes.len(),"passed":passed,"skipped":0,"outcomes":*outcomes})).unwrap());
    assert_eq!(
        outcomes.len(),
        81,
        "all upstream and session wire predicates must execute"
    );
    assert_eq!(
        passed,
        outcomes.len(),
        "unchanged upstream CONDSTORE predicates failed"
    );
}
