...
async fn spf_strict_refuses_only_the_host_the_domain_disowns() {
    ...
    for (peer, domain, expected, refused) in [
        ...
        // `exists:` is the lookup a sealed table declines, so it reaches a
        // resolver pointed nowhere: genuinely unknown, and strict says so.
...
/// Remembering only that a domain had been decided, without what was decided,
/// would let a host the domain disowns in by asking twice.
#[tokio::test]
...
/// A deployment that turned no check on must not acquire a new reason to
/// close sessions.
#[tokio::test]
...
/// Counting reverse paths without noticing that each became a message would
/// cut off a sender the domain explicitly authorizes.
#[tokio::test]
