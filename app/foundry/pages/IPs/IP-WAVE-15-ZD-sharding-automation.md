...
---
# IP-WAVE-15-ZD-sharding-automation: Docs Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/docs/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: docs must interpret ADR-0348 through its own bounded context: block-types.
SCOPE-004: docs uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: docs uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: docs uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: docs.
STANCE-002: Owner team: axis-docs.
...
STANCE-005: Bounded context anchor: block-types.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for docs.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for docs because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; docs must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where docs participates.
...
ROLLBACK-002: For docs, rollback starts from the audit-chain row that recorded the automation event.
