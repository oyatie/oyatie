---
doc_class: Runbook
doc_id: RUNBOOK-DOCS-AUTO_REBALANCE
microservice: docs
...
# Runbook: Docs Auto Rebalance

## Operator Contract
- Runbook id: docs-auto-rebalance.
- Primary service namespace: `docs`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-docs-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=docs; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n docs rollout status deploy/docs --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service docs --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service docs --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service docs --cell $CELL --window 30m`.
