...
- `docs_ws_lease_count` > 80% of cell max.
- `docs_ws_lease_acquisition_p99_seconds > 1`.
...
2. Identify top tenants by WS lease count:
   ```promql
   topk(5, sum by (tenant_id) (docs_ws_lease_count))
   ```
...
```bash
kubectl scale deployment -n docs docs-collab-crdt-worker --replicas=200
kubectl scale deployment -n docs docs-document-store-rest --replicas=100
```

Verify within 10 min:
```promql
sum(docs_ws_lease_acquisition_p99_seconds)
```
...
```bash
kubectl set env deployment/docs-document-store-rest -n docs PG_POOL_SIZE_PER_POD=20
kubectl rollout restart deployment/docs-document-store-rest -n docs
```
...
|---|---|---|
| `docs_ws_lease_count` per-tenant | < 1k for starter, < 10k for pro/enterprise | within 15 min |
| `docs_ws_lease_acquisition_p99_seconds` | < 1s | within 15 min |
