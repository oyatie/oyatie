...
- `docs_export_pipeline_failure_rate > 10%` on rolling 5min window.
- `docs_export_pdf_p99_seconds > 5s` (target 3s).
- gVisor seccomp violation alert (Sev-1 escape attempt).
- Tenant reports "PDF export looks corrupted / missing content."
- OOXML import fidelity below threshold (`docs_ooxml_import_fidelity_ratio < 0.95`).
...
3. Identify affected backend:
   ```bash
   kubectl -n docs logs -l app=docs-export-import-worker --tail=200 | grep -E "(pandoc|weasyprint|chromium|gvisor)"
   ```
4. Check Pandoc / WeasyPrint / Chromium version in deployed image:
   ```bash
   kubectl -n docs get deployment docs-export-import-worker -o jsonpath='{.spec.template.spec.containers[0].image}'
   ```
...
| 2 | Update deployed image tag back to previous LTS pin: `helm upgrade docs <chart> --set exportImport.image.tag=<previous-tag>`. |
| 3 | Restart worker pods: `kubectl rollout restart deployment/docs-export-import-worker -n docs`. |
...
| Step | Action |
|---|---|
| 1 | Identify affected feature via `docs_ooxml_import_lost_feature_count` metric (per-feature label). |
...
| Step | Action |
|---|---|
| 1 | Check queue depth: `docs_export_queue_depth_seconds`. |
| 2 | Identify storm source: `topk(5, sum by (tenant_id) (rate(docs_export_job_submitted_total[5m])))`. |
| 3 | Apply per-tenant rate-limit. |
| 4 | Scale gVisor pool: `kubectl scale deployment/docs-export-import-worker -n docs --replicas=200`. |
...
|---|---|---|
| `docs_export_pipeline_failure_rate` | < 1% | within 15 min |
| `docs_export_pdf_p99_seconds` | < 3s | within 15 min |
| `docs_ooxml_import_fidelity_ratio` | ≥ 0.95 | within 15 min |
...
- Update LEAN check `governance-export-sandbox-conformance` if a new sandbox-escape vector discovered.
