...
3. **`docs_collab_conflict_surfaced_total` rate > 0.5/s for ≥ 5 min on a single (tenant, document_id) tuple** — abnormal conflict density; likely indicates a CRDT regression OR a coordinated DoS attempt OR a real organizational disagreement about document content.
4. **`docs_collab_silent_loss_attempt_total > 0`** — Sev-1 (load-bearing invariant breach; never expected to fire; mirrors workflow-studio AC-06).
...
1. Identify affected (tenant_id, document_id): query `kubectl -n docs logs -l app=docs-collab-crdt-worker --tail=500 | grep <tenant_id>` OR Grafana dashboard `dashboards/collab-health.json` filtered to that tenant.
2. Identify CRDT op stream window: read `docs_collab_op_stream_seq` for the (tenant, document) bracket.
...
| 2 | Reconstruct CRDT op stream from Postgres seal-deltas + Valkey ephemeral state (if still present): `cargo run -p docs-collab-crdt-domain --bin reconstruct -- --tenant <h> --document <d>`. | ≤ 10 min |
...
| 4 | If Sev-1: **stop all save-paths for the affected (tenant, document)**: `cargo run -p dev-cli -- vcs override-paths --microservice docs --halt-saves --tenant <h> --document <d>` (requires 2-person rule). | ≤ 10 min |
...
| Step | Action |
|---|---|
| 1 | Check `governance-crdt-cross-microservice-consistency` lane history. |
...
After recovery:
- `docs_collab_conflict_surfaced_total` rate returns to baseline (≤ 0.1/s per tenant).
- `docs_collab_silent_loss_attempt_total == 0` (held to zero is load-bearing).
