...
- `docs_embed_source_staleness_seconds > 600` (10 min) on a (embedding_doc, embed_ref) tuple.
- `docs_cross_pack_embed_timeout_rate > 5%`.
- Embed-resolver returns "stale snapshot" banner repeatedly for a tenant.
- `docs_embed_loop_detected_total > 0` (FM-17 recursive embed loop).
...
4. Check cross-pack mesh:
   ...
   ```
5. Check loop detection: `docs_embed_loop_detected_total`.
...
| Step | Action |
|---|---|
| 1 | Check queue depth: `docs_embed_resolver_queue_depth_seconds`. |
| 2 | Scale embed-resolver-worker pool: `kubectl scale deployment/docs-embed-resolver-worker -n docs --replicas=50`. |
...
|---|---|---|
| `docs_embed_source_staleness_seconds` p99 | < 60s | within 15 min |
| `docs_cross_pack_embed_timeout_rate` | < 1% | within 15 min |
| `docs_embed_loop_detected_total` rate | 0 | should be 0 |
| `docs_embed_resolver_queue_depth_seconds` | < 60s | within 30 min |
