...
- LEAN scan `check-public-collection-drift` detects a row with `visibility=public` lacking tenant + tenant-admin co-signed opt-in.
- `docs_per_block_acl_violation_total > 0` (AC-04 breach).
- Tenant reports "my private doc is showing up in search / external access logs."
- `docs_per_block_acl_check_p99 > 50ms` (latency-spike variant; Section C below).
...
| Step | Action |
|---|---|
| 1 | Halt affected code path: `cargo run -p dev-cli -- vcs override-paths --microservice docs --halt-path <bc>`. |
...
| Step | Action |
|---|---|
| 1 | Check cache hit ratio: `docs_per_block_acl_cache_hit_ratio`. |
...
| `check-public-collection-drift` lane green | yes |
