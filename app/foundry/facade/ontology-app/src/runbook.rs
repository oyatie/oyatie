...
pub const RUNBOOKS: &[RunbookEntry] = &[
    ...
    RunbookEntry {
        ...
        runbook: Runbook {
            ...
            first_check: "The refusal body's gate and HTTP status. A 503 with gate log is the action log refusing to be written. A 403 with gate authorization for every caller at once splits on its cause: one naming a tenant this process does not serve is the tenant roster and the operator list disagreeing, a configuration fault; one naming the policy decision point is either the runtime guard's circuit, open after five consecutive runtime faults such as a decision past its 250 ms deadline or a crash, or a policy deny for every caller, such as no operator configured with the foundry-operator role, which has the same body as the open circuit. A 403 the writer issued, at gate authorization, parameters or admission, refuses one submission and is the denial-trail objective's event; a 401, 400, 409 or a 403 for one caller is that request's credential, body, idempotency key or grant.",
            mitigation: "A refusal at one caller is that caller's request to fix. An unserved-tenant refusal for every caller is repaired by a configuration whose operator list names a served tenant; a restart on the same configuration reproduces it. The policy engine is compiled into this binary, so a universal refusal naming the policy decision point has no authorizer to reconnect; the circuit retries the engine after a 30 s cooldown, while a policy deny never clears and is the configured roles to fix. A restart reloads the compiled-in bundle and re-opens the log, and logs boot refused naming the policy seed or the action log if either is the fault.",
