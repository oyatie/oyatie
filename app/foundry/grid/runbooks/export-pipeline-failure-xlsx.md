...
date: 2026-05-17
related_artifacts:
  - app/sheets/failure-modes.md (FM-04)
  - app/sheets/threat-model.md §"T-S-04" + §"T-D-03" + §"T-E-05"
  - app/sheets/PRD.md AC-12
  - app/sheets/decisions/ADR-SHEETS-0007 (XLSX export fidelity)
...
1. **`sheets_xlsx_export_failure_rate > 0.05` for ≥ 10 min**.
2. **`sheets_xlsx_export_p99_seconds > 30` for ≥ 10 min** (budget breach; target ≤ 5s p95).
...
| Step | Action |
|---|---|
| 1 | Run XLSX best-effort round-trip corpus: `cargo run -p dev-cli -- sheets corpus run --kind xlsx-roundtrip`. |
...
After recovery:
- `sheets_xlsx_export_failure_rate < 0.01` for ≥ 1h.
- `sheets_xlsx_export_p95_seconds < 5` for 100k-cell workbook (per AC-12).
