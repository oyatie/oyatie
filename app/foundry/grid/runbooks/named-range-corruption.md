...
date: 2026-05-17
related_artifacts:
  - app/sheets/failure-modes.md (FM-06)
  - app/sheets/PRD.md FR-18 + AC-22
...
1. **`sheets_named_range_orphan_total > 0`** — named-range refs in formulas with no matching definition.
2. **`sheets_formula_error_total{error_kind="#NAME?"} rate > threshold`** for ≥ 5 min.
...
1. Identify affected (tenant, workbook, named_range): query `sheets_named_range_orphan_top_n`.
...
| Step | Action |
|---|---|
| 1 | Restore named-range from version-history: `cargo run -p dev-cli -- sheets named-range restore --tenant <h> --workbook <w> --name <name> --from-version <version_sha>`. |
...
- Run targeted replay on affected workbooks: `cargo run -p dev-cli -- sheets replay --workbook <w> --reason "named-range restored"`.

## Verification

After recovery:
- `sheets_named_range_orphan_total == 0`.
- `sheets_formula_error_total{error_kind="#NAME?"}` at baseline.
...
- `app/sheets/runbooks/collab-conflict-resolution-crdt.md`.
