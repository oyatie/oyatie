...
date: 2026-05-17
related_artifacts:
  - app/sheets/failure-modes.md (FM-01 collab desync)
  - app/sheets/threat-model.md §"T-T-01" CRDT op forgery + §"T-T-02" workbook corruption
  - app/sheets/PRD.md §"Functional Requirements" FR-06 + AC-06
  - /specs/app/sheets.json §anti_patterns silent_merge_on_concurrent_edit
...
3. **`sheets_collab_conflict_surfaced_total` rate > 0.5/s for ≥ 5 min on a single (tenant, workbook_id) tuple** — abnormal conflict density.
4. **`sheets_collab_silent_loss_attempt_total > 0`** — Sev-1 (load-bearing invariant breach; never expected to fire).
...
2. Identify CRDT op stream window: read `sheets_collab_op_stream_seq` for the bracket.
...
| 2 | Reconstruct CRDT op stream from Postgres cell-edit-seal-deltas + Valkey ephemeral state: `cargo run -p sheets-collab-crdt-domain --bin reconstruct -- --tenant <h> --workbook <w>`. |
...
| 4 | If Sev-1: **stop all save-paths for the affected (tenant, workbook)**: `cargo run -p dev-cli -- vcs override-paths --microservice sheets --halt-saves --tenant <h> --workbook <w>` (requires 2-person rule). |
...
After recovery:
- `sheets_collab_conflict_surfaced_total` rate returns to baseline (≤ 0.1/s per tenant).
- `sheets_collab_silent_loss_attempt_total == 0` (held to zero is load-bearing).
...
- Update `app/sheets/PRD.md` and `failure-modes.md` if a new failure pattern surfaced.

## References

- `app/sheets/PRD.md` FR-06 + AC-06.
- `app/sheets/threat-model.md` T-T-01, T-T-02.
- `app/sheets/failure-modes.md` FM-01.
- `/specs/app/sheets.json` §anti_patterns.
