...
1. **`sheets_chart_render_seconds{quantile="0.95"} > 0.2` for ≥ 10 min** (AC-13 budget breach).
2. **`sheets_chart_count_per_sheet > 100` for an active sheet** (soft cap).
...
1. Identify slow sheets: query `sheets_chart_render_slow_top_n`.
2. Verify chart count per sheet: `sheets_chart_count_per_sheet_top_n`.
...
| Step | Action |
|---|---|
| 1 | Run chart-render benchmark corpus: `cargo run -p sheets-charts-adapter-leptos-wasm --bin bench`. |
...
After recovery:
- `sheets_chart_render_seconds{quantile="0.95"} ≤ 0.2`.
- No `sheets_chart_count_per_sheet > 100` active warnings.
