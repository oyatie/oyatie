---
doc_class: Runbook
doc_id: RUNBOOK-SUPPLY_CHAIN_PLANNING-COLD_MERGE
...
# Runbook: Supply Chain Planning Cold Merge

## Operator Contract
- Runbook id: supply-chain-planning-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-supply-chain-planning-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=supply-chain-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n supply-chain-planning rollout status deploy/supply-chain-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service supply-chain-planning --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service supply-chain-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service supply-chain-planning --cell $CELL --window 30m`.
