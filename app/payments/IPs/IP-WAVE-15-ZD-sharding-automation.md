...
---
# IP-WAVE-15-ZD-sharding-automation: Payments Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/payments/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: payments must interpret ADR-0348 through its own bounded context: charge.
SCOPE-004: payments uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: payments uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: payments uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: payments.
STANCE-002: Owner team: axis-payments.
...
STANCE-005: Bounded context anchor: charge.
STANCE-006: Cell placement class input: Tier-0.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: PCI-DSS-L1-v4, kr-fss, eu-psd2-sca, us-state-mtl, ccpa-cpra-2023, au-aml-ctf, br-lgpd-finance, in-rbi, cn-pipl-2021, coppa-minor-refusal, sox-itgc, PCI-DSS, us-healthcare, hipaa, gdpr, soc2-type2, lgpd, eu-ai-act, soc2, iso27001, SOC 2 Type 2, kr-pipa, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for payments.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for payments because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; payments must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where payments participates.
...
ROLLBACK-002: For payments, rollback starts from the audit-chain row that recorded the automation event.
