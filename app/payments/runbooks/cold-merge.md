---
doc_class: Runbook
doc_id: RUNBOOK-PAYMENTS-COLD_MERGE
...
# Runbook: Payments Cold Merge

## Operator Contract
- Runbook id: payments-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-payments-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=payments; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n payments rollout status deploy/payments --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service payments --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service payments --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service payments --cell $CELL --window 30m`.
