...
## Trigger Conditions
- Page on alert `PaymentsKYCAMLScreeningPipelineStallCritical` when `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `PaymentsKYCAMLScreeningPipelineStallSloBurn` when `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `oya_payments_kyc_aml_screening_pipeline_stall_correctness_ratio < 0.9999` and the affected label set includes `tenant_id`, `cell_id`, or `principal_id`.
- Open a sev1 if `oya_payments_kyc_aml_screening_pipeline_stall_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate payments-kyc-aml-screening-pipeline-stall --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio` exceeds the threshold documented in `app/payments/slos/charge-api-availability.openslo.yaml`.
...
- Kubernetes events include `reason=PaymentsKYCAMLScreeningPipelineStallDegraded` on deployment `payments`.
...
- Metric pattern: `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio` rises before `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_payments_kyc_aml_screening_pipeline_stall_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_payments_kyc_aml_screening_pipeline_stall_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific metric pattern: `oya_payments_kyc_aml_screening_pipeline_stall_ledger_mismatch_total` rises while `oya_payments_kyc_aml_screening_pipeline_stall_aml_review_lag_seconds` is flat; inspect local worker health before escalating vendors.
- Service-specific metric pattern: `oya_payments_kyc_aml_screening_pipeline_stall_aml_review_lag_seconds` rises while `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio` is flat; suspect stale export, stale recommendation, stale projection, or vendor dependency lag.
...
5. Read structured logs: `kubectl -n payments logs deploy/payments --since=30m | rg "payments.kyc_aml_screening_pipeline_stall.incident_state|PaymentsKYCAMLScreeningPipelineStallCritical|EVT_PAYMENTS_KYC_AML_SCREENING_PIPELINE_STALL_INCIDENT"`.
6. Query Loki directly: `logcli query '{namespace="payments",runbook="kyc-aml-screening-pipeline-stall"}' --since=30m --limit=200`.
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_payments_kyc_aml_screening_pipeline_stall_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_payments_kyc_aml_screening_pipeline_stall_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate payments-kyc-aml-screening-pipeline-stall --production-snapshot --cell $CELL`.
16. Run payments contract smoke: `cargo run -p oya-dev-cli -- gate validate payments-contract --microservice payments --scenario kyc-aml-screening-pipeline-stall`.
17. Check API contract smoke: `curl -s https://payments.internal.oyatie.dev/v1/payments/kyc-aml-screening-pipeline-stall/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
23. Check SLO files: `ls app/payments/slos/*.openslo.yaml | sort | rg "charge|kyc"`.
...
25. Run targeted SQL state query: `psql $OYA_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from payments_kyc_aml_screening_pipeline_stall_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric oya_payments_kyc_aml_screening_pipeline_stall_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_payments_kyc_aml_screening_pipeline_stall_queue_depth grow while oya_payments_kyc_aml_screening_pipeline_stall_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths app/payments/runbooks/kyc-aml-screening-pipeline-stall.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://payments.internal.oyatie.dev/v1/payments/kyc-aml-screening-pipeline-stall/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: confirmed money movement correctness risk.
  - Required action: keep `payments-kyc-aml-screening-pipeline-stall-circuit-breaker` open until `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: PSP dependency degraded.
  - Required action: keep `payments-kyc-aml-screening-pipeline-stall-circuit-breaker` open until `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio` is below 0.01 for 3 evaluator windows.
...
## Resolution Steps
1. Identify code owner path: `rg "kyc_aml_screening_pipeline_stall|PaymentsKYCAMLScreeningPipelineStallCritical|payments.kyc_aml_screening_pipeline_stall.incident_state" crates app/payments -g "!app/payments/runbooks/**"`.
2. Patch catalog invariant: `edit app/payments/catalog/oya-payments-charge-domain.yaml or the matching refund/dispute/payout/KYC catalog record where kyc_aml_screening_pipeline_stall state transition is validated`.
...
6. Add regression gate: `cargo run -p oya-dev-cli -- gate validate payments-contract --fixture incident-kyc-aml-screening-pipeline-stall.json`.
7. Add gate evidence: `cargo run -p oya-dev-cli -- gate validate payments-kyc-aml-screening-pipeline-stall --fixture incident-kyc-aml-screening-pipeline-stall.json`.
8. Add SLO assertion: `update app/payments/slos/charge-api-availability.openslo.yaml with alert PaymentsKYCAMLScreeningPipelineStallCritical when this was a missing alert`.
9. Add dashboard panel: `update app/payments/dashboards/psp-routing.json with oya_payments_kyc_aml_screening_pipeline_stall_error_ratio, oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds, and oya_payments_kyc_aml_screening_pipeline_stall_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_payments_kyc_aml_screening_pipeline_stall_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `app/payments/catalog/oya-payments-charge-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `app/payments/catalog/oya-payments-refund-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `app/payments/catalog/oya-payments-dispute-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `app/payments/catalog/oya-payments-payout-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `app/payments/dashboards/psp-routing.json`: verify panel coverage for `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio`, `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds`, and `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio`.
...
- `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds < 120` for all production cells.
- `oya_payments_kyc_aml_screening_pipeline_stall_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio` is below the threshold documented in `app/payments/slos/charge-api-availability.openslo.yaml`.
...
## Handoff Notes
- Do not hand off with only the alert name; include `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio`, `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds`, `oya_payments_kyc_aml_screening_pipeline_stall_queue_depth`, `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio`, current breaker state, and audit seal status.
...
- `app/payments/catalog/` for component and owner vocabulary; existing runbook topic `kyc-aml-screening-pipeline-stall` was preserved as the scenario anchor.
