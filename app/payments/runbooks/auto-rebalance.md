---
doc_class: Runbook
doc_id: RUNBOOK-PAYMENTS-AUTO_REBALANCE
...
# Runbook: Payments Auto Rebalance

## Operator Contract
- Runbook id: payments-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-payments-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=payments; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n payments rollout status deploy/payments --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service payments --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service payments --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service payments --cell $CELL --window 30m`.
