...
entry_gate: |
  ...
  on `dev` ≥ 1 week each.
exit_gate: |
  All 25 IPs (IP-001..IP-025) merged; governance-doc-set-payments
...
|---|---|---|---|
| Stripe | US / EU / global | `payments-adapter-stripe` | Full: charges / refunds / payouts / disputes / subscriptions / sub-merchant. |
| Adyen | EU / interchange-plus | `payments-adapter-adyen` | Full: charges / refunds / payouts / MarketPay. |
| Toss Payments | KR | `payments-adapter-toss` | Charges / refunds / payouts (KR-FSS-licensed). |
| KakaoPay | KR | `payments-adapter-kakaopay` | Charges / refunds (wallet only). |
| LINE Pay | JP / TW / TH | `payments-adapter-line-pay` | Charges / refunds. |
| WeChat Pay | CN | `payments-adapter-wechat-pay` | Charges only (CN-PIPL data-residency). |
| Alipay | CN / global | `payments-adapter-alipay` | Charges only. |
...
Sequencing: domain → kernel → usecase → adapter → REST / gRPC / worker → app. Each IP is single-PR-sized. Acceptance: `cargo nextest run --workspace -p payments-* --release` + lane-green on `governance-doc-set-payments`.
