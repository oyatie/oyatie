...
|---|---|---|
| `charge` | `oya-payments-charge-{kernel,domain,usecase,api,adapter,adapter-postgres,adapter-stripe,adapter-adyen,adapter-toss,adapter-kakaopay,adapter-linepay,adapter-wechatpay,adapter-alipay,adapter-paypal,rest,worker,sdk,app}` | One-shot charge surface; PSP routing |
...
| `webhook-dispatch` | `oya-payments-webhook-dispatch-{kernel,domain,usecase,api,adapter,rest,worker,sdk,app}` | Outbound webhook delivery + replay |
| `idempotency` | `oya-payments-idempotency-{kernel,domain,usecase,api,adapter,adapter-valkey,sdk}` | Idempotency-Key store |
| `fraud-screening` | `oya-payments-fraud-screening-{kernel,domain,usecase,api,adapter,rest,worker,sdk,app}` | Risk engine; ML + rule-based |
| `psp-routing` | `oya-payments-psp-routing-{kernel,domain,usecase,api,adapter,sdk}` | PSP-route decision logic |
| `audit-chain-bridge` | `oya-payments-audit-chain-bridge-{kernel,domain,usecase,api,adapter,worker,sdk}` | Ed25519 seal emission per ADR-0028 |
| `regulator-pull` | `oya-payments-regulator-pull-{kernel,domain,usecase,api,adapter,rest,sdk,app}` | 4-eye regulator-audit pull endpoint |
...
- ADR-0346 — the retired `./bin/oya verify --ci-required` path is historical/provenance-only per ADR-0515; verification and merge authority live in the cloud-ci Rust gate apps behind the single protected `oya-ci-required` context, and legacy `oya` CLI output is local feedback only. Historical enforcement names: `oya-governance-oya-verify-ci-mirror-coverage`, `oya-governance-oya-verify-ci-step-exit-semantics`, `oya-governance-oya-verify-skip-flag-allowlist`, `oya-governance-oya-submit-calls-verify`, `oya-governance-oya-verify-exit-code-contract`.
...
- ADR-0349 — Jenkins (LTS) wording is bridge/historical substrate, not destination CI authority, per ADR-0515; GitHub Actions plus the cloud-ci Rust gate apps behind the single `oya-ci-required` context are the live CI runner/authority, and ArgoCD/Argo Rollouts remain CD bridge/reference adapters where separately authorized. Historical enforcement names: `oya-governance-jenkins-github-actions-parity`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-governance-jenkins-jcasc-only`, `oya-governance-deploy-audit-chain-emit`.
