---
doc_class: Runbook
doc_id: RUNBOOK-PLANT_MAINTENANCE-HOT_SPLIT
...
# Runbook: Plant Maintenance Hot Split

## Operator Contract
- Runbook id: plant-maintenance-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plant-maintenance-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plant-maintenance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plant-maintenance rollout status deploy/plant-maintenance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plant-maintenance --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service plant-maintenance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service plant-maintenance --cell $CELL --window 30m`.
