# Healthcare Integration Runbook: Audit Completeness Gap

Service: healthcare-integration  
Surface: local operational primitive suite  
Primary SLO: audit-completeness  
Domain focus: hl7, fhir, hipaa, phi

## Trigger
- Alert `healthcare-integration-audit-completeness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_healthcare_integration_audit_completeness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when audit-completeness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `healthcare-integration-local-audit-completeness` and policy file name to the ticket.
