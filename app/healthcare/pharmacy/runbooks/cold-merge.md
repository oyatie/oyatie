---
doc_class: Runbook
doc_id: RUNBOOK-PHARMACY-COLD_MERGE
...
# Runbook: Pharmacy Cold Merge

## Operator Contract
- Runbook id: pharmacy-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-pharmacy-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=pharmacy; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n pharmacy rollout status deploy/pharmacy --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service pharmacy --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service pharmacy --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service pharmacy --cell $CELL --window 30m`.
