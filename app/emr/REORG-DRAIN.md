...
- **OVERRULE 3d / DECIDED FLAT (discovery 90/91):** product rail owns `app/emr/**` — never dump onto `integ/app` or nest under `integ/healthcare` / `app/healthcare/<ctx>`.
...
- Wave-1 absorb: product scaffold reclaim `oya/emr/**` → `app/emr/**` from `4aa692919b12` (AUDIT-FINDINGS excluded as dump-class).
- Judgment: envelopes **1.16.11+** per-product rail (OVERRULE 1.16.9 nest + #1611 delete).
- Wave-2 deepen (FLAT): in-tree durable cites `microservices/emr/` → `app/emr/`; REORG-DRAIN encodes multi-product grain.
...
- `oya/emr/**` resurrect or deletes — N/A / not this rail.
- `Cargo.lock` / root workspace membership — lock tip only.
- `specs/**` hub edits — `integ/specs` only (**no specs push** from this rail).
- Sibling products under `app/*` other than `emr`.
- Nest under `app/healthcare/emr/**` or birth `integ/healthcare` — **BAN**.
