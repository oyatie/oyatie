---
doc_class: Runbook
doc_id: RUNBOOK-APPLICATION-AUTO_REBALANCE
...
# Runbook: Application Auto Rebalance

## Operator Contract
- Runbook id: application-auto-rebalance.
- Primary service namespace: `application`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-application-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=application; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n application rollout status deploy/application --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service application --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service application --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service application --cell $CELL --window 30m`.
