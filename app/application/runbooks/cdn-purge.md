...
1. **FM-01 CDN cache poisoning** — automated: `application_bundle_sri_mismatch_total > 0` for ≥ 1 min raises Sev-1 page; worker auto-invokes global purge.
...
3. **FM-11 purge queue backlog** — `application_cdn_purge_queue_depth > 100` for ≥ 3 min; Sev-3 successor-IP.
...
1. Confirm the offending asset URL: read `application_bundle_sri_mismatch_total{url=...}` from Mimir.
2. Confirm the rollback bundle version: query `application_bundle_version_active{environment="production"}` and the prior in `application_bundle_version_prior`.
...
| 3 | Invoke purge: `cargo run -p dev-cli -- application cdn purge --pack <pack> --pattern "/assets/*" --reason "<rfc>"`. The CLI: <br>a. fetches CDN admin token from OpenBao;<br>b. issues per-POP purge requests in parallel;<br>c. polls purge-job status until all POPs report complete (≤ 60 s p99);<br>d. emits `CdnPurgeRequested` event into audit-chain;<br>e. sets `application_bundle_version_active` to the rollback SHA. | ≤ 60 s |
...
After manual purge: backfill `application_bundle_purge_manual_total{reason="..."}` counter and file an Issue for "why did CLI fail".
...
- `application_bundle_sri_mismatch_total == 0` for ≥ 5 min.
- `application_cdn_purge_queue_depth == 0`.
...
1. Re-promote the legitimate bundle: `cargo run -p dev-cli -- application bundle promote --sha <prior-sha>`.
