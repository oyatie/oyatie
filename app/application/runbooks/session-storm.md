...
`application_session_create_rate` > 10× rolling baseline OR
`application_auth_rate_limit_block_total` spikes (credential-stuffing
signal).
...
1. Identify storm class:
   - **Credential stuffing**: `application_auth_rate_limit_block_total{reason="rate_limit"}` is high; many distinct source IPs.
...
| 2 | Pre-checks | ≤ 5 min |
| 3 | If credential-stuffing: engage WAF challenge mode: `cargo run -p dev-cli -- application waf raise --pack <pack> --mode challenge` — Cloudflare bot management / OCI WAF puts JavaScript challenge in front of sign-in path | ≤ 5 min |
| 4 | Raise per-IP rate limit: `cargo run -p dev-cli -- application auth rate-limit set --pack <pack> --per-ip-per-min 10 --per-user-per-min 5` (down from defaults 60/30) | ≤ 2 min |
| 5 | Scale auth-gateway HPA min: `kubectl scale deployment/application-auth-gateway-rest --replicas=8` (override min until storm subsides) | ≤ 2 min |
| 6 | If Valkey memory pressure: trigger cluster expansion (add a shard pair) `kubectl scale statefulset/application-valkey-cluster --replicas=8` (from 6 → 8) | ≤ 10 min |
| 7 | Tighten session-store eviction policy temporarily: `cargo run -p dev-cli -- application session set-eviction-aggressive --pack <pack>` (kicks in if memory still pressured) | ≤ 2 min |
| 8 | Monitor `application_session_create_rate` falling; `application_valkey_memory_used_ratio < 0.7` | ≤ 30 min |
...
- `application_session_create_rate` returns to ≤ 2× baseline for ≥ 15 min.
- `application_valkey_memory_used_ratio < 0.7`.
- `application_auth_signin_success_rate > 99 %` over recent 5 min.
- No alarm on `application_session_evicted_unexpected_total`.
