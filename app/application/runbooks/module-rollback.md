...
1. **FM-02 module integrity failure** — `application_module_signature_invalid_total > 0` (automated; Sev-1 page).
...
1. Confirm the failing module: `application_module_signature_invalid_total{module="...", version="..."}`.
2. Confirm the prior good version: query `application_module_version_prior{module="..."}`; verify its `signer_key_id` matches the registered publisher key in OpenBao.
...
| 7 | If publisher-key compromise suspected: rotate via `cargo run -p dev-cli -- application module rotate-publisher-key --module <name>` and force re-publish | ≤ 30 min |
...
- `application_module_version_active{module="..."}` reflects the rollback.
- `application_module_signature_invalid_total == 0` for ≥ 5 min.
...
1. Operator with OpenBao admin grant: `cargo run -p dev-cli -- application module rotate-publisher-key --module <name>`.
