...
1. **FM-04 WASM corruption** — `application_wasm_instantiate_fail_total > 0`; canary cohort fails on bundle instantiate.
2. **FM-10 Leptos hydration regression** — `application_hydration_error_rate > 1%` for ≥ 1 min.
3. **FM-16 TTI breach** — `application_tti_p99_seconds > 2` for ≥ 5 min (multi-window burn-rate).
...
1. Confirm the failing bundle version: `application_bundle_version_active{environment="production"}`.
2. Confirm auto-rollback triggered (frontend-bundle-serve worker): `application_bundle_pointer_reverted_total > 0` recently.
...
- `application_wasm_instantiate_fail_total == 0` for ≥ 30 min.
- `application_tti_p99_seconds <= 2` for ≥ 30 min on rebuilt bundle.
- `application_hydration_error_rate < 0.1%` for ≥ 30 min.
