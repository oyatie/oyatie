---
doc_class: Runbook
doc_id: RUNBOOK-APPLICATION-COLD_MERGE
...
# Runbook: Application Cold Merge

## Operator Contract
- Runbook id: application-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-application-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=application; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n application rollout status deploy/application --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service application --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service application --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service application --cell $CELL --window 30m`.
