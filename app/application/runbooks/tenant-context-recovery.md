...
`application_tenant_context_resolve_fail_total > 0` for ≥ 30 s, OR
...
1. Confirm scope of failure: `application_tenant_context_resolve_fail_total` by `pack` label — is it one pack or all?
...
| 5 | If JWKS rotation race: pin previous JWKS for grace period via `cargo run -p dev-cli -- application auth jwks-pin --kid <prev-kid>` (10-min override) | ≤ 2 min |
| 6 | If middleware crashloop: kubectl rollout restart; verify deployment recovers | ≤ 5 min |
| 7 | Monitor `application_tenant_context_resolve_fail_total` returning to 0 | ≤ 10 min |
...
- `application_tenant_context_resolve_fail_total == 0` for ≥ 5 min.
