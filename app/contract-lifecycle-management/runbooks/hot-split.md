---
doc_class: Runbook
doc_id: RUNBOOK-CONTRACT_LIFECYCLE_MANAGEMENT-HOT_SPLIT
...
## Operator Contract
- Runbook id: contract-lifecycle-management-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contract-lifecycle-management-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contract-lifecycle-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
...
3. Verify alerts: `oya observability alerts list --service contract-lifecycle-management --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service contract-lifecycle-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service contract-lifecycle-management --cell $CELL --window 30m`.
