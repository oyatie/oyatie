---
doc_class: Runbook
doc_id: RUNBOOK-INCIDENT_MANAGEMENT-COLD_MERGE
...
# Runbook: Incident Management Cold Merge

## Operator Contract
- Runbook id: incident-management-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-incident-management-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=incident-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n incident-management rollout status deploy/incident-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service incident-management --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service incident-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service incident-management --cell $CELL --window 30m`.
