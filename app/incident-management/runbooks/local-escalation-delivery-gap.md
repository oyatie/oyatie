# Incident Management Runbook: Escalation Delivery Gap

Service: incident-management  
Surface: local operational primitive suite  
Primary SLO: escalation-delivery  
Domain focus: war-room, escalation, page, acknowledge

## Trigger
- Alert `incident-management-escalation-delivery` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_incident_management_escalation_delivery_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when escalation-delivery is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `incident-management-local-escalation-delivery` and policy file name to the ticket.
