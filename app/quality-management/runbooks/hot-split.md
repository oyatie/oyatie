---
doc_class: Runbook
doc_id: RUNBOOK-QUALITY_MANAGEMENT-HOT_SPLIT
...
# Runbook: Quality Management Hot Split

## Operator Contract
- Runbook id: quality-management-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-quality-management-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=quality-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n quality-management rollout status deploy/quality-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service quality-management --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service quality-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service quality-management --cell $CELL --window 30m`.
