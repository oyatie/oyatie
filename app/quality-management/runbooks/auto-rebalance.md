---
doc_class: Runbook
doc_id: RUNBOOK-QUALITY_MANAGEMENT-AUTO_REBALANCE
...
# Runbook: Quality Management Auto Rebalance

## Operator Contract
- Runbook id: quality-management-auto-rebalance.
- Primary service namespace: `quality-management`.
...
- Owning team: axis-quality-management + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-quality-management-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=quality-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n quality-management rollout status deploy/quality-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service quality-management --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service quality-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service quality-management --cell $CELL --window 30m`.
