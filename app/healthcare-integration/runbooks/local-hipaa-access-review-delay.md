# Healthcare Integration Runbook: Hipaa Access Review Delay

Service: healthcare-integration  
Surface: local operational primitive suite  
Primary SLO: hipaa-access-review-latency  
Domain focus: hl7, fhir, hipaa, phi

## Trigger
- Alert `healthcare-integration-hipaa-access-review-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_healthcare_integration_hipaa_access_review_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when hipaa-access-review-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `healthcare-integration-local-hipaa-access-review-latency` and policy file name to the ticket.
