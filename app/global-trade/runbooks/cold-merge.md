---
doc_class: Runbook
doc_id: RUNBOOK-GLOBAL_TRADE-COLD_MERGE
...
# Runbook: Global Trade Cold Merge

## Operator Contract
- Runbook id: global-trade-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-global-trade-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=global-trade; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n global-trade rollout status deploy/global-trade --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service global-trade --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service global-trade --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service global-trade --cell $CELL --window 30m`.
