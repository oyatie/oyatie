---
doc_class: Runbook
doc_id: RUNBOOK-GLOBAL_TRADE-HOT_SPLIT
...
# Runbook: Global Trade Hot Split

## Operator Contract
- Runbook id: global-trade-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-global-trade-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=global-trade; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n global-trade rollout status deploy/global-trade --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service global-trade --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service global-trade --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service global-trade --cell $CELL --window 30m`.
