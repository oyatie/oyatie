---
doc_class: Runbook
doc_id: RUNBOOK-GLOBAL_TRADE-AUTO_REBALANCE
...
# Runbook: Global Trade Auto Rebalance

## Operator Contract
- Runbook id: global-trade-auto-rebalance.
- Primary service namespace: `global-trade`.
...
- Owning team: axis-global-trade + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-global-trade-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=global-trade; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n global-trade rollout status deploy/global-trade --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service global-trade --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service global-trade --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service global-trade --cell $CELL --window 30m`.
