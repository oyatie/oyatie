# sites DR failover runbook

## Purpose

Recover the sites service inside the declared ADR-0343 DR envelope without changing tenant version pins or weakening Cedar admission.

## Recovery order

1. Freeze new mutating ingress for sites at api-gateway while read-only routes stay available when data freshness permits.
...
3. shift DNS and publish workers to the warm region, promote CMS metadata, and invalidate CDN manifests after object-store version recovery.
