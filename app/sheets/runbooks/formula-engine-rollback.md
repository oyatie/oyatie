...
date: 2026-05-17
related_artifacts:
  - microservices/sheets/failure-modes.md (FM-03)
  - microservices/sheets/threat-model.md §"T-T-03" (formula-engine function tampering)
  - microservices/sheets/PRD.md AC-11
  - microservices/sheets/decisions/ADR-SHEETS-0002 (formula-engine conformance)
...
1. **`oya-governance-sheets-formula-engine-correctness` CI lane fails post-release** (LibreOffice Calc reference corpus mismatch).
2. **Tenant reports**: "formula X now gives different result than yesterday" — triage as suspected formula-engine regression.
3. **`oya_sheets_formula_engine_corpus_mismatch_rate > 0` runtime audit** (synthetic corpus runs against production formula-engine).
4. **`oya_sheets_formula_eval_error_rate > 0.01` for ≥ 10 min** (cell-eval errors spike post-release).
...
1. Identify failing function(s): query `oya_sheets_formula_engine_corpus_mismatch_function_name_top_n`.
...
| 2 | Run `oya-governance-sheets-formula-engine-correctness` lane against hotfix: full corpus pass required. |
...
After recovery:
- `oya-governance-sheets-formula-engine-correctness` lane exit 0.
- `oya_sheets_formula_engine_corpus_mismatch_rate == 0` for ≥ 1h.
- `oya_sheets_formula_eval_error_rate` returns to baseline.
...
- `microservices/sheets/PRD.md` AC-11.
- `microservices/sheets/threat-model.md` T-T-03.
- `microservices/sheets/failure-modes.md` FM-03.
- `microservices/sheets/decisions/ADR-SHEETS-0002` (formula-engine conformance).
