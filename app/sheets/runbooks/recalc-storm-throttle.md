...
1. **`oya_sheets_recalc_queue_depth > 100` for ≥ 5 min** on a single workbook OR cluster-wide.
...
| Step | Action |
|---|---|
| 1 | Verify external-source health: `oya_sheets_connected_query_external_source_p99_seconds`. |
...
After recovery:
- `oya_sheets_recalc_queue_depth` returns to baseline (≤ 20).
