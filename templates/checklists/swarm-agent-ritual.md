...
purpose: |
  ...
  Installed runtime surfaces MAY cite this path; no repository-local runtime overlay is authoritative.
...
This file is the single canonical in-repo copy (INV-DOC-9). Dispatch cards cite this checklist + role + lane mission — not the whole programme SSOT. Repository-local agent runtime dot-directories stay ignored and untracked.
...
1. **Lens selection** — ≥2 task-fit lenses from `docs/AGENTS.md` § Engineering principles & review lenses; full battery only at root decisions; leaves inherit.
