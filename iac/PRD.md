...
- Error budget policy: `iac/runbooks/error-budget-policy.md` (extends observability template).
...
|---|---|---|
| AC-01 | A new µservice's `iac/helm/<chart>/Chart.yaml` + `values.yaml` lands in the registry within 5 minutes of PR merge | end-to-end test under `iac/tests/e2e/registry-onboarding.rs` |
| AC-02 | Render is deterministic: re-running on the same input produces an identical content digest | integration test `iac/tests/integration/render-determinism.rs` |
...
| AC-11 | Apply latency p99 ≤ 5min per µservice (excluding workload-health waits) | load test under `iac/tests/load/apply-latency.rs` |
...
- ADR-0339 adoption keeps reusable HCL in `iac/modules/<context>/<primitive>/`; `cloud-iac` owns primitive selection and tenant-scoped variables.
