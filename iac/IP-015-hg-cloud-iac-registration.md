...
Updates `/specs/hyperscaler-gates.json` + adds a bootstrap-script under `microservices/cloud-iac/scripts/bootstrap.sh` + a self-apply integration test.
...
| `microservices/cloud-iac/scripts/bootstrap.sh` | create — initial substrate apply via cloud-k8s + cloud-secrets |
| `microservices/cloud-iac/tests/e2e/self_apply.rs` | create — verify cloud-iac can apply its own IaC after bootstrap |
...
```bash
#!/usr/bin/env bash
# microservices/cloud-iac/scripts/bootstrap.sh
