...
the service-local module registry under `iac/tofu/modules/`
and the OpenTofu runner chart under `iac/iac/helm/opentofu/`.

## Real service paths

| Path | Contract this IP protects |
|---|---|
| `iac/tofu/modules/cloud-account/main.tofu` | account bootstrap variables and `account_id`/`account_alias` outputs |
| `iac/tofu/modules/vpc/main.tofu` | network module surface consumed before namespace bootstrap |
| `iac/tofu/modules/dns/main.tofu` | DNS module surface for pack/environment endpoints |
| `iac/tofu/modules/kms/main.tofu` | KMS key material references for state and backup encryption |
| `iac/tofu/modules/secrets-bootstrap/main.tofu` | OpenBao seed outputs consumed by secret projection |
| `iac/tofu/modules/k8s-namespace-bootstrap/main.tofu` | `namespace_name`, `service_account_name`, and `argocd_project_name` outputs |
| `iac/iac/helm/opentofu/Chart.yaml` | runner deployment package |
| `iac/iac/helm/opentofu/values.yaml` | runner image/version/resource contract |
...
- `iac/cross-microservice-handoffs.md` outbound rows for
...
- `iac/contracts/openapi/cloud-iac.yaml` operations
...
- `find iac -name "*.tf"` returns no service-local module
  roots.
- `tofu validate` is run for each existing directory under
  `iac/tofu/modules/`.
- `helm lint iac/iac/helm/opentofu` passes before any
...
```bash
find iac -name "*.tf" -print
find iac/tofu/modules -maxdepth 2 -name main.tofu -print
helm lint iac/iac/helm/opentofu
```
...
- Surface evidence: `iac/contracts/openapi/cloud-iac.yaml`, `iac/contracts/asyncapi/cloud-iac-events.yaml`, `iac/contracts/proto/cloud-iac.proto`, `iac/IP-GITOPS-001-terraform-to-opentofu-migration.md`.
