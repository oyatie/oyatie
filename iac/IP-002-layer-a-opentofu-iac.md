...
Helm chart for OpenTofu self-hosted runner (Terraform-Cloud-equivalent) + per-pack S3 state-bucket OpenTofu config under `iac/iac/`. Provides the OpenTofu execution surface used by iac-renderer (`-adapter-opentofu`) and iac-applier.
...
|---|---|---|
| `iac/iac/helm/opentofu/Chart.yaml` | create | OpenTofu runner chart |
| `iac/iac/helm/opentofu/values.yaml` | create | LTS-pinned OpenTofu version; HA runner replicas |
| `iac/iac/terraform/per-pack-state-bucket.tf` | create | S3-compatible state bucket per pack; SSE-KMS; versioned; WORM |
| `iac/iac/terraform/state-bucket-iam.tf` | create | per-pack IAM policy for state-bucket access |
| `iac/iac/kustomize/base/kustomization.yaml` | update | add opentofu chart to shared base |
...
tofu validate iac/iac/terraform/
