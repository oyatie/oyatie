...
# Data Protection Impact Assessment: cloud-iac
...
- Event surface: automation events for this microservice include required cloud-ci/ci gate outcomes, governance lane rename evidence, sharding automation decisions, and ArgoCD GitOps CD transitions.
...
- Cedar decision id, audit-chain event id, `presubmit` run id, ArgoCD sync id, cosign verification result, and governance lane id.
...
### ADR-Bound Controls
- ADR-0346 supersession: retired local oya verifier/gate/check wrappers are not authority mechanisms for active work; protected-branch merge readiness is the cloud-ci/ci `presubmit` status backed by shared Rust gate logic and the local-authority enforcer.
- ADR-0347: every `governance-*` CI lane prefix in the Oyatie corpus RENAMES to `governance-*` in a single bulk-rename pull request (Wave 15-ZB). Enforced by retired-fitness-residue detection, `governance-lane-prefix-vocabulary`, and `governance-rename-inventory-presence`.
...
- ADR-0349 supersession: GitHub Actions produces the live required `presubmit` context until the owned ci runner cutover; ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `presubmit`, `governance-argocd-application-cosign-verified`, `governance-argocd-tenant-namespace-isolation`, and `governance-deploy-audit-chain-emit`.

### DPIA Assessment
- Necessity: automation evidence is necessary to prove required CI gate completeness, governance lane vocabulary, tenant placement, rebalance reversibility, dynamic shard threshold decisions, and CI/CD deployment accountability.
...
- Access control: service principals, cloud-ci/ci principals, ArgoCD principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
### Residual Risk
- Residual risk is medium until Wave 15-ZA/ZB/ZD/ZE implementation PRs land shared Rust gate logic, governance rename, sharding automation bodies, and ArgoCD substrate files.
- Residual risk becomes low when the ADR-0347, ADR-0348, and current cloud-ci/ci enforcement lanes promote from advisory/report-only to BLOCKER and pass for this microservice through `presubmit`.
