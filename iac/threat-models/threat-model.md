...
- ADR-0346 supersession: retired local oya verifier/gate/check wrappers must not decide merge readiness; the required CI authority is the cloud-ci/oya-ci `oya-ci-required` status backed by shared Rust gate logic.
...
- ADR-0349 supersession: GitHub Actions produces the live required `oya-ci-required` context until owned oya-ci runner cutover; ArgoCD cosign/audit-chain lanes preserve GitOps CD controls.
