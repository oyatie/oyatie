...
Author Helm + Kustomize manifests for the GitOps reconciler stack (ArgoCD primary; Flux alternative for tenant choice) plus the Flux Helm-controller + Kustomize-controller under `iac/iac/helm/`. Deploys to the dedicated cloud-iac control-plane Kubernetes cluster per PRD Layer-A. Versions pinned to LTS per `docs/standards/observability-slo.md` §"Supply-chain conformance".
...
|---|---|---|
| `iac/iac/helm/argocd/Chart.yaml` | create | upstream dep on argoproj/argo-cd chart at pinned LTS |
| `iac/iac/helm/argocd/values.yaml` | create | HA replicas + Valkey Sentinel + admission webhook |
| `iac/iac/helm/flux/{Chart.yaml,values.yaml}` | create | source-controller + kustomize-controller + helm-controller |
| `iac/iac/helm/helm-controller/{Chart.yaml,values.yaml}` | create | Flux Helm-controller standalone deployment |
| `iac/iac/helm/kustomize-controller/{Chart.yaml,values.yaml}` | create | Flux Kustomize-controller standalone deployment |
| `iac/iac/kustomize/base/kustomization.yaml` | create | shared base referencing all 4 charts |
| `iac/iac/kustomize/overlays/pack-kr/kustomization.yaml` | create | pack-kr overlay (initial active pack) |
...
helm lint iac/iac/helm/flux
helm lint iac/iac/helm/helm-controller
helm lint iac/iac/helm/kustomize-controller
kubectl --dry-run=client apply -k iac/iac/kustomize/overlays/pack-kr
...
- Test files: `iac/tests/iac/{argocd,flux,helm-controller,kustomize-controller}.bats`.
...
- Surface evidence: `iac/runbooks/restore-drill-quarterly.md`, `iac/runbooks/seaweedfs-volume-failover.md`, `iac/manifest.json`, `iac/IP-001-layer-a-argocd-flux-iac.md`.
