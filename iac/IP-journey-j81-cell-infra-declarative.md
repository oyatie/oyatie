...
## B. Approach
Use the iac-registry as the audit index and the iac-applier event stream as the immutable proof source. The OpenAPI contract in `iac/contracts/openapi/cloud-iac.yaml` exposes read-only registry evidence; `iac/policy/auditor-scope.cedar` limits the auditor principal; `iac/dashboards/registry-health.json` and `iac/dashboards/drift-coverage.json` supply operational context.

## C. Deliverables
- Add j81 audit-pull examples to `iac/contracts/openapi/cloud-iac.yaml`.
- Add an `iac.registry.audit_pulled` style event example to `iac/contracts/asyncapi/cloud-iac-events.yaml`.
- Bind auditor authorization to `iac/policy/auditor-scope.cedar`.
- Reference restore and registry recovery in `iac/runbooks/registry-restore.md`.
...
3. Attach SLSA provenance evidence from `iac/slos/slsa-provenance-completeness.openslo.yaml`.
4. Require drift status from `iac/dashboards/drift-coverage.json` before the audit bundle is considered complete.
...
- The rollback/restore path names `iac/runbooks/registry-restore.md`.

## F. Evidence
- Journey: `docs/user-journeys/j81-kr-csap-sovereign-cell-audit-pull/README.md`.
- Service anchor: `iac/ARCHITECTURE.md`.
- Runbook: `iac/runbooks/registry-restore.md`.
...
- Surface evidence: `iac/contracts/openapi/cloud-iac.yaml`, `iac/contracts/asyncapi/cloud-iac-events.yaml`, `iac/contracts/proto/cloud-iac.proto`, `iac/IP-journey-j81-cell-infra-declarative.md`.
