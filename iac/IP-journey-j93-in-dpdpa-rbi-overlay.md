...
## B. Approach
Use the iac-validator to enforce DPDPA/RBI pack inputs before apply, iac-registry to store state refs, and iac-rollback to compensate failed or revoked overlays. Real anchors are `iac/contracts/openapi/cloud-iac.yaml`, `iac/policy/tenant-scope.cedar`, and `iac/slos/iac-validator-availability.openslo.yaml`.

## C. Deliverables
- India pack validation examples in OpenAPI.
- Denied and permitted validation events in AsyncAPI.
- Registry catalog link to `iac/catalog/oya-cloud-iac-iac-registry-usecase.yaml`.
- Rollback runbook link to `iac/runbooks/rollback-orchestration.md`.
...
- SLO: `iac/slos/iac-validator-availability.openslo.yaml`.
- Catalog: `iac/catalog/oya-cloud-iac-iac-registry-usecase.yaml`.
...
- Surface evidence: `iac/contracts/openapi/cloud-iac.yaml`, `iac/contracts/asyncapi/cloud-iac-events.yaml`, `iac/contracts/proto/cloud-iac.proto`, `iac/IP-journey-j93-in-dpdpa-rbi-overlay.md`.
...
- Surface evidence: `iac/runbooks/restore-drill-quarterly.md`, `iac/runbooks/seaweedfs-volume-failover.md`, `iac/manifest.json`, `iac/IP-journey-j93-in-dpdpa-rbi-overlay.md`.
