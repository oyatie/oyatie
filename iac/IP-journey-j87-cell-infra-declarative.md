...
## B. Approach
Use the existing self-hosted renderer and applier crates from `iac/manifest.json`. The bundle includes contract examples from `iac/contracts/proto/cloud-iac.proto`, signed module provenance from `iac/slos/slsa-provenance-completeness.openslo.yaml`, and operational recovery through `iac/runbooks/stuck-apply-recovery.md`.

## C. Deliverables
- Air-gap bundle fields in `iac/contracts/openapi/cloud-iac.yaml`.
- Offline apply event examples in `iac/contracts/asyncapi/cloud-iac-events.yaml`.
- Evidence links to `iac/runbooks/stuck-apply-recovery.md` and `iac/runbooks/restore-drill-quarterly.md`.
- Capability references to `iac/capabilities/iac-apply.yaml` and `iac/capabilities/iac-rollback.yaml`.
...
- `iac/competitor-parity-matrix.md` claim boundaries remain respected.

## F. Evidence
- Journey: `docs/user-journeys/j87-fedramp-high-il5-air-gap-deployment/README.md`.
- Runbook: `iac/runbooks/stuck-apply-recovery.md`.
...
- Surface evidence: `iac/contracts/openapi/cloud-iac.yaml`, `iac/contracts/asyncapi/cloud-iac-events.yaml`, `iac/contracts/proto/cloud-iac.proto`, `iac/IP-journey-j87-cell-infra-declarative.md`.
