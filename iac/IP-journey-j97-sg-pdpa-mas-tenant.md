...
## B. Approach
Bind provisioning to `iac/policy/iac-isolation.md`, `iac/runbooks/restore-drill-quarterly.md`, and `iac/slos/iac-rollback-latency.openslo.yaml`. The validator enforces pack and cell posture; applier mutates only after signed validation.

## C. Deliverables
- Singapore tenant examples in `iac/contracts/openapi/cloud-iac.yaml`.
- Incident/rollback event examples in `iac/contracts/asyncapi/cloud-iac-events.yaml`.
- Isolation and restore drill evidence references.
- Registry binding through `iac/catalog/oya-cloud-iac-iac-registry-kernel.yaml`.
...
- Rollback evidence is tied to `iac/slos/iac-rollback-latency.openslo.yaml`.
- No generic `TASK-###` or `FM-###` rows remain.

## F. Evidence
- Journey: `docs/user-journeys/j97-sg-pdpa-mas-singapore-tenant/README.md`.
- SLO: `iac/slos/iac-rollback-latency.openslo.yaml`.
- Runbook: `iac/runbooks/restore-drill-quarterly.md`.
...
- Surface evidence: `iac/contracts/openapi/cloud-iac.yaml`, `iac/contracts/asyncapi/cloud-iac-events.yaml`, `iac/contracts/proto/cloud-iac.proto`, `iac/IP-journey-j97-sg-pdpa-mas-tenant.md`.
...
- Surface evidence: `iac/runbooks/restore-drill-quarterly.md`, `iac/runbooks/seaweedfs-volume-failover.md`, `iac/manifest.json`, `iac/IP-journey-j97-sg-pdpa-mas-tenant.md`.
