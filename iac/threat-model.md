...
| SLSA L3 attestation verifier | Cedar policies under `iac/policy/` |
...
| Cedar policy fragments | `INTERNAL_ONLY` | Medium | git history | `iac/policy/*.cedar` |
...
- Mitigations:
  ...
  - Per-changeset evidence at `iac/evidence/multispectrum/<change_id>-<unix_ts>.json` is git-committed; commit signed by author.
...
|---|---|---|
| SOC 2 Type 2 (2017 TSC + 2022 PoF) | CC6.x (logical access) ← Cedar + RBAC; CC7.x (system operations) ← runbooks + alerting; CC8.x (change management) ← PR review + ChangeSet; CC9.x (risk mitigation) ← residual-risk acceptance | `iac/compliance.md` |
| ISO 27001:2022 | Annex A.5–A.8 controls covered as cited inline in each threat row | `iac/compliance.md` |
| GDPR | Arts. 5, 6, 25, 28, 30, 32, 33 cited inline | `iac/dpia.md` + `iac/compliance.md` |
| OpenSSF SLSA L3 | Build-provenance + attestation chain; per-apply verification | `iac/compliance.md` §"SLSA L3" |
