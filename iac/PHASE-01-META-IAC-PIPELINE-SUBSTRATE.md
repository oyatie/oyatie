...
exit_gate: |
  ...
  branch-protected `presubmit` fan-in reports green for workspace tests,
...
|---|---|---|---|
| `cloud-iac` | `iac-renderer`, `iac-validator`, `iac-applier`, `iac-rollback`, `iac-registry` | All under `microservices/cloud-iac/` per ADR-0131 | `cloud-iac-iac-renderer-{kernel,domain,usecase,api,adapter,adapter-helm,adapter-kustomize,adapter-opentofu,rest,worker,sdk,app}` + `cloud-iac-iac-validator-{kernel,domain,usecase,api,adapter,rest,worker,app}` + `cloud-iac-iac-applier-{kernel,domain,usecase,api,adapter,adapter-argocd,rest,worker,app}` + `cloud-iac-iac-rollback-{kernel,domain,usecase,api,adapter,rest,worker,app}` + `cloud-iac-iac-registry-{kernel,domain,usecase,api,adapter,adapter-postgres,rest,worker,sdk,app}` |

Plus repo-wide cross-cutting artifacts:
- `Cargo.toml` (workspace) — register the new crates under `microservices/cloud-iac/src/crates/`.
- `/specs/hyperscaler-gates.json` — register HG-CLOUD-IAC gate per ADR-0123.
- `.github/branch-protection.yaml` — add `cloud-iac-iac-smoke` to required_status_checks on `dev`.
...
| [`IP-003-iac-renderer-kernel.md`](IP-003-iac-renderer-kernel.md) | `cloud-iac-iac-renderer-kernel`: port traits + entities + sealed traits | pending | axis-cloud-iac | — |
...
| [`IP-006-iac-validator-kernel-domain-usecase.md`](IP-006-iac-validator-kernel-domain-usecase.md) | `cloud-iac-iac-validator-*` core stack (kernel + domain + usecase + adapter) | pending | axis-cloud-iac | IP-003, IP-005 |
| [`IP-007-iac-applier-kernel-domain-usecase.md`](IP-007-iac-applier-kernel-domain-usecase.md) | `cloud-iac-iac-applier-*` core stack + `-adapter-argocd` | pending | axis-cloud-iac | IP-003, IP-006 |
| [`IP-008-iac-registry-postgres.md`](IP-008-iac-registry-postgres.md) | `cloud-iac-iac-registry-*` core stack + `-adapter-postgres`; iac-state-index schema | pending | axis-cloud-iac | IP-003 |
| [`IP-009-iac-rollback-engine.md`](IP-009-iac-rollback-engine.md) | `cloud-iac-iac-rollback-*` core stack; coordinate with SLO gate rollback | pending | axis-cloud-iac | IP-007 |
...
| [`IP-013-sdk-and-observability-slo.md`](IP-013-sdk-and-observability-slo.md) | `cloud-iac-iac-renderer-sdk` + `-iac-registry-sdk` + OpenSLO manifests at `microservices/cloud-iac/slos/` | pending | axis-cloud-iac | IP-011 |
...
cloud-ci/ci governance gate `lean-a2` for --microservice cloud-iac is green in the branch-protected `presubmit` context
cloud-ci/ci governance gate `port-location` for --microservice cloud-iac is green in the branch-protected `presubmit` context
...
cloud-ci/ci governance gate `statelessness` for --microservice cloud-iac is green in the branch-protected `presubmit` context
...
cloud-ci/ci governance gate `cloud-iac-render-determinism` is green in the branch-protected `presubmit` context
cloud-ci/ci governance gate `cloud-iac-apply-scope-isolation` is green in the branch-protected `presubmit` context
cloud-ci/ci governance gate `cloud-iac-drift-detection-coverage` is green in the branch-protected `presubmit` context
cloud-ci/ci governance gate `cloud-iac-provenance-slsa-l3` is green in the branch-protected `presubmit` context
```
...
|---|---|---|
| Render determinism | `cargo nextest run -p cloud-iac-iac-renderer-usecase --test render_determinism` | identical input → identical content digest |
| Apply scope isolation | `cargo nextest run -p cloud-iac-iac-applier-usecase --test apply_scope_isolation` | apply refused when manifest references resources outside µservice scope |
...
| SLSA L3 verification | `cargo nextest run -p cloud-iac-iac-registry-usecase --test slsa_l3_verify` | apply refused for unsigned chart; accepted for signed |

### Workflow + Ontology integration gates

```text
cloud-ci/ci governance gate `workflow-event-registry` for --microservice cloud-iac is green in the branch-protected `presubmit` context
cloud-ci/ci governance gate `ontology-type-registry` for --microservice cloud-iac is green in the branch-protected `presubmit` context
```
...
|---|---|---|---|
| `cloud-iac-iac-renderer-kernel` | `kernel` | (nothing project-internal) | all other layers |
| `cloud-iac-iac-renderer-domain` | `domain` | `kernel` | `usecase`, `adapter`, `rest`, `worker`, `app` |
| `cloud-iac-iac-renderer-usecase` | `usecase` | `domain`, `kernel` | `adapter`, `rest`, `worker`, `app` |
| `cloud-iac-iac-renderer-adapter` | `adapter` | `usecase`, `domain`, `kernel` | `rest`, `worker`, `app` |
| `cloud-iac-iac-renderer-adapter-helm` | `adapter` | `usecase`, `domain`, `kernel` | same |
| `cloud-iac-iac-renderer-adapter-kustomize` | `adapter` | `usecase`, `domain`, `kernel` | same |
| `cloud-iac-iac-renderer-adapter-opentofu` | `adapter` | `usecase`, `domain`, `kernel` | same |
| `cloud-iac-iac-renderer-rest` | `rest` | `usecase`, `domain`, `kernel`, `api` | `adapter` directly (uses ports) |
| `cloud-iac-iac-renderer-worker` | `worker` | `usecase`, `domain`, `kernel` | `adapter` directly |
| `cloud-iac-iac-renderer-sdk` | `sdk` | `kernel`, `api` | other layers |
| `cloud-iac-iac-renderer-app` | `app` | composition root | none — but only wiring |
...
Validated by `governance-multispectrum-evidence` lane against `/specs/multispectrum-review.json` v2.4.0; PRs without conforming evidence refused.
...
- Memory: `feedback_clean_architecture_requirements.md`, `feedback_quality_performance_scalability_bar.md`, `feedback_naming_justification.md`, `feedback_vcs_canonical_2026_05_16.md`.
