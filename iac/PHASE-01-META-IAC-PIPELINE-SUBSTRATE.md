...
|---|---|---|---|
| `cloud-iac` | `iac-renderer`, `iac-validator`, `iac-applier`, `iac-rollback`, `iac-registry` | All under `microservices/cloud-iac/` per ADR-0131 | `oya-cloud-iac-iac-renderer-{kernel,domain,usecase,api,adapter,adapter-helm,adapter-kustomize,adapter-opentofu,rest,worker,sdk,app}` + `oya-cloud-iac-iac-validator-{kernel,domain,usecase,api,adapter,rest,worker,app}` + `oya-cloud-iac-iac-applier-{kernel,domain,usecase,api,adapter,adapter-argocd,rest,worker,app}` + `oya-cloud-iac-iac-rollback-{kernel,domain,usecase,api,adapter,rest,worker,app}` + `oya-cloud-iac-iac-registry-{kernel,domain,usecase,api,adapter,adapter-postgres,rest,worker,sdk,app}` |
