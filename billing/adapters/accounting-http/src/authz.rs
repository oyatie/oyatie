//! Fail-closed authorization seam for the Accounting journal money-mutation
//! control plane (AUTH-005 / money-CRIT class; ADR-0593).
//!
//! ## Why this module exists
//!
//! `billing-accounting-http-adapter` is the HTTP runtime adapter for the
//! accounting money-mutation surfaces (`POST /accounting/v1/journals`,
//! `/payroll-postings`, `/vat-workflow-plans`). Before this seam,
//! [`crate::accounting_runtime_chain`] returned an EMPTY `MiddlewareChain::new()`,
...
//! journal or payroll posting against ANY tenant they name. This is the AUTH-005
//! unauthenticated / caller-supplied-authz class on a money surface (the money
//! CRITICAL finding for accounting-journal).
//!
...
//!    `action = accounting.journal.{post,payroll_posting,vat_workflow}` on the
//!    target `{tenant, operation, request_id}` via a PDP
//!    [`AccountingMutationAuthorizer`] port (`ensure_authorized`). The resource
//!    the PDP sees is bound to the VERIFIED principal's tenant — never flattened
//!    to caller input — so cross-tenant mutation is deniable.
//! 3. The boundary REFUSES to serve a money mutation without both ports running:
//!    [`crate::accounting_runtime_chain`] requires an [`AccountingAuthzProvider`]
//!    and the [`AccountingAuthzMiddleware`] short-circuits 401/403 BEFORE the
//!    handler deserializes the body. There is no default-allow fallback and no
//!    `Default` impl. The handlers additionally REJECT (403) any body whose
//!    `tenant_id` does not equal the verified tenant the middleware injected (no
//!    cross-tenant body substitution — true blast radius).
//!
//! ## Clean architecture (ADR-0131 / ports-for-owned-stack doctrine)
//!
//! [`PrincipalVerifier`] and [`AccountingMutationAuthorizer`] are PORTS owned by
//! this adapter crate. The concrete cloud-iam Cedar PDP client and the
...
/// drop-in alternate. Built from the request `Authorization` header by the
...
/// obtain one by running a real [`PrincipalVerifier`].
///
/// **Limits of this guarantee:** this is *structural* defense-in-depth, not a
/// cryptographic proof. The real security guarantee comes from the *combination*
/// of: (1) bearer/mTLS verification at the edge before the body is processed,
/// (2) the PDP authorization decision bound to the verified tenant, and (3) the
/// handler's body-tenant cross-check.
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum AccountingMutationAuthorizationError {
...
/// The accounting money-mutation action a decision is made against. Carried
/// explicitly so the PDP keys on the precise operation, never inferring it.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AccountingMutationAction {
    /// `accounting.journal.post`.
    JournalPost,
    /// `accounting.journal.payroll_posting`.
    PayrollPosting,
    /// `accounting.journal.vat_workflow`.
    VatWorkflow,
}

impl AccountingMutationAction {
    /// The canonical surface string the PDP keys the action on.
    #[must_use]
    pub const fn surface(self) -> &'static str {
        match self {
            Self::JournalPost => "accounting.journal.post",
            Self::PayrollPosting => "accounting.journal.payroll_posting",
            Self::VatWorkflow => "accounting.journal.vat_workflow",
        }
    }
}

/// The resource an accounting money-mutation decision is made against.
///
...
/// authorize an accounting mutation against tenant B's ledger. The PDP must deny
/// when the verified tenant is not permitted for the action.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AccountingMutationResource {
    ...
    pub action: AccountingMutationAction, // data_class: INTERNAL_ONLY
...
///    `Err(AccountingMutationAuthorizationError::Refused)`. Never propagate an
///    internal error as `Ok(())`.
...
///    [`AccountingAuthzProvider`] wrapper calls `catch_unwind` as a
///    **test/debug-only best-effort** backstop only; it MUST NOT be relied upon
///    in production. Adapters MUST NOT panic — use `Err(Refused)`.
pub trait AccountingMutationAuthorizer: Send + Sync {
    /// Authorize `principal` to perform the money mutation on `resource`, or
    /// refuse.
    ///
    /// # Errors
    /// [`AccountingMutationAuthorizationError`] on an explicit deny or any PDP
    /// fault (timeout, network, unavailability — all MUST be `Refused`;
    /// fail-closed: the caller maps this to HTTP 403).
    fn ensure_authorized(
        &self,
        principal: &VerifiedPrincipal,
        resource: &AccountingMutationResource,
    ) -> Result<(), AccountingMutationAuthorizationError>;
}

/// The authz provider the boundary depends on: a principal verifier PORT plus an
/// accounting-mutation authorizer PORT. The composition root REFUSES to serve
/// without one configured (no default-allow fallback, no `Default` impl).
#[derive(Clone)]
pub struct AccountingAuthzProvider {
    verifier: Arc<dyn PrincipalVerifier>, // data_class: INTERNAL_ONLY
    authorizer: Arc<dyn AccountingMutationAuthorizer>, // data_class: INTERNAL_ONLY
}

impl AccountingAuthzProvider {
    /// Assemble the provider from a principal verifier and an accounting
    /// authorizer.
    #[must_use]
    pub fn new(
        verifier: Arc<dyn PrincipalVerifier>,
        authorizer: Arc<dyn AccountingMutationAuthorizer>,
    ) -> Self {
        Self {
            verifier,
            authorizer,
    ...
    /// backstop. In production the release profile sets `panic = "abort"`, so
    /// `catch_unwind` has NO effect and the process aborts. The real fail-closed
    /// guarantee comes from the [`AccountingMutationAuthorizer`] adapter contract:
    /// adapters MUST map every fault to `Err(Refused)` and MUST NOT panic.
    ///
    /// # Errors
    /// [`AccountingMutationAuthorizationError`] — caller maps to HTTP 403.
    pub fn ensure_authorized(
        ...
        .unwrap_or(Err(AccountingMutationAuthorizationError::Refused))
    }
}
...
#[must_use]
pub fn action_for_template(template: &str) -> Option<AccountingMutationAction> {
    match template {
        crate::ACCOUNTING_JOURNALS_PATH => Some(AccountingMutationAction::JournalPost),
        crate::ACCOUNTING_PAYROLL_POSTINGS_PATH => Some(AccountingMutationAction::PayrollPosting),
        crate::ACCOUNTING_VAT_WORKFLOW_PLANS_PATH => Some(AccountingMutationAction::VatWorkflow),
        _ => None,
    }
}
...
pub struct AccountingAuthzMiddleware {
    provider: AccountingAuthzProvider,
}

impl AccountingAuthzMiddleware {
    #[must_use]
    pub fn new(provider: AccountingAuthzProvider) -> Self {
        Self { provider }
    }
    ...
    fn unauthorized_401() -> HttpResponse {
        HttpResponse::new(401)
            ...
            .with_body(
                br#"{"error":{"code":"UNAUTHENTICATED","message":"accounting money mutation requires a verified principal"}}"#
                    .to_vec(),
            )
    }
    ...
    fn forbidden_403() -> HttpResponse {
        HttpResponse::new(403)
            .with_header("content-type", "application/json")
            .with_body(
                br#"{"error":{"code":"FORBIDDEN","message":"principal not authorized for this accounting money mutation"}}"#
                    .to_vec(),
            )
    }
}

impl Middleware<HttpRequest, HttpResponse> for AccountingAuthzMiddleware {
    fn handle(
        &self,
        ...
        let resource = AccountingMutationResource {
...
mod tests {
    ...
    const SECRET: &str = "accounting-break-glass";

    fn verifier() -> ConfiguredBearerPrincipalVerifier {
        match ConfiguredBearerPrincipalVerifier::new(SECRET, "sp_accounting", "ten_acme") {
    ...
    struct AllowAll;
    impl AccountingMutationAuthorizer for AllowAll {
        fn ensure_authorized(
            &self,
    ...
    struct Panicker;
    impl AccountingMutationAuthorizer for Panicker {
        fn ensure_authorized(
            &self,
            _principal: &VerifiedPrincipal,
            _resource: &AccountingMutationResource,
    ...
    fn resource() -> AccountingMutationResource {
        AccountingMutationResource {
            tenant_id: "ten_acme".to_string(),
            action: AccountingMutationAction::JournalPost,
            route_template: crate::ACCOUNTING_JOURNALS_PATH.to_string(),
        }
    }
    ...
    fn verifier_refuses_empty_secret_at_construction() {
        assert!(matches!(
            ConfiguredBearerPrincipalVerifier::new("  ", "sp_accounting", "ten_acme"),
    ...
    fn verifier_binds_identity_from_config_not_input() {
        ...
        let verified = verifier
            ...
            .unwrap();
        assert_eq!(verified.principal_id(), "sp_accounting");
    ...
    fn provider_maps_panicking_authorizer_to_refused() {
        let provider = AccountingAuthzProvider::new(Arc::new(verifier()), Arc::new(Panicker));
        let principal = VerifiedPrincipal::new_for_test("sp_accounting", "ten_acme");
        assert_eq!(
            provider
                .ensure_authorized(&principal, &resource())
                .unwrap_err(),
            AccountingMutationAuthorizationError::Refused
        );
    }

    #[test]
    fn provider_allows_when_authorizer_allows() {
        let provider = AccountingAuthzProvider::new(Arc::new(verifier()), Arc::new(AllowAll));
        let principal = VerifiedPrincipal::new_for_test("sp_accounting", "ten_acme");
    ...
    fn action_surfaces_are_canonical() {
        assert_eq!(
            AccountingMutationAction::JournalPost.surface(),
            "accounting.journal.post"
        );
        assert_eq!(
            AccountingMutationAction::PayrollPosting.surface(),
            "accounting.journal.payroll_posting"
        );
        assert_eq!(
            AccountingMutationAction::VatWorkflow.surface(),
            "accounting.journal.vat_workflow"
        );
    }
    ...
    fn action_for_template_only_maps_mutation_routes() {
        assert_eq!(
            action_for_template(crate::ACCOUNTING_JOURNALS_PATH),
            Some(AccountingMutationAction::JournalPost)
        );
        assert_eq!(
            action_for_template(crate::ACCOUNTING_PAYROLL_POSTINGS_PATH),
            Some(AccountingMutationAction::PayrollPosting)
        );
        assert_eq!(
            action_for_template(crate::ACCOUNTING_VAT_WORKFLOW_PLANS_PATH),
            Some(AccountingMutationAction::VatWorkflow)
        );
        assert_eq!(action_for_template(crate::ACCOUNTING_HEALTH_PATH), None);
    }
}
