---
doc_class: Runbook
doc_id: RUNBOOK-FINOPS_PORTAL-AUTO_REBALANCE
...
# Runbook: Finops Portal Auto Rebalance

## Operator Contract
- Runbook id: finops-portal-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-finops-portal-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=finops-portal; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n finops-portal rollout status deploy/finops-portal --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service finops-portal --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service finops-portal --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service finops-portal --cell $CELL --window 30m`.
