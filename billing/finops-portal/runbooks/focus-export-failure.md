...
## Trigger Conditions
- Page on alert `FinopsPortalFocusExportFailureCritical` when `oya_finops_portal_focus_export_failure_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `FinopsPortalFocusExportFailureSloBurn` when `oya_finops_portal_focus_export_failure_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev1 if `oya_finops_portal_focus_export_failure_correctness_ratio < 0.9999` and the affected label set includes `tenant_id`, `cell_id`, or `principal_id`.
- Open a sev1 if `oya_finops_portal_focus_export_failure_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `finops-portal.focus-export-failure.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate finops-portal-focus-export-failure --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `oya_finops_portal_focus_export_failure_allocation_delta_cents` exceeds the threshold documented in `billing/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml`.
...
- Metric pattern: `oya_finops_portal_focus_export_failure_error_ratio` rises before `oya_finops_portal_focus_export_failure_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_finops_portal_focus_export_failure_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_finops_portal_focus_export_failure_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific metric pattern: `oya_finops_portal_focus_export_failure_invoice_render_lag_seconds` rises while `oya_finops_portal_focus_export_failure_recommendation_staleness_seconds` is flat; inspect local worker health before escalating vendors.
- Service-specific metric pattern: `oya_finops_portal_focus_export_failure_recommendation_staleness_seconds` rises while `oya_finops_portal_focus_export_failure_error_ratio` is flat; suspect stale export, stale recommendation, stale projection, or vendor dependency lag.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_finops_portal_focus_export_failure_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_finops_portal_focus_export_failure_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_finops_portal_focus_export_failure_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_finops_portal_focus_export_failure_allocation_delta_cents{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate finops-portal-focus-export-failure --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p oya-cloud-finops-domain focus_export_failure -- --nocapture`.
17. Check API contract smoke: `curl -s https://finops-portal.internal.oyatie.dev/v1/finops/focus-export-failure/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
23. Check SLO files: `ls billing/finops-portal/slos/*.openslo.yaml | sort | rg "tenant|focus"`.
...
25. Run targeted SQL state query: `psql $OYA_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from finops_portal_focus_export_failure_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric oya_finops_portal_focus_export_failure_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_finops_portal_focus_export_failure_queue_depth grow while oya_finops_portal_focus_export_failure_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths billing/finops-portal/runbooks/focus-export-failure.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://finops-portal.internal.oyatie.dev/v1/finops/focus-export-failure/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: tenant bill correctness risk.
  - Required action: keep `finops-portal-focus-export-failure-circuit-breaker` open until `oya_finops_portal_focus_export_failure_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: export or dashboard freshness degradation.
  - Required action: keep `finops-portal-focus-export-failure-circuit-breaker` open until `oya_finops_portal_focus_export_failure_error_ratio` is below 0.01 for 3 evaluator windows.
...
## Resolution Steps
1. Identify code owner path: `rg "focus_export_failure|FinopsPortalFocusExportFailureCritical|finops_portal.focus_export_failure.incident_state" crates microservices/finops-portal -g "!billing/finops-portal/runbooks/**"`.
2. Patch domain invariant: `edit oya-cloud-finops-domain where focus_export_failure state transition is validated`.
...
6. Add regression test: `cargo test -p oya-cloud-finops-domain focus_export_failure_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p oya-dev-cli -- gate validate finops-portal-focus-export-failure --fixture incident-focus-export-failure.json`.
8. Add SLO assertion: `update billing/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml with alert FinopsPortalFocusExportFailureCritical when this was a missing alert`.
9. Add dashboard panel: `update billing/finops-portal/dashboards/tenant-cost-drilldown.grafana.json with oya_finops_portal_focus_export_failure_error_ratio, oya_finops_portal_focus_export_failure_lag_seconds, and oya_finops_portal_focus_export_failure_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_finops_portal_focus_export_failure_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `oya-cloud-finops-domain`: inspect for `focus_export_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-cloud-finops-kernel`: inspect for `focus_export_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-cloud-finops-api`: inspect for `focus_export_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-cloud-billing-kernel`: inspect for `focus_export_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `billing/finops-portal/dashboards/tenant-cost-drilldown.grafana.json`: verify panel coverage for `oya_finops_portal_focus_export_failure_error_ratio`, `oya_finops_portal_focus_export_failure_lag_seconds`, and `oya_finops_portal_focus_export_failure_allocation_delta_cents`.
...
- `oya_finops_portal_focus_export_failure_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `oya_finops_portal_focus_export_failure_lag_seconds < 120` for all production cells.
- `oya_finops_portal_focus_export_failure_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `oya_finops_portal_focus_export_failure_allocation_delta_cents` is below the threshold documented in `billing/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml`.
...
## Handoff Notes
- Do not hand off with only the alert name; include `oya_finops_portal_focus_export_failure_error_ratio`, `oya_finops_portal_focus_export_failure_lag_seconds`, `oya_finops_portal_focus_export_failure_queue_depth`, `oya_finops_portal_focus_export_failure_allocation_delta_cents`, current breaker state, and audit seal status.
...
- `billing/finops-portal/catalog/` for component and owner vocabulary; existing runbook topic `focus-export-failure` was preserved as the scenario anchor.
