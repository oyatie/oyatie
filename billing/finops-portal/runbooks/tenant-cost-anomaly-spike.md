...
## Trigger Conditions
- Page on alert `FinopsPortalTenantCostAnomalySpikeCritical` when `finops_portal_tenant_cost_anomaly_spike_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `FinopsPortalTenantCostAnomalySpikeSloBurn` when `finops_portal_tenant_cost_anomaly_spike_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev2 if `finops_portal_tenant_cost_anomaly_spike_correctness_ratio < 0.9999` and the affected label set includes `tenant_id`, `cell_id`, or `principal_id`.
- Open a sev1 if `finops_portal_tenant_cost_anomaly_spike_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `cargo run -p dev-cli -- gate validate finops-portal-tenant-cost-anomaly-spike --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `finops_portal_tenant_cost_anomaly_spike_allocation_delta_cents` exceeds the threshold documented in `microservices/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml`.
...
- Metric pattern: `finops_portal_tenant_cost_anomaly_spike_error_ratio` rises before `finops_portal_tenant_cost_anomaly_spike_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `finops_portal_tenant_cost_anomaly_spike_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `finops_portal_tenant_cost_anomaly_spike_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific metric pattern: `finops_portal_tenant_cost_anomaly_spike_invoice_render_lag_seconds` rises while `finops_portal_tenant_cost_anomaly_spike_recommendation_staleness_seconds` is flat; inspect local worker health before escalating vendors.
- Service-specific metric pattern: `finops_portal_tenant_cost_anomaly_spike_recommendation_staleness_seconds` rises while `finops_portal_tenant_cost_anomaly_spike_error_ratio` is flat; suspect stale export, stale recommendation, stale projection, or vendor dependency lag.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=finops_portal_tenant_cost_anomaly_spike_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=finops_portal_tenant_cost_anomaly_spike_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=finops_portal_tenant_cost_anomaly_spike_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=finops_portal_tenant_cost_anomaly_spike_allocation_delta_cents{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p dev-cli -- gate validate finops-portal-tenant-cost-anomaly-spike --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p cloud-finops-domain tenant_cost_anomaly_spike -- --nocapture`.
17. Check API contract smoke: `curl -s https://finops-portal.internal.oyatie.dev/v1/finops/tenant-cost-anomaly-spike/incident-handoff -H "x-tenant: $TENANT"`.
...
25. Run targeted SQL state query: `psql $OYATIE_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from finops_portal_tenant_cost_anomaly_spike_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric finops_portal_tenant_cost_anomaly_spike_error_ratio --window 30m --threshold 0.02`.
...
2. Does finops_portal_tenant_cost_anomaly_spike_queue_depth grow while finops_portal_tenant_cost_anomaly_spike_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/finops-portal/runbooks/tenant-cost-anomaly-spike.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://finops-portal.internal.oyatie.dev/v1/finops/tenant-cost-anomaly-spike/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: tenant bill correctness risk.
  - Required action: keep `finops-portal-tenant-cost-anomaly-spike-circuit-breaker` open until `finops_portal_tenant_cost_anomaly_spike_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: export or dashboard freshness degradation.
  - Required action: keep `finops-portal-tenant-cost-anomaly-spike-circuit-breaker` open until `finops_portal_tenant_cost_anomaly_spike_error_ratio` is below 0.01 for 3 evaluator windows.
...
2. Patch domain invariant: `edit cloud-finops-domain where tenant_cost_anomaly_spike state transition is validated`.
...
6. Add regression test: `cargo test -p cloud-finops-domain tenant_cost_anomaly_spike_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate finops-portal-tenant-cost-anomaly-spike --fixture incident-tenant-cost-anomaly-spike.json`.
8. Add SLO assertion: `update microservices/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml with alert FinopsPortalTenantCostAnomalySpikeCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/finops-portal/dashboards/tenant-cost-drilldown.grafana.json with finops_portal_tenant_cost_anomaly_spike_error_ratio, finops_portal_tenant_cost_anomaly_spike_lag_seconds, and finops_portal_tenant_cost_anomaly_spike_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric finops_portal_tenant_cost_anomaly_spike_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `cloud-finops-domain`: inspect for `tenant_cost_anomaly_spike` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `cloud-finops-kernel`: inspect for `tenant_cost_anomaly_spike` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `cloud-finops-api`: inspect for `tenant_cost_anomaly_spike` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `cloud-billing-kernel`: inspect for `tenant_cost_anomaly_spike` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `microservices/finops-portal/dashboards/tenant-cost-drilldown.grafana.json`: verify panel coverage for `finops_portal_tenant_cost_anomaly_spike_error_ratio`, `finops_portal_tenant_cost_anomaly_spike_lag_seconds`, and `finops_portal_tenant_cost_anomaly_spike_allocation_delta_cents`.
...
- `finops_portal_tenant_cost_anomaly_spike_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `finops_portal_tenant_cost_anomaly_spike_lag_seconds < 120` for all production cells.
- `finops_portal_tenant_cost_anomaly_spike_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `finops_portal_tenant_cost_anomaly_spike_allocation_delta_cents` is below the threshold documented in `microservices/finops-portal/slos/tenant-invoice-render-latency.openslo.yaml`.
...
## Handoff Notes
- Do not hand off with only the alert name; include `finops_portal_tenant_cost_anomaly_spike_error_ratio`, `finops_portal_tenant_cost_anomaly_spike_lag_seconds`, `finops_portal_tenant_cost_anomaly_spike_queue_depth`, `finops_portal_tenant_cost_anomaly_spike_allocation_delta_cents`, current breaker state, and audit seal status.
