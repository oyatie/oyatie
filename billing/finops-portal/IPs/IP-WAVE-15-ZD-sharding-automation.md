...
SCOPE-005: finops-portal uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: finops-portal uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: finops-portal.
STANCE-002: Owner team: ops-finops.
...
STANCE-005: Bounded context anchor: tenant-billing-presentation.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_query.
STANCE-008: Compliance pack input: generic, kr, eu, us-healthcare, us-financial, us-public-sector, soc2, iso27001-2022, gdpr, kr-pipa, iso27001, soc2-type2, hipaa, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for finops-portal.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for finops-portal because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; finops-portal must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where finops-portal participates.
...
ROLLBACK-002: For finops-portal, rollback starts from the audit-chain row that recorded the automation event.
