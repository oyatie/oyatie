...
- (ii)  Linus-grade:             FAIL — There is silent-regression risk already visible: `/specs/cross-cutting/root-hub-pointers.json` still points `knowledge_graph_catalog.current_path` at `/registries/cross-cutting/knowledge-graph-catalog.json`, while tracked HEAD has `/registries/cross-cutting/knowledge-graph-semantic.json`, `knowledge-graph-kinetic.json`, and `knowledge-graph-dynamic.json` instead. That is exactly the kind of stale pointer a load-bearing validator should catch.
- (iii) verified-claims:         FAIL — `git ls-files` confirms these cited files exist: `/specs/cross-cutting/active-machine-readable-artifact-contract.json`, `/registries/cross-cutting/artifact-capabilities-registry.json`, `/specs/cross-cutting/artifact-profile-defaults.json`, `/specs/cross-cutting/markdown-retirement-policy.json`, `.omc/ledger/markdown-retirement-ledger.json`, `/specs/cross-cutting/root-hub-pointers.json`, `/specs/cross-cutting/master-plan-sequencing.json`, `/specs/cross-cutting/hyperscaler-gates.json`, `/specs/cross-cutting/evidence-taxonomy.json`, `/specs/cross-cutting/stop-conditions.json`, `/specs/cross-cutting/final-report-schema.json`, `/specs/cross-cutting/plan-schema.json`, `docs/decisions/ADR-0069-active-machine-readable-artifact-contract.md`, and `crates/oya-check-active-artifact-contract/{Cargo.toml,src/lib.rs}`. But `git ls-files` does NOT return `/evidence/audits/consensus/2026-05-13-direction/planner-v1.md`, `/registries/cross-cutting/knowledge-graph-catalog.json`, or the architect-named `semantic-knowledge-graph-catalog.json` / `kinetic-knowledge-graph-catalog.json` / `dynamic-knowledge-graph-catalog.json` paths. Actual tracked split paths are `/registries/cross-cutting/knowledge-graph-semantic.json`, `/registries/cross-cutting/knowledge-graph-kinetic.json`, and `/registries/cross-cutting/knowledge-graph-dynamic.json`.
...
3. Correct all cited paths to tracked HEAD names and fix `/specs/cross-cutting/root-hub-pointers.json` knowledge-graph pointer before claiming verified-claims.
