...
(a) Line 18 frontmatter typo fixed:                       PASS — line 18 now says stale path was `contracts/workflow.openapi.yaml` and canonical path is `contracts/workflow.openapi.yaml`; it also explicitly calls out the v14 self-referential `workflow.openapi.yaml → workflow.openapi.yaml` typo as corrected.
