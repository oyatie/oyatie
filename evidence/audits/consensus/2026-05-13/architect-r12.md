...
2. workflow.openapi.yaml removed:   PASS — `rg workflow` returns no hits; workflow refs use `contracts/workflow.openapi.yaml` at lines 134, 273, 311, 483, 492, 497.
...
3. Line 18: verification_round fix (2) says stale `contracts/workflow.openapi.yaml` was removed and canonical path is the same path. Required fix: change the metadata summary to stale `contracts/workflow.openapi.yaml` removed; body already satisfies this.
