---
doc_class: Runbook
doc_id: RUNBOOK-AUTO_REBALANCE_TRIGGER
...
# Runbook: Auto Rebalance Trigger

## Scenario
- observability detects load skew and the service must create or refuse an auto-rebalance job safely.
...
- Evidence 01: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 02: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 03: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 04: Emit audit-chain row with runbook=auto-rebalance-trigger, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 05: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 06: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 07: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 08: Emit audit-chain row with runbook=auto-rebalance-trigger, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 09: Emit audit-chain row with runbook=auto-rebalance-trigger, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 10: Emit audit-chain row with runbook=auto-rebalance-trigger, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 11: Emit audit-chain row with runbook=auto-rebalance-trigger, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 12: Emit audit-chain row with runbook=auto-rebalance-trigger, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 13: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Queued, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 14: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 15: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 16: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 17: Emit audit-chain row with runbook=auto-rebalance-trigger, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 18: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 19: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 20: Emit audit-chain row with runbook=auto-rebalance-trigger, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 21: Emit audit-chain row with runbook=auto-rebalance-trigger, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 22: Emit audit-chain row with runbook=auto-rebalance-trigger, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 23: Emit audit-chain row with runbook=auto-rebalance-trigger, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 24: Emit audit-chain row with runbook=auto-rebalance-trigger, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 25: Emit audit-chain row with runbook=auto-rebalance-trigger, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
