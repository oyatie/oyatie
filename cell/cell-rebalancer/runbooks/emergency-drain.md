---
doc_class: Runbook
doc_id: RUNBOOK-EMERGENCY_DRAIN
...
## Scenario
- a source cell must be drained because it is degraded, isolated, or being demoted.
...
- Evidence 01: Emit audit-chain row with runbook=emergency-drain, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 02: Emit audit-chain row with runbook=emergency-drain, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 03: Emit audit-chain row with runbook=emergency-drain, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 04: Emit audit-chain row with runbook=emergency-drain, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 05: Emit audit-chain row with runbook=emergency-drain, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 06: Emit audit-chain row with runbook=emergency-drain, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 07: Emit audit-chain row with runbook=emergency-drain, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 08: Emit audit-chain row with runbook=emergency-drain, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 09: Emit audit-chain row with runbook=emergency-drain, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 10: Emit audit-chain row with runbook=emergency-drain, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 11: Emit audit-chain row with runbook=emergency-drain, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 12: Emit audit-chain row with runbook=emergency-drain, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 13: Emit audit-chain row with runbook=emergency-drain, state=Queued, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 14: Emit audit-chain row with runbook=emergency-drain, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 15: Emit audit-chain row with runbook=emergency-drain, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 16: Emit audit-chain row with runbook=emergency-drain, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 17: Emit audit-chain row with runbook=emergency-drain, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 18: Emit audit-chain row with runbook=emergency-drain, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 19: Emit audit-chain row with runbook=emergency-drain, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 20: Emit audit-chain row with runbook=emergency-drain, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 21: Emit audit-chain row with runbook=emergency-drain, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 22: Emit audit-chain row with runbook=emergency-drain, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 23: Emit audit-chain row with runbook=emergency-drain, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 24: Emit audit-chain row with runbook=emergency-drain, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 25: Emit audit-chain row with runbook=emergency-drain, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
