---
doc_class: Runbook
doc_id: RUNBOOK-ROLLBACK_TENANT_MIGRATION
...
# Runbook: Rollback Tenant Migration

## Scenario
- a tenant migration must be reversed or frozen at a safe state.
...
- Evidence 01: Emit audit-chain row with runbook=rollback-tenant-migration, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 02: Emit audit-chain row with runbook=rollback-tenant-migration, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 03: Emit audit-chain row with runbook=rollback-tenant-migration, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 04: Emit audit-chain row with runbook=rollback-tenant-migration, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 05: Emit audit-chain row with runbook=rollback-tenant-migration, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 06: Emit audit-chain row with runbook=rollback-tenant-migration, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 07: Emit audit-chain row with runbook=rollback-tenant-migration, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 08: Emit audit-chain row with runbook=rollback-tenant-migration, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 09: Emit audit-chain row with runbook=rollback-tenant-migration, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 10: Emit audit-chain row with runbook=rollback-tenant-migration, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 11: Emit audit-chain row with runbook=rollback-tenant-migration, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 12: Emit audit-chain row with runbook=rollback-tenant-migration, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 13: Emit audit-chain row with runbook=rollback-tenant-migration, state=Queued, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 14: Emit audit-chain row with runbook=rollback-tenant-migration, state=Validated, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 15: Emit audit-chain row with runbook=rollback-tenant-migration, state=Migrating, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 16: Emit audit-chain row with runbook=rollback-tenant-migration, state=Succeeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 17: Emit audit-chain row with runbook=rollback-tenant-migration, state=PartiallySucceeded, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 18: Emit audit-chain row with runbook=rollback-tenant-migration, state=Aborted, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 19: Emit audit-chain row with runbook=rollback-tenant-migration, state=Failed, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 20: Emit audit-chain row with runbook=rollback-tenant-migration, state=Pending, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 21: Emit audit-chain row with runbook=rollback-tenant-migration, state=SourceQuiesce, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 22: Emit audit-chain row with runbook=rollback-tenant-migration, state=DataTransfer, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 23: Emit audit-chain row with runbook=rollback-tenant-migration, state=TargetActivate, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 24: Emit audit-chain row with runbook=rollback-tenant-migration, state=CutoverComplete, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
- Evidence 25: Emit audit-chain row with runbook=rollback-tenant-migration, state=RolledBack, operator principal, Cedar decision id, HLC timestamp, source cell, target cell, and rollback pointer.
