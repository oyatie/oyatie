...
//!
//! ADR-0010 authorizes `regional-packs/<pack-id>/` as the shared/versioned
//! pack-authoring root, while accepted ADR-0064 requires canonical-base
//! neutrality and pack isolation. This module is deliberately fixture-scoped:
//! it validates one declared regional-pack manifest and the paths that manifest
//! names, without claiming repo-wide production enforcement.
...
pub fn load_regional_pack_manifest(
    ...
    let manifest_path = regional_pack_manifest_path(repo_root, pack_code);
    ...
    let manifest: RegionalPackManifest = serde_yaml::from_str(&manifest_text).map_err(|error| {
        RegionalPackManifestError::ParseFailed {
            path: manifest_path.clone(),
            reason: error.to_string(),
        }
    })?;
    validate_manifest_shape(&manifest, pack_code)?;
    Ok(manifest)
}

pub fn validate_regional_pack_manifest(
    repo_root: impl AsRef<Path>,
    pack_code: &str,
) -> Result<RegionalPackManifestGateReport, RegionalPackManifestError> {
    let repo_root = repo_root.as_ref();
    let manifest = load_regional_pack_manifest(repo_root, pack_code)?;
    let checks = vec![
        canonical_base_neutrality_check(repo_root, &manifest)?,
        cross_pack_refusal_check(repo_root, &manifest)?,
    ];

    Ok(RegionalPackManifestGateReport {
        pack_id: manifest.pack.id,
        pack_code: manifest.pack.code,
        manifest_path: regional_pack_manifest_path(repo_root, pack_code),
        checks,
    })
}

fn regional_pack_manifest_path(repo_root: &Path, pack_code: &str) -> PathBuf {
    repo_root
        .join("regional-packs")
        .join(pack_code)
        .join("pack.yaml")
...
fn validate_manifest_shape(
    ...
    if manifest.pack.code != expected_pack_code {
        return shape_error(
            "pack.code",
            format!("must match regional-packs/{expected_pack_code}/ path"),
        );
    }
    if manifest.pack.id != format!("oya-pack-{expected_pack_code}") {
        return shape_error("pack.id", format!("must be oya-pack-{expected_pack_code}"));
    }
    if !matches!(
    ...
    if manifest.regional_pack.jurisdiction != expected_pack_code.to_ascii_uppercase() {
        return shape_error(
            "regional_pack.jurisdiction",
            format!("must be {}", expected_pack_code.to_ascii_uppercase()),
        );
    }
    require_non_empty(
...
fn canonical_base_neutrality_check(
    repo_root: &Path,
    manifest: &RegionalPackManifest,
) -> Result<GateCheck, RegionalPackManifestError> {
    let mut violations = Vec::new();
    let markers = forbidden_canonical_base_markers(manifest);

    for declared_path in &manifest.canonical_base.canonical_base_paths {
        let path = repo_root.join(declared_path);
        if !path.exists() {
            violations.push(GateViolation::new(
                declared_path,
                None,
                "declared canonical base path is missing",
            ));
            continue;
        }
        for file in collect_text_files(&path)? {
            let content = match fs::read_to_string(&file) {
                Ok(content) => content,
                Err(error) => {
                    violations.push(GateViolation::new(
                        display_repo_path(repo_root, &file),
                        None,
                        format!("cannot read canonical base file: {error}"),
                    ));
                    continue;
                }
            };
    ...
    Ok(GateCheck::from_violations(
        CANONICAL_BASE_NEUTRALITY_GATE,
        violations,
    ))
}

fn cross_pack_refusal_check(
    repo_root: &Path,
    manifest: &RegionalPackManifest,
) -> Result<GateCheck, RegionalPackManifestError> {
    let mut violations = Vec::new();
    if !manifest.pack_impl.allowed_pack_dependencies.is_empty() {
        violations.push(GateViolation::new(
            "regional-packs/{pack-code}/pack.yaml",
            None,
            "KR fixture must not allow pack-to-pack dependencies",
        ));
    }

    for declared_path in &manifest.pack_impl.pack_paths {
        let path = repo_root.join(declared_path);
        if !path.exists() {
            violations.push(GateViolation::new(
                declared_path,
                None,
                "declared pack implementation path is missing",
            ));
            continue;
        }
        for file in collect_text_files(&path)? {
            let content = match fs::read_to_string(&file) {
                ...
                Err(error) => {
                    violations.push(GateViolation::new(
                        display_repo_path(repo_root, &file),
                        None,
                        format!("cannot read pack implementation file: {error}"),
                    ));
                    continue;
                }
            };
            for (line_index, line) in content.lines().enumerate() {
                for other_pack in referenced_other_packs(line, manifest) {
                    violations.push(GateViolation::new(
                        display_repo_path(repo_root, &file),
                        Some(line_index + 1),
                        format!(
...
fn referenced_other_packs(line: &str, manifest: &RegionalPackManifest) -> Vec<String> {
    let mut refs = Vec::new();
    refs.extend(
        extract_prefixed_tokens(line, "oya-pack-")
            .into_iter()
            .filter(|pack_id| pack_id != &manifest.pack.id),
    );

    let regional_pack_prefix = "regional-packs/";
    for (_, tail) in line.match_indices(regional_pack_prefix) {
        let after_prefix = &tail[regional_pack_prefix.len()..];
        let code = after_prefix
            .split(|ch: char| !(ch.is_ascii_alphanumeric() || ch == '-'))
            .next()
            .unwrap_or_default();
        if !code.is_empty() && code != manifest.pack.code {
            refs.push(format!("regional-packs/{code}"));
        }
    }
...
fn extract_prefixed_tokens(line: &str, prefix: &str) -> Vec<String> {
...
fn collect_text_files(path: &Path) -> Result<Vec<PathBuf>, RegionalPackManifestError> {
    if path.is_file() {
        return Ok(if is_scanned_text_file(path) {
    ...
    for entry in WalkDir::new(path).follow_links(false) {
        let entry = entry.map_err(|error| RegionalPackManifestError::WalkFailed {
            path: path.to_path_buf(),
            reason: error.to_string(),
        })?;
        if entry.file_type().is_file() && is_scanned_text_file(entry.path()) {
