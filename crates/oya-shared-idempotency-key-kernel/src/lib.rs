...
//!
//! `oya-shared-idempotency-key-kernel` follows BNF v4.1:
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            IdempotencyStoreError::KeyTooShort => write!(
                f,
                "oya-shared-idempotency-key-kernel: idempotency key shorter than 16 bytes"
            ),
            ...
            IdempotencyStoreError::SkeletonNotYetImplemented(method) => write!(
                f,
                "oya-shared-idempotency-key-kernel: {method} is skeleton-only \
