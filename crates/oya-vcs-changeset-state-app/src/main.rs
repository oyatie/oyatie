//! `oya-vcs-changeset-state-app` — composition root for the
...
//!
//! ```text
//! oya-vcs-changeset-state-app append \
//!     --changeset <id> \
//!     --to-state <state> \
//!     [--from-state <state>] \
//!     [--emitted-by <agent>] \
...
const DEFAULT_EMITTER: &str = "oya-vcs-changeset-state-app";
...
const USAGE: &str = "Usage: oya-vcs-changeset-state-app append \
...
fn main() -> ExitCode {
    ...
    match run(argv) {
        ...
        Err(error) => {
            eprintln!("oya-vcs-changeset-state-app: {error}");
            ExitCode::FAILURE
        }
    }
}
