//! Enterprise Suite statutory filing evidence contract foundation.
//!
...
const PLAN_NAME: &str = "enterprise-suite-statutory-filing-evidence-plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseStatutoryFilingEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseStatutoryFilingEvidenceError {
    InvalidPlanName,
    InvalidServiceName,
    MissingRequirements,
...
pub fn enterprise_suite_statutory_filing_evidence_plan() -> EnterpriseStatutoryFilingEvidencePlan {
    EnterpriseStatutoryFilingEvidencePlan {
        plan_name: PLAN_NAME,
...
pub fn statutory_authority_urls(plan: &EnterpriseStatutoryFilingEvidencePlan) -> Vec<&'static str> {
    plan.requirements
        .iter()
...
pub fn validate_enterprise_suite_statutory_filing_evidence_plan(
    plan: &EnterpriseStatutoryFilingEvidencePlan,
) -> Result<(), EnterpriseStatutoryFilingEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseStatutoryFilingEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterpriseStatutoryFilingEvidenceError::InvalidServiceName);
    }
    if plan.requirements.len() < MIN_REQUIREMENT_COUNT {
        return Err(EnterpriseStatutoryFilingEvidenceError::MissingRequirements);
    }
    ...
    for jurisdiction in [
        ...
        if !jurisdictions.contains(&jurisdiction) {
            return Err(EnterpriseStatutoryFilingEvidenceError::MissingJurisdiction(
                jurisdiction,
            ));
        }
    }
    for rail_kind in [
        ...
        if !rail_kinds.contains(&rail_kind) {
            return Err(EnterpriseStatutoryFilingEvidenceError::MissingRailKind(
                rail_kind,
            ));
        }
    }
    ...
    {
        return Err(EnterpriseStatutoryFilingEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseStatutoryFilingEvidenceError::InvalidRequirementId,
    )?;
    validate_label(
        requirement.authority_name,
        EnterpriseStatutoryFilingEvidenceError::InvalidAuthorityName,
    )?;
    ...
    validate_prefixed_ref(
        ...
        EnterpriseStatutoryFilingEvidenceError::InvalidRulepackEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.payload_schema_ref,
        "schemas/statutory-filing/",
        EnterpriseStatutoryFilingEvidenceError::InvalidPayloadSchemaRef,
    )?;
    validate_prefixed_ref(
        requirement.payload_digest_ref,
        "evidence/statutory/",
        EnterpriseStatutoryFilingEvidenceError::InvalidPayloadDigestRef,
    )?;
    validate_prefixed_ref(
        requirement.submission_window_ref,
        "calendars/statutory/",
        EnterpriseStatutoryFilingEvidenceError::InvalidSubmissionWindowRef,
    )?;
    validate_prefixed_ref(
        requirement.required_receipt_schema_ref,
        "schemas/statutory-filing/",
        EnterpriseStatutoryFilingEvidenceError::InvalidReceiptSchemaRef,
    )?;
    validate_prefixed_ref(
        requirement.rollback_evidence_ref,
        "rollback/statutory/",
        EnterpriseStatutoryFilingEvidenceError::InvalidRollbackEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.credential_boundary_ref,
        "secrets-boundary/statutory/",
        EnterpriseStatutoryFilingEvidenceError::InvalidCredentialBoundaryRef,
    )?;
    require_control(
...
fn validate_authority_url(url: &str) -> Result<(), EnterpriseStatutoryFilingEvidenceError> {
    ...
    {
        return Err(EnterpriseStatutoryFilingEvidenceError::InvalidOfficialAuthorityUrl);
    }
    Ok(())
}
...
fn validate_label(
    value: &str,
    error: EnterpriseStatutoryFilingEvidenceError,
) -> Result<(), EnterpriseStatutoryFilingEvidenceError> {
...
fn require_control(
    ...
    if value {
        ...
        Err(EnterpriseStatutoryFilingEvidenceError::MissingRequiredControl(control))
    }
}
