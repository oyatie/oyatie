...
//! All CI tests use `oya_shared_olap_client_kernel::memory_adapter::InMemoryOlapClient`.
//!
...
mod tests {
    ...
    fn unimplemented_surfaced_honestly_on_query() {
        ...
        let table = oya_shared_olap_client_kernel::QualifiedTable::new(
            tid.clone(),
            oya_shared_olap_client_kernel::TableName::try_new("events").unwrap(),
        );
        let q = Query {
            source: table,
