...
use oya_enterprise_suite_local_runtime_composition::{
    enterprise_local_runtime_composition, validate_unique_method_paths,
};

#[test]
fn enterprise_local_runtime_composition_catalogs_child_routes_without_listener_claim() {
    let composition = enterprise_local_runtime_composition();

    assert_eq!(composition.name, "enterprise-local-runtime-composition");
    ...
    assert!(!composition.child_network_calls_attached);
    ...
    assert!(composition.routes.iter().any(|route| {
        route.service == "enterprise-suite"
            && route.method == "POST"
            && route.path == "/enterprise-suite/v1/policy-admissions"
    }));
}

#[test]
fn enterprise_local_runtime_composition_rejects_duplicate_method_paths() {
    let mut composition = enterprise_local_runtime_composition();
