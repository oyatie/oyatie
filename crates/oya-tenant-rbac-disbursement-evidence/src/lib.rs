//! Tenant RBAC disbursement evidence contract foundation.
//!
...
const PLAN_NAME: &str = "tenant-rbac-disbursement-evidence-plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TenantRbacDisbursementEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TenantRbacDisbursementEvidenceError {
    InvalidPlanName,
    InvalidServiceName,
    MissingRequirements,
...
pub fn tenant_rbac_disbursement_evidence_plan() -> TenantRbacDisbursementEvidencePlan {
    TenantRbacDisbursementEvidencePlan {
        plan_name: PLAN_NAME,
...
pub fn disbursement_network_or_authority_urls(
    plan: &TenantRbacDisbursementEvidencePlan,
) -> Vec<&'static str> {
    plan.requirements
        .iter()
...
pub fn validate_tenant_rbac_disbursement_evidence_plan(
    plan: &TenantRbacDisbursementEvidencePlan,
) -> Result<(), TenantRbacDisbursementEvidenceError> {
    validate_slug(
        plan.plan_name,
        TenantRbacDisbursementEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(TenantRbacDisbursementEvidenceError::InvalidServiceName);
    }
    if plan.requirements.len() < MIN_REQUIREMENT_COUNT {
        return Err(TenantRbacDisbursementEvidenceError::MissingRequirements);
    }
    ...
    for requirement in &plan.requirements {
        ...
        if !seen.insert(requirement.requirement_id) {
            return Err(TenantRbacDisbursementEvidenceError::DuplicateRequirement(
    ...
    for jurisdiction in [
        ...
        if !jurisdictions.contains(&jurisdiction) {
            return Err(TenantRbacDisbursementEvidenceError::MissingJurisdiction(
                jurisdiction,
            ));
        }
    }
    for rail_kind in [
        ...
        if !rail_kinds.contains(&rail_kind) {
            return Err(TenantRbacDisbursementEvidenceError::MissingRailKind(
                rail_kind,
            ));
        }
    }
    ...
    {
        return Err(TenantRbacDisbursementEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        TenantRbacDisbursementEvidenceError::InvalidRequirementId,
    )?;
    validate_label(
        requirement.network_or_authority_name,
        TenantRbacDisbursementEvidenceError::InvalidNetworkOrAuthorityName,
    )?;
    ...
    validate_prefixed_ref(
        ...
        TenantRbacDisbursementEvidenceError::InvalidSourceEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.payment_file_schema_ref,
        "schemas/disbursement/",
        TenantRbacDisbursementEvidenceError::InvalidPaymentFileSchemaRef,
    )?;
    validate_prefixed_ref(
        requirement.payment_digest_ref,
        "evidence/disbursement/",
        TenantRbacDisbursementEvidenceError::InvalidPaymentDigestRef,
    )?;
    validate_prefixed_ref(
        requirement.beneficiary_account_tokenization_ref,
        "privacy-boundary/disbursement/",
        TenantRbacDisbursementEvidenceError::InvalidBeneficiaryAccountTokenizationRef,
    )?;
    validate_prefixed_ref(
        requirement.approval_workflow_ref,
        "workflow/disbursement/",
        TenantRbacDisbursementEvidenceError::InvalidApprovalWorkflowRef,
    )?;
    validate_prefixed_ref(
        requirement.reconciliation_receipt_schema_ref,
        "schemas/disbursement-reconciliation/",
        TenantRbacDisbursementEvidenceError::InvalidReconciliationReceiptSchemaRef,
    )?;
    validate_prefixed_ref(
        requirement.rollback_or_reversal_runbook_ref,
        "rollback/disbursement/",
        TenantRbacDisbursementEvidenceError::InvalidRollbackOrReversalRunbookRef,
    )?;
    require_control(
...
fn validate_network_or_authority_url(url: &str) -> Result<(), TenantRbacDisbursementEvidenceError> {
    ...
    {
        return Err(TenantRbacDisbursementEvidenceError::InvalidOfficialNetworkOrAuthorityUrl);
    }
    Ok(())
}
...
fn validate_label(
    value: &str,
    error: TenantRbacDisbursementEvidenceError,
) -> Result<(), TenantRbacDisbursementEvidenceError> {
