...
//!
//! - Crate `oya-foundry-fitness-adapter-with-no-importer-kernel` —
...
/// check ignores port-declaring siblings (e.g. `oya-foundry-adapter-anthropic-api-kernel`).
...
///
/// Excludes port-declaring siblings like `oya-foundry-adapter-anthropic-api-kernel`
/// (last segment = `kernel`, not adapter) and the fitness kernel itself
/// (`oya-foundry-fitness-adapter-with-no-importer-kernel` — last segment
/// `kernel`).
...
///
/// Examples:
/// - `oya-foundry-account-adapter-inmemory` -> `oya-foundry-account`
/// - `oya-cloud-billing-adapter-aws` -> `oya-cloud-billing`
/// - `oya-foundry-claude-account-adapter` -> `oya-foundry-claude-account`
...
mod tests {
    ...
    fn accepts_adapter_with_matching_importer() {
        let report = check(&[
            krate("oya-foundry-claude-account-adapter"),
    ...
    fn variant_suffixed_adapter_matches_base_importer() {
        let report = check(&[
            krate("oya-foundry-account-adapter-inmemory"),
    ...
    fn ignores_adapter_kernel_crates() {
        ...
        let report = check(&[krate("oya-foundry-adapter-anthropic-api-kernel")]);
        assert!(report.is_clean());
    ...
    fn ignores_non_adapter_crates() {
        let report = check(&[
            krate("oya-foundry-supervisor-kernel"),
            krate("oya-foundry-supervisor-app"),
        ]);
        assert!(report.is_clean());
        assert_eq!(report.adapters_checked, 0);
