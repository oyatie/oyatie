//! Enterprise Suite authentication and authorization runtime foundation.
//!
//! This crate evaluates tenant-bound, scope-bound access decisions for the
//! Enterprise Suite local route catalog. It deliberately does not validate OIDC
...
pub struct EnterpriseAuthRoutePolicy {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseAuthRuntimePolicy {
    ...
    pub route_policies: Vec<EnterpriseAuthRoutePolicy>, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseAuthDecision {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseAuthRuntimeError {
    InvalidPolicy,
    InvalidRoutePolicy,
...
pub fn enterprise_auth_runtime_policy() -> EnterpriseAuthRuntimePolicy {
    EnterpriseAuthRuntimePolicy {
        policy_name: "enterprise-suite-auth-runtime",
        expected_issuer: "https://identity.oyatie.dev/enterprise-suite",
        expected_audience: "enterprise-suite-api",
        replay_nonce: "enterprise-suite-local-rehearsal-nonce",
        max_clock_skew_seconds: MAX_CLOCK_SKEW_SECONDS,
        max_session_age_seconds: MAX_SESSION_AGE_SECONDS,
        route_policies: enterprise_auth_route_policies(),
...
pub fn enterprise_auth_route_policies() -> Vec<EnterpriseAuthRoutePolicy> {
    vec![
        ...
        sensitive_route_policy(
            ...
            "enterprise-suite:group-close:rollup",
        ),
        route_policy(
            "POST",
            "/enterprise-suite/v1/cross-product-workflow-plans",
            "enterprise-suite:workflow:plan",
            false,
        ),
        route_policy(
            "POST",
            "/enterprise-suite/v1/incident-rollback-plans",
            "enterprise-suite:incident-rollback:plan",
            false,
        ),
        route_policy(
            "POST",
            "/enterprise-suite/v1/ops-commands",
            "enterprise-suite:ops-command:prepare",
            false,
        ),
        route_policy(
            "GET",
            "/enterprise-suite/v1/healthz",
            "enterprise-suite:health:read",
            false,
        ),
    ]
}

pub fn validate_enterprise_auth_runtime_policy(
    policy: &EnterpriseAuthRuntimePolicy,
) -> Result<(), EnterpriseAuthRuntimeError> {
    ...
    {
        return Err(EnterpriseAuthRuntimeError::InvalidPolicy);
    }
    ...
    {
        return Err(EnterpriseAuthRuntimeError::RuntimeAttachmentOverclaim);
    }
    ...
    for route in &policy.route_policies {
        ...
        if !seen.insert(key.clone()) {
            return Err(EnterpriseAuthRuntimeError::DuplicateRoutePolicy(key));
        }
    }
    Ok(())
}

pub fn authorize_enterprise_route(
    policy: &EnterpriseAuthRuntimePolicy,
    principal: &AuthenticatedPrincipal,
    request: &EnterpriseAuthRequest,
) -> Result<EnterpriseAuthDecision, EnterpriseAuthRuntimeError> {
    ...
    let route = policy
        ...
        .ok_or(EnterpriseAuthRuntimeError::RouteNotRegistered)?;

    if !principal.scopes.contains(&route.required_scope) {
        return Err(EnterpriseAuthRuntimeError::MissingScope);
    }
    if principal.assurance_level < route.min_assurance {
        return Err(EnterpriseAuthRuntimeError::AssuranceTooLow);
    }
    if route.mfa_required && !principal.mfa_present {
        return Err(EnterpriseAuthRuntimeError::MfaRequired);
    }
    if principal.break_glass && !route.break_glass_allowed {
        return Err(EnterpriseAuthRuntimeError::BreakGlassNotAllowed);
    }

    Ok(EnterpriseAuthDecision {
        authorized: true,
...
fn validate_route_policy(
    route: &EnterpriseAuthRoutePolicy,
) -> Result<(), EnterpriseAuthRuntimeError> {
    ...
    {
        return Err(EnterpriseAuthRuntimeError::InvalidRoutePolicy);
    }
    Ok(())
}
...
fn validate_principal(
    ...
    {
        return Err(EnterpriseAuthRuntimeError::InvalidPrincipal);
    }
    Ok(())
}

fn validate_request(request: &EnterpriseAuthRequest) -> Result<(), EnterpriseAuthRuntimeError> {
    if !valid_method(request.method)
        || !valid_path(request.path)
        || !valid_ref(request.target_tenant_id)
        || request.now_unix == 0
    {
        return Err(EnterpriseAuthRuntimeError::InvalidRequest);
    }
    Ok(())
}
...
fn validate_claims(
    ...
    if principal.issuer != policy.expected_issuer {
        return Err(EnterpriseAuthRuntimeError::InvalidIssuer);
    }
    if principal.audience != policy.expected_audience {
        return Err(EnterpriseAuthRuntimeError::InvalidAudience);
    }
    if principal.nonce != policy.replay_nonce {
        return Err(EnterpriseAuthRuntimeError::NonceMismatch);
    }
    if principal.tenant_id != request.target_tenant_id {
        return Err(EnterpriseAuthRuntimeError::TenantIsolationRequired);
    }
    if request
        .now_unix
    ...
    {
        return Err(EnterpriseAuthRuntimeError::IssuedInFuture);
    }
    if request.now_unix
        > principal
            .expires_at_unix
            .saturating_add(policy.max_clock_skew_seconds)
    {
        return Err(EnterpriseAuthRuntimeError::SessionExpired);
    }
    if request.now_unix.saturating_sub(principal.issued_at_unix) > policy.max_session_age_seconds {
        return Err(EnterpriseAuthRuntimeError::SessionTooOld);
    }
    Ok(())
}
