//! M02-P01 — openai subscription-auth adapter.
...
use oya_foundry_adapter_openai_subscription_kernel::{
...
impl OpenAiSubscriptionAdapter {
    ...
    fn synthesize_token_id(&self, sref: &SecretReference) -> String {
        ...
        let dbg = format!("{sref:?}");
        format!("mock-openai-subscription-{}", dbg.len())
    }
}

impl ProviderAuthPort for OpenAiSubscriptionAdapter {
...
mod tests {
    ...
    use oya_foundry_adapter_openai_subscription_kernel::{AUTH_MODE, AuthMode};
    ...
    fn adapter_authenticates_and_returns_token() {
        let a = OpenAiSubscriptionAdapter::new();
    ...
    fn adapter_clock_override_respected() {
        let a = OpenAiSubscriptionAdapter::new().with_clock(5_000, 60);
