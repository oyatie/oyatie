use oya_tenant_rbac_statutory_filing_evidence::{
    StatutoryFilingJurisdiction, StatutoryFilingRailKind, TenantRbacStatutoryFilingEvidenceError,
    statutory_authority_urls, tenant_rbac_statutory_filing_evidence_plan,
    validate_tenant_rbac_statutory_filing_evidence_plan,
};

#[test]
fn statutory_filing_evidence_plan_validates_controls_and_nonclaims() {
    let plan = tenant_rbac_statutory_filing_evidence_plan();
    validate_tenant_rbac_statutory_filing_evidence_plan(&plan)
        .expect("statutory filing evidence plan validates");

    assert_eq!(plan.plan_name, "tenant-rbac-statutory-filing-evidence-plan");
...
fn statutory_filing_evidence_plan_rejects_missing_requirements_duplicate_and_authority_drift() {
    let mut plan = tenant_rbac_statutory_filing_evidence_plan();
    plan.requirements.truncate(1);
    assert_eq!(
        validate_tenant_rbac_statutory_filing_evidence_plan(&plan),
        Err(TenantRbacStatutoryFilingEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_tenant_rbac_statutory_filing_evidence_plan(&plan),
        Err(TenantRbacStatutoryFilingEvidenceError::InvalidOfficialAuthorityUrl)
    );
}
...
fn statutory_filing_evidence_plan_rejects_unsafe_refs_and_runtime_overclaims() {
    ...
    assert_eq!(
        validate_tenant_rbac_statutory_filing_evidence_plan(&plan),
        Err(TenantRbacStatutoryFilingEvidenceError::InvalidPayloadDigestRef)
    );
    ...
    assert_eq!(
        validate_tenant_rbac_statutory_filing_evidence_plan(&plan),
        Err(
            TenantRbacStatutoryFilingEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_tenant_rbac_statutory_filing_evidence_plan(&plan),
        Err(TenantRbacStatutoryFilingEvidenceError::RuntimeAttachmentOverclaim)
    );
}
