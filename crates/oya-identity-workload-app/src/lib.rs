...
//! - [`oya_identity_workload_domain`] — the pure principal + lifecycle +
//!   PARC-decision kernel.
//! - [`oya_identity_workload_oidc_adapter`] — `ring`-backed JWS validation that
//!   yields a verified [`WorkloadPrincipal`].
//! - [`oya_identity_workload_authz_cedar_adapter`] — the real `cedar-policy`
