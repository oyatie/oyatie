...
use oya_enterprise_suite_tenant_workload_manifest::Fd001TenantWorkloadKind;
...
fn tenant_workload_identity_contract_validates_controls_and_nonclaims() {
    ...
    assert_eq!(
        ...
        "fd001-enterprise-tenant-workload-identity-contract"
    );
    assert_eq!(contract.program_name, "fd-001-enterprise-generic");
    assert_eq!(contract.substrate_name, "oyatie-cloud");
    assert_eq!(contract.tenant_namespace, "oyatie-fd001-enterprise-dev");
    assert_eq!(contract.trust_domain, "oyatie.dev");
    assert_eq!(
        contract.spiffe_id_prefix,
        "spiffe://oyatie.dev/fd001-enterprise/"
    );
    assert_eq!(contract.workload_manifest_count, 4);
    assert_eq!(
        contract.tenant_admission_policy_contract_name,
        "fd001-enterprise-tenant-admission-policy-contract"
    );
    ...
    assert!(contract.requirements.iter().all(|requirement| {
        ...
        .contains(&requirement.resource_kind)
            ...
            && requirement
                ...
                .starts_with("evidence/workload-identity/fd001-enterprise/")
            && requirement
                .source_manifest_ref
                .starts_with("crates/oya-enterprise-suite-tenant-workload-manifest/")
            && requirement
                .source_admission_policy_ref
                .starts_with("crates/oya-enterprise-suite-tenant-admission-policy/")
            && requirement
                .spiffe_id
                .starts_with("spiffe://oyatie.dev/fd001-enterprise/")
