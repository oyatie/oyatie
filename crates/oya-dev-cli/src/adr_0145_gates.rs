...
fn collect_manifests(root: &Path) -> Vec<ManifestEntry> {
    let mut out = Vec::new();
    let Ok(entries) = fs::read_dir(root) else {
        return out;
    };
...
mod tests {
    ...
    #[test]
    fn collect_manifests_on_nonexistent_root_returns_empty() {
        let manifests = collect_manifests(Path::new("/nonexistent/path"));
