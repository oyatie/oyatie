...
fn topo_sort(adrs: &[PlanningAdr]) -> Vec<PlanningAdr> {
    ...
    let mut remaining: BTreeMap<String, PlanningAdr> =
        adrs.iter().map(|adr| (adr.id.clone(), adr.clone())).collect();
...
fn projection_to_json(projection: &MasterplanProjection) -> serde_json::Value {
    ...
    root.insert("generator".into(), Value::String("oya gen masterplan".into()));
