use oya_enterprise_suite_tenant_workload_runtime_evidence::{
    ...
    TenantWorkloadRuntimeEvidenceRequirementKind,
    enterprise_suite_tenant_workload_runtime_evidence_plan,
    tenant_workload_runtime_evidence_doc_urls,
...
fn tenant_workload_runtime_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_tenant_workload_runtime_evidence_plan().expect("plan builds");
    validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan).expect("plan validates");

    assert_eq!(
        plan.plan_name,
        "fd001-tenant-workload-runtime-evidence-plan"
    );
    assert_eq!(plan.program_name, "fd-001-enterprise-generic");
    ...
    assert_eq!(plan.otel_service_namespace, "fd001-enterprise");
    assert_eq!(plan.tenant_claim, "tenant_id");
    assert_eq!(
        plan.tenant_workload_manifest_name,
        "fd001-enterprise-tenant-workload-manifest"
    );
...
fn tenant_workload_runtime_evidence_plan_preserves_ref_boundaries_and_source_contract() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            .starts_with("evidence/tenant-workload-runtime/fd001-enterprise/")
            && requirement
                .source_manifest_ref
                .starts_with("crates/oya-enterprise-suite-tenant-workload-manifest/")
...
fn tenant_workload_runtime_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = enterprise_suite_tenant_workload_runtime_evidence_plan().expect("plan builds");
    plan.requirements.truncate(4);
    assert_eq!(
        validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidOfficialDocUrl)
    );
}
...
fn tenant_workload_runtime_evidence_plan_rejects_unsafe_refs_missing_controls_and_overclaims() {
    ...
    plan.requirements[0].expected_evidence_ref =
        "evidence/tenant-workload-runtime/fd001-enterprise/client_secret";
    assert_eq!(
        validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidExpectedEvidenceRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan),
        Err(
            EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_suite_tenant_workload_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::RuntimeAttachmentOverclaim)
    );
}
