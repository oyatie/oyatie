...
//! exist before FD-001 Enterprise Suite, HR, Payroll, and Accounting services
...
const PROGRAM_NAME: &str = "fd-001-enterprise-generic";
...
const OTEL_SERVICE_NAMESPACE: &str = "fd001-enterprise";
const TENANT_CLAIM: &str = "tenant_id";
const SOURCE_MANIFEST_REF: &str = "crates/oya-enterprise-suite-tenant-workload-manifest/src/lib.rs::fd001_tenant_workload_manifest";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteTenantWorkloadRuntimeEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteTenantWorkloadRuntimeEvidenceError {
...
pub fn enterprise_suite_tenant_workload_runtime_evidence_plan() -> Result<
    EnterpriseSuiteTenantWorkloadRuntimeEvidencePlan,
...
> {
    ...
    validate_fd001_tenant_workload_manifest(&manifest)
        .map_err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::TenantWorkloadManifest)?;

    Ok(EnterpriseSuiteTenantWorkloadRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
...
pub fn validate_enterprise_suite_tenant_workload_runtime_evidence_plan(
    plan: &EnterpriseSuiteTenantWorkloadRuntimeEvidencePlan,
) -> Result<(), EnterpriseSuiteTenantWorkloadRuntimeEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.program_name != PROGRAM_NAME {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidProgramName);
    }
    if plan.substrate_name != SUBSTRATE_NAME {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidSubstrateName);
    }
    if plan.tenant_namespace != TENANT_NAMESPACE {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidTenantNamespace);
    }
    if plan.tenant_cell_id != TENANT_CELL_ID {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidTenantCellId);
    }
    if plan.residency_region != RESIDENCY_REGION {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidResidencyRegion);
    }
    if plan.otel_service_namespace != OTEL_SERVICE_NAMESPACE {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidOtelServiceNamespace);
    }
    if plan.tenant_claim != TENANT_CLAIM {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidTenantClaim);
    }
    if plan.tenant_workload_manifest_name != "fd001-enterprise-tenant-workload-manifest" {
        return Err(
            EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidTenantWorkloadManifestName,
        );
    }
    ...
    {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::MissingRequirements);
    }
...
fn runtime_requirements() -> Vec<TenantWorkloadRuntimeEvidenceRequirement> {
    vec![
        requirement(
            ...
            "evidence/tenant-workload-runtime/fd001-enterprise/namespace-observed.json",
        ),
        requirement(
            "resource-quota-usage-observed",
            TenantWorkloadRuntimeEvidenceRequirementKind::ResourceQuotaUsageObserved,
            "https://kubernetes.io/docs/concepts/policy/resource-quotas/",
            "evidence/tenant-workload-runtime/fd001-enterprise/resource-quota-usage.json",
        ),
        requirement(
            ...
            "evidence/tenant-workload-runtime/fd001-enterprise/network-policy-default-deny.json",
        ),
        requirement(
            "service-account-boundary-observed",
            TenantWorkloadRuntimeEvidenceRequirementKind::ServiceAccountBoundaryObserved,
            "https://kubernetes.io/docs/concepts/security/service-accounts/",
            "evidence/tenant-workload-runtime/fd001-enterprise/service-account-boundary.json",
        ),
        requirement(
            "pod-security-context-observed",
            TenantWorkloadRuntimeEvidenceRequirementKind::PodSecurityContextObserved,
            "https://kubernetes.io/docs/concepts/security/pod-security-standards/",
            "evidence/tenant-workload-runtime/fd001-enterprise/pod-security-context.json",
        ),
        requirement(
            "workload-scheduled",
            TenantWorkloadRuntimeEvidenceRequirementKind::WorkloadScheduled,
            "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/",
            "evidence/tenant-workload-runtime/fd001-enterprise/workload-scheduled.json",
        ),
        requirement(
            ...
            "evidence/tenant-workload-runtime/fd001-enterprise/resource-requests-limits.json",
        ),
        requirement(
            "readiness-probe-observed",
            TenantWorkloadRuntimeEvidenceRequirementKind::ReadinessProbeObserved,
            "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#container-probes",
            "evidence/tenant-workload-runtime/fd001-enterprise/readiness-probe.json",
        ),
        requirement(
            "liveness-probe-observed",
            TenantWorkloadRuntimeEvidenceRequirementKind::LivenessProbeObserved,
            "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#container-probes",
            "evidence/tenant-workload-runtime/fd001-enterprise/liveness-probe.json",
        ),
        requirement(
...
fn validate_nonclaims(
    ...
    {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_runtime_requirements(
    ...
    for requirement in &plan.requirements {
        ...
        if !seen_requirements.insert(requirement.requirement_id) {
            return Err(
                EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::DuplicateRequirement(
    ...
    for kind in required_requirement_kinds() {
        if !seen_kinds.contains(&kind) {
            return Err(
                EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::MissingRequirementKind(kind),
            );
        }
    }
    Ok(())
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidRequirementId,
    )?;
    ...
    validate_prefixed_ref(
        ...
        "evidence/tenant-workload-runtime/fd001-enterprise/",
        EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.source_manifest_ref,
        "crates/oya-enterprise-suite-tenant-workload-manifest/",
        EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidSourceManifestRef,
    )?;
...
fn validate_workload_scope(
    ...
    if value != "all-fd001-workloads" || has_unsafe_text(value) || has_path_traversal(value) {
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidWorkloadScope);
    }
    Ok(())
}

fn validate_doc_url(value: &str) -> Result<(), EnterpriseSuiteTenantWorkloadRuntimeEvidenceError> {
    if !matches!(
        value,
        "https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/"
            | "https://kubernetes.io/docs/concepts/policy/resource-quotas/"
        ...
        return Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn validate_slug(
    value: &str,
    error: EnterpriseSuiteTenantWorkloadRuntimeEvidenceError,
) -> Result<(), EnterpriseSuiteTenantWorkloadRuntimeEvidenceError> {
    if value.is_empty()
...
fn require_control(
    ...
    if value {
        ...
        Err(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError::MissingRequiredControl(name))
    }
}
