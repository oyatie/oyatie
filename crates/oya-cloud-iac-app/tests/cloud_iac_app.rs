...
use oya_cloud_iac_domain::{ModuleRegistry, OpenTofuModuleRelease};
...
const RELEASE_INDEX_JSON: &str =
    include_str!("../../../microservices/cloud-iac/tofu/modules/release-index.json");
    ...
    registry
        .publish(
            OpenTofuModuleRelease::new(
                ...
                "opentofu",
                "1.0.0",
                "git::https://git.oyatie.internal/oyatie/oyatie.git//microservices/cloud-iac/tofu/modules/vpc?ref=v1.0.0",
                format!("sha256:{}", "a".repeat(64)),
                "evidence://cloud-iac/app-entrypoint/vpc/1.0.0",
            )
...
fn boundary() -> CloudIacModuleRegistryApiBoundaryContext {
    CloudIacModuleRegistryApiBoundaryContext {
        request_id: "req_cloud_iac_app_entrypoint_test_001".to_string(),
    }
}
...
fn request_auth_policy_blocks_module_registry_and_artifact_paths_without_bearer_while_leaving_health_public()
 {
    ...
    for path in [
        ...
        let wrong = dispatch_cloud_iac_app_request(
            &service,
...
fn app_service_registers_health_liveness_and_module_registry_routes() {
    ...
    assert_eq!(
        body_text(&health),
        r#"{"status":"ok","service":"cloud-iac","check":"healthz"}"#
    );

    let live = dispatch_cloud_iac_app_request(
        &service,
        ...
        r#"{{
          "modules": [
            {{
              ...
              "archive_sha256": "{TEST_ARTIFACT_SHA256}",
              "archive_media_type": "archive/zip",
              "evidence_ref": "evidence://cloud-iac/modules/unit-artifact/0.1.0/local-foundation"
            }}
          ]
        }}"#
    );
...
fn release_index_backed_app_returns_s3_or_gcs_object_source_locations_without_live_object_store_claim()
 {
    let s3_source = format!(
        "s3::https://s3.amazonaws.com/oyatie-cloud-iac-modules/oyatie/unit-artifact/0.1.0/{TEST_ARTIFACT_ARCHIVE}"
    );
    let release_index = format!(
        r#"{{
          "modules": [
            {{
              ...
              "evidence_ref": "evidence://cloud-iac/modules/unit-artifact/0.1.0/object-source"
            }}
          ]
        }}"#
    );
    ...
    let gcs_source = format!(
        "gcs::https://www.googleapis.com/storage/v1/oyatie-cloud-iac-modules/oyatie/unit-artifact/0.1.0/{TEST_ARTIFACT_ARCHIVE}"
    );
    ...
    let bad_secret_source = release_index.replace(
        &s3_source,
        "s3::https://s3.amazonaws.com/oyatie-cloud-iac-modules/oyatie/unit-artifact/0.1.0/oyatie-unit-artifact-opentofu-0.1.0.zip?token=abc",
    );
    assert!(load_release_index_seed_from_str(&bad_secret_source).is_err());

    let wrong_archive_source = release_index.replace(
        &s3_source,
        "s3::https://s3.amazonaws.com/oyatie-cloud-iac-modules/oyatie/unit-artifact/0.1.0/oyatie-other-opentofu-0.1.0.zip",
    );
        ...
        r#"{{
          "modules": [
            {{
              ...
              "archive_sha256": "{TEST_ARTIFACT_SHA256}",
              "archive_media_type": "archive/zip",
              "evidence_ref": "evidence://cloud-iac/modules/digest-mismatch/0.1.0/local-foundation"
            }}
          ]
        }}"#
    );
        ...
        r#"{{
          "modules": [
            {{
              ...
              "archive_media_type": "archive/zip",
              "evidence_ref": "evidence://cloud-iac/modules/missing-artifact/0.1.0/local-foundation"
            }}
          ]
        }}"#
    );
