//! Executable ERP/SAP parity composition map for Enterprise Suite planning.
//!
...
//! does not create an ERP suite service, attach cloud infrastructure, deploy a
...
pub struct ErpParityMapCapabilities {
    ...
    pub erp_suite_microservice_created: bool,           // data_class: PUBLIC
...
const ERP_PARITY_MODULES: &[ErpParityModuleCoverage] = &[
    ...
    ErpParityModuleCoverage {
        ...
        evidence_refs: &["docs/architecture/enterprise-software-coverage-matrix-2026-05-21.md#3.1"],
        cloud_integration_ready: false,
        production_runtime_claimed: false,
    },
    ErpParityModuleCoverage {
        ...
        evidence_refs: &[
            ...
            "docs/architecture/enterprise-software-coverage-matrix-2026-05-21.md#3.1",
        ],
        cloud_integration_ready: false,
        production_runtime_claimed: false,
    },
    ...
    ErpParityModuleCoverage {
        ...
        tier: ParityTier::PartnerOrPackOverlay,
        status: "pack-overlay-no-vertical-suite",
        evidence_refs: &["docs/architecture/enterprise-software-coverage-matrix-2026-05-21.md#3.1"],
        cloud_integration_ready: false,
        production_runtime_claimed: false,
    },
...
pub fn enterprise_suite_erp_parity_map() -> &'static [ErpParityModuleCoverage] {
    ERP_PARITY_MODULES
}

pub fn erp_parity_map_capabilities() -> ErpParityMapCapabilities {
    ErpParityMapCapabilities {
        map_name: "enterprise-suite-erp-sap-parity-composition-map",
        sap_module_count: ERP_PARITY_MODULES.len(),
        compositional_parity_map_attached: true,
        erp_suite_microservice_created: false,
...
pub fn validate_erp_parity_map(rows: &[ErpParityModuleCoverage]) -> Result<(), ErpParityMapError> {
    ...
    for row in rows {
        ...
        if let Some(destination) = row
            ...
            .iter()
            .copied()
            .find(|destination| is_forbidden_erp_suite_destination(destination))
        {
            return Err(ErpParityMapError::ForbiddenErpSuiteDestination {
...
pub fn is_forbidden_erp_suite_destination(destination: &str) -> bool {
    let normalized = destination.trim().to_ascii_lowercase();
    normalized == "microservices/erp"
        || normalized.starts_with("microservices/erp/")
        || normalized == "microservices/erp-suite"
        || normalized.starts_with("microservices/erp-suite/")
}
