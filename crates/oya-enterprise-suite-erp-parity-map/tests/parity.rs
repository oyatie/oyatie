use oya_enterprise_suite_erp_parity_map::{
    ErpParityMapError, ParityTier, SapModuleCode, enterprise_suite_erp_parity_map,
    erp_parity_map_capabilities, find_erp_module, is_forbidden_erp_suite_destination,
    validate_erp_parity_map,
};

#[test]
fn erp_parity_map_covers_sap_modules_without_erp_suite_service() {
    let rows = enterprise_suite_erp_parity_map();
    ...
    for row in rows {
        ...
        for destination in row.oyatie_destinations {
            assert!(!is_forbidden_erp_suite_destination(destination));
        }
    }
}
...
fn capabilities_keep_runtime_and_cloud_non_claims_false() {
    ...
    assert!(!capabilities.erp_suite_microservice_created);
...
#[test]
fn validation_rejects_erp_suite_destination() {
    let mut rows = enterprise_suite_erp_parity_map().to_vec();
    rows[0].oyatie_destinations = &["microservices/erp"];
    let error = validate_erp_parity_map(&rows).expect_err("ERP suite destination is rejected");
    assert_eq!(
        error,
        ErpParityMapError::ForbiddenErpSuiteDestination {
