//! Enterprise Suite deployed listener runtime evidence contract.
//!
//! This review-only crate records the listener runtime evidence that must exist
//! before FD-001 tenant workloads can claim a deployed Enterprise Suite listener
...
const PLAN_NAME: &str = "enterprise-suite-listener-runtime-evidence-plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteListenerRuntimeEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteListenerRuntimeEvidenceError {
...
pub fn enterprise_suite_listener_runtime_evidence_plan()
-> Result<EnterpriseSuiteListenerRuntimeEvidencePlan, EnterpriseSuiteListenerRuntimeEvidenceError> {
    ...
    validate_enterprise_suite_listener_gateway_plan(&listener_gateway_plan)
        .map_err(EnterpriseSuiteListenerRuntimeEvidenceError::ListenerGateway)?;

    Ok(EnterpriseSuiteListenerRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
        ...
        requirements: vec![
            requirement(
                ...
                "evidence/listener-runtime/enterprise-suite/service-clusterip.json",
            ),
            requirement(
                "gateway-httproute-accepted",
                ListenerRuntimeEvidenceRequirementKind::GatewayHttpRouteAccepted,
                "enterprise-suite-gateway-route",
                "https://gateway-api.sigs.k8s.io/concepts/api-overview/",
                "evidence/listener-runtime/enterprise-suite/gateway-httproute-accepted.json",
            ),
            requirement(
                "tls-certificate-bound",
                ListenerRuntimeEvidenceRequirementKind::TlsCertificateBound,
                "enterprise-suite-gateway-listener",
                "https://gateway-api.sigs.k8s.io/concepts/api-overview/",
                "evidence/listener-runtime/enterprise-suite/tls-certificate-bound.json",
            ),
            requirement(
                ...
                "evidence/listener-runtime/enterprise-suite/readiness-probe.json",
            ),
            requirement(
                ...
                "enterprise-suite-runtime",
                "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/",
                "evidence/listener-runtime/enterprise-suite/liveness-probe.json",
            ),
            requirement(
                ...
                "enterprise-suite-runtime",
                "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/",
                "evidence/listener-runtime/enterprise-suite/synthetic-health-check.json",
            ),
            requirement(
                ...
                "https://kubernetes.io/docs/concepts/services-networking/service/",
                "evidence/listener-runtime/enterprise-suite/route-authz.json",
            ),
            requirement(
                "network-policy-ingress-denied",
                ListenerRuntimeEvidenceRequirementKind::NetworkPolicyIngressDenied,
                "enterprise-suite-network-policy",
                "https://kubernetes.io/docs/concepts/services-networking/network-policies/",
                "evidence/listener-runtime/enterprise-suite/network-policy-ingress-denied.json",
            ),
            requirement(
                ...
                "evidence/listener-runtime/enterprise-suite/endpoint-slice-ready.json",
            ),
            requirement(
                ...
                "enterprise-suite-runtime",
                "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/",
                "evidence/listener-runtime/enterprise-suite/graceful-shutdown.json",
            ),
            requirement(
                ...
                "evidence/listener-runtime/enterprise-suite/access-log-trace-correlation.json",
            ),
            requirement(
                ...
                "evidence/listener-runtime/enterprise-suite/listener-audit-event.json",
            ),
        ],
...
pub fn validate_enterprise_suite_listener_runtime_evidence_plan(
    plan: &EnterpriseSuiteListenerRuntimeEvidencePlan,
) -> Result<(), EnterpriseSuiteListenerRuntimeEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseSuiteListenerRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidServiceName);
    }
    if plan.substrate_name != SUBSTRATE_NAME {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidSubstrateName);
    }
    if plan.tenant_namespace != TENANT_NAMESPACE {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidTenantNamespace);
    }
    if plan.listener_gateway_plan_name != "enterprise-suite-listener-gateway-plan" {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidListenerGatewayPlanName);
    }
    if plan.listener_gateway_route_count != EXPECTED_ROUTE_COUNT {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidListenerGatewayRouteCount);
    }
    if plan.listener_gateway_service_type != "ClusterIP" {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidListenerGatewayServiceType);
    }
    if plan.requirements.len() < MIN_REQUIREMENT_COUNT {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::MissingRequirements);
    }
    for control in [
        (
    ...
    {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::RuntimeAttachmentOverclaim);
    }
    ...
    for requirement in &plan.requirements {
        ...
        if !seen_requirements.insert(requirement.requirement_id) {
            return Err(
                EnterpriseSuiteListenerRuntimeEvidenceError::DuplicateRequirement(
    ...
    for kind in required_requirement_kinds() {
        if !seen_kinds.contains(&kind) {
            return Err(EnterpriseSuiteListenerRuntimeEvidenceError::MissingRequirementKind(kind));
        }
    }
    Ok(())
}
...
fn requirement(
    ...
    ListenerRuntimeEvidenceRequirement {
        ...
        source_plan_ref: "crates/oya-enterprise-suite-listener-gateway/src/lib.rs::enterprise_suite_listener_gateway_plan",
        tenant_namespace: TENANT_NAMESPACE,
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseSuiteListenerRuntimeEvidenceError::InvalidRequirementId,
    )?;
    ...
    validate_prefixed_ref(
        ...
        "evidence/listener-runtime/enterprise-suite/",
        EnterpriseSuiteListenerRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.source_plan_ref,
        "crates/oya-enterprise-suite-listener-gateway/",
        EnterpriseSuiteListenerRuntimeEvidenceError::InvalidSourcePlanRef,
    )?;
...
fn validate_slug(
    value: &str,
    error: EnterpriseSuiteListenerRuntimeEvidenceError,
) -> Result<(), EnterpriseSuiteListenerRuntimeEvidenceError> {
    if value.is_empty()
...
fn validate_workload_scope(value: &str) -> Result<(), EnterpriseSuiteListenerRuntimeEvidenceError> {
    if value.is_empty() || has_unsafe_text(value) || has_path_traversal(value) {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidWorkloadScope);
    }
    Ok(())
}

fn validate_doc_url(url: &str) -> Result<(), EnterpriseSuiteListenerRuntimeEvidenceError> {
    let allowed = [
        "https://kubernetes.io/docs/concepts/services-networking/service/",
        "https://gateway-api.sigs.k8s.io/concepts/api-overview/",
        "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/",
    ...
    if !allowed.contains(&url) {
        return Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn require_control(
    ...
    if enabled {
        ...
        Err(EnterpriseSuiteListenerRuntimeEvidenceError::MissingRequiredControl(field))
    }
}
