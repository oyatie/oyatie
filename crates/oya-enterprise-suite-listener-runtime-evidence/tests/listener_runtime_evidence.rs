use oya_enterprise_suite_listener_runtime_evidence::{
    EnterpriseSuiteListenerRuntimeEvidenceError, ListenerRuntimeEvidenceRequirementKind,
    enterprise_suite_listener_runtime_evidence_plan, listener_runtime_evidence_doc_urls,
...
fn listener_runtime_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_listener_runtime_evidence_plan().expect("plan builds");
    validate_enterprise_suite_listener_runtime_evidence_plan(&plan).expect("plan validates");

    assert_eq!(
        plan.plan_name,
        "enterprise-suite-listener-runtime-evidence-plan"
    );
    ...
    assert_eq!(
        ...
        "enterprise-suite-listener-gateway-plan"
    );
...
fn listener_runtime_evidence_plan_preserves_ref_and_route_boundaries() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            .starts_with("evidence/listener-runtime/enterprise-suite/")
            && requirement
                .source_plan_ref
                .starts_with("crates/oya-enterprise-suite-listener-gateway/")
...
fn listener_runtime_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = enterprise_suite_listener_runtime_evidence_plan().expect("plan builds");
    plan.requirements.truncate(2);
    assert_eq!(
        validate_enterprise_suite_listener_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteListenerRuntimeEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_listener_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidOfficialDocUrl)
    );
}
...
fn listener_runtime_evidence_plan_rejects_unsafe_refs_missing_controls_and_overclaims() {
    ...
    plan.requirements[0].expected_evidence_ref =
        "evidence/listener-runtime/enterprise-suite/secret-token";
    assert_eq!(
        validate_enterprise_suite_listener_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteListenerRuntimeEvidenceError::InvalidExpectedEvidenceRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_listener_runtime_evidence_plan(&plan),
        Err(
            EnterpriseSuiteListenerRuntimeEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_suite_listener_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteListenerRuntimeEvidenceError::RuntimeAttachmentOverclaim)
    );
}
