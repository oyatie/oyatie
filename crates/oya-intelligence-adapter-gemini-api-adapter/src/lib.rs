//! M02-P01 — gemini api-auth adapter.
...
use oya_intelligence_adapter_gemini_api_kernel::{
...
impl GeminiApiAdapter {
    ...
    fn synthesize_token_id(&self, sref: &SecretReference) -> String {
        ...
        let dbg = format!("{sref:?}");
        format!("mock-gemini-api-{}", dbg.len())
    }
}

impl ProviderAuthPort for GeminiApiAdapter {
...
mod tests {
    ...
    use oya_intelligence_adapter_gemini_api_kernel::{AUTH_MODE, AuthMode};
    ...
    fn adapter_clock_override_respected() {
        let a = GeminiApiAdapter::new().with_clock(5_000, 60);
