...
use oya_enterprise_suite_domain::{
    EnterpriseSuiteDomainError, IncidentFirstAction, IncidentPlanStatus, IncidentRemediationRoute,
    IncidentRollbackInput, IncidentTrigger, plan_incident_rollback,
};
...
fn test_rollback_first_no_manual_ssh() {
    ...
    assert_eq!(
        ...
        "audit/enterprise-suite/incidents/canary-slo"
    );
    assert_eq!(
        plan.incident_evidence_ref.value.value,
        "audit/enterprise-suite/incidents/inc_canary_slo_001.json"
    );
    assert_eq!(
        plan.rollback_evidence_ref.value.value,
        "audit/enterprise-suite/incidents/rollback-first"
    );
    assert_eq!(
        plan.convergence_ref.value.value,
        "opentofu/enterprise-suite/fixes/inc_canary_slo_001"
    );
    assert_eq!(
...
fn test_manual_ssh_incident_route_is_refused() {
    ...
    assert_eq!(error, EnterpriseSuiteDomainError::ManualSshRefused);
}
...
fn test_remediation_before_rollback_is_refused() {
    ...
    assert_eq!(
        error,
        EnterpriseSuiteDomainError::RollbackOrQuarantineMustBeFirst
    );
}
...
fn test_missing_incident_evidence_is_refused() {
    ...
    assert_eq!(error, EnterpriseSuiteDomainError::InvalidAuditEvidenceRef);
}

#[test]
fn test_missing_opentofu_or_ops_convergence_ref_is_refused() {
    let error = plan_incident_rollback(IncidentRollbackInput {
        convergence_ref: "ssh/enterprise-suite/manual-hotfix".to_owned(),
        ..valid_input()
    })
    .expect_err("incident fixes must converge through OpenTofu or ops");

    assert_eq!(error, EnterpriseSuiteDomainError::InvalidConvergenceRef);
}
...
fn test_quarantine_first_can_converge_through_ops() {
    let plan = plan_incident_rollback(IncidentRollbackInput {
        ...
        convergence_ref: "ops/enterprise-suite/quarantine/inc_prod_002".to_owned(),
    ...
    assert_eq!(
        plan.convergence_ref.value.value,
        "ops/enterprise-suite/quarantine/inc_prod_002"
    );
}
