//! Enterprise Suite domain foundation.
//!
//! This crate owns pure suite-level invariants that span HR, Payroll, and
...
//! audit-chain emission, deployment, or child-service writes.
...
const SUITE_DECISION_SCHEMA_VERSION: u32 = 1;
...
const POLICY_GATEWAY_PREFIX: &str = "policy/enterprise-suite/";
...
const CROSS_PRODUCT_WORKFLOW_SCHEMA_VERSION: u32 = 1;
...
pub enum EnterpriseChildProduct {
    Hr,
    Payroll,
    Accounting,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd)]
pub enum SuitePolicyDecisionStatus {
    Accepted,
}
...
pub enum WorkflowRoutingOwner {
    Workflow,
    ChildService(EnterpriseChildProduct),
    AiAgent,
}
...
pub struct ChildWriteInput {
    pub child_product: EnterpriseChildProduct, // data_class: INTERNAL_ONLY
    pub write_kind: SuiteWriteKind,            // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SuitePolicyDecision {
    pub child_product: Classified<EnterpriseChildProduct>, // data_class: INTERNAL_ONLY
    pub write_kind: Classified<SuiteWriteKind>,            // data_class: INTERNAL_ONLY
    ...
    pub decision_status: Classified<SuitePolicyDecisionStatus>, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CrossProductWorkflowInput {
    pub tenant_id: String,                     // data_class: INTERNAL_ONLY
    ...
    pub child_products: Vec<EnterpriseChildProduct>, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CrossProductWorkflowPlan {
    pub tenant_id: Classified<TenantId>, // data_class: INTERNAL_ONLY
    ...
    pub child_products: Classified<Vec<EnterpriseChildProduct>>, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteDomainError {
    InvalidTenantId,
    InvalidLegalEntityId,
    InvalidGroupId,
    InvalidAuditEvidenceRef,
    BypassedSuitePolicyGateway,
...
pub fn admit_child_write(
    input: ChildWriteInput,
) -> Result<SuitePolicyDecision, EnterpriseSuiteDomainError> {
    require_prefixed(
        &input.tenant_id,
        TENANT_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidTenantId,
    )?;
    require_prefixed(
        &input.legal_entity_id,
        LEGAL_ENTITY_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidLegalEntityId,
    )?;
    let payload_data_class = input
        .payload_data_class
        .ok_or(EnterpriseSuiteDomainError::MissingPayloadDataClass)?;
    require_prefixed(
        &input.audit_evidence_ref,
        AUDIT_EVIDENCE_PREFIX,
        EnterpriseSuiteDomainError::InvalidAuditEvidenceRef,
    )?;
    require_prefixed(
    ...
    if input.sequence == 0 {
        return Err(EnterpriseSuiteDomainError::InvalidSequence);
    }

    Ok(SuitePolicyDecision {
        child_product: internal(input.child_product),
        ...
        decision_status: internal(SuitePolicyDecisionStatus::Accepted),
        schema_version: public(SUITE_DECISION_SCHEMA_VERSION),
    })
}

pub fn roll_up_group_close_status(
    input: GroupRollupInput,
) -> Result<GroupCloseRollup, EnterpriseSuiteDomainError> {
    require_prefixed(
        &input.tenant_id,
        TENANT_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidTenantId,
    ...
    require_prefixed(
        &input.group_id,
        GROUP_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidGroupId,
    )?;
    require_prefixed(
        &input.dashboard_projection_ref,
        PROJECTION_REF_PREFIX,
        EnterpriseSuiteDomainError::InvalidDashboardProjectionRef,
    )?;
    if input.legal_entities.is_empty() {
        return Err(EnterpriseSuiteDomainError::EmptyLegalEntitySet);
    }
    ...
    for entity in input.legal_entities {
        if entity.tenant_id != input.tenant_id {
            return Err(EnterpriseSuiteDomainError::CrossTenantLegalEntity);
        }
        require_prefixed(
        ...
        {
            return Err(EnterpriseSuiteDomainError::MissingCloseEvidence);
        }
...
pub fn plan_cross_product_workflow(
    input: CrossProductWorkflowInput,
) -> Result<CrossProductWorkflowPlan, EnterpriseSuiteDomainError> {
    require_prefixed(
        &input.tenant_id,
        TENANT_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidTenantId,
    ...
    require_prefixed(
        &input.workflow_ref,
        WORKFLOW_REF_PREFIX,
        EnterpriseSuiteDomainError::InvalidWorkflowRef,
    )?;
    require_prefixed(
        &input.object_graph_relationship_ref,
        OBJECT_GRAPH_REF_PREFIX,
        EnterpriseSuiteDomainError::InvalidObjectGraphRelationshipRef,
    )?;
    if input.routing_owner != WorkflowRoutingOwner::Workflow {
        return Err(EnterpriseSuiteDomainError::WorkflowRoutingBypass);
    }
    if input.relationship_owner != ObjectGraphRelationshipOwner::ObjectGraph {
        return Err(EnterpriseSuiteDomainError::ObjectGraphBypass);
    }
    if input.gate_closure_authority == GateClosureAuthority::AiSuggestion {
        return Err(EnterpriseSuiteDomainError::AiCannotCloseDeterministicGate);
    }
    require_cross_product_children(&input.child_products)?;
    ...
    let ai_suggestion_ref = match input.ai_suggestion_ref {
        Some(value) => {
            require_prefixed(
                ...
                EnterpriseSuiteDomainError::InvalidAiSuggestionRef,
            )?;
    ...
    Ok(CrossProductWorkflowPlan {
        ...
        child_products: internal(input.child_products),
        ...
        idempotency_key: internal(input.idempotency_key),
        schema_version: public(CROSS_PRODUCT_WORKFLOW_SCHEMA_VERSION),
    })
}

pub fn plan_incident_rollback(
    input: IncidentRollbackInput,
) -> Result<IncidentRollbackPlan, EnterpriseSuiteDomainError> {
    require_prefixed(
        &input.tenant_id,
        TENANT_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidTenantId,
    ...
    require_prefixed(
        &input.incident_id,
        INCIDENT_ID_PREFIX,
        EnterpriseSuiteDomainError::InvalidIncidentId,
    )?;
    if !matches!(
        input.first_action,
        IncidentFirstAction::Rollback | IncidentFirstAction::Quarantine
    ) {
        return Err(EnterpriseSuiteDomainError::RollbackOrQuarantineMustBeFirst);
    }
    if input.remediation_route == IncidentRemediationRoute::ManualSsh {
        return Err(EnterpriseSuiteDomainError::ManualSshRefused);
    }
    require_prefixed(
...
fn require_cross_product_children(
    child_products: &[EnterpriseChildProduct],
) -> Result<(), EnterpriseSuiteDomainError> {
    let required = [
    ...
    if required
        .iter()
        .any(|required_product| !child_products.contains(required_product))
    {
        return Err(EnterpriseSuiteDomainError::MissingChildProduct);
    }
    Ok(())
}
...
fn deterministic_gate_evidence(
...
) -> Result<Vec<DeterministicGateEvidence>, EnterpriseSuiteDomainError> {
    ...
    for required_gate in required_workflow_gates() {
        ...
        {
            return Err(EnterpriseSuiteDomainError::MissingDeterministicGateEvidence);
        }
    }
    Ok(evidence)
}
...
fn require_convergence_ref(value: &str) -> Result<(), EnterpriseSuiteDomainError> {
    if value.starts_with(OPENTOFU_CONVERGENCE_PREFIX) {
        require_prefixed(
            value,
            OPENTOFU_CONVERGENCE_PREFIX,
            EnterpriseSuiteDomainError::InvalidConvergenceRef,
        )
        ...
        Err(EnterpriseSuiteDomainError::InvalidConvergenceRef)
    }
}
...
fn require_prefixed(
    value: &str,
    prefix: &str,
    error: EnterpriseSuiteDomainError,
) -> Result<(), EnterpriseSuiteDomainError> {
...
fn require_non_empty_key(value: &str) -> Result<(), EnterpriseSuiteDomainError> {
    if value.trim().is_empty() || value.contains("..") || value.chars().any(char::is_whitespace) {
        return Err(EnterpriseSuiteDomainError::InvalidIdempotencyKey);
    }
    Ok(())
}
