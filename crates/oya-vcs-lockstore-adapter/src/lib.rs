...
use oya_vcs_kernel::{Claim, ClaimCompatibility, ClaimState, claim_compatibility};
...
mod tests {
    use super::*;
    use oya_vcs_kernel::{ArtifactPointer, SymbolId, SymbolLanguage};

    fn symbol(name: &str) -> SymbolId {
        SymbolId::new(
            SymbolLanguage::Rust,
            ArtifactPointer::file("crates/oya-vcs-lockstore-adapter/src/lib.rs").unwrap(),
            name,
        )
        .unwrap()
    }
