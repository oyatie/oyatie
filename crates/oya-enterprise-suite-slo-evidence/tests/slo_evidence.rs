use oya_enterprise_suite_slo_evidence::{
    EnterpriseSloEvidenceError, EnterpriseSloKind, enterprise_suite_slo_evidence_plan,
    openslo_manifest_paths, validate_enterprise_suite_slo_evidence_plan,
};

#[test]
fn slo_evidence_plan_validates_error_budget_otel_and_nonclaims() {
    let plan = enterprise_suite_slo_evidence_plan();
    validate_enterprise_suite_slo_evidence_plan(&plan).expect("SLO evidence plan validates");
    ...
    assert!(
        plan.dashboard_ref
            .starts_with("dashboards/enterprise-suite/")
    );
...
fn slo_evidence_plan_preserves_openslo_manifest_paths_and_objective_mix() {
    ...
    assert!(
        paths
            .iter()
            .all(|path| path.starts_with("microservices/enterprise-suite/slos/"))
    );
    assert!(paths.iter().all(|path| path.ends_with(".openslo.yaml")));
    assert!(
        plan.objectives
            .iter()
            .any(|objective| objective.kind == EnterpriseSloKind::Availability)
    );
    assert!(
        plan.objectives
            .iter()
            .any(|objective| objective.kind == EnterpriseSloKind::Latency)
    );
    assert!(
        plan.objectives
            .iter()
            .any(|objective| objective.kind == EnterpriseSloKind::Freshness)
    );
    assert!(
        plan.objectives
            .iter()
            .any(|objective| objective.kind == EnterpriseSloKind::Correctness)
    );
}

#[test]
fn slo_evidence_plan_rejects_missing_objectives_or_unsafe_refs() {
    let mut plan = enterprise_suite_slo_evidence_plan();
    plan.objectives.truncate(1);
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::MissingObjectives)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::InvalidManifestPath)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::InvalidMetricName)
    );
}
...
fn slo_evidence_plan_rejects_runtime_or_production_overclaims() {
    ...
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::RuntimeAttachmentOverclaim)
    );
...
fn slo_evidence_plan_requires_actionable_burn_rate_windows() {
    ...
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::InvalidBurnRateWindow)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_slo_evidence_plan(&plan),
        Err(EnterpriseSloEvidenceError::InvalidBurnRateThreshold)
    );
}
