use oya_tenant_rbac_identity_provider_verification::{
    IdentityProviderVerificationError, render_identity_provider_verification_checklist,
...
fn identity_provider_verification_plan_covers_oidc_discovery_jwks_and_claims() {
    let plan = tenant_rbac_identity_provider_verification_plan();
...
fn identity_provider_verification_plan_rejects_bad_issuer_audience_or_algorithm() {
    let mut plan = tenant_rbac_identity_provider_verification_plan();
    plan.issuer = "http://identity.oyatie.com/tenant-rbac";
    assert_eq!(
        validate_tenant_rbac_identity_provider_verification_plan(&plan),
...
fn identity_provider_verification_plan_rejects_claim_drift_and_duplicates() {
    ...
    let mut plan = tenant_rbac_identity_provider_verification_plan();
    plan.required_claims[0].expected_value = "https://identity.other.example/tenant-rbac";
    assert_eq!(
        validate_tenant_rbac_identity_provider_verification_plan(&plan),
