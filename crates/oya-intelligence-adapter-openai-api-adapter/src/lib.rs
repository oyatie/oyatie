//! M02-P01 — openai api-auth adapter.
...
use oya_intelligence_adapter_openai_api_kernel::{
...
impl OpenAiApiAdapter {
    ...
    fn synthesize_token_id(&self, sref: &SecretReference) -> String {
        ...
        let dbg = format!("{sref:?}");
        format!("mock-openai-api-{}", dbg.len())
    }
}

impl ProviderAuthPort for OpenAiApiAdapter {
...
mod tests {
    ...
    use oya_intelligence_adapter_openai_api_kernel::{AUTH_MODE, AuthMode};
    ...
    fn adapter_authenticates_and_returns_token() {
        let a = OpenAiApiAdapter::new();
        let t = a.authenticate(&sref("sref://test-key")).unwrap();
        assert_eq!(t.provider_family(), ProviderFamily::OpenAiOrCodex);
    ...
    fn adapter_reports_correct_mode_and_family() {
        let a = OpenAiApiAdapter::new();
        assert_eq!(a.auth_mode(), AuthMode::Api);
        assert_eq!(a.provider_family(), ProviderFamily::OpenAiOrCodex);
        assert_eq!(AUTH_MODE, AuthMode::Api);
    }
    ...
    fn adapter_clock_override_respected() {
        let a = OpenAiApiAdapter::new().with_clock(5_000, 60);
