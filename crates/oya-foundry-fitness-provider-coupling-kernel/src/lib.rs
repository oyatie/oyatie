...
mod tests {
    ...
    fn neutral_imports_pass() {
        let imports = vec![
            imp("oya-foundry-account-kernel", "src/lib.rs", 5, "std::fmt"),
            imp(
                "oya-foundry-account-domain",
                "src/lib.rs",
                10,
                "oya_foundry_account_kernel::ProviderFamily",
            ),
        ];
    ...
    fn adapter_crate_allowed_to_use_provider_name() {
        let imports = vec![
            imp(
                "oya-foundry-adapter-anthropic-api-kernel",
                "src/lib.rs",
                5,
                "anthropic_sdk::Client",
            ),
            imp(
                "oya-foundry-adapter-openai-api-kernel",
                "src/lib.rs",
                5,
                "openai_sdk::ChatRequest",
            ),
    ...
    fn provider_family_token_in_neutral_import_not_flagged() {
        ...
        let imports = vec![imp(
            "oya-foundry-route-policy-kernel",
            "src/lib.rs",
            5,
            "oya_foundry_account_kernel::ProviderFamily",
        )];
