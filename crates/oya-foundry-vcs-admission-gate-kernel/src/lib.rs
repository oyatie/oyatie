...
//!
//! - Crate `oya-foundry-vcs-admission-gate-kernel` —
...
//! - Companion `oya-foundry-vcs-admission-gate-app` —
...
fn check_supply_chain_workflow(text: &str, violations: &mut Vec<AdmissionViolation>) {
    ...
    let invokes_rust_app = text.contains("oya-foundry-vcs-provider-execution-gate-app");
    if !invokes_legacy_script && !invokes_rust_app {
        push(
            violations,
            "SUPPLY_CHAIN_WORKFLOW_MISSING_PROVIDER_EXECUTION_INVOCATION",
            "oya-foundry-fitness-supply-chain workflow must invoke either scripts/check-oya-vcs-provider-execution.sh --mode ci (legacy) or oya-foundry-vcs-provider-execution-gate-app (Wave 3)",
        );
    }
}
