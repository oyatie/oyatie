...
fn audit_event_emit_appends_chain_and_outbox_once_and_replays_idempotently() {
    ...
    let first =
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, request.clone())
            .expect("first emit appends audit chain and outbox");
    let second = emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, request)
...
fn audit_event_emit_rejects_envelope_payload_drift_before_mutation() {
    ...
    let tenant_error =
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, tenant_drift)
    ...
    let idempotency_error =
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, idempotency_drift)
...
fn audit_event_emit_separates_missing_producer_from_denied_authorization() {
    ...
    let authn_error =
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, unauthenticated)
    ...
    let authz_error = emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, denied)
...
fn audit_event_emit_rejects_invalid_labels_before_mutation() {
    ...
    assert!(matches!(
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, invalid_plane),
    ...
    assert!(matches!(
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, invalid_purpose),
    ...
    let class_error =
        emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, invalid_class)
            .expect_err(
                "audit payload accepts privacy-program data classes, not operational labels",
            );
    assert!(matches!(
        class_error,
...
fn audit_event_emit_rejects_reused_idempotency_key_with_new_fingerprint() {
    ...
    let error = emit_audit_event_from_app(&mut chain, &mut outbox, &mut idempotency, request)
