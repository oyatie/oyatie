...
//!
//! Wires the `oya-shared-hyperscaler-metrics-kernel` trait to a
...
//!
//! ```ignore
//! use oya_shared_hyperscaler_metrics_kernel::MetricsContext;
...
pub struct PrometheusHyperscalerMetrics {
    ctx: MetricsContext,
    /// `oya_<ms>_capability_circuit_state{capability_id, state}` — gauge.
    capability_circuit_state: GaugeVec,
    /// `oya_<ms>_capability_retry_budget_exhausted_total{capability_id}` — counter.
    capability_retry_budget_exhausted: IntCounterVec,
    /// `oya_<ms>_responses_429_total{tenant_id}` — counter.
...
mod tests {
    ...
    fn responses_429_counter_increments_per_tenant() {
        ...
        let metric = gathered
            .iter()
            .find(|mf| mf.get_name() == "oya_messenger_responses_429_total")
            .unwrap();
    ...
    fn all_seven_canonical_families_appear_in_registry() {
        ...
        for expected in [
            "oya_audit_chain_capability_circuit_state",
            "oya_audit_chain_capability_retry_budget_exhausted_total",
            "oya_audit_chain_responses_429_total",
            "oya_audit_chain_responses_5xx_total",
            "oya_audit_chain_responses_total",
            "oya_audit_chain_request_success_total",
            "oya_audit_chain_request_total",
        ] {
            assert!(
    ...
    fn microservice_label_bound_once() {
        ...
        let metric = gathered
            .iter()
            .find(|mf| mf.get_name() == "oya_messenger_responses_total")
            .unwrap();
        for m in metric.get_metric() {
            let label = m
                .get_label()
