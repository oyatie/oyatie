//! Enterprise Suite Postgres/RLS runtime evidence contract foundation.
//!
...
//! Enterprise Suite storage is claimed. It binds the existing review-only
...
const PLAN_NAME: &str = "enterprise-suite-postgres-rls-runtime-evidence-plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterprisePostgresRlsRuntimeEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterprisePostgresRlsRuntimeEvidenceError {
...
pub fn enterprise_postgres_rls_runtime_evidence_plan()
-> Result<EnterprisePostgresRlsRuntimeEvidencePlan, EnterprisePostgresRlsRuntimeEvidenceError> {
    let storage_plan = enterprise_postgres_rls_storage_plan();
    validate_enterprise_postgres_rls_storage_plan(&storage_plan)
        .map_err(EnterprisePostgresRlsRuntimeEvidenceError::StoragePlan)?;

    Ok(EnterprisePostgresRlsRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
        ...
        probes: vec![
            probe(
                "tls-verify-full-enterprise-suite",
            ...
            probe(
                "migration-digest-enterprise-policy-admissions",
                "enterprise_policy_admissions",
                EnterprisePostgresRecordKind::PolicyAdmission,
            ...
            probe(
                "backup-restore-rehearsal-enterprise-suite",
                "enterprise_ops_commands",
                EnterprisePostgresRecordKind::OpsCommand,
            ...
            probe(
                "pitr-rehearsal-enterprise-suite",
                "enterprise_policy_admissions",
                EnterprisePostgresRecordKind::PolicyAdmission,
...
pub fn validate_enterprise_postgres_rls_runtime_evidence_plan(
    plan: &EnterprisePostgresRlsRuntimeEvidencePlan,
) -> Result<(), EnterprisePostgresRlsRuntimeEvidenceError> {
    let storage_plan = enterprise_postgres_rls_storage_plan();
    validate_enterprise_postgres_rls_storage_plan(&storage_plan)
        .map_err(EnterprisePostgresRlsRuntimeEvidenceError::StoragePlan)?;
    ...
    validate_slug(
        plan.plan_name,
        EnterprisePostgresRlsRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidServiceName);
    }
    if plan.schema_name != SCHEMA_NAME {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidSchemaName);
    }
    if plan.runtime_role != RUNTIME_ROLE {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidRuntimeRole);
    }
    if plan.tenant_context_setting != TENANT_CONTEXT_SETTING {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidTenantContextSetting);
    }
    if plan.storage_plan_table_count != storage_plan.tables.len() {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidTableCount);
    }
    if plan.probes.len() < MIN_PROBE_COUNT {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::MissingProbes);
    }
    ...
    for probe in &plan.probes {
        ...
        if !seen_probes.insert(probe.probe_id) {
            return Err(EnterprisePostgresRlsRuntimeEvidenceError::DuplicateProbe(
    ...
    for table_name in storage_tables {
        if !seen_tables.contains(table_name) {
            return Err(
                EnterprisePostgresRlsRuntimeEvidenceError::MissingStoragePlanTable(
                    table_name.to_owned(),
                ),
            );
        }
    ...
    for probe_kind in required_probe_kinds() {
        if !seen_probe_kinds.contains(&probe_kind) {
            return Err(EnterprisePostgresRlsRuntimeEvidenceError::MissingProbeKind(
                probe_kind,
            ));
        }
    }
    ...
    {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn probe(
    ...
    PostgresRlsRuntimeProbe {
        ...
        probe_kind,
        official_doc_url,
        source_migration_ref: "crates/oya-enterprise-suite-postgres-rls-storage/src/lib.rs::render_enterprise_postgres_rls_migration",
        runtime_check_ref,
        expected_evidence_ref: "evidence/postgres-rls-runtime/enterprise-suite/probe-results.jsonl",
...
fn validate_probe(
    ...
    validate_slug(
        probe.probe_id,
        EnterprisePostgresRlsRuntimeEvidenceError::InvalidProbeId,
    )?;
    if !storage_tables.contains(probe.table_name) {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidTableName);
    }
    ...
    validate_prefixed_ref(
        ...
        EnterprisePostgresRlsRuntimeEvidenceError::InvalidSourceMigrationRef,
    )?;
    validate_prefixed_ref(
        probe.runtime_check_ref,
        "runtime-check/postgres-rls/",
        EnterprisePostgresRlsRuntimeEvidenceError::InvalidRuntimeCheckRef,
    )?;
    validate_prefixed_ref(
        probe.expected_evidence_ref,
        "evidence/postgres-rls-runtime/",
        EnterprisePostgresRlsRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    require_control(
...
fn validate_doc_url(url: &str) -> Result<(), EnterprisePostgresRlsRuntimeEvidenceError> {
    if is_unsafe_ref(url)
        ...
        .contains(&url)
    {
        return Err(EnterprisePostgresRlsRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn validate_slug(
    value: &str,
    error: EnterprisePostgresRlsRuntimeEvidenceError,
) -> Result<(), EnterprisePostgresRlsRuntimeEvidenceError> {
...
fn require_control(
    ...
    if value {
        ...
        Err(EnterprisePostgresRlsRuntimeEvidenceError::MissingRequiredControl(control))
    }
}
