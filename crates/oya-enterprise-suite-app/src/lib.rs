//! Enterprise Suite application-layer envelopes.
//!
...
use oya_enterprise_suite_domain::{
    AiSuggestionRef, AuditEvidenceRef, CrossProductWorkflowPlan, DeterministicGate,
...
const ENTERPRISE_OPS_TOPIC: &str = "audit.enterprise-suite.ops.command";
const ENTERPRISE_WORKFLOW_TOPIC: &str = "workflow.enterprise-suite.cross-product.dispatch";
const ENTERPRISE_INCIDENT_ROLLBACK_TOPIC: &str = "incident.enterprise-suite.rollback.plan";
const TENANT_ID_PREFIX: &str = "ten_";
const AUDIT_EVIDENCE_PREFIX: &str = "audit/";
const OPENTOFU_PLAN_PREFIX: &str = "opentofu/";

#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd)]
pub enum EnterpriseOpsRoute {
    MakefileTarget,
    OyaOps,
    OpsConsole,
    ManualSsh,
...
pub struct EnterpriseOpsCommandInput {
    pub tenant_id: String,            // data_class: INTERNAL_ONLY
    pub route: EnterpriseOpsRoute,    // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CrossProductWorkflowEnvelope {
    pub topic: Classified<String>,             // data_class: INTERNAL_ONLY
    pub tenant_id: Classified<TenantId>,       // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteAppError {
    InvalidTenantId,
    InvalidEvidenceRef,
...
pub fn prepare_enterprise_ops_envelope(
    input: EnterpriseOpsCommandInput,
) -> Result<EnterpriseOpsEnvelope, EnterpriseSuiteAppError> {
    if input.route == EnterpriseOpsRoute::ManualSsh {
        return Err(EnterpriseSuiteAppError::ManualSshRefused);
    }
    require_prefixed(
        &input.tenant_id,
        TENANT_ID_PREFIX,
        EnterpriseSuiteAppError::InvalidTenantId,
    )?;
    require_prefixed(
        &input.evidence_ref,
        AUDIT_EVIDENCE_PREFIX,
        EnterpriseSuiteAppError::InvalidEvidenceRef,
    )?;
    require_prefixed(
        &input.change_plan_ref,
        OPENTOFU_PLAN_PREFIX,
        EnterpriseSuiteAppError::InvalidChangePlanRef,
    )?;
    require_non_empty_key(&input.idempotency_key)?;

    Ok(EnterpriseOpsEnvelope {
        topic: internal(ENTERPRISE_OPS_TOPIC.to_owned()),
...
pub fn prepare_cross_product_workflow_envelope(
    plan: &CrossProductWorkflowPlan,
) -> CrossProductWorkflowEnvelope {
    CrossProductWorkflowEnvelope {
        topic: internal(ENTERPRISE_WORKFLOW_TOPIC.to_owned()),
...
pub fn prepare_incident_rollback_envelope(plan: &IncidentRollbackPlan) -> IncidentRollbackEnvelope {
    IncidentRollbackEnvelope {
        topic: internal(ENTERPRISE_INCIDENT_ROLLBACK_TOPIC.to_owned()),
        tenant_id: internal(plan.tenant_id.value.clone()),
...
fn require_prefixed(
    value: &str,
    prefix: &str,
    error: EnterpriseSuiteAppError,
) -> Result<(), EnterpriseSuiteAppError> {
...
fn require_non_empty_key(value: &str) -> Result<(), EnterpriseSuiteAppError> {
    if value.trim().is_empty() || value.contains("..") || value.chars().any(char::is_whitespace) {
        return Err(EnterpriseSuiteAppError::InvalidIdempotencyKey);
    }
    Ok(())
}
