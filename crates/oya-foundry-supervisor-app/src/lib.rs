...
use oya_foundry_account_domain::check_silent_switch;
use oya_foundry_route_policy_kernel::{RouteConstraints, RoutePolicy};
use oya_foundry_settings_template_adapter::TemplateStore;
use oya_foundry_settings_template_kernel::{DriftState, SettingsRenderer};
    ...
    U: UsageWindowPort,
{
    ...
    pub async fn tick_once(&self, now_epoch_secs: u64) -> Result<TickOutcome, SupervisorError> {
        ...
        let domain_accounts: Vec<oya_foundry_account_domain::ProviderAccount> = accounts
            .iter()
            .map(|a| {
                let mut acc = oya_foundry_account_domain::ProviderAccount::new(
                    a.id.clone(),
                    a.provider_family,
                );
        ...
        let exp = match RoutePolicy::select(&domain_accounts, &constraints) {
            ...
            Err(e) => {
                ...
                return Err(SupervisorError::NoEligibleAccount {
                    chosen: oya_foundry_supervisor_kernel::AccountId(message_id.0.clone()),
        ...
        let driver_tier = oya_foundry_supervisor_kernel::AutonomyTier::T3PropAct; // In real impl, read from driver registry
        ...
        let ticket = SessionTicket {
            ...
            message_id: message_id.clone(),
            request_id: oya_foundry_supervisor_kernel::RequestId(format!("req-{}", now_epoch_secs)),
    ...
    async fn verify_account_settings(
        ...
        let mut domain_acc =
            oya_foundry_account_domain::ProviderAccount::new(acc.id.clone(), acc.provider_family);
