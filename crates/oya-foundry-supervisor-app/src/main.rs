...
use oya_foundry_gemini_account_adapter::GeminiDriver;
use oya_foundry_jsonl_supervisor_adapter::{JsonlInboxStore, JsonlOutboxSink};
use oya_foundry_settings_template_adapter::{MultiProviderRenderer, TemplateStore};
use oya_foundry_supervisor_app::SupervisorApp;
...
use oya_foundry_supervisor_security_adapter::CedarAutonomyCeilingAdapter;
...
impl oya_foundry_supervisor_kernel::SessionDriver for Driver {
    ...
    fn spawn_for_message(
        &self,
        ticket: &oya_foundry_supervisor_kernel::SessionTicket,
    ) -> Result<
        oya_foundry_supervisor_kernel::SpawnedSession,
        oya_foundry_supervisor_kernel::SupervisorError,
    > {
        match self {
    ...
    fn inject_message(
        &self,
        session: &oya_foundry_supervisor_kernel::SpawnedSession,
        msg: &[u8],
    ) -> Result<(), oya_foundry_supervisor_kernel::SupervisorError> {
        match self {
    ...
    fn drain_response(
        &self,
        session: &oya_foundry_supervisor_kernel::SpawnedSession,
    ) -> Result<Vec<u8>, oya_foundry_supervisor_kernel::SupervisorError> {
        match self {
    ...
    fn health_check(&self) -> oya_foundry_supervisor_kernel::DriverHealth {
        match self {
