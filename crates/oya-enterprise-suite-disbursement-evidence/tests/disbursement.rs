use oya_enterprise_suite_disbursement_evidence::{
    DisbursementJurisdiction, DisbursementRailKind, EnterpriseDisbursementEvidenceError,
    disbursement_network_or_authority_urls, enterprise_suite_disbursement_evidence_plan,
    validate_enterprise_suite_disbursement_evidence_plan,
};

#[test]
fn disbursement_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_disbursement_evidence_plan();
    validate_enterprise_suite_disbursement_evidence_plan(&plan)
        .expect("disbursement evidence plan validates");

    assert_eq!(
        plan.plan_name,
        "enterprise-suite-disbursement-evidence-plan"
    );
...
fn disbursement_evidence_plan_rejects_missing_requirements_duplicate_and_network_drift() {
    let mut plan = enterprise_suite_disbursement_evidence_plan();
    plan.requirements.truncate(1);
    assert_eq!(
        validate_enterprise_suite_disbursement_evidence_plan(&plan),
        Err(EnterpriseDisbursementEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_disbursement_evidence_plan(&plan),
        Err(EnterpriseDisbursementEvidenceError::DuplicateRequirement(
    ...
    assert_eq!(
        validate_enterprise_suite_disbursement_evidence_plan(&plan),
        Err(EnterpriseDisbursementEvidenceError::InvalidOfficialNetworkOrAuthorityUrl)
    );
}
...
fn disbursement_evidence_plan_rejects_unsafe_refs_missing_controls_and_runtime_overclaims() {
    ...
    assert_eq!(
        validate_enterprise_suite_disbursement_evidence_plan(&plan),
        Err(EnterpriseDisbursementEvidenceError::InvalidPaymentDigestRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_disbursement_evidence_plan(&plan),
        Err(EnterpriseDisbursementEvidenceError::RuntimeAttachmentOverclaim)
    );
}
