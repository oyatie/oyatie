//! Enterprise Suite Workflow runtime evidence contract.
//!
...
//! child services, export runtime telemetry, emit audit-chain events, or claim
...
use oya_enterprise_suite_workflow_adapter_inmemory::{
    EnterpriseSuiteWorkflowQueueCapabilities, enterprise_suite_workflow_queue_capabilities,
};

const SCHEMA_VERSION: u32 = 1;
const MIN_REQUIREMENT_COUNT: usize = 14;
const PLAN_NAME: &str = "enterprise-suite-workflow-runtime-evidence-plan";
...
const SOURCE_PLAN_REF: &str = "crates/oya-enterprise-suite-workflow-adapter-inmemory/src/lib.rs::enterprise_suite_workflow_queue_capabilities";
...
pub enum WorkflowRuntimeEvidenceRequirementKind {
    ...
    ChildServiceCallBoundaryObserved,
...
pub struct WorkflowRuntimeEvidenceRequirement {
    ...
    pub requires_child_service_boundary: bool, // data_class: PUBLIC
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteWorkflowRuntimeEvidencePlan {
    ...
    pub child_service_boundary_required: bool, // data_class: PUBLIC
    ...
    pub child_service_calls_runtime_attached: bool, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteWorkflowRuntimeEvidenceError {
    InvalidPlanName,
    InvalidServiceName,
...
pub fn enterprise_suite_workflow_runtime_evidence_plan()
-> Result<EnterpriseSuiteWorkflowRuntimeEvidencePlan, EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    let capabilities = enterprise_suite_workflow_queue_capabilities();
    validate_workflow_queue_capabilities(&capabilities)?;

    Ok(EnterpriseSuiteWorkflowRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
        ...
        child_service_boundary_required: true,
        ...
        child_service_calls_runtime_attached: false,
...
pub fn validate_enterprise_suite_workflow_runtime_evidence_plan(
    plan: &EnterpriseSuiteWorkflowRuntimeEvidencePlan,
) -> Result<(), EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidServiceName);
    }
    if plan.substrate_name != SUBSTRATE_NAME {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidSubstrateName);
    }
    if plan.tenant_namespace != TENANT_NAMESPACE {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidTenantNamespace);
    }
    if plan.workflow_queue_adapter != WORKFLOW_QUEUE_ADAPTER {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidWorkflowQueueAdapter);
    }
    if plan.requirements.len() < MIN_REQUIREMENT_COUNT || plan.schema_version != SCHEMA_VERSION {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::MissingRequirements);
    }
...
fn runtime_requirements() -> Vec<WorkflowRuntimeEvidenceRequirement> {
    vec![
        requirement(
            ...
            "evidence/workflow-runtime/enterprise-suite/workflow-definition-version.json",
        ),
        requirement(
            ...
            "evidence/workflow-runtime/enterprise-suite/deterministic-gate-set.json",
        ),
        requirement(
            ...
            "evidence/workflow-runtime/enterprise-suite/dispatch-idempotency.json",
        ),
        requirement(
            ...
            "workflow-execution",
            "https://serverlessworkflow.io/",
            "evidence/workflow-runtime/enterprise-suite/execution-state-transition.json",
        ),
        requirement(
            ...
            "evidence/workflow-runtime/enterprise-suite/durable-queue-ack.json",
        ),
        requirement(
            ...
            "broker-publish",
            "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
            "evidence/workflow-runtime/enterprise-suite/broker-publish-confirmed.json",
        ),
        requirement(
            ...
            "broker-delivery",
            "https://serverlessworkflow.io/",
            "evidence/workflow-runtime/enterprise-suite/broker-delivery-retry.json",
        ),
        requirement(
            ...
            "broker-delivery",
            "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
            "evidence/workflow-runtime/enterprise-suite/dead-letter-route.json",
        ),
        requirement(
            ...
            "tenant-partition",
            "https://cloudevents.io/",
            "evidence/workflow-runtime/enterprise-suite/tenant-partition.json",
        ),
        requirement(
            ...
            "payload-integrity",
            "https://cloudevents.io/",
            "evidence/workflow-runtime/enterprise-suite/payload-digest.json",
        ),
        requirement(
            "child-service-call-boundary-observed",
            WorkflowRuntimeEvidenceRequirementKind::ChildServiceCallBoundaryObserved,
            "child-service-boundary",
            "https://serverlessworkflow.io/",
            "evidence/workflow-runtime/enterprise-suite/child-service-boundary.json",
        ),
        requirement(
            ...
            "evidence/workflow-runtime/enterprise-suite/otel-messaging-trace.json",
        ),
        requirement(
            ...
            "https://cloudevents.io/",
            "evidence/workflow-runtime/enterprise-suite/workflow-audit-event.json",
        ),
        requirement(
            ...
            "workflow-replay",
            "https://serverlessworkflow.io/",
            "evidence/workflow-runtime/enterprise-suite/replay-recovery.json",
        ),
    ]
}
...
fn requirement(
    ...
    let requires_child_service_boundary = matches!(
        requirement_kind,
        WorkflowRuntimeEvidenceRequirementKind::ChildServiceCallBoundaryObserved
    );
    ...
    WorkflowRuntimeEvidenceRequirement {
        ...
        requires_child_service_boundary,
...
fn validate_workflow_queue_capabilities(
    capabilities: &EnterpriseSuiteWorkflowQueueCapabilities,
) -> Result<(), EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    if capabilities.adapter != WORKFLOW_QUEUE_ADAPTER
        ...
        || capabilities.child_service_calls_attached
        || capabilities.audit_chain_emission_attached
        || capabilities.schema_version != SCHEMA_VERSION
    {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::WorkflowQueueCapabilityDrift);
    }
    Ok(())
}
...
fn validate_required_controls(
    ...
    for control in [
        ...
        (
            plan.child_service_boundary_required,
            "child_service_boundary_required",
        ),
        (
...
fn validate_nonclaims(
    ...
    if plan.workflow_engine_runtime_attached
        ...
        || plan.child_service_calls_runtime_attached
    ...
    {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_runtime_requirements(
    ...
    for requirement in &plan.requirements {
        ...
        if !seen_requirements.insert(requirement.requirement_id) {
            return Err(
                EnterpriseSuiteWorkflowRuntimeEvidenceError::DuplicateRequirement(
    ...
    for kind in required_requirement_kinds() {
        if !seen_kinds.contains(&kind) {
            return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::MissingRequirementKind(kind));
        }
    }
    Ok(())
}
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidRequirementId,
    )?;
    ...
    validate_prefixed_ref(
        ...
        "evidence/workflow-runtime/enterprise-suite/",
        EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.source_plan_ref,
        "crates/oya-enterprise-suite-workflow-adapter-inmemory/",
        EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidSourcePlanRef,
    )?;
    ...
    if matches!(
        ...
        require_control(
            requirement.requires_child_service_boundary,
            "requirement_requires_child_service_boundary",
        )?;
    }
    if matches!(
        requirement.requirement_kind,
...
fn validate_slug(
    value: &str,
    error: EnterpriseSuiteWorkflowRuntimeEvidenceError,
) -> Result<(), EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    if value.is_empty()
...
fn validate_workload_scope(value: &str) -> Result<(), EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    if value.is_empty() || has_unsafe_text(value) || has_path_traversal(value) {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidWorkloadScope);
    }
    Ok(())
}

fn validate_doc_url(url: &str) -> Result<(), EnterpriseSuiteWorkflowRuntimeEvidenceError> {
    let allowed = [
        "https://serverlessworkflow.io/",
        "https://github.com/serverlessworkflow/specification",
        "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
    ...
    if !allowed.contains(&url) {
        return Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn require_control(
    ...
    if enabled {
        ...
        Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::MissingRequiredControl(field))
    }
}
