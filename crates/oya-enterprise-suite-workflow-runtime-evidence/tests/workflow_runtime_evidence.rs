use oya_enterprise_suite_workflow_runtime_evidence::{
    EnterpriseSuiteWorkflowRuntimeEvidenceError, WorkflowRuntimeEvidenceRequirementKind,
    enterprise_suite_workflow_runtime_evidence_plan,
    validate_enterprise_suite_workflow_runtime_evidence_plan, workflow_runtime_evidence_doc_urls,
};

#[test]
fn workflow_runtime_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_workflow_runtime_evidence_plan().expect("plan builds");
    validate_enterprise_suite_workflow_runtime_evidence_plan(&plan).expect("plan validates");

    assert_eq!(
        plan.plan_name,
        "enterprise-suite-workflow-runtime-evidence-plan"
    );
    ...
    assert!(plan.child_service_boundary_required);
    ...
    assert!(!plan.child_service_calls_runtime_attached);
...
fn workflow_runtime_evidence_plan_preserves_ref_boundaries_and_inmemory_source_contract() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            .starts_with("evidence/workflow-runtime/enterprise-suite/")
            && requirement
                .source_plan_ref
                .starts_with("crates/oya-enterprise-suite-workflow-adapter-inmemory/")
...
fn workflow_runtime_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = enterprise_suite_workflow_runtime_evidence_plan().expect("plan builds");
    plan.requirements.truncate(3);
    assert_eq!(
        validate_enterprise_suite_workflow_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_workflow_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidOfficialDocUrl)
    );
}
...
fn workflow_runtime_evidence_plan_rejects_unsafe_refs_missing_controls_and_overclaims() {
    ...
    plan.requirements[0].expected_evidence_ref =
        "evidence/workflow-runtime/enterprise-suite/password-material";
    assert_eq!(
        validate_enterprise_suite_workflow_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::InvalidExpectedEvidenceRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_workflow_runtime_evidence_plan(&plan),
        Err(
            EnterpriseSuiteWorkflowRuntimeEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_suite_workflow_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteWorkflowRuntimeEvidenceError::RuntimeAttachmentOverclaim)
    );
}
