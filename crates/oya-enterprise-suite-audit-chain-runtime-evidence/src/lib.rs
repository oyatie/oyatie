//! Enterprise Suite audit-chain runtime evidence contract.
//!
...
//! existing Enterprise Suite audit-chain emission plan. It does not attach a
...
use oya_enterprise_suite_audit_chain_emission::{
    EnterpriseAuditChainEmissionError, EnterpriseAuditChainEmissionPlan,
    enterprise_suite_audit_chain_emission_plan,
    validate_enterprise_suite_audit_chain_emission_plan,
};

const SCHEMA_VERSION: u32 = 1;
const MIN_REQUIREMENT_COUNT: usize = 15;
const PLAN_NAME: &str = "enterprise-suite-audit-chain-runtime-evidence-plan";
...
const SOURCE_PLAN_REF: &str = "crates/oya-enterprise-suite-audit-chain-emission/src/lib.rs::enterprise_suite_audit_chain_emission_plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteAuditChainRuntimeEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteAuditChainRuntimeEvidenceError {
...
pub fn enterprise_suite_audit_chain_runtime_evidence_plan() -> Result<
    EnterpriseSuiteAuditChainRuntimeEvidencePlan,
    EnterpriseSuiteAuditChainRuntimeEvidenceError,
> {
    let emission_plan = enterprise_suite_audit_chain_emission_plan();
    validate_enterprise_suite_audit_chain_emission_plan(&emission_plan)
        .map_err(EnterpriseSuiteAuditChainRuntimeEvidenceError::AuditChainEmission)?;

    Ok(EnterpriseSuiteAuditChainRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
...
pub fn validate_enterprise_suite_audit_chain_runtime_evidence_plan(
    plan: &EnterpriseSuiteAuditChainRuntimeEvidencePlan,
) -> Result<(), EnterpriseSuiteAuditChainRuntimeEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidServiceName);
    }
    if plan.substrate_name != SUBSTRATE_NAME {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidSubstrateName);
    }
    if plan.tenant_namespace != TENANT_NAMESPACE {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidTenantNamespace);
    }
    if plan.tenant_partition != TENANT_PARTITION {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidTenantPartition);
    }
    if plan.audit_chain_emission_plan_name != "enterprise-suite-audit-chain-emission" {
        return Err(
            EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidAuditChainEmissionPlanName,
        );
    }
    if plan.outbox_topic != OUTBOX_TOPIC || has_unsafe_text(plan.outbox_topic) {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidOutboxTopic);
    }
    ...
    {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::MissingRequirements);
    }
...
fn runtime_requirements(
    emission_plan: &EnterpriseAuditChainEmissionPlan,
    ...
    vec![
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/cloudevent-envelope.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/trace-context.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/otel-log-record.json",
            emission_plan,
        ),
        requirement(
            ...
            "tenant-partition",
            "https://cloudevents.io/",
            "evidence/audit-chain-runtime/enterprise-suite/tenant-partition.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/idempotency-dedupe.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/payload-digest.json",
            emission_plan,
        ),
        requirement(
            ...
            "payload-redaction",
            "https://opentelemetry.io/docs/specs/otel/logs/data-model/",
            "evidence/audit-chain-runtime/enterprise-suite/sensitive-payload-redaction.json",
            emission_plan,
        ),
        requirement(
            ...
            "write-ahead-log",
            "https://cloudevents.io/",
            "evidence/audit-chain-runtime/enterprise-suite/wal-append.json",
            emission_plan,
        ),
        requirement(
            ...
            "broker-outbox",
            "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
            "evidence/audit-chain-runtime/enterprise-suite/outbox-publish.json",
            emission_plan,
        ),
        requirement(
            "broker-ack-observed",
            ...
            "broker-delivery",
            "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
            "evidence/audit-chain-runtime/enterprise-suite/broker-ack.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/merkle-leaf.json",
            emission_plan,
        ),
        requirement(
            "merkle-root-sealed",
            ...
            "merkle-root",
            "https://www.rfc-editor.org/rfc/rfc9162",
            "evidence/audit-chain-runtime/enterprise-suite/merkle-root-seal.json",
            emission_plan,
        ),
        requirement(
            ...
            "evidence/audit-chain-runtime/enterprise-suite/sink-ingestion.json",
            emission_plan,
        ),
        requirement(
            ...
            "replay-recovery",
            "https://www.asyncapi.com/docs/reference/specification/v3.0.0",
            "evidence/audit-chain-runtime/enterprise-suite/replay-recovery.json",
            emission_plan,
        ),
        requirement(
            ...
            "failure-path",
            "https://opentelemetry.io/docs/specs/semconv/general/events/",
            "evidence/audit-chain-runtime/enterprise-suite/failure-path-audit.json",
            emission_plan,
        ),
    ]
}
...
fn validate_nonclaims(
    ...
    {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_runtime_requirements(
    ...
    for requirement in &plan.requirements {
        ...
        if !seen_requirements.insert(requirement.requirement_id) {
            return Err(
                EnterpriseSuiteAuditChainRuntimeEvidenceError::DuplicateRequirement(
    ...
    for kind in required_requirement_kinds() {
        if !seen_kinds.contains(&kind) {
            return Err(
                EnterpriseSuiteAuditChainRuntimeEvidenceError::MissingRequirementKind(kind),
            );
        }
    }
    Ok(())
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidRequirementId,
    )?;
    ...
    validate_prefixed_ref(
        ...
        "evidence/audit-chain-runtime/enterprise-suite/",
        EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.source_plan_ref,
        "crates/oya-enterprise-suite-audit-chain-emission/",
        EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidSourcePlanRef,
    )?;
...
fn validate_slug(
    value: &str,
    error: EnterpriseSuiteAuditChainRuntimeEvidenceError,
) -> Result<(), EnterpriseSuiteAuditChainRuntimeEvidenceError> {
    if value.is_empty()
...
fn validate_event_scope(value: &str) -> Result<(), EnterpriseSuiteAuditChainRuntimeEvidenceError> {
    if value.is_empty() || has_unsafe_text(value) || has_path_traversal(value) {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidEventScope);
    }
    Ok(())
}

fn validate_doc_url(url: &str) -> Result<(), EnterpriseSuiteAuditChainRuntimeEvidenceError> {
    let allowed = [
        "https://cloudevents.io/",
        "https://www.w3.org/TR/trace-context/",
        "https://opentelemetry.io/docs/specs/otel/logs/data-model/",
    ...
    if !allowed.contains(&url) {
        return Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn require_control(
    ...
    if enabled {
        ...
        Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::MissingRequiredControl(field))
    }
}
