use oya_enterprise_suite_audit_chain_runtime_evidence::{
    AuditChainRuntimeEvidenceRequirementKind, EnterpriseSuiteAuditChainRuntimeEvidenceError,
    audit_chain_runtime_evidence_doc_urls, enterprise_suite_audit_chain_runtime_evidence_plan,
...
fn audit_chain_runtime_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_audit_chain_runtime_evidence_plan().expect("plan builds");
    validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan).expect("plan validates");

    assert_eq!(
        plan.plan_name,
        "enterprise-suite-audit-chain-runtime-evidence-plan"
    );
    ...
    assert_eq!(
        ...
        "enterprise-suite-audit-chain-emission"
    );
...
fn audit_chain_runtime_evidence_plan_preserves_ref_boundaries_and_source_contract() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            .starts_with("evidence/audit-chain-runtime/enterprise-suite/")
            && requirement
                .source_plan_ref
                .starts_with("crates/oya-enterprise-suite-audit-chain-emission/")
...
fn audit_chain_runtime_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = enterprise_suite_audit_chain_runtime_evidence_plan().expect("plan builds");
    plan.requirements.truncate(4);
    assert_eq!(
        validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidOfficialDocUrl)
    );
}
...
fn audit_chain_runtime_evidence_plan_rejects_unsafe_refs_missing_controls_and_overclaims() {
    ...
    plan.requirements[0].expected_evidence_ref =
        "evidence/audit-chain-runtime/enterprise-suite/client_secret";
    assert_eq!(
        validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::InvalidExpectedEvidenceRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan),
        Err(
            EnterpriseSuiteAuditChainRuntimeEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_suite_audit_chain_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteAuditChainRuntimeEvidenceError::RuntimeAttachmentOverclaim)
    );
}
