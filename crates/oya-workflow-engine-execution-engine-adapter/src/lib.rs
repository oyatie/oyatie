...
impl RetryPolicyEvaluator for WorkflowExecutionMemoryAdapter {
    ...
    ) -> Result<Option<u64>, oya_workflow_engine_execution_engine_usecase::ExecutionEngineKernelError>
    {
        if !is_safe_retry(attempt) || attempt.attempt == 0 {
            return Err(
                oya_workflow_engine_execution_engine_usecase::ExecutionEngineKernelError::UnsafeMetadata,
            );
        }
