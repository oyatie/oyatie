...
use oya_shared_scim_server_kernel::{
    CounterIdGen, Email, FilterExpr, Group, GroupMembership, InMemoryGroupStore,
    InMemoryUserStore, ListQuery, NewGroup, NewUser, PatchOp, PatchOpKind, PatchOperation,
    ReferenceScimServer, ScimError, ScimId, ScimServer, ScimType, TenantId, User, UserName,
    parse_filter,
};
use serde_json::json;
...
fn new_user(name: &str) -> NewUser {
    NewUser {
        ...
        name: Some(UserName { given_name: Some("First".into()), family_name: Some("Last".into()), ..Default::default() }),
        display_name: Some(name.into()),
        active: true,
        emails: vec![Email { value: format!("{name}@example.com"), r#type: Some("work".into()), primary: Some(true), display: None }],
        enterprise: None,
        oyatie: None,
    }
}
...
fn create_then_get_user_roundtrips() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("create");
...
fn create_user_username_uniqueness() {
    let s = srv();
    s.create_user(&tenant(), new_user("bob"), 1_700_000_000).expect("create1");
    let err = s.create_user(&tenant(), new_user("bob"), 1_700_000_001).unwrap_err();
...
fn list_users_paginates() {
    let s = srv();
    for i in 0..5 {
        s.create_user(&tenant(), new_user(&format!("user{i}")), 1_700_000_000 + i as i64).expect("create");
    }
    let page1 = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 2, filter: None }).expect("list");
    assert_eq!(page1.total_results, 5);
    assert_eq!(page1.resources.len(), 2);
    assert_eq!(page1.start_index, 1);
    let page2 = s.list_users(&tenant(), &ListQuery { start_index: 3, items_per_page: 2, filter: None }).expect("list");
...
fn list_users_caps_items_per_page() {
    ...
    let r = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 9_999, filter: None }).expect("list");
...
fn patch_user_replaces_active() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("carol"), 1_700_000_000).expect("create");
    let patched = s
        .patch_user(
            &tenant(),
            &u.id,
...
fn patch_user_unknown_path_returns_invalidpath() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("dave"), 1_700_000_000).expect("create");
    let err = s
        .patch_user(
            &tenant(),
            &u.id,
...
fn delete_user_then_get_404s() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("eve"), 1_700_000_000).expect("create");
...
fn delete_unknown_user_404s() {
    let s = srv();
    let err = s.delete_user(&tenant(), &ScimId("missing".into())).unwrap_err();
    assert_eq!(err.status, 404);
}
...
fn create_group_then_patch_members() {
    let s = srv();
    let g = s
        .create_group(&tenant(), NewGroup { display_name: "engineers".into(), members: vec![] }, 1_700_000_000)
        .expect("create");
    let alice = s.create_user(&tenant(), new_user("alice"), 1_700_000_001).expect("u");
    let patched = s
        .patch_group(
            &tenant(),
            &g.id,
...
fn replace_user_preserves_created_timestamp() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("frank"), 1_700_000_000).expect("create");
    let original_created = u.meta.created.clone();
    let mut nu = new_user("frank");
    nu.display_name = Some("Frank Renamed".into());
    let replaced = s.replace_user(&tenant(), &u.id, nu, 1_700_000_500).expect("replace");
...
fn filter_eq_matches_username() {
    let s = srv();
    s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("c");
    s.create_user(&tenant(), new_user("bob"), 1_700_000_001).expect("c");
    let r = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 100, filter: Some(r#"userName eq "alice""#.into()) }).expect("list");
...
fn filter_co_matches_substring() {
    ...
    s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("c");
    s.create_user(&tenant(), new_user("bob"), 1_700_000_001).expect("c");
    let r = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 100, filter: Some(r#"userName co "li""#.into()) }).expect("list");
    assert_eq!(r.total_results, 1);
}
...
fn filter_and_combines_conditions() {
    let s = srv();
    s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("c");
    s.create_user(&tenant(), new_user("alex"), 1_700_000_001).expect("c");
    s.create_user(&tenant(), new_user("bob"), 1_700_000_002).expect("c");
    let r = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 100, filter: Some(r#"userName sw "al" and active eq "true""#.into()) }).expect("list");
...
fn filter_pr_present() {
    let s = srv();
    s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("c");
    let r = s.list_users(&tenant(), &ListQuery { start_index: 1, items_per_page: 100, filter: Some("userName pr".into()) }).expect("list");
    assert_eq!(r.total_results, 1);
}
...
fn group_create_requires_displayname() {
    let s = srv();
    let err = s.create_group(&tenant(), NewGroup { display_name: String::new(), members: vec![] }, 1_700_000_000).unwrap_err();
    assert_eq!(err.status, 400);
}
...
fn patch_remove_email_by_value() {
    let s = srv();
    let u = s.create_user(&tenant(), new_user("alice"), 1_700_000_000).expect("c");
    let p = s.patch_user(&tenant(), &u.id, &PatchOp {
        schemas: vec![PatchOp::SCHEMA.to_owned()],
        operations: vec![PatchOperation {
...
fn tenant_isolation_users_dont_leak_across_tenants() {
    ...
    s.create_user(&t1, new_user("alice"), 1_700_000_000).expect("c");
    s.create_user(&t2, new_user("alice"), 1_700_000_000).expect("c"); // same userName different tenant OK
...
fn group_membership_query_does_not_break_users() {
    let s = srv();
    let g = s.create_group(&tenant(), NewGroup { display_name: "team".into(), members: vec![GroupMembership { value: ScimId("u1".into()), display: None, r#ref: None }] }, 1_700_000_000).expect("g");
