use oya_tenant_rbac_postgres_rls_storage::{
    TenantRbacPostgresRlsStorageError, render_tenant_rbac_postgres_rls_migration,
    tenant_rbac_postgres_rls_storage_plan, validate_tenant_rbac_postgres_rls_storage_plan,
};

#[test]
fn postgres_rls_storage_plan_covers_platform_tables_with_tenant_rls() {
    let plan = tenant_rbac_postgres_rls_storage_plan();
    validate_tenant_rbac_postgres_rls_storage_plan(&plan).expect("plan validates");
...
fn postgres_rls_storage_plan_renders_review_only_migration_sql() {
    let plan = tenant_rbac_postgres_rls_storage_plan();
    let sql = render_tenant_rbac_postgres_rls_migration(&plan).expect("migration renders");

    assert!(sql.contains("CREATE SCHEMA IF NOT EXISTS tenant_rbac"));
    assert!(sql.contains("CREATE TABLE IF NOT EXISTS tenant_rbac.tenant_rbac_policy_admissions"));
    assert!(sql.contains("PRIMARY KEY (tenant_id, idempotency_key)"));
    assert!(sql.contains(
        "ALTER TABLE tenant_rbac.tenant_rbac_policy_admissions ENABLE ROW LEVEL SECURITY"
    ));
    assert!(sql.contains(
        "ALTER TABLE tenant_rbac.tenant_rbac_policy_admissions FORCE ROW LEVEL SECURITY"
    ));
    assert!(sql.contains("CREATE POLICY tenant_rbac_policy_admissions_tenant_rls"));
...
fn postgres_rls_storage_plan_rejects_missing_force_rls_or_tenant_column() {
    let mut plan = tenant_rbac_postgres_rls_storage_plan();
    plan.tables[0].force_row_level_security = false;
    assert_eq!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::InvalidTable)
    );
    ...
    assert_eq!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::MissingTenantColumn)
    );
}
...
fn postgres_rls_storage_plan_rejects_delete_policy_and_runtime_overclaims() {
    ...
    assert_eq!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::DeletePolicyForbidden)
    );
    ...
    assert_eq!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::RuntimeAttachmentOverclaim)
    );
}
...
fn postgres_rls_storage_plan_rejects_duplicate_tables_and_bad_idempotency_scope() {
    ...
    assert!(matches!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::DuplicateTable(_))
    ));
    ...
    assert_eq!(
        validate_tenant_rbac_postgres_rls_storage_plan(&plan),
        Err(TenantRbacPostgresRlsStorageError::InvalidIdempotencyScope)
    );
}
