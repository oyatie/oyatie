//! `oya-foundry-subagent-runtime-kernel` — port-in-kernel substrate for
//! the per-facet subagent invocation that closes the
//! `subagent_runtime_pending=true` gap left by
//! `tools/oya-foundry-pr-review-dispatcher-app` (IP-004),
//! `tools/oya-foundry-vcs-ci-fix-loop-dispatcher-app` (IP-005), and
//! `tools/oya-foundry-vcs-merge-queue-fix-loop-app` (IP-006).
//!
//! ## What lives here
//!
...
//!   `tools/oya-foundry-subagent-runtime-app/` binary (one ring out)
...
/// `specs/cross-cutting/multispectrum-review.json#consensus_debate_protocol`.
...
/// by IP-004's `tools/oya-foundry-pr-review-dispatcher-app::parse_recommendation`.
/// The shape matches `consensus_debate_protocol.rounds.round_1_independent.required_keys`
/// from `specs/cross-cutting/multispectrum-review.json`.
///
