//! Enterprise Suite Postgres/RLS parameterized write contract.
//!
//! This review-only crate defines the static SQL statement contract that later
//! durable Postgres adapters must use when Enterprise Suite metadata leaves the
...
use oya_enterprise_suite_postgres_rls_storage::{
    EnterprisePostgresRecordKind, EnterprisePostgresRlsStorageError,
    EnterprisePostgresRlsStoragePlan, EnterprisePostgresRlsTable,
    enterprise_postgres_rls_storage_plan, validate_enterprise_postgres_rls_storage_plan,
};

const SCHEMA_VERSION: u32 = 1;
const MIN_STATEMENT_COUNT: usize = 5;
const CONTRACT_NAME: &str = "enterprise-suite-postgres-rls-write-contract";
...
const SOURCE_STORAGE_PLAN_REF: &str = "crates/oya-enterprise-suite-postgres-rls-storage/src/lib.rs::enterprise_postgres_rls_storage_plan";
...
pub struct EnterprisePostgresRlsWriteStatement {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterprisePostgresRlsWriteContract {
    ...
    pub statements: Vec<EnterprisePostgresRlsWriteStatement>, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterprisePostgresRlsWriteContractError {
    StoragePlan(EnterprisePostgresRlsStorageError),
    InvalidContractName,
    InvalidServiceName,
    InvalidSchemaName,
    InvalidRuntimeRole,
...
pub fn enterprise_postgres_rls_write_contract()
-> Result<EnterprisePostgresRlsWriteContract, EnterprisePostgresRlsWriteContractError> {
    let storage_plan = enterprise_postgres_rls_storage_plan();
    validate_enterprise_postgres_rls_storage_plan(&storage_plan)
        .map_err(EnterprisePostgresRlsWriteContractError::StoragePlan)?;

    Ok(EnterprisePostgresRlsWriteContract {
...
pub fn validate_enterprise_postgres_rls_write_contract(
    contract: &EnterprisePostgresRlsWriteContract,
) -> Result<(), EnterprisePostgresRlsWriteContractError> {
    let storage_plan = enterprise_postgres_rls_storage_plan();
    validate_enterprise_postgres_rls_storage_plan(&storage_plan)
        .map_err(EnterprisePostgresRlsWriteContractError::StoragePlan)?;
    ...
    validate_slug(
        ...
        EnterprisePostgresRlsWriteContractError::InvalidContractName,
    )?;
    if contract.service_name != SERVICE_NAME {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidServiceName);
    }
    if contract.schema_name != SCHEMA_NAME {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidSchemaName);
    }
    if contract.runtime_role != RUNTIME_ROLE {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidRuntimeRole);
    }
    if contract.tenant_context_setting != TENANT_CONTEXT_SETTING {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidTenantContextSetting);
    }
    if contract.storage_plan_table_count != storage_plan.tables.len() {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidStoragePlanTableCount);
    }
    if contract.statements.len() < MIN_STATEMENT_COUNT || contract.schema_version != SCHEMA_VERSION
    {
        return Err(EnterprisePostgresRlsWriteContractError::MissingStatements);
    }
...
fn write_statement(
    storage_plan: &EnterprisePostgresRlsStoragePlan,
    table: &EnterprisePostgresRlsTable,
) -> EnterprisePostgresRlsWriteStatement {
...
fn validate_nonclaims(
    ...
    {
        return Err(EnterprisePostgresRlsWriteContractError::RuntimeAttachmentOverclaim);
    }
    Ok(())
}
...
fn validate_statements(
    ...
    for statement in &contract.statements {
        ...
        if !seen_statements.insert(statement.table_name) {
            return Err(EnterprisePostgresRlsWriteContractError::DuplicateStatement(
    ...
    for table_name in storage_tables {
        if !seen_statements.contains(table_name) {
            return Err(
                EnterprisePostgresRlsWriteContractError::MissingStoragePlanTable(
                    table_name.to_owned(),
                ),
            );
        }
...
fn validate_statement(
    ...
    if !storage_tables.contains(statement.table_name) || !valid_identifier(statement.table_name) {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidTableName);
    }
    ...
    if statement.tenant_context_sql != "SET LOCAL app.tenant_id = $1" {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidTenantContextSql);
    }
    ...
    {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidParameterOrder);
    }
    for url in &statement.official_doc_urls {
    ...
    validate_prefixed_ref(
        ...
        "crates/oya-enterprise-suite-postgres-rls-storage/",
        EnterprisePostgresRlsWriteContractError::InvalidSourceStoragePlanRef,
    )?;
    for control in [
        (
...
fn validate_insert_sql(
    ...
    let expected_prefix = format!("INSERT INTO enterprise_suite.{} ", statement.table_name);
    if !statement.insert_sql.starts_with(&expected_prefix)
        ...
        || statement
            ...
            .contains(" DELETE ")
    {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidInsertSql);
    }
    Ok(())
}
...
fn validate_select_sql(
    ...
    let expected_from = format!(" FROM enterprise_suite.{} ", statement.table_name);
    if !statement
        ...
        || statement
            ...
            .contains(" DELETE ")
    {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidSelectSql);
    }
    Ok(())
}

fn validate_doc_url(url: &str) -> Result<(), EnterprisePostgresRlsWriteContractError> {
    if !matches!(
        url,
        INSERT_DOC_URL | SET_DOC_URL | RLS_DOC_URL | LIBPQ_DOC_URL
    ) {
        return Err(EnterprisePostgresRlsWriteContractError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn validate_slug(
    value: &str,
    error: EnterprisePostgresRlsWriteContractError,
) -> Result<(), EnterprisePostgresRlsWriteContractError> {
    if value.is_empty()
...
fn validate_sql_text(sql: &str) -> Result<(), EnterprisePostgresRlsWriteContractError> {
    if sql.trim() != sql
        || sql.is_empty()
        || sql.contains(';')
        || sql.contains("--")
...
fn require_control(
    ...
    if value {
        ...
        Err(EnterprisePostgresRlsWriteContractError::MissingRequiredControl(control))
    }
}
