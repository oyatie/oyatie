use oya_enterprise_suite_postgres_rls_storage::EnterprisePostgresRecordKind;
use oya_enterprise_suite_postgres_rls_write_contract::{
    ...
    postgres_rls_write_contract_doc_urls, validate_enterprise_postgres_rls_write_contract,
};

#[test]
fn postgres_rls_write_contract_validates_controls_and_nonclaims() {
    let contract = enterprise_postgres_rls_write_contract().expect("contract builds");
    validate_enterprise_postgres_rls_write_contract(&contract).expect("contract validates");

    assert_eq!(
        contract.contract_name,
        "enterprise-suite-postgres-rls-write-contract"
    );
...
fn postgres_rls_write_contract_covers_storage_plan_tables_and_docs() {
    ...
    assert!(contract.statements.iter().any(|statement| {
        statement.record_kind == EnterprisePostgresRecordKind::CrossProductWorkflowPlan
            && statement.table_name == "enterprise_cross_product_workflow_plans"
    }));
...
fn postgres_rls_write_contract_rejects_missing_duplicate_and_doc_drift() {
    let mut contract = enterprise_postgres_rls_write_contract().expect("contract builds");
    contract.statements.truncate(2);
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(EnterprisePostgresRlsWriteContractError::MissingStatements)
    );
    ...
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(EnterprisePostgresRlsWriteContractError::DuplicateStatement(
    ...
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(EnterprisePostgresRlsWriteContractError::InvalidOfficialDocUrl)
    );
}
...
fn postgres_rls_write_contract_rejects_unparameterized_sql_missing_controls_and_overclaims() {
    ...
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(EnterprisePostgresRlsWriteContractError::InvalidInsertSql)
    );
    ...
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(
            EnterprisePostgresRlsWriteContractError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_postgres_rls_write_contract(&contract),
        Err(EnterprisePostgresRlsWriteContractError::RuntimeAttachmentOverclaim)
    );
}
