...
const SUBNET_ID: &str = "oya:cloud:region-home:ten_kr:subnet:prod-a";
const LB_ID: &str = "oya:cloud:region-home:ten_kr:lb-v7:frontdoor";
...
fn lb_body(resource_id: &str) -> CloudNetworkLbCreateRequest {
    CloudNetworkLbCreateRequest {
        ...
        listeners: vec![CloudNetworkLbListenerCreateRequest {
            ...
            tls_certificate: Some("cert/region-home/ten_kr/frontdoor".to_string()),
        }],
        ...
        mtls: Some(CloudNetworkLbMtlsConfigCreateRequest {
            ca_bundle_ref: "cert/region-home/ten_kr/mesh-ca".to_string(),
...
fn create_request(request_id: &str, idempotency_key: &str) -> CloudNetworkLbCreateApiRequest {
    CloudNetworkLbCreateApiRequest {
        ...
        principal: principal_for("sp_network_lb_admin"),
        authorization: authorization_for("sp_network_lb_admin", &[CLOUD_NETWORK_LB_CREATE_SURFACE]),
        body: lb_body(LB_ID),
    }
}
...
fn lb_create_api_creates_l7_grpc_once_and_replays_same_idempotent_result() {
    ...
    let first =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, request.clone())
            .expect("authorized load balancer create succeeds");
    let second = create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, request)
...
fn lb_create_api_rejects_path_body_lb_drift_before_catalog_mutation() {
    ...
    request.body.resource_id = "oya:cloud:region-home:ten_kr:lb-v7:other".to_string();
    ...
    assert_eq!(
        ...
        CloudNetworkLbApiError::LoadBalancerIdMismatch {
            path_load_balancer_id: LB_ID.to_string(),
            body_resource_id: "oya:cloud:region-home:ten_kr:lb-v7:other".to_string(),
        }
    );
...
#[test]
fn lb_create_api_separates_missing_authentication_from_denied_authorization() {
    let mut catalog = CloudNetworkCatalog::default();
    seed_network(&mut catalog);
    let mut ledger = CloudNetworkLbCreateIdempotencyLedger::default();
    ...
    let authn_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, unauthenticated)
    ...
    let mut denied = create_request("req-network-lb-authz", "idem-network-lb-authz");
    denied.authorization.allowed_surfaces = vec!["cloud.network.vpc.create".to_string()];
    let authz_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, denied)
            .expect_err("authorization decision excludes LB create");
    ...
    assert_eq!(authz_error.lb_create_status_code(), 403);
    assert!(ledger.is_empty());
    assert_eq!(catalog.load_balancers().count(), 0);
}
...
fn lb_create_api_rejects_reused_idempotency_key_with_new_fingerprint() {
    ...
    assert_eq!(
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, drifted),
...
fn lb_create_api_maps_unknown_vpc_and_subnet_targets() {
    ...
    let vpc_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, unknown_vpc)
    ...
    let subnet_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, missing_subnet)
...
fn lb_create_api_maps_l7_tls_and_grpc_mtls_invariants() {
    ...
    let tls_error = create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, no_tls)
    ...
    let mtls_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, no_mtls)
...
fn lb_create_api_rejects_invalid_kind_policy_and_data_class() {
    ...
    let kind_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, invalid_kind)
    ...
    let policy_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, invalid_policy)
    ...
    let class_error =
        create_cloud_network_load_balancer_from_api(&mut catalog, &mut ledger, invalid_class)
