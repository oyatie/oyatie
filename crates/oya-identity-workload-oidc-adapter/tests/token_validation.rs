...
fn genuine_workload_token_validates_into_active_principal() {
    ...
    let claims = format!(
        r#"{{"iss":"https://idp.oyatie.dev","aud":"oya-cloud-storage","exp":{},"iat":{},"tenant_id":"ten_globex","sub":"wl_backup_agent","owning_capability":"cap.cloud.storage","scope":"storage.object.read storage.object.list"}}"#,
        now + 600,
        now
    );
    let (token, jwk) = mint(&claims, "globex-2026");
    let jwks = Jwks::new().add_key(jwk);
    let config = ValidationConfig::new("https://idp.oyatie.dev", "oya-cloud-storage");
...
fn token_signed_by_untrusted_key_is_rejected() {
    let now: i64 = 1_750_000_000;
    let claims = format!(
        r#"{{"iss":"https://idp.oyatie.dev","aud":"oya-cloud-storage","exp":{},"tenant_id":"ten_globex","sub":"wl_x","owning_capability":"cap.cloud.storage"}}"#,
        now + 600
    );
