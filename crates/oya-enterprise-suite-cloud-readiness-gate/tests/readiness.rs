use oya_enterprise_suite_cloud_readiness_gate::{
    CloudReadinessBlocker, CloudReadinessGateError, enterprise_suite_cloud_readiness_report,
    required_cloud_blockers, validate_cloud_claim,
};

#[test]
fn cloud_readiness_gate_reports_local_rehearsal_ready_but_cloud_blocked() {
    let report = enterprise_suite_cloud_readiness_report().expect("readiness report builds");
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-idp-verification-1779553800.json")
    );
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-idp-runtime-evidence-1779703200.json")
    );
    assert!(report.evidence_refs.contains(
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-postgres-tx-contract-1779706200.json")
    );
    assert!(
        report.evidence_refs.contains(
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-listener-gateway-1779553200.json")
    );
    assert!(report.evidence_refs.contains(
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-slo-evidence-1779664200.json")
    );
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-workflow-execution-1779664800.json")
    );
    assert!(report.evidence_refs.contains(
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-disbursement-evidence-1779666000.json")
    );
}
...
fn cloud_readiness_gate_includes_auth_runtime_foundation_without_oidc_provider_claim() {
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-auth-runtime-1779552000.json")
    );
}
...
fn cloud_readiness_gate_includes_postgres_rls_storage_plan_without_database_claim() {
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-postgres-rls-storage-1779552600.json")
    );
    assert!(
        report.evidence_refs.contains(
            &"evidence/multispectrum/cs-ent-suite-postgres-write-contract-1779705600.json"
...
fn cloud_readiness_gate_includes_audit_chain_emission_plan_without_runtime_claim() {
    ...
    assert!(
        report
            .evidence_refs
            .contains(&"evidence/multispectrum/cs-ent-suite-audit-chain-emission-1779661200.json")
    );
}
...
fn cloud_claim_validation_detects_false_positive_ready_flag() {
    let mut report = enterprise_suite_cloud_readiness_report().expect("readiness report builds");
