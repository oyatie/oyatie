//! Enterprise Suite cloud-integration readiness gate.
//!
...
use oya_enterprise_suite_audit_chain_emission::{
    ...
    validate_enterprise_suite_audit_chain_emission_plan,
};
use oya_enterprise_suite_audit_chain_runtime_evidence::{
    EnterpriseSuiteAuditChainRuntimeEvidenceError,
    enterprise_suite_audit_chain_runtime_evidence_plan,
    validate_enterprise_suite_audit_chain_runtime_evidence_plan,
};
use oya_enterprise_suite_auth_runtime::{
    EnterpriseAuthRuntimeError, enterprise_auth_runtime_policy,
    validate_enterprise_auth_runtime_policy,
};
use oya_enterprise_suite_cloud_deployment_evidence::{
    EnterpriseCloudDeploymentEvidenceError, enterprise_cloud_deployment_evidence_plan,
...
use oya_enterprise_suite_disbursement_evidence::{
    EnterpriseDisbursementEvidenceError, enterprise_suite_disbursement_evidence_plan,
    validate_enterprise_suite_disbursement_evidence_plan,
};
use oya_enterprise_suite_erp_parity_map::{
    ErpParityMapError, enterprise_suite_erp_parity_map, validate_erp_parity_map,
};
use oya_enterprise_suite_identity_provider_runtime_evidence::{
    EnterpriseSuiteIdentityProviderRuntimeEvidenceError,
    enterprise_suite_identity_provider_runtime_evidence_plan,
    validate_enterprise_suite_identity_provider_runtime_evidence_plan,
};
...
use oya_enterprise_suite_listener_runtime_evidence::{
    EnterpriseSuiteListenerRuntimeEvidenceError, enterprise_suite_listener_runtime_evidence_plan,
    validate_enterprise_suite_listener_runtime_evidence_plan,
};
use oya_enterprise_suite_local_inmemory_harness::EnterpriseLocalInMemoryHarness;
use oya_enterprise_suite_local_runtime_composition::{
    EnterpriseLocalRuntimeCompositionError, enterprise_local_runtime_composition,
    validate_unique_method_paths,
};
use oya_enterprise_suite_postgres_rls_runtime_evidence::{
    EnterprisePostgresRlsRuntimeEvidenceError, enterprise_postgres_rls_runtime_evidence_plan,
...
use oya_enterprise_suite_tenant_workload_runtime_evidence::{
    EnterpriseSuiteTenantWorkloadRuntimeEvidenceError,
    enterprise_suite_tenant_workload_runtime_evidence_plan,
    validate_enterprise_suite_tenant_workload_runtime_evidence_plan,
};
use oya_enterprise_suite_workflow_adapter_inmemory::enterprise_suite_workflow_queue_capabilities;
use oya_enterprise_suite_workflow_runtime_evidence::{
    EnterpriseSuiteWorkflowRuntimeEvidenceError, enterprise_suite_workflow_runtime_evidence_plan,
    validate_enterprise_suite_workflow_runtime_evidence_plan,
};
...
pub struct EnterpriseSuiteCloudReadinessReport {
...
pub enum CloudReadinessGateError {
    RouteComposition(EnterpriseLocalRuntimeCompositionError),
    AuditChainEmission(EnterpriseAuditChainEmissionError),
    AuditChainRuntimeEvidence(EnterpriseSuiteAuditChainRuntimeEvidenceError),
    AuthRuntime(EnterpriseAuthRuntimeError),
    IdentityProviderVerification(IdentityProviderVerificationError),
    IdentityProviderRuntimeEvidence(EnterpriseSuiteIdentityProviderRuntimeEvidenceError),
    DeploymentManifest(CloudDeploymentManifestError),
    CloudDeploymentEvidence(EnterpriseCloudDeploymentEvidenceError),
    ...
    TenantWorkloadRuntimeEvidence(EnterpriseSuiteTenantWorkloadRuntimeEvidenceError),
    PostgresRlsStorage(EnterprisePostgresRlsStorageError),
    PostgresRlsWriteContract(EnterprisePostgresRlsWriteContractError),
    PostgresRlsTransactionContract(EnterprisePostgresRlsTransactionContractError),
    PostgresRlsRuntimeEvidence(EnterprisePostgresRlsRuntimeEvidenceError),
    ListenerGateway(EnterpriseSuiteListenerGatewayError),
    ListenerRuntimeEvidence(EnterpriseSuiteListenerRuntimeEvidenceError),
    WorkflowRuntimeEvidence(EnterpriseSuiteWorkflowRuntimeEvidenceError),
    SloEvidence(EnterpriseSloEvidenceError),
    StatutoryFilingEvidence(EnterpriseStatutoryFilingEvidenceError),
    DisbursementEvidence(EnterpriseDisbursementEvidenceError),
    ErpParityMap(ErpParityMapError),
    CloudClaimBlocked(Vec<CloudReadinessBlocker>),
    CloudClaimMissingLocalGate(&'static str),
}

pub fn enterprise_suite_cloud_readiness_report()
-> Result<EnterpriseSuiteCloudReadinessReport, CloudReadinessGateError> {
    ...
    let harness_capabilities = EnterpriseLocalInMemoryHarness::new().capabilities();
    let workflow_queue_capabilities = enterprise_suite_workflow_queue_capabilities();
    let workflow_runtime_evidence_plan = enterprise_suite_workflow_runtime_evidence_plan()
        .map_err(CloudReadinessGateError::WorkflowRuntimeEvidence)?;
    validate_enterprise_suite_workflow_runtime_evidence_plan(&workflow_runtime_evidence_plan)
        .map_err(CloudReadinessGateError::WorkflowRuntimeEvidence)?;
    ...
    let cloud_deployment_evidence_plan = enterprise_cloud_deployment_evidence_plan()
        .map_err(CloudReadinessGateError::CloudDeploymentEvidence)?;
    validate_enterprise_cloud_deployment_evidence_plan(&cloud_deployment_evidence_plan)
        .map_err(CloudReadinessGateError::CloudDeploymentEvidence)?;
    ...
    validate_fd001_tenant_secret_boundary_contract(&tenant_secret_boundary_contract)
        .map_err(CloudReadinessGateError::TenantSecretBoundary)?;
    let tenant_workload_runtime_evidence_plan =
        enterprise_suite_tenant_workload_runtime_evidence_plan()
    ...
    .map_err(CloudReadinessGateError::TenantWorkloadRuntimeEvidence)?;
    let audit_chain_plan = enterprise_suite_audit_chain_emission_plan();
    validate_enterprise_suite_audit_chain_emission_plan(&audit_chain_plan)
        .map_err(CloudReadinessGateError::AuditChainEmission)?;
    let audit_chain_runtime_evidence_plan = enterprise_suite_audit_chain_runtime_evidence_plan()
        .map_err(CloudReadinessGateError::AuditChainRuntimeEvidence)?;
    validate_enterprise_suite_audit_chain_runtime_evidence_plan(&audit_chain_runtime_evidence_plan)
        .map_err(CloudReadinessGateError::AuditChainRuntimeEvidence)?;
    ...
    let identity_provider_plan = enterprise_suite_identity_provider_verification_plan();
    validate_enterprise_suite_identity_provider_verification_plan(&identity_provider_plan)
    ...
    .map_err(CloudReadinessGateError::IdentityProviderRuntimeEvidence)?;
    let storage_plan = enterprise_postgres_rls_storage_plan();
    validate_enterprise_postgres_rls_storage_plan(&storage_plan)
        .map_err(CloudReadinessGateError::PostgresRlsStorage)?;
    let postgres_write_contract = enterprise_postgres_rls_write_contract()
        .map_err(CloudReadinessGateError::PostgresRlsWriteContract)?;
    validate_enterprise_postgres_rls_write_contract(&postgres_write_contract)
        .map_err(CloudReadinessGateError::PostgresRlsWriteContract)?;
    let postgres_transaction_contract = enterprise_postgres_rls_transaction_contract()
        .map_err(CloudReadinessGateError::PostgresRlsTransactionContract)?;
    validate_enterprise_postgres_rls_transaction_contract(&postgres_transaction_contract)
        .map_err(CloudReadinessGateError::PostgresRlsTransactionContract)?;
    let postgres_runtime_evidence_plan = enterprise_postgres_rls_runtime_evidence_plan()
        .map_err(CloudReadinessGateError::PostgresRlsRuntimeEvidence)?;
    validate_enterprise_postgres_rls_runtime_evidence_plan(&postgres_runtime_evidence_plan)
        .map_err(CloudReadinessGateError::PostgresRlsRuntimeEvidence)?;
    let listener_gateway_plan = enterprise_suite_listener_gateway_plan();
    validate_enterprise_suite_listener_gateway_plan(&listener_gateway_plan)
        .map_err(CloudReadinessGateError::ListenerGateway)?;
    let listener_runtime_evidence_plan = enterprise_suite_listener_runtime_evidence_plan()
        .map_err(CloudReadinessGateError::ListenerRuntimeEvidence)?;
    validate_enterprise_suite_listener_runtime_evidence_plan(&listener_runtime_evidence_plan)
        .map_err(CloudReadinessGateError::ListenerRuntimeEvidence)?;
    let slo_plan = enterprise_suite_slo_evidence_plan();
    validate_enterprise_suite_slo_evidence_plan(&slo_plan)
        .map_err(CloudReadinessGateError::SloEvidence)?;
    let statutory_filing_plan = enterprise_suite_statutory_filing_evidence_plan();
    validate_enterprise_suite_statutory_filing_evidence_plan(&statutory_filing_plan)
        .map_err(CloudReadinessGateError::StatutoryFilingEvidence)?;
    let disbursement_plan = enterprise_suite_disbursement_evidence_plan();
    validate_enterprise_suite_disbursement_evidence_plan(&disbursement_plan)
        .map_err(CloudReadinessGateError::DisbursementEvidence)?;
    let parity_rows = enterprise_suite_erp_parity_map();
    ...
    let cloud_deployment_manifest_ready = deployment_manifest.service_name == "enterprise-suite"
    ...
    let workflow_execution_reference_ready = workflow_queue_capabilities
        ...
        && !workflow_queue_capabilities.child_service_calls_attached
    ...
    let workflow_runtime_evidence_plan_ready = workflow_runtime_evidence_plan
        ...
        && workflow_runtime_evidence_plan.child_service_boundary_required
        ...
        && !workflow_runtime_evidence_plan.child_service_calls_runtime_attached
    ...
    Ok(EnterpriseSuiteCloudReadinessReport {
        report_name: "enterprise-suite-cloud-readiness-gate",
        ...
        blockers,
        evidence_refs: vec![
            "evidence/multispectrum/cs-ent-suite-local-runtime-composition-1779541200.json",
            "evidence/multispectrum/cs-ent-suite-local-inmemory-harness-1779541800.json",
            "evidence/multispectrum/cs-ent-suite-erp-parity-map-1779542400.json",
            "evidence/multispectrum/cs-ent-suite-cloud-deployment-manifest-1779551400.json",
            "evidence/multispectrum/cs-ent-suite-cloud-deployment-evidence-1779702000.json",
            "evidence/multispectrum/cs-ent-suite-tenant-workload-manifest-1779701400.json",
            "evidence/multispectrum/cs-ent-suite-tenant-admission-policy-1779706800.json",
            "evidence/multispectrum/cs-ent-suite-tenant-resource-quota-contract-1779709200.json",
            "evidence/multispectrum/cs-ent-suite-tenant-availability-contract-1779709800.json",
            "evidence/multispectrum/cs-ent-suite-tenant-autoscaling-contract-1779710400.json",
            "evidence/multispectrum/cs-ent-suite-tenant-cost-allocation-contract-1779711000.json",
            "evidence/multispectrum/cs-ent-suite-tenant-residency-contract-1779711600.json",
            "evidence/multispectrum/cs-ent-suite-tenant-workload-identity-contract-1779712200.json",
            "evidence/multispectrum/cs-ent-suite-tenant-egress-policy-contract-1779708600.json",
            "evidence/multispectrum/cs-ent-suite-image-provenance-contract-1779707400.json",
            "evidence/multispectrum/cs-ent-suite-secret-boundary-contract-1779708000.json",
            "evidence/multispectrum/cs-ent-suite-tenant-runtime-evidence-1779705000.json",
            ...
            "evidence/multispectrum/cs-ent-suite-idp-runtime-evidence-1779703200.json",
            "evidence/multispectrum/cs-ent-suite-audit-chain-emission-1779661200.json",
            "evidence/multispectrum/cs-ent-suite-audit-runtime-evidence-1779704400.json",
            "evidence/multispectrum/cs-ent-suite-postgres-rls-storage-1779552600.json",
            "evidence/multispectrum/cs-ent-suite-postgres-write-contract-1779705600.json",
            "evidence/multispectrum/cs-ent-suite-postgres-tx-contract-1779706200.json",
            "evidence/multispectrum/cs-ent-suite-postgres-rls-runtime-evidence-1779666600.json",
            "evidence/multispectrum/cs-ent-suite-listener-gateway-1779553200.json",
            "evidence/multispectrum/cs-ent-suite-listener-runtime-evidence-1779702600.json",
            "evidence/multispectrum/cs-ent-suite-slo-evidence-1779664200.json",
            "evidence/multispectrum/cs-ent-suite-workflow-execution-1779664800.json",
            "evidence/multispectrum/cs-ent-suite-workflow-runtime-evidence-1779703800.json",
            "evidence/multispectrum/cs-ent-suite-statutory-filing-evidence-1779665400.json",
            "evidence/multispectrum/cs-ent-suite-disbursement-evidence-1779666000.json",
        ],
...
pub fn validate_cloud_claim(
    report: &EnterpriseSuiteCloudReadinessReport,
