//! Enterprise Suite identity-provider runtime evidence contract.
//!
...
use oya_enterprise_suite_identity_provider_verification::{
    EnterpriseSuiteIdentityProviderVerificationPlan, IdentityProviderVerificationError,
    enterprise_suite_identity_provider_verification_plan,
    validate_enterprise_suite_identity_provider_verification_plan,
};

const SCHEMA_VERSION: u32 = 1;
const MIN_REQUIREMENT_COUNT: usize = 15;
const PLAN_NAME: &str = "enterprise-suite-identity-provider-runtime-evidence-plan";
...
const SOURCE_PLAN_REF: &str = "crates/oya-enterprise-suite-identity-provider-verification/src/lib.rs::enterprise_suite_identity_provider_verification_plan";
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteIdentityProviderRuntimeEvidencePlan {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteIdentityProviderRuntimeEvidenceError {
...
pub fn enterprise_suite_identity_provider_runtime_evidence_plan() -> Result<
    EnterpriseSuiteIdentityProviderRuntimeEvidencePlan,
    EnterpriseSuiteIdentityProviderRuntimeEvidenceError,
> {
    let verification_plan = enterprise_suite_identity_provider_verification_plan();
    validate_enterprise_suite_identity_provider_verification_plan(&verification_plan).map_err(
        EnterpriseSuiteIdentityProviderRuntimeEvidenceError::IdentityProviderVerification,
    )?;

    Ok(EnterpriseSuiteIdentityProviderRuntimeEvidencePlan {
        plan_name: PLAN_NAME,
...
pub fn validate_enterprise_suite_identity_provider_runtime_evidence_plan(
    plan: &EnterpriseSuiteIdentityProviderRuntimeEvidencePlan,
) -> Result<(), EnterpriseSuiteIdentityProviderRuntimeEvidenceError> {
    validate_slug(
        plan.plan_name,
        EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidPlanName,
    )?;
    if plan.service_name != SERVICE_NAME {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidServiceName);
    }
    if plan.substrate_name != SUBSTRATE_NAME {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidSubstrateName);
    }
    if plan.tenant_namespace != TENANT_NAMESPACE {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidTenantNamespace);
    }
    if plan.identity_provider_verification_plan_name
        != "enterprise-suite-identity-provider-verification-plan"
    {
        return Err(
            EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidIdentityProviderVerificationPlanName,
        );
    }
    if !valid_https_url(plan.issuer) {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidIssuer);
    }
    if !valid_label(plan.audience) {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidAudience);
    }
    ...
    {
        return Err(
            EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidDiscoveryDocumentUrl,
        );
    }
    if !valid_https_url(plan.jwks_uri) || !plan.jwks_uri.ends_with("/.well-known/jwks.json") {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidJwksUri);
    }
    ...
    {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::MissingRequirements);
    }
...
fn runtime_requirements(
    verification_plan: &EnterpriseSuiteIdentityProviderVerificationPlan,
    ...
    vec![
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/discovery-document.json",
            verification_plan,
        ),
        requirement(
            ...
            "identity-provider-discovery",
            "https://openid.net/specs/openid-connect-discovery-1_0.html",
            "evidence/identity-provider-runtime/enterprise-suite/issuer-metadata.json",
            verification_plan,
        ),
        requirement(
            "jwks-fetched",
            ...
            "jwks-cache",
            "https://openid.net/specs/openid-connect-discovery-1_0.html",
            "evidence/identity-provider-runtime/enterprise-suite/jwks-fetch.json",
            verification_plan,
        ),
        requirement(
            "jwks-kid-matched",
            ...
            "evidence/identity-provider-runtime/enterprise-suite/jwks-kid-match.json",
            verification_plan,
        ),
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/jwt-signature.json",
            verification_plan,
        ),
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/algorithm-allowlist.json",
            verification_plan,
        ),
        requirement(
            ...
            "claim-validator",
            "https://openid.net/specs/openid-connect-core-1_0.html",
            "evidence/identity-provider-runtime/enterprise-suite/issuer-claim.json",
            verification_plan,
        ),
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/audience-claim.json",
            verification_plan,
        ),
        requirement(
            ...
            "claim-validator",
            "https://www.rfc-editor.org/rfc/rfc7519",
            "evidence/identity-provider-runtime/enterprise-suite/temporal-claims.json",
            verification_plan,
        ),
        requirement(
            "nonce-replay-denied",
            ...
            "nonce-store",
            "https://openid.net/specs/openid-connect-core-1_0.html",
            "evidence/identity-provider-runtime/enterprise-suite/nonce-replay-denied.json",
            verification_plan,
        ),
        requirement(
            "tenant-claim-mapped",
            ...
            "tenant-router",
            "https://www.rfc-editor.org/rfc/rfc7519",
            "evidence/identity-provider-runtime/enterprise-suite/tenant-claim-map.json",
            verification_plan,
        ),
        requirement(
            ...
            "route-policy",
            "https://www.rfc-editor.org/rfc/rfc7519",
            "evidence/identity-provider-runtime/enterprise-suite/route-scope-authorization.json",
            verification_plan,
        ),
        requirement(
            ...
            "https://openid.net/specs/openid-connect-core-1_0.html",
            "evidence/identity-provider-runtime/enterprise-suite/sensitive-route-mfa.json",
            verification_plan,
        ),
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/key-rotation-overlap.json",
            verification_plan,
        ),
        requirement(
            ...
            "evidence/identity-provider-runtime/enterprise-suite/auth-failure-audit-event.json",
            verification_plan,
        ),
    ]
}
...
fn validate_nonclaims(
    ...
    {
        return Err(
            EnterpriseSuiteIdentityProviderRuntimeEvidenceError::RuntimeAttachmentOverclaim,
        );
    }
    Ok(())
}
...
fn validate_runtime_requirements(
    ...
    for requirement in &plan.requirements {
        ...
        if !seen_requirements.insert(requirement.requirement_id) {
            return Err(
                EnterpriseSuiteIdentityProviderRuntimeEvidenceError::DuplicateRequirement(
    ...
    for kind in required_requirement_kinds() {
        if !seen_kinds.contains(&kind) {
            return Err(
                EnterpriseSuiteIdentityProviderRuntimeEvidenceError::MissingRequirementKind(kind),
            );
        }
    }
    Ok(())
...
fn validate_requirement(
    ...
    validate_slug(
        ...
        EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidRequirementId,
    )?;
    ...
    validate_prefixed_ref(
        ...
        "evidence/identity-provider-runtime/enterprise-suite/",
        EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidExpectedEvidenceRef,
    )?;
    validate_prefixed_ref(
        requirement.source_plan_ref,
        "crates/oya-enterprise-suite-identity-provider-verification/",
        EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidSourcePlanRef,
    )?;
...
fn validate_slug(
    value: &str,
    error: EnterpriseSuiteIdentityProviderRuntimeEvidenceError,
) -> Result<(), EnterpriseSuiteIdentityProviderRuntimeEvidenceError> {
    if value.is_empty()
...
fn validate_workload_scope(
    ...
    if value.is_empty() || has_unsafe_text(value) || has_path_traversal(value) {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidWorkloadScope);
    }
    Ok(())
}

fn validate_doc_url(url: &str) -> Result<(), EnterpriseSuiteIdentityProviderRuntimeEvidenceError> {
    let allowed = [
        "https://openid.net/specs/openid-connect-discovery-1_0.html",
        "https://openid.net/specs/openid-connect-core-1_0.html",
        "https://www.rfc-editor.org/rfc/rfc7515",
    ...
    if !allowed.contains(&url) {
        return Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidOfficialDocUrl);
    }
    Ok(())
}
...
fn require_control(
    ...
    if enabled {
        ...
        Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::MissingRequiredControl(field))
    }
}
