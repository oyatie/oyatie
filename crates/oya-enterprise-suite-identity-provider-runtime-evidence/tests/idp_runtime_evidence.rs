use oya_enterprise_suite_identity_provider_runtime_evidence::{
    ...
    IdentityProviderRuntimeEvidenceRequirementKind,
...
fn idp_runtime_evidence_plan_validates_controls_and_nonclaims() {
    let plan = enterprise_suite_identity_provider_runtime_evidence_plan().expect("plan builds");
    validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan)
        .expect("plan validates");

    assert_eq!(
        plan.plan_name,
        "enterprise-suite-identity-provider-runtime-evidence-plan"
    );
...
fn idp_runtime_evidence_plan_preserves_ref_and_claim_boundaries() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            .starts_with("evidence/identity-provider-runtime/enterprise-suite/")
            && requirement
                .source_plan_ref
                .starts_with("crates/oya-enterprise-suite-identity-provider-verification/")
            && requirement.tenant_namespace == "oyatie-fd001-enterprise-dev"
            && requirement.expected_issuer == "https://identity.oyatie.dev/enterprise-suite"
            && requirement.expected_audience == "enterprise-suite-api"
...
fn idp_runtime_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = enterprise_suite_identity_provider_runtime_evidence_plan().expect("plan builds");
    plan.requirements.truncate(3);
    assert_eq!(
        validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::MissingRequirements)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidOfficialDocUrl)
    );
}
...
fn idp_runtime_evidence_plan_rejects_unsafe_refs_missing_controls_and_overclaims() {
    ...
    plan.requirements[0].expected_evidence_ref =
        "evidence/identity-provider-runtime/enterprise-suite/password-material";
    assert_eq!(
        validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::InvalidExpectedEvidenceRef)
    );
    ...
    assert_eq!(
        validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan),
        Err(
            EnterpriseSuiteIdentityProviderRuntimeEvidenceError::MissingRequiredControl(
    ...
    assert_eq!(
        validate_enterprise_suite_identity_provider_runtime_evidence_plan(&plan),
        Err(EnterpriseSuiteIdentityProviderRuntimeEvidenceError::RuntimeAttachmentOverclaim)
    );
}
