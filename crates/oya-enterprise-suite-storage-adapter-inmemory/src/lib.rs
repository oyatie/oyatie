//! Enterprise Suite in-memory storage adapter reference.
//!
//! SECURITY/OPERATIONS: NOT FOR PRODUCTION. This adapter is an in-process
//! reference implementation for Enterprise Suite metadata storage seams. It is
...
//! Workflow execution, OpenTofu execution, child-service network call, or
...
use oya_enterprise_suite_app::{
    CrossProductWorkflowEnvelope, EnterpriseOpsEnvelope, IncidentRollbackEnvelope,
};
use oya_enterprise_suite_domain::{GroupCloseRollup, SuitePolicyDecision};
...
pub enum EnterpriseSuiteStoredRecordKind {
    PolicyAdmission,
    GroupCloseRollup,
...
pub struct EnterpriseSuiteStoredRecord {
    pub kind: EnterpriseSuiteStoredRecordKind, // data_class: INTERNAL_ONLY
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EnterpriseSuiteStorageCapabilities {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EnterpriseSuiteStorageError {
    DuplicateIdempotencyKey(String),
    InvalidIdempotencyKey(String),
    MissingRecord(String),
}

pub trait EnterpriseSuiteStoragePort {
    fn reserve_idempotency_key(&mut self, key: &str) -> Result<(), EnterpriseSuiteStorageError>;
    fn put_record(
        &mut self,
        record: EnterpriseSuiteStoredRecord,
    ) -> Result<(), EnterpriseSuiteStorageError>;
    fn get_record(&self, idempotency_key: &str) -> Option<&EnterpriseSuiteStoredRecord>;
    fn require_record(
        &self,
        idempotency_key: &str,
    ) -> Result<&EnterpriseSuiteStoredRecord, EnterpriseSuiteStorageError>;
    fn list_records(&self) -> Vec<&EnterpriseSuiteStoredRecord>;
    fn len(&self) -> usize;
    fn is_empty(&self) -> bool;
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct InMemoryEnterpriseSuiteStore {
    records_by_idempotency_key: BTreeMap<String, EnterpriseSuiteStoredRecord>,
    reserved_idempotency_keys: BTreeSet<String>,
}

impl InMemoryEnterpriseSuiteStore {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn persist_policy_decision(
        &mut self,
        decision: &SuitePolicyDecision,
    ) -> Result<EnterpriseSuiteStoredRecord, EnterpriseSuiteStorageError> {
        let record = EnterpriseSuiteStoredRecord {
            kind: EnterpriseSuiteStoredRecordKind::PolicyAdmission,
    ...
    pub fn persist_group_close_rollup(
        ...
        let record = EnterpriseSuiteStoredRecord {
            kind: EnterpriseSuiteStoredRecordKind::GroupCloseRollup,
    ...
    pub fn persist_cross_product_workflow(
        &mut self,
        envelope: &CrossProductWorkflowEnvelope,
    ) -> Result<EnterpriseSuiteStoredRecord, EnterpriseSuiteStorageError> {
        let record = EnterpriseSuiteStoredRecord {
            kind: EnterpriseSuiteStoredRecordKind::CrossProductWorkflowPlan,
    ...
    pub fn persist_incident_rollback(
        ...
        let record = EnterpriseSuiteStoredRecord {
            kind: EnterpriseSuiteStoredRecordKind::IncidentRollbackPlan,
            topic: envelope.topic.value.clone(),
            tenant_id: envelope.tenant_id.value.value.clone(),
    ...
    pub fn persist_ops_command(
        &mut self,
        envelope: &EnterpriseOpsEnvelope,
    ) -> Result<EnterpriseSuiteStoredRecord, EnterpriseSuiteStorageError> {
        let record = EnterpriseSuiteStoredRecord {
            kind: EnterpriseSuiteStoredRecordKind::OpsCommand,
            topic: envelope.topic.value.clone(),
            tenant_id: envelope.tenant_id.value.value.clone(),
...
impl EnterpriseSuiteStoragePort for InMemoryEnterpriseSuiteStore {
    fn reserve_idempotency_key(&mut self, key: &str) -> Result<(), EnterpriseSuiteStorageError> {
        validate_idempotency_key(key)?;
        if self.records_by_idempotency_key.contains_key(key)
            || !self.reserved_idempotency_keys.insert(key.to_owned())
        {
            return Err(EnterpriseSuiteStorageError::DuplicateIdempotencyKey(
                key.to_owned(),
            ));
        }
        Ok(())
    ...
    fn require_record(
        ...
        self.get_record(idempotency_key)
            .ok_or_else(|| EnterpriseSuiteStorageError::MissingRecord(idempotency_key.to_owned()))
    }
...
pub fn enterprise_suite_storage_capabilities() -> EnterpriseSuiteStorageCapabilities {
    EnterpriseSuiteStorageCapabilities {
...
fn validate_idempotency_key(key: &str) -> Result<(), EnterpriseSuiteStorageError> {
    if key.trim().is_empty()
        || key.trim() != key
        || key.contains("..")
        || key.chars().any(char::is_whitespace)
    {
        return Err(EnterpriseSuiteStorageError::InvalidIdempotencyKey(
            key.to_owned(),
        ));
    }
    Ok(())
