---
doc_class: Runbook
doc_id: RUNBOOK-FEATURE_FLAGS-AUTO_REBALANCE
...
# Runbook: Feature Flags Auto Rebalance

## Operator Contract
- Runbook id: feature-flags-auto-rebalance.
- Primary service namespace: `feature-flags`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-feature-flags-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=feature-flags; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n feature-flags rollout status deploy/feature-flags --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service feature-flags --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service feature-flags --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service feature-flags --cell $CELL --window 30m`.
