---
doc_class: Runbook
doc_id: RUNBOOK-FEATURE_FLAGS-COLD_MERGE
...
# Runbook: Feature Flags Cold Merge

## Operator Contract
- Runbook id: feature-flags-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-feature-flags-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=feature-flags; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n feature-flags rollout status deploy/feature-flags --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service feature-flags --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service feature-flags --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service feature-flags --cell $CELL --window 30m`.
