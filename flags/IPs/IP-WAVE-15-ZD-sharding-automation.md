...
---
# IP-WAVE-15-ZD-sharding-automation: Feature Flags Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/feature-flags/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: feature-flags must interpret ADR-0348 through its own bounded context: flag.
SCOPE-004: feature-flags uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: feature-flags uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: feature-flags uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: feature-flags.
...
STANCE-005: Bounded context anchor: flag.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: us, eu, kr, healthcare, fedramp, PCI-DSS, eu-ai-act, soc2, iso27001, gdpr, hipaa, us-healthcare, fedramp-high, gdpr-eu, soc2-type2, kr-isms-p, il5, kr-pipa.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for feature-flags.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for feature-flags because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; feature-flags must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where feature-flags participates.
...
ROLLBACK-002: For feature-flags, rollback starts from the audit-chain row that recorded the automation event.
