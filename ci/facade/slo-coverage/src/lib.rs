...
//!
//! Portable conformance gate for OpenSLO corpus declarations (ADR-0718 re-pointed the gate off
//! the retired `registry/catalog/*.yaml` mirror). The producer owns all repository I/O: it
//! enumerates every tracked `*.openslo.yaml`, keys each row by its repo-relative path, and parses
//! the envelope's `metadata.name` into rows shaped as `{"crate_id", "source_path", "slo"}`. This crate
...
pub const CODE_CENSUS_DROP_UNATTRIBUTED: &str = "slo_census_drop_unattributed";
...
pub const CODE_CATALOG_CENSUS_GROWTH_UNATTRIBUTED: &str = "slo_census_growth_unattributed";
...
#[must_use]
pub fn evaluate_corpus_census(observed: usize, frozen: usize) -> Option<CensusFinding> {
    if observed == frozen {
        return None;
    }
    let (code, cause) = if observed < frozen {
        (
            CODE_CENSUS_DROP_UNATTRIBUTED,
    ...
    Some(CensusFinding {
        code,
        detail: format!(
            "the slo-coverage face enumerated {observed} corpus rows but the frozen census pin is \
...
pub fn evaluate_keyed(input: &Value) -> BTreeSet<Finding> {
    ...
    let rows = match input.get("rows").and_then(Value::as_array) {
        ...
        _ => {
            findings.insert(Finding::new("slo_empty_corpus", "<empty-slo-catalog>"));
            return findings;
        }
    };
...
mod tests {
    ...
    fn the_census_pin_fires_on_the_deletions_the_floor_slept_through() {
        ...
        let finding = evaluate_corpus_census(755, 776).expect("a 21-row deletion must be caught");
        assert_eq!(finding.code, CODE_CENSUS_DROP_UNATTRIBUTED);
        assert!(
        ...
        assert_eq!(evaluate_corpus_census(755, 755), None);
        ...
        let partial_collapse = evaluate_corpus_census(after_partial_collapse, 745)
            .expect("a partial collapse must be caught");
        assert_eq!(partial_collapse.code, CODE_CENSUS_DROP_UNATTRIBUTED);
        ...
        let growth = evaluate_corpus_census(746, 745).expect("growth must be attributed too");
    ...
    fn a_collapsed_enumeration_still_fails_closed() {
        let finding = evaluate_corpus_census(0, 745).expect("zero rows must never be a pass");
        assert_eq!(finding.code, CODE_CENSUS_DROP_UNATTRIBUTED);
        ...
        assert_eq!(evaluate_corpus_census(0, 0), None);
    }
    ...
    fn empty_corpus_is_red_to_prevent_false_green() {
        ...
        assert_eq!(findings.len(), 1, "got {findings:?}");
        let finding = findings.iter().next().unwrap();
        assert_eq!(finding.code, "slo_empty_corpus");
