...
fn active_artifact_contract_graph_is_controller_only_and_direct_edits_are_red() {
    ...
    let graph = artifacts
        .iter()
        .find(|artifact| artifact.get("artifact_id").and_then(Value::as_str) == Some(GRAPH_ID))
    ...
    assert_eq!(
        graph["generator"]["operation_id"].as_str(),
    ...
    let rules = manifest
        ...
        .and_then(Value::as_array)
        .expect("live generated-artifact manifest must contain generated_path_rules");
    assert!(
        rules.iter().any(|rule| {
            rule.get("pattern").and_then(Value::as_str) == Some(GRAPH_PATH)
                && rule.get("rule_kind").and_then(Value::as_str) == Some("path_suffix")
        }),
        "the non-.generated graph path must be classified explicitly"
    );

    let registry = read_json(repo_root().join("registry/artifact-capabilities-registry.json"));
    assert!(
        registry["rows"].as_array().is_some_and(|rows| !rows
            .iter()
            .any(|row| { row.get("artifact_id").and_then(Value::as_str) == Some(GRAPH_ID) })),
        "a de-committed projection must not masquerade as a tracked active authority row"
    );

    let diff = format!(
        "A\t{GRAPH_PATH}\nM\t{GRAPH_PATH}\nR100\tstaging/old-graph.json\t{GRAPH_PATH}\nC100\tstaging/copied-graph.json\t{GRAPH_PATH}\n"
    );
