...
fn require_resolved_mode(
    expected: app::CacheMode,
    observed: &str,
    build_class: &str,
) -> Result<(), String> {
    if observed.trim() == expected.as_str() {
        Ok(())
    } else {
        Err(format!(
            "cache mode for class `{build_class}` degraded or escalated: policy resolved \
             `{expected}`, runtime reported `{observed}`"
        ))
    }
}
...
fn run(args: Vec<String>) -> Result<ExitCode, String> {
    ...
    match command.as_str() {
        ...
        "assert-warm" => {
            ...
            let policy = app::load_policy(&root)?;
            let license = app::load_license(&root)?;
            let expected = app::resolve(&policy, &license, &build_class)?;
            if let Err(finding) = require_resolved_mode(expected.mode, &mode, &build_class) {
                eprintln!("{finding}");
...
mod tests {
    ...
    #[test]
    fn resolved_mode_refuses_runtime_degradation_or_escalation() {
        assert!(require_resolved_mode(app::CacheMode::Bypass, "bypass", "cold").is_ok());
        assert!(require_resolved_mode(app::CacheMode::WarmReadWrite, "warm-rw", "writer").is_ok());
        assert!(
            require_resolved_mode(app::CacheMode::WarmReadWrite, "bypass", "writer")
                .unwrap_err()
                .contains("degraded or escalated")
        );
        assert!(
            require_resolved_mode(app::CacheMode::Bypass, "warm-ro", "cold")
                .unwrap_err()
                .contains("degraded or escalated")
        );
    }
