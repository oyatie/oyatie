...
/// never be mistaken for an executed one. This is the specific false-green the previous
...
";
    ...
    // RED 4: a `buck2 run` chained after `buck2 test` on the SAME line (the `gate` job's pwsh
    // step does exactly this) is not a test execution and must not be harvested as one.
    let chained = "\
jobs:
  gate:
    steps:
...
";
    ...
    // RED 3: recursive coverage is not execution. A package with no test rule runs nothing under
...
fn every_gate_crate_is_registered_in_oya_ci_required_workflow() {
    ...
    // ── REGISTRATION IS EXECUTION (2026-08-01, the 46->2 matrix collapse).
    //
...
fn oya_ci_configured_gates_have_disposition_and_required_workflow_authority() {
    ...
    for (gate_id, input_kind) in configured {
        let has_required_lane = match input_kind.as_str() {
            ...
            // SSOT (the ci keystone move renamed the crates semantically), then require that dir's
            // package to be covered by a pattern a fan-in-reachable job actually executes.
            //
            // This used to be `workflow.contains("crate: {dir},")` — a matrix line. The 46->2
...
fn every_gate_lane_is_a_dependency_of_the_fan_in_job() {
    ...
    // a job that sits in `needs:` but is missing from the `needs.<job>.result` success chain can
    // fail while the required context still goes green. Being depended on is not the same as
    // being checked, and `fan_in_mentions_job` requires both.
    //
