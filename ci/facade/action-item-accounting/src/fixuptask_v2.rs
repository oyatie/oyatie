...
pub fn collect_fixuptask_candidate_jsonl(root: &Path, path: &str) -> Result<Value, CollectError> {
    ...
    for (index, raw) in text.lines().enumerate() {
        ...
        let value = parse_duplicate_key_free_json(line.as_bytes()).map_err(|message| {
            CollectError::Parse {
                line: index + 1,
                message,
            }
...
fn parse_duplicate_key_free_json(bytes: &[u8]) -> Result<Value, String> {
    let mut deserializer = serde_json::Deserializer::from_slice(bytes);
    DuplicateKeyFreeJson::deserialize(&mut deserializer).map_err(|error| error.to_string())?;
...
pub fn evaluate_materialized_gate(root: &Path) -> Result<BTreeSet<Finding>, CollectError> {
    ...
    let facts = parse_duplicate_key_free_json(text.as_bytes())
        .map_err(|message| CollectError::Parse { line: 1, message })?;
