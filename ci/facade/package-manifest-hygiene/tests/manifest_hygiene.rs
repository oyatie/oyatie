...
fn manifest_hygiene_is_born_blocking_on_the_live_corpus() {
    ...
    // NOTE: this face is scoped to `oya-*`-prefixed crates only (collect_manifest_hygiene), so its
    ...
    // load-bearing "born-blocking" proof is the verdict/findings assertions below, which the
    // pure evaluator ALREADY fails closed on for an empty `rows` array (see
    // `evaluate_keyed`'s `<empty-rows>` guard) — so no magnitude assertion is needed here.
    assert!(!rows.is_empty(), "the manifest-hygiene face must enumerate at least one crate");
