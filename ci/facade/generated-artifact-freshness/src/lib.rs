...
/// The four pinned generator tool names the install manifest MUST fingerprint. The handshake
/// compares the manifest's key set EXACTLY against this list (no missing, no extra entries), so a
/// malformed or partially migrated manifest cannot certify a replaced tool.
const REQUIRED_PRE_PUSH_TOOLS: [&str; 4] = [
...
fn verify_committed_tree_with_determinism(
    ...
    let decommitted = read_decommitted_face_names(repo_root);
    let report = if decommitted.is_empty() {
        let regenerated_faces = regenerate_faces_with_tools(repo_root, tools, None)?;
        verify_committed_tree(repo_root, regenerated_faces)?
    } else {
        ...
        report.findings.sort();
        report.findings.dedup();
        verify_committed_report(report)
    };
    // TOCTOU guard: the generators ran against working-tree bytes. Re-assert the FULL tree is
    // still byte-identical to HEAD AFTER generation, so an editor/formatter/background process
    // that changed a tracked input mid-run cannot certify a mixture of HEAD and post-checkout
    // bytes (a false green, especially for de-committed faces where regenerated bytes are not
    // compared to a committed copy).
    assert_committed_tree_clean(repo_root)?;
    Ok(report)
}
...
pub fn write_pre_push_verifier_manifest(
    ...
    // Record the installed hook binary's own fingerprint so a later reinstall can verify that the
    // current `pre-push` is still this verifier BEFORE granting replacement permission — a
    // manifest left behind after the user swapped in another tool's hook must not authorize an
    // overwrite of unrelated user hook state.
    let hook_fingerprint = content_fingerprint(hook_path)?;
...
/// FNV-1a content fingerprint of a file's bytes (change detection, not a security hash).
pub fn content_fingerprint(path: &Path) -> Result<String, FreshnessError> {
    let bytes = std::fs::read(path)
        .map_err(|error| FreshnessError::new(format!("read {}: {error}", path.display())))?;
    Ok(fnv1a64_hex(&bytes))
}

/// True when the installed manifest records a hook fingerprint equal to the CURRENT `hook_path`
/// bytes — i.e. the installed `pre-push` is still the oya verifier this sidecar manifest was
/// written for. Binds replacement permission to the installed hook's identity rather than the
/// manifest's mere existence, so a manifest left behind after the user replaced the hook with
/// another tool's cannot authorize overwriting it.
pub fn manifest_owns_installed_hook(
    hooks_dir: &Path,
    hook_path: &Path,
    ...
    let recorded = value
        .get("hook_fingerprint")
        .and_then(|value| value.as_str())
        .unwrap_or_default();
    let actual = content_fingerprint(hook_path)?;
    Ok(recorded == actual)
}
...
pub fn read_pre_push_verifier_manifest(
    ...
    // Fail closed on a manifest that does not fingerprint EVERY pinned tool: the key set must
    // exactly match the four required names, so a syntactically valid manifest that omits (or
...
/// Fingerprint of EVERY input that determines the pinned generator tool builds. The pinned tools
/// are built from the four generator crates' sources, so the fingerprint covers each crate root's
/// Rust sources (including `build.rs`), manifests, and Buck rules, the workspace
/// `Cargo.toml`/`Cargo.lock`, and the transitive manifest-declared path dependencies of those
/// crates (their sources + manifests). Read strictly as DATA (never executed) at both install and
/// push time, so ANY generator build-input change — a crate's `Cargo.toml`, the workspace
...
/// `build.rs` and the whole `src/` tree), the crate manifest (`Cargo.toml`/`Cargo.lock`), and any
/// Buck rule file — skipping build output (`target/`) and VCS metadata.
fn collect_build_input_files(dir: &Path, out: &mut Vec<PathBuf>) -> Result<(), FreshnessError> {
    let entries = std::fs::read_dir(dir).map_err(|error| {
        FreshnessError::new(format!("read build-input dir {}: {error}", dir.display()))
    })?;
    for entry in entries {
        let entry = entry.map_err(|error| {
            FreshnessError::new(format!(
                "read build-input entry in {}: {error}",
                dir.display()
            ))
        })?;
        let path = entry.path();
        ...
        if path.is_dir() {
            ...
            collect_build_input_files(&path, out)?;
        } else {
            let is_rs = path.extension().and_then(|ext| ext.to_str()) == Some("rs");
            let is_manifest = name == "Cargo.toml" || name == "Cargo.lock";
            let is_buck = name == "BUCK";
            if is_rs || is_manifest || is_buck {
                out.push(path);
            }
        }
    }
...
/// The manifest-declared path inputs of the crate rooted at `crate_root`, resolved to absolute
/// paths. A crate without a manifest (or with an unparsable one) contributes no inputs.
fn path_dependency_roots(crate_root: &Path) -> Result<Vec<PathBuf>, FreshnessError> {
...
/// change detection (not a security hash). Public so the hook binary can key per-worktree state
/// (e.g. pinned-tool generations) with the same hash.
...
mod pre_push_verifier_protocol_tests {
    ...
    /// Create a fake installed `pre-push` hook file so manifests can bind to the hook's identity.
    fn fixture_hook(hooks: &Path) -> PathBuf {
        let hook = hooks.join("pre-push");
        std::fs::write(&hook, "fake oya pre-push verifier binary\n").expect("write hook");
        hook
    }
    ...
    #[test]
    fn partial_tool_fingerprints_fail_closed() {
        let root = temp_root("oya-pre-push-manifest-partial-tools");
        write_protocol_fixture(&root, PRE_PUSH_VERIFIER_PROTOCOL_VERSION, None);
        let hooks = root.join("hooks");
        std::fs::create_dir_all(&hooks).expect("create hooks dir");
        let tools = fixture_tools_dir(&root);
        // A syntactically valid manifest with the right protocol and source fingerprint but a
        // tool_fingerprints key set that is NOT exactly the four required names must fail closed
        // (here: four entries, one padded with a non-tool key, so `producer` is missing).
        let manifest = serde_json::json!({
            "protocol_version": PRE_PUSH_VERIFIER_PROTOCOL_VERSION,
            "tool_fingerprints": {
                (PRE_PUSH_TOOL_EMITTER): "deadbeef",
                (PRE_PUSH_TOOL_MASTERPLAN_GENERATOR): "deadbeef",
                (PRE_PUSH_TOOL_ARCHITECTURE_GRAPH_GENERATOR): "deadbeef",
                "not-a-pinned-tool": "deadbeef",
            },
            "generator_source_fingerprint": generator_source_fingerprint(&root).expect("source"),
        });
        std::fs::write(
            hooks.join(PRE_PUSH_VERIFIER_MANIFEST_FILE),
        ...
        .expect("write");
        let error = read_pre_push_verifier_manifest(&hooks, &tools, &root).expect_err("partial");
        let text = error.to_string();
        assert!(text.contains(PRE_PUSH_TOOL_PRODUCER), "{text}");
        assert!(text.contains("reinstall"), "{text}");
    }
    ...
    #[test]
    fn replaced_hook_loses_manifest_ownership() {
        let root = temp_root("oya-pre-push-hook-ownership");
        write_protocol_fixture(&root, PRE_PUSH_VERIFIER_PROTOCOL_VERSION, None);
        let hooks = root.join("hooks");
        std::fs::create_dir_all(&hooks).expect("create hooks dir");
        let tools = fixture_tools_dir(&root);
        ...
        assert!(!manifest_owns_installed_hook(&hooks, &hook).expect("not owned"));
    }

    #[test]
    fn generator_fingerprint_covers_manifests_and_path_deps() {
        let root = temp_root("oya-pre-push-source-fingerprint-inputs");
        write_protocol_fixture(&root, PRE_PUSH_VERIFIER_PROTOCOL_VERSION, None);
        // A manifest-declared path dependency OUTSIDE the five generator src dirs, plus a
        // workspace Cargo.lock: both must be covered by the generator-source fingerprint.
        let dep_root = root.join("external/emitter-dep");
        ...
        std::fs::write(dep_root.join("src/lib.rs"), "pub fn emit() {}\n")
            .expect("write dep source");
        std::fs::write(root.join("Cargo.lock"), "# lock v1\n").expect("write lock");
        let scm = root.join("ci/facade/scm-facts-snapshot");
        std::fs::create_dir_all(&scm).expect("create scm root");
        std::fs::write(
            scm.join("Cargo.toml"),
            "[package]\nname = \"scm-facts-snapshot\"\nversion = \"0.1.0\"\n\n[dependencies.emitter-dep]\npath = \"../../../external/emitter-dep\"\n",
        )
        .expect("write scm manifest");
        let before = generator_source_fingerprint(&root).expect("fingerprint");
        // A change INSIDE a path dependency's source must change the fingerprint.
        std::fs::write(
            dep_root.join("src/lib.rs"),
            "pub fn emit() { /* changed */ }\n",
        )
        .expect("change dep source");
        let after_dep = generator_source_fingerprint(&root).expect("fingerprint");
        assert_ne!(
            before, after_dep,
            "path-dependency source change must change the fingerprint"
        );
        // A change to the workspace Cargo.lock must change the fingerprint.
        std::fs::write(root.join("Cargo.lock"), "# lock v2\n").expect("change lock");
        let after_lock = generator_source_fingerprint(&root).expect("fingerprint");
        assert_ne!(
