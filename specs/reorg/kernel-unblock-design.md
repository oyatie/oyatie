...
**Class:** codemod enhancement.

`cloud/cloud-kernel/Cargo.toml` expresses 5 internal edges as `[workspace.dependencies]` path-deps;
`cargo::rewrite_dep_tables_recursive` walks only `[dependencies]`/`[dev-dependencies]`/
`[build-dependencies]`/`[target.<cfg>]` and deliberately skips `[package]`/`[workspace]`.

**Acceptance criteria:**
- `tools/oya-reorg-codemod-app/src/cargo.rs` gains `[workspace.dependencies]` path-dep recompute
  (same `resolve_target` mapping; fail-closed on ambiguity).
- A unit test in `tools/oya-reorg-codemod-app/tests/fixture_roundtrip.rs` proves a
  workspace-dependency path-dep moves with the plan.
- The kernel plan (post-B1 re-anchor) dry-runs clean.
...
1. Land the codemod B2 enhancement (own PR, gate-green; the codemod is otherwise stationary).
2. Land the B1 workspace split (asterinas re-anchor + kuberos scaffold, own PR, gate-green).
