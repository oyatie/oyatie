...
| File | Status |
|------|--------|
| `governance-check-remainder-move-plan.json` | **LIVE executable** — libs remainder (`libs/oya-check-*` + `libs/oya-governance-*` → `governance/check/*`, 30 crates); also carries spent parent sheets + community + payroll + HR crate-dir pairs so origin/dev...HEAD stays a singleton bijection |
...
> **Spent leaf (this stacked PR):** HR absorb #2192 occupied the singleton via `hr-move-plan.json`; payroll absorb #2194 then occupied it via `payroll-move-plan.json` (carrying spent HR pairs); community absorb #2201 occupied it via `community-move-plan.json`; sheets absorb #2202 occupied it via `sheets-move-plan.json` (sheets plus spent community + payroll + HR pairs). This libs/governance-check remainder absorb deletes `sheets-move-plan.json` as spent and occupies the singleton with `governance-check-remainder-move-plan.json` (30 remainder crates plus spent sheets + community + payroll + HR pairs). DRAFT until #2203 (global-trade) releases the singleton.
>
