---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_DATA-AUTO_REBALANCE
...
# Runbook: Cloud Data Auto Rebalance

## Operator Contract
- Runbook id: cloud-data-auto-rebalance.
- Primary service namespace: `cloud-data`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-data-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-data; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-data rollout status deploy/cloud-data --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-data --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-data --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-data --cell $CELL --window 30m`.
