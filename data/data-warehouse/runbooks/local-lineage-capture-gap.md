# Data Warehouse Runbook: Lineage Capture Gap

Service: data-warehouse  
Surface: local operational primitive suite  
Primary SLO: lineage-capture-completeness  
Domain focus: pipeline, sla-tier, freshness, completeness

## Trigger
- Alert `data-warehouse-lineage-capture-completeness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_warehouse_lineage_capture_completeness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when lineage-capture-completeness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-warehouse-local-lineage-capture-completeness` and policy file name to the ticket.
