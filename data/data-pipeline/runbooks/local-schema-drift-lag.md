# Data Pipeline Runbook: Schema Drift Lag

Service: data-pipeline  
Surface: local operational primitive suite  
Primary SLO: schema-drift-latency  
Domain focus: ingest, transform, quality, freshness

## Trigger
- Alert `data-pipeline-schema-drift-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_pipeline_schema_drift_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when schema-drift-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-pipeline-local-schema-drift-latency` and policy file name to the ticket.
