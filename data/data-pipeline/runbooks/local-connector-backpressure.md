# Data Pipeline Runbook: Connector Backpressure

Service: data-pipeline  
Surface: local operational primitive suite  
Primary SLO: transform-latency  
Domain focus: ingest, transform, quality, freshness

## Trigger
- Alert `data-pipeline-transform-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_pipeline_transform_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when transform-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-pipeline-local-transform-latency` and policy file name to the ticket.
