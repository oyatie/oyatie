//! Fail-closed authorization seam for the `cloud.network.vpc.create` control
//! plane (AUTH-005 / C10 class; ADR-0587).
//!
//! ## Why this module exists
//!
//! VPC creation
//! ([`crate::create_cloud_network_vpc_from_api`]) is a MUTATING
//! multi-tenant control plane. Before this seam the only "authz" was the
//! request-supplied `CloudNetworkVpcApiAuthorization` blob, whose
//! `allowed_surfaces` list the boundary merely cross-checked for internal
//! consistency. An attacker who can reach the call sets
//! `allowed_surfaces = ["cloud.network.vpc.create"]` (with a matching
...
//!    `action = cloud.network.vpc.create` on the TARGET `{tenant, vpc}`
//!    via a PDP [`VpcCreateAuthorizer`] port (`ensure_authorized`). The target
...
//!    default-allow fallback): [`create_cloud_network_vpc_from_api`]
//!    takes a required `&CloudNetworkVpcAuthzProvider`.
//!
//! ## Clean architecture (ADR-0131 / ports-for-owned-stack doctrine)
//!
//! [`PrincipalVerifier`] and [`VpcCreateAuthorizer`] are PORTS owned by this
...
/// cross-check in [`crate::create_cloud_network_vpc_from_api`].
///
...
pub struct VpcCreateResource {
    ...
    /// The VPC resource id being created (from the path/body, already bound
    /// equal).
...
///
/// The decision is
/// `decide(principal, action = cloud.network.vpc.create, resource)`. Adapter: the
...
///    `Err(VpcCreateAuthorizationError::Refused)` so the caller can map them to
...
pub trait VpcCreateAuthorizer: Send + Sync {
    ...
    ///
    /// # Errors
    /// [`VpcCreateAuthorizationError`] on an explicit deny or any PDP fault
...
impl CloudNetworkVpcAuthzProvider {
    ...
    ///
    /// # Errors
    /// [`VpcCreateAuthorizationError`] — caller maps to HTTP 403.
