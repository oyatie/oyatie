...
pub use authz::{
    AuthzProviderConfigError, CallerCredential, CloudNetworkLbAuthzProvider,
    ConfiguredBearerPrincipalVerifier, LbCreateAuthorizationError, LbCreateAuthorizer,
    LbCreateResource, PrincipalVerificationError, PrincipalVerifier, VerifiedPrincipal,
    constant_time_eq,
};
...
impl CloudNetworkLbApiErrorCode {
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CallerUnauthenticated => "CLOUD_NETWORK_LB_CALLER_UNAUTHENTICATED",
            Self::VerifiedPrincipalMismatch => "CLOUD_NETWORK_LB_VERIFIED_PRINCIPAL_MISMATCH",
            Self::VerifiedTenantMismatch => "CLOUD_NETWORK_LB_VERIFIED_TENANT_MISMATCH",
...
pub struct CloudNetworkLbCreateApiRequest {
    ...
    /// identity, never a grant. (C9 fix: a request blob never authorizes.)
...
impl CloudNetworkLbApiError {
    ...
    pub fn code(&self) -> CloudNetworkLbApiErrorCode {
        match self {
            ...
            Self::CallerUnauthenticated => CloudNetworkLbApiErrorCode::CallerUnauthenticated,
            Self::VerifiedPrincipalMismatch { .. } => {
                CloudNetworkLbApiErrorCode::VerifiedPrincipalMismatch
            }
            Self::VerifiedTenantMismatch { .. } => {
                CloudNetworkLbApiErrorCode::VerifiedTenantMismatch
            }
    ...
    fn message(&self) -> &'static str {
        match self {
            ...
            Self::AuthorizationDenied { .. } => {
                "The policy decision point denied the requested Cloud Network load balancer surface"
            }
...
/// [`create_cloud_network_load_balancer_from_api`], which holds the injected
/// [`authz::CloudNetworkLbAuthzProvider`]. Shape validation alone NEVER grants
/// the request.
...
/// Create a Cloud Network load balancer from an API request, FAIL-CLOSED.
///
/// The flow is (C9 fix; ADR-0587):
...
///    ([`CloudNetworkLbApiError::CallerUnauthenticated`]) on missing/invalid;
...
///    principal/tenant);
/// 4. AUTHORIZE via the PDP against the TARGET `{tenant, load_balancer}` derived
///    from the trusted body → `403` on deny/fault (fail-closed, never allow);
...
pub fn create_cloud_network_load_balancer_from_api(
    ...
    authz_provider: &authz::CloudNetworkLbAuthzProvider,
    request: CloudNetworkLbCreateApiRequest,
) -> Result<CloudNetworkLbCreateSuccessResponse, CloudNetworkLbApiError> {
...
/// load-balancer id). No request-supplied blob authorizes anything.
fn authorize_request(
    ...
    let verified = authz_provider
        ...
        .map_err(|error| match error {
            ...
            | authz::PrincipalVerificationError::InvalidCredential => {
                CloudNetworkLbApiError::CallerUnauthenticated
            }
        })?;
    ...
    if verified.principal_id() != request.principal.principal_id {
        return Err(CloudNetworkLbApiError::VerifiedPrincipalMismatch {
    ...
    if verified.tenant_id() != request.body.tenant_id {
        return Err(CloudNetworkLbApiError::VerifiedTenantMismatch {
    ...
    let resource = authz::LbCreateResource {
        tenant_id: request.body.tenant_id.clone(),
        load_balancer_id: request.body.resource_id.clone(),
    };
    authz_provider
        .ensure_authorized(&verified, &resource)
        .map_err(|error| match error {
            authz::LbCreateAuthorizationError::Denied
            | authz::LbCreateAuthorizationError::Refused => {
