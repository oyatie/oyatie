//! Fail-closed authorization seam for the `cloud.network.lb.create` control plane.
//!
...
//! resource}` by a [`LbCreateAuthorizer`] port. The request-supplied principal id is never the source of
...
pub struct LbCreateResource {
    ...
    /// The load balancer resource id being created, already bound equal to the request body.
...
/// parse failure, unavailability, expiry — to `Err(LbCreateAuthorizationError::Refused)`, MUST enforce its own
...
pub trait LbCreateAuthorizer: Send + Sync {
    ...
    ///
    /// # Errors
    /// [`LbCreateAuthorizationError`] on an explicit deny or any PDP fault; the caller maps this to 403.
...
/// The authz provider the boundary depends on: a [`PrincipalVerifier`] port plus a [`LbCreateAuthorizer`]
...
impl CloudNetworkLbAuthzProvider {
    ...
    /// Authorize the verified principal for the LB-create resource via the PDP port; default-deny/fail-closed.
    ///
    /// # Errors
    /// [`LbCreateAuthorizationError`] — caller maps to 403.
