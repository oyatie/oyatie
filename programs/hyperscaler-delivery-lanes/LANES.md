...
| **R3-postmerge-trunk** | L4 | Promoted-SHA re-verify; cache-writer; completion packets ([template](R3-postmerge-packet-template.md); example filled: [1559](../cas-fabric/evidence/1559-post-merge-completion-packet.json)) | Yes (docs/automation only until trusted push) | After merges exist | No baseline write from PR head; PR-head green ≠ packet |
| **R4-local-assist** | L0–L1 | Pre-push: OWNERS+YAML tip **and** `.grok/bin/preflight-ci-infra` (+ `mm-drive preflight-ci` / merge-check receipt) for CI-infra paths | Yes (tips + briefs + open-pr-fleet) | Reduces FULL thrash | Tool under `.grok/bin` only — **not** `infra/*`; non-admission |
...
4. **R4** — use `.grok/bin/preflight-ci-infra` before CI-infra pushes (wired into fleet + merge-check)
5. **R8** — already MVP’d (`mm-drive` + `preflight-ci`); optional polish
