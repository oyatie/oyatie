...
---
# IP-WAVE-15-ZD-sharding-automation: Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/connector/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: connector must interpret ADR-0348 through its own bounded context: connector-catalog.
SCOPE-004: connector uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: connector uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: connector uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: connector.
STANCE-002: Owner team: council-architecture.
...
STANCE-005: Bounded context anchor: connector-catalog.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: us, eu, kr, us-healthcare, hipaa, gdpr, cn-pipl-2021, soc2, iso27001, soc2-type2, eu-ai-act, fedramp, il5, il6, lgpd.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for connect.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for connect because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; connect must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where connect participates.
...
ROLLBACK-002: For connect, rollback starts from the audit-chain row that recorded the automation event.
