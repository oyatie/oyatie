---
doc_class: Runbook
doc_id: RUNBOOK-CONNECTOR-AUTO_REBALANCE
...
# Runbook: Auto Rebalance

## Operator Contract
- Runbook id: connector-auto-rebalance.
- Primary service namespace: `connector`.
...
- Owning team: council-architecture.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-connector-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=connector; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n connect rollout status deploy/connect --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service connect --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service connect --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service connect --cell $CELL --window 30m`.
