//! `oya-shared-connector-kernel` — the enterprise integration substrate.
//!
//! # Purpose
//!
...
//! live in sibling crates named `oya-connector-<vendor>-adapter` and
...
//!   `oya-check-ontology-projection-coverage` enforces coverage.
//! * **Audit chain.** Every Connector call must seal an event through
//!   [`AuditSealHandle`] (ADR-0145). The check
//!   `oya-check-audit-chain-seal-coverage` enforces coverage.
...
/// a projection; `oya-check-ontology-projection-coverage` enforces this.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// in `oya-connector-<vendor>-adapter` crates.
///
/// # Synchronous shape
///
