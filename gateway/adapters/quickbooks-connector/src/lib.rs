//! `gateway-quickbooks-connector` — QuickBooks Online connector.
//!
//! # Coverage
//!
//! QuickBooks Online Accounting API:
//! * `Customer` — accounts receivable parties
//! * `Vendor` — accounts payable parties
...
impl QuickbooksConnector {
    ...
    fn seed(&self) {
        for tenant in ["sandbox", "t-1"] {
            for i in 1..=4 {
                ...
                d.insert("DisplayName", EntityValue::Str(format!("QBO Customer {i}")));
            ...
            for i in 1..=2 {
                ...
                let mut d = EntityDoc::new();
                d.insert("Id", EntityValue::Str(id.clone()));
                d.insert("DisplayName", EntityValue::Str(format!("QBO Vendor {i}")));
            ...
            for i in 1..=2 {
                ...
                let mut d = EntityDoc::new();
                d.insert("Id", EntityValue::Str(id.clone()));
                d.insert("CustomerRef", EntityValue::Str("1".into()));
                d.insert("TotalAmt", EntityValue::Int(250_000));
                self.put(tenant, "Invoice", &id, d);
            }
        }
    }
    ...
    fn check_kind(&self, kind: &str) -> Result<()> {
        match kind {
            "Customer" | "Vendor" | "Invoice" | "Bill" => Ok(()),
...
impl Connector for QuickbooksConnector {
    ...
    fn get(&self, ctx: &ConnectorCtx, entity_kind: &str, id: &str) -> Result<EntityDoc> {
        ...
        self.lock_store()
            ...
            .and_then(|m| m.get(id))
            .cloned()
            .ok_or_else(|| ConnectorError::NotFound(format!("qbo {entity_kind}/{id}")))
    }
    fn create(
        &self,
        ctx: &ConnectorCtx,
    ...
    fn subscribe(
        ...
        Err(ConnectorError::Unsupported(
            "qbo webhooks are app-scoped; per-tenant streams deferred".into(),
        ))
    }
    ...
    fn rate_limits(&self) -> RateLimitDescriptor {
        // 500 req/min per realm (~8/sec).
        RateLimitDescriptor {
            requests_per_second: 8,
            burst_capacity: 40,
            daily_quota: Some(500_000),
            note: "QuickBooks Online 500 req/min per realm".to_owned(),
        }
    }
...
mod tests {
    ...
    fn ctx() -> ConnectorCtx {
        ConnectorCtx::new(
            ...
            PrincipalId::new("svc-qbo").unwrap(),
            SecretReference::new("sref://t-1/qbo/oauth").unwrap(),
    ...
    #[test]
    fn provider() {
        assert_eq!(QuickbooksConnector::new().provider_id(), "quickbooks");
    }
    #[test]
    fn list_customers() {
        assert_eq!(
    ...
    fn list_uses_keyset_page_boundaries() {
        let s = QuickbooksConnector::new();
        for i in 1..=101 {
            let mut d = EntityDoc::new();
            let id = format!("bill-{i:08}");
            d.insert("Id", EntityValue::Str(id.clone()));
            s.put("t-1", "Bill", &id, d);
        }

        let first = s.list(&ctx(), "Bill", None).unwrap();
        ...
        let second = s.list(&ctx(), "Bill", first.next_cursor).unwrap();
        assert_eq!(second.items.len(), 1);
        assert!(second.next_cursor.is_none());

        let exact = QuickbooksConnector::new();
        for i in 1..=100 {
            let mut d = EntityDoc::new();
            let id = format!("bill-{i:08}");
            d.insert("Id", EntityValue::Str(id.clone()));
            exact.put("t-1", "Bill", &id, d);
        }
        let exact_page = exact.list(&ctx(), "Bill", None).unwrap();
    ...
    fn unsupported_entity() {
        assert!(matches!(
            QuickbooksConnector::new().list(&ctx(), "Foo", None),
    ...
    fn idempotent() {
        let s = QuickbooksConnector::new();
        let mut d = EntityDoc::new();
        d.insert("DisplayName", EntityValue::Str("X".into()));
        let k = ik("k");
    ...
    fn update_then_get() {
        ...
        s.update(
            ...
            "Customer",
            "1",
            PatchOp::set("DisplayName", EntityValue::Str("Renamed".into())),
            ik("u"),
        )
        .unwrap();
        assert_eq!(
            s.get(&ctx(), "Customer", "1").unwrap().get("DisplayName"),
    ...
    fn subscribe_unsupported() {
        assert!(matches!(
            QuickbooksConnector::new().subscribe(&ctx(), &["Customer".into()]),
            Err(ConnectorError::Unsupported(_))
        ));
    }
    #[test]
    ...
    fn ontology_projections_present() {
        assert!(QuickbooksConnector::new().ontology_projections().len() >= 3);
    }
    #[test]
    ...
    fn idempotency_conflict_redacts_tenant_and_key() {
        ...
        first.insert("DisplayName", EntityValue::Str("Y".into()));
        let _ = s.create(&ctx(), "Customer", first, key.clone()).unwrap();

        let mut second = EntityDoc::new();
        second.insert("DisplayName", EntityValue::Str("Z".into()));
