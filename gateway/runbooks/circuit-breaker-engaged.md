...
- `api_gateway_upstream_circuit_open_total` > 0 for >2min.
...
1. Identify affected upstream µservice + cell:
   - `api_gateway_upstream_circuit_open_total by (upstream, cell)`.
...
2. **Verify gateway side:**
   - mTLS handshake metric: `api_gateway_upstream_mtls_handshake_failed_total`.
   - Circuit half-open success rate: `api_gateway_upstream_circuit_half_open_success_total`.
...
6. **Monitor recovery:**
   - `api_gateway_upstream_circuit_state{upstream=<svc>}` → `closed`.
