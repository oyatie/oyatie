---
doc_class: Runbook
doc_id: RUNBOOK-API_GATEWAY-AUTO_REBALANCE
...
# Runbook: Api Gateway Auto Rebalance

## Operator Contract
- Runbook id: api-gateway-auto-rebalance.
- Primary service namespace: `api-gateway`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-api-gateway-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=api-gateway; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n api-gateway rollout status deploy/api-gateway --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service api-gateway --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service api-gateway --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service api-gateway --cell $CELL --window 30m`.
