...
bounded_context: canary-route-shift
...
slot_owner: ZF-9
sharding_role: routing-boundary
---
# IP-WAVE-15-ZD-sharding-automation: Api Gateway Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/api-gateway/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: api-gateway must interpret ADR-0348 through its own bounded context: canary-route-shift.
SCOPE-004: api-gateway uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: api-gateway uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: api-gateway uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: api-gateway.
STANCE-002: Owner team: axis-network.
STANCE-003: Sharding role: routing-boundary.
STANCE-004: Role statement: switches request routing at the same transaction boundary as placement or shard topology changes.
STANCE-005: Bounded context anchor: canary-route-shift.
STANCE-006: Cell placement class input: Tier-4.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: us, eu, kr, cn-pipl-2021, us-healthcare, fedramp-high, il5, il6, ksa-pdpl, ae-pdpl, jp, sg, au, in, br, soc2, iso27001, gdpr, fedramp, hipaa, soc2-type2, lgpd.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for api-gateway.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for api-gateway because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; api-gateway must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where api-gateway participates.
...
ADR349-ENFORCED-BY-001: governance-jenkins-github-actions-parity (new lane; refuses Jenkinsfile / .github/workflows drift such that a CI step exists in one surface but not the other across the per-microservice CI-parity contract enumerated in D-3 below; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
...
ROLLBACK-002: For api-gateway, rollback starts from the audit-chain row that recorded the automation event.
