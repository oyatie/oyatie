use iam_tenant_rbac_deployment_evidence::{
    ...
    cloud_deployment_evidence_doc_urls, tenant_rbac_deployment_evidence_plan,
    validate_tenant_rbac_deployment_evidence_plan,
};

#[test]
fn cloud_deployment_evidence_plan_validates_controls_and_nonclaims() {
    let plan = tenant_rbac_deployment_evidence_plan().expect("plan builds");
    validate_tenant_rbac_deployment_evidence_plan(&plan).expect("plan validates");

    assert_eq!(plan.plan_name, "tenant-rbac-deployment-evidence-plan");
...
fn cloud_deployment_evidence_plan_preserves_evidence_ref_boundaries() {
    ...
    assert!(plan.requirements.iter().all(|requirement| {
        requirement
            ...
            && requirement
                .source_manifest_ref
                .starts_with("crates/tenant-rbac-deployment-manifest/")
...
fn cloud_deployment_evidence_plan_rejects_missing_duplicate_and_doc_drift() {
    let mut plan = tenant_rbac_deployment_evidence_plan().expect("plan builds");
    plan.requirements.truncate(2);
    assert_eq!(
        validate_tenant_rbac_deployment_evidence_plan(&plan),
