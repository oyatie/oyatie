//! Live real-handshake mTLS E2E for the iam PDP service (G002
...
use iam_pdp_app::mtls_transport::MtlsContext;
...
// SPIFFE identity (`iam-pdp` in `oyatie.cell-7`) and chains to the trusted
...
async fn empty_bundle_refuses_mtls_boot() {
    ...
    let start_err = StartError::Mtls(iam_pdp_app::mtls::MtlsBootError::TrustBundleEmpty);
...
async fn cell_pin_boot_refuses_server_leaf_with_no_uri_san() {
    ...
    assert!(
        matches!(
            err,
            iam_pdp_app::mtls::MtlsBootError::CellPinUndeterminable(_)
        ),
        "got {err:?}"
    );
}
