...
use iam_pdp_app::mtls_transport::{MtlsContext, MtlsMaterialError};
use iam_pdp_app::server::{self, BootError};
...
async fn production_boot_fails_closed_on_absent_mount() {
    ...
    let absent = std::env::temp_dir().join(format!(
        "iam-pdp-boot-{}-absent-does-not-exist",
        std::process::id()
    ));
...
async fn pdp_fails_closed_when_operator_has_not_produced_the_secret() {
    ...
    let absent = std::env::temp_dir().join(format!(
        "iam-pdp-operator-{}-absent-secret",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&absent);
