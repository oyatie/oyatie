//! Tenant RBAC identity-provider verification foundation.
//!
//! This control-plane crate records the OpenID discovery, JWKS, JWT
//! claim, key-rotation, and route-scope checks required before Tenant RBAC
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TenantRbacIdentityProviderVerificationPlan {
...
pub fn tenant_rbac_identity_provider_verification_plan()
-> TenantRbacIdentityProviderVerificationPlan {
    let auth_policy = tenant_rbac_auth_runtime_policy();
    TenantRbacIdentityProviderVerificationPlan {
        plan_name: "tenant-rbac-identity-provider-verification-plan",
        issuer: auth_policy.expected_issuer,
        audience: auth_policy.expected_audience,
        discovery_document_url: "https://identity.oyatie.com/tenant-rbac/.well-known/openid-configuration",
        jwks_uri: "https://identity.oyatie.com/tenant-rbac/.well-known/jwks.json",
        authorization_endpoint: "https://identity.oyatie.com/tenant-rbac/oauth2/v1/authorize",
        token_endpoint: "https://identity.oyatie.com/tenant-rbac/oauth2/v1/token",
        userinfo_endpoint: "https://identity.oyatie.com/tenant-rbac/oauth2/v1/userinfo",
        subject_claim: "sub",
...
pub fn validate_tenant_rbac_identity_provider_verification_plan(
    plan: &TenantRbacIdentityProviderVerificationPlan,
...
pub fn render_identity_provider_verification_checklist(
    ...
    validate_tenant_rbac_identity_provider_verification_plan(plan)?;
    Ok(format!(
...
fn required_claims(
    ...
    vec![
        ...
        claim("tenant_id", "oyatie-tenant", "tenant-rbac-tenant-isolation"),
        claim("scp", "route-required-scope", "tenant-rbac-auth-runtime"),
