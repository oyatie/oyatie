...
- Identity secondary: PagerDuty `identity-federation-primary`.
...
- Metric `cloud_iam_federation_login_success_ratio` drops below 0.995.
- Metric `cloud_iam_saml_assertion_validation_error_total` spikes.
- Metric `cloud_iam_oidc_jwks_fetch_error_total` spikes.
- Metric `cloud_iam_idp_metadata_age_seconds` exceeds 82800.
- Metric `cloud_iam_jit_provisioning_queue_depth` exceeds 500.
- Metric `cloud_iam_jit_provisioning_lag_seconds` exceeds 300.
- Metric `cloud_iam_session_token_issue_error_ratio` exceeds 0.01.
...
6. Query login success: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=cloud_iam_federation_login_success_ratio{cell="'$CELL'"}'`.
7. Query SAML failures: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=rate(cloud_iam_saml_assertion_validation_error_total[5m])'`.
8. Query OIDC failures: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=rate(cloud_iam_oidc_callback_error_total[5m])'`.
9. Query metadata age: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=cloud_iam_idp_metadata_age_seconds{tenant_id="'$TENANT'",idp="'$IDP'"}'`.
10. Query JIT lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=cloud_iam_jit_provisioning_lag_seconds{tenant_id="'$TENANT'"}'`.
...
## Mitigation
1. Freeze risky IdP config changes: incident hold PR against `dev` (normal VCS PR; branch-protected GitHub Actions `presubmit` required; local/Jenkins rehearsals are non-authoritative).
...
8. Run federation tests: `cargo test -p cloud-iam-api federation -- --nocapture`.
9. Run JIT tests: `cargo test -p cloud-iam-domain jit -- --nocapture`.
10. Verify the branch-protected production-snapshot gate for `cloud-iam-federation` in `presubmit` / cloud-ci for `$CELL`; do not use local dev-cli output as merge authority.
...
- `cloud_iam_federation_login_success_ratio` is above 0.995.
- `cloud_iam_jit_provisioning_queue_depth` is zero for the affected tenant.
- `cloud_iam_jit_provisioning_lag_seconds` is below 60.
...
## Escalation Path
- Page `cloud-iam-primary` for all federation Sev1 incidents.
- Page `identity-federation-primary` when metadata, JWKS, or JIT is implicated.
- Page `cloud-kms-primary` when session signer lease fails.
- Page `cloud-network-primary` when ACS or callback ingress fails.
- Page `audit-chain-primary` when session issuance cannot seal.
