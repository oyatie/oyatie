---
doc_class: Runbook
doc_id: RUNBOOK-CONSENT_GRAPH-COLD_MERGE
...
# Runbook: Consent Graph Cold Merge

## Operator Contract
- Runbook id: consent-graph-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-consent-graph-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=consent-graph; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n consent-graph rollout status deploy/consent-graph --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service consent-graph --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service consent-graph --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service consent-graph --cell $CELL --window 30m`.
