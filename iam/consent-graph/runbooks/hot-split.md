---
doc_class: Runbook
doc_id: RUNBOOK-CONSENT_GRAPH-HOT_SPLIT
...
# Runbook: Consent Graph Hot Split

## Operator Contract
- Runbook id: consent-graph-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-consent-graph-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=consent-graph; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n consent-graph rollout status deploy/consent-graph --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service consent-graph --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service consent-graph --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service consent-graph --cell $CELL --window 30m`.
