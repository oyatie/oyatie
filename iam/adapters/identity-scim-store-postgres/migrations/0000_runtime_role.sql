-- SCIM identity runtime role contract (applied BEFORE
-- 0001_identity_scim_store.sql).
--
-- The RLS policies in 0001 scope `TO identity_scim_runtime` (NOT PUBLIC), so
...
-- ADR-0569 D3 / data/adapters/outbox-adapter-postgres/migrations/0000_runtime_role.sql).
--
-- The runtime role:
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'identity_scim_runtime') THEN
        CREATE ROLE identity_scim_runtime NOLOGIN NOBYPASSRLS;
    END IF;
END
$$;
CREATE SCHEMA IF NOT EXISTS identity_scim;
GRANT USAGE ON SCHEMA identity_scim TO identity_scim_runtime;
