...
| OpenSK vendored reference | ADR-0508, `tools/opensk-vendored/README.md`, `iam/authn-device-firmware/catalog.yaml` | Yes: reference-vendoring status, non-goals, and oya-authn-device gates are explicit. |
...
| ADR-0508 | OpenSK is the Phase-1 authenticator-side reference; oya-authn-device is the Tier-3 bespoke hardware destination. | `tools/opensk-vendored/README.md` reference directory and `iam/authn-device-firmware/catalog.yaml`. | This IP does not vendor upstream OpenSK source or build firmware. |
