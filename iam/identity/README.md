...
- `crates/identity-workload-domain` — PURE domain kernel (zero deps): the
...
- `crates/identity-workload-authz-cedar-adapter` — Cedar authorization gate:
...
- `crates/identity-workload-oidc-adapter` — OIDC token validation core:
...
Verify: `cargo nextest run -p identity-workload-domain \
-p identity-workload-authz-cedar-adapter -p identity-workload-oidc-adapter`
