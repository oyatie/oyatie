...
23. Check catalog components: `find iam/identity/catalog -maxdepth 1 -type f | sort | rg "identity|ip"`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths iam/identity/runbooks/ip-block-incident.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "ip_block_incident|IdentityIpBlockIncidentCritical|identity.ip_block_incident.incident_state" crates microservices/identity -g "!iam/identity/runbooks/**"`.
...
8. Add SLO assertion: `update iam/observability/slos/identity/* with alert IdentityIpBlockIncidentCritical when this was a missing alert`.
9. Add dashboard panel: `update iam/identity/dashboards/scim-provisioning-health.json with oya_identity_ip_block_incident_error_ratio, oya_identity_ip_block_incident_lag_seconds, and oya_identity_ip_block_incident_queue_depth`.
