...
## Trigger Conditions
- Page on alert `IdentityWebauthnRpIdRotationCritical` when `oya_identity_webauthn_rp_id_rotation_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `IdentityWebauthnRpIdRotationSloBurn` when `oya_identity_webauthn_rp_id_rotation_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `oya_identity_webauthn_rp_id_rotation_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `oya_identity_webauthn_rp_id_rotation_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `identity.webauthn-rp-id-rotation.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate identity-webauthn-rp-id-rotation --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `oya_identity_webauthn_rp_id_rotation_error_ratio` rises before `oya_identity_webauthn_rp_id_rotation_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_identity_webauthn_rp_id_rotation_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_identity_webauthn_rp_id_rotation_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_identity_webauthn_rp_id_rotation_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_identity_webauthn_rp_id_rotation_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_identity_webauthn_rp_id_rotation_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate identity-webauthn-rp-id-rotation --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p oya-identity-domain webauthn_rp_id_rotation -- --nocapture`.
16. Check API contract smoke: `curl -s https://identity.internal.oyatie.dev/v1/identity/webauthn-rp-id-rotation/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
23. Check catalog components: `find iam/identity/catalog -maxdepth 1 -type f | sort | rg "identity|webauthn"`.
24. Confirm no cross-cell spread: `oya ops cells query --metric oya_identity_webauthn_rp_id_rotation_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_identity_webauthn_rp_id_rotation_queue_depth grow while oya_identity_webauthn_rp_id_rotation_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths iam/identity/runbooks/webauthn-rp-id-rotation.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://identity.internal.oyatie.dev/v1/identity/webauthn-rp-id-rotation/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: policy or key mismatch.
  - Required action: keep `identity-webauthn-rp-id-rotation-circuit-breaker` open until `oya_identity_webauthn_rp_id_rotation_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "webauthn_rp_id_rotation|IdentityWebauthnRpIdRotationCritical|identity.webauthn_rp_id_rotation.incident_state" crates microservices/identity -g "!iam/identity/runbooks/**"`.
2. Patch domain invariant: `edit oya-identity-domain where webauthn_rp_id_rotation state transition is validated`.
...
6. Add regression test: `cargo test -p oya-identity-domain webauthn_rp_id_rotation_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p oya-dev-cli -- gate validate identity-webauthn-rp-id-rotation --fixture incident-webauthn-rp-id-rotation.json`.
8. Add SLO assertion: `update iam/observability/slos/identity/* with alert IdentityWebauthnRpIdRotationCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/identity/dashboards/jwks-availability.json with oya_identity_webauthn_rp_id_rotation_error_ratio, oya_identity_webauthn_rp_id_rotation_lag_seconds, and oya_identity_webauthn_rp_id_rotation_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_identity_webauthn_rp_id_rotation_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `oya-identity-domain`: inspect for webauthn_rp_id_rotation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `oya-cloud-iam-domain`: inspect for webauthn_rp_id_rotation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `oya-cloud-iam-api`: inspect for webauthn_rp_id_rotation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- oya_identity_webauthn_rp_id_rotation_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- oya_identity_webauthn_rp_id_rotation_lag_seconds < 120 for all production cells.
- oya_identity_webauthn_rp_id_rotation_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include oya_identity_webauthn_rp_id_rotation_error_ratio, oya_identity_webauthn_rp_id_rotation_lag_seconds, oya_identity_webauthn_rp_id_rotation_queue_depth, current breaker state, and audit seal status.
