...
23. Check catalog components: `find iam/identity/catalog -maxdepth 1 -type f | sort | rg "identity|jwks"`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths iam/identity/runbooks/jwks-rotation.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "jwks_rotation|IdentityJwksRotationCritical|identity.jwks_rotation.incident_state" crates microservices/identity -g "!iam/identity/runbooks/**"`.
...
8. Add SLO assertion: `update iam/observability/slos/identity/* with alert IdentityJwksRotationCritical when this was a missing alert`.
