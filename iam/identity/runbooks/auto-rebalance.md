---
doc_class: Runbook
doc_id: RUNBOOK-IDENTITY-AUTO_REBALANCE
...
# Runbook: Identity Auto Rebalance

## Operator Contract
- Runbook id: identity-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-identity-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=identity; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n identity rollout status deploy/identity --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service identity --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service identity --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service identity --cell $CELL --window 30m`.
