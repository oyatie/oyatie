...
## Trigger Conditions
- Page on alert `IdentityPasskeyResetCritical` when `identity_passkey_reset_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `IdentityPasskeyResetSloBurn` when `identity_passkey_reset_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `identity_passkey_reset_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `identity_passkey_reset_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `identity.passkey-reset.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p dev-cli -- gate validate identity-passkey-reset --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `identity_passkey_reset_error_ratio` rises before `identity_passkey_reset_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `identity_passkey_reset_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `identity_passkey_reset_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=identity_passkey_reset_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=identity_passkey_reset_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=identity_passkey_reset_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p dev-cli -- gate validate identity-passkey-reset --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p identity-domain passkey_reset -- --nocapture`.
16. Check API contract smoke: `curl -s https://identity.internal.oyatie.dev/v1/identity/passkey-reset/incident-handoff -H "x-tenant: $TENANT"`.
...
24. Confirm no cross-cell spread: `oya ops cells query --metric identity_passkey_reset_error_ratio --window 30m --threshold 0.02`.
...
2. Does identity_passkey_reset_queue_depth grow while identity_passkey_reset_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/identity/runbooks/passkey-reset.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://identity.internal.oyatie.dev/v1/identity/passkey-reset/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: policy or key mismatch.
  - Required action: keep `identity-passkey-reset-circuit-breaker` open until `identity_passkey_reset_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "passkey_reset|IdentityPasskeyResetCritical|identity.passkey_reset.incident_state" crates microservices/identity -g "!microservices/identity/runbooks/**"`.
2. Patch domain invariant: `edit identity-domain where passkey_reset state transition is validated`.
...
6. Add regression test: `cargo test -p identity-domain passkey_reset_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate identity-passkey-reset --fixture incident-passkey-reset.json`.
8. Add SLO assertion: `update microservices/identity/slos/* with alert IdentityPasskeyResetCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/identity/dashboards/jwks-availability.json with identity_passkey_reset_error_ratio, identity_passkey_reset_lag_seconds, and identity_passkey_reset_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric identity_passkey_reset_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `identity-domain`: inspect for passkey_reset invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `cloud-iam-domain`: inspect for passkey_reset invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `cloud-iam-api`: inspect for passkey_reset invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- identity_passkey_reset_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- identity_passkey_reset_lag_seconds < 120 for all production cells.
- identity_passkey_reset_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include identity_passkey_reset_error_ratio, identity_passkey_reset_lag_seconds, identity_passkey_reset_queue_depth, current breaker state, and audit seal status.
