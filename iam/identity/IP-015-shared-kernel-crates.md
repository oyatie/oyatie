...
# IP-015 — `shared-*` kernel crate exports + reference impls
...
- `shared-oidc-client-kernel` (IP-002 / verifier kernel)
- `shared-webauthn-server-kernel` (IP-004)
- `shared-scim-server-kernel` (IP-007)

Plus the 2 advisory-mode check crates:

- `check-step-up-auth-coverage` (per ADR-0189)
- `check-authz-tier-discipline` (per ADR-0191)
...
|---|---|---|
| shared-oidc-client-kernel | 16 | branches: 90%; lines: 95% |
| shared-webauthn-server-kernel | 10 | branches: 85%; lines: 90% |
| shared-scim-server-kernel | 22 | branches: 90%; lines: 95% |
| check-step-up-auth-coverage | 10 | branches: 80%; lines: 85% |
| check-authz-tier-discipline | 11 | branches: 80%; lines: 85% |
...
```sh
cargo build -p shared-oidc-client-kernel \
            -p shared-webauthn-server-kernel \
            -p shared-scim-server-kernel \
            -p check-step-up-auth-coverage \
            -p check-authz-tier-discipline
cargo test -p ...
# 69 passed; 0 failed.
```
