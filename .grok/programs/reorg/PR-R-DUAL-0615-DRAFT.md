# Draft PR — R-DUAL-0615 dual-home oya/* disposition inventory

**Bead:** oyatie-oso.20  
**Branch:** `agent/rr-dual-home-0615-20260805`  
**Base:** `dev`  
**Class:** mixed inventory (docs/evidence only)  
**Capability span:** multi (storage / comms / observability → app destinations)

## Proposed title

```
docs(reorg): R-DUAL-0615 dual-home oya/* disposition inventory (ADR-0615)
```

## Body (paste into `gh pr create`)
...
## `gh` create command (after commit + push)

```bash
gh pr create --base dev --draft \
  --title "docs(reorg): R-DUAL-0615 dual-home oya/* disposition inventory (ADR-0615)" \
  --body-file .grok/programs/reorg/PR-R-DUAL-0615-DRAFT-body.md
```
