# drive DR failover runbook

## Purpose

Recover the drive service inside the declared ADR-0343 DR envelope without changing tenant version pins or weakening Cedar admission.

## Recovery order

1. Freeze new mutating ingress for drive at api-gateway while read-only routes stay available when data freshness permits.
...
3. promote a warm region, reattach object-storage version pointers, and throttle preview/export workers until DLP queues converge.
