---
doc_class: Runbook
doc_id: RUNBOOK-DRIVE-AUTO_REBALANCE
microservice: drive
...
# Runbook: Drive Auto Rebalance

## Operator Contract
- Runbook id: drive-auto-rebalance.
- Primary service namespace: `drive`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-drive-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=drive; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n drive rollout status deploy/drive --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service drive --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service drive --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service drive --cell $CELL --window 30m`.
