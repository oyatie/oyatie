---
doc_class: Runbook
doc_id: RUNBOOK-DRIVE-COLD_MERGE
microservice: drive
...
# Runbook: Drive Cold Merge

## Operator Contract
- Runbook id: drive-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-drive-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=drive; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n drive rollout status deploy/drive --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service drive --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service drive --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service drive --cell $CELL --window 30m`.
