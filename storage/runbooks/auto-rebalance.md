---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_STORAGE-AUTO_REBALANCE
...
# Runbook: Cloud Storage Auto Rebalance

## Operator Contract
- Runbook id: cloud-storage-auto-rebalance.
- Primary service namespace: `cloud-storage`.
...
- Owning team: axis-cloud-storage.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-storage-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-storage; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-storage rollout status deploy/cloud-storage --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-storage --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service cloud-storage --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service cloud-storage --cell $CELL --window 30m`.
