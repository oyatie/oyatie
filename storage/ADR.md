...
| `ports/draft/provider` | Owner-local object/block backend traits, DTOs, validation, and receipts used by every retained S3/OCI backend adapter; region/AZ/cell identities arrive through the agreed `cell-location` port | Draft compatibility only; foreign data/KMS/compute/residency model edges still require agreed ports before P0 completes |
