---
doc_class: Runbook
doc_id: RUNBOOK-RECORDINGS-COLD_MERGE
...
# Runbook: Recordings Cold Merge

## Operator Contract
- Runbook id: recordings-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-recordings-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=recordings; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n recordings rollout status deploy/recordings --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service recordings --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service recordings --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service recordings --cell $CELL --window 30m`.
