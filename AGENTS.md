...
This file states that ruling. Do not go looking for a decision record behind
it, and do not cite one as its authority: a canonical file carries its own
ruling or it is not authority. Decision identifiers remain valid provenance.
Root `README.md`, `AGENTS.md` and `CLAUDE.md` are the only `.md` files the
repository-layout gate will accept a change to, so a ruling lands here or
nowhere.
