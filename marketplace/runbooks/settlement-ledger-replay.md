...
## Trigger Conditions
- Page on alert `SettlementLedgerReplayCritical` when `marketplace_settlement_ledger_replay_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `SettlementLedgerReplaySloBurn` when `marketplace_settlement_ledger_replay_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open sev1 if `marketplace_settlement_ledger_replay_lag_seconds` exceeds the threshold documented in `microservices/marketplace/slos/deal-accept-latency.openslo.yaml`.
- Open sev1 if `marketplace_settlement_ledger_replay_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `marketplace.settlement-ledger-replay.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p dev-cli -- gate validate marketplace-settlement-ledger-replay --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `marketplace_settlement_ledger_replay_lag_seconds` is red while `marketplace_settlement_ledger_replay_audit_emit_total{status="sealed"}` is flat.
...
- Metric pattern: `marketplace_settlement_ledger_replay_error_ratio` rises before `marketplace_settlement_ledger_replay_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `marketplace_settlement_ledger_replay_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `marketplace_settlement_ledger_replay_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific pattern: `marketplace_settlement_ledger_replay_lag_seconds` rises while `marketplace_settlement_ledger_replay_dependency_error_ratio` is flat; inspect local state before escalating Stripe support.
- Service-specific pattern: `marketplace_settlement_ledger_replay_dependency_error_ratio` rises while `marketplace_settlement_ledger_replay_lag_seconds` is flat; inspect vendor or adjacent-service dependency health before local rollback.
...
- Failure mode 7: regional partition; keep prod-us-east-1 as evidence leader and reject cross-region mutation until `marketplace_settlement_ledger_replay_state_hash_match == 1`.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=marketplace_settlement_ledger_replay_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=marketplace_settlement_ledger_replay_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=marketplace_settlement_ledger_replay_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=marketplace_settlement_ledger_replay_lag_seconds{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p dev-cli -- gate validate marketplace-settlement-ledger-replay --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p cloud-marketplace-domain settlement_ledger_replay -- --nocapture`.
17. Check API contract smoke: `curl -s https://marketplace.internal.oyatie.dev/v1/marketplace/settlement-ledger-replay/incident-handoff -H "x-tenant: $TENANT"`.
...
25. Run targeted SQL state query: `psql $OYATIE_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from marketplace_settlement_ledger_replay_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric marketplace_settlement_ledger_replay_error_ratio --window 30m --threshold 0.02`.
...
2. Does marketplace_settlement_ledger_replay_queue_depth grow while marketplace_settlement_ledger_replay_error_ratio is flat?
...
19. Confirm user impact reduced: `curl -s https://marketplace.internal.oyatie.dev/v1/marketplace/settlement-ledger-replay/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: confirmed DealSet correctness risk.
  - Required action: keep `marketplace-settlement-ledger-replay-circuit-breaker` open until `marketplace_settlement_ledger_replay_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: dependency saturation or replay backlog.
  - Required action: keep `marketplace-settlement-ledger-replay-circuit-breaker` open until `marketplace_settlement_ledger_replay_error_ratio` is below 0.01 for 3 evaluator windows.
...
2. Patch domain invariant: `edit cloud-marketplace-domain where settlement_ledger_replay state transition is validated`.
...
6. Add regression test: `cargo test -p cloud-marketplace-domain settlement_ledger_replay_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate marketplace-settlement-ledger-replay --fixture incident-settlement-ledger-replay.json`.
...
9. Add dashboard panel: `update microservices/marketplace/dashboards/audit-evidence.json with marketplace_settlement_ledger_replay_error_ratio, marketplace_settlement_ledger_replay_lag_seconds, and marketplace_settlement_ledger_replay_lag_seconds`.
...
14. Watch burn rate: `oya ops watch --metric marketplace_settlement_ledger_replay_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `cloud-marketplace-domain`: inspect for `settlement_ledger_replay` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `saas-plugin-marketplace-kernel`: inspect for `settlement_ledger_replay` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `microservices/marketplace/dashboards/audit-evidence.json`: verify panel coverage for `marketplace_settlement_ledger_replay_error_ratio`, `marketplace_settlement_ledger_replay_lag_seconds`, and `marketplace_settlement_ledger_replay_lag_seconds`.
...
- `marketplace_settlement_ledger_replay_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `marketplace_settlement_ledger_replay_lag_seconds < 120` for all production cells.
- `marketplace_settlement_ledger_replay_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `marketplace_settlement_ledger_replay_lag_seconds` is below the threshold documented in `microservices/marketplace/slos/deal-accept-latency.openslo.yaml`.
...
- Capacity math: if `marketplace_settlement_ledger_replay_queue_depth` is 5000 and the worker drains 25 items/second, the best-case drain is 200 seconds before retries; page earlier when drain time exceeds 300 seconds.
...
- Rollback checkpoint 2: before opening `marketplace-settlement-ledger-replay-circuit-breaker`, capture `marketplace_settlement_ledger_replay_request_rate` and `marketplace_settlement_ledger_replay_success_ratio` from Mimir.
...
## Handoff Notes
- Do not hand off with only the alert name; include `marketplace_settlement_ledger_replay_error_ratio`, `marketplace_settlement_ledger_replay_lag_seconds`, `marketplace_settlement_ledger_replay_queue_depth`, `marketplace_settlement_ledger_replay_lag_seconds`, current breaker state, and audit seal status.
...
- The runbook remains current when `SettlementLedgerReplayCritical`, `SettlementLedgerReplaySloBurn`, `marketplace_settlement_ledger_replay_lag_seconds`, `oya.marketplace.settlement_ledger_replay.incident_hold`, and `marketplace-settlement-ledger-replay-circuit-breaker` all resolve to live telemetry, flag, or breaker records.
