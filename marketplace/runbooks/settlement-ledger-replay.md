...
- Open sev1 if `oya_marketplace_settlement_ledger_replay_lag_seconds` exceeds the threshold documented in `marketplace/observability/slos/deal-accept-latency.openslo.yaml`.
...
- Primary dashboard: `https://grafana.dev.oyatie.internal/d/marketplace-ops/settlement-ledger-replay?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=101` backed by `marketplace/dashboards/audit-evidence.json`.
- Secondary dashboard: `https://grafana.dev.oyatie.internal/d/marketplace-ops/settlement-ledger-replay?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=202` backed by `marketplace/dashboards/policy-deny-rate.json`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths marketplace/runbooks/settlement-ledger-replay.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "settlement_ledger_replay|SettlementLedgerReplayCritical|marketplace.settlement_ledger_replay.incident_state" crates microservices/marketplace -g "!marketplace/runbooks/**"`.
...
8. Add SLO assertion: `update marketplace/observability/slos/deal-accept-latency.openslo.yaml with alert SettlementLedgerReplayCritical when this was a missing alert`.
9. Add dashboard panel: `update marketplace/dashboards/audit-evidence.json with oya_marketplace_settlement_ledger_replay_error_ratio, oya_marketplace_settlement_ledger_replay_lag_seconds, and oya_marketplace_settlement_ledger_replay_lag_seconds`.
...
- `marketplace/dashboards/audit-evidence.json`: verify panel coverage for `oya_marketplace_settlement_ledger_replay_error_ratio`, `oya_marketplace_settlement_ledger_replay_lag_seconds`, and `oya_marketplace_settlement_ledger_replay_lag_seconds`.
...
- Service-specific signal `oya_marketplace_settlement_ledger_replay_lag_seconds` is below the threshold documented in `marketplace/observability/slos/deal-accept-latency.openslo.yaml`.
...
- `marketplace/manifest.json` for owner, dependency, capability, and bounded-context vocabulary; topic `settlement-ledger-replay` is the scenario anchor.
