---
doc_class: Runbook
doc_id: RUNBOOK-MARKETPLACE-AUTO_REBALANCE
...
# Runbook: Marketplace Auto Rebalance

## Operator Contract
- Runbook id: marketplace-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-marketplace-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=marketplace; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n marketplace rollout status deploy/marketplace --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service marketplace --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service marketplace --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service marketplace --cell $CELL --window 30m`.
