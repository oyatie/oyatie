---
doc_class: Runbook
title: Seller Onboarding Deny Spike
status: Accepted
date: 2026-05-20
...
# Runbook: Seller Onboarding Deny Spike
...
- API authority: `https://marketplace.internal.oyatie.dev/v1/marketplace/seller-onboarding-deny-spike/incident-handoff`.
- Audit event class: `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT` with ADR-0263 fields `incident_id`, `tenant_id`, `cell_id`, `microservice`, `runbook_id`, `decision_id`, `evidence_hash`, `operator_id`.
- Stop condition: mitigation has held for 30 minutes, `SellerOnboardingDenySpikeCritical` is green, and every Cross-microservice handoff API returns `202 accepted`.
...
- Open sev1 if `oya_marketplace_seller_onboarding_deny_total` exceeds the threshold documented in `marketplace/observability/slos/deal-accept-latency.openslo.yaml`.
...
- Primary dashboard: `https://grafana.dev.oyatie.internal/d/marketplace-ops/seller-onboarding-deny-spike?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=101` backed by `marketplace/dashboards/audit-evidence.json`.
- Secondary dashboard: `https://grafana.dev.oyatie.internal/d/marketplace-ops/seller-onboarding-deny-spike?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=202` backed by `marketplace/dashboards/policy-deny-rate.json`.
...
- Operators see Grafana panel `audit-evidence.json / Seller Onboarding Deny Spike burn rate` turn red before the primary alert resolves.
...
## Diagnostic Steps
1. Set incident variables: `export INCIDENT_ID=INC-marketplace-seller-onboarding-deny-spike-$(date -u +%Y%m%dT%H%M%SZ); export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Confirm active alerts: `curl -s https://marketplace.internal.oyatie.dev/v1/marketplace/alerts?runbook=seller-onboarding-deny-spike | jq .alerts`.
3. Check Kubernetes rollout: `kubectl -n marketplace rollout status deploy/marketplace-seller-onboarding-deny-spike-worker --timeout=60s`.
...
```
- Branch A (confirmed DealSet correctness risk): use the matching mitigation block below and record `decision_branch=A` in `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT`.
- Branch B (dependency saturation or replay backlog): use the matching mitigation block below and record `decision_branch=B` in `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT`.
- Branch C (policy, permit, or tenant-scope drift): use the matching mitigation block below and record `decision_branch=C` in `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT`.
- Branch D (customer-visible or regulated evidence gap): use the matching mitigation block below and record `decision_branch=D` in `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT`.
...
3. Freeze risky automation: `oya flags set oya.marketplace.seller_onboarding_deny_spike.incident_hold=true --cell $CELL --tenant $TENANT --reason $INCIDENT_ID`.
4. Enable circuit breaker: `oya ops breaker open marketplace-seller-onboarding-deny-spike-circuit-breaker --cell $CELL --tenant $TENANT --ttl 30m --reason $INCIDENT_ID`.
5. Reduce blast radius: `kubectl -n marketplace scale deploy/marketplace-seller-onboarding-deny-spike-worker --replicas=1`.
6. Protect tenant boundary: `oya tenancy quarantine --tenant $TENANT --reason marketplace-seller-onboarding-deny-spike --ttl 60m`.
7. Pause promotion: incident hold PR against `dev` (plain `git`; Jenkins + `oya gate run-all --ci-required` required).
8. Drain queue safely: `oya ops marketplace seller-onboarding-deny-spike drain --cell $CELL --tenant $TENANT --max-items 500 --dry-run`.
9. Execute bounded drain: `oya ops marketplace seller-onboarding-deny-spike drain --cell $CELL --tenant $TENANT --max-items 500 --confirm $INCIDENT_ID`.
10. Replay missing audit events: `oya audit-chain replay --event-class EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT --incident $INCIDENT_ID --from evidence/incidents/$INCIDENT_ID.json`.
11. Rollback last deploy if causal: `kubectl -n marketplace rollout undo deploy/marketplace-seller-onboarding-deny-spike-worker`.
12. Raise HPA cap if saturation is proven: `kubectl -n marketplace patch hpa marketplace-seller-onboarding-deny-spike-worker --type merge -p '{"spec":{"maxReplicas":12}}'`.
13. Throttle hot tenant: `oya ops rate-limit set --tenant $TENANT --surface marketplace.seller-onboarding-deny-spike --rps 25 --ttl 30m`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths marketplace/runbooks/seller-onboarding-deny-spike.md,evidence/incidents/$INCIDENT_ID.json`.
...
8. Add SLO assertion: `update marketplace/observability/slos/deal-accept-latency.openslo.yaml with alert SellerOnboardingDenySpikeCritical when this was a missing alert`.
9. Add dashboard panel: `update marketplace/dashboards/audit-evidence.json with oya_marketplace_seller_onboarding_deny_spike_error_ratio, oya_marketplace_seller_onboarding_deny_spike_lag_seconds, and oya_marketplace_seller_onboarding_deny_total`.
...
18. Seal resolution audit: `oya audit-chain emit --event-class EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT --incident $INCIDENT_ID --field resolution=complete --field runbook=seller-onboarding-deny-spike`.
19. Verify seal: `oya audit-chain verify --event-class EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT --incident $INCIDENT_ID`.
...
- `marketplace/dashboards/audit-evidence.json`: verify panel coverage for `oya_marketplace_seller_onboarding_deny_spike_error_ratio`, `oya_marketplace_seller_onboarding_deny_spike_lag_seconds`, and `oya_marketplace_seller_onboarding_deny_total`.
...
- Service-specific signal `oya_marketplace_seller_onboarding_deny_total` is below the threshold documented in `marketplace/observability/slos/deal-accept-latency.openslo.yaml`.
...
- Feature flag `oya.marketplace.seller_onboarding_deny_spike.incident_hold` is false for the affected tenant unless long-term hold is approved.
...
- Rollback command for deployment: `kubectl -n marketplace rollout undo deploy/marketplace-seller-onboarding-deny-spike-worker`.
- Rollback command for tenant throttle: `oya ops rate-limit clear --tenant $TENANT --surface marketplace.seller-onboarding-deny-spike --reason rollback-$INCIDENT_ID`.
- Stop rollback if `EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT` cannot be emitted; preserve the current state and escalate to audit-chain before additional mutation.
...
runbook_id: marketplace-seller-onboarding-deny-spike
microservice: marketplace
event_class: EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT
incident_id: <INC-...>
severity: sev1
status: draft
detected_at: <UTC>
...
# Seller Onboarding Deny Spike postmortem

## Summary
- What happened in marketplace/seller-onboarding-deny-spike.
...
- Emit EVT_MARKETPLACE_SELLER_ONBOARDING_DENY_SPIKE_INCIDENT with incident_id, tenant_id, cell_id, principal_id, decision_id, evidence_hash, operator_id, runbook_id.
...
- Customer communications: use status page component `oyatie-marketplace-seller-onboarding-deny-spike` and keep private details in the incident channel.
...
- Notify `payments`: `oya incident handoff --target payments --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID --severity sev1 --branch A`; expect `202 accepted`.
- Require `payments` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `treasury`: `oya incident handoff --target treasury --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID --severity sev1 --branch B`; expect `202 accepted`.
- Require `treasury` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `finops-portal`: `oya incident handoff --target finops-portal --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID --severity sev1 --branch C`; expect `202 accepted`.
- Require `finops-portal` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `ontology`: `oya incident handoff --target ontology --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID --severity sev1 --branch D`; expect `202 accepted`.
- Require `ontology` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `workflow-engine`: `oya incident handoff --target workflow-engine --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID --severity sev1 --branch A`; expect `202 accepted`.
- Require `workflow-engine` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Observability handoff API: `oya incident handoff --target observability --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID`.
- Governance handoff API: `oya incident handoff --target governance --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID`.
- Compliance handoff API: `oya incident handoff --target compliance --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID`.
- Audit-chain handoff API: `oya incident handoff --target audit-chain --source marketplace --runbook seller-onboarding-deny-spike --incident $INCIDENT_ID`.
...
- `marketplace/manifest.json` for owner, dependency, capability, and bounded-context vocabulary; topic `seller-onboarding-deny-spike` is the scenario anchor.
