...
fn active_artifact_contract_graph_projection_is_complete_and_byte_exact() {
    ...
    assert_eq!(
        ...
        "the registered producer must emit exactly one ordered declares edge for every registry row"
    );
    ...
    assert_eq!(
        ...
        "the registered producer must emit the exact canonical byte projection for every registry row"
    );
}
