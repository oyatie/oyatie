---
doc_class: Runbook
doc_id: RUNBOOK-PLUGIN_APP_STORE-HOT_SPLIT
...
# Runbook: Plugin App Store Hot Split

## Operator Contract
- Runbook id: plugin-app-store-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plugin-app-store-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plugin-app-store; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plugin-app-store rollout status deploy/plugin-app-store --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plugin-app-store --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service plugin-app-store --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service plugin-app-store --cell $CELL --window 30m`.
