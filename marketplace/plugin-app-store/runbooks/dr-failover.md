# plugin-app-store DR failover runbook

## Purpose

Recover the plugin-app-store service inside the declared ADR-0343 DR envelope without changing tenant version pins or weakening Cedar admission.

## Recovery order

1. Freeze new mutating ingress for plugin-app-store at api-gateway while read-only routes stay available when data freshness permits.
...
3. quarantine plugin execution, promote catalog/install metadata, reseal vetting evidence, and re-enable sandbox admission only after policy replay passes.
