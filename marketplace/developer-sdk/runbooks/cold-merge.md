---
doc_class: Runbook
doc_id: RUNBOOK-DEVELOPER_SDK-COLD_MERGE
...
# Runbook: Developer Sdk Cold Merge

## Operator Contract
- Runbook id: developer-sdk-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-developer-sdk-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=developer-sdk; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n developer-sdk rollout status deploy/developer-sdk --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service developer-sdk --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service developer-sdk --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service developer-sdk --cell $CELL --window 30m`.
