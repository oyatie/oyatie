...
# See ADR-0119: vanilla kubeadm + containerd is the canonical stack.
# Edge cells (M07+) may still sanction k3s under a separate future ADR.
set -e
echo "ERROR: k3s install retired per ADR-0119 (on-prem stack: kubeadm + containerd + Istio + Envoy)." >&2
