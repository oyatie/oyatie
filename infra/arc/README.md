...
**Status: RETIRED.** Custom Actions Runner Controller scale sets `arm64` and
`live-postgres-arm64` are decommissioned. Merge CI is **GitHub-hosted
`ubuntu-latest` only** (`presubmit`). Soft multi-arch stays on GitHub-hosted
...
When you have `kubectl` / Helm / Argo access to `talos`:

1. **Sync** Argo Applications `arm64` and `live-postgres-arm64` (or
...
4. **Remove** (or disable) Argo Application entries in
   `infra/gitops/values.yaml` for `arc`, `arm64`,
   `live-postgres-arm64`, `live-postgres-network-policy` after a reviewed
...
5. **Clear** any forced repo runner labels / webhook routing that still name
   `arm64` or `live-postgres-arm64` (GitHub Settings → Actions → Runners).
