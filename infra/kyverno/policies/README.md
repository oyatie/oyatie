...
3. Re-run the BuildKit Jobs for:
   - `registry.registry.svc.cluster.local:5000/ci-webhook-gateway:dev`
   - `registry.registry.svc.cluster.local:5000/rust-ci:dev`
   - `registry.registry.svc.cluster.local:5000/cloud-intelligence:dev`
4. After all three images have keyless signatures, Rekor entries, SLSA
   ...
   `verify-registry-images-keyless` `ClusterPolicy` in `Enforce` mode.
