...
# NOTE: api.oyatie.com is a PUBLIC API gateway and is intentionally NOT
# Cloudflare-Access-gated. End-caller authentication is the backend's job
# (JWT / API keys / OAuth at the workspace-shell layer; later OCI API
# Gateway at M3-P03). Cloudflare's WAF, Rate Limiting, and Bot Management
...
output "access_app_ids" {
  value = {
    ...
    // api intentionally absent — public API gateway, not Access-gated.
  }
}
