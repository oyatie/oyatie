#!/usr/bin/env bash
# init.sh — install Cluster API onto the Oyatie management (hub) cluster.
#
# Run this AFTER the hub is up (USB-installed Talos, kubeconfig in hand). No throwaway kind
# cluster needed — the USB-formed hub IS the bootstrap target; clusterctl inits directly onto it.
#
#   export KUBECONFIG=<hub kubeconfig>           # from `talosctl kubeconfig` on the hub
...
# Then the hub provisions spokes from git-committed Cluster CRs (infra/capi/clusters/).
set -euo pipefail
...
: "${KUBECONFIG:?export KUBECONFIG to the hub cluster}"
kubectl get nodes >/dev/null 2>&1 || { echo "hub cluster not reachable via KUBECONFIG" >&2; exit 1; }
...
echo
echo "CAPI installed on the hub. Verify: clusterctl describe cluster <name>  /  kubectl get providers -A"
