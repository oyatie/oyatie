#!/usr/bin/env bash
# gen-usb.sh <hub|node> — build a bootable Talos USB image for bare-metal auto-install.
#
#   hub  : config BAKED into the ISO (offline). Boot the hub machine(s) -> auto-install ->
#          the CAPI management cluster forms itself. Needs HUB_ENDPOINT.
#   node : GENERIC image (Kata baked, no secrets). Boots with talos.config=$CONFIG_URL and
#          fetches its role/cluster config from the hub. One image for ALL bare-metal nodes.
#
...
# Output ISO -> dd to USB:  sudo dd if=_out/<preset>-metal-amd64.iso of=/dev/sdX bs=4M oflag=sync status=progress
set -euo pipefail

PRESET="${1:?usage: gen-usb.sh <hub|node>}"
...
case "$PRESET" in
  hub)
    : "${HUB_ENDPOINT:?set HUB_ENDPOINT=https://<hub-ip-or-vip>:6443}"
    CLUSTER="${CLUSTER:-oya-hub}"
    ...
    talosctl gen config "$CLUSTER" "$HUB_ENDPOINT" \
      --with-secrets "$SECRETS/secrets.yaml" \
      --kubernetes-version "$K8S_VERSION" \
      --install-disk "$INSTALL_DISK" \
      --output-types controlplane \
      --output "$SECRETS/hub-controlplane.yaml" \
      --config-patch "@$HERE/patches/hub-cp.yaml" \
      --force
    echo ">> building hub ISO (vanilla + embedded config)"
    imager iso --arch "$ARCH" --embedded-config-path /secrets/hub-controlplane.yaml
    mv -f "$OUT/metal-${ARCH}.iso" "$OUT/hub-metal-${ARCH}.iso" 2>/dev/null || true
    ;;
  node)
    : "${CONFIG_URL:?set CONFIG_URL=https://join.oyatie.dev/config (hub config endpoint)}"
    ...
    # bakes the Kata extension + a talos.config kernel arg pointing at the hub config endpoint.
    ...
    ;;
  *) echo "unknown preset: $PRESET (want hub|node)" >&2; exit 1 ;;
esac

echo
echo "DONE. Image(s) in $OUT:"; ls -lh "$OUT"/*.iso 2>/dev/null | awk '{print "  "$5,$NF}'
echo "Write to USB:  sudo dd if=$OUT/${PRESET}-metal-${ARCH}.iso of=/dev/sdX bs=4M oflag=sync status=progress"
