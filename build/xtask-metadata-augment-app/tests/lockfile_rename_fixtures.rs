...
version = "0.2.0"
"#;
    let m = make_map(&[("oya-platform-secrets-kernel", "oya-shared-secrets-domain")]);
    let out = rewrite(content, &m);
...
checksum = "deadbeef"
"#;
    let m = make_map(&[("oya-platform-eventing-kernel", "oya-shared-eventing-domain")]);
    let out = rewrite(content, &m);
