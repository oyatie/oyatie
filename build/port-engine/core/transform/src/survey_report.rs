//! What a survey REPORTS, as data a caller reads rather than prose.
//!
//! Split from `survey.rs` because the shapes and the measurement are different concerns. The
//! four outcomes are the substance: `translated` and `refused` are what the engine can and cannot
//! do, `deferred` is what someone decided not to do, and `uncaptured` is work indisputably lost —
//! keeping them apart is what stops a number that flatters the engine.

use std::collections::BTreeMap;

use port_engine_api::UnitId;
