...
/// Attempt to port the sentinel-identity corpus, returning the refusal.
///
/// # Errors
/// [`PipelineError`] — a `Transform` refusal is the SUCCESSFUL outcome for this input.
pub fn port_go_refused_sentinel() -> Result<usize, PipelineError> {
    refuse(admit_embedded_fixture_sentinel_v1().map_err(PipelineError::Admit)?)
}
