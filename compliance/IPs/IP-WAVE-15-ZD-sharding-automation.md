...
bounded_context: auditor-engagement-read
...
slot_owner: ZF-9
sharding_role: pack-policy-owner
---
# IP-WAVE-15-ZD-sharding-automation: Compliance Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/compliance/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: compliance must interpret ADR-0348 through its own bounded context: auditor-engagement-read.
SCOPE-004: compliance uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: compliance uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: compliance uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: compliance.
STANCE-002: Owner team: axis-compliance.
STANCE-003: Sharding role: pack-policy-owner.
STANCE-004: Role statement: evaluates compliance-pack eligibility and refusal evidence for candidate target cells.
STANCE-005: Bounded context anchor: auditor-engagement-read.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_workflow_run.
STANCE-008: Compliance pack input: soc2-type2, gdpr, hipaa, PCI-DSS, soc2, iso27001-2022, kr-pipa, iso27001, il5, il6, fedramp-high, eu-ai-act, cn-pipl-2021, fedramp.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for compliance.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for compliance because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; compliance must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where compliance participates.
...
ROLLBACK-002: For compliance, rollback starts from the audit-chain row that recorded the automation event.
