...
## Trigger Conditions
- Page on alert `ComplianceManualEvidenceUploadRejectedCritical` when `oya_compliance_manual_evidence_upload_rejected_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `ComplianceManualEvidenceUploadRejectedSloBurn` when `oya_compliance_manual_evidence_upload_rejected_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `oya_compliance_manual_evidence_upload_rejected_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `oya_compliance_manual_evidence_upload_rejected_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate compliance-manual-evidence-upload-rejected --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `oya_compliance_manual_evidence_upload_rejected_error_ratio` rises before `oya_compliance_manual_evidence_upload_rejected_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_compliance_manual_evidence_upload_rejected_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_compliance_manual_evidence_upload_rejected_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_compliance_manual_evidence_upload_rejected_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_compliance_manual_evidence_upload_rejected_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_compliance_manual_evidence_upload_rejected_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate compliance-manual-evidence-upload-rejected --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p oya-compliance-domain manual_evidence_upload_rejected -- --nocapture`.
16. Check API contract smoke: `curl -s https://compliance.internal.oyatie.dev/v1/compliance/manual-evidence-upload-rejected/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
23. Check catalog components: `find compliance/catalog -maxdepth 1 -type f | sort | rg "compliance|manual"`.
24. Confirm no cross-cell spread: `oya ops cells query --metric oya_compliance_manual_evidence_upload_rejected_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_compliance_manual_evidence_upload_rejected_queue_depth grow while oya_compliance_manual_evidence_upload_rejected_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths compliance/runbooks/manual-evidence-upload-rejected.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://compliance.internal.oyatie.dev/v1/compliance/manual-evidence-upload-rejected/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: single tenant isolated.
  - Required action: keep `compliance-manual-evidence-upload-rejected-circuit-breaker` open until `oya_compliance_manual_evidence_upload_rejected_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "manual_evidence_upload_rejected|ComplianceManualEvidenceUploadRejectedCritical|compliance.manual_evidence_upload_rejected.incident_state" crates compliance -g "!compliance/runbooks/**"`.
2. Patch domain invariant: `edit oya-compliance-domain where manual_evidence_upload_rejected state transition is validated`.
...
6. Add regression test: `cargo test -p oya-compliance-domain manual_evidence_upload_rejected_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p oya-dev-cli -- gate validate compliance-manual-evidence-upload-rejected --fixture incident-manual-evidence-upload-rejected.json`.
8. Add SLO assertion: `update compliance/observability/slos/* with alert ComplianceManualEvidenceUploadRejectedCritical when this was a missing alert`.
9. Add dashboard panel: `update compliance/dashboards/breach-notification-sla.json with oya_compliance_manual_evidence_upload_rejected_error_ratio, oya_compliance_manual_evidence_upload_rejected_lag_seconds, and oya_compliance_manual_evidence_upload_rejected_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_compliance_manual_evidence_upload_rejected_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `oya-compliance-domain`: inspect for manual_evidence_upload_rejected invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `oya-regional-pack-domain`: inspect for manual_evidence_upload_rejected invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `oya-regional-pack-api`: inspect for manual_evidence_upload_rejected invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- oya_compliance_manual_evidence_upload_rejected_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- oya_compliance_manual_evidence_upload_rejected_lag_seconds < 120 for all production cells.
- oya_compliance_manual_evidence_upload_rejected_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include oya_compliance_manual_evidence_upload_rejected_error_ratio, oya_compliance_manual_evidence_upload_rejected_lag_seconds, oya_compliance_manual_evidence_upload_rejected_queue_depth, current breaker state, and audit seal status.
