---
doc_class: Runbook
doc_id: RUNBOOK-COMPLIANCE-AUTO_REBALANCE
...
# Runbook: Compliance Auto Rebalance

## Operator Contract
- Runbook id: compliance-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-compliance-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=compliance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n compliance rollout status deploy/compliance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service compliance --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service compliance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service compliance --cell $CELL --window 30m`.
