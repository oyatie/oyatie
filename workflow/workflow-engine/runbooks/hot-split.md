---
doc_class: Runbook
doc_id: RUNBOOK-WORKFLOW_ENGINE-HOT_SPLIT
...
# Runbook: Workflow Engine Hot Split

## Operator Contract
- Runbook id: workflow-engine-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-workflow-engine-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=workflow-engine; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n workflow-engine rollout status deploy/workflow-engine --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service workflow-engine --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service workflow-engine --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service workflow-engine --cell $CELL --window 30m`.
