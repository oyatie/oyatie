...
ip_id: IP-journey-j80-tenant-pack-scope
...
# IP-journey-j80-tenant-pack-scope - tenancy

## Goal

Implement the `tenant-pack-scope` slice for `tenancy` so j80 can satisfy `KR-PIPA + KR-CSAP` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/tenancy/contracts/openapi/j80-tenant-pack-scope-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/tenancy/contracts/asyncapi/j80-tenant-pack-scope-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/tenancy/contracts/proto/j80-tenant-pack-scope-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/tenancy/policy/j80-tenant-pack-scope.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/tenancy/runbooks/j80-tenant-pack-scope-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/tenancy/tests/j80_tenant_pack_scope_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J80TenancyTenantPackScopeStarted`.
- `J80TenancyTenantPackScopePermitted`.
- `J80TenancyTenantPackScopeDenied`.
- `J80TenancyTenantPackScopeCommitted`.
- `J80TenancyTenantPackScopeRolledBack`.

## Implementation rows

### IP row 001 - tenancy tenant-pack-scope
Scope: implement one tenancy behavior for j80, not a cross-service refactor.
