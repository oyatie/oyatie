...
tenancy owns tenant scope, pack activation state, and audience-type boundaries for j98-au-privacy-apra-cps-234-tenant. The slice is a flat per-microservice implementation plan under tenancy/, matching ADR-0131.
...
- Surface evidence: `tenancy/IP-journey-j98-au-privacy-apra-cps234.md` matched `emission`; anchors `tenancy/manifest.json, crates/oya-tenancy-api/src/lib.rs`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
