...
ip_id: IP-journey-j78-tenant-pack-scope
...
# IP-journey-j78-tenant-pack-scope - tenancy

## Goal

Implement the `tenant-pack-scope` slice for `tenancy` so j78 can satisfy `EU-NIS2` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/tenancy/contracts/openapi/j78-tenant-pack-scope-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/tenancy/contracts/asyncapi/j78-tenant-pack-scope-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/tenancy/contracts/proto/j78-tenant-pack-scope-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/tenancy/policy/j78-tenant-pack-scope.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/tenancy/runbooks/j78-tenant-pack-scope-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/tenancy/tests/j78_tenant_pack_scope_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J78TenancyTenantPackScopeStarted`.
- `J78TenancyTenantPackScopePermitted`.
- `J78TenancyTenantPackScopeDenied`.
- `J78TenancyTenantPackScopeCommitted`.
- `J78TenancyTenantPackScopeRolledBack`.

## Implementation rows

### IP row 001 - tenancy tenant-pack-scope
Scope: implement one tenancy behavior for j78, not a cross-service refactor.
...
- Surface evidence: `microservices/tenancy/IP-journey-j78-tenant-pack-scope.md` matched `openapi, asyncapi, .proto`; contract files `microservices/tenancy/contracts/openapi/tenancy.yaml, microservices/tenancy/contracts/asyncapi/tenant-events.yaml, microservices/tenancy/contracts/proto/tenancy.proto`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
