...
acceptance_lanes: [cargo-nextest, governance-openslo-conformance]
---
      ...
      ratioMetric:
        good:
          metricSource:
            type: Prometheus
            spec:
              query: 'sum(rate(http_requests_total{job="tenancy-tenant-lifecycle-rest",status!~"5.."}[5m]))'
        total:
          metricSource:
            type: Prometheus
            spec:
              query: 'sum(rate(http_requests_total{job="tenancy-tenant-lifecycle-rest"}[5m]))'
  objectives:
      ...
      ratioMetric:
        good:
          metricSource:
            type: Prometheus
            spec:
              query: 'sum(rate(tenancy_synthetic_cross_tenant_probe_success_total[5m]))'
        total:
          metricSource:
            type: Prometheus
            spec:
              query: 'sum(rate(tenancy_synthetic_cross_tenant_probe_total[5m]))'
  objectives:
...
cargo nextest run -p tenancy-tenant-lifecycle-adapter-postgres --test synthetic_cross_tenant_probe
cargo run -p dev-cli -- gate validate openslo-conformance --microservice tenancy
```
...
- Surface evidence: `microservices/tenancy/IP-014-tests-load-drills-observability-slos.md` matched `p99, SLO`; anchors `microservices/tenancy/runbooks/dr-pair-promotion-drill.md, crates/tenancy-api/src/lib.rs`; type anchor `crates/tenancy-api/src/lib.rs::TenantCreateApiRequest`.
