...
impl<S: LifecycleStore> AppState<S> {
    /// Wrap a lifecycle provider for serving.
    #[must_use]
    pub fn new(provider: TenantLifecycleProvider<S>) -> Self {
        Self {
            provider: Arc::new(Mutex::new(provider)),
        }
    }

    fn lock(&self) -> Result<MutexGuard<'_, TenantLifecycleProvider<S>>, HandlerError> {
        self.provider.lock().map_err(|_| {
            err(
                StatusCode::INTERNAL_SERVER_ERROR,
                "INTERNAL",
                "lifecycle provider lock poisoned",
            )
        })
    }
}
...
/// Build the axum router for the tenant lifecycle service over `provider`.
pub fn build_router<S>(provider: TenantLifecycleProvider<S>) -> Router
where
...
/// Build a router backed by the in-memory store (single-node bring-up).
pub fn build_inmemory_router() -> Router {
    build_router(TenantLifecycleProvider::new(
        InMemoryTenantLifecycleStore::new(),
    ))
}
...
/// in-memory store. Production swaps a persistent store behind the same port.
///
/// # Errors
...
pub async fn serve(listen_addr: &str) -> Result<(), BootError> {
    ...
    tracing::info!(addr = listen_addr, "tenancy-tenant-lifecycle listening");
