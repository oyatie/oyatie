...
| R-04 (cross-pack misroute) | Pack-pinning enforced at OTel-collector + at tenancy-lifecycle-adapter; integration tests; runtime detector `tenancy_pack_misroute_total` triggers Sev-1 | L | axis-tenancy + ops-sre-reliability |
| R-05 (DSR incomplete) | LEAN check `governance-dsr-handler-conformance`: every µservice must register handler; quarterly drill; missing-receipt halt-and-escalate with DPO sign-off path; receipt-aggregation Merkle root publishes "expected_n vs received_n" | M (engineering-discipline residual; acceptable given drill + halt-and-escalate) | council-privacy + every µservice owner |
