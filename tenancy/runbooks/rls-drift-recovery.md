...
23. Check catalog components: `find tenancy/catalog -maxdepth 1 -type f | sort | rg "tenancy|rls"`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths tenancy/runbooks/rls-drift-recovery.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "rls_drift_recovery|TenancyRlsDriftRecoveryCritical|tenancy.rls_drift_recovery.incident_state" crates microservices/tenancy -g "!tenancy/runbooks/**"`.
...
8. Add SLO assertion: `update tenancy/observability/slos/* with alert TenancyRlsDriftRecoveryCritical when this was a missing alert`.
9. Add dashboard panel: `update tenancy/dashboards/quota-utilisation.json with oya_tenancy_rls_drift_recovery_error_ratio, oya_tenancy_rls_drift_recovery_lag_seconds, and oya_tenancy_rls_drift_recovery_queue_depth`.
...
- `tenancy/dashboards/quota-utilisation.json`: verify this surface only when the incident evidence points there.
