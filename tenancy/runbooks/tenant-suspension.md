...
## Trigger Conditions
- Page on alert `TenancyTenantSuspensionCritical` when `tenancy_tenant_suspension_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `TenancyTenantSuspensionSloBurn` when `tenancy_tenant_suspension_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `tenancy_tenant_suspension_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `tenancy_tenant_suspension_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `tenancy.tenant-suspension.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p dev-cli -- gate validate tenancy-tenant-suspension --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `tenancy_tenant_suspension_error_ratio` rises before `tenancy_tenant_suspension_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `tenancy_tenant_suspension_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `tenancy_tenant_suspension_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=tenancy_tenant_suspension_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=tenancy_tenant_suspension_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=tenancy_tenant_suspension_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p dev-cli -- gate validate tenancy-tenant-suspension --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p tenancy-domain tenant_suspension -- --nocapture`.
16. Check API contract smoke: `curl -s https://tenancy.internal.oyatie.dev/v1/tenancy/tenant-suspension/incident-handoff -H "x-tenant: $TENANT"`.
...
24. Confirm no cross-cell spread: `oya ops cells query --metric tenancy_tenant_suspension_error_ratio --window 30m --threshold 0.02`.
...
2. Does tenancy_tenant_suspension_queue_depth grow while tenancy_tenant_suspension_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/tenancy/runbooks/tenant-suspension.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://tenancy.internal.oyatie.dev/v1/tenancy/tenant-suspension/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: single tenant isolated.
  - Required action: keep `tenancy-tenant-suspension-circuit-breaker` open until `tenancy_tenant_suspension_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "tenant_suspension|TenancyTenantSuspensionCritical|tenancy.tenant_suspension.incident_state" crates microservices/tenancy -g "!microservices/tenancy/runbooks/**"`.
2. Patch domain invariant: `edit tenancy-domain where tenant_suspension state transition is validated`.
...
6. Add regression test: `cargo test -p tenancy-domain tenant_suspension_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate tenancy-tenant-suspension --fixture incident-tenant-suspension.json`.
8. Add SLO assertion: `update microservices/tenancy/slos/* with alert TenancyTenantSuspensionCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/tenancy/dashboards/dr-pairing-state.json with tenancy_tenant_suspension_error_ratio, tenancy_tenant_suspension_lag_seconds, and tenancy_tenant_suspension_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric tenancy_tenant_suspension_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `tenancy-domain`: inspect for tenant_suspension invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `tenancy-kernel`: inspect for tenant_suspension invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `tenancy-api`: inspect for tenant_suspension invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- tenancy_tenant_suspension_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- tenancy_tenant_suspension_lag_seconds < 120 for all production cells.
- tenancy_tenant_suspension_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include tenancy_tenant_suspension_error_ratio, tenancy_tenant_suspension_lag_seconds, tenancy_tenant_suspension_queue_depth, current breaker state, and audit seal status.
