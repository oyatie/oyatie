...
tenancy owns tenant scope, pack activation state, and audience-type boundaries for j99-cross-jurisdiction-multi-pack-conflict-resolution. The slice is a flat per-microservice implementation plan under tenancy/, matching ADR-0131.
...
- Surface evidence: `tenancy/IP-journey-j99-multi-pack-conflict-resolution.md` matched `finops, emission`; anchors `tenancy/manifest.json, crates/oya-tenancy-api/src/lib.rs`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
