...
///
/// WHY THESE RETURN A LONE SIGNATURE WHERE THE COMMIT OBSERVERS BORN BESIDE
/// THEM RETURN THE RECORD WITH IT. A lone signature normally puts the caller in
/// charge of pairing it with a record. It does not here: the payload pins the
/// row it attests BY DIGEST — `committed_binding_digest`,
/// `committed_issuance_revision` and `committed_issuance_digest` — so a caller
/// pairing it with a different row is refused by the verifier rather than
/// believed. One attestation type also serves both
/// [`crate::CommittedServingAuthorityInstallationClaimV1`] and
/// [`crate::CommittedServingAuthorityFreezeClaimV1`], so a claim-typed observer
/// would have to pick the claim shape for the caller.
