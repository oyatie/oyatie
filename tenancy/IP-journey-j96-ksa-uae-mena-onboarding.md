...
tenancy owns tenant scope, pack activation state, and audience-type boundaries for j96-ksa-uae-mena-tenant-onboarding. The slice is a flat per-microservice implementation plan under tenancy/, matching ADR-0131.
...
- Surface evidence: `tenancy/IP-journey-j96-ksa-uae-mena-onboarding.md` matched `emission`; anchors `tenancy/manifest.json, crates/oya-tenancy-api/src/lib.rs`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
