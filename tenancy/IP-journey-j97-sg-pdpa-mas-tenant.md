...
tenancy owns tenant scope, pack activation state, and audience-type boundaries for j97-sg-pdpa-mas-singapore-tenant. The slice is a flat per-microservice implementation plan under microservices/tenancy/, matching ADR-0131.
...
- Surface evidence: `microservices/tenancy/IP-journey-j97-sg-pdpa-mas-tenant.md` matched `emission`; anchors `microservices/tenancy/manifest.json, crates/oya-tenancy-api/src/lib.rs`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
