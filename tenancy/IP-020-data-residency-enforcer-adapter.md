...
> **Delivery note (2026-08-20).** Implemented in tenancy/adapters/data-residency-enforcer as `tenancy-data-residency-enforcer`, collapsed into that ONE crate
...
> was available to this lane. Landed: the fail-closed cross-border decision cascade and the denial audit path. Deferred and named as a gap in the crate's `lib.rs` header:
> Cedar evaluation itself — Cedar remains the policy AUTHORITY and this engine only mirrors it, with nothing automatically binding the two. The crate names in the tables below are this plan's original
...
| `tenancy/policy/data-residency.cedar` | extend if needed | Entity/action names used by adapter. |
...
- Cedar decision tests use real `tenancy/policy/data-residency.cedar`.
- Every denial carries audit evidence and no outbound event/RPC is dispatched.
- `rg "home_jurisdiction|source_pack|target_pack" tenancy/contracts` finds the event contract labels after the implementation.
- `cargo run -p oya-dev-cli -- gate validate tenancy-residency-conformance --microservice tenancy`.

## F. Evidence

- `tenancy/PRD.md` data-residency section states cross-pack replication is forbidden by default.
- `tenancy/policy/data-residency.cedar` is the real policy file.
- `tenancy/contracts/asyncapi/tenant-events.yaml` is the outbound event surface.
- `tenancy/IP-019-dr-pairing-controller.md` depends on this guard for same-jurisdiction DR.
...
- Surface evidence: `tenancy/IP-020-data-residency-enforcer-adapter.md` matched `asyncapi`; contract files `tenancy/contracts/openapi/tenancy.yaml, tenancy/contracts/asyncapi/tenant-events.yaml, tenancy/contracts/proto/tenancy.proto`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
...
- Surface evidence: `tenancy/IP-020-data-residency-enforcer-adapter.md` matched `payment`; anchors `tenancy/runbooks/dr-pair-promotion-drill.md, crates/oya-tenancy-api/src/lib.rs`; type anchor `crates/oya-tenancy-api/src/lib.rs::TenantCreateApiRequest`.
