---
doc_class: Runbook
doc_id: RUNBOOK-COMMS_EMAIL-COLD_MERGE
...
# Runbook: Comms Email Cold Merge

## Operator Contract
- Runbook id: comms-email-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-comms-email-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=comms-email; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n comms-email rollout status deploy/comms-email --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service comms-email --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service comms-email --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service comms-email --cell $CELL --window 30m`.
