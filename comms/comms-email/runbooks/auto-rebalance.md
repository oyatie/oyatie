---
doc_class: Runbook
doc_id: RUNBOOK-COMMS_EMAIL-AUTO_REBALANCE
...
# Runbook: Comms Email Auto Rebalance

## Operator Contract
- Runbook id: comms-email-auto-rebalance.
- Primary service namespace: `comms-email`.
...
- Owning team: oya-substrate-comms.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-comms-email-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=comms-email; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n comms-email rollout status deploy/comms-email --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service comms-email --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service comms-email --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service comms-email --cell $CELL --window 30m`.
