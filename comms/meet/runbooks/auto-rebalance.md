---
doc_class: Runbook
doc_id: RUNBOOK-MEET-AUTO_REBALANCE
microservice: meet
...
# Runbook: Meet Auto Rebalance

## Operator Contract
- Runbook id: meet-auto-rebalance.
- Primary service namespace: `meet`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-meet-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=meet; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n meet rollout status deploy/meet --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service meet --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service meet --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service meet --cell $CELL --window 30m`.
