...
/// `oya_<microservice>_*` strings differently.
#[derive(Clone, Debug, Eq, PartialEq)]
...
mod tests {
    ...
    fn request_telemetry_binding_uses_canonical_metric_names() {
        ...
        assert_eq!(
            ...
            "oya_message_stream_request_total"
        );
        assert_eq!(
            binding.request_success_metric,
            "oya_message_stream_request_success_total"
        );
        assert_eq!(
            binding.responses_total_metric,
            "oya_message_stream_responses_total"
        );
        assert_eq!(
            binding.responses_5xx_metric,
            "oya_message_stream_responses_5xx_total"
        );
        assert_eq!(
            binding.responses_429_metric,
            "oya_message_stream_responses_429_total"
        );
    }
