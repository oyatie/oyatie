# Contact Center Runbook: Emergency Bypass Audit

Service: contact-center  
Surface: local operational primitive suite  
Primary SLO: agent-presence-freshness  
Domain focus: omnichannel, routing, call, transfer

## Trigger
- Alert `contact-center-agent-presence-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_contact_center_agent_presence_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when agent-presence-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `contact-center-local-agent-presence-freshness` and policy file name to the ticket.
