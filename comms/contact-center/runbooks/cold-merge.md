---
doc_class: Runbook
doc_id: RUNBOOK-CONTACT_CENTER-COLD_MERGE
...
# Runbook: Contact Center Cold Merge

## Operator Contract
- Runbook id: contact-center-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contact-center-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contact-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n contact-center rollout status deploy/contact-center --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service contact-center --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service contact-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service contact-center --cell $CELL --window 30m`.
