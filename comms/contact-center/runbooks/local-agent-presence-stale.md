# Contact Center Runbook: Agent Presence Stale

Service: contact-center  
Surface: local operational primitive suite  
Primary SLO: recording-consent-correctness  
Domain focus: omnichannel, routing, call, transfer

## Trigger
- Alert `contact-center-recording-consent-correctness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_contact_center_recording_consent_correctness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when recording-consent-correctness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `contact-center-local-recording-consent-correctness` and policy file name to the ticket.
