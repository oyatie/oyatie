---
doc_class: Runbook
doc_id: RUNBOOK-EMERGENCY-COLD_MERGE
...
# Runbook: Emergency Cold Merge

## Operator Contract
- Runbook id: emergency-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-emergency-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=emergency; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n emergency rollout status deploy/emergency --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service emergency --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service emergency --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service emergency --cell $CELL --window 30m`.
