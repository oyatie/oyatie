---
doc_class: Runbook
doc_id: RUNBOOK-MESSENGER-COLD_MERGE
...
# Runbook: Messenger Cold Merge

## Operator Contract
- Runbook id: messenger-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-messenger-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=messenger; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n messenger rollout status deploy/messenger --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service messenger --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service messenger --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service messenger --cell $CELL --window 30m`.
