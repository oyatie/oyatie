---
doc_class: Runbook
doc_id: RUNBOOK-MESSENGER-HOT_SPLIT
...
# Runbook: Messenger Hot Split

## Operator Contract
- Runbook id: messenger-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-messenger-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=messenger; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n messenger rollout status deploy/messenger --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service messenger --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service messenger --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service messenger --cell $CELL --window 30m`.
