...
companion_docs:
  ...
  - microservices/sheets/README.md
...
`sheets` keeps its existing bounded context and flat `microservices/sheets/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `sheets`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `sheets` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_query` drives CPU `0.18`, RAM `512`, storage `40`, and connection pool variables.
