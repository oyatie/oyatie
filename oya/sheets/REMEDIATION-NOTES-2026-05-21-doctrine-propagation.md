...
- Value: 0.18 vCPU, 512 MiB RAM, 40 GB storage, Valkey/PostgreSQL/outbound connections 3/3/6, scaling_dimension=per_query, cell_placement_class=Tier-3.
- ADR: ADR-0340 capacity declaration and ADR-0248 cell criticality numbering.
- Why: 0.18 vCPU and 512 MiB per tenant reflect formula dependency graphs and 100k/1m-cell recalculation claims.
...
- Why: 1800s RTO and 120s RPO beat the HIPAA floor while allowing recalc snapshots to converge without over-promising zero-loss semantics.
...
- Why: Sheets is a first-party application for tenant workbook data. It performs formula and recalc work owned by Oyatie code, not tenant-customer code execution, so Tier 2 runtime plus Tier-3 application placement satisfies the ADR-0340 co-variance table.
...
- Why: Sheets exposes workbook APIs, event channels, and proto surfaces, so public version pinning is part of tenant contract stability.
...
- Why: Sheets consumes the common product OSS stack; no local stewardship override is needed because registry defaults already bind owners and SLA.
