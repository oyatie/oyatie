---
doc_class: Runbook
doc_id: RUNBOOK-SHEETS-COLD_MERGE
microservice: sheets
...
# Runbook: Sheets Cold Merge

## Operator Contract
- Runbook id: sheets-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-sheets-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=sheets; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n sheets rollout status deploy/sheets --timeout=60s`.
3. Verify alerts: operation `observability.alerts_list(service=sheets, runbook=cold-merge, since=30m)`.
4. Verify primary metric: operation `metrics.query(name=oya_sharding_cold_merge_candidate_count, service=sheets, cell=$CELL, window=30m)`.
5. Verify secondary metric: operation `metrics.query(name=oya_sharding_cold_merge_quiet_hours, service=sheets, cell=$CELL, window=30m)`.
