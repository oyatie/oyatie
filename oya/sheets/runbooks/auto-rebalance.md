---
doc_class: Runbook
doc_id: RUNBOOK-SHEETS-AUTO_REBALANCE
microservice: sheets
...
# Runbook: Sheets Auto Rebalance

## Operator Contract
- Runbook id: sheets-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-sheets-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=sheets; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n sheets rollout status deploy/sheets --timeout=60s`.
3. Verify alerts: operation `observability.alerts_list(service=sheets, runbook=auto-rebalance, since=30m)`.
4. Verify primary metric: operation `metrics.query(name=oya_sharding_auto_rebalance_candidate_count, service=sheets, cell=$CELL, window=30m)`.
5. Verify secondary metric: operation `metrics.query(name=oya_sharding_auto_rebalance_migration_seconds_p99, service=sheets, cell=$CELL, window=30m)`.
