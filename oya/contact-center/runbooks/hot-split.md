---
doc_class: Runbook
doc_id: RUNBOOK-CONTACT_CENTER-HOT_SPLIT
...
# Runbook: Contact Center Hot Split

## Operator Contract
- Runbook id: contact-center-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contact-center-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contact-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n contact-center rollout status deploy/contact-center --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service contact-center --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service contact-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service contact-center --cell $CELL --window 30m`.
