---
doc_class: Runbook
doc_id: RUNBOOK-CONTACT_CENTER-AUTO_REBALANCE
...
# Runbook: Contact Center Auto Rebalance

## Operator Contract
- Runbook id: contact-center-auto-rebalance.
- Primary service namespace: `contact-center`.
...
- Owning team: axis-contact-center + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contact-center-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contact-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n contact-center rollout status deploy/contact-center --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service contact-center --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service contact-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service contact-center --cell $CELL --window 30m`.
