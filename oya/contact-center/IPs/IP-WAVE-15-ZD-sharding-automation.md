...
---
# IP-WAVE-15-ZD-sharding-automation: Contact Center Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/contact-center/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: contact-center must interpret ADR-0348 through its own bounded context: voice-routing.
SCOPE-004: contact-center uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: contact-center uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: contact-center uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: contact-center.
STANCE-002: Owner team: axis-contact-center + council-product.
...
STANCE-005: Bounded context anchor: voice-routing.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_message.
STANCE-008: Compliance pack input: SOC-2, ISO-27001, GDPR, HIPAA-2024, PCI-DSS-L1-v4, KR-PIPA, TCPA.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for contact-center.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for contact-center because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; contact-center must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where contact-center participates.
...
ROLLBACK-002: For contact-center, rollback starts from the audit-chain row that recorded the automation event.
