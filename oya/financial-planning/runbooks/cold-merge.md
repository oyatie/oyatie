---
doc_class: Runbook
doc_id: RUNBOOK-FINANCIAL_PLANNING-COLD_MERGE
...
# Runbook: Financial Planning Cold Merge

## Operator Contract
- Runbook id: financial-planning-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-financial-planning-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=financial-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n financial-planning rollout status deploy/financial-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service financial-planning --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service financial-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service financial-planning --cell $CELL --window 30m`.
