...
Primary SLO: budget-lock-success  
Domain focus: budget, forecast, variance, close

## Trigger
- Alert `financial-planning-budget-lock-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_financial_planning_budget_lock_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when budget-lock-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `financial-planning-local-budget-lock-success` and policy file name to the ticket.
