# Financial Planning Runbook: Forecast Recalc Stall

Service: financial-planning  
Surface: local operational primitive suite  
Primary SLO: forecast-recalc-latency  
Domain focus: budget, forecast, variance, close

## Trigger
- Alert `financial-planning-forecast-recalc-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_financial_planning_forecast_recalc_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when forecast-recalc-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `financial-planning-local-forecast-recalc-latency` and policy file name to the ticket.
