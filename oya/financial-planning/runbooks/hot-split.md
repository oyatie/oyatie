---
doc_class: Runbook
doc_id: RUNBOOK-FINANCIAL_PLANNING-HOT_SPLIT
...
# Runbook: Financial Planning Hot Split

## Operator Contract
- Runbook id: financial-planning-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-financial-planning-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=financial-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n financial-planning rollout status deploy/financial-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service financial-planning --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service financial-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service financial-planning --cell $CELL --window 30m`.
