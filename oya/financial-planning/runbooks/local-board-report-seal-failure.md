...
Primary SLO: board-report-seal-completeness  
Domain focus: budget, forecast, variance, close

## Trigger
- Alert `financial-planning-board-report-seal-completeness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_financial_planning_board_report_seal_completeness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when board-report-seal-completeness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `financial-planning-local-board-report-seal-completeness` and policy file name to the ticket.
