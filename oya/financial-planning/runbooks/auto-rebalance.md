---
doc_class: Runbook
doc_id: RUNBOOK-FINANCIAL_PLANNING-AUTO_REBALANCE
...
# Runbook: Financial Planning Auto Rebalance

## Operator Contract
- Runbook id: financial-planning-auto-rebalance.
- Primary service namespace: `financial-planning`.
...
- Owning team: axis-financial-planning + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-financial-planning-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=financial-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n financial-planning rollout status deploy/financial-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service financial-planning --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service financial-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service financial-planning --cell $CELL --window 30m`.
