...
bounded_context: presentation-authoring
...
---
# IP-WAVE-15-ZD-sharding-automation: Slides Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/slides/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: slides must interpret ADR-0348 through its own bounded context: presentation-authoring.
SCOPE-004: slides uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: slides uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: slides uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: slides.
STANCE-002: Owner team: axis-slides.
...
STANCE-005: Bounded context anchor: presentation-authoring.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_user.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, iso27001, SOC 2 Type 2, gdpr, hipaa, lgpd, PCI-DSS, fedramp, soc2, cn-pipl-2021, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for slides.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for slides because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; slides must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where slides participates.
...
ROLLBACK-002: For slides, rollback starts from the audit-chain row that recorded the automation event.
