---
doc_class: Runbook
doc_id: RUNBOOK-SLIDES-HOT_SPLIT
microservice: slides
...
# Runbook: Slides Hot Split

## Operator Contract
- Runbook id: slides-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-slides-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=slides; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n slides rollout status deploy/slides --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service slides --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service slides --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service slides --cell $CELL --window 30m`.
