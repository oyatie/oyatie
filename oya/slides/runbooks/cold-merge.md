---
doc_class: Runbook
doc_id: RUNBOOK-SLIDES-COLD_MERGE
microservice: slides
...
# Runbook: Slides Cold Merge

## Operator Contract
- Runbook id: slides-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-slides-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=slides; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n slides rollout status deploy/slides --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service slides --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service slides --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service slides --cell $CELL --window 30m`.
