---
doc_class: Runbook
doc_id: RUNBOOK-SLIDES-AUTO_REBALANCE
microservice: slides
...
# Runbook: Slides Auto Rebalance

## Operator Contract
- Runbook id: slides-auto-rebalance.
- Primary service namespace: `slides`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-slides-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=slides; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n slides rollout status deploy/slides --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service slides --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service slides --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service slides --cell $CELL --window 30m`.
