---
doc_class: Runbook
doc_id: RUNBOOK-INCIDENT_MANAGEMENT-AUTO_REBALANCE
...
# Runbook: Incident Management Auto Rebalance

## Operator Contract
- Runbook id: incident-management-auto-rebalance.
- Primary service namespace: `incident-management`.
...
- Owning team: axis-incident-management + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-incident-management-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=incident-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n incident-management rollout status deploy/incident-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service incident-management --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service incident-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service incident-management --cell $CELL --window 30m`.
