---
doc_class: Runbook
doc_id: RUNBOOK-INCIDENT_MANAGEMENT-HOT_SPLIT
...
# Runbook: Incident Management Hot Split

## Operator Contract
- Runbook id: incident-management-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-incident-management-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=incident-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n incident-management rollout status deploy/incident-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service incident-management --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service incident-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service incident-management --cell $CELL --window 30m`.
