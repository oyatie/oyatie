# Incident Management Runbook: Mobile Push Provider Failover

Service: incident-management  
Surface: local operational primitive suite  
Primary SLO: war-room-creation-latency  
Domain focus: war-room, escalation, page, acknowledge

## Trigger
- Alert `incident-management-war-room-creation-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_incident_management_war_room_creation_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when war-room-creation-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `incident-management-local-war-room-creation-latency` and policy file name to the ticket.
