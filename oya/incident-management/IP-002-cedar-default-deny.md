# IP-002 Incident Management Cedar default deny

Service: incident-management
ChangeSet scope: microservices/incident-management/IP-002-cedar-default-deny.md
...
## B. Approach
The technical mechanism is a default-deny Cedar gate for every incident command action. It is implemented through the existing flat `incident-management` ownership boundary, not a vendor-named suite or a generic platform helper.
Use `policy/sre-incident-command-authorization.cedar` as the first guard surface and confirm every mutation carries `tenant_id`, `principal_id`, `audience_type=ONCALL_RESPONDER`, `home_cell`, `jurisdiction_code`, `data_class`, `traceparent`, `idempotency_key`, and `audit_event_class`.
