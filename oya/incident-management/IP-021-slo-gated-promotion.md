# IP-021 Incident Management SLO-gated promotion

Service: incident-management
ChangeSet scope: microservices/incident-management/IP-021-slo-gated-promotion.md
...
## A. Problem
This IP closes the SLO-gated promotion gap for `incident-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `slo-gated-promotion` works for paging, escalation, incident rooms, stakeholder updates, and postmortems are time-critical and must keep evidence when systems are degraded.
...
The success condition is an implementation plan a cold engineer can trace from this IP to concrete files such as `microservices/incident-management/slos/*.openslo.yaml` without inventing a hidden service boundary.
...
## C. Deliverables
- D01: `microservices/incident-management/PRD.md` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D02: `microservices/incident-management/ARCHITECTURE.md` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D03: `microservices/incident-management/manifest.json` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D04: `microservices/incident-management/competitor-parity-matrix.md` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D05: `microservices/incident-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D06: `microservices/incident-management/slos/*.openslo.yaml` — create or reconcile for `slo-gated-promotion` evidence and contract traceability.
- D07: `microservices/incident-management/contracts/openapi-v1.yaml` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D08: `microservices/incident-management/contracts/asyncapi-v1.yaml` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D09: `microservices/incident-management/contracts/incident-management-v1.proto` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D10: `microservices/incident-management/src/domain/mod.rs` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D11: `microservices/incident-management/src/usecase/mod.rs` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D12: `microservices/incident-management/src/adapter/http.rs` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D13: `microservices/incident-management/src/adapter/asyncapi.rs` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D14: `microservices/incident-management/policies/local-page-dispatch-guard.cedar` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D15: `microservices/incident-management/policies/local-escalation-policy-control.cedar` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D16: `microservices/incident-management/policies/local-war-room-open-approval.cedar` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D17: `microservices/incident-management/policy/sre-incident-command-authorization.cedar` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
- D18: `microservices/incident-management/capabilities/page-dispatch.yaml` — modify or bind for `slo-gated-promotion` evidence and contract traceability.
...
5. Bind adapter behavior through `promotion gate` and `microservices/incident-management/src/adapter/mod.rs`, using typed errors from `microservices/incident-management/src/error.rs` instead of stringly provider failures.
...
- `microservices/incident-management/slos/*.openslo.yaml`
...
|---|---|---|
| PagerDuty | Provides a native `SLO-gated promotion` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `slo-gated-promotion` to tenant scope, Cedar deny evidence, audit-chain records, and flat `incident-management` ownership. |
...
- Do not move `slo-gated-promotion` into a sibling service such as `observability` or `messenger` unless a later ADR changes ownership.
