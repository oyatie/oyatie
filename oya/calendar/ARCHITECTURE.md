...
## §principals
This anchor is closed for `calendar` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.calendar.runtime` owns normal `T0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.calendar.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/calendar/backfill-replay.md` when present.
- Platform principal `oyatie.calendar.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.calendar.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.calendar.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `T0-suggest` is evaluated as `<tenant>.calendar.t0-suggest` before any `calendar` state mutation.
...
## §cedar-gates
This anchor is closed for `calendar` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/calendar/policy/auditor-scope.cedar`, `microservices/calendar/policy/ci-scope.cedar`, `microservices/calendar/policy/data-residency.md`, `microservices/calendar/policy/event-isolation.md`, `microservices/calendar/policy/public-read.cedar`, `microservices/calendar/policy/tenant-scope.cedar`.
- Action namespace uses `calendar::<bounded_context>::<verb>`; the first protected action is `calendar::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/calendar/policy/auditor-scope.cedar`, `microservices/calendar/policy/ci-scope.cedar`, `microservices/calendar/policy/data-residency.md`, `microservices/calendar/policy/event-isolation.md`, `microservices/calendar/policy/public-read.cedar`, `microservices/calendar/policy/tenant-scope.cedar`.
- Example: `calendar::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `calendar` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `calendar.availability_resolver` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `calendar.event_store` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `calendar.ics_import_export` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `calendar.invitation_flow` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `calendar.recurrence_engine` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
This anchor is closed for `calendar` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `calendar` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `calendar` only through contracts `microservices/calendar/contracts/asyncapi/calendar-events.yaml`, `microservices/calendar/contracts/openapi/calendar.yaml`, `microservices/calendar/contracts/proto/calendar.proto`.
- If product: `calendar` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `availability-resolver`, `event-store`, `ics-import-export`, `invitation-flow`, `recurrence-engine`, `room-booking`.
...
## §policy-evaluation
This anchor is closed for `calendar` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/calendar/policy/auditor-scope.cedar`, `microservices/calendar/policy/ci-scope.cedar`, `microservices/calendar/policy/data-residency.md`, `microservices/calendar/policy/event-isolation.md`, `microservices/calendar/policy/public-read.cedar`, `microservices/calendar/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `calendar::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `calendar.availability_resolver`.
...
## §cell-eligibility
This anchor is closed for `calendar` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `calendar` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `calendar` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `calendar.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `T0-suggest` can request classification or summarization with `audience_tag=calendar.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `calendar` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `calendar` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `availability-resolver` is read/written as an ontology node or edge projection when `calendar` needs cross-surface discovery.
- Bounded-context entity `event-store` is read/written as an ontology node or edge projection when `calendar` needs cross-surface discovery.
- Bounded-context entity `ics-import-export` is read/written as an ontology node or edge projection when `calendar` needs cross-surface discovery.
- Bounded-context entity `invitation-flow` is read/written as an ontology node or edge projection when `calendar` needs cross-surface discovery.
...
## §transport
This anchor is closed for `calendar` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/calendar/contracts/asyncapi/calendar-events.yaml`, `microservices/calendar/contracts/openapi/calendar.yaml`, `microservices/calendar/contracts/proto/calendar.proto`.
...
- IaC transport evidence: `microservices/calendar/iac/helm/Chart.yaml`, `microservices/calendar/iac/helm/templates/cronjob.yaml`, `microservices/calendar/iac/helm/templates/deployment.yaml`, `microservices/calendar/iac/helm/templates/hpa.yaml`, `microservices/calendar/iac/helm/templates/networkpolicy.yaml`, `microservices/calendar/iac/helm/templates/pdb.yaml`; +6 more.
- Example: `calendar` `T0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `calendar` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `availability-resolver`, `event-store`, `ics-import-export`, `invitation-flow`, `recurrence-engine`, `room-booking` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/calendar/iac/helm/Chart.yaml`, `microservices/calendar/iac/helm/templates/cronjob.yaml`, `microservices/calendar/iac/helm/templates/deployment.yaml`, `microservices/calendar/iac/helm/templates/hpa.yaml`, `microservices/calendar/iac/helm/templates/networkpolicy.yaml`, `microservices/calendar/iac/helm/templates/pdb.yaml`; +6 more.
- Catalog crates/components: `microservices/calendar/catalog/oya-calendar-availability-resolver-adapter-valkey.yaml`, `microservices/calendar/catalog/oya-calendar-availability-resolver-kernel.yaml`, `microservices/calendar/catalog/oya-calendar-event-store-adapter-postgres.yaml`, `microservices/calendar/catalog/oya-calendar-event-store-app.yaml`, `microservices/calendar/catalog/oya-calendar-event-store-domain.yaml`, `microservices/calendar/catalog/oya-calendar-event-store-kernel.yaml`; +18 more.
- Example: `T0-suggest` API runs as a Kata-isolated container; scheduled/background work for `calendar` uses a separate worker principal.
...
## §observability
This anchor is closed for `calendar` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/calendar/slos/agenda-render-latency.openslo.yaml`, `microservices/calendar/slos/caldav-availability.openslo.yaml`, `microservices/calendar/slos/freebusy-query-latency.openslo.yaml`, `microservices/calendar/slos/ics-import-throughput.openslo.yaml`, `microservices/calendar/slos/notification-delivery-freshness.openslo.yaml`, `microservices/calendar/slos/room-conflict-detection-correctness.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.calendar.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `calendar_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `calendar_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/calendar/slos/agenda-render-latency.openslo.yaml`.
...
- Example trace: `calendar.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `calendar` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `calendar` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/calendar/policy/auditor-scope.cedar`, `microservices/calendar/policy/ci-scope.cedar`, `microservices/calendar/policy/data-residency.md`, `microservices/calendar/policy/event-isolation.md`, `microservices/calendar/policy/public-read.cedar`, `microservices/calendar/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `calendar` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/calendar`.
...
This anchor is closed for `calendar` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `calendar` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `calendar` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `calendar` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `calendar` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `calendar` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `calendar` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/calendar/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/calendar/runbooks/availability-cache-rebuild.md`, `microservices/calendar/runbooks/caldav-sync-loop.md`, `microservices/calendar/runbooks/calendar-bridge-mail-loop-detection.md`, `microservices/calendar/runbooks/calendar-restore.md`, `microservices/calendar/runbooks/ics-import-failure.md`, `microservices/calendar/runbooks/recurrence-storm.md`; +6 more.
