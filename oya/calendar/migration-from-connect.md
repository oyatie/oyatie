...
# Migration: `oya-calendar-*` → `oya-calendar-*`
...
| Reason | ADR-0132 no-grouping forward-policy + ADR-0139 per-µservice SLO authority + ADR-0131 per-µservice flat layout + the 6-BC calendar surface (events / recurrence / availability / room-booking / invitation-flow / ics-import-export) is only addressable at µservice granularity, not at Connect-platform granularity |
...
`oya-calendar-*` crate has a 1:1 replacement under the new prefix.

### Crate import-path map

| Legacy `oya-calendar-*` path | New `oya-calendar-*` path |
|---|---|
| `oya-calendar-domain` | (split per BC; see note below) |
| `oya-calendar-event-kernel` | `oya-calendar-event-store-kernel` |
| `oya-calendar-event-domain` | `oya-calendar-event-store-domain` |
| `oya-calendar-event-usecase` | `oya-calendar-event-store-usecase` |
| `oya-calendar-event-api` | `oya-calendar-event-store-api` |
| `oya-calendar-event-adapter` | `oya-calendar-event-store-adapter` |
| `oya-calendar-event-adapter-postgres` | `oya-calendar-event-store-adapter-postgres` |
| `oya-calendar-event-rest` | `oya-calendar-event-store-rest` |
| `oya-calendar-event-worker` | `oya-calendar-event-store-worker` |
| `oya-calendar-event-sdk` | `oya-calendar-event-store-sdk` |
| `oya-calendar-event-app` | `oya-calendar-event-store-app` |
| `oya-calendar-recurrence-kernel` | `oya-calendar-recurrence-engine-kernel` |
| `oya-calendar-recurrence-domain` | `oya-calendar-recurrence-engine-domain` |
| `oya-calendar-recurrence-usecase` | `oya-calendar-recurrence-engine-usecase` |
| `oya-calendar-recurrence-api` | `oya-calendar-recurrence-engine-api` |
| `oya-calendar-recurrence-adapter` | `oya-calendar-recurrence-engine-adapter` |
| `oya-calendar-recurrence-app` | `oya-calendar-recurrence-engine-app` |
| `oya-calendar-availability-kernel` | `oya-calendar-availability-resolver-kernel` |
| `oya-calendar-availability-domain` | `oya-calendar-availability-resolver-domain` |
| `oya-calendar-availability-usecase` | `oya-calendar-availability-resolver-usecase` |
| `oya-calendar-availability-api` | `oya-calendar-availability-resolver-api` |
| `oya-calendar-availability-adapter` | `oya-calendar-availability-resolver-adapter` |
| `oya-calendar-availability-adapter-valkey` | `oya-calendar-availability-resolver-adapter-valkey` |
| `oya-calendar-availability-rest` | `oya-calendar-availability-resolver-rest` |
| `oya-calendar-availability-worker` | `oya-calendar-availability-resolver-worker` |
| `oya-calendar-availability-app` | `oya-calendar-availability-resolver-app` |
| `oya-calendar-room-kernel` | `oya-calendar-room-booking-kernel` |
| `oya-calendar-room-domain` | `oya-calendar-room-booking-domain` |
| `oya-calendar-room-usecase` | `oya-calendar-room-booking-usecase` |
| `oya-calendar-room-api` | `oya-calendar-room-booking-api` |
| `oya-calendar-room-adapter` | `oya-calendar-room-booking-adapter` |
| `oya-calendar-room-rest` | `oya-calendar-room-booking-rest` |
| `oya-calendar-room-app` | `oya-calendar-room-booking-app` |
| `oya-calendar-invitation-kernel` | `oya-calendar-invitation-flow-kernel` |
| `oya-calendar-invitation-domain` | `oya-calendar-invitation-flow-domain` |
| `oya-calendar-invitation-usecase` | `oya-calendar-invitation-flow-usecase` |
| `oya-calendar-invitation-api` | `oya-calendar-invitation-flow-api` |
| `oya-calendar-invitation-adapter` | `oya-calendar-invitation-flow-adapter` |
| `oya-calendar-invitation-worker` | `oya-calendar-invitation-flow-worker` |
| `oya-calendar-invitation-app` | `oya-calendar-invitation-flow-app` |
| `oya-calendar-ics-kernel` | `oya-calendar-ics-import-export-kernel` |
| `oya-calendar-ics-domain` | `oya-calendar-ics-import-export-domain` |
| `oya-calendar-ics-usecase` | `oya-calendar-ics-import-export-usecase` |
| `oya-calendar-ics-api` | `oya-calendar-ics-import-export-api` |
| `oya-calendar-ics-adapter` | `oya-calendar-ics-import-export-adapter` |
| `oya-calendar-ics-adapter-icalendar` | `oya-calendar-ics-import-export-adapter-icalendar` |
| `oya-calendar-ics-adapter-caldav` | `oya-calendar-ics-import-export-adapter-caldav-radicale` (per ADR-CAL-0001 — backend-qualified) |
| `oya-calendar-ics-rest` | `oya-calendar-ics-import-export-rest` |
| `oya-calendar-ics-app` | `oya-calendar-ics-import-export-app` |

> **`oya-calendar-domain` split.** The legacy bundled crate
...
> `oya-calendar-domain` must each pick the specific replacement
...
`oya-calendar-*`. They are therefore not part of the migration
...
- **`oya-calendar-tzdb-refresh-worker`** — automated IANA tzdb
  refresh per ADR-CAL-0004; the legacy calendar surface had
  no tzdb refresh worker.
- **Cross-tenant availability with policy-bounded disclosure** — PRD
  FR-10 differentiator; legacy `oya-calendar-*` had no
...
[dependencies]
oya-calendar-event-kernel = { workspace = true }
oya-calendar-event-usecase = { workspace = true }
oya-calendar-recurrence-kernel = { workspace = true }
oya-calendar-invitation-kernel = { workspace = true }
...
5. **ADR-CAL-0001 → ADR-CAL-0004** — calendar-specific decisions
   ...
   granularity, not at the platform level.

## Migration Guide (step-by-step)

For each consumer crate that imports `oya-calendar-*`:
...
# manual disambiguation needed for the `oya-calendar-domain`
# split case):
...
cargo nextest run --features calendar-strangler-canary
```
...
cargo tree -e normal -p your-crate | grep oya-calendar   # expect empty
...
| K8s namespace | `connector` | `calendar` |
...
| 2. Adapter soak | `oya-calendar-migration-adapter` shims legacy symbols → new impl | pending | All consumers compile against adapter; 3-month soak elapses |
...
- [ ] **Replacement is production-proven and covers all critical use cases.**
  ```bash
  buck2 build //:quality-lane-registry-authority-check # lane=hyperscaler-maturity-claims --microservice calendar
...
- [ ] **Old code, tests, documentation, configuration removed** (per Phase 5):
  ```bash
  find crates -maxdepth 1 -type d -name "oya-calendar-*" | wc -l   # expect 0
