---
doc_class: Runbook
doc_id: RUNBOOK-EMR-AUTO_REBALANCE
microservice: emr
...
# Runbook: Emr Auto Rebalance

## Operator Contract
- Runbook id: emr-auto-rebalance.
- Primary service namespace: `emr`.
...
- Owning team: axis-emr + council-clinical + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-emr-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=emr; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n emr rollout status deploy/emr --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service emr --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service emr --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service emr --cell $CELL --window 30m`.
