...
- Values: 0.1 vCPU, 256 MiB RAM, 5 GB storage per active tenant; connections valkey=3, postgres=3, outbound_http=3; scaling_dimension=per_user; cell_placement_class=Tier-3.
- ADR: ADR-0340 capacity declaration plus ADR-0248 cellular class.
- Rejected: template-stamped values copied from another service; notes keeps Tier-2 because its E2E design is client-derived; the committed cost is object/WAL recovery and sync cache recovery, not Kata substrate placement.
...
- Values: pod_runtime_tier=2; evidence=microservices/notes/PRD.md, microservices/notes/IP-011-collab-edit-loro.md, microservices/notes/IP-014-e2e-key-management.md, microservices/notes/runbooks/e2e-key-rotation-and-recovery.md.
