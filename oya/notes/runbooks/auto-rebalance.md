---
doc_class: Runbook
doc_id: RUNBOOK-NOTES-AUTO_REBALANCE
microservice: notes
...
# Runbook: Notes Auto Rebalance

## Operator Contract
- Runbook id: notes-auto-rebalance.
- Primary service namespace: `notes`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-notes-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=notes; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n notes rollout status deploy/notes --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service notes --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service notes --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service notes --cell $CELL --window 30m`.
