---
doc_class: Runbook
doc_id: RUNBOOK-NOTES-HOT_SPLIT
microservice: notes
...
# Runbook: Notes Hot Split

## Operator Contract
- Runbook id: notes-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-notes-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=notes; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n notes rollout status deploy/notes --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service notes --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service notes --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service notes --cell $CELL --window 30m`.
