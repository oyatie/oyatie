---
doc_class: Runbook
doc_id: RUNBOOK-NOTES-COLD_MERGE
microservice: notes
...
# Runbook: Notes Cold Merge

## Operator Contract
- Runbook id: notes-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-notes-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=notes; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n notes rollout status deploy/notes --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service notes --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service notes --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service notes --cell $CELL --window 30m`.
