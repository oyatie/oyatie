---
doc_class: Runbook
doc_id: RUNBOOK-AUDIT_CHAIN-COLD_MERGE
...
# Runbook: Audit Chain Cold Merge

## Operator Contract
- Runbook id: audit-chain-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-audit-chain-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=audit-chain; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n audit-chain rollout status deploy/audit-chain --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service audit-chain --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service audit-chain --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service audit-chain --cell $CELL --window 30m`.
