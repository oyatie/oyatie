...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=audit-chain-signature-verification-failure --production-snapshot` exits non-zero against the latest production evidence bundle.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=audit-chain-signature-verification-failure --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=audit-chain-signature-verification-failure --fixture incident-signature-verification-failure.json`.
