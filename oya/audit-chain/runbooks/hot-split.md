---
doc_class: Runbook
doc_id: RUNBOOK-AUDIT_CHAIN-HOT_SPLIT
...
# Runbook: Audit Chain Hot Split

## Operator Contract
- Runbook id: audit-chain-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-audit-chain-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=audit-chain; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n audit-chain rollout status deploy/audit-chain --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service audit-chain --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service audit-chain --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service audit-chain --cell $CELL --window 30m`.
