---
doc_class: Runbook
doc_id: RUNBOOK-RECORDINGS-AUTO_REBALANCE
...
# Runbook: Recordings Auto Rebalance

## Operator Contract
- Runbook id: recordings-auto-rebalance.
- Primary service namespace: `recordings`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-recordings-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=recordings; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n recordings rollout status deploy/recordings --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service recordings --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service recordings --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service recordings --cell $CELL --window 30m`.
