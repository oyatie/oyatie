---
doc_class: Runbook
doc_id: RUNBOOK-SOCIAL-COLD_MERGE
microservice: social
...
# Runbook: Social Cold Merge

## Operator Contract
- Runbook id: social-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-social-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=social; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n social rollout status deploy/social --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service social --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service social --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service social --cell $CELL --window 30m`.
