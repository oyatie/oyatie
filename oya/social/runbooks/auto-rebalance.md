---
doc_class: Runbook
doc_id: RUNBOOK-SOCIAL-AUTO_REBALANCE
microservice: social
...
# Runbook: Social Auto Rebalance

## Operator Contract
- Runbook id: social-auto-rebalance.
- Primary service namespace: `social`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-social-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=social; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n social rollout status deploy/social --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service social --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service social --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service social --cell $CELL --window 30m`.
