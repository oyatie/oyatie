---
doc_class: Runbook
doc_id: RUNBOOK-SOCIAL-HOT_SPLIT
microservice: social
...
# Runbook: Social Hot Split

## Operator Contract
- Runbook id: social-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-social-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=social; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n social rollout status deploy/social --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service social --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service social --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service social --cell $CELL --window 30m`.
