...
ip_id: IP-journey-j82-finance-risk-console
...
# IP-journey-j82-finance-risk-console - finops-portal

## Goal

Implement the `finance-risk-console` slice for `finops-portal` so j82 can satisfy `KR-FSS` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/finops-portal/contracts/openapi/j82-finance-risk-console-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/finops-portal/contracts/asyncapi/j82-finance-risk-console-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/finops-portal/contracts/proto/j82-finance-risk-console-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/finops-portal/policy/j82-finance-risk-console.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/finops-portal/runbooks/j82-finance-risk-console-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/finops-portal/tests/j82_finance_risk_console_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J82FinopsPortalFinanceRiskConsoleStarted`.
- `J82FinopsPortalFinanceRiskConsolePermitted`.
- `J82FinopsPortalFinanceRiskConsoleDenied`.
- `J82FinopsPortalFinanceRiskConsoleCommitted`.
- `J82FinopsPortalFinanceRiskConsoleRolledBack`.

## Implementation rows

### IP row 001 - finops-portal finance-risk-console
Scope: implement one finops-portal behavior for j82, not a cross-service refactor.
