...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/finops-portal/manifest.json
  - microservices/finops-portal/PRD.md
  - microservices/finops-portal/README.md
  - microservices/finops-portal/ARCHITECTURE.md
---

# Architecture: Finops Portal

## Architecture Boundary

`finops-portal` keeps its existing bounded context and flat `microservices/finops-portal/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `finops-portal`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `finops-portal` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/tenant-namespace@v1[both], aws-guest/postgres-wal-g@v1[both], oyatie-as-cloud-provider/iceberg-snapshot@v1[both], oyatie-as-cloud-provider/clickhouse-iceberg-layered@v1[both], on-prem/openbao-policy@v1[both].
...
- Capacity: `per_query` drives CPU `0.12`, RAM `256`, storage `6`, and connection pool variables.
