---
doc_class: Runbook
doc_id: RUNBOOK-FINOPS_PORTAL-COLD_MERGE
...
# Runbook: Finops Portal Cold Merge

## Operator Contract
- Runbook id: finops-portal-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-finops-portal-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=finops-portal; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n finops-portal rollout status deploy/finops-portal --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service finops-portal --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service finops-portal --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service finops-portal --cell $CELL --window 30m`.
