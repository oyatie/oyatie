...
This anchor is closed for `finops-portal` against ADR-0250 §D-1: certification-ready evidence roster and audit scope.

### Service-specific answer
- Certification scope for `finops-portal` covers packs `soc2-type2`, `iso27001-2022`, `gdpr`, `kr-pipa`.
...
- Primary evidence files: `microservices/finops-portal/slos/anomaly-explanation-latency.openslo.yaml`, `microservices/finops-portal/slos/cost-allocation-policy-change-latency.openslo.yaml`, `microservices/finops-portal/slos/credit-application-correctness.openslo.yaml`, `microservices/finops-portal/slos/drilldown-query-latency-p99.openslo.yaml`, `microservices/finops-portal/slos/focus-export-availability.openslo.yaml`, `microservices/finops-portal/slos/quarterly-regulator-evidence-emit-correctness.openslo.yaml`; +17 more.
- Example: `tenant-billing-presentation` readiness requires a signed audit event, an OpenAPI/AsyncAPI schema, an SLO target, and a pack-specific retention statement before launch.
...
- Service: `finops-portal`; owner `ops-finops`; tier `product`; audience `B2C_CONSUMER + B2B_TENANT`.
...
## §pack-overlay-roster
This anchor is closed for `finops-portal` against ADR-0251 §D-2: pack activation, overlays and per-pack Cedar deltas.

### Service-specific answer
- Active/expected pack roster: `soc2-type2`, `iso27001-2022`, `gdpr`, `kr-pipa`.
- Pack overlays modify Cedar fragments `microservices/finops-portal/policy/cedar/abuse-defence.cedar`, `microservices/finops-portal/policy/cedar/action-authorization.cedar`, `microservices/finops-portal/policy/cedar/auditor-scope.cedar`, `microservices/finops-portal/policy/cedar/ci-scope.cedar`, `microservices/finops-portal/policy/cedar/customer-success-credit-application.cedar`, `microservices/finops-portal/policy/cedar/data-residency.cedar`; +3 more without changing domain code.
...
- Example: `tenant-billing-presentation` under KR pack uses KR cell routing, KR breach-notification timers, and pack-local audit retention.
...
This anchor is closed for `finops-portal` against ADR-0284 §D-1: platform_owner indirection and hard-coded brand-string audit.

### Service-specific answer
- Runtime platform-owner string is configured as `platform_owner.display_name`; `finops-portal` does not hard-code user-visible owner names in API or UI output.
...
- Surfaces audited for display strings: `microservices/finops-portal/contracts/cost-allocation-policy-internal.proto`, `microservices/finops-portal/contracts/focus-export-internal.asyncapi.yaml`, `microservices/finops-portal/contracts/tenant-invoice-public.openapi.yaml`, `microservices/finops-portal/runbooks/budget-alert-runaway-firings.md`, `microservices/finops-portal/runbooks/cost-allocation-policy-rollback.md`, `microservices/finops-portal/runbooks/credit-application-reconciliation.md`; +11 more.
...
- Example: `tenant-billing-presentation` error text says `platform owner` or config-resolved name, while audit event principal remains `oyatie.finops-portal.runtime`.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.6.A: detection-event categories and routing topology.

### Service-specific answer
- `finops-portal` emits detection signals through ADR-0263 audit pipeline, not an ungoverned side channel.
- Detection families applicable here: policy violation, insider risk, account-takeover, content/transaction abuse where `tenant-billing-presentation` touches those data classes.
- Signal sources: `microservices/finops-portal/policy/cedar/abuse-defence.cedar`, `microservices/finops-portal/policy/cedar/action-authorization.cedar`, `microservices/finops-portal/policy/cedar/auditor-scope.cedar`, `microservices/finops-portal/policy/cedar/ci-scope.cedar`, `microservices/finops-portal/policy/cedar/customer-success-credit-application.cedar`, `microservices/finops-portal/policy/cedar/data-residency.cedar`; +18 more.
- Example event class: `oya.finops.portal.tenant.billing.presentation.risk_signal_emitted` with risk score, reason code, and tenant-safe dimensions.
...
This anchor is closed for `finops-portal` against ADR-0310 §D-1: detection-to-investigation evidence handoff and case binding.

### Service-specific answer
- Investigation handoff starts from a signed detection event emitted by `finops-portal` and ends in a case record with immutable evidence pointers.
- Cedar permit `oyatie.finops-portal.investigation.open` gates who may create, read, export, or close a case.
- Evidence pack includes audit event id, policy decision hash, affected `tenant-billing-presentation` resource ids, tenant id, data classes, and SLO/degraded-mode context.
...
- Example: a suspicious `tenant-billing-presentation` mutation opens a case only after risk threshold and Cedar permit both pass; low-confidence signals remain queued for aggregation.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 8: privileged access, break-glass and UEBA controls.

### Service-specific answer
- Operators of `finops-portal` have no standing unredacted tenant-data access; JIT elevation uses identity step-up and Cedar approval.
- Break-glass access requires reason, scope, expiry, reviewer where required, and post-hoc audit review.
- Sensitive surfaces: `microservices/finops-portal/contracts/cost-allocation-policy-internal.proto`, `microservices/finops-portal/contracts/focus-export-internal.asyncapi.yaml`, `microservices/finops-portal/contracts/tenant-invoice-public.openapi.yaml`, `finops_portal.tenant_billing_presentation`, `finops_portal.cost_allocation_policy`, `finops_portal.anomaly_explanation`; +2 more.
...
- Example: reading `finops_portal.tenant_billing_presentation` outside declared incident purpose creates a high-risk insider signal and routes to investigation.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 9: threat feed sources, freshness and degraded-mode policy.

### Service-specific answer
- `finops-portal` consumes central threat intelligence for IP/domain reputation, credential stuffing, bot fingerprints, sanctions/abuse lists where applicable, and malicious package indicators.
...
- Enforcement points: `microservices/finops-portal/policy/cedar/abuse-defence.cedar`, `microservices/finops-portal/policy/cedar/action-authorization.cedar`, `microservices/finops-portal/policy/cedar/auditor-scope.cedar`, `microservices/finops-portal/policy/cedar/ci-scope.cedar`, `microservices/finops-portal/policy/cedar/customer-success-credit-application.cedar`, `microservices/finops-portal/policy/cedar/data-residency.cedar`; +15 more.
- Example: `tenant-billing-presentation` with malicious IP reputation receives stricter quota/challenge; high-risk legal/financial flows can halt pending investigation.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 16: signing, encryption, ECH and PQC key rotation cadence.

### Service-specific answer
- Signing keys for `oya.finops-portal` audit events rotate at ≤90 days or immediately on suspected compromise.
...
- Secret paths use `${openbao:secret/<tenant_id>/finops-portal/<key-class>}` and never embed raw tenant ids in metrics labels.
- Runbook evidence: `microservices/finops-portal/runbooks/budget-alert-runaway-firings.md`, `microservices/finops-portal/runbooks/cost-allocation-policy-rollback.md`, `microservices/finops-portal/runbooks/credit-application-reconciliation.md`, `microservices/finops-portal/runbooks/focus-export-failure.md`, `microservices/finops-portal/runbooks/quarterly-regulator-emit-miss.md`, `microservices/finops-portal/runbooks/tenant-budget-exhausted.md`; +2 more.
- Example: `tenant-billing-presentation` credential rotation drains in-flight requests with old key id, validates new key id, then retires old leases after audit-chain seal.
...
## §crypto-agility-plan
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 20: algorithm roster, deprecation triggers and migration windows.

### Service-specific answer
- `finops-portal` uses algorithm policy from sidecar/config; domain code never hard-codes cipher or signature choices.
...
- Affected surfaces: `microservices/finops-portal/contracts/cost-allocation-policy-internal.proto`, `microservices/finops-portal/contracts/focus-export-internal.asyncapi.yaml`, `microservices/finops-portal/contracts/tenant-invoice-public.openapi.yaml`, `microservices/finops-portal/iac/ech-config.yaml`, `microservices/finops-portal/iac/edge-waf.yaml`, `microservices/finops-portal/iac/helm/finops-portal/Chart.yaml`; +9 more.
...
- Example: `tenant-billing-presentation` accepts classical TLS during PQC migration but prefers hybrid when both peers support it and records negotiated group in telemetry.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 12: pentest scope, bounty intake and remediation SLO.

### Service-specific answer
- `finops-portal` is in annual full-scope pentest and every major `tenant-billing-presentation` launch adds targeted test scope before production promotion.
- In-scope assets: `microservices/finops-portal/contracts/cost-allocation-policy-internal.proto`, `microservices/finops-portal/contracts/focus-export-internal.asyncapi.yaml`, `microservices/finops-portal/contracts/tenant-invoice-public.openapi.yaml`, `microservices/finops-portal/iac/ech-config.yaml`, `microservices/finops-portal/iac/edge-waf.yaml`, `microservices/finops-portal/iac/helm/finops-portal/Chart.yaml`; +18 more.
...
- Example: a researcher bypassing `finops-portal` tenant scoping gets safe-harbor handling and an investigation case, not abuse-defence friction by default.
...
## §facility-controls
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 13: inherited data-center, cell and physical-access controls.

### Service-specific answer
- `finops-portal` inherits facility controls from `cell`, `cloud-iac`, and the active provider cell; no direct human facility access is owned by this µservice.
...
- Example: `tenant-billing-presentation` in a regulated cell can only schedule onto node pools with matching facility attestation and residency tag.
...
## §supply-chain-risk
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 19: SBOM, signed artifacts, dependency pinning and provenance.

### Service-specific answer
- `finops-portal` dependency inventory spans crates/catalog, containers, Helm/Kustomize/OpenTofu, Cedar fragments, contracts, and generated SDKs.
- Inventory artifacts: `microservices/finops-portal/catalog/bnf-v4.1.yaml`, `microservices/finops-portal/catalog/oya-finops-portal-budget-alert-kernel.yaml`, `microservices/finops-portal/catalog/oya-finops-portal-commitment-management-domain.yaml`, `microservices/finops-portal/catalog/oya-finops-portal-forecasting-usecase.yaml`, `microservices/finops-portal/catalog/oya-finops-portal-rightsizing-recommender-worker.yaml`, `microservices/finops-portal/catalog/oya-finops-portal-showback-chargeback-domain.yaml`; +15 more.
...
- Example: `tenant-billing-presentation` image promotion requires cosign signature, SLSA provenance, vulnerability scan, license check, and matching manifest/catalog record.
...
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.5: applicable safety/security/policy edge cases and fallbacks.

### Service-specific answer
- Applicable rows for `finops-portal` include account recovery, mistaken mutation, regional outage, regulator deadline, audit access, and delegated-agent authority where relevant.
- Safety invariant: `tenant-billing-presentation` never creates human harm through unnecessary friction, lost recovery path, or silent data loss.
...
- Example: `tenant-billing-presentation` during regional outage preserves audit evidence locally and blocks cross-border transfer if pack policy forbids DR failover.
...
## §data-classification
This anchor is closed for `finops-portal` against documentation-rigor.md §3.2.4 Domain 14: data classes, retention, encryption and transfer restrictions.

### Service-specific answer
- Data classes processed: `INTERNAL_ONLY`, `AUDIT`, `PII_QUASI`.
- State/event surfaces carrying classification: `finops_portal.tenant_billing_presentation`, `finops_portal.cost_allocation_policy`, `finops_portal.anomaly_explanation`, `finops_portal.focus_export`, `finops_portal.credit_ledger`.
...
- Example: `tenant-billing-presentation` labels identifiers as `PII_IDENTIFYING`, operational logs as `AUDIT`, and aggregate metrics as `INTERNAL_ONLY` unless manifest narrows them.
