...
impl ServiceConfig {
    pub fn local_default(tenant_id: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            observability: ObservabilityConfig {
                service_namespace: "oyatie.treasury".to_string(),
                audit_topic: "audit.treasury".to_string(),
                metrics_prefix: "oya_treasury".to_string(),
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.service_name != "treasury" {
            return Err(ServiceError::validation(
                "service_name",
                "service_name must remain treasury",
            ));
        }
