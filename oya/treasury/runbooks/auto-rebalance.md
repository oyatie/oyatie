---
doc_class: Runbook
doc_id: RUNBOOK-TREASURY-AUTO_REBALANCE
...
# Runbook: Treasury Auto Rebalance
...
## Operator Contract
- Runbook id: treasury-auto-rebalance.
- Primary service namespace: `treasury`.
...
- Owning team: axis-treasury + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-treasury-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=treasury; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n treasury rollout status deploy/treasury --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service treasury --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service treasury --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service treasury --cell $CELL --window 30m`.
