---
doc_class: Runbook
doc_id: RUNBOOK-TREASURY-COLD_MERGE
...
## Operator Contract
- Runbook id: treasury-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-treasury-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=treasury; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n treasury rollout status deploy/treasury --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service treasury --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service treasury --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service treasury --cell $CELL --window 30m`.
