---
doc_class: Runbook
doc_id: RUNBOOK-HEALTHCARE_INTEGRATION-COLD_MERGE
...
# Runbook: Healthcare Integration Cold Merge

## Operator Contract
- Runbook id: healthcare-integration-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-healthcare-integration-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=healthcare-integration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n healthcare-integration rollout status deploy/healthcare-integration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service healthcare-integration --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service healthcare-integration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service healthcare-integration --cell $CELL --window 30m`.
