---
doc_class: Runbook
doc_id: RUNBOOK-HEALTHCARE_INTEGRATION-HOT_SPLIT
...
# Runbook: Healthcare Integration Hot Split

## Operator Contract
- Runbook id: healthcare-integration-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-healthcare-integration-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=healthcare-integration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n healthcare-integration rollout status deploy/healthcare-integration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service healthcare-integration --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service healthcare-integration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service healthcare-integration --cell $CELL --window 30m`.
