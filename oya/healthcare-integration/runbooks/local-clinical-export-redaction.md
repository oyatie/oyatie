...
Primary SLO: hl7-ack-latency  
Domain focus: hl7, fhir, hipaa, phi

## Trigger
- Alert `healthcare-integration-hl7-ack-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_healthcare_integration_hl7_ack_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when hl7-ack-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `healthcare-integration-local-hl7-ack-latency` and policy file name to the ticket.
