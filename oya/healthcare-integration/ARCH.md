...
arch_id: ARCH-healthcare-integration
...
companion_docs:
  ...
  - microservices/healthcare-integration/README.md
  - microservices/healthcare-integration/ARCHITECTURE.md
---

# Architecture: Healthcare Integration

## Architecture Boundary

`healthcare-integration` keeps its existing bounded context and flat `microservices/healthcare-integration/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `healthcare-integration`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `healthcare-integration` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/tenant-namespace@v1[both], aws-guest/postgres-wal-g@v1[both], aws-guest/valkey-cluster@v1[both], oci-guest/tenant-namespace@v1[both], oci-guest/valkey-cluster@v1[both], on-prem/tenant-namespace@v1[both], colo/tenant-namespace@v1[both], oyatie-as-cloud-provider/shard-cell@v1[both].
...
- Capacity: `per_workflow_run` drives CPU `0.35`, RAM `768`, storage `10`, and connection pool variables.
- DR: RTO `1800` seconds and RPO `300` seconds constrain backup and failover primitives.
