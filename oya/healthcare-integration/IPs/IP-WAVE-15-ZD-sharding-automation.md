...
bounded_context: patient-record
...
---
# IP-WAVE-15-ZD-sharding-automation: Healthcare Integration Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/healthcare-integration/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: healthcare-integration must interpret ADR-0348 through its own bounded context: patient-record.
SCOPE-004: healthcare-integration uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: healthcare-integration uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: healthcare-integration uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: healthcare-integration.
STANCE-002: Owner team: axis-healthcare-integration + council-product.
...
STANCE-005: Bounded context anchor: patient-record.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_workflow_run.
STANCE-008: Compliance pack input: HIPAA-2024, SOC-2, ISO-27001, GDPR, KR-Medical-Devices, EU-MDR, GxP.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for healthcare-integration.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for healthcare-integration because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; healthcare-integration must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where healthcare-integration participates.
...
ROLLBACK-002: For healthcare-integration, rollback starts from the audit-chain row that recorded the automation event.
