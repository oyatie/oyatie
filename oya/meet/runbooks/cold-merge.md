---
doc_class: Runbook
doc_id: RUNBOOK-MEET-COLD_MERGE
microservice: meet
...
# Runbook: Meet Cold Merge

## Operator Contract
- Runbook id: meet-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-meet-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=meet; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n meet rollout status deploy/meet --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service meet --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service meet --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service meet --cell $CELL --window 30m`.
