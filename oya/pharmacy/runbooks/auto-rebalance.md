---
doc_class: Runbook
doc_id: RUNBOOK-PHARMACY-AUTO_REBALANCE
...
# Runbook: Pharmacy Auto Rebalance

## Operator Contract
- Runbook id: pharmacy-auto-rebalance.
- Primary service namespace: `pharmacy`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-pharmacy-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=pharmacy; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n pharmacy rollout status deploy/pharmacy --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service pharmacy --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service pharmacy --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service pharmacy --cell $CELL --window 30m`.
