...
bounded_context: medication-catalog
...
---
# IP-WAVE-15-ZD-sharding-automation: Pharmacy Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/pharmacy/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: pharmacy must interpret ADR-0348 through its own bounded context: medication-catalog.
SCOPE-004: pharmacy uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: pharmacy uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: pharmacy uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: pharmacy.
STANCE-002: Owner team: axis-pharmacy.
...
STANCE-005: Bounded context anchor: medication-catalog.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_workflow_run.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, hipaa, dea-controlled-substance, gdpr, lgpd, soc2, iso27001, cn-pipl-2021, fedramp, il5, pci-dss, eu-ai-act, kr-pipa, state-board-of-pharmacy, dscsa, usp-797, usp-800, 340b, ncpdp-script, surescripts, fda-21-cfr-part-11, 42-cfr-part-2.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for pharmacy.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for pharmacy because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; pharmacy must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where pharmacy participates.
...
ROLLBACK-002: For pharmacy, rollback starts from the audit-chain row that recorded the automation event.
