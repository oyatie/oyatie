---
doc_class: Runbook
doc_id: RUNBOOK-DATA_PIPELINE-AUTO_REBALANCE
...
# Runbook: Data Pipeline Auto Rebalance

## Operator Contract
- Runbook id: data-pipeline-auto-rebalance.
- Primary service namespace: `data-pipeline`.
...
- Owning team: axis-data-pipeline + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-pipeline-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-pipeline; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-pipeline rollout status deploy/data-pipeline --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-pipeline --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service data-pipeline --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service data-pipeline --cell $CELL --window 30m`.
