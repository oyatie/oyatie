---
doc_class: Runbook
doc_id: RUNBOOK-DATA_PIPELINE-HOT_SPLIT
...
# Runbook: Data Pipeline Hot Split

## Operator Contract
- Runbook id: data-pipeline-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-pipeline-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-pipeline; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-pipeline rollout status deploy/data-pipeline --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-pipeline --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service data-pipeline --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service data-pipeline --cell $CELL --window 30m`.
