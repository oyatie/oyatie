---
doc_class: Runbook
doc_id: RUNBOOK-DATA_PIPELINE-COLD_MERGE
...
# Runbook: Data Pipeline Cold Merge

## Operator Contract
- Runbook id: data-pipeline-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-pipeline-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-pipeline; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-pipeline rollout status deploy/data-pipeline --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-pipeline --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service data-pipeline --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service data-pipeline --cell $CELL --window 30m`.
