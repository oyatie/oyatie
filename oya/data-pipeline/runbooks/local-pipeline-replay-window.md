# Data Pipeline Runbook: Pipeline Replay Window

Service: data-pipeline  
Surface: local operational primitive suite  
Primary SLO: lineage-capture  
Domain focus: ingest, transform, quality, freshness

## Trigger
- Alert `data-pipeline-lineage-capture` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_pipeline_lineage_capture_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when lineage-capture is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-pipeline-local-lineage-capture` and policy file name to the ticket.
