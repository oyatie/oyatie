---
doc_class: Runbook
doc_id: RUNBOOK-PATIENT_MONITORING-HOT_SPLIT
...
# Runbook: Patient Monitoring Hot Split

## Operator Contract
- Runbook id: patient-monitoring-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-patient-monitoring-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=patient-monitoring; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n patient-monitoring rollout status deploy/patient-monitoring --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service patient-monitoring --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service patient-monitoring --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service patient-monitoring --cell $CELL --window 30m`.
