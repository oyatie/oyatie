---
doc_class: Runbook
doc_id: RUNBOOK-PATIENT_MONITORING-COLD_MERGE
...
# Runbook: Patient Monitoring Cold Merge

## Operator Contract
- Runbook id: patient-monitoring-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-patient-monitoring-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=patient-monitoring; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n patient-monitoring rollout status deploy/patient-monitoring --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service patient-monitoring --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service patient-monitoring --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service patient-monitoring --cell $CELL --window 30m`.
