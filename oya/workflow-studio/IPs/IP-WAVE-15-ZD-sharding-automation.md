...
---
# IP-WAVE-15-ZD-sharding-automation: Workflow Studio Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/workflow-studio/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: workflow-studio must interpret ADR-0348 through its own bounded context: workflow-studio.
SCOPE-004: workflow-studio uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: workflow-studio uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: workflow-studio uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: workflow-studio.
STANCE-002: Owner team: axis-workflow-studio.
...
STANCE-005: Bounded context anchor: workflow-studio.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for workflow-studio.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for workflow-studio because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; workflow-studio must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where workflow-studio participates.
...
ROLLBACK-002: For workflow-studio, rollback starts from the audit-chain row that recorded the automation event.
