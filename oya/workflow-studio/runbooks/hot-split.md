---
doc_class: Runbook
doc_id: RUNBOOK-WORKFLOW_STUDIO-HOT_SPLIT
...
# Runbook: Workflow Studio Hot Split

## Operator Contract
- Runbook id: workflow-studio-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-workflow-studio-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=workflow-studio; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n workflow-studio rollout status deploy/workflow-studio --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service workflow-studio --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service workflow-studio --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service workflow-studio --cell $CELL --window 30m`.
