---
doc_class: Runbook
doc_id: RUNBOOK-WORKFLOW_STUDIO-COLD_MERGE
...
# Runbook: Workflow Studio Cold Merge

## Operator Contract
- Runbook id: workflow-studio-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-workflow-studio-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=workflow-studio; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n workflow-studio rollout status deploy/workflow-studio --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service workflow-studio --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service workflow-studio --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service workflow-studio --cell $CELL --window 30m`.
