---
doc_class: Runbook
doc_id: RUNBOOK-WORKFLOW_STUDIO-AUTO_REBALANCE
...
# Runbook: Workflow Studio Auto Rebalance

## Operator Contract
- Runbook id: workflow-studio-auto-rebalance.
...
- Owning team: axis-workflow-studio.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-workflow-studio-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=workflow-studio; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n workflow-studio rollout status deploy/workflow-studio --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service workflow-studio --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service workflow-studio --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service workflow-studio --cell $CELL --window 30m`.
