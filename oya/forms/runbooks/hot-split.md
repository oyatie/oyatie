---
doc_class: Runbook
doc_id: RUNBOOK-FORMS-HOT_SPLIT
microservice: forms
...
# Runbook: Forms Hot Split

## Operator Contract
- Runbook id: forms-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-forms-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=forms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n forms rollout status deploy/forms --timeout=60s`.
3. Verify alerts: operation `observability.alerts_list(service=forms, runbook=hot-split, since=30m)`.
4. Verify primary metric: operation `metrics.query(name=oya_sharding_hot_split_threshold_breach_total, service=forms, cell=$CELL, window=30m)`.
5. Verify secondary metric: operation `metrics.query(name=oya_sharding_hot_split_duration_seconds_p99, service=forms, cell=$CELL, window=30m)`.
