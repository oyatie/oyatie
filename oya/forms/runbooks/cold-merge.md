---
doc_class: Runbook
doc_id: RUNBOOK-FORMS-COLD_MERGE
microservice: forms
...
# Runbook: Forms Cold Merge

## Operator Contract
- Runbook id: forms-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-forms-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=forms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n forms rollout status deploy/forms --timeout=60s`.
3. Verify alerts: operation `observability.alerts_list(service=forms, runbook=cold-merge, since=30m)`.
4. Verify primary metric: operation `metrics.query(name=oya_sharding_cold_merge_candidate_count, service=forms, cell=$CELL, window=30m)`.
5. Verify secondary metric: operation `metrics.query(name=oya_sharding_cold_merge_quiet_hours, service=forms, cell=$CELL, window=30m)`.
...
5. Generate the execution plan: operation `cell_rebalancer.dynamic_sharding_cold_merge_plan(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, dry_run=true, output=json)`.
...
7. Execute only after two-person incident authorization: operation `cell_rebalancer.dynamic_sharding_cold_merge_execute(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, confirm=true)`.
...
## Rollback Path
1. Freeze further cohorts: operation `feature_flags.set(key=sharding_automation.cold-merge.hold, value=true, service=$SERVICE, cell=$CELL, reason=$INCIDENT_ID)`.
2. Restore routing from the last sealed audit-chain rollback pointer.
3. Execute rollback: operation `cell_rebalancer.dynamic_sharding_cold_merge_rollback(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, source=audit_chain)`.
