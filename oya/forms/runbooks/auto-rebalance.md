---
doc_class: Runbook
doc_id: RUNBOOK-FORMS-AUTO_REBALANCE
microservice: forms
...
# Runbook: Forms Auto Rebalance

## Operator Contract
- Runbook id: forms-auto-rebalance.
- Primary service namespace: `forms`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-forms-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=forms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n forms rollout status deploy/forms --timeout=60s`.
3. Verify alerts: operation `observability.alerts_list(service=forms, runbook=auto-rebalance, since=30m)`.
4. Verify primary metric: operation `metrics.query(name=oya_sharding_auto_rebalance_candidate_count, service=forms, cell=$CELL, window=30m)`.
5. Verify secondary metric: operation `metrics.query(name=oya_sharding_auto_rebalance_migration_seconds_p99, service=forms, cell=$CELL, window=30m)`.
...
5. Generate the execution plan: operation `cell_rebalancer.auto_rebalance_plan(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, dry_run=true, output=json)`.
...
7. Execute only after two-person incident authorization: operation `cell_rebalancer.auto_rebalance_execute(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, confirm=true)`.
...
## Rollback Path
1. Freeze further cohorts: operation `feature_flags.set(key=sharding_automation.auto-rebalance.hold, value=true, service=$SERVICE, cell=$CELL, reason=$INCIDENT_ID)`.
2. Restore routing from the last sealed audit-chain rollback pointer.
3. Execute rollback: operation `cell_rebalancer.auto_rebalance_rollback(service=$SERVICE, cell=$CELL, tenant=$TENANT, incident=$INCIDENT_ID, source=audit_chain)`.
