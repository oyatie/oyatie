---
doc_class: Runbook
doc_id: RUNBOOK-TASKS-HOT_SPLIT
microservice: tasks
...
# Runbook: Tasks Hot Split

## Operator Contract
- Runbook id: tasks-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tasks-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tasks; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tasks rollout status deploy/tasks --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tasks --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service tasks --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service tasks --cell $CELL --window 30m`.
