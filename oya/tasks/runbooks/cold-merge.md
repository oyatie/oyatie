---
doc_class: Runbook
doc_id: RUNBOOK-TASKS-COLD_MERGE
microservice: tasks
...
# Runbook: Tasks Cold Merge

## Operator Contract
- Runbook id: tasks-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tasks-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tasks; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tasks rollout status deploy/tasks --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tasks --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service tasks --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service tasks --cell $CELL --window 30m`.
