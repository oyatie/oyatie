---
doc_class: Runbook
doc_id: RUNBOOK-ANALYTICS-COLD_MERGE
...
# Runbook: Analytics Cold Merge

## Operator Contract
- Runbook id: analytics-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-analytics-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=analytics; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n analytics rollout status deploy/analytics --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service analytics --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service analytics --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service analytics --cell $CELL --window 30m`.
