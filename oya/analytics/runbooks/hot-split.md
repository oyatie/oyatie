---
doc_class: Runbook
doc_id: RUNBOOK-ANALYTICS-HOT_SPLIT
...
# Runbook: Analytics Hot Split

## Operator Contract
- Runbook id: analytics-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-analytics-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=analytics; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n analytics rollout status deploy/analytics --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service analytics --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service analytics --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service analytics --cell $CELL --window 30m`.
