---
doc_class: Runbook
doc_id: RUNBOOK-ANALYTICS-AUTO_REBALANCE
...
# Runbook: Analytics Auto Rebalance

## Operator Contract
- Runbook id: analytics-auto-rebalance.
- Primary service namespace: `analytics`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-analytics-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=analytics; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n analytics rollout status deploy/analytics --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service analytics --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service analytics --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service analytics --cell $CELL --window 30m`.
