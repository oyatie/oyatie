...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/analytics/manifest.json
  - microservices/analytics/PRD.md
  - microservices/analytics/README.md
  - microservices/analytics/ARCHITECTURE.md
---
...
`analytics` keeps its existing bounded context and flat `microservices/analytics/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `analytics`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `analytics` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/k8s-namespace-bootstrap@v1[both], oci-guest/secrets-bootstrap@v1[both].
...
- Capacity: `per_query` drives CPU `0.35`, RAM `1024`, storage `40`, and connection pool variables.
