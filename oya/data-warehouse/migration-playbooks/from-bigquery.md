...
# Migration Playbook: Google BigQuery to Oyatie data-warehouse

## Vendor Identity + Categorization
- Vendor product family: Google Cloud BigQuery analytics warehouse.
- Edition/scope: BigQuery Enterprise/Enterprise Plus with datasets, tables, views, routines, reservations, Data Transfer Service, authorized views, policy tags, and row access policies.
- Source documentation family: Google BigQuery export/unload guidance, information schema/catalog references, job/query history docs, access-policy docs, and retention/replication documentation.
- Target microservice owner: axis-data-warehouse + council-product; all tenant movement remains tenant-scoped per ADR-0244.
- Classification: vendor-specific migration from Google BigQuery into Oyatie data-warehouse, not a generic data-load template.
...
- Identity note 1: datasets are regional and cannot be casually crossed; assess it before mapping starts because it changes target object identity or replay order.
- Identity note 2: nested/repeated fields require Avro/Parquet type preservation; assess it before mapping starts because it changes target object identity or replay order.
- Identity note 3: policy tags live in Data Catalog/Dataplex taxonomy; assess it before mapping starts because it changes target object identity or replay order.
- Identity note 4: authorized views encode access edges; assess it before mapping starts because it changes target object identity or replay order.
- Identity note 5: streaming buffer rows may not export immediately; assess it before mapping starts because it changes target object identity or replay order.
...
## Phase 1: Extract
- Named tool: BigQuery extract job orchestrator with INFORMATION_SCHEMA snapshots, bq CLI/API export, Cloud Storage checksums, and job history crawler.
- Named format: Avro or Parquet export to Cloud Storage, JSON table/schema manifest, INFORMATION_SCHEMA rows, and Cloud Audit Logs extract.
- Named throughput: Target 1-4 TB/hour through extract jobs to regional Cloud Storage; partition by table/date and watch slot/reservation pressure for validation queries.
- Named rate-limits: BigQuery extract quotas, job concurrency, table export format restrictions, regional dataset/storage alignment, Cloud Storage write quotas, and rate-limited API/job polling.
...
- Gotcha 1: datasets are regional and cannot be casually crossed.
  - Preventive control: add preflight probe and a named quarantine bucket for records affected by `datasets are regional and cannot be casually `.
...
- Gotcha 2: nested/repeated fields require Avro/Parquet type preservation.
  - Preventive control: add preflight probe and a named quarantine bucket for records affected by `nested/repeated fields require Avro/Parquet t`.
  - Recovery action: pause only the affected object family, keep other families extracting, and re-run the cursor after the control is fixed.
  - Evidence: attach sample source id, target staging id, checksum, and operator note to the checkpoint bundle.
- Gotcha 3: policy tags live in Data Catalog/Dataplex taxonomy.
  - Preventive control: add preflight probe and a named quarantine bucket for records affected by `policy tags live in Data Catalog/Dataplex tax`.
  - Recovery action: pause only the affected object family, keep other families extracting, and re-run the cursor after the control is fixed.
  - Evidence: attach sample source id, target staging id, checksum, and operator note to the checkpoint bundle.
- Gotcha 4: authorized views encode access edges.
  - Preventive control: add preflight probe and a named quarantine bucket for records affected by `authorized views encode access edges`.
  - Recovery action: pause only the affected object family, keep other families extracting, and re-run the cursor after the control is fixed.
  - Evidence: attach sample source id, target staging id, checksum, and operator note to the checkpoint bundle.
- Gotcha 5: streaming buffer rows may not export immediately.
  - Preventive control: add preflight probe and a named quarantine bucket for records affected by `streaming buffer rows may not export immediat`.
  - Recovery action: pause only the affected object family, keep other families extracting, and re-run the cursor after the control is fixed.
  - Evidence: attach sample source id, target staging id, checksum, and operator note to the checkpoint bundle.

### Extract Steps
- Step: schema snapshot.
  - Execute with the named tool and record vendor job/cursor identifiers for Google BigQuery.
...
|---:|---|---|---|---|
| 1 | `table_catalog` | `warehouse.database.name` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 2 | `table_schema` | `warehouse.schema.name` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
...
| 12 | `clustering_ordinal_position` | `warehouse.table.clustering_or_sorting` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
...
| 14 | `size_bytes` | `warehouse.table.storage_bytes` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 15 | `time_partitioning_expiration` | `warehouse.retention.policy_ref` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 16 | `schema_owner` | `warehouse.owner.principal_ref` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 17 | `iam_policy` | `warehouse.access.grant_set` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 18 | `policy_tag` | `warehouse.policy.masking_ref` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 19 | `row_access_policy_name` | `warehouse.policy.row_access_ref` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 20 | `labels` | `warehouse.tag.name` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 21 | `reservation_id` | `warehouse.compute_pool.external_ref` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 22 | `job_id` | `warehouse.job.external_vendor_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 23 | `query` | `warehouse.job.statement_text` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 24 | `creation_time` | `warehouse.job.started_at` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 25 | `total_slot_ms` | `warehouse.job.compute_cost_units` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 26 | `external_data_configuration.source_uris` | `warehouse.external_location.uri` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 27 | `format` | `warehouse.file_format.name` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 28 | `export_uri` | `warehouse.extract.file_uri` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 29 | `transfer_config_id` | `warehouse.ingest_pipeline.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 30 | `scheduled_query_id` | `warehouse.scheduled_job.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 31 | `change_history` | `warehouse.change_stream.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 32 | `linked_dataset` | `warehouse.share.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 33 | `replication_status` | `warehouse.replication.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 34 | `authorized_view` | `warehouse.role.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |
| 35 | `load_job_id` | `warehouse.ingest_event.external_id` | map warehouse metadata and lineage with type-aware normalization | metadata sample and row-count/hash checks match source catalog |

### Field-Level Mapping Notes
- Mapping 1: `table_catalog` becomes `warehouse.database.name` for Google BigQuery.
...
- Mapping 2: `table_schema` becomes `warehouse.schema.name` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 3: `table_name` becomes `warehouse.table.name` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 4: `table_type` becomes `warehouse.table.kind` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 5: `column_name` becomes `warehouse.column.name` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 6: `data_type` becomes `warehouse.column.type` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 7: `numeric_precision` becomes `warehouse.column.numeric_precision` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 8: `numeric_scale` becomes `warehouse.column.numeric_scale` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 9: `is_nullable` becomes `warehouse.column.nullable` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 10: `column_default` becomes `warehouse.column.default_expression` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 11: `description` becomes `warehouse.asset.description` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 12: `clustering_ordinal_position` becomes `warehouse.table.clustering_or_sorting` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 13: `row_count` becomes `warehouse.table.row_count_estimate` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 14: `size_bytes` becomes `warehouse.table.storage_bytes` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 15: `time_partitioning_expiration` becomes `warehouse.retention.policy_ref` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 16: `schema_owner` becomes `warehouse.owner.principal_ref` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 17: `iam_policy` becomes `warehouse.access.grant_set` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 18: `policy_tag` becomes `warehouse.policy.masking_ref` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 19: `row_access_policy_name` becomes `warehouse.policy.row_access_ref` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 20: `labels` becomes `warehouse.tag.name` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 21: `reservation_id` becomes `warehouse.compute_pool.external_ref` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 22: `job_id` becomes `warehouse.job.external_vendor_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 23: `query` becomes `warehouse.job.statement_text` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 24: `creation_time` becomes `warehouse.job.started_at` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 25: `total_slot_ms` becomes `warehouse.job.compute_cost_units` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 26: `external_data_configuration.source_uris` becomes `warehouse.external_location.uri` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 27: `format` becomes `warehouse.file_format.name` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 28: `export_uri` becomes `warehouse.extract.file_uri` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 29: `transfer_config_id` becomes `warehouse.ingest_pipeline.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 30: `scheduled_query_id` becomes `warehouse.scheduled_job.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 31: `change_history` becomes `warehouse.change_stream.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 32: `linked_dataset` becomes `warehouse.share.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 33: `replication_status` becomes `warehouse.replication.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 34: `authorized_view` becomes `warehouse.role.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
- Mapping 35: `load_job_id` becomes `warehouse.ingest_event.external_id` for Google BigQuery.
  - Transform detail: map warehouse metadata and lineage with type-aware normalization; record source raw value and normalized target value in the evidence ledger.
  - Validation detail: metadata sample and row-count/hash checks match source catalog; include one canonical sample and one edge sample for this field when it is active in the tenant.
  - Rollback detail: remove target staging rows keyed by the vendor external id and restore the previous mapping version pointer.
...
- Named regression-check process: warehouse-migration-bigquery-regression-pack: schema/type parity, sample-row hash, aggregate fact checks, policy/grant graph, BI query replay, and cost/throughput envelope validation.
...
### Cutover Runbook
- Cutover action 1: execute Google BigQuery to Oyatie data-warehouse action lane 1 with release-manager timestamping.
...
- Cutover action 2: execute Google BigQuery to Oyatie data-warehouse action lane 2 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 3: execute Google BigQuery to Oyatie data-warehouse action lane 3 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 4: execute Google BigQuery to Oyatie data-warehouse action lane 4 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 5: execute Google BigQuery to Oyatie data-warehouse action lane 5 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 6: execute Google BigQuery to Oyatie data-warehouse action lane 6 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 7: execute Google BigQuery to Oyatie data-warehouse action lane 7 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 8: execute Google BigQuery to Oyatie data-warehouse action lane 8 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 9: execute Google BigQuery to Oyatie data-warehouse action lane 9 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 10: execute Google BigQuery to Oyatie data-warehouse action lane 10 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 11: execute Google BigQuery to Oyatie data-warehouse action lane 11 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 12: execute Google BigQuery to Oyatie data-warehouse action lane 12 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 13: execute Google BigQuery to Oyatie data-warehouse action lane 13 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 14: execute Google BigQuery to Oyatie data-warehouse action lane 14 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Cutover action 15: execute Google BigQuery to Oyatie data-warehouse action lane 15 with release-manager timestamping.
  - Expected state: exactly one authority flag changes or one reconciliation proof is attached; no untracked mutation is allowed.
...
- Verification check 1: cardinality parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `cardinality_parity` against frozen source and target snapshots.
...
- Verification check 2: field hash parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `field_hash_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 3: relationship graph parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `relationship_graph_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 4: status/lifecycle parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `status/lifecycle_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 5: owner/principal parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `owner/principal_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 6: permission/policy parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `permission/policy_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 7: attachment checksum parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `attachment_checksum_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 8: late delta replay.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `late_delta_replay` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 9: dashboard/report parity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `dashboard/report_parity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 10: audit-event continuity.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `audit-event_continuity` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 11: SLO replay.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `SLO_replay` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
- Verification check 12: rollback drill.
  - Method: run data-warehouse-migration-bigquery-regression-suite module `rollback_drill` against frozen source and target snapshots.
  - Pass rule: target delta is within the named gate and every exception has a source id, target id, owner, and disposition.
  - Recovery: quarantine the failing object family, patch mapping or extractor, replay from checkpoint, and rerun this check only plus its dependencies.
...
- Teardown step 1: disable writes for Google BigQuery.
...
- Teardown step 2: archive exports for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 3: revoke credentials for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 4: turn off webhooks/connectors for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 5: retire scheduled jobs for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 6: remove temporary network access for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 7: close support bridge for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.
- Teardown step 8: publish final evidence for Google BigQuery.
  - Guardrail: confirm no unresolved delta or active legal hold depends on this access before execution.
  - Rollback: restore only the minimal read-only or connector permission needed for support; do not re-enable broad write authority.

## Specific Failure Modes
- Failure 1: Export skips streaming or recently loaded rows.
  - Detection: data-warehouse-migration-bigquery-regression-suite emits a red signal with vendor id, target id, object family, cursor, and checksum context.
...
## Checkpoint
- Checkpoint bundle: migration-playbooks-w2-2026-05-20 / data-warehouse / Google BigQuery.
