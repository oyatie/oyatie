---
doc_class: Runbook
doc_id: RUNBOOK-DATA_WAREHOUSE-COLD_MERGE
...
# Runbook: Data Warehouse Cold Merge

## Operator Contract
- Runbook id: data-warehouse-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-warehouse-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-warehouse; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-warehouse rollout status deploy/data-warehouse --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-warehouse --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service data-warehouse --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service data-warehouse --cell $CELL --window 30m`.
