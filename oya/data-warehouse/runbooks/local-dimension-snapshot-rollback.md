# Data Warehouse Runbook: Dimension Snapshot Rollback

Service: data-warehouse  
Surface: local operational primitive suite  
Primary SLO: query-latency  
Domain focus: pipeline, sla-tier, freshness, completeness

## Trigger
- Alert `data-warehouse-query-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_warehouse_query_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when query-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-warehouse-local-query-latency` and policy file name to the ticket.
