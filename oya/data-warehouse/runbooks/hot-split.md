---
doc_class: Runbook
doc_id: RUNBOOK-DATA_WAREHOUSE-HOT_SPLIT
...
# Runbook: Data Warehouse Hot Split

## Operator Contract
- Runbook id: data-warehouse-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-warehouse-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-warehouse; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-warehouse rollout status deploy/data-warehouse --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-warehouse --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service data-warehouse --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service data-warehouse --cell $CELL --window 30m`.
