---
doc_class: Runbook
doc_id: RUNBOOK-QUALITY_MANAGEMENT-COLD_MERGE
...
# Runbook: Quality Management Cold Merge

## Operator Contract
- Runbook id: quality-management-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-quality-management-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=quality-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n quality-management rollout status deploy/quality-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service quality-management --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service quality-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service quality-management --cell $CELL --window 30m`.
