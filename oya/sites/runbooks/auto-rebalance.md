---
doc_class: Runbook
doc_id: RUNBOOK-SITES-AUTO_REBALANCE
microservice: sites
...
# Runbook: Sites Auto Rebalance

## Operator Contract
- Runbook id: sites-auto-rebalance.
- Primary service namespace: `sites`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-sites-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=sites; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n sites rollout status deploy/sites --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service sites --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service sites --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service sites --cell $CELL --window 30m`.
