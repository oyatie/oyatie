---
doc_class: Runbook
doc_id: RUNBOOK-SITES-HOT_SPLIT
microservice: sites
...
# Runbook: Sites Hot Split

## Operator Contract
- Runbook id: sites-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-sites-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=sites; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n sites rollout status deploy/sites --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service sites --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service sites --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service sites --cell $CELL --window 30m`.
