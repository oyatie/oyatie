---
doc_class: Runbook
doc_id: RUNBOOK-SITES-COLD_MERGE
microservice: sites
...
# Runbook: Sites Cold Merge

## Operator Contract
- Runbook id: sites-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-sites-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=sites; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n sites rollout status deploy/sites --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service sites --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service sites --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service sites --cell $CELL --window 30m`.
