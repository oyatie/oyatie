...
bounded_context: bulk-translate
...
---
# IP-WAVE-15-ZD-sharding-automation: Translate Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/translate/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-006: translate uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: translate.
STANCE-002: Owner team: axis-translate.
...
STANCE-005: Bounded context anchor: bulk-translate.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, gdpr, eu-ai-act, hipaa, kr-pipa, lgpd, SOC 2 Type 2, NIS2, soc2, iso27001, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for translate.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for translate because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; translate must not create silent tenant, cell, or shard transitions.
...
ROLLBACK-002: For translate, rollback starts from the audit-chain row that recorded the automation event.
