---
doc_class: Runbook
doc_id: RUNBOOK-TRANSLATE-AUTO_REBALANCE
...
# Runbook: Translate Auto Rebalance

## Operator Contract
- Runbook id: translate-auto-rebalance.
- Primary service namespace: `translate`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-translate-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=translate; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n translate rollout status deploy/translate --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service translate --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service translate --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service translate --cell $CELL --window 30m`.
