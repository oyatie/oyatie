---
doc_class: Runbook
doc_id: RUNBOOK-TRANSLATE-COLD_MERGE
...
# Runbook: Translate Cold Merge

## Operator Contract
- Runbook id: translate-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-translate-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=translate; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n translate rollout status deploy/translate --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service translate --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service translate --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service translate --cell $CELL --window 30m`.
