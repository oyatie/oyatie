---
doc_class: Runbook
doc_id: RUNBOOK-CONTRACT_LIFECYCLE_MANAGEMENT-AUTO_REBALANCE
...
# Runbook: Contract Lifecycle Management Auto Rebalance

## Operator Contract
- Runbook id: contract-lifecycle-management-auto-rebalance.
- Primary service namespace: `contract-lifecycle-management`.
...
- Owning team: axis-contract-lifecycle-management + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contract-lifecycle-management-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contract-lifecycle-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n contract-lifecycle-management rollout status deploy/contract-lifecycle-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service contract-lifecycle-management --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service contract-lifecycle-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service contract-lifecycle-management --cell $CELL --window 30m`.
