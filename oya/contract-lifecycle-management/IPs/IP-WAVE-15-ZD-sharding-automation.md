...
bounded_context: contract-intake
...
---
# IP-WAVE-15-ZD-sharding-automation: Contract Lifecycle Management Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/contract-lifecycle-management/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: contract-lifecycle-management must interpret ADR-0348 through its own bounded context: contract-intake.
SCOPE-004: contract-lifecycle-management uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: contract-lifecycle-management uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: contract-lifecycle-management uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: contract-lifecycle-management.
STANCE-002: Owner team: axis-contract-lifecycle-management + council-product.
...
STANCE-005: Bounded context anchor: contract-intake.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_workflow_run.
STANCE-008: Compliance pack input: soc-2, iso-27001, gdpr, sox-404, eidas, esign, kr-pipa, hipaa-baa, sec-17a-4.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for contract-lifecycle-management.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for contract-lifecycle-management because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; contract-lifecycle-management must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where contract-lifecycle-management participates.
...
ROLLBACK-002: For contract-lifecycle-management, rollback starts from the audit-chain row that recorded the automation event.
