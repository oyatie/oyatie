...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/consent-graph/manifest.json
  - microservices/consent-graph/PRD.md
  - microservices/consent-graph/README.md
  - microservices/consent-graph/ARCHITECTURE.md
---

# Architecture: Consent Graph

## Architecture Boundary

`consent-graph` keeps its existing bounded context and flat `microservices/consent-graph/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `consent-graph`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `consent-graph` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/postgresql-cluster@v1[paid], on-prem/service-mesh-waypoint@v1[paid], colo/valkey-cluster@v1[paid], oyatie-as-cloud-provider/audit-chain-merkle-seal@v1[paid].
...
- Capacity: `per_message` drives CPU `0.12`, RAM `192`, storage `4`, and connection pool variables.
