---
doc_class: Runbook
doc_id: RUNBOOK-CONSENT_GRAPH-AUTO_REBALANCE
...
# Runbook: Consent Graph Auto Rebalance

## Operator Contract
- Runbook id: consent-graph-auto-rebalance.
- Primary service namespace: `consent-graph`.
...
- Owning team: axis-consent-graph.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-consent-graph-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=consent-graph; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n consent-graph rollout status deploy/consent-graph --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service consent-graph --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service consent-graph --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service consent-graph --cell $CELL --window 30m`.
