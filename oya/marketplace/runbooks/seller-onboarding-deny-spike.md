...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=marketplace-seller-onboarding-deny-spike --production-snapshot` exits non-zero against the latest production evidence bundle.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=marketplace-seller-onboarding-deny-spike --production-snapshot --cell $CELL`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/marketplace/runbooks/seller-onboarding-deny-spike.md,evidence/incidents/$INCIDENT_ID.json`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=marketplace-seller-onboarding-deny-spike --fixture incident-seller-onboarding-deny-spike.json`.
