---
doc_class: Runbook
doc_id: RUNBOOK-MARKETPLACE-COLD_MERGE
...
# Runbook: Marketplace Cold Merge

## Operator Contract
- Runbook id: marketplace-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-marketplace-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=marketplace; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n marketplace rollout status deploy/marketplace --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service marketplace --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service marketplace --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service marketplace --cell $CELL --window 30m`.
