---
doc_class: Runbook
doc_id: RUNBOOK-PERFORMANCE_MANAGEMENT-AUTO_REBALANCE
...
# Runbook: Performance Management Auto Rebalance

## Operator Contract
- Runbook id: performance-management-auto-rebalance.
- Primary service namespace: `performance-management`.
...
- Owning team: axis-performance-management + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-performance-management-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=performance-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n performance-management rollout status deploy/performance-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service performance-management --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service performance-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service performance-management --cell $CELL --window 30m`.
