---
doc_class: Runbook
doc_id: RUNBOOK-PERFORMANCE_MANAGEMENT-COLD_MERGE
...
# Runbook: Performance Management Cold Merge

## Operator Contract
- Runbook id: performance-management-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-performance-management-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=performance-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n performance-management rollout status deploy/performance-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service performance-management --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service performance-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service performance-management --cell $CELL --window 30m`.
