---
doc_class: Runbook
doc_id: RUNBOOK-PERFORMANCE_MANAGEMENT-HOT_SPLIT
...
# Runbook: Performance Management Hot Split

## Operator Contract
- Runbook id: performance-management-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-performance-management-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=performance-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n performance-management rollout status deploy/performance-management --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service performance-management --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service performance-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service performance-management --cell $CELL --window 30m`.
