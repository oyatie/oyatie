# Performance Management Runbook: Feedback Submit Failures

Service: performance-management  
Surface: local operational primitive suite  
Primary SLO: feedback-submit-success  
Domain focus: review, calibration, rating, feedback

## Trigger
- Alert `performance-management-feedback-submit-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_performance_management_feedback_submit_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when feedback-submit-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `performance-management-local-feedback-submit-success` and policy file name to the ticket.
