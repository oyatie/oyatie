# IP-016 Performance Management backfill and replay worker

Service: performance-management
ChangeSet scope: microservices/performance-management/IP-016-backfill-replay-worker.md
...
## A. Problem
This IP closes the backfill and replay worker gap for `performance-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `backfill-replay-worker` works for reviews, goals, calibration, engagement pulses, succession evidence, and compensation handoff cannot collapse into generic HR records.
...
The success condition is an implementation plan a cold engineer can trace from this IP to concrete files such as `microservices/performance-management/backfill-replay.md` without inventing a hidden service boundary.

## B. Approach
Implement `backfill-replay-worker` as a service-local slice, not as a shared platform facility. The technical mechanism is idempotent correction of projections and imported records bound to `src/worker/ replay lane` and checked before user-visible promotion.
...
The approach explicitly covers checkpointing, dry-run, rollback bundle, poison queue; those are the failure modes that a stamped IP did not name.

## C. Deliverables
- D01: `microservices/performance-management/PRD.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D02: `microservices/performance-management/ARCHITECTURE.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D03: `microservices/performance-management/manifest.json` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D04: `microservices/performance-management/competitor-parity-matrix.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D05: `microservices/performance-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D06: `microservices/performance-management/backfill-replay.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D07: `microservices/performance-management/contracts/openapi-v1.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D08: `microservices/performance-management/contracts/asyncapi-v1.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D09: `microservices/performance-management/contracts/performance-management-v1.proto` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D10: `microservices/performance-management/src/domain/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D11: `microservices/performance-management/src/usecase/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D12: `microservices/performance-management/src/adapter/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D13: `microservices/performance-management/src/config.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D14: `microservices/performance-management/policies/local-review-cycle-scope.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D15: `microservices/performance-management/policies/local-calibration-lock-control.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D16: `microservices/performance-management/policies/local-engagement-pulse-anonymity.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D17: `microservices/performance-management/policy/review-calibration-authorization.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D18: `microservices/performance-management/capabilities/goal-cycle-open.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
...
2. Add or update the `backfill-replay-worker` contract shape in `backfill-replay.md` and keep request/event/proto field names aligned with ADR-0105 layer naming.
...
5. Bind adapter behavior through `src/worker/ replay lane` and `microservices/performance-management/src/adapter/mod.rs`, using typed errors from `microservices/performance-management/src/error.rs` instead of stringly provider failures.
...
8. Update dashboards/SLOs or the service operating bar to expose success, refusal, replay, latency, and audit completeness for `backfill-replay-worker`.
...
## E. Acceptance
- A reviewer can trace `backfill-replay-worker` from this IP to at least one real contract, one real policy file, one real source file, and one capability/catalog artifact.
...
- The observability evidence includes metric, trace, structured log, audit event id, policy decision id, and low-cardinality labels for `backfill-replay-worker`.
...
- `microservices/performance-management/backfill-replay.md`
...
|---|---|---|
| Lattice | Provides a native `backfill and replay worker` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `backfill-replay-worker` to tenant scope, Cedar deny evidence, audit-chain records, and flat `performance-management` ownership. |
...
- Do not move `backfill-replay-worker` into a sibling service such as `people-records` or `compensation` unless a later ADR changes ownership.
...
- ha_trigger_evidence: `microservices/performance-management/IP-016-backfill-replay-worker.md` matched [`SLO`].
...
- evidence_paths: [`microservices/performance-management/IP-016-backfill-replay-worker.md`, `microservices/performance-management/manifest.json`, `microservices/performance-management/ARCHITECTURE.md`, `microservices/performance-management/PRD.md`, `microservices/performance-management/multi-region.md`, `microservices/performance-management/capacity-model.md`].

## Sustainability emission (per ADR-0344)

- metering_trigger_evidence: `microservices/performance-management/IP-016-backfill-replay-worker.md` matched [`emission`].
- per_call_audit_row_fields: `cost_usd_minor_units`, `co2_grams`, `watt_hours`, `provider`, `region`, `cell`.
- carbon_aware_scheduling: eligible only for deferrable work after compliance-pack exclusions and active RTO/RPO floors are satisfied; excluded from realtime Tier 0/1 paths.
- finops_portal_rollup_axes: `tenant`, `product`, `capability`, `provider`, `cell`.
- evidence_paths: [`microservices/performance-management/IP-016-backfill-replay-worker.md`, `microservices/performance-management/manifest.json`, `microservices/performance-management/capacity-model.md`, `microservices/performance-management/compliance.md`, `microservices/performance-management/ARCHITECTURE.md`].
