...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd)]
pub enum PayrollBackendParityCapability {
...
pub struct PayrollBackendParityProfileInput {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PayrollBackendParityCapabilityEvidence {
    pub capability: Classified<PayrollBackendParityCapability>, // data_class: PUBLIC
    ...
    pub live_money_movement_claimed: Classified<bool>,          // data_class: PUBLIC
    pub tax_filing_submission_claimed: Classified<bool>,        // data_class: PUBLIC
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PayrollBackendParityProfile {
    ...
    pub source_evidence_refs: Classified<Vec<EvidenceRef>>, // data_class: INTERNAL_ONLY
    pub capabilities: Classified<Vec<PayrollBackendParityCapabilityEvidence>>, // data_class: INTERNAL_ONLY
...
/// Builds a preview backend parity coverage profile for payroll contracts.
///
/// This is contract metadata only. It intentionally does not claim production
/// runtime, live money movement, statutory certification, tax filing, provider
/// integration, or cloud deployment.
pub fn build_payroll_backend_parity_profile(
    input: PayrollBackendParityProfileInput,
) -> Result<PayrollBackendParityProfile, PayrollDomainError> {
    validate_identifier(
        &input.tenant_id,
        TENANT_ID_PREFIX,
    ...
    Ok(PayrollBackendParityProfile {
        ...
        profile_evidence_ref: internal(EvidenceRef {
        ...
        nonclaims: public(vec![
            "preview backend parity coverage contracts only; no operational or certified payroll parity".to_owned(),
            "no bank/payment/disbursement rails".to_owned(),
            "no tax filing submission".to_owned(),
            "no live provider integrations".to_owned(),
            "no shared cloud/Kubernetes deployment substrate".to_owned(),
        ]),
        schema_version: public(PAYROLL_BACKEND_PARITY_PROFILE_SCHEMA_VERSION),
    })
}
...
const PAYROLL_BACKEND_PARITY_CAPABILITIES: &[PayrollBackendParityCapability] = &[
