...
ingress forwards GitHub deliveries) so the threat surface is the untrusted
inbound webhook.
...
| **Information disclosure** | Secret leaks via logs/Debug. | `WebhookSecret` is redacted in `Debug`; the secret is only read from `OYA_GITHUB_WEBHOOK_SECRET` (injected from OpenBao via `sref`); never written to disk. |
...
(update GitHub + OpenBao together). ADR-0112's 90-day rotation TTL guidance
