---
doc_class: Runbook
doc_id: RUNBOOK-COMMS_EMAIL-HOT_SPLIT
...
# Runbook: Comms Email Hot Split

## Operator Contract
- Runbook id: comms-email-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-comms-email-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=comms-email; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n comms-email rollout status deploy/comms-email --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service comms-email --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service comms-email --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service comms-email --cell $CELL --window 30m`.
