...
bounded_context: transactional-send
...
---
# IP-WAVE-15-ZD-sharding-automation: Comms Email Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/comms-email/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: comms-email must interpret ADR-0348 through its own bounded context: transactional-send.
SCOPE-004: comms-email uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: comms-email uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: comms-email uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: comms-email.
STANCE-002: Owner team: oya-substrate-comms.
...
STANCE-005: Bounded context anchor: transactional-send.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_message.
STANCE-008: Compliance pack input: gdpr, us-healthcare, hipaa, soc2, iso27001-2022, kr-pipa, iso27001, eu-ai-act, PCI-DSS, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for comms-email.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for comms-email because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; comms-email must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where comms-email participates.
...
ROLLBACK-002: For comms-email, rollback starts from the audit-chain row that recorded the automation event.
