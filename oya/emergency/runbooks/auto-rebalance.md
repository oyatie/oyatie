---
doc_class: Runbook
doc_id: RUNBOOK-EMERGENCY-AUTO_REBALANCE
...
# Runbook: Emergency Auto Rebalance

## Operator Contract
- Runbook id: emergency-auto-rebalance.
- Primary service namespace: `emergency`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-emergency-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=emergency; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n emergency rollout status deploy/emergency --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service emergency --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service emergency --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service emergency --cell $CELL --window 30m`.
