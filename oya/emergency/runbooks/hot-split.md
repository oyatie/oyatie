---
doc_class: Runbook
doc_id: RUNBOOK-EMERGENCY-HOT_SPLIT
...
# Runbook: Emergency Hot Split

## Operator Contract
- Runbook id: emergency-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-emergency-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=emergency; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n emergency rollout status deploy/emergency --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service emergency --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service emergency --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service emergency --cell $CELL --window 30m`.
