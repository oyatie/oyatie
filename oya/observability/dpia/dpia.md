...
# Data Protection Impact Assessment: observability
...
- ADR-0349: Jenkins (LTS) is the sole CI orchestrator and ArgoCD is the canonical self-hostable GitOps CD substrate; GitHub Actions is retired, and ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `oya-governance-jenkins-canonical-no-gha-residue`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-governance-jenkins-jcasc-only`, and `oya-governance-deploy-audit-chain-emit`.
