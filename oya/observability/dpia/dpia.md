...
- ADR-0515: Jenkins (LTS) is the sole CI orchestrator and ArgoCD is the canonical self-hostable GitOps CD substrate; GitHub Actions is retired, and ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `oya-ci-required`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-ci-required`, and `oya-governance-deploy-audit-chain-emit`.
