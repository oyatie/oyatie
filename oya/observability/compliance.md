...
This anchor is closed for `observability` against ADR-0250 §D-1: certification-ready evidence roster and audit scope.

### Service-specific answer
- Certification scope for `observability` covers packs `kr`, `eu`, `us`, `us-healthcare`, `jp`, `sg`; +5 more.
...
- Primary evidence files: `microservices/observability/slos/alerting-fanout-latency.openslo.yaml`, `microservices/observability/slos/clickhouse-ingest-throughput.openslo.yaml`, `microservices/observability/slos/log-ingest-availability.openslo.yaml`, `microservices/observability/slos/metric-ingest-availability.openslo.yaml`, `microservices/observability/slos/query-latency-logs.openslo.yaml`, `microservices/observability/slos/query-latency-prom.openslo.yaml`; +18 more.
- Example: `eligibility-query` readiness requires a signed audit event, an OpenAPI/AsyncAPI schema, an SLO target, and a pack-specific retention statement before launch.
...
## §pack-overlay-roster
This anchor is closed for `observability` against ADR-0251 §D-2: pack activation, overlays and per-pack Cedar deltas.

### Service-specific answer
- Active/expected pack roster: `kr`, `eu`, `us`, `us-healthcare`, `jp`, `sg`; +5 more.
- Pack overlays modify Cedar fragments `microservices/observability/policy/auditor-scope.cedar`, `microservices/observability/policy/ci-scope.cedar`, `microservices/observability/policy/data-residency.md`, `microservices/observability/policy/public-read.cedar`, `microservices/observability/policy/tenant-isolation.md`, `microservices/observability/policy/tenant-scope.cedar` without changing domain code.
...
- Example: `eligibility-query` under KR pack uses KR cell routing, KR breach-notification timers, and pack-local audit retention.
...
This anchor is closed for `observability` against ADR-0284 §D-1: platform_owner indirection and hard-coded brand-string audit.

### Service-specific answer
- Runtime platform-owner string is configured as `platform_owner.display_name`; `observability` does not hard-code user-visible owner names in API or UI output.
...
- Surfaces audited for display strings: `microservices/observability/contracts/asyncapi/eligibility-events.yaml`, `microservices/observability/contracts/metric-naming-convention.md`, `microservices/observability/contracts/openapi/slo-engine.yaml`, `microservices/observability/contracts/proto/slo-engine.proto`, `microservices/observability/runbooks/canary-graduation.md`, `microservices/observability/runbooks/clickhouse-restore.md`; +20 more.
...
- Example: `eligibility-query` error text says `platform owner` or config-resolved name, while audit event principal remains `oyatie.observability.runtime`.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.6.A: detection-event categories and routing topology.

### Service-specific answer
- `observability` emits detection signals through ADR-0263 audit pipeline, not an ungoverned side channel.
- Detection families applicable here: policy violation, insider risk, account-takeover, content/transaction abuse where `eligibility-query` touches those data classes.
- Signal sources: `microservices/observability/policy/auditor-scope.cedar`, `microservices/observability/policy/ci-scope.cedar`, `microservices/observability/policy/data-residency.md`, `microservices/observability/policy/public-read.cedar`, `microservices/observability/policy/tenant-isolation.md`, `microservices/observability/policy/tenant-scope.cedar`; +14 more.
- Example event class: `oya.observability.eligibility.query.risk_signal_emitted` with risk score, reason code, and tenant-safe dimensions.
...
This anchor is closed for `observability` against ADR-0310 §D-1: detection-to-investigation evidence handoff and case binding.

### Service-specific answer
- Investigation handoff starts from a signed detection event emitted by `observability` and ends in a case record with immutable evidence pointers.
- Cedar permit `oyatie.observability.investigation.open` gates who may create, read, export, or close a case.
- Evidence pack includes audit event id, policy decision hash, affected `eligibility-query` resource ids, tenant id, data classes, and SLO/degraded-mode context.
...
- Example: a suspicious `eligibility-query` mutation opens a case only after risk threshold and Cedar permit both pass; low-confidence signals remain queued for aggregation.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 8: privileged access, break-glass and UEBA controls.

### Service-specific answer
- Operators of `observability` have no standing unredacted tenant-data access; JIT elevation uses identity step-up and Cedar approval.
- Break-glass access requires reason, scope, expiry, reviewer where required, and post-hoc audit review.
- Sensitive surfaces: `microservices/observability/contracts/asyncapi/eligibility-events.yaml`, `microservices/observability/contracts/metric-naming-convention.md`, `microservices/observability/contracts/openapi/slo-engine.yaml`, `microservices/observability/contracts/proto/slo-engine.proto`, `observability.observability`.
...
- Example: reading `observability.observability` outside declared incident purpose creates a high-risk insider signal and routes to investigation.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 9: threat feed sources, freshness and degraded-mode policy.

### Service-specific answer
- `observability` consumes central threat intelligence for IP/domain reputation, credential stuffing, bot fingerprints, sanctions/abuse lists where applicable, and malicious package indicators.
...
- Enforcement points: `microservices/observability/policy/auditor-scope.cedar`, `microservices/observability/policy/ci-scope.cedar`, `microservices/observability/policy/data-residency.md`, `microservices/observability/policy/public-read.cedar`, `microservices/observability/policy/tenant-isolation.md`, `microservices/observability/policy/tenant-scope.cedar`; +12 more.
- Example: `eligibility-query` with malicious IP reputation receives stricter quota/challenge; high-risk legal/financial flows can halt pending investigation.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 16: signing, encryption, ECH and PQC key rotation cadence.

### Service-specific answer
- Signing keys for `oya.observability` audit events rotate at ≤90 days or immediately on suspected compromise.
...
- Secret paths use `${openbao:secret/<tenant_id>/observability/<key-class>}` and never embed raw tenant ids in metrics labels.
- Runbook evidence: `microservices/observability/runbooks/canary-graduation.md`, `microservices/observability/runbooks/clickhouse-restore.md`, `microservices/observability/runbooks/clickhouse.md`, `microservices/observability/runbooks/evaluator-down.md`, `microservices/observability/runbooks/held-promotion-recovery.md`, `microservices/observability/runbooks/mimir-outage.md`; +4 more.
- Example: `eligibility-query` credential rotation drains in-flight requests with old key id, validates new key id, then retires old leases after audit-chain seal.
...
## §crypto-agility-plan
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 20: algorithm roster, deprecation triggers and migration windows.

### Service-specific answer
- `observability` uses algorithm policy from sidecar/config; domain code never hard-codes cipher or signature choices.
...
- Affected surfaces: `microservices/observability/contracts/asyncapi/eligibility-events.yaml`, `microservices/observability/contracts/metric-naming-convention.md`, `microservices/observability/contracts/openapi/slo-engine.yaml`, `microservices/observability/contracts/proto/slo-engine.proto`, `microservices/observability/iac/helm/alertmanager/Chart.yaml`, `microservices/observability/iac/helm/alertmanager/values.yaml`; +10 more.
...
- Example: `eligibility-query` accepts classical TLS during PQC migration but prefers hybrid when both peers support it and records negotiated group in telemetry.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 12: pentest scope, bounty intake and remediation SLO.

### Service-specific answer
- `observability` is in annual full-scope pentest and every major `eligibility-query` launch adds targeted test scope before production promotion.
- In-scope assets: `microservices/observability/contracts/asyncapi/eligibility-events.yaml`, `microservices/observability/contracts/metric-naming-convention.md`, `microservices/observability/contracts/openapi/slo-engine.yaml`, `microservices/observability/contracts/proto/slo-engine.proto`, `microservices/observability/iac/helm/alertmanager/Chart.yaml`, `microservices/observability/iac/helm/alertmanager/values.yaml`; +16 more.
...
- Example: a researcher bypassing `observability` tenant scoping gets safe-harbor handling and an investigation case, not abuse-defence friction by default.
...
## §facility-controls
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 13: inherited data-center, cell and physical-access controls.

### Service-specific answer
- `observability` inherits facility controls from `cell`, `cloud-iac`, and the active provider cell; no direct human facility access is owned by this µservice.
...
- Example: `eligibility-query` in a regulated cell can only schedule onto node pools with matching facility attestation and residency tag.
...
## §supply-chain-risk
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 19: SBOM, signed artifacts, dependency pinning and provenance.

### Service-specific answer
- `observability` dependency inventory spans crates/catalog, containers, Helm/Kustomize/OpenTofu, Cedar fragments, contracts, and generated SDKs.
- Inventory artifacts: `microservices/observability/catalog/oya-observability-otel-ingest-adapter.yaml`, `microservices/observability/catalog/oya-observability-otel-ingest-api.yaml`, `microservices/observability/catalog/oya-observability-otel-ingest-app.yaml`, `microservices/observability/catalog/oya-observability-otel-ingest-kernel.yaml`, `microservices/observability/catalog/oya-observability-otel-ingest-usecase.yaml`, `microservices/observability/catalog/oya-observability-slo-engine-adapter-mimir.yaml`; +22 more.
...
- Example: `eligibility-query` image promotion requires cosign signature, SLSA provenance, vulnerability scan, license check, and matching manifest/catalog record.
...
This anchor is closed for `observability` against documentation-rigor.md §3.2.5: applicable safety/security/policy edge cases and fallbacks.

### Service-specific answer
- Applicable rows for `observability` include account recovery, mistaken mutation, regional outage, regulator deadline, audit access, and delegated-agent authority where relevant.
- Safety invariant: `eligibility-query` never creates human harm through unnecessary friction, lost recovery path, or silent data loss.
...
- Example: `eligibility-query` during regional outage preserves audit evidence locally and blocks cross-border transfer if pack policy forbids DR failover.
...
## §data-classification
This anchor is closed for `observability` against documentation-rigor.md §3.2.4 Domain 14: data classes, retention, encryption and transfer restrictions.

### Service-specific answer
- Data classes processed: `AUDIT`, `INTERNAL_ONLY`, `PII_QUASI`.
- State/event surfaces carrying classification: `observability.observability`.
...
- Example: `eligibility-query` labels identifiers as `PII_IDENTIFYING`, operational logs as `AUDIT`, and aggregate metrics as `INTERNAL_ONLY` unless manifest narrows them.
