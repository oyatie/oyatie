...
SCOPE-006: observability uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-019: CI/CD substrate stance is ADR-0515 single-context CI authority once the rollout wave authors deployment surfaces.
...
ADR349-PURPOSE-001: GitHub Actions plus branch protection are the live CI substrate, cloud-ci Rust apps produce the single protected `oya-ci-required` context, and Argo CD is only a separately authorized bridge/reference CD adapter for the Oyatie corpus.
...
ADR349-ENFORCED-BY-001: oya-ci-required (new lane; refuses newly introduced GitHub Actions CI definitions and requires cloud-ci Rust gate coverage for the per-microservice CI contract; promoted to BLOCKER per ADR-0515 single-context CI doctrine)
ADR349-ENFORCED-BY-002: oya-governance-argocd-application-cosign-verified (new lane; refuses ArgoCD Application CRD sources that reference an image without a cosign-verify policy attached per D-6 + ADR-0181; promoted to BLOCKER per ADR-0515 single-context CI doctrine)
ADR349-ENFORCED-BY-003: oya-governance-argocd-tenant-namespace-isolation (new lane; refuses ArgoCD Application authoring that crosses tenant namespaces without a Cedar policy gate granting cross-tenant access per D-11 + ADR-0243; promoted to BLOCKER per ADR-0515 single-context CI doctrine)
ADR349-ENFORCED-BY-004: oya-ci-required (new lane; refuses any non-cloud-ci producer for the protected `oya-ci-required` context per D-1; promoted to BLOCKER per ADR-0515 single-context CI doctrine)
ADR349-ENFORCED-BY-005: oya-governance-deploy-audit-chain-emit (new lane; refuses ArgoCD sync transitions that do not emit an audit-chain row per ADR-0263 D.4 deploy-event class; promoted to BLOCKER per ADR-0515 single-context CI doctrine)
...
PLAN-026: Use ADR-0515 `oya-ci-required` evidence when self-hosted CI is introduced.
...
ACCEPT-015: The file cites current CI/CD authority from ADR-0515.
