...
- Audit event class: `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT` with ADR-0263 fields `incident_id`, `tenant_id`, `cell_id`, `microservice`, `runbook_id`, `decision_id`, `evidence_hash`, `operator_id`.
- Stop condition: mitigation has held for 30 minutes, ObservabilityEvaluatorDownCritical is green, and all handoff APIs in Cross-µservice Coordination return `202 accepted`.
...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=observability-evaluator-down --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Alertmanager route: `oyatie-observability-evaluator-down-critical`; silence only with incident commander approval and `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT` evidence.
- Synthetic probe: `oya ops probe observability evaluator-down --cell prod-us-east-1 --tenant synthetic-canary` returns `healthy=true`.
- Drift detector: `registry/observability/evaluator-down/expected-state.json` hash differs from live `https://observability.internal.oyatie.dev/v1/admin/state-hash`.

## Symptoms
- User-facing impact: evaluator down blocks or corrupts the observability control path for affected tenants.
- Operators see Grafana panel `tail-sampling-fidelity / Evaluator Down burn rate` turn red before the primary alert resolves.
...
- Log signature `audit_emit_status=stalled event_class=EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT` means mitigation cannot be closed until replay succeeds.

## Diagnostic Steps
1. Set incident variables: `export INCIDENT_ID=INC-observability-evaluator-down-$(date -u +%Y%m%dT%H%M%SZ); export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Confirm active alerts: `curl -s https://observability.internal.oyatie.dev/v1/alerts?runbook=evaluator-down | jq .alerts`.
3. Check Kubernetes rollout: `kubectl -n observability rollout status deploy/observability-evaluator-down-worker --timeout=60s`.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=observability-evaluator-down --production-snapshot --cell $CELL`.
...
20. Check recent deploy: `kubectl -n observability rollout history deploy/observability-evaluator-down-worker | tail -20`.
...
```
- Branch A (single tenant isolated): use the matching mitigation block below and record `decision_branch=A` in `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT`.
- Branch B (fleet-wide propagation): use the matching mitigation block below and record `decision_branch=B` in `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT`.
- Branch C (dependency regression): use the matching mitigation block below and record `decision_branch=C` in `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT`.
- Branch D (operator ceremony incomplete): use the matching mitigation block below and record `decision_branch=D` in `EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT`.
...
3. Freeze risky automation: `oya flags set oya.observability.evaluator_down.incident_hold=true --cell $CELL --tenant $TENANT --reason $INCIDENT_ID`.
4. Enable circuit breaker: `oya ops breaker open observability-evaluator-down-circuit-breaker --cell $CELL --tenant $TENANT --ttl 30m --reason $INCIDENT_ID`.
5. Reduce blast radius: `kubectl -n observability scale deploy/observability-evaluator-down-worker --replicas=1`.
6. Protect tenant boundary: `oya tenancy quarantine --tenant $TENANT --reason observability-evaluator-down --ttl 60m`.
7. Pause promotion: keep the incident rollback/fix PR unmerged and require Jenkins promotion checks to remain held/failing for `$INCIDENT_ID` (runbook: evaluator-down).
8. Drain queue safely: `oya ops observability evaluator-down drain --cell $CELL --tenant $TENANT --max-items 500 --dry-run`.
9. Execute bounded drain: `oya ops observability evaluator-down drain --cell $CELL --tenant $TENANT --max-items 500 --confirm $INCIDENT_ID`.
10. Replay missing audit events: `oya audit-chain replay --event-class EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT --incident $INCIDENT_ID --from evidence/incidents/$INCIDENT_ID.json`.
11. Rollback last deploy if causal: `kubectl -n observability rollout undo deploy/observability-evaluator-down-worker`.
12. Raise HPA cap if saturation: `kubectl -n observability patch hpa observability-evaluator-down-worker --type merge -p '{"spec":{"maxReplicas":12}}'`.
13. Throttle hot tenant: `oya ops rate-limit set --tenant $TENANT --surface observability.evaluator-down --rps 25 --ttl 30m`.
14. Block abusive principal: `oya identity principal suspend --principal suspected-abuse --tenant $TENANT --reason $INCIDENT_ID`.
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/observability/runbooks/evaluator-down.md,evidence/incidents/$INCIDENT_ID.json`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=observability-evaluator-down --fixture incident-evaluator-down.json`.
...
18. Seal resolution audit: `oya audit-chain emit --event-class EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT --incident $INCIDENT_ID --field resolution=complete --field runbook=evaluator-down`.
19. Verify seal: `oya audit-chain verify --event-class EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT --incident $INCIDENT_ID`.
...
- feature flag oya.observability.evaluator_down.incident_hold is false for the affected tenant unless long-term hold is approved.
...
runbook_id: observability-evaluator-down
microservice: observability
event_class: EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT
incident_id: <INC-...>
severity: sev1
status: draft
detected_at: <UTC>
...
# Evaluator Down postmortem

## Summary
- What happened in observability/evaluator-down.
...
- Emit EVT-OBSERVABILITY-EVALUATOR_DOWN-INCIDENT with incident_id, tenant_id, cell_id, principal_id, decision_id, evidence_hash, operator_id, runbook_id.
...
- Customer communications: use status page component `oyatie-observability-evaluator-down` and keep private details in the incident channel.
...
- Notify `identity`: `oya incident handoff --target identity --source observability --runbook evaluator-down --incident $INCIDENT_ID --severity sev1 --branch A`; expect `202 accepted`.
- Require `identity` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `tenancy`: `oya incident handoff --target tenancy --source observability --runbook evaluator-down --incident $INCIDENT_ID --severity sev1 --branch B`; expect `202 accepted`.
- Require `tenancy` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `governance`: `oya incident handoff --target governance --source observability --runbook evaluator-down --incident $INCIDENT_ID --severity sev1 --branch C`; expect `202 accepted`.
- Require `governance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `compliance`: `oya incident handoff --target compliance --source observability --runbook evaluator-down --incident $INCIDENT_ID --severity sev1 --branch D`; expect `202 accepted`.
- Require `compliance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Observability handoff API: `oya incident handoff --target observability --source observability --runbook evaluator-down --incident $INCIDENT_ID`.
- Governance handoff API: `oya incident handoff --target governance --source observability --runbook evaluator-down --incident $INCIDENT_ID`.
- Compliance handoff API: `oya incident handoff --target compliance --source observability --runbook evaluator-down --incident $INCIDENT_ID`.
- Identity handoff API: `oya incident handoff --target identity --source observability --runbook evaluator-down --incident $INCIDENT_ID`.
...
- Existing thin runbook topic `evaluator-down` was preserved as the scenario anchor while replacing generic steps with concrete commands.
