...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=observability-mimir-outage --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=observability-mimir-outage --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=observability-mimir-outage --fixture incident-mimir-outage.json`.
...
9. Add dashboard panel: `update microservices/observability/dashboards/tenant-slo-overview.json with oya_observability_mimir_outage_error_ratio, oya_observability_mimir_outage_lag_seconds, and oya_observability_mimir_outage_queue_depth`.
