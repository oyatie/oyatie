...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=payments-kyc-aml-screening-pipeline-stall --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio` exceeds the threshold documented in `microservices/payments/slos/charge-api-availability.openslo.yaml`.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=payments-kyc-aml-screening-pipeline-stall --production-snapshot --cell $CELL`.
16. Run payments contract smoke: `buck2 build //:quality-lane-registry-authority-check # lane=payments-contract --microservice payments --scenario kyc-aml-screening-pipeline-stall`.
...
2. Patch catalog invariant: `edit microservices/payments/catalog/oya-payments-charge-domain.yaml or the matching refund/dispute/payout/KYC catalog record where kyc_aml_screening_pipeline_stall state transition is validated`.
...
6. Add regression gate: `buck2 build //:quality-lane-registry-authority-check # lane=payments-contract --fixture incident-kyc-aml-screening-pipeline-stall.json`.
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=payments-kyc-aml-screening-pipeline-stall --fixture incident-kyc-aml-screening-pipeline-stall.json`.
...
9. Add dashboard panel: `update microservices/payments/dashboards/psp-routing.json with oya_payments_kyc_aml_screening_pipeline_stall_error_ratio, oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds, and oya_payments_kyc_aml_screening_pipeline_stall_queue_depth`.
...
- `microservices/payments/catalog/oya-payments-charge-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `microservices/payments/catalog/oya-payments-refund-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `microservices/payments/catalog/oya-payments-dispute-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `microservices/payments/catalog/oya-payments-payout-domain.yaml`: inspect for `kyc_aml_screening_pipeline_stall` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `microservices/payments/dashboards/psp-routing.json`: verify panel coverage for `oya_payments_kyc_aml_screening_pipeline_stall_error_ratio`, `oya_payments_kyc_aml_screening_pipeline_stall_lag_seconds`, and `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio`.
...
- Service-specific signal `oya_payments_kyc_aml_screening_pipeline_stall_psp_decline_ratio` is below the threshold documented in `microservices/payments/slos/charge-api-availability.openslo.yaml`.
