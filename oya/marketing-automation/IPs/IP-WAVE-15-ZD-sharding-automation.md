...
---
# IP-WAVE-15-ZD-sharding-automation: Marketing Automation Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/marketing-automation/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: marketing-automation must interpret ADR-0348 through its own bounded context: segment.
SCOPE-004: marketing-automation uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: marketing-automation uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: marketing-automation uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: marketing-automation.
STANCE-002: Owner team: axis-marketing-automation + council-product.
...
STANCE-005: Bounded context anchor: segment.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_message.
STANCE-008: Compliance pack input: SOC-2, ISO-27001, GDPR, LGPD, KR-PIPA, CPRA, CAN-SPAM, CASL, HIPAA, ePrivacy-Directive, TCPA, EU-AI-Act-Marketing-Personalization.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for marketing-automation.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for marketing-automation because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; marketing-automation must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where marketing-automation participates.
...
ROLLBACK-002: For marketing-automation, rollback starts from the audit-chain row that recorded the automation event.
