---
doc_class: Runbook
doc_id: RUNBOOK-MARKETING_AUTOMATION-HOT_SPLIT
...
# Runbook: Marketing Automation Hot Split

## Operator Contract
- Runbook id: marketing-automation-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-marketing-automation-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=marketing-automation; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n marketing-automation rollout status deploy/marketing-automation --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service marketing-automation --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service marketing-automation --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service marketing-automation --cell $CELL --window 30m`.
