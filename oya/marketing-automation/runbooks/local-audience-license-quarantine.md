# Marketing Automation Runbook: Audience License Quarantine

Service: marketing-automation  
Surface: local operational primitive suite  
Primary SLO: journey-trigger-latency  
Domain focus: campaign, journey, segment, suppression

## Trigger
- Alert `marketing-automation-journey-trigger-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_marketing_automation_journey_trigger_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when journey-trigger-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `marketing-automation-local-journey-trigger-latency` and policy file name to the ticket.
