---
doc_class: Runbook
doc_id: RUNBOOK-MARKETING_AUTOMATION-AUTO_REBALANCE
...
# Runbook: Marketing Automation Auto Rebalance

## Operator Contract
- Runbook id: marketing-automation-auto-rebalance.
- Primary service namespace: `marketing-automation`.
...
- Owning team: axis-marketing-automation + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-marketing-automation-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=marketing-automation; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n marketing-automation rollout status deploy/marketing-automation --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service marketing-automation --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service marketing-automation --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service marketing-automation --cell $CELL --window 30m`.
