---
doc_class: Runbook
doc_id: RUNBOOK-MARKETING_AUTOMATION-COLD_MERGE
...
# Runbook: Marketing Automation Cold Merge

## Operator Contract
- Runbook id: marketing-automation-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-marketing-automation-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=marketing-automation; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n marketing-automation rollout status deploy/marketing-automation --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service marketing-automation --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service marketing-automation --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service marketing-automation --cell $CELL --window 30m`.
