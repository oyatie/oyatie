# Marketing Automation Runbook: Webhook Provider Degradation

Service: marketing-automation  
Surface: local operational primitive suite  
Primary SLO: suppression-enforcement  
Domain focus: campaign, journey, segment, suppression

## Trigger
- Alert `marketing-automation-suppression-enforcement` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_marketing_automation_suppression_enforcement_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when suppression-enforcement is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `marketing-automation-local-suppression-enforcement` and policy file name to the ticket.
