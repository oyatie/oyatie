...
ip_id: IP-004-cluster-health-and-recovery
status: planned
...
The previous slice for `IP-004-cluster-health-and-recovery` was too close to a design-anchor shell: it named the intended control but did not bind the work to ops-dashboard-control-center's actual contracts, policy files, SLOs, and runbooks. This IP closes the `cluster health read model` gap for the ops-dashboard-control-center µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
Technical target set: cluster-health-query.yaml, cluster-health catalog/API, cell-operator.json, cluster-health-freshness.openslo.yaml. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `cluster health read model` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: cluster-health-query.yaml, cluster-health catalog/API, cell-operator.json, cluster-health-freshness.openslo.yaml.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `cluster health read model`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from cluster-health-query.yaml, cluster-health catalog/API, cell-operator.json, cluster-health-freshness.openslo.yaml; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: cluster-health-query.yaml, cluster-health catalog/API, cell-operator.json, cluster-health-freshness.openslo.yaml.
...
|---|---|---|
| AWS internal console and Backstage service catalog | Mature external control surface to compare against. | AWS internal console and Backstage service catalog are counterpart read surfaces; Oyatie adds Cedar-scoped recovery evidence and cell-aware health freshness. |
...
1. Represent cluster/node/cell health as observed signals, not operator decisions.
2. Poll Kubernetes, SPIFFE, Cilium, and recovery workflow state through adapters outside the kernel.
3. Expose read-only health through `cluster-health-query.yaml`; mutation requires separate recovery approval IP.
4. AWS regional health and Backstage service catalog are counterparts; Oyatie adds cell and SVID evidence.
5. Tests cover stale health, SVID expiry, Cilium drop alert, and recovery workflow projection.
...
- Hook 11.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 11.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 11.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 11.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 11.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 11.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 11.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-004-cluster-health-and-recovery.md` matched [`SLO`].
