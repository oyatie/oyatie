...
bounded_context: incident-command
...
slot_owner: ZF-9
sharding_role: operator-surface-owner
---
...
SCOPE-005: ops-dashboard-control-center uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: ops-dashboard-control-center uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: ops-dashboard-control-center.
STANCE-002: Owner team: ops-sre-reliability.
STANCE-003: Sharding role: operator-surface-owner.
STANCE-004: Role statement: shows automation state, refusal reasons, and rollback controls without becoming the authority.
STANCE-005: Bounded context anchor: incident-command.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, soc2, iso27001, gdpr, hipaa, fedramp-high, lgpd, soc2-type2, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for ops-dashboard-control-center.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for ops-dashboard-control-center because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; ops-dashboard-control-center must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where ops-dashboard-control-center participates.
...
ROLLBACK-002: For ops-dashboard-control-center, rollback starts from the audit-chain row that recorded the automation event.
