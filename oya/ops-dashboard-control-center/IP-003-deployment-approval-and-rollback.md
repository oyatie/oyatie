...
ip_id: IP-003-deployment-approval-and-rollback
status: planned
...
The previous slice for `IP-003-deployment-approval-and-rollback` was too close to a design-anchor shell: it named the intended control but did not bind the work to ops-dashboard-control-center's actual contracts, policy files, SLOs, and runbooks. This IP closes the `deployment command approval/rollback` gap for the ops-dashboard-control-center µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
Technical target set: deployment-approve.yaml, rollback-execute.yaml, deployment-command catalog/API, deployment-rollback.md runbook. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `deployment command approval/rollback` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: deployment-approve.yaml, rollback-execute.yaml, deployment-command catalog/API, deployment-rollback.md runbook.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `deployment command approval/rollback`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from deployment-approve.yaml, rollback-execute.yaml, deployment-command catalog/API, deployment-rollback.md runbook; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: deployment-approve.yaml, rollback-execute.yaml, deployment-command catalog/API, deployment-rollback.md runbook.
...
|---|---|---|
| GitHub deployment environments and Argo CD | Mature external control surface to compare against. | GitHub deployment environments and Argo CD supply precedent for approval gates, while Stripe Internal Admin supplies per-action audit expectations. |
...
1. Keep deployment approval, hold, freeze-window, and rollback as distinct commands with distinct audit events.
2. Read Argo/GitOps state but do not execute arbitrary host mutation from the dashboard.
3. Rollback requires separate rationale and capability binding to `rollback-execute.yaml`.
4. GitHub deployment environments are the approval counterpart; Stripe Internal Admin is the audit trail counterpart.
5. Tests inject failed progressive rollout, freeze-active denial, and rollback idempotency.
...
- Hook 10.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 10.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 10.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 10.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 10.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 10.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 10.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-003-deployment-approval-and-rollback.md` matched [`SLO`].
