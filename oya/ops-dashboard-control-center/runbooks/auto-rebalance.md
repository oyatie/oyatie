---
doc_class: Runbook
doc_id: RUNBOOK-OPS_DASHBOARD_CONTROL_CENTER-AUTO_REBALANCE
...
# Runbook: Ops Dashboard Control Center Auto Rebalance

## Operator Contract
- Runbook id: ops-dashboard-control-center-auto-rebalance.
- Primary service namespace: `ops-dashboard-control-center`.
...
- Owning team: ops-sre-reliability.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ops-dashboard-control-center-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ops-dashboard-control-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n ops-dashboard-control-center rollout status deploy/ops-dashboard-control-center --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service ops-dashboard-control-center --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service ops-dashboard-control-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service ops-dashboard-control-center --cell $CELL --window 30m`.
