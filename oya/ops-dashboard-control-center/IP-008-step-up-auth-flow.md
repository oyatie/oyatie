...
ip_id: IP-008-step-up-auth-flow
status: planned
...
Technical target set: step-up-auth-challenge.yaml, step-up-auth-required.cedar, step-up-auth-latency.openslo.yaml, prod-credential-sidecar.yaml. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `step-up auth challenge` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: step-up-auth-challenge.yaml, step-up-auth-required.cedar, step-up-auth-latency.openslo.yaml, prod-credential-sidecar.yaml.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `step-up auth challenge`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from step-up-auth-challenge.yaml, step-up-auth-required.cedar, step-up-auth-latency.openslo.yaml, prod-credential-sidecar.yaml; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: step-up-auth-challenge.yaml, step-up-auth-required.cedar, step-up-auth-latency.openslo.yaml, prod-credential-sidecar.yaml.
...
|---|---|---|
| Stripe Internal Admin and AWS IAM Identity Center provide the step-up precedent; Oyatie adds hardware-key quorum and emergency-services bypass evidence. | Mature external control surface to compare against. | Stripe Internal Admin and AWS IAM Identity Center provide the step-up precedent; Oyatie adds hardware-key quorum and emergency-services bypass evidence. |
...
1. Issue T2/T3/quorum step-up sessions through OpenBao with explicit TTL and Cedar context injection.
2. Never bypass audit emission, including emergency-services bypass paths.
3. Stripe Internal Admin and AWS IAM Identity Center are step-up counterparts.
4. A11y acceptance is part of the implementation, not a design afterthought.
5. Tests cover passkey, hardware-key quorum, expiry, emergency bypass, and stale session denial.
...
- Hook 15.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 15.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 15.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 15.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 15.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 15.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 15.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-008-step-up-auth-flow.md` matched [`SLO`].
