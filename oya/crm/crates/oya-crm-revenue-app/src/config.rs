...
impl ServiceConfig {
    pub fn local_default(tenant_id: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            observability: ObservabilityConfig {
                service_namespace: "oyatie.crm".to_string(),
                audit_topic: "audit.crm".to_string(),
                metrics_prefix: "oya_crm".to_string(),
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.service_name != "crm" {
            return Err(ServiceError::validation(
                "service_name",
                "service_name must remain crm",
            ));
        }
