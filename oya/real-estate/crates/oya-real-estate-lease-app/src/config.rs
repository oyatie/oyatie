...
impl ServiceConfig {
    pub fn local_default(tenant_id: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            observability: ObservabilityConfig {
                service_namespace: "oyatie.real_estate".to_string(),
                audit_topic: "audit.real_estate".to_string(),
                metrics_prefix: "oya_real_estate".to_string(),
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.service_name != "real-estate" {
            return Err(ServiceError::validation(
                "service_name",
                "service_name must remain real-estate",
            ));
        }
