...
use oya_real_estate_lease_app::domain::{Capability, IdempotencyKey, TenantId};
use oya_real_estate_lease_app::{public_api_surface, scaffold};

#[test]
fn scaffold_declares_expected_contracts() {
    let scaffold = scaffold();
    assert_eq!(scaffold.microservice, "real-estate");
    ...
    assert_eq!(scaffold.contracts.grpc, "contracts/real-estate-v1.proto");
}
...
fn scaffold_declares_domain_capabilities() {
    let scaffold = scaffold();
    assert!(scaffold.capabilities.contains(&Capability::LeaseContract));
    assert!(scaffold.capabilities.contains(&Capability::FacilityMaster));
}
...
fn idempotency_key_requires_stable_length() {
    ...
    assert!(IdempotencyKey::new("real-estate-key-0001").is_ok());
}
