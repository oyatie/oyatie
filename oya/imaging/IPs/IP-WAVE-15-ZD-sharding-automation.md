...
date: 2026-05-21
owner_team: axis-imaging + council-clinical
bounded_context: DICOMSubstrate
...
---
# IP-WAVE-15-ZD-sharding-automation: Imaging Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/imaging/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: imaging must interpret ADR-0348 through its own bounded context: DICOMSubstrate.
SCOPE-004: imaging uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: imaging uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: imaging uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: imaging.
STANCE-002: Owner team: axis-imaging + council-clinical.
...
STANCE-005: Bounded context anchor: DICOMSubstrate.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: HIPAA-2024, GDPR, SOC-2, ISO-27001, EU-AI-Act, EU-MDR, KR-Medical-Devices, GxP, FDA-21-CFR-Part-11, ACR-Accreditation, MQSA-Mammography, EU-MDR-Class-IIa-for-AI-CADe, NEMA-DICOM-PS3-conformance, IHE-Radiology.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for imaging.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for imaging because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; imaging must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where imaging participates.
...
ROLLBACK-002: For imaging, rollback starts from the audit-chain row that recorded the automation event.
