---
doc_class: Runbook
doc_id: RUNBOOK-IMAGING-HOT_SPLIT
...
# Runbook: Imaging Hot Split

## Operator Contract
- Runbook id: imaging-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-imaging-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=imaging; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n imaging rollout status deploy/imaging --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service imaging --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service imaging --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service imaging --cell $CELL --window 30m`.
