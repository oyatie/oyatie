---
doc_class: Runbook
doc_id: RUNBOOK-IMAGING-AUTO_REBALANCE
...
# Runbook: Imaging Auto Rebalance

## Operator Contract
- Runbook id: imaging-auto-rebalance.
- Primary service namespace: `imaging`.
...
- Owning team: axis-imaging + council-clinical.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-imaging-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=imaging; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n imaging rollout status deploy/imaging --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service imaging --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service imaging --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service imaging --cell $CELL --window 30m`.
