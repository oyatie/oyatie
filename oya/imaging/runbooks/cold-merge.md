---
doc_class: Runbook
doc_id: RUNBOOK-IMAGING-COLD_MERGE
...
# Runbook: Imaging Cold Merge

## Operator Contract
- Runbook id: imaging-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-imaging-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=imaging; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n imaging rollout status deploy/imaging --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service imaging --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service imaging --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service imaging --cell $CELL --window 30m`.
