...
# IP-014-branch-protection-and-hyperscaler-gates: branch-protection + hyperscaler-gates HG-SDK registration

## Intent

Register HG-SDK in /specs/hyperscaler-gates.json; add payout-settlement-correctness, kyc-pipeline-correctness, codegen-determinism BLOCKER lanes on dev + staging; register release/developer-sdk/{staging,production} pattern protection. **DEFERRED to parent-wiring-todo per scope-lock; this IP authors the diff preview only.**
...
|---|---|---|
| `evidence/parent-wiring-todo-plugin-app-store-batch.json#hg-sdk-registration` | TODO | HG-SDK row to register |
...
```rust
# scope-locked: diff preview only
```
...
| Test | Verifies |
|---|---|
| `branch-protection-yaml-schema-valid` | After parent-wiring agent applies |
| `hg-sdk-gate-discoverable` | oya gate validate hyperscaler-maturity-claims lists HG-SDK |
...
Test fixtures live under `microservices/developer-sdk/tests/fixtures/ip-014-branch-protection-and-hyperscaler-gates/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.

## Halt Conditions

This IP MUST halt (no merge; no promotion) if any of the following observe:

- Parent-wiring agent applies; CI fails on schema validation.
...
[`IP-015-stripe-connect-parity-end-to-end-drill`](IP-015-stripe-connect-parity-end-to-end-drill.md)
