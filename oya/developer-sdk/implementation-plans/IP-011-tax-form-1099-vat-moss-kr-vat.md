...
impl_plan_id: IP-011-tax-form-1099-vat-moss-kr-vat
status: pending
...
# IP-011-tax-form-1099-vat-moss-kr-vat: Tax-form generation (1099-MISC US + EU VAT MOSS + KR VAT)

## Intent

Generate 1099-MISC for US developers, EU VAT MOSS quarterly returns, KR VAT monthly returns; PDF + structured data; SeaweedFS storage; downloadable in dev portal at year-end.
...
New / modified crates:
- `oya-developer-sdk-tax-form-kernel`
- `oya-developer-sdk-tax-form-domain`
- `oya-developer-sdk-tax-form-usecase`
...
|---|---|---|
| `microservices/developer-sdk/src/crates/oya-developer-sdk-tax-form-domain/src/form_1099_misc.rs` | create | 1099-MISC field population per IRS spec |
| `microservices/developer-sdk/src/crates/oya-developer-sdk-tax-form-domain/src/eu_vat_moss.rs` | create | EU VAT MOSS XML |
| `microservices/developer-sdk/src/crates/oya-developer-sdk-tax-form-domain/src/kr_vat.rs` | create | KR VAT 부가가치세 신고서 |
...
```rust
pub fn emit_1099_misc(dev: &Developer, ytd: &YtdEarnings) -> Form1099MiscPdf {
    Form1099MiscPdf {
        payer_tin: PLATFORM_TIN.to_string(),
        recipient_tin: dev.tax_id.clone(),
        recipient_name: dev.legal_name.clone(),
        box_1_rents: Decimal::ZERO,
        box_2_royalties: Decimal::ZERO,
        box_3_other_income: ytd.payout_total,
        // ... per IRS 2026 spec
    }
}
```
...
```bash
cargo check -p oya-developer-sdk-tax-form-kernel --all-features
cargo build -p oya-developer-sdk-tax-form-kernel --all-features
cargo clippy -p oya-developer-sdk-tax-form-kernel --all-features -- -D warnings
cargo nextest run -p oya-developer-sdk-tax-form-kernel --all-features
cargo deny check --hide-inclusion-graph
cargo doc -p oya-developer-sdk-tax-form-kernel --no-deps
...
| Test | Verifies |
|---|---|
| `test_1099_misc_box_3_matches_payout_total` | Aggregate match |
| `test_1099_misc_irs_schema_valid` | IRS XSD passes |
| `test_eu_vat_moss_xml_valid` | EU MOSS XSD passes |
| `test_kr_vat_format_valid` | NTS Hometax format check |
| `test_tax_form_storage_encrypted_at_rest` | SeaweedFS encryption verified |
...
Test fixtures live under `microservices/developer-sdk/tests/fixtures/ip-011-tax-form-1099-vat-moss-kr-vat/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.

## Halt Conditions

This IP MUST halt (no merge; no promotion) if any of the following observe:

- 1099 box totals disagree with payout ledger.
- EU/KR submission rejected by tax authority.
...
- `ip_id`: `IP-011-tax-form-1099-vat-moss-kr-vat`
...
[`IP-012-package-registry-vendored`](IP-012-package-registry-vendored.md)

## References

- IRS 1099-MISC 2026 spec
- EU VAT MOSS docs
- KR NTS Hometax
