...
impl_plan_id: IP-012-package-registry-vendored
status: pending
...
# IP-012-package-registry-vendored: In-house package registry (vendored npm + cargo + nuget + pypi)

## Intent

Vendor in-house package registries for each SDK family per ADR-0211 (no reliance on npmjs.com / crates.io / nuget.org / pypi.org for first-party SDK distribution). Sonatype Nexus or Verdaccio for npm; cargo-registry; in-house NuGet; bandersnatch-mirrored PyPI.
...
New / modified crates:
- `microservices/developer-sdk/iac/registry/npm`
- `microservices/developer-sdk/iac/registry/cargo`
- `microservices/developer-sdk/iac/registry/nuget`
- `microservices/developer-sdk/iac/registry/pypi`
...
|---|---|---|
| `microservices/developer-sdk/iac/registry/npm/Chart.yaml` | create | Verdaccio Helm chart |
| `microservices/developer-sdk/iac/registry/cargo/Chart.yaml` | create | cargo registry chart |
| `microservices/developer-sdk/iac/registry/nuget/Chart.yaml` | create | BaGet chart |
| `microservices/developer-sdk/iac/registry/pypi/Chart.yaml` | create | pypiserver chart |
...
```rust
# Helm charts per per-registry-substrate pattern
```
...
```bash
cargo check -p microservices/developer-sdk/iac/registry/npm --all-features
cargo build -p microservices/developer-sdk/iac/registry/npm --all-features
cargo clippy -p microservices/developer-sdk/iac/registry/npm --all-features -- -D warnings
...
| Test | Verifies |
|---|---|
| `npm-publish-roundtrip` | Publish + npm install round-trip works |
| `cargo-publish-roundtrip` | Cargo publish + cargo add round-trip works |
| `nuget-publish-roundtrip` | dotnet pack + dotnet add package round-trip |
| `pypi-publish-roundtrip` | twine upload + pip install round-trip |
...
Test fixtures live under `microservices/developer-sdk/tests/fixtures/ip-012-package-registry-vendored/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.

## Halt Conditions

This IP MUST halt (no merge; no promotion) if any of the following observe:

- First-party SDK package published to external registry (ADR-0211 violation).
...
- `ip_id`: `IP-012-package-registry-vendored`
