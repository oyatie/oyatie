---
doc_class: Runbook
doc_id: RUNBOOK-DEVELOPER_SDK-HOT_SPLIT
...
# Runbook: Developer Sdk Hot Split

## Operator Contract
- Runbook id: developer-sdk-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-developer-sdk-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=developer-sdk; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n developer-sdk rollout status deploy/developer-sdk --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service developer-sdk --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service developer-sdk --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service developer-sdk --cell $CELL --window 30m`.
