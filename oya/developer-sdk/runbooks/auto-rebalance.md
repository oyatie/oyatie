---
doc_class: Runbook
doc_id: RUNBOOK-DEVELOPER_SDK-AUTO_REBALANCE
...
# Runbook: Developer Sdk Auto Rebalance

## Operator Contract
- Runbook id: developer-sdk-auto-rebalance.
- Primary service namespace: `developer-sdk`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-developer-sdk-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=developer-sdk; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n developer-sdk rollout status deploy/developer-sdk --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service developer-sdk --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service developer-sdk --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service developer-sdk --cell $CELL --window 30m`.
