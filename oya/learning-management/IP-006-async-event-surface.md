# IP-006 Learning Management async event surface

Service: learning-management
ChangeSet scope: microservices/learning-management/IP-006-async-event-surface.md
...
## A. Problem
This IP closes the async event surface gap for `learning-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `async-event-surface` works for course catalogs, enrollment, learning paths, assessments, credential assertions, and regulated attestations need an auditable learning substrate.
...
The success condition is an implementation plan a cold engineer can trace from this IP to concrete files such as `microservices/learning-management/contracts/asyncapi-v1.yaml` without inventing a hidden service boundary.
...
## C. Deliverables
- D01: `microservices/learning-management/PRD.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D02: `microservices/learning-management/ARCHITECTURE.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D03: `microservices/learning-management/manifest.json` — modify or bind for `async-event-surface` evidence and contract traceability.
- D04: `microservices/learning-management/competitor-parity-matrix.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D05: `microservices/learning-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D06: `microservices/learning-management/contracts/asyncapi-v1.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D07: `microservices/learning-management/contracts/openapi-v1.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D08: `microservices/learning-management/contracts/learning-management-v1.proto` — modify or bind for `async-event-surface` evidence and contract traceability.
- D09: `microservices/learning-management/src/domain/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D10: `microservices/learning-management/src/usecase/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D11: `microservices/learning-management/src/adapter/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D12: `microservices/learning-management/src/config.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D13: `microservices/learning-management/policies/local-course-publish-approval.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D14: `microservices/learning-management/policies/local-cohort-enrollment-scope.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D15: `microservices/learning-management/policies/local-certificate-issue-gate.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D16: `microservices/learning-management/policy/credential-training-authorization.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D17: `microservices/learning-management/capabilities/course-enroll.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D18: `microservices/learning-management/capabilities/completion-seal.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
...
5. Bind adapter behavior through `adapter async boundary` and `microservices/learning-management/src/adapter/mod.rs`, using typed errors from `microservices/learning-management/src/error.rs` instead of stringly provider failures.
...
|---|---|---|
| Workday Learning | Provides a native `async event surface` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `async-event-surface` to tenant scope, Cedar deny evidence, audit-chain records, and flat `learning-management` ownership. |
...
- Do not move `async-event-surface` into a sibling service such as `community` or `workflow-engine` unless a later ADR changes ownership.
