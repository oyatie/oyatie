# Learning Management Runbook: Live Session Join Drop

Service: learning-management  
Surface: local operational primitive suite  
Primary SLO: live-session-join-success  
Domain focus: course, cohort, assessment, content

## Trigger
- Alert `learning-management-live-session-join-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_learning_management_live_session_join_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when live-session-join-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `learning-management-local-live-session-join-success` and policy file name to the ticket.
