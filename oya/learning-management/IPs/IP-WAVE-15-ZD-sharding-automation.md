...
bounded_context: course-catalog
...
---
# IP-WAVE-15-ZD-sharding-automation: Learning Management Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/learning-management/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: learning-management must interpret ADR-0348 through its own bounded context: course-catalog.
SCOPE-004: learning-management uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: learning-management uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: learning-management uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: learning-management.
STANCE-002: Owner team: axis-learning-management + council-product.
...
STANCE-005: Bounded context anchor: course-catalog.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_user.
STANCE-008: Compliance pack input: SOC-2, ISO-27001, GDPR, KR-PIPA, FERPA, KOSA.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for learning-management.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for learning-management because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; learning-management must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where learning-management participates.
...
ROLLBACK-002: For learning-management, rollback starts from the audit-chain row that recorded the automation event.
