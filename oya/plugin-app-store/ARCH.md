...
companion_docs:
  ...
  - microservices/plugin-app-store/README.md
  - microservices/plugin-app-store/ARCHITECTURE.md
---

# Architecture: Plugin App Store

## Architecture Boundary

`plugin-app-store` keeps its existing bounded context and flat `microservices/plugin-app-store/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `plugin-app-store`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `plugin-app-store` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oyatie-as-cloud-provider/k8s-namespace-bootstrap@v1[both], oyatie-as-cloud-provider/secrets-bootstrap@v1[both], oyatie-as-cloud-provider/kms@v1[both], oyatie-as-cloud-provider/vpc@v1[both].
...
- Runtime isolation: pod runtime tier `0` informs module nodepool selection under ADR-0338.
- Capacity: `per_capability` drives CPU `0.25`, RAM `512`, storage `2`, and connection pool variables.
