...
impl_plan_id: IP-011-subscription-billing-aggregation
status: pending
...
# IP-011-subscription-billing-aggregation: subscription state machine + billing aggregation feeding finops-portal

## Intent

Per-plugin subscription state machine (active → paused → cancelled → reactivated); per-tenant billing aggregator emitting one consolidated line-item batch to finops-portal nightly; correctness verified by byte-equal reconciliation against per-event subscription log.
...
New / modified crates:
- `oya-plugin-app-store-subscription-billing-kernel`
- `oya-plugin-app-store-subscription-billing-domain`
- `oya-plugin-app-store-subscription-billing-usecase`
- `oya-plugin-app-store-subscription-billing-adapter-postgres`
- `oya-plugin-app-store-subscription-billing-worker`
...
|---|---|---|
| `microservices/plugin-app-store/src/crates/oya-plugin-app-store-subscription-billing-domain/src/state_machine.rs` | create | Subscription state transitions |
| `microservices/plugin-app-store/src/crates/oya-plugin-app-store-subscription-billing-domain/src/aggregator.rs` | create | pure per-tenant aggregation |
| `microservices/plugin-app-store/src/crates/oya-plugin-app-store-subscription-billing-worker/src/main.rs` | create | nightly aggregator job |
...
```rust
pub fn aggregate(events: &[BillingEvent], tenant_id: &TenantId) -> Aggregate {
    let line_items: BTreeMap<PluginId, Decimal> = events.iter()
        .filter(|e| e.tenant_id == *tenant_id)
        .fold(BTreeMap::new(), |mut acc, e| {
            *acc.entry(e.plugin_id.clone()).or_default() += e.amount;
            acc
        });
    Aggregate { tenant_id: tenant_id.clone(), line_items }
}
```
...
```bash
cargo check -p oya-plugin-app-store-subscription-billing-kernel --all-features
cargo build -p oya-plugin-app-store-subscription-billing-kernel --all-features
cargo clippy -p oya-plugin-app-store-subscription-billing-kernel --all-features -- -D warnings
cargo nextest run -p oya-plugin-app-store-subscription-billing-kernel --all-features
cargo deny check --hide-inclusion-graph
cargo doc -p oya-plugin-app-store-subscription-billing-kernel --no-deps
...
| Test | Verifies |
|---|---|
| `test_aggregation_byte_equal_to_event_log_sum` | Per-tenant total matches event-log sum |
| `test_state_machine_pause_resume` | Pause → resume → active |
| `test_state_machine_cancel_terminal` | Cancel → no further state changes |
| `test_finops_portal_handoff_idempotent` | Re-running aggregator emits identical batch |
...
Test fixtures live under `microservices/plugin-app-store/tests/fixtures/ip-011-subscription-billing-aggregation/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.

## Halt Conditions

This IP MUST halt (no merge; no promotion) if any of the following observe:

- Aggregator total disagrees with event-log sum.
- Subscription transitions out of cancelled state.
...
- `ip_id`: `IP-011-subscription-billing-aggregation`
...
[`IP-012-audit-stream-per-plugin-action`](IP-012-audit-stream-per-plugin-action.md)
