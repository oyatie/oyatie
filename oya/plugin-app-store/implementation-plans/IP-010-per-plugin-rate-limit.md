...
impl_plan_id: IP-010-per-plugin-rate-limit
status: pending
...
# IP-010-per-plugin-rate-limit: per-installation rate-limit (default 100 req/s; per-plugin override)

## Intent

Per-installation token bucket in Valkey; default 100 req/s sustained + 200 burst; per-plugin override path requires admin approval; bypass attempts → audit-logged + 429.
...
```rust
// Lua script (atomic): decrement bucket, return remaining
local bucket = KEYS[1]
...
```bash
cargo check -p oya-plugin-app-store-per-plugin-rate-limit-kernel --all-features
cargo build -p oya-plugin-app-store-per-plugin-rate-limit-kernel --all-features
cargo clippy -p oya-plugin-app-store-per-plugin-rate-limit-kernel --all-features -- -D warnings
cargo nextest run -p oya-plugin-app-store-per-plugin-rate-limit-kernel --all-features
cargo deny check --hide-inclusion-graph
cargo doc -p oya-plugin-app-store-per-plugin-rate-limit-kernel --no-deps
...
| Test | Verifies |
|---|---|
| `test_token_bucket_steady_state` | Sustained rate enforced |
| `test_token_bucket_burst` | Burst capacity respected |
| `test_rate_limit_429_after_exhaustion` | 429 emitted on exhausted bucket |
| `test_override_requires_admin_approval` | Override path Cedar-gated |
| `test_bypass_attempt_audit_logged` | Bypass attempt emits audit-chain seal |
...
Test fixtures live under `microservices/plugin-app-store/tests/fixtures/ip-010-per-plugin-rate-limit/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.

## Halt Conditions

This IP MUST halt (no merge; no promotion) if any of the following observe:

- Rate-limit bypass succeeds without admin approval.
- Bypass attempt missing audit-chain seal.
...
- `ip_id`: `IP-010-per-plugin-rate-limit`
...
[`IP-011-subscription-billing-aggregation`](IP-011-subscription-billing-aggregation.md)

## References

- PRD §per-plugin-rate-limit
