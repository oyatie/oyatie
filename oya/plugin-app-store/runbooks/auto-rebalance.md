---
doc_class: Runbook
doc_id: RUNBOOK-PLUGIN_APP_STORE-AUTO_REBALANCE
...
# Runbook: Plugin App Store Auto Rebalance

## Operator Contract
- Runbook id: plugin-app-store-auto-rebalance.
- Primary service namespace: `plugin-app-store`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plugin-app-store-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plugin-app-store; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plugin-app-store rollout status deploy/plugin-app-store --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plugin-app-store --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service plugin-app-store --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service plugin-app-store --cell $CELL --window 30m`.
