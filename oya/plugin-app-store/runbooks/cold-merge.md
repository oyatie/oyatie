---
doc_class: Runbook
doc_id: RUNBOOK-PLUGIN_APP_STORE-COLD_MERGE
...
# Runbook: Plugin App Store Cold Merge

## Operator Contract
- Runbook id: plugin-app-store-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plugin-app-store-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plugin-app-store; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plugin-app-store rollout status deploy/plugin-app-store --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plugin-app-store --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service plugin-app-store --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service plugin-app-store --cell $CELL --window 30m`.
