...
impl ServiceConfig {
    pub fn local_default(tenant_id: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            observability: ObservabilityConfig {
                service_namespace: "oyatie.supply_chain_planning".to_string(),
                audit_topic: "audit.supply_chain_planning".to_string(),
                metrics_prefix: "oya_supply_chain_planning".to_string(),
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.service_name != "supply-chain-planning" {
            return Err(ServiceError::validation(
                "service_name",
                "service_name must remain supply-chain-planning",
            ));
        }
