...
ip_id: IP-journey-j04-survivor-lockout
journey_id: j04-dv-survivor-shelter-mode
microservice: identity
role: survivor-lockout
status: draft
date: 2026-05-20
related_adrs:
  - ADR-0301
...
# IP - j04 - identity - survivor-lockout

Goal: implement the identity portion of Domestic violence survivor shelter mode so A survivor activates shelter mode and locks an abuser out of shared family account surfaces without alerting the abuser.
Binding ADR: ADR-0301. This IP is one independently reviewable slice and must not weaken the common critical-path ADR pack.

## Scope

In scope: survivor-lockout, JSON Schema validation, Cedar authorization, audit-chain emission, observability, failure branches, and integration tests for j04.
...
|---|---|---|---|
| shelter-mode-activation | identity.survivor-lockout table or event stream | docs/user-journeys/j04-dv-survivor-shelter-mode/schemas/shelter-mode-activation.json | pack-controlled, minimum audit retention |
| abuser-lockout-decision | identity.survivor-lockout table or event stream | docs/user-journeys/j04-dv-survivor-shelter-mode/schemas/abuser-lockout-decision.json | pack-controlled, minimum audit retention |
| safe-contact-route | identity.survivor-lockout table or event stream | docs/user-journeys/j04-dv-survivor-shelter-mode/schemas/safe-contact-route.json | pack-controlled, minimum audit retention |
...
openapi: 3.2.0
info:
  title: identity j04 survivor-lockout
  version: 1.0.0
paths:
  /journeys/j04/identity/survivor-lockout:
    post:
      operationId: j04IdentitySurvivorLockout
...
asyncapi: 3.1.0
info:
  title: identity j04 events
  version: 1.0.0
channels:
  j04.identity.survivor-lockout.accepted:
    address: j04.identity.survivor-lockout.accepted
```
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/identity/IP-journey-j04-survivor-lockout.md` matched `SLO`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/identity/IP-journey-j04-survivor-lockout.md` matched `emission`.
