...
ip_id: IP-journey-j114-dual-context-principal-binding
...
# IP - identity role in j114: Employee secondment cross-tenant
...
identity owns only the dual-context-principal-binding slice for j114. It does not absorb another service responsibility,
does not bypass Cedar, and does not write into another tenant-owned store without an explicit grant.

## Acceptance criteria

1. identity exposes or consumes the typed j114 contract without ad hoc string parsing.
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/identity/IP-journey-j114-dual-context-principal-binding.md` matched `SLO`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/identity/IP-journey-j114-dual-context-principal-binding.md` matched `emission`.
