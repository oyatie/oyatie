---
doc_class: Runbook
doc_id: RUNBOOK-IDENTITY-COLD_MERGE
...
# Runbook: Identity Cold Merge

## Operator Contract
- Runbook id: identity-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-identity-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=identity; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n identity rollout status deploy/identity --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service identity --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service identity --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service identity --cell $CELL --window 30m`.
