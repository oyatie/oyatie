...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=identity-passkey-cross-device-debug --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=identity-passkey-cross-device-debug --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=identity-passkey-cross-device-debug --fixture incident-passkey-cross-device-debug.json`.
