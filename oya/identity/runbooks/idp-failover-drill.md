...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=identity-idp-failover-drill --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=identity-idp-failover-drill --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=identity-idp-failover-drill --fixture incident-idp-failover-drill.json`.
