...
- Audit event class: `EVT-IDENTITY-JWKS_ROTATION-INCIDENT` with ADR-0263 fields `incident_id`, `tenant_id`, `cell_id`, `microservice`, `runbook_id`, `decision_id`, `evidence_hash`, `operator_id`.
- Stop condition: mitigation has held for 30 minutes, IdentityJwksRotationCritical is green, and all handoff APIs in Cross-µservice Coordination return `202 accepted`.
...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=identity-jwks-rotation --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Alertmanager route: `oyatie-identity-jwks-rotation-critical`; silence only with incident commander approval and `EVT-IDENTITY-JWKS_ROTATION-INCIDENT` evidence.
- Synthetic probe: `oya ops probe identity jwks-rotation --cell prod-us-east-1 --tenant synthetic-canary` returns `healthy=true`.
- Drift detector: `registry/identity/jwks-rotation/expected-state.json` hash differs from live `https://identity.internal.oyatie.dev/v1/admin/state-hash`.

## Symptoms
- User-facing impact: jwks rotation blocks or corrupts the identity control path for affected tenants.
- Operators see Grafana panel `jwks-availability / JWKS Rotation burn rate` turn red before the primary alert resolves.
...
- Log signature `audit_emit_status=stalled event_class=EVT-IDENTITY-JWKS_ROTATION-INCIDENT` means mitigation cannot be closed until replay succeeds.

## Diagnostic Steps
1. Set incident variables: `export INCIDENT_ID=INC-identity-jwks-rotation-$(date -u +%Y%m%dT%H%M%SZ); export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Confirm active alerts: `curl -s https://identity.internal.oyatie.dev/v1/alerts?runbook=jwks-rotation | jq .alerts`.
3. Check Kubernetes rollout: `kubectl -n identity rollout status deploy/identity-jwks-rotation-worker --timeout=60s`.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=identity-jwks-rotation --production-snapshot --cell $CELL`.
...
20. Check recent deploy: `kubectl -n identity rollout history deploy/identity-jwks-rotation-worker | tail -20`.
...
```
- Branch A (policy or key mismatch): use the matching mitigation block below and record `decision_branch=A` in `EVT-IDENTITY-JWKS_ROTATION-INCIDENT`.
- Branch B (rollback is safe and bounded): use the matching mitigation block below and record `decision_branch=B` in `EVT-IDENTITY-JWKS_ROTATION-INCIDENT`.
- Branch C (rollback would widen access): use the matching mitigation block below and record `decision_branch=C` in `EVT-IDENTITY-JWKS_ROTATION-INCIDENT`.
- Branch D (manual two-person approval required): use the matching mitigation block below and record `decision_branch=D` in `EVT-IDENTITY-JWKS_ROTATION-INCIDENT`.
...
3. Freeze risky automation: `oya flags set oya.identity.jwks_rotation.incident_hold=true --cell $CELL --tenant $TENANT --reason $INCIDENT_ID`.
4. Enable circuit breaker: `oya ops breaker open identity-jwks-rotation-circuit-breaker --cell $CELL --tenant $TENANT --ttl 30m --reason $INCIDENT_ID`.
5. Reduce blast radius: `kubectl -n identity scale deploy/identity-jwks-rotation-worker --replicas=1`.
6. Protect tenant boundary: `oya tenancy quarantine --tenant $TENANT --reason identity-jwks-rotation --ttl 60m`.
7. Pause promotion: incident hold PR against `dev` (plain `git`; `oya-ci-required` + Buck2 evidence required).
8. Drain queue safely: `oya ops identity jwks-rotation drain --cell $CELL --tenant $TENANT --max-items 500 --dry-run`.
9. Execute bounded drain: `oya ops identity jwks-rotation drain --cell $CELL --tenant $TENANT --max-items 500 --confirm $INCIDENT_ID`.
10. Replay missing audit events: `oya audit-chain replay --event-class EVT-IDENTITY-JWKS_ROTATION-INCIDENT --incident $INCIDENT_ID --from evidence/incidents/$INCIDENT_ID.json`.
11. Rollback last deploy if causal: `kubectl -n identity rollout undo deploy/identity-jwks-rotation-worker`.
12. Raise HPA cap if saturation: `kubectl -n identity patch hpa identity-jwks-rotation-worker --type merge -p '{"spec":{"maxReplicas":12}}'`.
13. Throttle hot tenant: `oya ops rate-limit set --tenant $TENANT --surface identity.jwks-rotation --rps 25 --ttl 30m`.
14. Block abusive principal: `oya identity principal suspend --principal suspected-abuse --tenant $TENANT --reason $INCIDENT_ID`.
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/identity/runbooks/jwks-rotation.md,evidence/incidents/$INCIDENT_ID.json`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=identity-jwks-rotation --fixture incident-jwks-rotation.json`.
...
18. Seal resolution audit: `oya audit-chain emit --event-class EVT-IDENTITY-JWKS_ROTATION-INCIDENT --incident $INCIDENT_ID --field resolution=complete --field runbook=jwks-rotation`.
19. Verify seal: `oya audit-chain verify --event-class EVT-IDENTITY-JWKS_ROTATION-INCIDENT --incident $INCIDENT_ID`.
...
- feature flag oya.identity.jwks_rotation.incident_hold is false for the affected tenant unless long-term hold is approved.
...
microservice: identity
event_class: EVT-IDENTITY-JWKS_ROTATION-INCIDENT
incident_id: <INC-...>
severity: sev2
status: draft
detected_at: <UTC>
...
# JWKS Rotation postmortem

## Summary
- What happened in identity/jwks-rotation.
...
- Emit EVT-IDENTITY-JWKS_ROTATION-INCIDENT with incident_id, tenant_id, cell_id, principal_id, decision_id, evidence_hash, operator_id, runbook_id.
...
- Customer communications: use status page component `oyatie-identity-jwks-rotation` and keep private details in the incident channel.
...
- Notify `tenancy`: `oya incident handoff --target tenancy --source identity --runbook jwks-rotation --incident $INCIDENT_ID --severity sev2 --branch A`; expect `202 accepted`.
- Require `tenancy` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `governance`: `oya incident handoff --target governance --source identity --runbook jwks-rotation --incident $INCIDENT_ID --severity sev2 --branch B`; expect `202 accepted`.
- Require `governance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `compliance`: `oya incident handoff --target compliance --source identity --runbook jwks-rotation --incident $INCIDENT_ID --severity sev2 --branch C`; expect `202 accepted`.
- Require `compliance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `observability`: `oya incident handoff --target observability --source identity --runbook jwks-rotation --incident $INCIDENT_ID --severity sev2 --branch D`; expect `202 accepted`.
- Require `observability` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Observability handoff API: `oya incident handoff --target observability --source identity --runbook jwks-rotation --incident $INCIDENT_ID`.
- Governance handoff API: `oya incident handoff --target governance --source identity --runbook jwks-rotation --incident $INCIDENT_ID`.
- Compliance handoff API: `oya incident handoff --target compliance --source identity --runbook jwks-rotation --incident $INCIDENT_ID`.
- Identity handoff API: `oya incident handoff --target identity --source identity --runbook jwks-rotation --incident $INCIDENT_ID`.
...
- Existing thin runbook topic `jwks-rotation` was preserved as the scenario anchor while replacing generic steps with concrete commands.
