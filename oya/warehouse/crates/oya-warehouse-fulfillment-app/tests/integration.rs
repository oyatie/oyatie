...
use oya_warehouse_fulfillment_app::adapter::AdapterRegistry;
use oya_warehouse_fulfillment_app::config::ServiceConfig;
use oya_warehouse_fulfillment_app::domain::{Capability, IdempotencyKey, TenantId};
use oya_warehouse_fulfillment_app::{public_api_surface, scaffold};

#[test]
fn scaffold_declares_expected_contracts() {
    let scaffold = scaffold();
    assert_eq!(scaffold.microservice, "warehouse");
    ...
    assert_eq!(scaffold.contracts.grpc, "contracts/warehouse-v1.proto");
}
...
fn scaffold_declares_domain_capabilities() {
    let scaffold = scaffold();
    assert!(scaffold.capabilities.contains(&Capability::InboundDelivery));
    assert!(scaffold.capabilities.contains(&Capability::PickingWave));
}
...
fn idempotency_key_requires_stable_length() {
    ...
    assert!(IdempotencyKey::new("warehouse-key-0001").is_ok());
}
