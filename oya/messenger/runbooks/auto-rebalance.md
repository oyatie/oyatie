---
doc_class: Runbook
doc_id: RUNBOOK-MESSENGER-AUTO_REBALANCE
...
# Runbook: Messenger Auto Rebalance

## Operator Contract
- Runbook id: messenger-auto-rebalance.
- Primary service namespace: `messenger`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-messenger-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=messenger; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n messenger rollout status deploy/messenger --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service messenger --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service messenger --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service messenger --cell $CELL --window 30m`.
