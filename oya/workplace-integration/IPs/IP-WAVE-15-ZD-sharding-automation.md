...
SCOPE-006: workplace-integration uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors CI workflow, Helm, and ArgoCD surfaces.
...
STANCE-019: CI/CD substrate stance is ADR-0349 retired build-server bridge plus ArgoCD parity once the rollout wave authors deployment surfaces.
...
ADR346-PURPOSE-001: branch-protected `oya-ci-required` is the live CI authority; former local mirror wording is historical bridge evidence only.
...
ADR346-ENFORCED-BY-001: oya-governance-oya-verify-ci-mirror-coverage (new lane; refuses corpus changes to `shared Rust gate source` that do not invoke cargo fmt + cargo check + cargo clippy + cargo nextest + shared Rust gate matrix by static analysis; promoted to BLOCKER 14 days post Wave 15-ZA implementation lands)
...
ADR346-ENFORCED-BY-004: oya-governance-oya-submit-calls-verify (new lane; refuses changes to protected PR submission that bypass `oya-ci-required` per D-10 -- preserves the existing call chain, refuses regressions)
...
ADR349-PURPOSE-001: GitHub Actions `oya-ci-required` and ArgoCD bridge are the two canonical self-hostable CI/CD substrates for the Oyatie corpus.
ADR349-PURPOSE-002: retired build-server bridge augments rather than replaces GitHub Actions; GitHub Actions remains the hosted PR review CI surface.
...
ADR349-PURPOSE-004: Both substrates are provisioned via OpenTofu modules under `microservices/cloud-iac/modules/<context>/retired-build-server/` and `/argocd/`.
ADR349-PURPOSE-005: Cosign verification, tenant namespace isolation, JCasC-only retired build-server bridge state, and audit-chain deploy emission are enforced by governance lanes.
ADR349-ENFORCED-BY-001: oya-governance-ci-required-parity (new lane; refuses CI workflow / .github/workflows drift such that a CI step exists in one surface but not the other across the per-microservice CI-parity contract enumerated in D-3 below; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
...
VERIFY-025: Downstream implementation must prove CI workflow parity when self-hosted CI lands.
...
ACCEPT-015: The file cites retired build-server bridge plus ArgoCD substrate expectations from ADR-0349.
