...
arch_id: ARCH-workplace-integration
...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/workplace-integration/manifest.json
  - microservices/workplace-integration/PRD.md
  - microservices/workplace-integration/README.md
  - microservices/workplace-integration/ARCHITECTURE.md
---

# Architecture: Workplace Integration

## Architecture Boundary

`workplace-integration` keeps its existing bounded context and flat `microservices/workplace-integration/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
### ADR-0346 Local CI Mirror (superseded authority)
- Historical local verifier/gate CLI doctrine is superseded by ADR-0513 for CI and merge authority.
- Future architecture changes must collect Buck2 evidence and enter the Rust/Prow Kubernetes-native `oya-ci-required` controller path; local commands remain advisory unless represented by Buck2/Prow evidence.
- Reusable verifier logic should live as Rust libraries, Buck2 targets, and Prow jobs rather than a revived CLI gate.
...
### ADR-0349 CI/CD Context (superseded authority)
- Jenkins and ArgoCD are historical context only and are not interim CI/CD authorities for this microservice.
- Durable direction is Kubernetes-native oya-ci with Prow-style jobs for CI and release-conveyor-like promotion/deployment seams for CD.
- GitHub and GitHub Actions remain temporary PR/publication and shadow-evidence adapters while native SCM/CI/CD matures; they must not become first-class durable authority.

## ADR-0339 integration
- Integration state: PROPOSED for `workplace-integration`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `workplace-integration` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/tenant-namespace@v1[both], aws-guest/postgres-wal-g@v1[both], colo/audit-chain-merkle-seal@v1[both], on-prem/openbao-policy@v1[both], oyatie-as-cloud-provider/object-storage-versioned@v1[both].
...
- Capacity: `per_workflow_run` drives CPU `0.09`, RAM `224`, storage `5`, and connection pool variables.
