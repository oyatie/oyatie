...
## Doctrine Anchors
- ADR-0346 current authority: branch-protected `oya-ci-required` is the live CI authority; former local mirror wording is historical bridge evidence only and must not become a runbook step.
...
- ADR-0349 purpose wording: GitHub Actions `oya-ci-required` and ArgoCD bridge are the two canonical self-hostable CI/CD substrates for the Oyatie corpus.
- ADR-0349 enforced_by lanes: `oya-governance-ci-required-parity`; `oya-governance-argocd-application-cosign-verified`; `oya-governance-argocd-tenant-namespace-isolation`; `oya-governance-declarative-ci-config-only`; `oya-governance-deploy-audit-chain-emit`.
...
- Trigger 7: retired build-server bridge or GitHub Actions parity drift blocks the release train for the sharding automation lane.
...
3. Verify alerts through the observability API for service `workplace-integration`, runbook `cold-merge`, and the last 30 minutes.
...
8. Verify audit-chain availability through the control-plane health endpoint for the selected cell and tenant cohort.
9. Verify GitOps Application sync health from controller status; local ArgoCD commands are diagnostic only.
10. Verify retired build-server bridge/GitHub Actions parity evidence exists for this service before declaring the runbook complete.
...
6. If retired build-server bridge/GitHub Actions parity is unknown, keep the change in report-only state until branch-protected `oya-ci-required` evidence is available.
...
- Evidence 7: retired build-server bridge build id or GitHub Actions run id proving CI/CD parity for this service.
- Evidence 8: `oya-ci-required` local mirror result before any push related to this runbook.
...
6. Confirm retired build-server bridge and GitHub Actions parity evidence is attached per ADR-0349.
