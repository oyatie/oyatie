...
- Trigger from CI when the trusted Rust/Prow `oya-ci-required` controller reports a non-zero production-snapshot validation for `workplace-integration-engagement-agreement-dual-signature` against the latest production evidence bundle.
...
15. Confirm production-snapshot evidence for `workplace-integration-engagement-agreement-dual-signature` in `$CELL` through the trusted Rust/Prow `oya-ci-required` controller and attach the Buck2 artifact reference.
...
7. Attach Buck2/Prow fixture evidence for `workplace-integration-engagement-agreement-dual-signature` using `incident-engagement-agreement-dual-signature.json` in the `oya-ci-required` evidence bundle.
