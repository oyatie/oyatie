...
- Current SSOT chain (`registry/stores/design-store.json`, `registry/stores/instructions-store.json`, `specs/masterplan.json`, and `specs/root-hub-pointers.json`): branch-protected `oya-ci-required` is the live merge authority; local command output is transition evidence only and must not become destination enforcement.
- [ADR-0346](../../docs/decisions/ADR-0346-oya-verify-must-run-full-ci-mirror.md): retained as historical/bridge context only where it does not conflict with the current SSOT cloud-native CI authority.
- [ADR-0347](../../docs/decisions/ADR-0347-governance-fitness-bulk-rename.md): Every `oya-governance-*` CI lane prefix RENAMES to `oya-governance-*` in one Wave 15-ZB bulk-rename pull request rather than 34 per-lane migration IPs. Enforced by `oya-governance-retired-vocab-residue`, `oya-governance-lane-prefix-vocabulary`, and `oya-governance-rename-inventory-presence`.
