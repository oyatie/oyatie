...
## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/community/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-005: community uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: community uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
