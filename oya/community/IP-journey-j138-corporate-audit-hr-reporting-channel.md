...
ip_id: IP-journey-j138-community-hr-reporting-channel
journey_id: j138-corporate-audit-fraud-investigation-via-pattern-detection
microservice: community
role: hr-reporting-channel
status: draft
date: 2026-05-20
authority_tier: 3
owner_team: axis-community + axis-internal-audit + axis-hr
parallel_work_compatibility: depends on identity B2B_HR_ADMIN permit resolution
...
# IP - j138-corporate-audit-fraud-investigation-via-pattern-detection - community - hr-reporting-channel

## A. Intent

This implementation plan binds the `community` slice of `j138-corporate-audit-fraud-investigation-via-pattern-detection` to real community contracts, Cedar fragments, event channels, proto3 services, SLOs, and counterpart behavior. The previous generated row loop has been consolidated: only rows with a grounded community action remain, and speculative rows without a backing artifact are removed rather than expanded.
...
|---:|---|---|---|---|---|
| 1 | List or create the scoped community space for `hr-reporting-channel`. | Trigger: journey entry or tenant navigation; actor: `TenantMember` or `AnonymousPrincipal` for public spaces. | Reads `SpaceList` or creates postable space state under tenant_id. | OpenAPI `listSpaces` in `microservices/community/contracts/openapi/community.yaml` and `microservices/community/policy/tenant-scope.cedar`. | Matches Discourse category access scoped by group/tenant. |
...
- Surface evidence: `microservices/community/IP-journey-j138-corporate-audit-hr-reporting-channel.md` matched `openapi, asyncapi, .proto`; contract files `microservices/community/contracts/openapi/community.yaml, microservices/community/contracts/asyncapi/community-events.yaml, microservices/community/contracts/proto/community.proto`; type anchor `microservices/community/manifest.json`.
...
- Surface evidence: `microservices/community/IP-journey-j138-corporate-audit-hr-reporting-channel.md` matched `SLO, payment`; anchors `microservices/community/manifest.json`; type anchor `microservices/community/manifest.json`.
