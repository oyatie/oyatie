...
## Trigger Conditions
- Page on alert `ModeratorDecisionAppealProtocolCritical` when `oya_community_moderator_decision_appeal_protocol_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `ModeratorDecisionAppealProtocolSloBurn` when `oya_community_moderator_decision_appeal_protocol_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open sev2 if `oya_community_moderator_appeal_sla_seconds` exceeds the threshold documented in `microservices/community/slos/audit-chain-seal-latency.openslo.yaml`.
- Open sev2 if `oya_community_moderator_decision_appeal_protocol_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=community-moderator-decision-appeal-protocol --production-snapshot` exits non-zero against the latest production evidence bundle.
- Primary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/moderator-decision-appeal-protocol?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=101` backed by `microservices/community/dashboards/moderation-queue-depth.json`.
- Secondary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/moderator-decision-appeal-protocol?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=202` backed by `microservices/community/dashboards/post-throughput.json`.
...
- Service-specific metric `oya_community_moderator_appeal_sla_seconds` is red while `oya_community_moderator_decision_appeal_protocol_audit_emit_total{status="sealed"}` is flat.
...
- Metric pattern: `oya_community_moderator_decision_appeal_protocol_error_ratio` rises before `oya_community_moderator_decision_appeal_protocol_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_community_moderator_decision_appeal_protocol_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_community_moderator_decision_appeal_protocol_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific pattern: `oya_community_moderator_appeal_sla_seconds` rises while `oya_community_moderator_decision_appeal_protocol_dependency_error_ratio` is flat; inspect local state before escalating Cloudflare Trust and Safety.
- Service-specific pattern: `oya_community_moderator_decision_appeal_protocol_dependency_error_ratio` rises while `oya_community_moderator_appeal_sla_seconds` is flat; inspect vendor or adjacent-service dependency health before local rollback.
...
- Failure mode 7: regional partition; keep prod-us-east-1 as evidence leader and reject cross-region mutation until `oya_community_moderator_decision_appeal_protocol_state_hash_match == 1`.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_community_moderator_decision_appeal_protocol_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_community_moderator_decision_appeal_protocol_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_community_moderator_decision_appeal_protocol_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_community_moderator_appeal_sla_seconds{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=community-moderator-decision-appeal-protocol --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p oya-community-moderation-queue-domain moderator_decision_appeal_protocol -- --nocapture`.
17. Check API contract smoke: `curl -s https://community.internal.oyatie.dev/v1/community/moderator-decision-appeal-protocol/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
25. Run targeted SQL state query: `psql $OYA_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from community_moderator_decision_appeal_protocol_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric oya_community_moderator_decision_appeal_protocol_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_community_moderator_decision_appeal_protocol_queue_depth grow while oya_community_moderator_decision_appeal_protocol_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/community/runbooks/moderator-decision-appeal-protocol.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://community.internal.oyatie.dev/v1/community/moderator-decision-appeal-protocol/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: confirmed CommunityThread correctness risk.
  - Required action: keep `community-moderator-decision-appeal-protocol-circuit-breaker` open until `oya_community_moderator_decision_appeal_protocol_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: dependency saturation or replay backlog.
  - Required action: keep `community-moderator-decision-appeal-protocol-circuit-breaker` open until `oya_community_moderator_decision_appeal_protocol_error_ratio` is below 0.01 for 3 evaluator windows.
...
## Resolution Steps
1. Identify code owner path: `rg "moderator_decision_appeal_protocol|ModeratorDecisionAppealProtocolCritical|community.moderator_decision_appeal_protocol.incident_state" crates microservices/community -g "!microservices/community/runbooks/**"`.
2. Patch domain invariant: `edit oya-community-moderation-queue-domain where moderator_decision_appeal_protocol state transition is validated`.
...
6. Add regression test: `cargo test -p oya-community-moderation-queue-domain moderator_decision_appeal_protocol_incident_regression -- --nocapture`.
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=community-moderator-decision-appeal-protocol --fixture incident-moderator-decision-appeal-protocol.json`.
8. Add SLO assertion: `update microservices/community/slos/audit-chain-seal-latency.openslo.yaml with alert ModeratorDecisionAppealProtocolCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/community/dashboards/moderation-queue-depth.json with oya_community_moderator_decision_appeal_protocol_error_ratio, oya_community_moderator_decision_appeal_protocol_lag_seconds, and oya_community_moderator_appeal_sla_seconds`.
...
14. Watch burn rate: `oya ops watch --metric oya_community_moderator_decision_appeal_protocol_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `oya-community-moderation-queue-domain`: inspect for `moderator_decision_appeal_protocol` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-community-post-store-worker`: inspect for `moderator_decision_appeal_protocol` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-community-voting-engine-domain`: inspect for `moderator_decision_appeal_protocol` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `oya-community-search-index-worker`: inspect for `moderator_decision_appeal_protocol` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `microservices/community/dashboards/moderation-queue-depth.json`: verify panel coverage for `oya_community_moderator_decision_appeal_protocol_error_ratio`, `oya_community_moderator_decision_appeal_protocol_lag_seconds`, and `oya_community_moderator_appeal_sla_seconds`.
...
- `oya_community_moderator_decision_appeal_protocol_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `oya_community_moderator_decision_appeal_protocol_lag_seconds < 120` for all production cells.
- `oya_community_moderator_decision_appeal_protocol_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `oya_community_moderator_appeal_sla_seconds` is below the threshold documented in `microservices/community/slos/audit-chain-seal-latency.openslo.yaml`.
...
- Capacity math: if `oya_community_moderator_decision_appeal_protocol_queue_depth` is 5000 and the worker drains 25 items/second, the best-case drain is 200 seconds before retries; page earlier when drain time exceeds 300 seconds.
...
- Rollback checkpoint 2: before opening `community-moderator-decision-appeal-protocol-circuit-breaker`, capture `oya_community_moderator_decision_appeal_protocol_request_rate` and `oya_community_moderator_decision_appeal_protocol_success_ratio` from Mimir.
...
## Handoff Notes
- Do not hand off with only the alert name; include `oya_community_moderator_decision_appeal_protocol_error_ratio`, `oya_community_moderator_decision_appeal_protocol_lag_seconds`, `oya_community_moderator_decision_appeal_protocol_queue_depth`, `oya_community_moderator_appeal_sla_seconds`, current breaker state, and audit seal status.
...
- `microservices/community/manifest.json` for owner, dependency, capability, and bounded-context vocabulary; topic `moderator-decision-appeal-protocol` is the scenario anchor.

## Checkpoint Closure Criteria
- The runbook remains current when `ModeratorDecisionAppealProtocolCritical`, `ModeratorDecisionAppealProtocolSloBurn`, `oya_community_moderator_appeal_sla_seconds`, `oya.community.moderator_decision_appeal_protocol.incident_hold`, and `community-moderator-decision-appeal-protocol-circuit-breaker` all resolve to live telemetry, flag, or breaker records.
