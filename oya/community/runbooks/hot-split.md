---
doc_class: Runbook
doc_id: RUNBOOK-COMMUNITY-HOT_SPLIT
...
# Runbook: Community Hot Split

## Operator Contract
- Runbook id: community-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-community-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=community; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n community rollout status deploy/community --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service community --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service community --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service community --cell $CELL --window 30m`.
