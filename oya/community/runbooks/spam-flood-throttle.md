...
- API authority: `https://community.internal.oyatie.dev/v1/community/spam-flood-throttle/incident-handoff`.
- Audit event class: `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT` with ADR-0263 fields `incident_id`, `tenant_id`, `cell_id`, `microservice`, `runbook_id`, `decision_id`, `evidence_hash`, `operator_id`.
- Stop condition: mitigation has held for 30 minutes, `SpamFloodThrottleCritical` is green, and every Cross-microservice handoff API returns `202 accepted`.
...
- Open sev2 if `oya_community_spam_flood_rate` exceeds the threshold documented in `microservices/community/slos/audit-chain-seal-latency.openslo.yaml`.
...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=community-spam-flood-throttle --production-snapshot` exits non-zero against the latest production evidence bundle.
- Primary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/spam-flood-throttle?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=101` backed by `microservices/community/dashboards/moderation-queue-depth.json`.
- Secondary dashboard: `https://grafana.dev.oyatie.internal/d/community-ops/spam-flood-throttle?orgId=1&var-cell=prod-us-east-1&var-pack=canonical-base&viewPanel=202` backed by `microservices/community/dashboards/post-throughput.json`.
...
- Operators see Grafana panel `moderation-queue-depth.json / Spam Flood Throttle burn rate` turn red before the primary alert resolves.
...
## Diagnostic Steps
1. Set incident variables: `export INCIDENT_ID=INC-community-spam-flood-throttle-$(date -u +%Y%m%dT%H%M%SZ); export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Confirm active alerts: `curl -s https://community.internal.oyatie.dev/v1/community/alerts?runbook=spam-flood-throttle | jq .alerts`.
3. Check Kubernetes rollout: `kubectl -n community rollout status deploy/community-spam-flood-throttle-worker --timeout=60s`.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=community-spam-flood-throttle --production-snapshot --cell $CELL`.
...
21. Check recent deploy: `kubectl -n community rollout history deploy/community-spam-flood-throttle-worker | tail -20`.
...
```
- Branch A (confirmed CommunityThread correctness risk): use the matching mitigation block below and record `decision_branch=A` in `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT`.
- Branch B (dependency saturation or replay backlog): use the matching mitigation block below and record `decision_branch=B` in `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT`.
- Branch C (policy, permit, or tenant-scope drift): use the matching mitigation block below and record `decision_branch=C` in `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT`.
- Branch D (customer-visible or regulated evidence gap): use the matching mitigation block below and record `decision_branch=D` in `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT`.
...
3. Freeze risky automation: `oya flags set oya.community.spam_flood_throttle.incident_hold=true --cell $CELL --tenant $TENANT --reason $INCIDENT_ID`.
4. Enable circuit breaker: `oya ops breaker open community-spam-flood-throttle-circuit-breaker --cell $CELL --tenant $TENANT --ttl 30m --reason $INCIDENT_ID`.
5. Reduce blast radius: `kubectl -n community scale deploy/community-spam-flood-throttle-worker --replicas=1`.
6. Protect tenant boundary: `oya tenancy quarantine --tenant $TENANT --reason community-spam-flood-throttle --ttl 60m`.
7. Pause promotion: incident hold PR against `dev` (plain `git`; `oya-ci-required` + Buck2 evidence required).
8. Drain queue safely: `oya ops community spam-flood-throttle drain --cell $CELL --tenant $TENANT --max-items 500 --dry-run`.
9. Execute bounded drain: `oya ops community spam-flood-throttle drain --cell $CELL --tenant $TENANT --max-items 500 --confirm $INCIDENT_ID`.
10. Replay missing audit events: `oya audit-chain replay --event-class EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT --incident $INCIDENT_ID --from evidence/incidents/$INCIDENT_ID.json`.
11. Rollback last deploy if causal: `kubectl -n community rollout undo deploy/community-spam-flood-throttle-worker`.
12. Raise HPA cap if saturation is proven: `kubectl -n community patch hpa community-spam-flood-throttle-worker --type merge -p '{"spec":{"maxReplicas":12}}'`.
13. Throttle hot tenant: `oya ops rate-limit set --tenant $TENANT --surface community.spam-flood-throttle --rps 25 --ttl 30m`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/community/runbooks/spam-flood-throttle.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "spam_flood_throttle|SpamFloodThrottleCritical|community.spam_flood_throttle.incident_state" crates microservices/community -g "!microservices/community/runbooks/**"`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=community-spam-flood-throttle --fixture incident-spam-flood-throttle.json`.
8. Add SLO assertion: `update microservices/community/slos/audit-chain-seal-latency.openslo.yaml with alert SpamFloodThrottleCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/community/dashboards/moderation-queue-depth.json with oya_community_spam_flood_throttle_error_ratio, oya_community_spam_flood_throttle_lag_seconds, and oya_community_spam_flood_rate`.
...
18. Seal resolution audit: `oya audit-chain emit --event-class EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT --incident $INCIDENT_ID --field resolution=complete --field runbook=spam-flood-throttle`.
19. Verify seal: `oya audit-chain verify --event-class EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT --incident $INCIDENT_ID`.
...
- `microservices/community/dashboards/moderation-queue-depth.json`: verify panel coverage for `oya_community_spam_flood_throttle_error_ratio`, `oya_community_spam_flood_throttle_lag_seconds`, and `oya_community_spam_flood_rate`.
...
- Service-specific signal `oya_community_spam_flood_rate` is below the threshold documented in `microservices/community/slos/audit-chain-seal-latency.openslo.yaml`.
...
- Feature flag `oya.community.spam_flood_throttle.incident_hold` is false for the affected tenant unless long-term hold is approved.
...
- Rollback command for deployment: `kubectl -n community rollout undo deploy/community-spam-flood-throttle-worker`.
- Rollback command for tenant throttle: `oya ops rate-limit clear --tenant $TENANT --surface community.spam-flood-throttle --reason rollback-$INCIDENT_ID`.
- Stop rollback if `EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT` cannot be emitted; preserve the current state and escalate to audit-chain before additional mutation.
...
runbook_id: community-spam-flood-throttle
microservice: community
event_class: EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT
incident_id: <INC-...>
severity: sev2
status: draft
detected_at: <UTC>
...
# Spam Flood Throttle postmortem

## Summary
- What happened in community/spam-flood-throttle.
...
- Emit EVT_COMMUNITY_SPAM_FLOOD_THROTTLE_INCIDENT with incident_id, tenant_id, cell_id, principal_id, decision_id, evidence_hash, operator_id, runbook_id.
...
- Customer communications: use status page component `oyatie-community-spam-flood-throttle` and keep private details in the incident channel.
...
- Notify `identity`: `oya incident handoff --target identity --source community --runbook spam-flood-throttle --incident $INCIDENT_ID --severity sev2 --branch A`; expect `202 accepted`.
- Require `identity` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `tenancy`: `oya incident handoff --target tenancy --source community --runbook spam-flood-throttle --incident $INCIDENT_ID --severity sev2 --branch B`; expect `202 accepted`.
- Require `tenancy` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `compliance`: `oya incident handoff --target compliance --source community --runbook spam-flood-throttle --incident $INCIDENT_ID --severity sev2 --branch C`; expect `202 accepted`.
- Require `compliance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `audit-chain`: `oya incident handoff --target audit-chain --source community --runbook spam-flood-throttle --incident $INCIDENT_ID --severity sev2 --branch D`; expect `202 accepted`.
- Require `audit-chain` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `observability`: `oya incident handoff --target observability --source community --runbook spam-flood-throttle --incident $INCIDENT_ID --severity sev2 --branch A`; expect `202 accepted`.
- Require `observability` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Observability handoff API: `oya incident handoff --target observability --source community --runbook spam-flood-throttle --incident $INCIDENT_ID`.
- Governance handoff API: `oya incident handoff --target governance --source community --runbook spam-flood-throttle --incident $INCIDENT_ID`.
- Compliance handoff API: `oya incident handoff --target compliance --source community --runbook spam-flood-throttle --incident $INCIDENT_ID`.
- Audit-chain handoff API: `oya incident handoff --target audit-chain --source community --runbook spam-flood-throttle --incident $INCIDENT_ID`.
...
- `microservices/community/manifest.json` for owner, dependency, capability, and bounded-context vocabulary; topic `spam-flood-throttle` is the scenario anchor.
