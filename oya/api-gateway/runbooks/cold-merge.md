---
doc_class: Runbook
doc_id: RUNBOOK-API_GATEWAY-COLD_MERGE
...
# Runbook: Api Gateway Cold Merge

## Operator Contract
- Runbook id: api-gateway-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-api-gateway-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=api-gateway; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n api-gateway rollout status deploy/api-gateway --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service api-gateway --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service api-gateway --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service api-gateway --cell $CELL --window 30m`.
