---
doc_class: Runbook
doc_id: RUNBOOK-API_GATEWAY-HOT_SPLIT
...
# Runbook: Api Gateway Hot Split

## Operator Contract
- Runbook id: api-gateway-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-api-gateway-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=api-gateway; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n api-gateway rollout status deploy/api-gateway --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service api-gateway --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service api-gateway --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service api-gateway --cell $CELL --window 30m`.
