---
doc_class: Runbook
doc_id: RUNBOOK-DESIGN_COLLABORATION-AUTO_REBALANCE
...
# Runbook: Design Collaboration Auto Rebalance

## Operator Contract
- Runbook id: design-collaboration-auto-rebalance.
- Primary service namespace: `design-collaboration`.
...
- Owning team: axis-design-collaboration + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-design-collaboration-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=design-collaboration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n design-collaboration rollout status deploy/design-collaboration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service design-collaboration --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service design-collaboration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service design-collaboration --cell $CELL --window 30m`.
