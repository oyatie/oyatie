---
doc_class: Runbook
doc_id: RUNBOOK-DESIGN_COLLABORATION-HOT_SPLIT
...
# Runbook: Design Collaboration Hot Split

## Operator Contract
- Runbook id: design-collaboration-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-design-collaboration-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=design-collaboration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n design-collaboration rollout status deploy/design-collaboration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service design-collaboration --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service design-collaboration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service design-collaboration --cell $CELL --window 30m`.
