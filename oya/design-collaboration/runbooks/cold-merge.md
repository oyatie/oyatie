---
doc_class: Runbook
doc_id: RUNBOOK-DESIGN_COLLABORATION-COLD_MERGE
...
# Runbook: Design Collaboration Cold Merge

## Operator Contract
- Runbook id: design-collaboration-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-design-collaboration-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=design-collaboration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n design-collaboration rollout status deploy/design-collaboration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service design-collaboration --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service design-collaboration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service design-collaboration --cell $CELL --window 30m`.
