...
ip_id: IP-journey-j130-whistleblower-protection-pack
journey_id: j130-auditor-receives-bribery-attempt-via-personal-messenger
microservice: compliance
role: whistleblower-protection-pack
status: draft
related_adrs:
...
# IP-journey-j130-whistleblower-protection-pack — Compliance µservice: whistleblower-protection pack with US + EU overlays

## Goal

Implement compliance pack:

1. **`pack-global-whistleblower-baseline`** — anonymity + scope-bounded
   evidence + audit-chain seal requirements.
2. **`pack-us-dodd-frank-section-922`** — SEC §922 financial reward
   structure + protection.
3. **`pack-us-sarbanes-oxley-806`** — SOX 806 publicly-traded company
   employee protection.
4. **`pack-eu-whistleblower-directive-2019-1937`** — EU per-member-state
   transposition + cross-border protection.

## Files to author

| File | Purpose | Lines |
|---|---|---:|
| `microservices/compliance/packs/pack-global-whistleblower-baseline/manifest.yaml` | Pack | ~120 |
| `microservices/compliance/packs/pack-us-dodd-frank-section-922/manifest.yaml` | Pack | ~100 |
| `microservices/compliance/packs/pack-us-sarbanes-oxley-806/manifest.yaml` | Pack | ~100 |
| `microservices/compliance/packs/pack-eu-whistleblower-directive-2019-1937/manifest.yaml` | Pack | ~120 |
| `microservices/compliance/packs/pack-global-whistleblower-baseline/controls/anonymity.yaml` | Control | ~80 |
| `microservices/compliance/packs/pack-global-whistleblower-baseline/controls/retaliation-protection.yaml` | Control | ~80 |
| `microservices/compliance/packs/pack-us-dodd-frank-section-922/controls/financial-reward.yaml` | Control | ~100 |
| `microservices/compliance/policy/whistleblower-pack-apply.cedar` | Cedar permit | ~30 |
| `microservices/compliance/runbooks/whistleblower-pack-update.md` | Runbook | ~160 |
| `microservices/compliance/tests/integration/whistleblower_pack_test.rs` | Tests | ~340 |
| `microservices/compliance/dashboards/whistleblower-pack-status.json` | Grafana | ~80 |
| `microservices/compliance/slos/whistleblower-pack-pull-latency.openslo.yaml` | SLO | ~40 |
...
```cedar
// whistleblower-pack-apply.cedar
permit (
  principal == Service::"compliance-pack-loader",
  action == Action::"compliance.apply_whistleblower_pack",
  resource is Tenant
);
```
...
| `community.GetApplicableAuthorities` | community → compliance | Per-pack authority resolution |
...
- Authority routing per jurisdiction returns correct authorities

## Observability emissions

- `oya_compliance_whistleblower_pack_active_tenants_gauge`
- `oya_compliance_whistleblower_pack_validation_total{outcome}`

## Acceptance criteria

- 4 packs validate.
- Per-jurisdiction authority resolution works.

## Cross-references

- `docs/user-journeys/j130-*/handshake.md` §2
- ADR-0300, ADR-0251

## Completion expansion — j130 compliance IP rigor pass
...
Implementation task 001: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 001: add property coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 001: document observability impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 002: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 003: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 004: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 004: add replay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 004: document optimization impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 005: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 006: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 006: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 006: document security impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 007: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 008: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 009: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 010: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 011: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 012: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 013: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 014: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 015: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 016: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 016: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 016: document security impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 017: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 018: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 019: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 020: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 021: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 022: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 023: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 024: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 025: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 026: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 026: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 026: document security impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 027: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 028: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 029: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 029: add pack-overlay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 029: document rollback impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 030: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 031: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 031: add property coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 031: document observability impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 032: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 033: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 034: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 034: add replay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 034: document optimization impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 035: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 036: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 036: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 036: document security impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 037: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 038: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 039: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 039: add pack-overlay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
Rigor 039: document rollback impact, capacity bound, rollback behavior, SLO signal, and operator evidence needed for an intern to implement safely.
Implementation task 040: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 041: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 042: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 043: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 044: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 045: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 046: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 046: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 047: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 048: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 049: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 050: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 051: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 051: add property coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 052: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 053: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 054: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 055: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 056: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 057: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 058: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 059: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 059: add pack-overlay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 060: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 061: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 061: add property coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 062: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 063: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 064: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 064: add replay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 065: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 066: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 066: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 067: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 068: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 069: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Verification 069: add pack-overlay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 070: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 071: in compliance, define the Cedar policy change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 072: in compliance, define the OpenAPI 3.2.0 contract change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 073: in compliance, define the AsyncAPI 3.1.0 event change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 073: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0297 scope grants it; refusals are success states, not exceptions.
...
Implementation task 074: in compliance, define the proto3 port change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 074: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0299 scope grants it; refusals are success states, not exceptions.
Verification 074: add replay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 075: in compliance, define the Postgres/RLS storage change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 075: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0311 scope grants it; refusals are success states, not exceptions.
...
Implementation task 076: in compliance, define the audit-chain emission change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 076: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0312 scope grants it; refusals are success states, not exceptions.
Verification 076: add chaos coverage proving compliance and community agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 077: in compliance, define the dashboard projection change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 078: in compliance, define the runbook hook change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
...
Implementation task 079: in compliance, define the integration fixture change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 079: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0297 scope grants it; refusals are success states, not exceptions.
Verification 079: add pack-overlay coverage proving compliance and identity agree on contract version, Cedar decision, audit event class, and replay cursor.
...
Implementation task 080: in compliance, define the domain model change for personal Messenger bribery attempt reported through whistleblower community path; inputs include tenant_id, actor_id, audience_type, purpose, case_id, idempotency_key, and traceparent.
Boundary rule 080: compliance MUST refuse cross-tenant or personal-surface access unless an explicit ADR-0299 scope grants it; refusals are success states, not exceptions.
...
- Surface evidence: `microservices/compliance/IP-journey-j130-whistleblower-protection-pack.md` matched `p99, SLO, multi-region, financial`; anchors `microservices/compliance/runbooks/phi-access-anomaly.md, crates/oya-shared-compliance-evidence-kernel/src/lib.rs`; type anchor `crates/oya-shared-compliance-evidence-kernel/src/lib.rs::EvidenceArtifact`.

## Sustainability emission (per ADR-0344)
- Per-call audit row emission: populate `cost_usd_minor_units`, `co2_grams`, and `watt_hours` with provider and region on every audit-chain row.
- Carbon-aware scheduling eligibility: opt-in only; do not defer Tier 0/1 workloads or realtime-mandated compliance-pack workloads (`eu-ai-act-annex-iii`, `hipaa-em-incident-response`, `pci-dss-realtime-fraud-detection`).
- finops-portal rollup axes affected: tenant / product / capability / provider / cell / compliance_pack.
- Surface evidence: `microservices/compliance/IP-journey-j130-whistleblower-protection-pack.md` matched `emission`; anchors `microservices/compliance/manifest.json, crates/oya-shared-compliance-evidence-kernel/src/lib.rs`; type anchor `crates/oya-shared-compliance-evidence-kernel/src/lib.rs::EvidenceArtifact`.
