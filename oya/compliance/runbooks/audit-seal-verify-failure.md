...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=compliance-audit-seal-verify-failure --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=compliance-audit-seal-verify-failure --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=compliance-audit-seal-verify-failure --fixture incident-audit-seal-verify-failure.json`.
