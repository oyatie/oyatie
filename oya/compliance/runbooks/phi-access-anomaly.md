...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=compliance-phi-access-anomaly --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=compliance-phi-access-anomaly --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=compliance-phi-access-anomaly --fixture incident-phi-access-anomaly.json`.
...
9. Add dashboard panel: `update microservices/compliance/dashboards/audit-chain-seal-health.json with oya_compliance_phi_access_anomaly_error_ratio, oya_compliance_phi_access_anomaly_lag_seconds, and oya_compliance_phi_access_anomaly_queue_depth`.
