---
doc_class: Runbook
doc_id: RUNBOOK-WHITEBOARD-COLD_MERGE
...
# Runbook: Whiteboard Cold Merge

## Operator Contract
- Runbook id: whiteboard-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-whiteboard-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=whiteboard; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n whiteboard rollout status deploy/whiteboard --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service whiteboard --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service whiteboard --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service whiteboard --cell $CELL --window 30m`.
