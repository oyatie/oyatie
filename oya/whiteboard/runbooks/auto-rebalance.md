---
doc_class: Runbook
doc_id: RUNBOOK-WHITEBOARD-AUTO_REBALANCE
...
# Runbook: Whiteboard Auto Rebalance

## Operator Contract
- Runbook id: whiteboard-auto-rebalance.
- Primary service namespace: `whiteboard`.
...
- Owning team: axis-whiteboard + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-whiteboard-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=whiteboard; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n whiteboard rollout status deploy/whiteboard --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service whiteboard --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service whiteboard --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service whiteboard --cell $CELL --window 30m`.
