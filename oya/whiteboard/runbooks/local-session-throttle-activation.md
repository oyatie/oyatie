# Whiteboard Runbook: Session Throttle Activation

Service: whiteboard  
Surface: local operational primitive suite  
Primary SLO: presence-freshness  
Domain focus: whiteboard, cursor, stroke, crdt

## Trigger
- Alert `whiteboard-presence-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_whiteboard_presence_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when presence-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `whiteboard-local-presence-freshness` and policy file name to the ticket.
