...
## §principals
This anchor is closed for `drive` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.drive.runtime` owns normal `T0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.drive.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/drive/backfill-replay.md` when present.
- Platform principal `oyatie.drive.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.drive.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.drive.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `T0-suggest` is evaluated as `<tenant>.drive.t0-suggest` before any `drive` state mutation.
...
## §cedar-gates
This anchor is closed for `drive` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/drive/policy/auditor-scope.cedar`, `microservices/drive/policy/ci-scope.cedar`, `microservices/drive/policy/data-residency.md`, `microservices/drive/policy/dual-context-isolation.md`, `microservices/drive/policy/public-read.cedar`, `microservices/drive/policy/tenant-scope.cedar`.
- Action namespace uses `drive::<bounded_context>::<verb>`; the first protected action is `drive::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/drive/policy/auditor-scope.cedar`, `microservices/drive/policy/ci-scope.cedar`, `microservices/drive/policy/data-residency.md`, `microservices/drive/policy/dual-context-isolation.md`, `microservices/drive/policy/public-read.cedar`, `microservices/drive/policy/tenant-scope.cedar`.
- Example: `drive::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `drive` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `drive.dlp_virus_scan` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `drive.file_store` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `drive.folder_hierarchy` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `drive.immutability_tier` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `drive.permissions` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
This anchor is closed for `drive` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `drive` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `drive` only through contracts `microservices/drive/contracts/asyncapi/drive-events.yaml`, `microservices/drive/contracts/openapi/drive.yaml`, `microservices/drive/contracts/proto/drive.proto`.
- If product: `drive` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `dlp-virus-scan`, `file-store`, `folder-hierarchy`, `immutability-tier`, `permissions`, `preview`; +4 more.
...
## §policy-evaluation
This anchor is closed for `drive` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/drive/policy/auditor-scope.cedar`, `microservices/drive/policy/ci-scope.cedar`, `microservices/drive/policy/data-residency.md`, `microservices/drive/policy/dual-context-isolation.md`, `microservices/drive/policy/public-read.cedar`, `microservices/drive/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `drive::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `drive.dlp_virus_scan`.
...
## §cell-eligibility
This anchor is closed for `drive` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `drive` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `drive` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `drive.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `T0-suggest` can request classification or summarization with `audience_tag=drive.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `drive` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `drive` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `dlp-virus-scan` is read/written as an ontology node or edge projection when `drive` needs cross-surface discovery.
- Bounded-context entity `file-store` is read/written as an ontology node or edge projection when `drive` needs cross-surface discovery.
- Bounded-context entity `folder-hierarchy` is read/written as an ontology node or edge projection when `drive` needs cross-surface discovery.
- Bounded-context entity `immutability-tier` is read/written as an ontology node or edge projection when `drive` needs cross-surface discovery.
...
## §transport
This anchor is closed for `drive` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/drive/contracts/asyncapi/drive-events.yaml`, `microservices/drive/contracts/openapi/drive.yaml`, `microservices/drive/contracts/proto/drive.proto`.
...
- IaC transport evidence: `microservices/drive/iac/helm/Chart.yaml`, `microservices/drive/iac/helm/templates/deployment.yaml`, `microservices/drive/iac/helm/templates/hpa.yaml`, `microservices/drive/iac/helm/templates/networkpolicy.yaml`, `microservices/drive/iac/helm/templates/pdb.yaml`, `microservices/drive/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Example: `drive` `T0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `drive` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `dlp-virus-scan`, `file-store`, `folder-hierarchy`, `immutability-tier`, `permissions`, `preview`; +4 more and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/drive/iac/helm/Chart.yaml`, `microservices/drive/iac/helm/templates/deployment.yaml`, `microservices/drive/iac/helm/templates/hpa.yaml`, `microservices/drive/iac/helm/templates/networkpolicy.yaml`, `microservices/drive/iac/helm/templates/pdb.yaml`, `microservices/drive/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/drive/catalog/oya-drive-dlp-virus-scan-adapter-clamav.yaml`, `microservices/drive/catalog/oya-drive-dlp-virus-scan-adapter-opswat.yaml`, `microservices/drive/catalog/oya-drive-file-store-adapter-garage.yaml`, `microservices/drive/catalog/oya-drive-file-store-adapter-postgres.yaml`, `microservices/drive/catalog/oya-drive-file-store-adapter-s3.yaml`, `microservices/drive/catalog/oya-drive-file-store-adapter-seaweedfs.yaml`; +18 more.
- Example: `T0-suggest` API runs as a Kata-isolated container; scheduled/background work for `drive` uses a separate worker principal.
...
## §observability
This anchor is closed for `drive` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/drive/slos/dlp-scan-correctness.openslo.yaml`, `microservices/drive/slos/download-first-byte-latency.openslo.yaml`, `microservices/drive/slos/file-list-latency.openslo.yaml`, `microservices/drive/slos/immutability-tier-correctness.openslo.yaml`, `microservices/drive/slos/preview-render-latency.openslo.yaml`, `microservices/drive/slos/search-latency.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.drive.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `drive_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `drive_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/drive/slos/dlp-scan-correctness.openslo.yaml`.
...
- Example trace: `drive.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `drive` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `drive` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/drive/policy/auditor-scope.cedar`, `microservices/drive/policy/ci-scope.cedar`, `microservices/drive/policy/data-residency.md`, `microservices/drive/policy/dual-context-isolation.md`, `microservices/drive/policy/public-read.cedar`, `microservices/drive/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `drive` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/drive`.
...
This anchor is closed for `drive` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `drive` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `drive` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `drive` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `drive` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `drive` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `drive` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/drive/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/drive/runbooks/dlp-quarantine-release.md`, `microservices/drive/runbooks/immutability-tier-violation.md`, `microservices/drive/runbooks/object-storage-degraded.md`, `microservices/drive/runbooks/share-link-takeover-incident.md`, `microservices/drive/runbooks/sync-conflict-resolution.md`, `microservices/drive/runbooks/upload-multipart-stuck.md`; +1 more.
