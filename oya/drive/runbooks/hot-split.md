---
doc_class: Runbook
doc_id: RUNBOOK-DRIVE-HOT_SPLIT
microservice: drive
...
# Runbook: Drive Hot Split

## Operator Contract
- Runbook id: drive-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-drive-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=drive; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n drive rollout status deploy/drive --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service drive --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service drive --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service drive --cell $CELL --window 30m`.
