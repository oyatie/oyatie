...
# IP-001: IaC bootstrap — Helm + Kustomize for Garage + SeaweedFS + SeaweedFS + Postgres + Valkey + Meilisearch + Tika + ClamAV + OPSWAT + LibreOffice-gVisor
...
| `microservices/drive/iac/helm/templates/networkpolicy.yaml` | created | mesh-only ingress; egress to OpenBao + Postgres + Valkey + object-store + Meili + Tika + ClamAV + OPSWAT + cross-µservice mesh; preview workers DENY all egress except object-store + OpenBao |
...
| `microservices/drive/iac/kustomize/overlays/pack-us-healthcare/...` | successor-IP | HIPAA pack (SeaweedFS + OPSWAT enabled) |
...
- E2E smoke: spin kind cluster; apply pack-kr overlay; verify all 11 BC deployments + Garage + Postgres + Valkey + Meilisearch + Tika + ClamAV + LibreOffice-gVisor reach Ready within 15 min.
...
- Garage — `garagehq.deuxfleurs.fr`; SeaweedFS — `min.io`; SeaweedFS — `github.com/seaweedfs/seaweedfs`.
