---
doc_class: Runbook
doc_id: RUNBOOK-FEATURE_FLAGS-HOT_SPLIT
...
# Runbook: Feature Flags Hot Split

## Operator Contract
- Runbook id: feature-flags-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-feature-flags-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=feature-flags; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n feature-flags rollout status deploy/feature-flags --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service feature-flags --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service feature-flags --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service feature-flags --cell $CELL --window 30m`.
