...
bounded_context: streaming-pipeline
...
---
# IP-WAVE-15-ZD-sharding-automation: Detection Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/detection/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: detection must interpret ADR-0348 through its own bounded context: streaming-pipeline.
SCOPE-004: detection uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: detection uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: detection uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: detection.
STANCE-002: Owner team: axis-detection.
...
STANCE-005: Bounded context anchor: streaming-pipeline.
STANCE-006: Cell placement class input: Tier-2.
STANCE-007: Capacity scaling dimension input: per_message.
STANCE-008: Compliance pack input: pack-us, pack-eu-ai-act, pack-kr, pack-jp, pack-cn-pipl, pack-us-healthcare, pack-br, pack-sg, pack-au, pack-ae, pack-ksa.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for detection.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for detection because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; detection must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where detection participates.
...
ROLLBACK-002: For detection, rollback starts from the audit-chain row that recorded the automation event.
