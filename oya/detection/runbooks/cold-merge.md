---
doc_class: Runbook
doc_id: RUNBOOK-DETECTION-COLD_MERGE
...
# Runbook: Detection Cold Merge

## Operator Contract
- Runbook id: detection-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-detection-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=detection; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n detection rollout status deploy/detection --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service detection --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service detection --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service detection --cell $CELL --window 30m`.
