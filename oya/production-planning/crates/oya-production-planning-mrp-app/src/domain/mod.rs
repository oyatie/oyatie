...
pub const CAPABILITIES: &[Capability] = &[
    Capability::ProductionOrderSchedule,
    Capability::MaterialRequirementPlan,
    Capability::CapacityLevel,
    Capability::ShopFloorRelease,
    Capability::EvidenceExport,
];
...
impl ServiceRecord {
    pub fn new(id: ResourceId, tenant_id: TenantId, capability: Capability) -> Self {
        Self {
            id,
            tenant_id,
            capability,
    ...
    pub fn release(&mut self) -> Result<()> {
        if matches!(self.state, ResourceState::Archived) {
            return Err(ServiceError::invariant(
                "state",
                "archived records cannot be released",
            ));
        }
    ...
    pub fn hold(&mut self) -> Result<()> {
        if matches!(self.state, ResourceState::Archived) {
            return Err(ServiceError::invariant(
                "state",
                "archived records cannot be held",
            ));
        }
        self.state = ResourceState::GovernanceHold;
        self.version += 1;
        Ok(())
    }
}
...
impl CapabilityDescriptor {
    pub fn descriptors() -> Vec<Self> {
        CAPABILITIES
            ...
            .map(|capability| Self {
                capability,
                bounded_context: BoundedContext::ProductionOrder,
