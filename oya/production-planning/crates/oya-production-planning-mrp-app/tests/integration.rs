...
use oya_production_planning_mrp_app::adapter::AdapterRegistry;
use oya_production_planning_mrp_app::config::ServiceConfig;
use oya_production_planning_mrp_app::domain::{Capability, IdempotencyKey, TenantId};
use oya_production_planning_mrp_app::{public_api_surface, scaffold};

#[test]
fn scaffold_declares_expected_contracts() {
    let scaffold = scaffold();
    assert_eq!(scaffold.microservice, "production-planning");
    ...
    assert_eq!(
        ...
        "contracts/production-planning-v1.proto"
    );
}
...
fn scaffold_declares_domain_capabilities() {
    ...
    assert!(
        scaffold
            .capabilities
            .contains(&Capability::ProductionOrderSchedule)
    );
    assert!(
        scaffold
            .capabilities
            .contains(&Capability::MaterialRequirementPlan)
    );
}
...
fn idempotency_key_requires_stable_length() {
    ...
    assert!(IdempotencyKey::new("production-planning-key-0001").is_ok());
}
