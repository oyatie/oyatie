---
doc_class: Runbook
doc_id: RUNBOOK-PRODUCTION_PLANNING-HOT_SPLIT
...
# Runbook: Production Planning Hot Split

## Operator Contract
- Runbook id: production-planning-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-production-planning-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=production-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n production-planning rollout status deploy/production-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service production-planning --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service production-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service production-planning --cell $CELL --window 30m`.
