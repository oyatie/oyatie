---
doc_class: Runbook
doc_id: RUNBOOK-PRODUCTION_PLANNING-AUTO_REBALANCE
...
# Runbook: Production Planning Auto Rebalance

## Operator Contract
- Runbook id: production-planning-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-production-planning-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=production-planning; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n production-planning rollout status deploy/production-planning --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service production-planning --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service production-planning --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service production-planning --cell $CELL --window 30m`.
