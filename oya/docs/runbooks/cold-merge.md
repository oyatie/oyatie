---
doc_class: Runbook
doc_id: RUNBOOK-DOCS-COLD_MERGE
microservice: docs
...
# Runbook: Docs Cold Merge

## Operator Contract
- Runbook id: docs-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-docs-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=docs; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n docs rollout status deploy/docs --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service docs --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service docs --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service docs --cell $CELL --window 30m`.
