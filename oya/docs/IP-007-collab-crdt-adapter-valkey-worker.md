...
impl_plan_id: IP-007-collab-crdt-adapter-valkey-worker
status: pending
...
# IP-007: collab-crdt -adapter-valkey + worker (WebSocket gateway)

## Intent

Implement PresenceBroadcast + CrdtOpSpool against Valkey 8.1 (RESP3 wire-compatible) cluster mode + a long-running WebSocket gateway worker that fans out CRDT ops via consistent-hash on `document_id`.

## ChangeSet boundary

3 crates: adapter-valkey + worker + sdk + app.

## Concrete File Targets

| Path | Action |
|---|---|
| `microservices/docs/src/crates/oya-docs-collab-crdt-adapter-valkey/src/{lib,presence,op_spool,lease}.rs` | create |
...
```bash
cargo nextest run -p oya-docs-collab-crdt-adapter-valkey -- single_flight_op_publish
...
This addendum converts the short implementation note into a buildable docs-service slice for CRDT Valkey worker; it does not rely on line count as proof.
...
For CRDT Valkey worker, the implementation must preserve dual-context separation, per-block ACL, legal hold, audit-chain records, and export/import safety described in `microservices/docs/PRD.md` and `microservices/docs/ARCHITECTURE.md`.

### Concrete mechanism
Primary artifact: `microservices/docs/catalog/oya-docs-collab-crdt-adapter-valkey.yaml`.
Domain entities or operational surfaces: presence, cursor fanout, backpressure.
...
1. Bind CRDT Valkey worker to the matching bounded context in `microservices/docs/manifest.json` and the catalog row named in this addendum.
...
|---|---|---|
| Google Docs / Microsoft Word Online | Native collaborative authoring with comments, sharing, history, and export. | `CRDT Valkey worker` must be first-party, tenant-scoped, auditable, and legal-hold aware. |
| Notion / Coda | Block-centric documents with embeds and structured workflows. | `CRDT Valkey worker` must preserve block ACL, embed policy checks, and cross-service refresh evidence instead of hidden workspace coupling. |
| Quip / GitHub | Team review and change history expectations. | `CRDT Valkey worker` must expose signed audit events, version/revert evidence, and contract-rendered developer docs. |
