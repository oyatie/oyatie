...
2. **Loro version pin matches workflow-studio**: both µservices pin to the same Loro semver line (`^1.0` at M03). Major-version upgrades are co-decided by axis-docs + axis-workflow; a fresh round-trip-corpus drill against the 100-doc reference corpus (docs side) + 100-spec corpus (workflow-studio side) is required before either µservice merges the upgrade.
