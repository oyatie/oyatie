---
doc_class: Runbook
doc_id: RUNBOOK-PLANT_MAINTENANCE-COLD_MERGE
...
# Runbook: Plant Maintenance Cold Merge

## Operator Contract
- Runbook id: plant-maintenance-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plant-maintenance-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plant-maintenance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plant-maintenance rollout status deploy/plant-maintenance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plant-maintenance --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service plant-maintenance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service plant-maintenance --cell $CELL --window 30m`.
