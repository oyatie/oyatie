---
doc_class: Runbook
doc_id: RUNBOOK-PLANT_MAINTENANCE-AUTO_REBALANCE
...
# Runbook: Plant Maintenance Auto Rebalance

## Operator Contract
- Runbook id: plant-maintenance-auto-rebalance.
- Primary service namespace: `plant-maintenance`.
...
- Owning team: axis-plant-maintenance + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-plant-maintenance-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=plant-maintenance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n plant-maintenance rollout status deploy/plant-maintenance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service plant-maintenance --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service plant-maintenance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service plant-maintenance --cell $CELL --window 30m`.
