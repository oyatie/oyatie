...
bounded_context: equipment-master
...
---
# IP-WAVE-15-ZD-sharding-automation: Plant Maintenance Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/plant-maintenance/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: plant-maintenance must interpret ADR-0348 through its own bounded context: equipment-master.
SCOPE-004: plant-maintenance uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: plant-maintenance uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: plant-maintenance uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: plant-maintenance.
STANCE-002: Owner team: axis-plant-maintenance + axis-erp-parity.
...
STANCE-005: Bounded context anchor: equipment-master.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for plant-maintenance.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for plant-maintenance because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; plant-maintenance must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where plant-maintenance participates.
...
ROLLBACK-002: For plant-maintenance, rollback starts from the audit-chain row that recorded the automation event.
