...
Primary SLO: problem-link-correctness  
Domain focus: incident, change, problem, cmdb

## Trigger
- Alert `itsm-problem-link-correctness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_itsm_problem_link_correctness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when problem-link-correctness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `itsm-local-problem-link-correctness` and policy file name to the ticket.
