# ITSM Runbook: Change Freeze Override

Service: itsm  
Surface: local operational primitive suite  
Primary SLO: mttr-objective  
Domain focus: incident, change, problem, cmdb

## Trigger
- Alert `itsm-mttr-objective` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_itsm_mttr_objective_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when mttr-objective is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `itsm-local-mttr-objective` and policy file name to the ticket.
