---
doc_class: Runbook
doc_id: RUNBOOK-ITSM-AUTO_REBALANCE
microservice: itsm
...
# Runbook: Itsm Auto Rebalance

## Operator Contract
- Runbook id: itsm-auto-rebalance.
- Primary service namespace: `itsm`.
...
- Owning team: axis-itsm + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-itsm-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=itsm; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n itsm rollout status deploy/itsm --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service itsm --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service itsm --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service itsm --cell $CELL --window 30m`.
