# ITSM Runbook: Change Failure Rate Spike

Service: itsm  
Surface: local operational primitive suite  
Primary SLO: change-failure-rate  
Domain focus: incident, change, problem, cmdb

## Trigger
- Alert `itsm-change-failure-rate` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_itsm_change_failure_rate_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when change-failure-rate is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `itsm-local-change-failure-rate` and policy file name to the ticket.
