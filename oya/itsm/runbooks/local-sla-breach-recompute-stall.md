# ITSM Runbook: Sla Breach Recompute Stall

Service: itsm  
Surface: local operational primitive suite  
Primary SLO: sla-breach-detection  
Domain focus: incident, change, problem, cmdb

## Trigger
- Alert `itsm-sla-breach-detection` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_itsm_sla_breach_detection_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when sla-breach-detection is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `itsm-local-sla-breach-detection` and policy file name to the ticket.
