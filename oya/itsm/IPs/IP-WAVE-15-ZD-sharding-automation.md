...
---
# IP-WAVE-15-ZD-sharding-automation: Itsm Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/itsm/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: itsm must interpret ADR-0348 through its own bounded context: on-call-schedule.
SCOPE-004: itsm uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: itsm uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: itsm uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: itsm.
STANCE-002: Owner team: axis-itsm + council-product.
...
STANCE-008: Compliance pack input: SOC-2, ISO-27001, ITIL, GDPR, KR-PIPA, FedRAMP-High, HIPAA.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for itsm.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for itsm because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; itsm must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where itsm participates.
...
ROLLBACK-002: For itsm, rollback starts from the audit-chain row that recorded the automation event.
