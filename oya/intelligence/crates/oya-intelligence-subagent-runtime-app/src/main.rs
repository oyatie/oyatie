//! `oya-intelligence-subagent-runtime-app` binary entrypoint.
//!
//! Fan-out command:
//!
//! ```text
//! oya-intelligence-subagent-runtime-app fan-out \
//!   --pr-number <NUM> \
//!   --change-id <ID> \
...
//!
//! ```text
//! oya-intelligence-subagent-runtime-app fix-loop \
...
fn main() -> ExitCode {
    ...
    match run(&args) {
        ...
        Err(err) => {
            eprintln!("oya-intelligence-subagent-runtime-app failed: {err}");
            ExitCode::FAILURE
        }
    }
}
...
fn run_fan_out(options: FanOutOptions) -> Result<String, String> {
    ...
    match options.mode {
        ...
        RuntimeMode::AnthropicApi => {
            ...
            // (typically `tools/oya-intelligence-pr-review-dispatcher-app`'s
...
fn execute_fan_out<P: SubagentPort>(
    ...
    port: &P,
    now_epoch: u64,
    panel: &[oya_intelligence_subagent_runtime_app::FacetSlug],
...
fn usage() -> String {
    "usage: oya-intelligence-subagent-runtime-app <fan-out|fix-loop> ...\n\n\
