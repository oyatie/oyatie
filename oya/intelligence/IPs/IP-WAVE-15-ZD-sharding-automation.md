...
slot_owner: ZF-9
sharding_role: ai-substrate-owner
---
# IP-WAVE-15-ZD-sharding-automation: Intelligence Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/intelligence/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: intelligence must interpret ADR-0348 through its own bounded context: model-routing.
SCOPE-004: intelligence uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: intelligence uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: intelligence uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: intelligence.
STANCE-002: Owner team: axis-intelligence.
STANCE-003: Sharding role: ai-substrate-owner.
STANCE-004: Role statement: keeps AI provider, guardrail, and evaluation workloads cell-aware without bypassing audit.
STANCE-005: Bounded context anchor: model-routing.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_request.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, us-federal, jp, sg, au, in, br, ae, ksa, cn, uk, eu-ai-act, soc2, iso27001, gdpr, fedramp, hipaa, SOC 2 Type 2, lgpd, kr-pipa.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for intelligence.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for intelligence because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; intelligence must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where intelligence participates.
...
ROLLBACK-002: For intelligence, rollback starts from the audit-chain row that recorded the automation event.
