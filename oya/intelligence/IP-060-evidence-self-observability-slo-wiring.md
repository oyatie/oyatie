...
| `microservices/intelligence/contracts/openapi/foundry-evidence.yaml` | edit | wire `/transparency/claim-matrix` endpoint to load from a generated registry + claim assertions |
| `microservices/intelligence/capabilities/eval/claim-matrix.json` | create | machine-readable claim-matrix sourced for the public-read endpoint |

## Acceptance Gates

```bash
buck2 build //:quality-lane-registry-authority-check # lane=self-slo-coverage --microservice foundry-evidence
buck2 build //:quality-lane-registry-authority-check # lane=agentic-slo-gated-promotion --microservice foundry-evidence
buck2 build //:quality-lane-registry-authority-check # lane=hyperscaler-maturity-claims --microservice foundry-evidence
```
