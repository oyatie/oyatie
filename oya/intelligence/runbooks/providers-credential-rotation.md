...
| 4 | Write the new credential to OpenBao at the **next** version of the SecretReference path: `openbao://<pack>/<tenant>/providers/<vendor>/<credential-name>?version=N+1`. Use `Intelligence control-plane operation: providers rotate --tenant <t> --vendor <v> --credential <name>` (2-person signed; OpenBao audit-emitted) | ≤ 3 min |
...
| 9 | Emit `CredentialRotated` audit-chain event (auto-emitted by the control-plane operation) | – |
...
| 2 | Pause router traffic to the subscription transport for this tenant: `Intelligence control-plane operation: providers pause --tenant <t> --vendor <v> --transport subscription --reason "<id>"`. Router routes to alternate (e.g., API transport if available, or alternate vendor) | ≤ 1 min |
...
| 6 | Resume router traffic: `Intelligence control-plane operation: providers resume --tenant <t> --vendor <v> --transport subscription` | ≤ 1 min |
