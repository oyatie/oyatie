---
doc_class: Runbook
doc_id: RUNBOOK-INTELLIGENCE-AUTO_REBALANCE
...
# Runbook: Intelligence Auto Rebalance

## Operator Contract
- Runbook id: intelligence-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-intelligence-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=intelligence; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n intelligence rollout status deploy/intelligence --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service intelligence --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service intelligence --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service intelligence --cell $CELL --window 30m`.
