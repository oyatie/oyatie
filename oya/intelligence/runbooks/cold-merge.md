---
doc_class: Runbook
doc_id: RUNBOOK-INTELLIGENCE-COLD_MERGE
...
# Runbook: Intelligence Cold Merge

## Operator Contract
- Runbook id: intelligence-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-intelligence-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=intelligence; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n intelligence rollout status deploy/intelligence --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service intelligence --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service intelligence --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service intelligence --cell $CELL --window 30m`.
