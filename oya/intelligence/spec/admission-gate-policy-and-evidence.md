...
canonical_path: microservices/intelligence/spec
---
...
The repository boundary is microservices/intelligence plus repo-level governance registries and docs referenced in cross-references.
...
| B-003 | Principal::"oyatie.agent.codex" | Action::"foundry.admission.evaluate_policy" | Resource::"policy:microservices/intelligence/policy/*.cedar" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
| B-004 | Principal::"oyatie.agent.codex" | Action::"foundry.admission.validate_evidence" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
...
| B-015 | Principal::"oyatie.agent.planner" | Action::"foundry.admission.validate_status" | Resource::"policy:microservices/intelligence/policy/*.cedar" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
| B-016 | Principal::"oyatie.agent.executor" | Action::"foundry.admission.receive_bundle" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
...
| B-027 | Principal::"oyatie.service.webhook-receiver" | Action::"foundry.admission.check_scope" | Resource::"policy:microservices/intelligence/policy/*.cedar" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
| B-028 | Principal::"oyatie.service.webhook-receiver" | Action::"foundry.admission.evaluate_policy" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
...
| B-039 | Principal::"oyatie.service.admission-gate" | Action::"foundry.admission.validate_evidence" | Resource::"policy:microservices/intelligence/policy/*.cedar" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
| B-040 | Principal::"oyatie.service.admission-gate" | Action::"foundry.admission.validate_status" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=admission-gate-policy-and-evidence | permit when evidence_hash and changeset_id are present |
...
permit(
  ...
  resource in Resource::"repo:oyatie/microservices/intelligence"
) when {
...
forbid(
  principal,
  action,
  resource in Resource::"repo:oyatie/microservices/intelligence/decisions"
) when {
...
- Guard 04: Principal::"oyatie.agent.executor" may invoke foundry.admission.validate_evidence on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `evidence_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 05: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.admission.validate_status on Resource::"repo:oyatie/microservices/intelligence" only while `status_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 16: Principal::"oyatie.service.webhook-receiver" may invoke foundry.admission.check_scope on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `queue_admitted` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 17: Principal::"oyatie.service.merge-queue" may invoke foundry.admission.evaluate_policy on Resource::"repo:oyatie/microservices/intelligence" only while `admission_refused` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 28: Principal::"oyatie.service.admission-gate" may invoke foundry.admission.refuse on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `bundle_received` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 29: Principal::"oyatie.service.completion-gate" may invoke foundry.admission.receive_bundle on Resource::"repo:oyatie/microservices/intelligence" only while `scope_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 40: Principal::"oyatie.human.reviewer" may invoke foundry.admission.validate_status on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `evidence_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 41: Principal::"oyatie.agent.codex" may invoke foundry.admission.admit on Resource::"repo:oyatie/microservices/intelligence" only while `status_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 52: Principal::"oyatie.agent.claude-opus" may invoke foundry.admission.evaluate_policy on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `queue_admitted` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 53: Principal::"oyatie.agent.planner" may invoke foundry.admission.validate_evidence on Resource::"repo:oyatie/microservices/intelligence" only while `admission_refused` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 64: Principal::"oyatie.agent.executor" may invoke foundry.admission.receive_bundle on Resource::"policy:microservices/intelligence/policy/*.cedar" only while `bundle_received` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
- Guard 65: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.admission.check_scope on Resource::"repo:oyatie/microservices/intelligence" only while `scope_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
3. Resource: Resource::"policy:microservices/intelligence/policy/*.cedar" is the primary target.
4. Policy: Cedar evaluates tenant_id=oyatie, workflow=governance_pipeline, related_adr=ADR-0111, and evidence_hash presence.
...
3. Resource: Resource::"repo:oyatie/microservices/intelligence" is the primary target.
4. Policy: Cedar evaluates tenant_id=oyatie, workflow=governance_pipeline, related_adr=ADR-0111, and evidence_hash presence.
...
| admission-gate-policy-and-evidence-matrix-03 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-04 | cargo test -p oya-vcs-review-mergequeue-kernel --scope evidence_checked --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves evidence_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-05 | cargo test -p oya-vcs-webhook-receiver-kernel --scope status_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves status_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-06 | cargo test -p oya-vcs-admission-gate-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-07 | cargo test -p oya-vcs-changebundle-kernel --scope queue_admitted --adr ADR-0111 | oya-vcs-changebundle-kernel | proves queue_admitted cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-08 | cargo test -p oya-vcs-promotion-controller-kernel --scope admission_refused --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves admission_refused cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-09 | cargo test -p oya-vcs-cli-ratchet-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-15 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-16 | cargo test -p oya-vcs-review-mergequeue-kernel --scope queue_admitted --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves queue_admitted cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-17 | cargo test -p oya-vcs-webhook-receiver-kernel --scope admission_refused --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves admission_refused cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-18 | cargo test -p oya-vcs-admission-gate-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-19 | cargo test -p oya-vcs-changebundle-kernel --scope bundle_received --adr ADR-0111 | oya-vcs-changebundle-kernel | proves bundle_received cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-20 | cargo test -p oya-vcs-promotion-controller-kernel --scope scope_checked --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves scope_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-21 | cargo test -p oya-vcs-cli-ratchet-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-27 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-28 | cargo test -p oya-vcs-review-mergequeue-kernel --scope bundle_received --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves bundle_received cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-29 | cargo test -p oya-vcs-webhook-receiver-kernel --scope scope_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves scope_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-30 | cargo test -p oya-vcs-admission-gate-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-31 | cargo test -p oya-vcs-changebundle-kernel --scope evidence_checked --adr ADR-0111 | oya-vcs-changebundle-kernel | proves evidence_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-32 | cargo test -p oya-vcs-promotion-controller-kernel --scope status_checked --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves status_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-33 | cargo test -p oya-vcs-cli-ratchet-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-39 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-40 | cargo test -p oya-vcs-review-mergequeue-kernel --scope evidence_checked --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves evidence_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-41 | cargo test -p oya-vcs-webhook-receiver-kernel --scope status_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves status_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-42 | cargo test -p oya-vcs-admission-gate-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-43 | cargo test -p oya-vcs-changebundle-kernel --scope queue_admitted --adr ADR-0111 | oya-vcs-changebundle-kernel | proves queue_admitted cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-44 | cargo test -p oya-vcs-promotion-controller-kernel --scope admission_refused --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves admission_refused cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-45 | cargo test -p oya-vcs-cli-ratchet-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-51 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-52 | cargo test -p oya-vcs-review-mergequeue-kernel --scope queue_admitted --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves queue_admitted cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-53 | cargo test -p oya-vcs-webhook-receiver-kernel --scope admission_refused --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves admission_refused cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-54 | cargo test -p oya-vcs-admission-gate-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-55 | cargo test -p oya-vcs-changebundle-kernel --scope bundle_received --adr ADR-0111 | oya-vcs-changebundle-kernel | proves bundle_received cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-56 | cargo test -p oya-vcs-promotion-controller-kernel --scope scope_checked --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves scope_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-57 | cargo test -p oya-vcs-cli-ratchet-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-63 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope fix_requested --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves fix_requested cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-64 | cargo test -p oya-vcs-review-mergequeue-kernel --scope bundle_received --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves bundle_received cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-65 | cargo test -p oya-vcs-webhook-receiver-kernel --scope scope_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves scope_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-66 | cargo test -p oya-vcs-admission-gate-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-67 | cargo test -p oya-vcs-changebundle-kernel --scope evidence_checked --adr ADR-0111 | oya-vcs-changebundle-kernel | proves evidence_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-68 | cargo test -p oya-vcs-promotion-controller-kernel --scope status_checked --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves status_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-69 | cargo test -p oya-vcs-cli-ratchet-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
...
| admission-gate-policy-and-evidence-matrix-75 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope policy_checked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves policy_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-76 | cargo test -p oya-vcs-review-mergequeue-kernel --scope evidence_checked --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves evidence_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-77 | cargo test -p oya-vcs-webhook-receiver-kernel --scope status_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves status_checked cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-78 | cargo test -p oya-vcs-admission-gate-kernel --scope cedar_permit --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves cedar_permit cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-79 | cargo test -p oya-vcs-changebundle-kernel --scope queue_admitted --adr ADR-0111 | oya-vcs-changebundle-kernel | proves queue_admitted cannot advance without policy, evidence, trace, and audit correlation |
| admission-gate-policy-and-evidence-matrix-80 | cargo test -p oya-vcs-promotion-controller-kernel --scope admission_refused --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves admission_refused cannot advance without policy, evidence, trace, and audit correlation |
...
| Foundry PRD | microservices/intelligence/PRD.md | Product-of-record Foundry surface. |
| Foundry architecture | microservices/intelligence/ARCHITECTURE.md | Top-level architecture for the single Foundry microservice. |
| Foundry policy | microservices/intelligence/policy/ | Cedar policy and markdown policy corpus. |
| Peer spec | microservices/intelligence/spec/changeset-state-machine.md | Cross-checks Foundry Changeset State Machine against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/merge-queue-projected-state.md | Cross-checks Foundry Merge Queue Projected State against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/webhook-driven-agent-invocation.md | Cross-checks Foundry Webhook Driven Agent Invocation against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/vcs-orchestrator-end-to-end.md | Cross-checks Foundry VCS Orchestrator End to End against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/agent-pipeline-isolation-worktree.md | Cross-checks Foundry Agent Pipeline Isolation Worktree against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/admission-gate-policy-and-evidence.md | Cross-checks Foundry Admission Gate Policy and Evidence against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/completion-gate-reviewer-and-ci.md | Cross-checks Foundry Completion Gate Reviewer and CI against Foundry Admission Gate Policy and Evidence. |
| Peer spec | microservices/intelligence/spec/agent-types-and-roles.md | Cross-checks Foundry Agent Types and Roles against Foundry Admission Gate Policy and Evidence. |
