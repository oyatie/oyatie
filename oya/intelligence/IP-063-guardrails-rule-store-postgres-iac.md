...
|---|---|---|
| `microservices/intelligence/iac/helm/postgres/Chart.yaml` | create | bitnami/postgresql-ha pinned LTS |
| `microservices/intelligence/iac/helm/postgres/values.yaml` | create | HA + RLS + pgaudit + TDE |
| `microservices/intelligence/iac/helm/postgres/values-pack-kr.yaml` | create | pack-kr |
| `microservices/intelligence/iac/postgres/migrations/001-init-schema.sql` | create | tables: rule_definitions, cedar_fragments, audit_mutation_log, classifier_model_versions |
| `microservices/intelligence/iac/postgres/migrations/002-rls-policies.sql` | create | Row-level security per tenant + pack |
| `microservices/intelligence/iac/postgres/migrations/003-indexes.sql` | create | per `capacity-model.md` access patterns |
| `microservices/intelligence/iac/postgres/migrations/manifest.yaml` | create | ordered migration list + checksums |
...
This IP is the `guardrails`-bounded-context slice for `IP-003: Postgres rule-store IaC + migrations`. The stamped version named a target but did not explain how the slice closes Foundry's product gap: inline safety and autonomy enforcement before provider invocation and before output release. The concrete gap is traceability from the implementation plan to real Foundry surfaces: `microservices/intelligence/capabilities/guardrails-classify-prompt.yaml`, `microservices/intelligence/capabilities/guardrails-enforce-autonomy.yaml`, `microservices/intelligence/capabilities/guardrails-validate-output.yaml`, `microservices/intelligence/contracts/openapi/guardrails-guardrails.yaml`, and the policy set `microservices/intelligence/policy/guardrails-tenant-scope.cedar`, `microservices/intelligence/policy/guardrails-guardrail-enforcement.md`, `microservices/intelligence/policy/guardrails-schema.cedarschema`.

### B. Technical approach
Implement the slice as a Foundry-owned ChangeSet, not as generic platform plumbing. The design starts at the capability or contract boundary, keeps tenant and principal fields in the DTO/event shape, and routes state changes through the `guardrails` policy envelope before any adapter call. The implementation must use existing catalog and crate naming from `microservices/intelligence/manifest.json`; the primary implementation anchor is `crates/oya-intelligence-autonomy-ceiling-kernel/src/lib.rs` plus the matching catalog records under `microservices/intelligence/catalog/`.
...
- Keep OpenAPI/AsyncAPI/proto parity across `microservices/intelligence/contracts/openapi/guardrails-guardrails.yaml`, `microservices/intelligence/contracts/asyncapi/guardrails-decision-events.yaml`, and `microservices/intelligence/contracts/proto/guardrails-guardrails.proto` when the slice exposes a wire surface.
- Bind authorization to `microservices/intelligence/policy/guardrails-tenant-scope.cedar`, `microservices/intelligence/policy/guardrails-guardrail-enforcement.md`, `microservices/intelligence/policy/guardrails-schema.cedarschema`; if a required Cedar entity or action is absent, add it to the Foundry policy file in the same ChangeSet.
- Bind SLO evidence to `microservices/intelligence/slos/guardrails-policy-eval-latency.openslo.yaml`, `microservices/intelligence/slos/guardrails-shadow-mode-false-positive-budget.openslo.yaml`; this IP is incomplete if the acceptance path cannot point to an OpenSLO file or a documented N/A.
- Keep capability metadata aligned with `microservices/intelligence/capabilities/guardrails-classify-prompt.yaml`, `microservices/intelligence/capabilities/guardrails-enforce-autonomy.yaml`, `microservices/intelligence/capabilities/guardrails-validate-output.yaml` so supervisor/runtime/evidence can reason about risk class and tenant availability.

### D. Implementation sequence
1. Read `microservices/intelligence/PRD.md` and the `guardrails` row in `microservices/intelligence/manifest.json`; record the exact bounded-context crate names before editing.
...
- Contract parity for `microservices/intelligence/contracts/openapi/guardrails-guardrails.yaml` and `microservices/intelligence/contracts/proto/guardrails-guardrails.proto` when DTOs or handlers change.
- Policy resolution against `microservices/intelligence/policy/guardrails-tenant-scope.cedar`, `microservices/intelligence/policy/guardrails-guardrail-enforcement.md`, `microservices/intelligence/policy/guardrails-schema.cedarschema`, including a tenant mismatch denial and a CI/synthetic principal allowance where applicable.
- SLO or dashboard linkage against `microservices/intelligence/slos/guardrails-policy-eval-latency.openslo.yaml`, `microservices/intelligence/slos/guardrails-shadow-mode-false-positive-budget.openslo.yaml`; no acceptance by line count alone.
