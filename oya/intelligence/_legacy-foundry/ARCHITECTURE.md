...
## §principals
This anchor is closed for `foundry` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.foundry.runtime` owns normal `eval-run` execution and never borrows a tenant principal.
- Platform principal `oyatie.foundry.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/foundry/backfill-replay.md` when present.
- Platform principal `oyatie.foundry.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.foundry.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.foundry.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `eval-run` is evaluated as `<tenant>.foundry.eval-run` before any `foundry` state mutation.
...
## §cedar-gates
This anchor is closed for `foundry` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/foundry/policy/eval-auditor-scope.cedar`, `microservices/foundry/policy/eval-ci-scope.cedar`, `microservices/foundry/policy/eval-data-residency.md`, `microservices/foundry/policy/eval-dp-analysis.md`, `microservices/foundry/policy/eval-synthetic-phi-only.md`, `microservices/foundry/policy/eval-tenant-isolation.md`; +10 more.
- Action namespace uses `foundry::<bounded_context>::<verb>`; the first protected action is `foundry::eval-run::execute`.
...
- Concrete fragments in scope: `microservices/foundry/policy/eval-auditor-scope.cedar`, `microservices/foundry/policy/eval-ci-scope.cedar`, `microservices/foundry/policy/eval-data-residency.md`, `microservices/foundry/policy/eval-dp-analysis.md`, `microservices/foundry/policy/eval-synthetic-phi-only.md`, `microservices/foundry/policy/eval-tenant-isolation.md`; +10 more.
- Example: `foundry::eval-run::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `foundry` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `foundry.foundry_eval` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `foundry.foundry_evidence` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `foundry.foundry_guardrails` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `foundry.foundry_providers` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `foundry.foundry_runtime` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
- Example: `eval-run` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `foundry` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `foundry` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `foundry` only through contracts `microservices/foundry/contracts/asyncapi/eval-eval-events.yaml`, `microservices/foundry/contracts/asyncapi/evidence-foundry-evidence-events.yaml`, `microservices/foundry/contracts/asyncapi/guardrails-decision-events.yaml`, `microservices/foundry/contracts/asyncapi/providers-provider-events.yaml`, `microservices/foundry/contracts/asyncapi/runtime-foundry-runtime-events.yaml`, `microservices/foundry/contracts/asyncapi/supervisor-foundry-supervisor-events.yaml`.
- If product: `foundry` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `foundry-eval`, `foundry-evidence`, `foundry-guardrails`, `foundry-providers`, `foundry-runtime`, `foundry-supervisor`.
- Example: `eval-run` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `foundry` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/foundry/policy/eval-auditor-scope.cedar`, `microservices/foundry/policy/eval-ci-scope.cedar`, `microservices/foundry/policy/eval-data-residency.md`, `microservices/foundry/policy/eval-dp-analysis.md`, `microservices/foundry/policy/eval-synthetic-phi-only.md`, `microservices/foundry/policy/eval-tenant-isolation.md`; +10 more with the full action/resource/context tuple.
...
- Example: `foundry::eval-run::execute` evaluates locally, then emits an audit event before the usecase writes `foundry.foundry_eval`.
...
## §self-modification
This anchor is closed for `foundry` against ADR-0247 §D-3: self-modification scope, approval and audit boundaries.

### Service-specific answer
- `foundry` answers `self-modification` using manifest, contracts, policy fragments, and service-local evidence instead of a cross-service placeholder.
- Primary capability example: `eval-run`; primary state surface `foundry.foundry_eval`; primary contracts `microservices/foundry/contracts/asyncapi/eval-eval-events.yaml`, `microservices/foundry/contracts/asyncapi/evidence-foundry-evidence-events.yaml`, `microservices/foundry/contracts/asyncapi/guardrails-decision-events.yaml`, `microservices/foundry/contracts/asyncapi/providers-provider-events.yaml`, `microservices/foundry/contracts/asyncapi/runtime-foundry-runtime-events.yaml`, `microservices/foundry/contracts/asyncapi/supervisor-foundry-supervisor-events.yaml`.
...
### Hyperscaler precedents
- Precedent 1: Google Binary Authorization admission control is the reference pattern for the control shape described here.
- Precedent 2: SLSA provenance attestation is the second reference pattern used to avoid a single-vendor cargo-cult design.
- The adaptation keeps the hyperscaler property that control evidence is observable, versioned, reversible, and tenant-scoped.
- The adaptation rejects hidden tribal knowledge: a cold reader can trace service, policy, storage, runtime, and audit evidence from this section.
...
- This expansion preserves the existing prose above and closes `self-modification` for `foundry` to the ≥50-line documentation-rigor floor.
- Service owner `axis-foundry` owns this answer; tier `product`; audience `B2C_CONSUMER + B2B_TENANT`.
- Primary capability/context: `eval-run`; bounded contexts: `foundry-eval`, `foundry-evidence`, `foundry-guardrails`, `foundry-providers`, `foundry-runtime`; +1 more.
- API surfaces: `microservices/foundry/contracts/asyncapi/eval-eval-events.yaml`, `microservices/foundry/contracts/asyncapi/evidence-foundry-evidence-events.yaml`, `microservices/foundry/contracts/asyncapi/guardrails-decision-events.yaml`, `microservices/foundry/contracts/asyncapi/providers-provider-events.yaml`, `microservices/foundry/contracts/asyncapi/runtime-foundry-runtime-events.yaml`; +5 more.
- Cedar/policy surfaces: `microservices/foundry/policy/eval-auditor-scope.cedar`, `microservices/foundry/policy/eval-ci-scope.cedar`, `microservices/foundry/policy/eval-data-residency.md`, `microservices/foundry/policy/eval-dp-analysis.md`, `microservices/foundry/policy/eval-synthetic-phi-only.md`; +5 more.
- State/event surfaces: `foundry.foundry_eval`, `foundry.foundry_evidence`, `foundry.foundry_guardrails`, `foundry.foundry_providers`, `foundry.foundry_runtime`; +1 more.
- SLO/dashboard evidence: `microservices/foundry/slos/eval-determinism-correctness.openslo.yaml`, `microservices/foundry/slos/eval-run-latency.openslo.yaml`, `microservices/foundry/slos/evidence-chain-integrity-correctness.openslo.yaml`, `microservices/foundry/slos/evidence-emit-latency.openslo.yaml`, `microservices/foundry/slos/guardrails-policy-eval-latency.openslo.yaml`; +9 more.
- Runbook/IaC evidence: `microservices/foundry/runbooks/eval-clickhouse-rebalance.md`, `microservices/foundry/runbooks/eval-eval-set-rollback.md`, `microservices/foundry/runbooks/eval-baseline-output-restore.md`, `microservices/foundry/runbooks/eval-gpu-pool-rebalance.md`, `microservices/foundry/runbooks/eval-parity-regression-triage.md`; +11 more.
- Compliance packs: `ae`, `au`, `br`, `eu`, `in`; +3 more; data classes: `INTERNAL_ONLY`, `AUDIT`, `PII_QUASI`.
...
- Tenant-scope invariant: every `foundry` `eval-run` request carries `tenant_id`, `principal_id`, `audience_type`, `home_cell`, `jurisdiction_code`, and `audit_event_class`.
...
- Credential invariant: provider/API/signing keys use `${openbao:secret/<tenant_id>/foundry/<credential>}` and sidecar/≤60s TTL behavior.
...
- Failure mode: stale tenant projection. `foundry` applies most-restrictive policy and emits degraded-mode evidence.
- Failure mode: Cedar mismatch. `foundry` fails closed for mutations and rolls back to prior soaked fragment.
- Failure mode: audit backpressure. `foundry` buffers bounded evidence and stops high-risk mutation before evidence loss.
- Failure mode: regional outage. `foundry` follows `multi-region.md` and does not cross pack residency boundaries for availability.
- Failure mode: key compromise. `foundry` revokes OpenBao leases, rotates keys, quarantines impacted events, and replays idempotent work.
- Concrete example: `eval-run` evaluates `<tenant>.foundry.eval-run` against policy, writes `foundry.foundry_eval`, and emits `oya.foundry.eval.run.completed`.
...
## §cell-eligibility
This anchor is closed for `foundry` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `eval-run` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `foundry` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `foundry` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `foundry.eval-run` and is included in every AI/risk/model call audit event.
...
- Example: `eval-run` can request classification or summarization with `audience_tag=foundry.eval-run`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `foundry` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `foundry` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `foundry-eval` is read/written as an ontology node or edge projection when `foundry` needs cross-surface discovery.
- Bounded-context entity `foundry-evidence` is read/written as an ontology node or edge projection when `foundry` needs cross-surface discovery.
- Bounded-context entity `foundry-guardrails` is read/written as an ontology node or edge projection when `foundry` needs cross-surface discovery.
- Bounded-context entity `foundry-providers` is read/written as an ontology node or edge projection when `foundry` needs cross-surface discovery.
...
- Example: `eval-run` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `foundry` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/foundry/contracts/asyncapi/eval-eval-events.yaml`, `microservices/foundry/contracts/asyncapi/evidence-foundry-evidence-events.yaml`, `microservices/foundry/contracts/asyncapi/guardrails-decision-events.yaml`, `microservices/foundry/contracts/asyncapi/providers-provider-events.yaml`, `microservices/foundry/contracts/asyncapi/runtime-foundry-runtime-events.yaml`, `microservices/foundry/contracts/asyncapi/supervisor-foundry-supervisor-events.yaml`.
...
- IaC transport evidence: `microservices/foundry/iac/cedar/guardrails-build.sh`, `microservices/foundry/iac/helm/eval/clickhouse/Chart.yaml`, `microservices/foundry/iac/helm/eval/clickhouse/values.yaml`, `microservices/foundry/iac/helm/eval/baseline-store/Chart.yaml`, `microservices/foundry/iac/helm/eval/baseline-store/values.yaml`, `microservices/foundry/iac/helm/eval/gpu-runner-pool/Chart.yaml`; +6 more.
- Example: `foundry` `eval-run` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `foundry` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `foundry-eval`, `foundry-evidence`, `foundry-guardrails`, `foundry-providers`, `foundry-runtime`, `foundry-supervisor` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/foundry/iac/cedar/guardrails-build.sh`, `microservices/foundry/iac/helm/eval/clickhouse/Chart.yaml`, `microservices/foundry/iac/helm/eval/clickhouse/values.yaml`, `microservices/foundry/iac/helm/eval/baseline-store/Chart.yaml`, `microservices/foundry/iac/helm/eval/baseline-store/values.yaml`, `microservices/foundry/iac/helm/eval/gpu-runner-pool/Chart.yaml`; +6 more.
- Catalog crates/components: `microservices/foundry/catalog/oya-foundry-eval-eval-runner-adapter-gpu.yaml`, `microservices/foundry/catalog/oya-foundry-eval-eval-runner-adapter-s3.yaml`, `microservices/foundry/catalog/oya-foundry-eval-eval-runner-adapter.yaml`, `microservices/foundry/catalog/oya-foundry-eval-eval-runner-api.yaml`, `microservices/foundry/catalog/oya-foundry-eval-eval-runner-app.yaml`, `microservices/foundry/catalog/oya-foundry-eval-eval-runner-domain.yaml`; +18 more.
- Example: `eval-run` API runs as a Kata-isolated container; scheduled/background work for `foundry` uses a separate worker principal.
...
## §observability
This anchor is closed for `foundry` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/foundry/slos/eval-determinism-correctness.openslo.yaml`, `microservices/foundry/slos/eval-run-latency.openslo.yaml`, `microservices/foundry/slos/evidence-chain-integrity-correctness.openslo.yaml`, `microservices/foundry/slos/evidence-emit-latency.openslo.yaml`, `microservices/foundry/slos/guardrails-policy-eval-latency.openslo.yaml`, `microservices/foundry/slos/guardrails-shadow-mode-false-positive-budget.openslo.yaml`; +18 more.
- Audit classes use prefix `oya.foundry.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `foundry_eval_run_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `foundry_eval_run_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/foundry/slos/eval-determinism-correctness.openslo.yaml`.
...
- Example trace: `foundry.eval-run` -> `policy.evaluate` -> `storage.eval_run` -> `audit.emit`.
...
### Hyperscaler precedents
- Precedent 1: Google SRE four baseline signals is the reference pattern for the control shape described here.
...
## §abuse-defence
This anchor is closed for `foundry` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `foundry` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `eval-run` action class.
...
- Cedar/IaC controls in scope: `microservices/foundry/policy/eval-auditor-scope.cedar`, `microservices/foundry/policy/eval-ci-scope.cedar`, `microservices/foundry/policy/eval-data-residency.md`, `microservices/foundry/policy/eval-dp-analysis.md`, `microservices/foundry/policy/eval-synthetic-phi-only.md`, `microservices/foundry/policy/eval-tenant-isolation.md`; +22 more.
- Canary payloads for `foundry` use fake `eval-run` identifiers and honey endpoints under `/.well-known/oya-canary/foundry`.
...
This anchor is closed for `foundry` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `foundry` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `foundry` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `foundry` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `eval-run` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `foundry` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `foundry` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `foundry` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/foundry/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/foundry/runbooks/eval-clickhouse-rebalance.md`, `microservices/foundry/runbooks/eval-eval-set-rollback.md`, `microservices/foundry/runbooks/eval-baseline-output-restore.md`, `microservices/foundry/runbooks/eval-gpu-pool-rebalance.md`, `microservices/foundry/runbooks/eval-parity-regression-triage.md`, `microservices/foundry/runbooks/eval-replay-divergence-investigation.md`; +6 more.
- Example: `eval-run` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
