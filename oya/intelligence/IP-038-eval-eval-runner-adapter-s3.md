...
S3-backend-qualified adapter (per ADR-0105 Amendment 3): baseline-output GET (Cosign-verified) + eval-run PUT + replay-trace GET (per-subject DEK unwrap via KMS).

## ChangeSet boundary

`microservices/intelligence/src/crates/oya-foundry-eval-eval-runner-adapter-s3/`.
...
| `src/crates/oya-foundry-eval-eval-runner-adapter-s3/src/baseline_reader.rs` | create — Cosign-verified GET |
