...
`microservices/intelligence/iac/helm/gpu-runner-pool/`: Chart.yaml + values.yaml + templates/ (DaemonSet or Deployment + ServiceAccount + Role + RoleBinding + NetworkPolicy + PriorityClass).
...
```bash
helm lint microservices/intelligence/iac/helm/gpu-runner-pool/
helm template microservices/intelligence/iac/helm/gpu-runner-pool/ | kubectl apply --dry-run=client -f -
