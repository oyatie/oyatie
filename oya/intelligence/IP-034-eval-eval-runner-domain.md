...
`microservices/intelligence/src/crates/oya-foundry-eval-eval-runner-domain/`.
...
Same as IP-003 + `buck2 build //:quality-lane-registry-authority-check # lane=layer-correctness --crate oya-foundry-eval-eval-runner-domain` (must depend only on kernel).
