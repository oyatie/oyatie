...
1. **FM-05 OIDC IdP outage** — `oya_application_oidc_idp_error_rate > 5%` for ≥ 1 min.
2. **FM-12 cookie scope misconfiguration** — lane / runtime probe alerts on `Domain=.oyatie.dev` drift.
3. **FM-14 SAML XSW attempt** — `oya_application_saml_xsw_block_total > 0` (security event; no service action required, but escalate).
...
| 4 | Rotate session HMAC key (invalidates all sessions): `cargo run -p oya-dev-cli -- application auth rotate-session-hmac --pack <pack>`. CLI: (a) generates new key in OpenBao; (b) marks old key as decrypt-only with 5-min sunset; (c) signals auth-gateway worker to refresh. | ≤ 5 min |
...
- `oya_application_oidc_signin_success_rate > 99%` for ≥ 15 min.
- `oya_application_saml_signin_success_rate > 99%` for ≥ 15 min.
- `oya_application_session_create_p99_seconds <= 0.2` for ≥ 15 min.
