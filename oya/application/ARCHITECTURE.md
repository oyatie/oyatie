...
## §principals
This anchor is closed for `application` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.application.runtime` owns normal `module-load` execution and never borrows a tenant principal.
- Platform principal `oyatie.application.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/application/backfill-replay.md` when present.
- Platform principal `oyatie.application.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.application.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.application.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `module-load` is evaluated as `<tenant>.application.module-load` before any `application` state mutation.
...
## §cedar-gates
This anchor is closed for `application` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/application/policy/auditor-scope.cedar`, `microservices/application/policy/ci-scope.cedar`, `microservices/application/policy/data-residency.md`, `microservices/application/policy/public-read.cedar`, `microservices/application/policy/route-isolation.md`, `microservices/application/policy/tenant-admin-console.cedar`; +1 more.
- Action namespace uses `application::<bounded_context>::<verb>`; the first protected action is `application::module-load::execute`.
...
- Concrete fragments in scope: `microservices/application/policy/auditor-scope.cedar`, `microservices/application/policy/ci-scope.cedar`, `microservices/application/policy/data-residency.md`, `microservices/application/policy/public-read.cedar`, `microservices/application/policy/route-isolation.md`, `microservices/application/policy/tenant-admin-console.cedar`; +1 more.
- Example: `application::module-load::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `application` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `application.auth_gateway` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `application.frontend_bundle_serve` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `application.module_loader` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `application.shell_routing` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `application.tenant_context` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
- Example: `module-load` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `application` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `application` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `application` only through contracts `microservices/application/contracts/asyncapi/application-events.yaml`, `microservices/application/contracts/openapi/application.yaml`, `microservices/application/contracts/openapi/tenant-admin-console.yaml`, `microservices/application/contracts/proto/application.proto`.
- If product: `application` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `auth-gateway`, `frontend-bundle-serve`, `module-loader`, `shell-routing`, `tenant-context`, `tenant-admin-console`.
- Example: `module-load` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `application` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/application/policy/auditor-scope.cedar`, `microservices/application/policy/ci-scope.cedar`, `microservices/application/policy/data-residency.md`, `microservices/application/policy/public-read.cedar`, `microservices/application/policy/route-isolation.md`, `microservices/application/policy/tenant-admin-console.cedar`; +1 more with the full action/resource/context tuple.
...
- Example: `application::module-load::execute` evaluates locally, then emits an audit event before the usecase writes `application.auth_gateway`.
...
## §cell-eligibility
This anchor is closed for `application` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `module-load` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
## §transport
This anchor is closed for `application` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/application/contracts/asyncapi/application-events.yaml`, `microservices/application/contracts/openapi/application.yaml`, `microservices/application/contracts/openapi/tenant-admin-console.yaml`, `microservices/application/contracts/proto/application.proto`.
...
- IaC transport evidence: `microservices/application/iac/helm/cdn-controller/Chart.yaml`, `microservices/application/iac/helm/cdn-controller/values.yaml`, `microservices/application/iac/helm/postgres/Chart.yaml`, `microservices/application/iac/helm/postgres/values.yaml`, `microservices/application/iac/helm/shell-app/Chart.yaml`, `microservices/application/iac/helm/shell-app/templates/deployment.yaml`; +4 more.
- Example: `application` `module-load` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `application` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `auth-gateway`, `frontend-bundle-serve`, `module-loader`, `shell-routing`, `tenant-context`, `tenant-admin-console` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/application/iac/helm/cdn-controller/Chart.yaml`, `microservices/application/iac/helm/cdn-controller/values.yaml`, `microservices/application/iac/helm/postgres/Chart.yaml`, `microservices/application/iac/helm/postgres/values.yaml`, `microservices/application/iac/helm/shell-app/Chart.yaml`, `microservices/application/iac/helm/shell-app/templates/deployment.yaml`; +4 more.
- Catalog crates/components: `microservices/application/catalog/oya-application-auth-gateway-adapter-oidc.yaml`, `microservices/application/catalog/oya-application-auth-gateway-adapter-saml.yaml`, `microservices/application/catalog/oya-application-auth-gateway-adapter.yaml`, `microservices/application/catalog/oya-application-auth-gateway-api.yaml`, `microservices/application/catalog/oya-application-auth-gateway-app.yaml`, `microservices/application/catalog/oya-application-auth-gateway-domain.yaml`; +18 more.
- Example: `module-load` API runs as a Kata-isolated container; scheduled/background work for `application` uses a separate worker principal.
...
## §observability
This anchor is closed for `application` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/application/slos/audit-seal.openslo.yaml`, `microservices/application/slos/module-load.openslo.yaml`, `microservices/application/slos/oidc-signin.openslo.yaml`, `microservices/application/slos/route-resolve.openslo.yaml`, `microservices/application/slos/tti.openslo.yaml`, `microservices/application/dashboards/module-load-success.json`; +2 more.
- Audit classes use prefix `oya.application.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `application_module_load_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `application_module_load_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/application/slos/audit-seal.openslo.yaml`.
...
- Example trace: `application.module-load` -> `policy.evaluate` -> `storage.module_load` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `application` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `application` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `module-load` action class.
...
- Cedar/IaC controls in scope: `microservices/application/policy/auditor-scope.cedar`, `microservices/application/policy/ci-scope.cedar`, `microservices/application/policy/data-residency.md`, `microservices/application/policy/public-read.cedar`, `microservices/application/policy/route-isolation.md`, `microservices/application/policy/tenant-admin-console.cedar`; +11 more.
- Canary payloads for `application` use fake `module-load` identifiers and honey endpoints under `/.well-known/oya-canary/application`.
...
This anchor is closed for `application` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `application` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `application` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `application` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `module-load` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `application` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `application` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `application` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/application/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/application/runbooks/auth-gateway-restart.md`, `microservices/application/runbooks/cdn-purge.md`, `microservices/application/runbooks/module-rollback.md`, `microservices/application/runbooks/session-storm.md`, `microservices/application/runbooks/tenant-context-recovery.md`, `microservices/application/runbooks/wasm-bundle-rebuild.md`.
- Example: `module-load` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
