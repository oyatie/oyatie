---
doc_class: Runbook
doc_id: RUNBOOK-ONTOLOGY-HOT_SPLIT
...
# Runbook: Ontology Hot Split

## Operator Contract
- Runbook id: ontology-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ontology-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ontology; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n ontology rollout status deploy/ontology --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service ontology --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service ontology --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service ontology --cell $CELL --window 30m`.
