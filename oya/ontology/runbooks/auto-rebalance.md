---
doc_class: Runbook
doc_id: RUNBOOK-ONTOLOGY-AUTO_REBALANCE
...
# Runbook: Ontology Auto Rebalance

## Operator Contract
- Runbook id: ontology-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ontology-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ontology; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n ontology rollout status deploy/ontology --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service ontology --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service ontology --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service ontology --cell $CELL --window 30m`.
