...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=ontology-cross-tenant-leak-recovery --production-snapshot` exits non-zero against the latest production evidence bundle.
...
15. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=ontology-cross-tenant-leak-recovery --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=ontology-cross-tenant-leak-recovery --fixture incident-cross-tenant-leak-recovery.json`.
