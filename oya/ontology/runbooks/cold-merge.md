---
doc_class: Runbook
doc_id: RUNBOOK-ONTOLOGY-COLD_MERGE
...
# Runbook: Ontology Cold Merge

## Operator Contract
- Runbook id: ontology-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ontology-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ontology; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n ontology rollout status deploy/ontology --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service ontology --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service ontology --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service ontology --cell $CELL --window 30m`.
