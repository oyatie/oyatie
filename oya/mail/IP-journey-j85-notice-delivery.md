...
ip_id: IP-journey-j85-notice-delivery
...
# IP-journey-j85-notice-delivery - mail

## Goal

Implement the `notice-delivery` slice for `mail` so j85 can satisfy `HIPAA-2024` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/mail/contracts/openapi/j85-notice-delivery-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/mail/contracts/asyncapi/j85-notice-delivery-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/mail/contracts/proto/j85-notice-delivery-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/mail/policy/j85-notice-delivery.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/mail/runbooks/j85-notice-delivery-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/mail/tests/j85_notice_delivery_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J85MailNoticeDeliveryStarted`.
- `J85MailNoticeDeliveryPermitted`.
- `J85MailNoticeDeliveryDenied`.
- `J85MailNoticeDeliveryCommitted`.
- `J85MailNoticeDeliveryRolledBack`.

## Implementation rows

### IP row 001 - mail notice-delivery
Scope: implement one mail behavior for j85, not a cross-service refactor.
...
- Surface evidence: `microservices/mail/IP-journey-j85-notice-delivery.md` matched `openapi, asyncapi, .proto`; contract files `microservices/mail/contracts/openapi/mail.yaml, microservices/mail/contracts/asyncapi/mail-events.yaml, microservices/mail/contracts/proto/mail.proto`; type anchor `crates/oya-shared-email-comms-kernel/src/lib.rs::OutboundMessage`.
