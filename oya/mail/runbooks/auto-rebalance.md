---
doc_class: Runbook
doc_id: RUNBOOK-MAIL-AUTO_REBALANCE
microservice: mail
...
# Runbook: Mail Auto Rebalance

## Operator Contract
- Runbook id: mail-auto-rebalance.
- Primary service namespace: `mail`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-mail-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=mail; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n mail rollout status deploy/mail --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service mail --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service mail --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service mail --cell $CELL --window 30m`.
