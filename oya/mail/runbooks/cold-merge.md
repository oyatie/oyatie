---
doc_class: Runbook
doc_id: RUNBOOK-MAIL-COLD_MERGE
microservice: mail
...
# Runbook: Mail Cold Merge

## Operator Contract
- Runbook id: mail-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-mail-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=mail; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n mail rollout status deploy/mail --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service mail --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service mail --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service mail --cell $CELL --window 30m`.
