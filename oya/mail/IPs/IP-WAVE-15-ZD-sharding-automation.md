...
bounded_context: dual-context-isolation
...
---
# IP-WAVE-15-ZD-sharding-automation: Mail Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/mail/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: mail must interpret ADR-0348 through its own bounded context: dual-context-isolation.
SCOPE-004: mail uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: mail uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: mail uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
...
STANCE-001: Microservice: mail.
STANCE-002: Owner team: axis-mail.
...
STANCE-005: Bounded context anchor: dual-context-isolation.
STANCE-006: Cell placement class input: Tier-3.
STANCE-007: Capacity scaling dimension input: per_message.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, gdpr, kr-pipa, hipaa, soc2, iso27001, NIS2, lgpd, SOC 2 Type 2, kr-isms-p, eu-ai-act, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for mail.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for mail because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; mail must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where mail participates.
...
ROLLBACK-002: For mail, rollback starts from the audit-chain row that recorded the automation event.
