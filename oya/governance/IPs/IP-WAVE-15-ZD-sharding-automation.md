...
bounded_context: aggregation-indexer
...
slot_owner: ZF-9
sharding_role: lane-owner
---
# IP-WAVE-15-ZD-sharding-automation: Governance Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/governance/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: governance must interpret ADR-0348 through its own bounded context: aggregation-indexer.
SCOPE-004: governance uses ADR-0513 as the Buck2/Prow verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: governance uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: governance uses ADR-0513 and ADR-0516 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors native oya-ci and temporary GitHub shadow surfaces.
SCOPE-007: This file records the rollback_path required by ADR-0348's IP-level reversibility lane.
SCOPE-008: The plan is accepted only when the file remains at least 150 lines and cites ADR-0513, ADR-0516, ADR-0347, and ADR-0348 by exact ID.

## 2. Microservice Stance
STANCE-001: Microservice: governance.
STANCE-002: Owner team: axis-governance.
STANCE-003: Sharding role: lane-owner.
STANCE-004: Role statement: owns the governance lane vocabulary and the report-only to blocker promotion checks.
STANCE-005: Bounded context anchor: aggregation-indexer.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_workflow_run.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, SOC 2 Type 2, iso27001, gdpr, soc2, kr-isms-p, hipaa, lgpd, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for governance.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for governance because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; governance must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where governance participates.
...
STANCE-018: CI stance is ADR-0513 Buck2/Prow verification before push for downstream code, schema, or workflow changes.
STANCE-019: CI/CD substrate stance is ADR-0513 plus ADR-0516: Prow/Kubernetes-native oya-ci is authority; GitHub Actions is a temporary lane-unlocker/shadow adapter only.
...
ADR346-SUPERSEDED-BY-ADR0513-001: Buck2 is the canonical local and CI build/test/check authority.
ADR346-SUPERSEDED-BY-ADR0513-002: Prow/Kubernetes-native oya-ci publishes the trusted required status; GitHub Actions remains temporary shadow evidence.
ADR346-SUPERSEDED-BY-ADR0513-003: Rust package manifests remain compatibility metadata only; they do not outrank Buck2 targets.
ADR346-SUPERSEDED-BY-ADR0513-004: LLVM source-based coverage is emitted through Buck2/Prow lanes rather than Tarpaulin or local package-manager tooling.
ADR346-ENFORCED-BY-001: buck2-authority-policy-check refuses guidance that makes package-manager commands merge authority.
ADR346-ENFORCED-BY-002: repo-hygiene-automation-check refuses retired local CLI and external CI/CD substrate command authority in cleaned surfaces.
ADR346-ENFORCED-BY-003: quality-lane-registry-authority-check keeps governance lane registry authority in Buck2/Prow.
...
## 6. Canonical ADR-0513 / ADR-0516 Wording
ADR0513-PURPOSE-001: Buck2 is the canonical build/test/check authority for the Oyatie corpus.
ADR0513-PURPOSE-002: Prow/Kubernetes-native oya-ci publishes the trusted required status and executes stateless Buck2 jobs with immutable evidence artifacts.
ADR0513-PURPOSE-003: Native CD consumes signed evidence and release-conveyor records; manual cluster mutation is not a deployment authority path.
ADR0516-PURPOSE-001: GitHub Actions is temporary lane-unlocker/shadow evidence while native SCM/CI/CD seams mature.
ADR0516-PURPOSE-002: GitHub repositories are publication/collaboration adapters, not permanent first-class SCM authority.
ADR0513-ENFORCED-BY-001: buck2-authority-policy-check refuses non-Buck2 command authority in active guidance.
ADR0513-ENFORCED-BY-002: oya-ci-prowjob-registry-check keeps native oya-ci jobs registered and reviewable.
ADR0513-ENFORCED-BY-003: repo-hygiene-automation-check refuses retired substrate residue in cleaned surfaces.
ADR0516-ENFORCED-BY-001: github-lane-unlocker-bridge-check keeps hosted CI/CD compatibility shadow-only.
...
PLAN-024: Use ADR-0513 Buck2/Prow verification before any downstream push.
PLAN-025: Use ADR-0347 governance lane names in downstream evidence.
PLAN-026: Use ADR-0513 native oya-ci parity when self-hosted CI is introduced.
PLAN-027: Use ADR-0513 signed-release evidence when deployment manifests are introduced.
...
ROLLBACK-002: For governance, rollback starts from the audit-chain row that recorded the automation event.
...
VERIFY-001: Static read confirms this file cites ADR-0513 by exact ID.
...
VERIFY-004: Static read confirms this file cites ADR-0516 by exact ID.
VERIFY-005: Static read confirms at least one ADR-0513 enforced_by lane appears.
...
VERIFY-008: Static read confirms at least one ADR-0516 enforced_by lane appears.
...
VERIFY-016: Downstream implementation must run ADR-0513 Buck2/Prow target evidence before push.
...
VERIFY-024: Downstream implementation must prove native CD emits deploy audit rows when deployment surfaces land.
VERIFY-025: Downstream implementation must prove GitHub shadow compatibility when hosted CI remains in use.
...
ACCEPT-014: The file cites Buck2/Prow expectations from ADR-0513.
ACCEPT-015: The file cites GitHub shadow compatibility expectations from ADR-0516.
