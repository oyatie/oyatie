---
doc_class: Runbook
doc_id: RUNBOOK-GOVERNANCE-HOT_SPLIT
...
# Runbook: Governance Hot Split

## Operator Contract
- Runbook id: governance-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-governance-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=governance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n governance rollout status deploy/governance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service governance --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service governance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service governance --cell $CELL --window 30m`.
