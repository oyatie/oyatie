...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=governance-lane-failure-triage --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=governance-lane-failure-triage --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=governance-lane-failure-triage --fixture incident-lane-failure-triage.json`.
...
- Governance control-plane evidence: inspect lane_failure_triage invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
