---
doc_class: Runbook
doc_id: RUNBOOK-GOVERNANCE-COLD_MERGE
...
# Runbook: Governance Cold Merge

## Operator Contract
- Runbook id: governance-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-governance-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=governance; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n governance rollout status deploy/governance --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service governance --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service governance --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service governance --cell $CELL --window 30m`.
