...
- Trigger from CI when `buck2 build //:quality-lane-registry-authority-check # lane=governance-consent-collection-pipeline-failure --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `buck2 build //:quality-lane-registry-authority-check # lane=governance-consent-collection-pipeline-failure --production-snapshot --cell $CELL`.
...
7. Add gate evidence: `buck2 build //:quality-lane-registry-authority-check # lane=governance-consent-collection-pipeline-failure --fixture incident-consent-collection-pipeline-failure.json`.
...
- Governance control-plane evidence: inspect consent_collection_pipeline_failure invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
