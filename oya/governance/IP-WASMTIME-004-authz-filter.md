...
ip_id: IP-WASMTIME-004-authz-filter
status: planned
...
# Authz filter at gateway

## A. Problem

The previous slice for `IP-WASMTIME-004-authz-filter` was too close to a design-anchor shell: it named the intended control but did not bind the work to governance's actual contracts, policy files, SLOs, and runbooks. This IP closes the `Cedar authorization before upstream dispatch` gap for the governance µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
Technical target set: policy/tenant-scope.cedar, policy/ci-scope.cedar, contracts/proto/governance.proto, runbooks/cedar-policy-rollback-protocol.md. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `Cedar authorization before upstream dispatch` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
5. Wire dashboard/SLO/runbook evidence from policy/tenant-scope.cedar, policy/ci-scope.cedar, contracts/proto/governance.proto, runbooks/cedar-policy-rollback-protocol.md; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: policy/tenant-scope.cedar, policy/ci-scope.cedar, contracts/proto/governance.proto, runbooks/cedar-policy-rollback-protocol.md.
...
|---|---|---|
| AWS Verified Permissions and GitHub branch protection | Mature external control surface to compare against. | AWS Verified Permissions and GitHub branch protection are counterpart authorization surfaces; Oyatie pushes deny evidence into the audit chain. |
...
1. Evaluate `tenant-scope.cedar` before request dispatch and return a structured 403 with `cedar_decision_id` on forbid.
2. Cache policy bundles for 30 seconds only when the bundle carries a valid governance signature and expiry.
3. Emit allow/deny traces into governance events without storing raw tenant secrets or request bodies.
4. On policy fetch failure, fail closed with 503 and alert through lane-failure triage rather than falling back to upstream auth.
5. Use AWS Verified Permissions as the Cedar counterpart and GitHub branch protection as the required-check analogy.
6. Test fuel exhaustion, stale policy version, missing principal, cross-tenant read, and happy-path permit.
...
- Hook 2.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 2.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 2.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 2.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 2.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 2.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 2.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
