...
8. **Cloud-ci conformance ratchet = CI-product invariant:** keep it canonical/configurable/reusable. Gate predicate `.rs` sources + `oya-ci.toml` + `oya-ci-config` crate + workflow `.yml` must stay byte-unchanged unless deliberately changing the product. Generated JSON faces are CI/controller outputs, not contributor merge surfaces: do not commit add/modify changes to `*.generated.json`; `infra/ci/materialize-cloud-ci-generated-faces.sh` materializes them before gates consume them, and `generated-output-diff-policy` rejects generated add/modify PR surfaces while allowing generated-output deletion retirements.
...
- [ ] **Root-surface cleanup:** 2026-06-09 audit says keep `.config/nextest.toml` (still consumed by tracked nextest/pre-push recovery checks), `bin/oya`, `plan/`, `.envrc`, `.gitattributes`, and `ADR-INVENTORY.tsv`; `.claire`, root `test/`, and root `tests/` remain absent; `.omc/` and `.omx/` durables are local-only and must stay gitignored; live planning authority now points at tracked `/specs`/`/registry`/`/evidence`/`/templates` surfaces instead of `.omc`/`.omx`; `platforms/BUCK` is orphaned from `.buckconfig` and removed as root cleanup.
