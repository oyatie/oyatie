...
use governance_adapter_with_no_importer_kernel::{
...
fn usage() -> String {
    "usage: governance-adapter-with-no-importer-app [--root PATH]...".into()
}
...
mod tests {
    ...
    fn extracts_package_name() {
        let manifest = r#"
[package]
name = "governance-adapter-with-no-importer-kernel"
edition.workspace = true
"#;
        assert_eq!(
            extract_package_name(manifest).as_deref(),
            Some("governance-adapter-with-no-importer-kernel")
        );
    }
