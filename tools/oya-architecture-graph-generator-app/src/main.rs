...
//!
//! Usage:
//!   oya-architecture-graph-generator [--write|--check]
...
use oya_architecture_graph_generator_app::{
    DEFAULT_GRAPH_SSOT, DEFAULT_MASTERPLAN, DEFAULT_OUTPUT, DEFAULT_TEMPLATE, RunOutcome, run,
};

const USAGE: &str = "usage: oya-architecture-graph-generator [--write|--check] \
...
fn main() -> ExitCode {
    ...
    match run(
        ...
        Ok(RunOutcome::Drifted { committed_path }) => {
            ...
            eprintln!("  run `oya-architecture-graph-generator --write` to regenerate it");
            ExitCode::FAILURE
        }
        Err(error) => {
