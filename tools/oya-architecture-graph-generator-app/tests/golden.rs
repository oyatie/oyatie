...
//! Its path is supplied via the `OYA_ARCH_GRAPH_GOLDEN` env var, defaulting to
...
fn golden_path() -> PathBuf {
    std::env::var("OYA_ARCH_GRAPH_GOLDEN")
        .map(PathBuf::from)
...
fn load_golden() -> Option<Value> {
    ...
    if !path.exists() {
        eprintln!(
            "SKIP: golden reference not found at {} (set OYA_ARCH_GRAPH_GOLDEN to override)",
            path.display()
        );
        return None;
    }
