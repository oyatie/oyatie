//! `oya-vcs-merge-queue-fix-loop-app` binary entrypoint.
//!
...
use oya_vcs_merge_queue_fix_loop_app::{
...
fn main() -> ExitCode {
    ...
    match run(&args, &RealFs) {
        ...
        Err(e) => {
            eprintln!("oya-vcs-merge-queue-fix-loop-app failed: {e}");
            ExitCode::FAILURE
        }
    }
}
...
pub fn run(args: &[String], fs_io: &dyn FilesystemIo) -> Result<String, String> {
        ...
        match event.kind {
            ...
            AdmissionEventKind::PrReviewFixRequested => {
                ...
                if let Err(e) = scheduler.admit(event.pr_number, event.changeset_id.clone()) {
                    ...
                    if !matches!(
                        e,
                        oya_vcs_merge_queue_fix_loop_app::SchedulerError::PrAlreadyAdmitted(_)
                    ) {
    ...
    while tick_actions < opts.max_ticks {
        ...
        tick_actions += 1;
        if matches!(action, oya_vcs_merge_queue_fix_loop_app::TickAction::Idle) {
            break;
        }
    }
...
fn usage() -> String {
    "usage: oya-vcs-merge-queue-fix-loop-app \\\n\
