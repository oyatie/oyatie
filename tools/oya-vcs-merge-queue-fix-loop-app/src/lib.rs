//! `oya-vcs-merge-queue-fix-loop-app` — merge-queue integration
//! layer (M01-P17-IP-006).
//!
//! ## What this crate does
//!
//! This is the BINARY-side integration layer for the merge-queue scheduler
//! that lives in `oya-vcs-review-mergequeue-kernel`. The kernel is
...
//!   `tools/oya-intelligence-pr-review-dispatcher-app` per IP-004. Schema lives
...
//!
//! Same caveat as IP-005's `oya-vcs-ci-fix-loop-dispatcher-app`:
...
pub use oya_vcs_review_mergequeue_kernel::parked_state::ParkedReason;
pub use oya_vcs_review_mergequeue_kernel::pr_retry_budget::{BudgetVerdict, MAX_ATTEMPTS_PER_PR};
pub use oya_vcs_review_mergequeue_kernel::scheduler::{
