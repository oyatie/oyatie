...
use governance_banned_primitives_kernel::{
...
fn usage() -> String {
    "usage: governance-banned-primitives-app [--root PATH ...] [--known-rationale ICM_ID ...]"
        .into()
}
...
mod tests {
    use super::*;
    use governance_banned_primitives_kernel::PrimitiveKind;
    ...
    fn ignores_sanctioned_grit_word() {
        let audit = scan_agent_instruction_file(
            "AGENTS.md",
            "<!-- agent-instructions:start -->\ngrit claim and tooling-agent-read log\n<!-- agent-instructions:end -->",
        )
        .expect("audit succeeds");
