//! # fabric-loop-state-app — ADR-0516 fabric two-plane drive-loop state
//!
...
pub const CUTOVER_TARGET_SERVICE_NAME: &str = "cloud-ci-loop-state-service";
...
pub const CUTOVER_TARGET_DESTINATION_HOME: &str =
    "cloud/cloud-ci/crates/cloud-ci-loop-state-app";
...
pub const DEFAULT_OPERATIONAL_PLANE_ROOT: &str = ".loop-state";
...
pub struct CutoverTarget {
    /// Deployed service identity (`cloud-ci-loop-state-service`).
...
mod tests {
    ...
    fn cutover_target_constants_are_the_canonical_record() {
        ...
        assert_eq!(target.service_name, "cloud-ci-loop-state-service");
