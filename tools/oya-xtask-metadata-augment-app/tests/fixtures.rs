...
"#;
    ...
    let name = doc["dependencies"]["oya-platform-tenant-kernel"]
        .as_str()
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let path = doc["dependencies"]["oya-platform-tenant-kernel"]["path"]
        .as_str()
        .expect("path value");
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let ws = doc["dependencies"]["oya-platform-tenant-kernel"]["workspace"]
        .as_bool()
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let opt = doc["dependencies"]["oya-platform-tenant-kernel"]["optional"]
        .as_bool()
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let name = doc["dev-dependencies"]["oya-platform-tenant-kernel"]
        .as_str()
        .expect("bare string");
    assert_eq!(name, "0.1.0");
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let path = doc["dev-dependencies"]["oya-platform-tenant-kernel"]["path"]
        .as_str()
        .expect("path");
    assert_eq!(path, "../oya-platform-tenant-kernel");
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let ws = doc["dev-dependencies"]["oya-platform-tenant-kernel"]["workspace"]
        .as_bool()
        .expect("workspace");
    assert!(ws);
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let opt = doc["dev-dependencies"]["oya-platform-tenant-kernel"]["optional"]
        .as_bool()
        .expect("optional");
    assert!(opt);
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let v = doc["build-dependencies"]["oya-platform-tenant-kernel"]
        .as_str()
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let path = doc["build-dependencies"]["oya-platform-tenant-kernel"]["path"]
        .as_str()
        .expect("path");
    assert_eq!(path, "../oya-platform-tenant-kernel");
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let ws = doc["build-dependencies"]["oya-platform-tenant-kernel"]["workspace"]
        .as_bool()
        .expect("workspace");
    assert!(ws);
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let opt = doc["build-dependencies"]["oya-platform-tenant-kernel"]["optional"]
        .as_bool()
        .expect("optional");
    assert!(opt);
}
...
"#;
    let doc: DocumentMut = input.parse().expect("parses");
    let v = doc["target"]["cfg(unix)"]["dependencies"]["oya-platform-tenant-kernel"]
        .as_str()
        .expect("version string");
    assert_eq!(v, "0.1.0");
}
