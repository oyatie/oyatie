...
//
// Canonical order is Cargo's: packages ordered by `name` (byte `Ord`), ties
// broken by version then source. We reproduce it with a STABLE sort by name
// only: entries with distinct names order by name (matching Cargo); entries
// that share a name (multi-version crates) inherit their existing relative
// order, which — because the input lockfile was itself Cargo-canonical — is
// already Cargo's (name, semver, source) order. Same reasoning applies to the
// dependency arrays. This needs neither a semver dependency nor a version
// parser, and reproduces Cargo's output byte-for-byte.
...
";
        ...
        let rename = map(&[("oya-cloud-ci-zed-app", "ci-alpha-gate")]);
