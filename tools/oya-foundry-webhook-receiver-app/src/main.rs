...
use clap::Parser;
use oya_foundry_webhook_receiver_app::{
...
/// CLI surface for `oya-foundry-webhook-receiver-app`.
#[derive(Debug, Parser)]
#[command(
    name = "oya-foundry-webhook-receiver-app",
...
fn main() -> ExitCode {
    ...
    let dispatch = match Dispatch::from_paths_or_optional(&paths) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("oya-foundry-webhook-receiver-app: failed to load dispatch state: {e}");
    ...
    let bind: SocketAddr = match cli.bind.parse() {
        ...
        Err(e) => {
            eprintln!(
                "oya-foundry-webhook-receiver-app: --bind {:?} is not a valid socket address: {e}",
                cli.bind
            );
            return ExitCode::from(2);
        }
...
fn run_simulate(dispatch: &Dispatch, path: &Path, skip_hmac: bool) -> ExitCode {
    match process_simulated_delivery(dispatch, path, skip_hmac) {
        ...
        Err(err) => {
            eprintln!("oya-foundry-webhook-receiver-app: {err}");
            ExitCode::from(2)
        }
    }
}
...
fn run_server(dispatch: Dispatch, bind: SocketAddr, skip_hmac: bool) -> ExitCode {
        ...
        .build()
    {
        Ok(r) => r,
        Err(e) => {
            eprintln!("oya-foundry-webhook-receiver-app: failed to start tokio runtime: {e}");
            return ExitCode::from(2);
        }
    };
    ...
    runtime.block_on(async move {
        let listener = match tokio::net::TcpListener::bind(bind).await {
            Ok(l) => l,
            Err(e) => {
                eprintln!("oya-foundry-webhook-receiver-app: failed to bind {bind}: {e}");
                return ExitCode::from(2);
            }
        };
        eprintln!("oya-foundry-webhook-receiver-app: listening on http://{bind}/webhook/github");
        if let Err(e) = axum::serve(listener, app).await {
            eprintln!("oya-foundry-webhook-receiver-app: server error: {e}");
            return ExitCode::from(2);
        }
        ExitCode::SUCCESS
    })
