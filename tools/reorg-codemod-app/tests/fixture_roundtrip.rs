...
use reorg_codemod_app::model::{CrateMove, MovePlan};
use reorg_codemod_app::oracle;
use reorg_codemod_app::plan::{ApplyOptions, apply_plan};
...
fn validation_failure_preserves_the_entire_tree_snapshot() {
    ...
    let plan = MovePlan {
        ...
        moves: vec![CrateMove {
            old_path: "crates/widget".to_owned(),
            new_path: "widget/core/widget".to_owned(),
            old_cargo_name: "widget".to_owned(),
...
///   crates/cap-core/      (engine)   depends on libs/shared-kernel via ../../libs/...
///   crates/cap-app/       (facade)   depends on cap-core (sibling, ../cap-core)
///                                          AND on libs/shared-kernel via ../../libs/...
///   crates/bystander/     (stays put — keeps crates/* non-empty so no members rewrite)
///   libs/shared-kernel/   (stays put)
///
...
fn build_fixture(root: &Path) {
    ...
    // crates/bystander (does not move; proves untouched crates stay byte-identical).
    w(
        root,
        "crates/bystander/Cargo.toml",
        "[package]\nname = \"bystander\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
    );
    w(
        root,
        "crates/bystander/src/lib.rs",
        "pub fn bystand() {}\n",
    );

    // libs/shared-kernel (does not move)
    w(
        root,
        "libs/shared-kernel/Cargo.toml",
        "[package]\nname = \"shared-kernel\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
    );
    w(
        root,
        "libs/shared-kernel/src/lib.rs",
        "pub fn root() {}\n",
    );
    w(
        root,
        "libs/shared-kernel/BUCK",
        "rust_library(\n    name = \"shared-kernel\",\n    crate = \"shared_kernel\",\n    crate_root = \"src/lib.rs\",\n    srcs = [\"src/lib.rs\"],\n)\n",
    );

    // crates/cap-core (engine) -> moves to cap/core/cap-core
    w(
        root,
        "crates/cap-core/Cargo.toml",
        "[package]\nname = \"cap-core\"\nversion = \"0.1.0\"\nedition = \"2021\"\n\n[lib]\nname = \"cap_core\"\npath = \"src/lib.rs\"\n\n[dependencies]\noya-shared-kernel = { path = \"../../libs/shared-kernel\" }\n",
    );
    w(
        root,
        "crates/cap-core/src/lib.rs",
        "use shared_kernel::root;\npub fn engine() { root(); }\n",
    );
    w(
        root,
        "crates/cap-core/BUCK",
        "rust_library(\n    name = \"cap-core\",\n    crate = \"cap_core\",\n    crate_root = \"src/lib.rs\",\n    srcs = [\"src/lib.rs\"],\n    deps = [\"//libs/shared-kernel:shared-kernel\"],\n)\n",
    );

    // crates/cap-app (facade) -> moves to cap/facade/cap-app
    w(
        root,
        "crates/cap-app/Cargo.toml",
        "[package]\nname = \"cap-app\"\nversion = \"0.1.0\"\nedition = \"2021\"\n\n[lib]\nname = \"cap_app\"\npath = \"src/lib.rs\"\n\n[dependencies]\noya-cap-core = { path = \"../cap-core\" }\noya-shared-kernel = { path = \"../../libs/shared-kernel\" }\n",
    );
    w(
        root,
        "crates/cap-app/src/lib.rs",
        "use cap_core::engine;\nuse shared_kernel::root;\npub fn facade() { engine(); root(); }\n",
    );
    w(
        root,
        "crates/cap-app/BUCK",
        "rust_library(\n    name = \"cap-app\",\n    crate = \"cap_app\",\n    crate_root = \"src/lib.rs\",\n    srcs = [\"src/lib.rs\"],\n    deps = [\n        \"//crates/cap-core:cap-core\",\n        \"//libs/shared-kernel:shared-kernel\",\n    ],\n)\n",
    );
}
...
fn capability_plan() -> MovePlan {
    MovePlan {
        ...
        moves: vec![
            CrateMove {
                old_path: "crates/cap-core".to_string(),
                new_path: "cap/core/cap-core".to_string(),
                old_cargo_name: "cap-core".to_string(),
                new_cargo_name: "cap-core".to_string(),
            },
            CrateMove {
                old_path: "crates/cap-app".to_string(),
                new_path: "cap/facade/cap-app".to_string(),
                old_cargo_name: "cap-app".to_string(),
...
fn forward_move_recomputes_paths_and_resolves() {
    ...
    assert!(!root.join("crates/cap-core").exists());
    assert!(!root.join("crates/cap-app").exists());
    ...
    // (3) the deep move-fatal path-dep RECOMPUTED: cap/core/cap-core -> libs/shared-kernel
    //     = ../../../libs/shared-kernel
    assert!(
        core_manifest.contains("path = \"../../../libs/shared-kernel\""),
    ...
    assert!(!app_manifest.contains("cap-core ="));
    assert!(
        app_manifest.contains("path = \"../../core/cap-core\""),
        "recomputed sibling path: {app_manifest}"
    );
    assert!(
        app_manifest.contains("path = \"../../../libs/shared-kernel\""),
    ...
    assert!(!app_src.contains("cap_core"));
    ...
    assert!(app_buck.contains("//libs/shared-kernel:shared-kernel"));
...
fn dry_run_passes_a_clean_capability_move_without_landing() {
    ...
    assert!(root.join("crates/cap-core").is_dir());
...
fn dry_run_fails_a_move_that_would_break_resolution() {
    ...
    assert!(root.join("crates/cap-app").is_dir());
    assert!(!root.join("cap").exists());
