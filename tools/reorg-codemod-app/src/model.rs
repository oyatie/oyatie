...
/// cloud-iam-domain` -> `iam/core/identity-domain`). Cargo names are kebab package names
/// (e.g. `cloud-iam-domain` -> `identity-domain`); the snake crate name (`identity_domain`)
...
pub const REORG_MOVE_MANIFEST_SCHEMA: &str = "ci/reorg-move-manifest/v1";
...
/// Convert a kebab cargo/package name to the snake crate identifier (`foo-bar` ->
/// `foo_bar`). This mirrors Cargo's own `[lib].name` defaulting and the `extern crate`
...
mod tests {
    ...
    fn snake_only_converts_dashes() {
        assert_eq!(snake("cloud-iam-domain"), "cloud_iam_domain");
    ...
    fn join_rel_normalizes_dotdot() {
        assert_eq!(
            join_rel("cloud/cloud-iam/crates/iam", "../domain").as_deref(),
            Some("cloud/cloud-iam/crates/domain")
        );
        assert_eq!(
            join_rel(
    ...
    fn recompute_deep_path_dep_to_libs() {
        // The move-fatal class: ../../../../libs/kernel from a deep crate.
        let new = recompute_rel_path_dep(
            "cloud/cloud-iam/crates/iam-app",
            "iam/facade/iam-app",
            "../../../../libs/kernel",
            &|_old| None,
        )
        .unwrap();
        // iam/facade/iam-app -> libs/kernel
        assert_eq!(new, "../../../libs/kernel");
        assert_eq!(
    ...
    fn inverse_swaps_every_side() {
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "cloud/cloud-iam/crates/iam".to_string(),
                new_path: "iam/core/iam".to_string(),
                old_cargo_name: "cloud-iam".to_string(),
        ...
        assert_eq!(inv.moves[0].new_path, "cloud/cloud-iam/crates/iam");
        assert_eq!(inv.moves[0].old_cargo_name, "iam-core");
        assert_eq!(inv.moves[0].new_cargo_name, "cloud-iam");
    ...
    fn validate_rejects_branded_crate_move_targets() {
        let path_plan = MovePlan {
            capability: "iam".to_string(),
            moves: vec![CrateMove {
                old_path: "cloud/cloud-iam/crates/cloud-iam-domain".to_string(),
                new_path: "oya/identity/crates/identity-domain".to_string(),
                old_cargo_name: "cloud-iam-domain".to_string(),
        ...
        assert!(matches!(
            ...
            Err(CodemodError::DeprecatedBrandTarget { which, value })
                if which == "new_path" && value == "oya/identity/crates/identity-domain"
        ));
        ...
        assert!(matches!(
            ...
            Err(CodemodError::DeprecatedBrandTarget { which, value })
                if which == "new_cargo_name" && value == "identity-domain"
        ));
    }
    ...
    fn validate_rejects_branded_artifact_targets() {
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "oya/calendar/crates/calendar-domain".to_string(),
            ...
            artifacts: vec![ArtifactMove {
                ...
                new_path: "registry/catalog/calendar-domain.yaml".to_string(),
            }],
        };
        assert!(matches!(
            plan.validate(),
            Err(CodemodError::DeprecatedBrandTarget { which, value })
                if which == "artifact new_path" && value == "registry/catalog/calendar-domain.yaml"
        ));
    ...
    fn validate_allows_debranded_targets_that_keep_cloud_as_a_non_prefix_descriptor() {
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "cloud/cloud-marketplace/crates/cloud-marketplace-domain".to_string(),
                new_path: "marketplace/core/cloud-domain".to_string(),
                old_cargo_name: "cloud-marketplace-domain".to_string(),
                new_cargo_name: "marketplace-cloud-domain".to_string(),
            }],
            artifacts: vec![ArtifactMove {
                old_path: "registry/catalog/cloud-marketplace-domain.yaml".to_string(),
    ...
    fn file_level_manifest_derives_from_candidate_new_descendants_mapping_back_to_old() {
        ...
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "cloud/cloud-observability/crates/cloud-observability-domain"
                    .to_string(),
                new_path: "observability/core/aggregate".to_string(),
                old_cargo_name: "cloud-observability-domain".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                (
                    "cloud/cloud-observability/crates/cloud-observability-domain/Cargo.toml"
                        .to_string(),
                    "observability/core/aggregate/Cargo.toml".to_string()
                ),
                (
                    "cloud/cloud-observability/crates/cloud-observability-domain/src/lib.rs"
                        .to_string(),
                    "observability/core/aggregate/src/lib.rs".to_string()
                ),
            ],
    ...
    fn crate_dir_pairs_emit_one_pair_per_landed_move() {
        let plan = MovePlan {
            ...
            moves: vec![
                ...
                CrateMove {
                    old_path: "cloud/cloud-observability/crates/cloud-observability-ghost"
                        .to_string(),
                    new_path: "observability/core/ghost".to_string(),
                    old_cargo_name: "cloud-observability-ghost".to_string(),
        ...
        assert_eq!(
            ...
            vec![(
                "cloud/cloud-observability/crates/cloud-observability-domain".to_string(),
    ...
    fn crate_ident_pairs_exposes_cargo_name_bijection() {
        let plan = MovePlan {
            ...
            moves: vec![
                ...
                CrateMove {
                    old_path: "cloud/cloud-iam/crates/cloud-iam-app".to_string(),
                    new_path: "iam/facade/iam-app".to_string(),
                    old_cargo_name: "cloud-iam-app".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ("cloud-iam-app".to_string(), "iam-app".to_string()),
                (
                    "cloud-iam-domain".to_string(),
    ...
    fn move_manifest_value_is_canonical_and_empty_for_no_move() {
        ...
        let full = move_manifest_value(
            ...
            &[("old".to_string(), "new-core".to_string())],
        );
        ...
        assert_eq!(full["crate_idents"][0]["old"], "old");
    ...
    fn mapping_emits_the_five_tuple() {
        ...
        assert_eq!(row.old_path, "cloud/cloud-iam/crates/iam");
        assert_eq!(row.new_path, "iam/core/iam");
        assert_eq!(row.old_cargo_name, "cloud-iam");
    ...
    fn obs_crate_move() -> CrateMove {
        CrateMove {
            old_path: "oya/observability/crates/observability-domain".to_string(),
            new_path: "observability/core/domain".to_string(),
            old_cargo_name: "observability-domain".to_string(),
    ...
    fn validate_rejects_artifact_new_path_colliding_with_another_artifact() {
        ...
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "oya/drive/crates/drive-domain".to_string(),
                new_path: "storage/core/drive-domain".to_string(),
                old_cargo_name: "drive-domain".to_string(),
    ...
    fn inverse_round_trips_artifacts() {
        let plan = MovePlan {
            ...
            artifacts: vec![
                ...
                ArtifactMove {
                    old_path: "registry/catalog/observability-domain.yaml".to_string(),
    ...
    fn rewrite_path_token_rewrites_a_nested_file_citation() {
        let text = "See `oya/intelligence/crates/intelligence-catalog-domain/src/lib.rs`, the graphql field.";
        let out = rewrite_path_token(
            text,
            "oya/intelligence/crates/intelligence-catalog-domain",
    ...
    fn rewrite_path_token_rewrites_a_bare_crate_dir_mention() {
        let text = "the crate at oya/intelligence/crates/intelligence-catalog-domain moved.";
        let out = rewrite_path_token(
            text,
            "oya/intelligence/crates/intelligence-catalog-domain",
            "intelligence/core/catalog-domain",
    ...
    fn rewrite_path_token_does_not_corrupt_a_longer_sibling_crate_name() {
        // `intelligence-catalog-domain-v2` is a DIFFERENT, unrelated crate that merely starts
        // with the same prefix; a naive substring replace would corrupt it. RIGHT-boundary proof.
        let text = "oya/intelligence/crates/intelligence-catalog-domain-v2/src/lib.rs";
        assert_eq!(
            rewrite_path_token(
                text,
                "oya/intelligence/crates/intelligence-catalog-domain",
    ...
    fn validate_accepts_new_path_that_merely_extends_an_old_path_as_a_glued_identifier() {
        let plan = MovePlan {
            ...
            moves: vec![
                CrateMove {
                    old_path: "libs/foo".to_string(),
                    new_path: "libs/foo".to_string() + "-shadow", // "libs/foo-shadow"
                    old_cargo_name: "foo".to_string(),
                    new_cargo_name: "foo-shadow".to_string(),
                },
                CrateMove {
                    old_path: "cloud/bar".to_string(),
                    new_path: "cap/bar".to_string(),
                    old_cargo_name: "bar".to_string(),
                    new_cargo_name: "bar".to_string(),
                },
            ],
            artifacts: vec![],
        };
        // "libs/foo-shadow" does NOT boundary-match "libs/foo" (glued identifier), so
    ...
    fn validate_rejects_a_plan_whose_new_path_contains_an_old_path_boundary_safe() {
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "libs/foo".to_string(),
                new_path: "libs/foo/nested".to_string(),
                old_cargo_name: "foo".to_string(),
        ...
        let plan = MovePlan {
            ...
            moves: vec![
                ...
                CrateMove {
                    ...
                    new_path: "libs/foo/relocated".to_string(),
                    old_cargo_name: "bar".to_string(),
