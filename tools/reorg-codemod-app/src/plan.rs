...
use workspace_members_kernel::{ResolveError, resolve_member_dirs_from_str};
...
/// ([`cargo_lock_transform_kernel::move_lockfile`]) — rename the `[[package]]` block + every
...
fn rewrite_cargo_lock(
    ...
    let rewritten = cargo_lock_transform_kernel::move_lockfile(
        &content,
        &rename_map,
        &cargo_lock_transform_kernel::GraphAdditions::empty(),
    )
...
fn compute_globs_to_prune(
    ...
    let entries =
        workspace_members_kernel::workspace_manifest_entries_from_str(root_manifest_text)
...
fn pattern_set_covers(root_manifest_text: &str, new_dir: &str) -> bool {
    let entries =
        match workspace_members_kernel::workspace_manifest_entries_from_str(root_manifest_text)
        {
            Ok(e) => e,
            Err(_) => return false,
        };
...
/// `ci.toml`'s `[justification].adr_dir` default (`docs/decisions`) — the two tools are
/// independent (this codemod carries no `ci_config_kernel` dependency) but conventionally
...
/// a moved crate's exact path (e.g. `oya/intelligence/crates/intelligence-catalog-domain/
...
mod tests {
    ...
    fn glob_matches_mirrors_kernel_semantics() {
        assert!(glob_matches("libs/oya-*", &["libs", "kernel"]));
        assert!(glob_matches(
            "cloud/*/crates/oya-*",
            &["cloud", "cloud-iam", "crates", "x"]
        ));
        assert!(!glob_matches("libs/oya-*", &["libs", "group", "x"]));
...
"#;
    ...
    fn build_adr_anchor_fixture(root: &Path) {
        ...
        wf(
            root,
            "crates/intelligence-catalog-domain/Cargo.toml",
            "[package]\nname = \"intelligence-catalog-domain\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        wf(
            root,
            "crates/intelligence-catalog-domain/src/lib.rs",
            "pub fn d() {}\n",
        );
        ...
        wf(
            ...
            "In `crates/intelligence-catalog-domain/src/lib.rs`, the \"graphql\" value is banned.\n",
        );
    ...
    fn adr_anchor_plan() -> MovePlan {
        MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "crates/intelligence-catalog-domain".to_string(),
                new_path: "intelligence/core/catalog-domain".to_string(),
                old_cargo_name: "intelligence-catalog-domain".to_string(),
    ...
    fn apply_rewrites_an_adr_doc_anchor_old_to_new() {
        ...
        assert!(
            !adr.contains("crates/intelligence-catalog-domain"),
        ...
        let plan = MovePlan {
            ...
            moves: vec![
                ...
                CrateMove {
                    ...
                    old_cargo_name: "a-nested".to_string(),
    ...
    fn write_nested_carveout(root: &Path) {
        ...
        wf(
            ...
            "[workspace]\nmembers = [\"crates/*\"]\nresolver = \"2\"\n\n[workspace.dependencies]\nhal = { package = \"cloud-kernel-hal-kernel\", path = \"crates/cloud-kernel-hal-kernel\" }\n",
        );
        wf(
            root,
            "cloud/cloud-kernel/crates/cloud-kernel-hal-kernel/Cargo.toml",
            "[package]\nname = \"cloud-kernel-hal-kernel\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        wf(
            root,
            "cloud/cloud-kernel/crates/cloud-kernel-hal-kernel/src/lib.rs",
            "pub fn hal() {}\n",
        );
        ...
        wf(
            root,
            "kernel/Cargo.toml",
            "[workspace]\nmembers = [\"core/*\"]\nresolver = \"2\"\n\n[workspace.dependencies]\nhal = { package = \"kernel-hal\", path = \"core/kernel-hal\" }\n",
        );
        wf(
            root,
            "kernel/core/kernel-hal/Cargo.toml",
            "[package]\nname = \"kernel-hal\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        wf(
            root,
            "kernel/core/kernel-hal/src/lib.rs",
            "pub fn hal() {}\n",
        );
    }
    ...
    fn apply_refuses_a_move_that_spans_cargo_workspaces() {
        ...
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "cloud/cloud-kernel/crates/cloud-kernel-hal-kernel".to_string(),
                new_path: "kernel/adapters/hal".to_string(),
                old_cargo_name: "cloud-kernel-hal-kernel".to_string(),
        ...
        assert!(
            root.join("cloud/cloud-kernel/crates/cloud-kernel-hal-kernel/Cargo.toml")
                .is_file(),
    ...
    fn nested_workspace_move_edits_the_nested_manifest_not_the_root() {
        ...
        let plan = MovePlan {
            capability: "kernel".to_string(),
            moves: vec![CrateMove {
                old_path: "kernel/core/kernel-hal".to_string(),
                new_path: "kernel/adapters/hal".to_string(),
                old_cargo_name: "kernel-hal".to_string(),
                new_cargo_name: "kernel-hal".to_string(),
            }],
            artifacts: vec![],
        };
    ...
    fn apply_refuses_a_move_whose_rust_source_escapes_the_crate_via_a_path_literal() {
        ...
        assert!(
            root.join("crates/a/src/lib.rs").is_file(),
            "source untouched"
        );
    ...
    fn apply_fails_before_mutation_when_workspace_member_resolution_fails() {
        ...
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "crates/widget".to_string(),
                new_path: "widget/core/widget".to_string(),
                old_cargo_name: "widget".to_string(),
        ...
        let crate_manifest_before =
            std::fs::read(root.join("crates/widget/Cargo.toml")).unwrap();
        ...
        assert!(root.join("crates/widget").is_dir());
    ...
    fn build_artifact_fixture(root: &Path) {
        ...
        wf(
            root,
            "crates/observability-domain/Cargo.toml",
            "[package]\nname = \"observability-domain\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        wf(
            root,
            "crates/observability-domain/src/lib.rs",
            "pub fn d() {}\n",
        );
    ...
    fn observability_plan_with_artifacts() -> MovePlan {
        MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "crates/observability-domain".to_string(),
    ...
    fn apply_co_moves_artifacts_and_records_them_in_dirs_moved() {
        ...
        assert!(
            !root
                .join("registry/catalog/observability-domain.yaml")
                .exists()
        );
        assert_eq!(
    ...
    fn manifest_files_list_carries_artifact_pairs_for_a_real_move() {
        ...
        assert_eq!(
            files.get("crates/observability-domain/src/lib.rs"),
        ...
        assert_eq!(
            files.get("registry/catalog/observability-domain.yaml"),
    ...
    fn apply_co_moves_artifact_nested_inside_moving_crate_dir() {
        ...
        wf(
            &root,
            "crates/svid-kernel/Cargo.toml",
            "[package]\nname = \"svid-kernel\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        wf(
            &root,
            "crates/svid-kernel/src/lib.rs",
            "pub fn f() {}\n",
        );
        // The SLO lives INSIDE the crate dir — exactly the iam svid-kernel scenario.
        wf(
            &root,
            "crates/svid-kernel/slos/svid-availability.openslo.yaml",
            "apiVersion: openslo/v1\nkind: SLO\n",
        );
        ...
        let plan = MovePlan {
            ...
            moves: vec![CrateMove {
                old_path: "crates/svid-kernel".to_string(),
                new_path: "iam/core/svid-kernel".to_string(),
                old_cargo_name: "svid-kernel".to_string(),
            ...
            artifacts: vec![ArtifactMove {
                old_path: "crates/svid-kernel/slos/svid-availability.openslo.yaml".to_string(),
        ...
        assert!(!root.join("crates/svid-kernel").exists());
        ...
        assert!(
            outcome.dirs_moved.contains(&(
                "crates/svid-kernel/slos/svid-availability.openslo.yaml".to_string(),
    ...
    fn apply_with_empty_artifacts_is_byte_identical_no_op_for_the_artifact_step() {
        ...
        assert!(
            root.join("registry/catalog/observability-domain.yaml")
                .is_file()
        );
        ...
        assert_eq!(
            outcome.dirs_moved,
            vec![(
                "crates/observability-domain".to_string(),
    ...
    /// The canonical (cargo-generated) lockfile the fixture starts from: `widget` is a member,
...
";
    ...
    /// After moving `widget` -> `zzz-widget`: the package block is RENAMED, the `alpha`
