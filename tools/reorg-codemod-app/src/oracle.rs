...
mod tests {
    ...
    fn make_cargo_fixture(root: &Path) {
        ...
        write(root, "crates/a/src/lib.rs", "pub use b::hello;\n");
        write(
            root,
            "crates/b/Cargo.toml",
            "[package]\nname = \"b\"\nversion = \"0.1.0\"\nedition = \"2021\"\n",
        );
        write(root, "crates/b/src/lib.rs", "pub fn hello() {}\n");
    }
    ...
    fn snapshot_captures_cargo_metadata_when_cargo_present() {
        ...
        assert!(snap.cargo_metadata.contains("a"));
        assert!(snap.cargo_metadata.contains("b"));
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn dry_run_passes_a_clean_move() {
        ...
        make_cargo_fixture(&root);
        // Move b to a capability home; recompute the path-dep in a.
        let plan = MovePlan {
            capability: "demo".to_string(),
            moves: vec![CrateMove {
                old_path: "crates/b".to_string(),
        ...
        assert!(root.join("crates/b/Cargo.toml").is_file());
    ...
    fn dry_run_fails_a_move_that_breaks_resolution() {
        ...
        make_cargo_fixture(&root);
        // A DELIBERATELY broken plan: rename b's package but craft a fixture where the
        // dependent references the OLD name through a hand-broken manifest. We simulate the
        // break by moving b out from under the members glob WITHOUT the engine being able
        // to fix the dependent — achieved by making a depend on a name the move renames,
        // while a stale second manifest hardcodes a now-wrong absolute path.
        // Simplest deterministic break: point a at a path that will not exist post-move
