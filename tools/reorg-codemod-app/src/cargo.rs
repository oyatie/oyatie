...
fn rewrite_one_dep_table(
    ...
    // Phase 1b: an ALIASED dep (`hal = { package = "hal", path = ... }`) names its real
...
/// glob/literal. The check uses the [`workspace_members_kernel`] resolver semantics. We
...
"#;
        let cm = cm(
            "cloud/cloud-iam/crates/cloud-iam",
            "iam/core/iam",
            "cloud-iam",
            "iam-core",
        );
        ...
        assert!(!out.contains("cloud-iam"));
        assert!(!out.contains("cloud_iam"));
    ...
    fn dep_path_recompute_for_moved_manifest_unmoved_target() {
        // manifest moves; one dep (../domain) target does NOT move.
        let text = r#"[package]
name = "iam-app"
...
"#;
        ...
        .unwrap();
        assert!(changed);
        // old target cloud/cloud-iam/crates/domain, new manifest iam/facade/iam-app
        // (3 segments) -> ../../../cloud/cloud-iam/crates/domain
        assert!(
            out.contains("path = \"../../../cloud/cloud-iam/crates/domain\""),
            "recomputed path: {out}"
        );
    ...
    fn dep_key_renamed_and_path_recomputed_when_target_moves() {
        // manifest cloud/other/crates/other points at cloud/cloud-iam/crates/domain
        // (3 ups to root, then down): ../../../cloud-iam/crates/domain.
        let text = r#"[package]
name = "other"

[dependencies]
domain = { path = "../../../cloud-iam/crates/domain" }
"#;
        let domain = cm(
            "cloud/cloud-iam/crates/domain",
            "iam/core/domain",
            "domain",
            "iam-domain",
        );
        let mut name_to_move: BTreeMap<&str, &CrateMove> = BTreeMap::new();
        name_to_move.insert("domain", &domain);
        let (out, changed) = rewrite_dependencies_in_manifest(
            text,
            "cloud/other/crates/other/Cargo.toml",
            "cloud/other/crates/other",
            None, // this manifest itself does NOT move
            &name_to_move,
            &|old| {
                if old == "cloud/cloud-iam/crates/domain" {
        ...
        // path recomputed: from cloud/other/crates/other to iam/core/domain
        // = ../../../../iam/core/domain
        assert!(
...
"#;
        ...
        assert!(out.contains("../../cloud/cloud-iam/crates/domain"));
    }
...
"#;
        let hal = cm(
            "cloud/cloud-kernel/crates/cloud-kernel-hal-kernel",
            "cloud/cloud-kernel/core/hal",
            "cloud-kernel-hal-kernel",
            "kernel-hal",
        );
        let mut name_to_move: BTreeMap<&str, &CrateMove> = BTreeMap::new();
        name_to_move.insert("cloud-kernel-hal-kernel", &hal);
        let (out, changed) = rewrite_dependencies_in_manifest(
            text,
            ...
            &name_to_move,
            &|old| {
                (old == "cloud/cloud-kernel/crates/cloud-kernel-hal-kernel")
...
[dependencies]
hal = { package = "hal", path = "../hal" }
"#;
        let hal = cm("libs/hal", "kernel/core/hal", "hal", "kernel-hal");
        let mut name_to_move: BTreeMap<&str, &CrateMove> = BTreeMap::new();
        name_to_move.insert("hal", &hal);
        let (out, changed) = rewrite_dependencies_in_manifest(
            text,
            "libs/consumer/Cargo.toml",
            "libs/consumer",
            None,
            &name_to_move,
            &|old| (old == "libs/hal").then(|| "kernel/core/hal".to_string()),
        )
        .unwrap();
        assert!(changed);
        assert!(
