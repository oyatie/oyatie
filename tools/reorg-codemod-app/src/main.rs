...
//! strangler invokes it, and merge authority stays in the cloud-ci/ci required contexts
...
/// canonical-JSON move-manifest (schema `ci/reorg-move-manifest/v1`) the rename-aware
...
fn cmd_manifest(args: &[String]) -> Result<ExitCode, String> {
    ...
    let load_old_crate_dirs = |p: &Path| -> Result<Vec<String>, CodemodError> {
        ...
        Ok(reorg_codemod_app::plan_probe_paths(&plan))
    };
    let plan_path = reorg_codemod_app::resolve_effective_active_move_plan(
        plan_path,
        &repo_root,
        load_old_crate_dirs,
    ...
    eprintln!(
        "reorg-codemod: move-manifest ({} file pairs, {} crate-dir pairs, {} crate-ident pairs) -> {}",
        file_pairs.len(),
        crate_dir_pairs.len(),
...
fn apply_outcome_json(o: &reorg_codemod_app::plan::ApplyOutcome) -> Value {
    json!({
...
fn print_usage() {
    eprintln!(
        "reorg-codemod — reversible capability-move codemod + green-snapshot oracle (ADR-0562)\n\
         \n\
         USAGE:\n\
         \x20 reorg-codemod apply    --plan <plan.json> [--repo-root <p>] [--revert]\n\
         \x20 reorg-codemod dry-run  --plan <plan.json> [--repo-root <p>] [--revert] [--with-buck] [--keep-shadow]\n\
         \x20 reorg-codemod snapshot [--repo-root <p>] [--with-buck]\n\
         \x20 reorg-codemod manifest [--plan <plan.json>] [--repo-root <p>] [--out <p>]\n\
         \n\
         Local bridge ONLY; merge authority stays in cloud-ci/ci. Ships UNUSED until the\n\
