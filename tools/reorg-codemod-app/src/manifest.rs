...
mod tests {
    ...
    fn artifact_only_plan_probes_its_artifact_paths() {
        ...
        let mixed = MovePlan {
            ...
            moves: vec![crate_move("libs/iam-domain")],
            artifacts: vec![crate::model::ArtifactMove {
    ...
    fn landed_plan_excluded_pending_plan_selected() {
        ...
        let load = |p: &Path| -> Result<Vec<String>, CodemodError> {
            if p.ends_with("z-pending-move-plan.json") {
                Ok(vec![
                    "cloud/cloud-ci/gates/cloud-ci-firewall-app".to_owned(),
                ])
            } else {
                Ok(vec!["libs/shared-pdp-adapter-cedar".to_owned()])
            }
        };
        let old_dir_absent = |d: &str| Ok(d == "libs/shared-pdp-adapter-cedar");
