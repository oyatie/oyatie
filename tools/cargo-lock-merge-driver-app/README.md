...
```bash
buck2 build //tools/cargo-lock-merge-driver-app:cargo-lock-merge-driver
```
...
git config merge.cargo-lock.driver "cargo-lock-merge-driver %O %A %B"
```
...
(pathname), `%S`/`%X`/`%Y` (conflict labels) — and `merge.faces` in this repo is already
...
```bash
buck2 test //tools/cargo-lock-merge-driver-app/...
```
