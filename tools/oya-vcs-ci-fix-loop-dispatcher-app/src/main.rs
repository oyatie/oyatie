//! `oya-vcs-ci-fix-loop-dispatcher-app` binary entrypoint.
//!
...
use oya_vcs_ci_fix_loop_dispatcher_app::{
...
fn main() -> ExitCode {
    ...
    match run(&args, &Filesystem::real()) {
        ...
        Err(error) => {
            eprintln!("oya-vcs-ci-fix-loop-dispatcher-app failed: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn usage() -> String {
    "usage: oya-vcs-ci-fix-loop-dispatcher-app \\\n\
