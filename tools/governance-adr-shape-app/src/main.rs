...
use governance_adr_shape_kernel::{AdrDocument, validate_adr_shape_fitness};
...
mod tests {
    use super::*;
    use governance_adr_shape_kernel::audit_adr_shape_fitness;

    fn filesystem_document(name: &str, text: &str) -> AdrDocument {
        let path = env::temp_dir().join(format!("adr-shape-{}-{name}", std::process::id()));
