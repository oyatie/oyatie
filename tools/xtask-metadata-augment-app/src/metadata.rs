...
mod tests {
    ...
    fn scratch_root() -> std::path::PathBuf {
        ...
        let root = std::env::temp_dir().join(format!(
            "metadata-augment-members-{}-{nanos}",
            std::process::id()
        ));
    ...
    fn workspace_members_are_expanded_before_metadata_checks() {
        ...
        write_member(&root, "tools/one");
        write_member(&root, "tools/two");
        write_member(&root, "cloud/cloud-kernel/crates/excluded");
        fs::write(
        ...
        assert_eq!(members, vec!["tools/one", "tools/two"]);
