...
use oya_cargo_lock_merge_driver_app::{merge_lockfiles, whole_file_conflict};
...
fn main() -> ExitCode {
    ...
    std::panic::set_hook(Box::new(|info| {
        eprintln!("oya-cargo-lock-merge-driver: panicked, abandoning the merge: {info}");
        std::process::exit(i32::from(ABORT_MERGE));
    }));

    let args: Vec<String> = std::env::args().skip(1).collect();
    // `%O %A %B` are read positionally. Git may substitute more — `%L` (conflict-marker size),
    // `%P` (pathname), `%S`/`%X`/`%Y` (conflict labels) — and `merge.oya-faces` in this very repo
    ...
    let [base_path, current_path, other_path, ..] = args.as_slice() else {
        eprintln!(
            "oya-cargo-lock-merge-driver: expected at least <base> <current> <other>, got {} \
    ...
    if let Err(err) = write_atomic(
        ...
        eprintln!("oya-cargo-lock-merge-driver: {err}");
...
fn conflict(reason: &str) -> ExitCode {
    eprintln!(
        "oya-cargo-lock-merge-driver: {reason}; every side is in the file under conflict markers \
