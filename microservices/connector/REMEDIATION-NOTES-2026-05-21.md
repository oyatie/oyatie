...
- Capacity model: declared manifest `0.06 vCPU`, `128 MiB RAM`, `1 GiB storage`, Valkey/Postgres/outbound baselines, `per_request` scaling, Tier-1 placement, `pod_runtime_tier=1`, and `2..40` replica bounds per ADR-0340. Rejected a generic workflow-engine capacity model because vendor rate limits and webhook admission are connector-owned bottlenecks. Cost: token-bucket sharding and per-vendor queue isolation.
...
- Frontmatter: added ADR-0338, ADR-0340, ADR-0341, ADR-0342, ADR-0343, ADR-0344, ADR-0345. ADR-0339 was not added because `connector` has no child `iac/<context>/` directories; ADR-0337 was not added because no Iceberg warehouse writer evidence exists.
...
- Cost: Tier-1 cell capacity must reserve token and webhook admission overhead until is fully retired.
...
- Values: pod_runtime_tier=1; evidence=microservices/connector/PRD.md, microservices/connector/IP-003-oauth-broker-domain-kernel.md, microservices/connector/runbooks/oauth-token-revocation-cascade.md, microservices/connector/iac/openbao-policy.hcl.
