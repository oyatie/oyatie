...
companion_docs:
  - microservices/connector/catalog/oya-connector-adapter-domain.yaml
  - microservices/connector/capabilities/connector-invoke.yaml
...
Implement `oya-connector-adapter-domain` — the per-connector typed action invocation engine with credential sidecar integration, circuit-breaker, exponential-backoff retry-with-jitter, per-connector rate-limit enforcement, and DLQ routing on exhaustion.
