...
companion_docs:
  ...
  - microservices/connector/catalog/oya-connector-catalog-kernel.yaml
...
Implement `oya-connector-catalog-domain` and `oya-connector-catalog-kernel` — the in-memory connector registry, category taxonomy, rate-limit profile indexing, and compliance-posture query surface.
...
- [ ] Crates compile with `cargo build -p oya-connector-catalog-domain`
