...
companion_docs:
  - microservices/connector/catalog/oya-connector-webhook-receiver-domain.yaml
...
Implement `oya-connector-webhook-receiver-domain` — per-tenant webhook endpoint provisioning, inbound HMAC signature verification (constant-time), replay-window enforcement (≤5min), idempotency-key deduplication, and payload enqueue.
...
2. **Replay attack spike** → circuit-breaker on per-tenant replay-block count; `WebhookReplayBlocked` event; alert on `oya_connector_webhook_replay_blocked_total > 50/5min`.
...
- `microservices/connector/policy/payload-signature-verification.cedar` — concrete artifact to verify or update.
- `microservices/connector/capabilities/webhook-endpoint-register.yaml` — concrete artifact to verify or update.
- `microservices/connector/runbooks/webhook-replay-attack-detected.md` — concrete artifact to verify or update.
- `microservices/connector/catalog/oya-connector-webhook-receiver-domain.yaml` — concrete artifact to verify or update.
...
- `microservices/connector/policy/payload-signature-verification.cedar`
- `microservices/connector/capabilities/webhook-endpoint-register.yaml`
