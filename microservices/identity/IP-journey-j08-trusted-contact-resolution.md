...
ip_id: IP-journey-j08-trusted-contact-resolution
journey_id: j08-elder-financial-abuse-detection
microservice: identity
role: trusted-contact-resolution
status: draft
date: 2026-05-20
related_adrs:
  - ADR-0303
...
# IP - j08 - identity - trusted-contact-resolution

Goal: implement the identity portion of Elder financial abuse detection so Yejin mother attempts a large transfer under scammer pressure and oyatie fires trusted-contact alert plus cool-off.
Binding ADR: ADR-0303. This IP is one independently reviewable slice and must not weaken the common critical-path ADR pack.

## Scope

In scope: trusted-contact-resolution, JSON Schema validation, Cedar authorization, audit-chain emission, observability, failure branches, and integration tests for j08.
...
|---|---|---|---|
| elder-transfer-attempt | identity.trusted-contact-resolution table or event stream | docs/user-journeys/j08-elder-financial-abuse-detection/schemas/elder-transfer-attempt.json | pack-controlled, minimum audit retention |
| trusted-contact-alert | identity.trusted-contact-resolution table or event stream | docs/user-journeys/j08-elder-financial-abuse-detection/schemas/trusted-contact-alert.json | pack-controlled, minimum audit retention |
| cooloff-release-decision | identity.trusted-contact-resolution table or event stream | docs/user-journeys/j08-elder-financial-abuse-detection/schemas/cooloff-release-decision.json | pack-controlled, minimum audit retention |
...
openapi: 3.2.0
info:
  title: identity j08 trusted-contact-resolution
  version: 1.0.0
paths:
  /journeys/j08/identity/trusted-contact-resolution:
    post:
      operationId: j08IdentityTrustedContactResolution
...
asyncapi: 3.1.0
info:
  title: identity j08 events
  version: 1.0.0
channels:
  j08.identity.trusted-contact-resolution.accepted:
    address: j08.identity.trusted-contact-resolution.accepted
```
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/identity/IP-journey-j08-trusted-contact-resolution.md` matched `SLO, financial`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/identity/IP-journey-j08-trusted-contact-resolution.md` matched `emission`.
