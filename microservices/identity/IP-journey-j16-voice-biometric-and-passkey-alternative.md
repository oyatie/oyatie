...
ip_id: IP-journey-j16-voice-biometric-and-passkey-alternative
journey_id: j16-disability-accommodation-voice-only-signup
microservice: identity
role: voice-biometric-and-passkey-alternative
status: draft
date: 2026-05-20
related_adrs:
  - ADR-0303
...
# IP - j16 - identity - voice-biometric-and-passkey-alternative

Goal: implement the identity portion of Voice-only disability accommodation signup so A post-stroke user signs up with voice-only interaction plus single-switch alternatives.
Binding ADR: ADR-0303. This IP is one independently reviewable slice and must not weaken the common critical-path ADR pack.

## Scope

In scope: voice-biometric-and-passkey-alternative, JSON Schema validation, Cedar authorization, audit-chain emission, observability, failure branches, and integration tests for j16.
...
|---|---|---|---|
| voice-only-signup-session | identity.voice-biometric-and-passkey-alternative table or event stream | docs/user-journeys/j16-disability-accommodation-voice-only-signup/schemas/voice-only-signup-session.json | pack-controlled, minimum audit retention |
| assistive-auth-decision | identity.voice-biometric-and-passkey-alternative table or event stream | docs/user-journeys/j16-disability-accommodation-voice-only-signup/schemas/assistive-auth-decision.json | pack-controlled, minimum audit retention |
| single-switch-fallback | identity.voice-biometric-and-passkey-alternative table or event stream | docs/user-journeys/j16-disability-accommodation-voice-only-signup/schemas/single-switch-fallback.json | pack-controlled, minimum audit retention |
...
openapi: 3.2.0
info:
  title: identity j16 voice-biometric-and-passkey-alternative
  version: 1.0.0
paths:
  /journeys/j16/identity/voice-biometric-and-passkey-alternative:
    post:
      operationId: j16IdentityVoiceBiometricAndPasskeyAlternative
...
asyncapi: 3.1.0
info:
  title: identity j16 events
  version: 1.0.0
channels:
  j16.identity.voice-biometric-and-passkey-alternative.accepted:
    address: j16.identity.voice-biometric-and-passkey-alternative.accepted
```
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/identity/IP-journey-j16-voice-biometric-and-passkey-alternative.md` matched `SLO`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/identity/IP-journey-j16-voice-biometric-and-passkey-alternative.md` matched `emission`.
