...
## §principals
This anchor is closed for `identity` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.identity.runtime` owns normal `oidc-token-issue` execution and never borrows a tenant principal.
- Platform principal `oyatie.identity.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/identity/backfill-replay.md` when present.
- Platform principal `oyatie.identity.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.identity.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.identity.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
- Cross-µservice callers expected by dependency graph: `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`, `cell`.
- Example: a tenant principal invoking `oidc-token-issue` is evaluated as `<tenant>.identity.oidc-token-issue` before any `identity` state mutation.
...
- Service: `identity`; owner `axis-identity`; tenant_class `substrate`; audience `B2B_TENANT + INTERNAL_OPERATOR`.
...
## §cedar-gates
This anchor is closed for `identity` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/identity/policy/cedar-acr-predicates.cedar`, `microservices/identity/policy/context-split.cedar`, `microservices/identity/policy/data-residency.md`, `microservices/identity/policy/dual-context-residency.cedar`, `microservices/identity/policy/operator-recovery.cedar`, `microservices/identity/policy/tenant-scope.cedar`.
- Action namespace uses `identity::<bounded_context>::<verb>`; the first protected action is `identity::oidc-token-issue::execute`.
...
- Concrete fragments in scope: `microservices/identity/policy/cedar-acr-predicates.cedar`, `microservices/identity/policy/context-split.cedar`, `microservices/identity/policy/data-residency.md`, `microservices/identity/policy/dual-context-residency.cedar`, `microservices/identity/policy/operator-recovery.cedar`, `microservices/identity/policy/tenant-scope.cedar`.
- Example: `identity::oidc-token-issue::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `identity` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `identity.zitadel_instance_controller` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `identity.oidc_issuer` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `identity.webauthn_relying_party` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `identity.scim_server` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `identity.hris_adapter` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
- Example: `oidc-token-issue` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `identity` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `identity` as `substrate`, so this section treats it as a substrate provider.
- Declared substrate/product dependencies: `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`, `cell`.
- If substrate: products consume `identity` only through contracts `microservices/identity/contracts/asyncapi/identity-events.yaml`, `microservices/identity/contracts/asyncapi/multi-context-events.yaml`, `microservices/identity/contracts/openapi/identity.yaml`, `microservices/identity/contracts/openapi/multi-context-split.yaml`, `microservices/identity/contracts/proto/identity.proto`, `microservices/identity/contracts/proto/multi_context_split.proto`.
- If product: `identity` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `zitadel-instance-controller`, `oidc-issuer`, `webauthn-relying-party`, `scim-server`, `hris-adapter`, `step-up-orchestrator`; +3 more.
- Example: `oidc-token-issue` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `identity` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/identity/policy/cedar-acr-predicates.cedar`, `microservices/identity/policy/context-split.cedar`, `microservices/identity/policy/data-residency.md`, `microservices/identity/policy/dual-context-residency.cedar`, `microservices/identity/policy/operator-recovery.cedar`, `microservices/identity/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `identity::oidc-token-issue::execute` evaluates locally, then emits an audit event before the usecase writes `identity.zitadel_instance_controller`.
...
## §cell-eligibility
This anchor is closed for `identity` against ADR-0248 §D-1: cell tenant_class, shard width, DR pair and shuffle-shard behavior.
...
- Example: `oidc-token-issue` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `identity` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `identity` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `identity.oidc-token-issue` and is included in every AI/risk/model call audit event.
...
- Example: `oidc-token-issue` can request classification or summarization with `audience_tag=identity.oidc-token-issue`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `identity` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `identity` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `zitadel-instance-controller` is read/written as an ontology node or edge projection when `identity` needs cross-surface discovery.
- Bounded-context entity `oidc-issuer` is read/written as an ontology node or edge projection when `identity` needs cross-surface discovery.
- Bounded-context entity `webauthn-relying-party` is read/written as an ontology node or edge projection when `identity` needs cross-surface discovery.
- Bounded-context entity `scim-server` is read/written as an ontology node or edge projection when `identity` needs cross-surface discovery.
...
- Example: `oidc-token-issue` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `identity` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/identity/contracts/asyncapi/identity-events.yaml`, `microservices/identity/contracts/asyncapi/multi-context-events.yaml`, `microservices/identity/contracts/openapi/identity.yaml`, `microservices/identity/contracts/openapi/multi-context-split.yaml`, `microservices/identity/contracts/proto/identity.proto`, `microservices/identity/contracts/proto/multi_context_split.proto`.
...
- IaC transport evidence: `microservices/identity/iac/helm/zitadel/Chart.yaml`, `microservices/identity/iac/helm/zitadel/templates/configmap.yaml`, `microservices/identity/iac/helm/zitadel/templates/deployment.yaml`, `microservices/identity/iac/helm/zitadel/templates/hpa.yaml`, `microservices/identity/iac/helm/zitadel/templates/ingress.yaml`, `microservices/identity/iac/helm/zitadel/templates/networkpolicy.yaml`; +6 more.
- Example: `identity` `oidc-token-issue` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `identity` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `zitadel-instance-controller`, `oidc-issuer`, `webauthn-relying-party`, `scim-server`, `hris-adapter`, `step-up-orchestrator`; +3 more and deploy as separate app/worker/API pods where files exist.
- Default shape is Kubernetes pods with Cloud Hypervisor + Kata isolation for tenant_class 0/1 paths; lower tiers inherit network policy and SPIFFE identity.
...
- IaC manifests in scope: `microservices/identity/iac/helm/zitadel/Chart.yaml`, `microservices/identity/iac/helm/zitadel/templates/configmap.yaml`, `microservices/identity/iac/helm/zitadel/templates/deployment.yaml`, `microservices/identity/iac/helm/zitadel/templates/hpa.yaml`, `microservices/identity/iac/helm/zitadel/templates/ingress.yaml`, `microservices/identity/iac/helm/zitadel/templates/networkpolicy.yaml`; +6 more.
- Catalog crates/components: `microservices/identity/catalog/oya-check-authz-tenant_class-discipline.yaml`, `microservices/identity/catalog/oya-check-step-up-auth-coverage.yaml`, `microservices/identity/catalog/oya-identity-audit-emitter-kernel.yaml`, `microservices/identity/catalog/oya-identity-oidc-issuer-kernel.yaml`, `microservices/identity/catalog/oya-identity-scim-server-kernel.yaml`, `microservices/identity/catalog/oya-identity-step-up-orchestrator-kernel.yaml`; +17 more.
- Example: `oidc-token-issue` API runs as a Kata-isolated container; scheduled/background work for `identity` uses a separate worker principal.
...
## §observability
This anchor is closed for `identity` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/identity/slos/aaguid-refresh-freshness.openslo.yaml`, `microservices/identity/slos/audit-emit-completeness.openslo.yaml`, `microservices/identity/slos/jwks-availability.openslo.yaml`, `microservices/identity/slos/oidc-token-issue-latency.openslo.yaml`, `microservices/identity/slos/oidc-token-verify-latency.openslo.yaml`, `microservices/identity/slos/scim-availability.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.identity.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `identity_oidc_token_issue_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `identity_oidc_token_issue_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/identity/slos/aaguid-refresh-freshness.openslo.yaml`.
...
- Example trace: `identity.oidc-token-issue` -> `policy.evaluate` -> `storage.oidc_token_issue` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `identity` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `identity` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `oidc-token-issue` action class.
...
- Cedar/IaC controls in scope: `microservices/identity/policy/cedar-acr-predicates.cedar`, `microservices/identity/policy/context-split.cedar`, `microservices/identity/policy/data-residency.md`, `microservices/identity/policy/dual-context-residency.cedar`, `microservices/identity/policy/operator-recovery.cedar`, `microservices/identity/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `identity` use fake `oidc-token-issue` identifiers and honey endpoints under `/.well-known/oya-canary/identity`.
...
This anchor is closed for `identity` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `identity` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `identity` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `identity` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `oidc-token-issue` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `identity` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `identity` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `identity` include `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`, `cell`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/identity/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/identity/runbooks/brute-force-mitigation.md`, `microservices/identity/runbooks/idp-failover-drill.md`, `microservices/identity/runbooks/ip-block-incident.md`, `microservices/identity/runbooks/jwks-rotation.md`, `microservices/identity/runbooks/passkey-cross-device-debug.md`, `microservices/identity/runbooks/passkey-reset.md`; +2 more.
- Example: `oidc-token-issue` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
