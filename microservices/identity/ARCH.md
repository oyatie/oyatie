...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/identity/manifest.json
  - microservices/identity/PRD.md
  - microservices/identity/README.md
  - microservices/identity/ARCHITECTURE.md
---
...
`identity` keeps its existing bounded context and flat `microservices/identity/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `identity`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `identity` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/postgresql-cluster@v1[paid], oci-guest/always-free/valkey-cluster@v1[demo_trial], on-prem/service-mesh-waypoint@v1[paid], oyatie-as-cloud-provider/openbao-secret-binding@v1[paid].
...
- Capacity: `per_request` drives CPU `0.18`, RAM `256`, storage `2`, and connection pool variables.
- DR: RTO `1800` seconds and RPO `0` seconds constrain backup and failover primitives.
