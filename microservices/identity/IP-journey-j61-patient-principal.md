...
impl_plan_id: IP-journey-j61-patient-principal
journey_id: j61
...
# IP: j61 `identity` — `patient-principal`

## Intent
Implement the `identity` slice of `j61-patient-intake-to-followup` for Yejin Park, physician treating a referred patient. The slice owns `patient-principal` in the cross-product workflow where patient intake creates a hospital-scoped principal, routes specialist review, records consultation, and HIPAA-seals follow-up.
...
- Journey contract: `docs/user-journeys/j61-patient-intake-to-followup/`.
...
| Path | Action | Purpose |
|---|---|---|
| `microservices/identity/contracts/openapi-j61-patient-principal.yaml` | create | OpenAPI 3.2.0 REST edge for the journey slice |
| `microservices/identity/contracts/asyncapi-j61-patient-principal.yaml` | create | AsyncAPI 3.1.0 events for the journey slice |
| `microservices/identity/contracts/j61-patient-principal.proto` | create | proto3 worker/internal contract |
| `microservices/identity/policy/j61-patient-principal.cedar` | create | Cedar default-deny permit for this slice |
| `microservices/identity/tests/j61-patient-principal.md` | create | executable test notes or harness fixture plan |
| `microservices/identity/dashboards/j61-patient-principal.json` | create | ADR-0263 metric and trace dashboard |
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/identity/IP-journey-j61-patient-principal.md` matched `SLO, p99, payment`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/identity/IP-journey-j61-patient-principal.md` matched `emission`.
