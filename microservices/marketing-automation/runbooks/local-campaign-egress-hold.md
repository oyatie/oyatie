# Marketing Automation Runbook: Campaign Egress Hold

Service: marketing-automation  
Surface: local operational primitive suite  
Primary SLO: deliverability-success  
Domain focus: campaign, journey, segment, suppression

## Trigger
- Alert `marketing-automation-deliverability-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_marketing_automation_deliverability_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when deliverability-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `marketing-automation-local-deliverability-success` and policy file name to the ticket.
