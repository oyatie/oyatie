...
## B. Approach
Implement `catalog-layer-registration` as a service-local slice, not as a shared suite facility. The technical mechanism is registry publication for crate/service layers bound to `ADR-0105 layer catalog rows` and checked before user-visible promotion.
