...
## B. Approach
Implement `backfill-replay-worker` as a service-local slice, not as a shared suite facility. The technical mechanism is idempotent correction of projections and imported records bound to `src/worker/ replay lane` and checked before user-visible promotion.
