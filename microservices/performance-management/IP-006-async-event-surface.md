# IP-006 Performance Management async event surface

Service: performance-management
ChangeSet scope: microservices/performance-management/IP-006-async-event-surface.md
...
## A. Problem
This IP closes the async event surface gap for `performance-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `async-event-surface` works for reviews, goals, calibration, engagement pulses, succession evidence, and compensation handoff cannot collapse into generic HR records.
...
The success condition is an implementation plan a cold engineer can trace from this IP to concrete files such as `microservices/performance-management/contracts/asyncapi-v1.yaml` without inventing a hidden service boundary.

## B. Approach
Implement `async-event-surface` as a service-local slice, not as a shared platform facility. The technical mechanism is worker outbox and replay path bound to `adapter async boundary` and checked before user-visible promotion.
...
The approach explicitly covers event schema versioning, idempotency, dead-letter replay, audit correlation; those are the failure modes that a stamped IP did not name.

## C. Deliverables
- D01: `microservices/performance-management/PRD.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D02: `microservices/performance-management/ARCHITECTURE.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D03: `microservices/performance-management/manifest.json` — modify or bind for `async-event-surface` evidence and contract traceability.
- D04: `microservices/performance-management/competitor-parity-matrix.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D05: `microservices/performance-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `async-event-surface` evidence and contract traceability.
- D06: `microservices/performance-management/contracts/asyncapi-v1.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D07: `microservices/performance-management/contracts/openapi-v1.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D08: `microservices/performance-management/contracts/performance-management-v1.proto` — modify or bind for `async-event-surface` evidence and contract traceability.
- D09: `microservices/performance-management/src/domain/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D10: `microservices/performance-management/src/usecase/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D11: `microservices/performance-management/src/adapter/mod.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D12: `microservices/performance-management/src/config.rs` — modify or bind for `async-event-surface` evidence and contract traceability.
- D13: `microservices/performance-management/policies/local-review-cycle-scope.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D14: `microservices/performance-management/policies/local-calibration-lock-control.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D15: `microservices/performance-management/policies/local-engagement-pulse-anonymity.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D16: `microservices/performance-management/policy/review-calibration-authorization.cedar` — modify or bind for `async-event-surface` evidence and contract traceability.
- D17: `microservices/performance-management/capabilities/goal-cycle-open.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
- D18: `microservices/performance-management/capabilities/review-evidence-seal.yaml` — modify or bind for `async-event-surface` evidence and contract traceability.
...
2. Add or update the `async-event-surface` contract shape in `contracts/asyncapi-v1.yaml` and keep request/event/proto field names aligned with ADR-0105 layer naming.
...
5. Bind adapter behavior through `adapter async boundary` and `microservices/performance-management/src/adapter/mod.rs`, using typed errors from `microservices/performance-management/src/error.rs` instead of stringly provider failures.
...
8. Update dashboards/SLOs or the service operating bar to expose success, refusal, replay, latency, and audit completeness for `async-event-surface`.
...
## E. Acceptance
- A reviewer can trace `async-event-surface` from this IP to at least one real contract, one real policy file, one real source file, and one capability/catalog artifact.
...
- The observability evidence includes metric, trace, structured log, audit event id, policy decision id, and low-cardinality labels for `async-event-surface`.
...
|---|---|---|
| Lattice | Provides a native `async event surface` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `async-event-surface` to tenant scope, Cedar deny evidence, audit-chain records, and flat `performance-management` ownership. |
...
- Do not move `async-event-surface` into a sibling service such as `people-records` or `compensation` unless a later ADR changes ownership.
...
- contract_surface: [`microservices/performance-management/contracts/asyncapi-v1.yaml`, `microservices/performance-management/contracts/hr-handoff-compensation.asyncapi.yaml`, `microservices/performance-management/contracts/hr-handoff-learning-management.asyncapi.yaml`, `microservices/performance-management/contracts/hr-handoff-people-records.asyncapi.yaml`, `microservices/performance-management/contracts/hr-handoff-recruiting.asyncapi.yaml`, `microservices/performance-management/contracts/hr-handoff-time-tracking.asyncapi.yaml`]; detected_types: OpenAPI, AsyncAPI, proto3; trigger_terms: [`asyncapi`, `openapi`, `.proto`].
...
- ha_trigger_evidence: `microservices/performance-management/IP-006-async-event-surface.md` matched [`SLO`].
...
- evidence_paths: [`microservices/performance-management/IP-006-async-event-surface.md`, `microservices/performance-management/manifest.json`, `microservices/performance-management/ARCHITECTURE.md`, `microservices/performance-management/PRD.md`, `microservices/performance-management/multi-region.md`, `microservices/performance-management/capacity-model.md`].

## Sustainability emission (per ADR-0344)

- metering_trigger_evidence: `microservices/performance-management/IP-006-async-event-surface.md` matched [`emission`].
- per_call_audit_row_fields: `cost_usd_minor_units`, `co2_grams`, `watt_hours`, `provider`, `region`, `cell`.
- carbon_aware_scheduling: eligible only for deferrable work after compliance-pack exclusions and active RTO/RPO floors are satisfied; excluded from realtime Tier 0/1 paths.
- finops_portal_rollup_axes: `tenant`, `product`, `capability`, `provider`, `cell`.
- evidence_paths: [`microservices/performance-management/IP-006-async-event-surface.md`, `microservices/performance-management/manifest.json`, `microservices/performance-management/capacity-model.md`, `microservices/performance-management/compliance.md`, `microservices/performance-management/ARCHITECTURE.md`].
