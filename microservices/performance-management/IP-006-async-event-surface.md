...
## B. Approach
Implement `async-event-surface` as a service-local slice, not as a shared suite facility. The technical mechanism is worker outbox and replay path bound to `adapter async boundary` and checked before user-visible promotion.
