...
arch_id: ARCH-performance-management
...
`performance-management` keeps its existing bounded context and flat `microservices/performance-management/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `performance-management`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `performance-management` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_user` drives CPU `0.07`, RAM `192`, storage `3`, and connection pool variables.
