...
## B. Approach
Implement `cost-budget-enforcer` as a service-local slice, not as a shared suite facility. The technical mechanism is budget checks before async and export work bound to `tenant/cell/context cost attribution` and checked before user-visible promotion.
