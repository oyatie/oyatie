...
## B. Approach
Implement `policy-eval-library-binding` as a service-local slice, not as a shared suite facility. The technical mechanism is Cedar caller-side evaluation before usecase execution bound to `governance policy port` and checked before user-visible promotion.
