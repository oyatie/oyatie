# Performance Management Runbook: Review Form Latency Burn

Service: performance-management  
Surface: local operational primitive suite  
Primary SLO: review-form-latency  
Domain focus: review, calibration, rating, feedback

## Trigger
- Alert `performance-management-review-form-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_performance_management_review_form_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when review-form-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `performance-management-local-review-form-latency` and policy file name to the ticket.
