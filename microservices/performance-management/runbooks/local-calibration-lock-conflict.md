# Performance Management Runbook: Calibration Lock Conflict

Service: performance-management  
Surface: local operational primitive suite  
Primary SLO: calibration-lock-conflict-rate  
Domain focus: review, calibration, rating, feedback

## Trigger
- Alert `performance-management-calibration-lock-conflict-rate` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_performance_management_calibration_lock_conflict_rate_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when calibration-lock-conflict-rate is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `performance-management-local-calibration-lock-conflict-rate` and policy file name to the ticket.
