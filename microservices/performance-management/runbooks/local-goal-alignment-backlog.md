# Performance Management Runbook: Goal Alignment Backlog

Service: performance-management  
Surface: local operational primitive suite  
Primary SLO: goal-alignment-latency  
Domain focus: review, calibration, rating, feedback

## Trigger
- Alert `performance-management-goal-alignment-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_performance_management_goal_alignment_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when goal-alignment-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `performance-management-local-goal-alignment-latency` and policy file name to the ticket.
