# Performance Management Runbook: Talent Grid Projection Gap

Service: performance-management  
Surface: local operational primitive suite  
Primary SLO: rating-distribution-freshness  
Domain focus: review, calibration, rating, feedback

## Trigger
- Alert `performance-management-rating-distribution-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_performance_management_rating_distribution_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when rating-distribution-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `performance-management-local-rating-distribution-freshness` and policy file name to the ticket.
