...
## B. Approach
Implement `chaos-drill-pack` as a service-local slice, not as a shared suite facility. The technical mechanism is failure injection with tenant/cell blast-radius controls bound to `drill scenario catalog` and checked before user-visible promotion.
