...
## B. Approach
Implement `capacity-admission-control` as a service-local slice, not as a shared suite facility. The technical mechanism is queue and concurrency control by tenant/cell/context bound to `admission policy` and checked before user-visible promotion.
