...
## B. Approach
Implement `observability-audit-events` as a service-local slice, not as a shared suite facility. The technical mechanism is metrics/traces/logs plus signed evidence bound to `audit-chain event envelope` and checked before user-visible promotion.
