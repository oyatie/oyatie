...
## B. Approach
Implement `abuse-defence-edge-waf` as a service-local slice, not as a shared suite facility. The technical mechanism is edge refusal path ahead of usecase execution bound to `iac/edge-waf.yaml when present` and checked before user-visible promotion.
