...
date: 2026-05-17
related_artifacts:
  - microservices/sheets/failure-modes.md (FM-05)
  - microservices/sheets/threat-model.md §"T-T-07" + §"T-I-01"
  - microservices/sheets/PRD.md AC-04
  - microservices/sheets/decisions/ADR-SHEETS-0006 (per-range ACL granularity)
...
1. **`oya_sheets_range_acl_drift_total > 0`** (quarterly drift audit detects mismatch).
2. **Tenant reports**: "I shouldn't see this column" OR "I should be able to edit this range but can't".
3. **`oya-governance-sheets-range-acl-cedar-required` lane fails** post-deploy.
...
1. Identify affected (tenant, workbook, range): query `oya_sheets_range_acl_drift_top_n`.
...
After recovery:
- `oya_sheets_range_acl_drift_total == 0` post re-sync.
- `oya-governance-sheets-range-acl-cedar-required` lane exit 0.
...
- `microservices/sheets/failure-modes.md` FM-05.
- `microservices/sheets/decisions/ADR-SHEETS-0006` (per-range ACL granularity).
- `microservices/sheets/policy/tenant-scope.cedar` PERMIT 9 + FORBID.
- Cedar v4.2 LTS — `cedarpolicy.com`.
- `microservices/sheets/incident-response.md` §"Regulatory Notifications".
