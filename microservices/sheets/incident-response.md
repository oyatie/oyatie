...
# Incident Response Playbook (sheets µservice)

## Purpose

End-to-end incident-response procedure for sheets events. Covers severity classification, response roles, escalation paths, communication templates, postmortem cadence, and per-pack regulatory-notification timelines. Cross-referenced from `failure-modes.md` and `compliance.md`.
...
|---|---|---|---|
| **Sev-1** | Production-tier C/I/A impact across multiple tenants; regulatory-notification triggers; data breach; pack-wide outage | ≤ 5 min (24/7 on-call paged) | FM-09 license-gate cluster-wide; FM-13 cross-tenant collab leak; FM-11 WASM corruption broad; FM-05 share-ACL confidentiality drift |
| **Sev-2** | Single-tenant or sub-tenant impact; operational degradation without data loss | ≤ 15 min (24/7 on-call paged) | FM-01 collab desync; FM-02 recalc storm; FM-03 formula-engine rollback; FM-08 CDN purge gap; FM-14 XLSX malicious upload; FM-16 round-trip regression |
| **Sev-3** | Localized impact; degraded but functional | ≤ 1h (business hours) | FM-06 named-range corruption single-tenant; FM-07 chart render degraded; FM-12 Postgres lock contention; FM-15 per-seat overage; FM-17 AI-formula timeout; FM-18 connected-sheets external failure |
...
|---|---|---|
| **Incident Commander (IC)** | Rotating on-call lead | Owns from declaration to closure |
...
| **Subject-Matter Expert (SME)** | axis-sheets (or council-design-system for UX-relevant incidents) | Diagnoses root cause; proposes mitigation |
| **Privacy Lead** | council-privacy chair | Activates for breach-suspect (Sev-1 confidentiality) |
...
```text
Alert fires (Sheets self-SLI → observability → Alertmanager → OnCall)
    ↓
...
Secondary on-call paged
    ↓ (no ack in 15 min)
Engineering manager (axis-sheets lead) paged + Slack alert
...
|---|---|---|
| **1. Detection** | Alert fires | ≤ 60s alert-to-page p99 |
| **2. Acknowledgement** | Primary on-call ack; opens `#inc-<id>` | ≤ 5 min (Sev-1) / ≤ 15 min (Sev-2) |
| **3. Triage** | IC declares severity; assigns roles | ≤ 10 min |
| **4. Containment** | OpsLead executes immediate-mitigation; Privacy Lead engaged if suspect | aim for stabilization within RTO |
...
| **7. Communication** | CommsLead notifies tenants; regulatory notification | per §"Regulatory Notifications" |
...
```
Subject: [Sev-1 / sheets] Incident in <pack>: <one-line summary>

We are investigating an incident affecting Sheets in <pack> that may impact your tenant.
Started at <ISO8601>.
Current status: <Investigating | Mitigating | Resolved>.
ETA to resolution: <est>.

What you may experience: <impact, e.g., workbook unavailable; saves degraded; XLSX export offline>
...
```
Subject: [Sev-2 / sheets] Degradation in <pack>: <one-line summary>
...
What you may experience: <impact, e.g., elevated save latency; recalc slow; collab sync delays;
                          XLSX export queued>
What we're doing: <action>
...
- Sev-1 affecting tenant: blocking banner "Sheets temporarily unavailable; please retry shortly."
- Sev-2 affecting tenant: non-blocking banner "Sheets degraded; some features may be slow."
- Sev-3 non-blocking: in-feature banner (e.g., "AI-formula temporarily unavailable; please proceed manually" / "XLSX export queued; please retry shortly").
...
### EU AI Act 2024 (AI-formula regulated-domain incidents)

For incidents involving AI-formula outputs in regulated-domain workflows:
...
| axis-sheets SME | weekly (3 engineers) | KR + EU primary; US business-hours fallback |
| council-design-system SME (UX-relevant) | named role | always-on-call |
...
- `microservices/sheets/failure-modes.md`.
- `microservices/sheets/compliance.md` §"Regulatory Notifications" + per-pack overlays.
- `microservices/sheets/multi-region.md`.
- `microservices/sheets/runbooks/*`.
- `microservices/sheets/dpia.md`.
- `microservices/sheets/threat-model.md`.
