...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that sheets
  (recalc engine + formula + CRDT collab + XLSX export) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/sheets.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml
---

# sheets — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`sheets`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-SHEETS`)
...
- **How sheets satisfies it**:
  - Capability calls in `microservices/sheets/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_sheets_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_sheets_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml` (`sheets-llm-capability-circuit-open`
    + `sheets-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/sheets/dashboards/` (per-capability panels).
  - Threat model: `microservices/sheets/threat-model.md` §"Dependency
...
- **How sheets satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_sheets_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml` (`sheets-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/sheets/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/sheets/capacity-model.md` (where
...
- **How sheets satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/sheets/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml` (`sheets-saturation-cpu-over-70pct`,
    `sheets-errors-5xx-rate-spike`, `sheets-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/sheets/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/sheets/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How sheets satisfies it**:
  - Every SLO in `microservices/sheets/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml` (`sheets-*-fast-burn-1h-14x` +
    `sheets-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/sheets/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/sheets/hyperscaler-conformance.md
test -f microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/sheets/iac/helm/visual-grid-rest/templates/prometheusrule.yaml || true
