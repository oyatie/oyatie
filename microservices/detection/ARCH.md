...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/detection/manifest.json
  - microservices/detection/PRD.md
  - microservices/detection/README.md
  - microservices/detection/ARCHITECTURE.md
---
...
`detection` keeps its existing bounded context and flat `microservices/detection/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `detection`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `detection` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/clickhouse-iceberg-layer@v1[paid], on-prem/kafka-stream@v1[paid], colo/postgresql-cluster@v1[paid], oyatie-as-cloud-provider/service-mesh-waypoint@v1[paid].
...
- Capacity: `per_message` drives CPU `0.3`, RAM `768`, storage `15`, and connection pool variables.
