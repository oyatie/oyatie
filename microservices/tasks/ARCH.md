...
companion_docs:
  ...
  - microservices/tasks/README.md
  - microservices/tasks/ARCHITECTURE.md
---
...
`tasks` keeps its existing bounded context and flat `microservices/tasks/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `tasks`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `tasks` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_user` drives CPU `0.22`, RAM `512`, storage `10`, and connection pool variables.
