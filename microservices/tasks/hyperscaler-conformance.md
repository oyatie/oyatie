...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that tasks
  (task list + recurring materialise + auto-assign + automation) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/tasks.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml
---

# tasks — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`tasks`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-TASKS`)
...
- **How tasks satisfies it**:
  - Capability calls in `microservices/tasks/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_tasks_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_tasks_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml` (`tasks-llm-capability-circuit-open`
    + `tasks-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/tasks/dashboards/` (per-capability panels).
  - Threat model: `microservices/tasks/threat-model.md` §"Dependency
...
- **How tasks satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_tasks_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml` (`tasks-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/tasks/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/tasks/capacity-model.md` (where
...
- **How tasks satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/tasks/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml` (`tasks-saturation-cpu-over-70pct`,
    `tasks-errors-5xx-rate-spike`, `tasks-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/tasks/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/tasks/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How tasks satisfies it**:
  - Every SLO in `microservices/tasks/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml` (`tasks-*-fast-burn-1h-14x` +
    `tasks-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/tasks/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/tasks/hyperscaler-conformance.md
test -f microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/tasks/iac/helm/tasks/templates/prometheusrule.yaml || true
