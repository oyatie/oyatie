---
doc_class: Runbook
doc_id: RUNBOOK-TASKS-AUTO_REBALANCE
microservice: tasks
...
# Runbook: Tasks Auto Rebalance

## Operator Contract
- Runbook id: tasks-auto-rebalance.
- Primary service namespace: `tasks`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tasks-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tasks; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tasks rollout status deploy/tasks --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tasks --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service tasks --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service tasks --cell $CELL --window 30m`.
