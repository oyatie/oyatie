...
|---|---|---|---|---|
| [ADR-SOC-0001](./ADR-SOC-0001-feed-ranking-algorithm.md) | Feed-ranking algorithm — hybrid chronological-first + heuristic-algorithmic in P01; ML-driven ranking deferred to P03 with EU AI Act high-risk obligations | Accepted | 2026-05-17 | derived gap from PRD §"Out-of-scope"; ranking model is high-risk per EU AI Act Annex III §1(a) |
...
| [ADR-SOC-0005](./ADR-SOC-0005-dual-context-feed-isolation.md) | Dual-context feed isolation — Personal pillar feed vs Professional pillar feed; same dual-context-isolation pattern as messenger/mail; data-model invariant per parallel ADR-0126 | Accepted | 2026-05-17 | parallel ADR-0126 inheritance + threat-model T-I-07 |
...
- Service-scoped ADRs may reference cross-cutting ADRs (`ADR-NNNN` at repo root) and sibling µservice ADRs (e.g., `ADR-MSGR-0004` referenced from `ADR-SOC-0004` as paired federation posture). Cross-µservice citations are encouraged where the decisions are genuinely paired.
...
|---|---|---|
| #1 (Ranking-model openness: closed-weights vs published-weights) | open | ADR-SOC follow-up; depends on EU AI Act notified-body engagement |
| #2 (Federation: AT Protocol in addition to ActivityPub) | open | ADR-SOC follow-up after federation MVP (ActivityPub) ships |
| #3 (Ads-substrate-stub fate: keep stubbed vs delete vs activate) | open | ADR-SOC follow-up after M03 |
| #4 (Self-observability emission posture) | Resolved | per-pack emission with per-tenant tags — resolved in IP-014 |
| #5 (Verified-handle uniqueness scope) | open | ADR-SOC follow-up |
