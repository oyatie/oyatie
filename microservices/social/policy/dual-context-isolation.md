...
Define the load-bearing dual-context invariants of the social substrate. Per parallel ADR-0238 (which inherits Bominal ADR-0208's dual-context model), every social entity carries a `context_kind: { Personal | Professional }` discriminator that determines:
