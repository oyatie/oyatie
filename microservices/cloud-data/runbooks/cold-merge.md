---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_DATA-COLD_MERGE
...
# Runbook: Cloud Data Cold Merge

## Operator Contract
- Runbook id: cloud-data-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-data-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-data; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-data rollout status deploy/cloud-data --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-data --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-data --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-data --cell $CELL --window 30m`.
