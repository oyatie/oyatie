...
A runnable example that establishes a federation peer, joins an MLS group, sends + receives encrypted messages with disclosure-rule enforcement — using `oya-connect-client` (target API; once IP-005 + IP-007 + IP-008 + IP-009 land).
...
[dependencies]
oya-connect-client = { path = "../../crates/oya-connect-client" }
...
grpcurl -plaintext \
    ...
    oya.connect.v1.ConnectService/SendMessage
```
...
`microservices/connect/reference-implementations/cross-tenant-message-rust-sdk.md` (this file). Runnable Cargo project lands at `microservices/connect/reference-implementations/federation-example/` once IP-005 + IP-007 + IP-008 + IP-009 land.
