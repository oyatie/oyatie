...
decision_owner: axis-connect + council-integrations
---
...
- The service PRD names `connect` as integration substrate, not a product endpoint.
...
- `connect` will treat `ConnectorWiring` as the canonical unit of integration.
...
- Contract test `connect-integration.yaml_paths_match_router`.
- Contract test `connect-integration-events.yaml_messages_match_event_codec`.
...
- Metric `oya_connect_action_total{connector,action,status}`.
- Metric `oya_connect_oauth_grant_total{connector,outcome}`.
- Metric `oya_connect_webhook_receive_total{connector,verify_outcome}`.
- Metric `oya_connect_dlq_depth{wiring_id}`.
- Metric `oya_connect_schema_drift_total{connector,severity}`.
