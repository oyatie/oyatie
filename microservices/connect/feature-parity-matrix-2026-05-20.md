# Connect Feature Parity Matrix - 2026-05-20
...
Primary local anchors: `microservices/connect/PRD.md:29-39`, `microservices/connect/README.md:16-32`, `microservices/connect/ARCHITECTURE.md:26-38`, and chat history `/Users/jasonlee/.claude/projects/-Users-jasonlee-oyatie/8f603fc7-eb0e-4752-ab03-f8ab63ce113d.jsonl:8923-8926`.
...
4. Connect relevance: Twilio is an external connector provider, not a replacement for connect.
5. Local evidence: Twilio already appears as a connector catalog record at `microservices/connect/catalog/connectors/twilio.yaml`.
6. Local evidence: connect PRD requires at least 500 connectors, `microservices/connect/PRD.md:156`.
7. Local evidence: connect PRD requires connector action invocation, `microservices/connect/PRD.md:128-137`.
8. Local evidence: connect PRD requires OAuth grant and webhook endpoint substrate, `microservices/connect/PRD.md:128-137`.
...
20. Surface TWI-12: connection limit metadata.
...
60. Current gap: no source tree or tests prove Twilio adapter behavior, as no `src/` or `tests/` directory exists under `microservices/connect/`.
...
5. Connect relevance: Sendbird should be modeled as a connector and event provider where Oyatie products need chat-provider integration.
6. Local evidence: no Sendbird connector catalog record appears in the 182-file inventory.
7. Local evidence: connect catalog has communication-adjacent providers such as Discord, Slack, SendGrid, Mailgun, and Twilio.
...
5. Connect relevance: Stream should be modeled as a connector and event provider with explicit rate-limit and webhook contracts.
6. Local evidence: no Stream connector catalog record appears in the 182-file inventory.
7. Local evidence: connect webhook receiver and connector action surfaces exist in PRD and contracts, cited by `microservices/connect/PRD.md:128-137` and `microservices/connect/contracts/openapi/connect-integration.yaml:176-218`.
...
| 3 | OAuth grant initiation | Applicable for OAuth-style providers | Sometimes not applicable | Token-based rather than OAuth in many flows | OpenAPI grant initiation `contracts/openapi/connect-integration.yaml:97-129` | Needs provider-specific profiles |
...
| 5 | Webhook endpoint registration | Needed | Needed | Needed | OpenAPI webhook registration `contracts/openapi/connect-integration.yaml:176-203` | Generic only |
| 6 | Webhook rotation | Needed | Needed | Needed | OpenAPI rotate endpoint `contracts/openapi/connect-integration.yaml:205-218` | Generic only |
...
| 10 | Outbound action idempotency | Needed | Needed | Needed | Contracts imply action requests; proto action service `contracts/proto/connect_integration.proto:123-125` | Explicit idempotency shape missing |
...
1. Catalog family summary: connect has a credible generic catalog base, but the union bar requires Twilio, Sendbird, and Stream as explicit communications-provider records.
...
6. Webhook family summary: connect has the right generic receiver, signature, and DLQ concepts.
7. Webhook gap: provider-specific HMAC, retry, timeout, duplicate, and event mapping examples are missing.
8. Action invocation summary: connect owns outbound connector action invocation.
...
22. Product-boundary summary: connect should not become chat UI or social product logic.
...
29. Union summary: connect has documentation primitives but lacks provider-specific records and executable proof.
...
2. Evidence H-01: `microservices/connect/manifest.json:3-6` versus `microservices/connect/PRD.md:29-39`.
...
8. Evidence H-03: no `microservices/connect/tests/` directory exists.
...
11. Evidence H-04: no `microservices/connect/src/` directory exists.
12. Impact H-04: parity remains design-only.
13. Gap H-05: current parity and benchmark docs use old counterpart sets.
14. Evidence H-05: `microservices/connect/competitor-parity-matrix.md:13-36` and `microservices/connect/benchmarks/connect-vs-slack-connect-vs-teams-external-vs-discord.md:9-13`.
...
23. Evidence H-08: no canonical context directories under `microservices/connect/iac/`.
...
26. Evidence H-09: no `microservices/connect/iac/oci-guest/always-free/` directory exists.
27. Impact H-09: demo_trial infrastructure caps cannot be enforced or benchmarked.
28. Gap H-10: OS support manifest is absent.
29. Evidence H-10: no `microservices/connect/supported-oses.json`.
...
38. Evidence H-13: OpenAPI has generic endpoint registration `contracts/openapi/connect-integration.yaml:176-218`.
...
41. Evidence H-14: `microservices/connect/cost-budget.md:42-47`.
...
44. Evidence H-15: PRD requires at least 500 connectors `microservices/connect/PRD.md:156`; prior audit says 30 seed connectors and 470-plus deferred `AUDIT-FINDINGS-2026-05-20.json:25-30`.
