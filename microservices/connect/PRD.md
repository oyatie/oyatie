...
doc_class: PRD
template_id: TPL-PRD
prd_id: PRD-connect-integration-substrate
...
Distinct from `workflow-engine` (which orchestrates *execution* of workflows): `connect` provides the **integration directory + connector adapters + OAuth dance + webhook receiver substrate**. Workflow engine consumes connect via library-first dispatch per ADR-0246.
...
9. **TIE-010**: As a TIE, I click "Connect Salesforce" and complete OAuth in <90s without ever pasting a raw token into oyatie.
...
25. **TIE-034**: As a TIE, the connector adapter emits OTel spans, metrics (`oya_connect_action_total{connector,action,status}`), and audit events per ADR-0263.
...
- **AS-01**: We do NOT execute workflows — that's workflow-engine. Connect provides the substrate; orchestration belongs to the consumer.
...
Evidence: modeling note at `docs/performance-budgets/connect-integration-substrate.md` derives each via Little's Law from observed peer-rate (Zapier handles ~3M tasks/min at peak; our 100k tasks/min target requires per-worker queue depth ≤30 with 100-worker pool — see §F-3 capacity model).

### Scalability

Connect MUST scale to: ≥500 connectors in catalog; ≥10M webhook receives/day per tenant at p99; ≥100k concurrent OAuth grants in flight across all tenants; ≥1M outbound connector actions/min platform-wide.
...
- Metrics: `oya_connect_action_total{connector,action,status}` (cardinality budget: 500 connectors × 50 actions × 5 statuses = 125k series); `oya_connect_oauth_grant_total{connector,outcome}`; `oya_connect_webhook_receive_total{connector,verify_outcome}`; `oya_connect_dlq_depth{wiring_id}`.
...
- `failover_runbook=runbooks/dr-failover.md`, resolved at `microservices/connect/runbooks/dr-failover.md`; backup substrates are `postgres_wal_g`, `object_storage_versioned`, `openbao_seal_unseal`, and `audit_chain_merkle_seal`.
...
- Why: connect is being retired as a product surface but remains a connector substrate; tenants need accepted webhooks, OAuth grants, and DLQ evidence to survive regional movement without duplicating third-party side effects.
...
- Cell placement class: `Tier-1` with manifest `pod_runtime_tier=1`; retiring Connect still owns OAuth broker, webhook receiver, signature verification, and connector-adapter retirement evidence.
...
Pack overlays activate per `microservices/connect/compliance.md`:
...
2. Schema-drift detection cadence — currently hourly; do we need real-time (vendor webhook on schema-change)? (Tracking: ICP-CONNECT-SCHEMA-DRIFT-RT)
...
## ADR-0339 adoption
- Lifecycle: PROPOSED for `connect` until service wrappers invoke signed shared OpenTofu modules and implementation evidence lands.
- ADR-0339 adoption keeps reusable HCL in `microservices/cloud-iac/modules/<context>/<primitive>/`; `connect` owns primitive selection and tenant-scoped variables.
