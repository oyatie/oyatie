...
# IP - connect role in j120 Tenant treasury multi-currency FX hedge

## Scope

connect owns the `bank-liquidity-provider-adapter` slice for j120. The service does not own the whole
...
1. connect exposes a tenant-scoped command or handler for `bank-liquidity-provider-adapter`.
...
openapi: 3.2.0
info:
  title: connect j120 bank-liquidity-provider-adapter API
  version: 1.0.0
paths:
...
asyncapi: 3.1.0
info:
  title: connect j120 bank-liquidity-provider-adapter events
  version: 1.0.0
channels:
  connect.journey.j120.bank-liquidity-provider-adapter:
    address: connect.journey.j120.bank-liquidity-provider-adapter
```
...
```proto
syntax = "proto3";
package oyatie.connect.journey.j120;
...
- T-001: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-002: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-003: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-004: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-005: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-006: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-007: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-008: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-009: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-010: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-011: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-012: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-013: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-014: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-015: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-016: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-017: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-018: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-019: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-020: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-021: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-022: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-023: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-024: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-025: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-026: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-027: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-028: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-029: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-030: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-031: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-032: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-033: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-034: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-035: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-036: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-037: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-038: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-039: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
- T-040: `connect` handles j120 `bank-liquidity-provider-adapter` with deterministic idempotency; expected evidence is policy decision, trace span, metric sample, and audit event.
...
IP buildability row 002: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 007: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 012: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 017: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 022: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 027: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 032: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 037: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 042: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 047: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 052: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 057: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 062: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 067: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 072: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 077: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 082: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 087: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 092: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 097: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 102: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 107: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 112: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 117: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 122: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 127: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 132: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 137: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 142: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 147: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 152: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 157: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 162: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 167: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 172: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 177: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 182: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 187: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 192: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 197: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 202: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 207: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 212: connect applies ADR-0297; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
...
IP buildability row 217: connect applies ADR-0308; connect can be implemented independently while preserving bank-liquidity-provider-adapter, policy evidence, and marketplace settlement
