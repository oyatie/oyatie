...
---
# IP-WAVE-15-ZD-sharding-automation: Connect Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/connect/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: connect must interpret ADR-0348 through its own bounded context: connector-catalog.
SCOPE-004: connect uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: connect uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: connect uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: connect.
