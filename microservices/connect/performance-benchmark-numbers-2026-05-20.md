# Connect Performance Benchmark Numbers - 2026-05-20

Audit target: `microservices/connect/`.
...
1. Local PRD performance and scale targets: `microservices/connect/PRD.md:143-158`.
2. Local capacity model and worker scaling assumptions: `microservices/connect/capacity-model.md:15-43`.
...
6. No runtime load test was run because `microservices/connect/src/`, `microservices/connect/tests/`, and canonical context IaC modules are absent.
...
27. OCI disclosure: no `microservices/connect/iac/oci-guest/always-free/` profile exists, so demo_trial caps are target constraints.
...
30. Workload scale basis: PRD requires at least 500 connectors, 10M webhooks/day per tenant at p99, 100k concurrent OAuth grants, and 1M outbound connector actions/min, cited by `microservices/connect/PRD.md:156-158`.
31. Capacity basis: capacity model assumes 1,000 tenants at GA, 50,000 tenants at 24 months, and 100M webhooks/day platform-wide, cited by `microservices/connect/capacity-model.md:15-20`.
32. Worker basis: capacity model models 50 RPS per worker and 2,000 workers for 100k actions/sec, cited by `microservices/connect/capacity-model.md:25-27`.
33. Webhook basis: capacity model models 1,000 RPS per webhook receiver instance and 24 instances for peak webhook load, cited by `microservices/connect/capacity-model.md:31-33`.
34. SLO basis: webhook receiver OpenSLO target is 0.995, cited by `microservices/connect/slos/webhook-receiver-throughput.openslo.yaml:4-39`.
35. SLO basis: connector availability OpenSLO target is 0.999, cited by `microservices/connect/slos/connector-availability.openslo.yaml:4-38`.
36. SLO basis: OAuth token health OpenSLO target is 0.995, cited by `microservices/connect/slos/oauth-token-health.openslo.yaml:4-38`.
37. SLO basis: DLQ overflow prevention OpenSLO target is 0.99, cited by `microservices/connect/slos/dlq-overflow-prevention.openslo.yaml:4-38`.
38. Failure basis: failure modes document covers webhook retry, signature failure, adapter throttling, and DLQ failure, cited by `microservices/connect/failure-modes.md:32-74`.
39. Security basis: PRD requires no secret logging, signed webhooks, replay windows, and policy-gated connector actions, cited by `microservices/connect/PRD.md:184-188`.
...
10. Twilio number T-10: connection establishment rate per subaccount is 110 per second, source: Twilio Conversations limits.
11. Twilio number T-11: connection establishment rate per account is 1,000 per second, source: Twilio Conversations limits.
...
16. Connect implication: Twilio adapter metadata must encode participant, media, sender, and request-rate limits rather than treating Twilio as generic webhook only.
...
14. Connect implication: Sendbird adapter metadata must encode both provider-global and per-user ceilings.

### §2.3 Stream Public Numbers

1. Stream number R-01: default `connect` endpoint rate limit example is 10,000 requests per minute at platform scope, source: Stream rate limits.
2. Stream number R-02: default `connect` endpoint rate limit example is 60 requests per minute at user scope, source: Stream rate limits.
...
14. Connect implication: Stream adapter metadata must encode platform/user rates, member-count caveats, message size, attachment size, webhook HMAC, and dedupe identifiers.

## §3 Oyatie Target Numbers - Single Industry-Leader Target Set

1. Target O-01: connector action latency p50 <= 5 ms local overhead, Oyatie target from `microservices/connect/PRD.md:143-150`.
2. Target O-02: connector action latency p95 <= 20 ms local overhead, Oyatie target from `microservices/connect/PRD.md:143-150`.
3. Target O-03: connector action latency p99 <= 50 ms local overhead, Oyatie target from `microservices/connect/PRD.md:143-150`.
4. Target O-04: OAuth or credential handoff p50 <= 2 s, Oyatie target from `microservices/connect/PRD.md:143-150`.
5. Target O-05: OAuth or credential handoff p95 <= 8 s, Oyatie target from `microservices/connect/PRD.md:143-150`.
6. Target O-06: OAuth or credential handoff p99 <= 15 s, Oyatie target from `microservices/connect/PRD.md:143-150`.
7. Target O-07: webhook acknowledgment p50 <= 30 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
8. Target O-08: webhook acknowledgment p95 <= 100 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
9. Target O-09: webhook acknowledgment p99 <= 200 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
10. Target O-10: webhook-to-downstream availability p50 <= 200 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
11. Target O-11: webhook-to-downstream availability p95 <= 1 s, Oyatie target from `microservices/connect/PRD.md:143-150`.
12. Target O-12: webhook-to-downstream availability p99 <= 2 s, Oyatie target from `microservices/connect/PRD.md:143-150`.
13. Target O-13: connector catalog search p50 <= 50 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
14. Target O-14: connector catalog search p95 <= 150 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
15. Target O-15: connector catalog search p99 <= 300 ms, Oyatie target from `microservices/connect/PRD.md:143-150`.
16. Target O-16: catalog size >= 500 connectors, Oyatie target from `microservices/connect/PRD.md:156`.
17. Target O-17: per-tenant webhook capacity >= 10M webhooks/day at p99, Oyatie target from `microservices/connect/PRD.md:157`.
18. Target O-18: concurrent OAuth grants >= 100,000, Oyatie target from `microservices/connect/PRD.md:157`.
19. Target O-19: outbound connector action throughput >= 1M actions/min, Oyatie target from `microservices/connect/PRD.md:158`.
20. Target O-20: outbound connector action throughput >= 16,667 actions/sec, estimated from 1M actions/min PRD target.
21. Target O-21: platform-wide webhook capacity baseline >= 100M webhooks/day, source: `microservices/connect/capacity-model.md:15-20`.
22. Target O-22: webhook receiver instance capacity target >= 1,000 RPS per instance, source: `microservices/connect/capacity-model.md:31-33`.
23. Target O-23: peak webhook receiver fleet >= 24 instances for modeled load, source: `microservices/connect/capacity-model.md:31-33`.
24. Target O-24: connector worker target >= 50 RPS per worker, source: `microservices/connect/capacity-model.md:25-27`.
25. Target O-25: connector worker fleet target can scale to 2,000 workers for 100k actions/sec modeled load, source: `microservices/connect/capacity-model.md:25-27`.
26. Target O-26: connector availability SLO target >= 0.999, source: `microservices/connect/slos/connector-availability.openslo.yaml:4-38`.
27. Target O-27: webhook receiver throughput SLO target >= 0.995, source: `microservices/connect/slos/webhook-receiver-throughput.openslo.yaml:4-39`.
28. Target O-28: OAuth token health SLO target >= 0.995, source: `microservices/connect/slos/oauth-token-health.openslo.yaml:4-38`.
29. Target O-29: DLQ overflow prevention SLO target >= 0.99, source: `microservices/connect/slos/dlq-overflow-prevention.openslo.yaml:4-38`.
...
31. Target O-31: DLQ replay enqueue <= 1 s p50, <= 5 s p95, <= 10 s p99, source: `microservices/connect/PRD.md:143-150`.
...
33. Target O-33: provider payload redaction must keep secret and PII fields out of logs, source: `microservices/connect/PRD.md:184-188` and `microservices/connect/dpia.md:60-69`.
...
39. Target O-39: connector credential secret exposure target is zero log events, source: `microservices/connect/PRD.md:184-188`.
...
16. Stream comparison: Stream default platform `connect` example of about 333/s is lower than Oyatie’s platform action target, so provider-specific backoff is required.
...
1. The strongest local numeric anchors are the PRD latency and scale targets in `microservices/connect/PRD.md:143-158`.
2. The strongest capacity anchors are `microservices/connect/capacity-model.md:15-43`.
