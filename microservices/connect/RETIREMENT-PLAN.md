...
microservice: connect (umbrella — retiring)
status: Retiring
...
# RETIREMENT-PLAN — Connect umbrella µservice
...
## Trigger — "Connect umbrella retires when…"

The Connect umbrella µservice (this folder, this plan, and any sibling
artifacts in `microservices/connect/`) retires when **all 8** of the
...
1. Delete `microservices/connect/` (this folder, this file, any siblings).
...
- All 15 IPs in `microservices/mail/PHASE-01-MAIL-DISSOLUTION-FROM-CONNECT.md`
  merged.
- ADR-0134 Phase 5 (legacy `oya-connect-mail-*` removal) complete.
...
- All 15 IPs in `microservices/messenger/PHASE-01-TEAM-CHANNELS-DM-THREADS.md`
  merged.
- ADR-0134 Phase 5 (legacy `oya-connect-messenger-*` removal) complete.
...
- All 15 IPs in `microservices/calendar/PHASE-01-CALENDAR-FOUNDATION.md`
  merged.
- ADR-0134 Phase 5 (legacy `oya-connect-calendar-*` removal) complete.
...
- All 15 IPs in `microservices/community/PHASE-01-COMMUNITY-SUBSTRATE.md`
  merged.
- No legacy `oya-connect-community-*` crates exist (verified via `find
  crates -maxdepth 1 -type d -name 'oya-connect-community-*' | wc -l`
...
- Net-new µservices. PRD must be authored. Phase-01 plan must be authored.
  ...
  must be authored. No legacy `oya-connect-{social,shorts,network,anonymous}
  -*` crates exist (verified via `find crates -maxdepth 1 -type d -name
  'oya-connect-{social,shorts,network,anonymous}-*' | wc -l` → 0); these
...
|---|---|---|
| `oya-governance-no-new-suite-bundles` (ADR-0132) | BLOCKER on dev | Already enforces no-new-suite forward-policy |
| `oya-governance-per-microservice-layout` (ADR-0131) | BLOCKER on dev | Already enforces per-µservice flat layout |
| `oya-governance-connect-legacy-symbol-zero-usage` (ADR-0134 §Phase 4) | REPORT-ONLY → BLOCKER | Blocks Phase 5 removal until usage = 0 |
| `oya-governance-connect-umbrella-retirement-readiness` (ADR-0134 §Phase 6 + this plan) | REPORT-ONLY → BLOCKER | Blocks Phase 6 retirement until all 8 HG-<MS> green at p99 sustained 30d |
...
- [ ] **Migration guides exist with concrete steps and examples.**
  ...
  # needed (no legacy `oya-connect-<ms>-*` crates existed); confirm via:
  for ms in community social shorts network anonymous; do
    find crates -maxdepth 1 -type d -name "oya-connect-$ms-*" | wc -l   # expect 0 each
  done
  ```
...
- [ ] **Deprecation notices removed (they served their purpose).**
  ...
  test ! -d microservices/connect/   # this folder absent → umbrella fully retired
  ```
