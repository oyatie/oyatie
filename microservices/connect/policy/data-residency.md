...
# Data Residency Contract — connect
...
Every tenant assigned a primary pack at onboarding. All connect-owned data for that tenant lives in the pack's region-pinned infrastructure. **Cross-pack movement forbidden by default.**
...
## Data classes stored in connect

| Data class | Storage | Residency rule |
|---|---|---|
| OAuth grant metadata (no raw tokens) | Postgres `connect.oauth_grants` | Pinned to tenant's pack region |
...
| Webhook payload digests | Postgres `connect.audit_events` | Pinned |
...
- Pack-kr tenant: `hooks.<tenant-id>.kr.oyatie.app` → resolves to ap-seoul-1 ingress
- Pack-eu tenant: `hooks.<tenant-id>.eu.oyatie.app` → eu-frankfurt-1
- Pack-us tenant: `hooks.<tenant-id>.us.oyatie.app` → us-ashburn-1

ECH config-id per ADR-0253 served on every Tier-0 ingress; HTTPS RR includes `ech=`.

## Vendor egress endpoints

When connect adapter calls a vendor (e.g., Salesforce), the call originates from the tenant's pack region. Vendor receives traffic from the pack-region IP range.
