...
companion_docs:
  ...
  - microservices/connect/runbooks/signature-verification-cascade-failure.md
  - microservices/connect/runbooks/dlq-overflow.md
doc_status: published
---

# Incident Response — connect
...
4. **Initial triage** (≤10min):
   - Check `oya_connect_*` baseline signals on Grafana dashboard `connect-overview`.
   - Check audit chain for recent privileged actions (`OAuthGrantRevoked`, `ProviderCredentialRotated`).
   - Check Cedar deny rate (`oya_cedar_deny_total{ms="connect"}`) for anomalies.
5. **Mitigation options** (in order):
   ...
   - Kill-switch (per ADR-0295): `kubectl annotate ns connect oya.kill-switch=true` — last resort.
