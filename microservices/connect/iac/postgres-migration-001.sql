...
-- ─── oauth_grants ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS connect.oauth_grants (
...
CREATE INDEX IF NOT EXISTS idx_oauth_grants_tenant_connector
    ON connect.oauth_grants (tenant_id, connector_name)
    WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_oauth_grants_expires_at
    ON connect.oauth_grants (expires_at)
    WHERE status = 'active';

-- RLS: tenant isolation enforced by generated JWT claim (set by tenancy µservice)
ALTER TABLE connect.oauth_grants ENABLE ROW LEVEL SECURITY;
CREATE POLICY oauth_grants_tenant_isolation ON connect.oauth_grants
...
-- ─── dlq_entries ──────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS connect.dlq_entries (
...
CREATE INDEX IF NOT EXISTS idx_dlq_entries_tenant_status
    ON connect.dlq_entries (tenant_id, status, next_retry_at)
    WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS idx_dlq_entries_connector
    ON connect.dlq_entries (connector_name, status);

ALTER TABLE connect.dlq_entries ENABLE ROW LEVEL SECURITY;
CREATE POLICY dlq_entries_tenant_isolation ON connect.dlq_entries
    USING (tenant_id = current_setting('app.current_tenant_id')::UUID);
