...
- `oya_connect_dlq_depth{tenant_id="<X>"} > tenant_retention_cap × 0.8`
...
oya_connect_dlq_depth{tenant_id="<X>"}

# Replay success rate
rate(oya_connect_dlq_replay_total{outcome="success"}[5m])
  / rate(oya_connect_dlq_replay_total[5m])
```
...
- If bug: regression test in `oya-connect-retry-and-dlq-domain`
...
- `microservices/connect/compliance.md` (retention by pack)
