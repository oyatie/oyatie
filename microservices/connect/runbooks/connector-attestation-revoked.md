...
companion_docs:
  ...
  - microservices/connect/policy/connector-catalog-publishing.cedar
...
- `oya_connect_catalog_attestation_verify_fail_total{connector="<X>"} > 0`
...
curl -s http://connect-adapter-worker.connect:9090/metrics \
  | grep "oya_connect_circuit_breaker_open_total{connector=\"${CONNECTOR_NAME}\"}"
# Expected: 0
