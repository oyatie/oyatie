---
doc_class: Runbook
doc_id: RUNBOOK-CONNECT-COLD_MERGE
...
# Runbook: Connect Cold Merge

## Operator Contract
- Runbook id: connect-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-connect-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=connect; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n connect rollout status deploy/connect --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service connect --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service connect --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service connect --cell $CELL --window 30m`.
