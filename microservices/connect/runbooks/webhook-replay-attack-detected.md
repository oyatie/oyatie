...
- `oya_connect_webhook_replay_blocked_total > N` (configurable; default N=10 in 5min)
...
1. **Confirm replay pattern** (≤2min)
   ...
   # Source IPs
   curl 'http://connect-webhook-receiver-edge.connect:9090/metrics' | \
     grep 'oya_connect_webhook_replay_blocked_total'
   ```
...
4. **Rotate webhook signing secret** (if compromise suspected)
   ...
   curl -X POST http://connect-webhook-receiver-edge.connect:8080/admin/rotate-secret \
...
curl http://connect-webhook-receiver-edge.connect:9090/metrics | grep oya_connect_webhook_replay_blocked_total

# Tenant traffic unaffected (UX-floor)
curl http://connect-webhook-receiver-edge.connect:9090/metrics | grep 'oya_connect_webhook_receive_p99_seconds{outcome="2xx"}'
# Expected: p99 ≤ 0.1s
```
