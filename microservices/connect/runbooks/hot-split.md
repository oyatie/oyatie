---
doc_class: Runbook
doc_id: RUNBOOK-CONNECT-HOT_SPLIT
...
# Runbook: Connect Hot Split

## Operator Contract
- Runbook id: connect-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-connect-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=connect; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n connect rollout status deploy/connect --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service connect --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service connect --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service connect --cell $CELL --window 30m`.
