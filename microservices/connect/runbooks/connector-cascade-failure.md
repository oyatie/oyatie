...
- `oya_connect_action_error_rate_5m{connector="<X>"} > 0.5` for 5+ minutes
- `oya_connect_circuit_breaker_open_total{connector="<X>"} > 0`
- PagerDuty alert `ConnectorCascadeFailure` fires
- Tenant reports widespread "vendor error" responses

## B. Pre-checks

1. Confirm scope: `kubectl exec -n connect deploy/connect-adapter-worker -- curl localhost:9090/metrics | grep oya_connect_circuit_breaker_open_total` — which connector(s)?
2. Check vendor status page (e.g., status.salesforce.com, status.stripe.com).
3. Check our egress network: `kubectl exec -n connect deploy/connect-adapter-worker -- nc -zv <vendor-host> 443`.
...
1. **Confirm vendor outage vs oyatie issue** (≤2min)
   ...
   - Audit `oya.connect.adapter-worker` for recent deploy correlation.
...
4. **Vendor outage path** (vendor-side issue)
   - Verify circuit-breaker is open (`oya_connect_circuit_breaker_open_total{connector="<X>"} > 0`).
...
5. **DLQ accumulation** (≤5min)
   - Check DLQ depth: `oya_connect_dlq_depth{connector="<X>"}`.
...
kubectl exec -n observability prometheus-0 -- promtool query instant \
  "oya_connect_action_error_rate_5m{connector='<X>'}"

# Circuit-breaker closed
curl http://connect-adapter-worker.connect:9090/metrics | grep oya_connect_circuit_breaker_open

# DLQ stable or draining
curl http://connect-adapter-worker.connect:9090/metrics | grep oya_connect_dlq_depth
```
...
- Update vendor SLA tracking in `microservices/connect/catalog/connectors/<connector>.yaml` if vendor-side.
- If oyatie-side regression: add property test to `oya-connect-connector-adapter-domain` covering the regression.
