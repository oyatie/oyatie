...
- `oya_connect_action_rate_limit_total{outcome="hit"} > threshold` for 10+ minutes
...
1. **Identify hotspot** (≤2min)
   ```promql
   topk(10, sum by (tenant_id, connector) (rate(oya_connect_action_rate_limit_total{outcome="hit"}[5m])))
   ```
...
sum(rate(oya_connect_action_rate_limit_total{outcome="hit"}[5m]))

# Shard distribution balanced
sum by (shard) (oya_valkey_keys_total{db="connect"})
```
