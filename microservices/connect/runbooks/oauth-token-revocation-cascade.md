...
1. **Quantify scope** (≤5min)
   ```sql
   SELECT count(*), tenant_id, status FROM connect.oauth_grants
...
```sql
SELECT count(*) FROM connect.oauth_grants
...
curl http://connect-oauth-broker.connect:9090/metrics | grep oya_connect_oauth_grant_revoke_total
```
...
- `microservices/connect/runbooks/admin-mfa-cascade.md` (referenced from ops-dashboard)
