...
- DLP egress scan alerts: `oya_connect_dlq_pii_egress_blocked_total > 0`
...
- `oya_connect_payload_pii_detected_total{connector="<X>",destination="external"} > 0`
...
3. **Confirm DLP block status** — did the egress scan catch it before transmission?
   ```bash
   curl -s http://connect-adapter-worker.connect:9090/metrics \
     | grep oya_connect_payload_pii_detected_total
...
2. **Revoke affected OAuth grants** (≤5min)
   ...
   for TENANT_ID in "${AFFECTED_TENANTS[@]}"; do
     curl -s -X POST \
       "http://connect-oauth-broker.connect/internal/grants/revoke-all" \
...
curl -s http://connect-adapter-worker.connect:9090/metrics \
  | grep "oya_connect_payload_pii_transmitted_total{connector=\"${CONNECTOR_NAME}\"}"
# Expected: counter should not be incrementing post-patch

# Confirm DLP block active
kubectl exec -n connect deploy/connect-adapter-worker -- \
  curl localhost:9090/metrics | grep oya_connect_dlq_pii_egress_blocked_total
...
- `microservices/connect/policy/data-residency.md`
- `microservices/connect/runbooks/connector-cascade-failure.md`
- `microservices/connect/compliance.md §pack-overlay-roster`
