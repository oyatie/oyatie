...
- `oya_connect_webhook_signature_verify_fail_total > threshold` (default 100/min)
...
1. **Identify scope** (≤2min)
   ```promql
   topk(10, sum by (tenant_id, connector) (rate(oya_connect_webhook_signature_verify_fail_total[5m])))
   ```
...
rate(oya_connect_webhook_signature_verify_fail_total[5m])

# Verify-success rate restored
rate(oya_connect_webhook_signature_verify_success_total[5m])
```
