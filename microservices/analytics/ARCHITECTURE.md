...
## §principals
This anchor is closed for `analytics` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.analytics.runtime` owns normal `tenant-dashboard-query` execution and never borrows a tenant principal.
- Platform principal `oyatie.analytics.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/analytics/backfill-replay.md` when present.
- Platform principal `oyatie.analytics.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.analytics.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.analytics.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `tenant-dashboard-query` is evaluated as `<tenant>.analytics.tenant-dashboard-query` before any `analytics` state mutation.
...
## §cedar-gates
This anchor is closed for `analytics` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/analytics/policy/audit-log-pii.cedar`, `microservices/analytics/policy/dashboard.cedar`, `microservices/analytics/policy/dual-context.cedar`, `microservices/analytics/policy/regulator-export.cedar`, `microservices/analytics/policy/residency.cedar`, `microservices/analytics/policy/tenant-bootstrap.cedar`.
- Action namespace uses `analytics::<bounded_context>::<verb>`; the first protected action is `analytics::tenant-dashboard-query::execute`.
...
- Concrete fragments in scope: `microservices/analytics/policy/audit-log-pii.cedar`, `microservices/analytics/policy/dashboard.cedar`, `microservices/analytics/policy/dual-context.cedar`, `microservices/analytics/policy/regulator-export.cedar`, `microservices/analytics/policy/residency.cedar`, `microservices/analytics/policy/tenant-bootstrap.cedar`.
- Example: `analytics::tenant-dashboard-query::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `analytics` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `analytics.tenant_dashboard_query` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `analytics.billing_rollup` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `analytics.audit_log_search` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `analytics.data_export` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- Planned table/event surface `analytics.tenant_dashboard_query_5` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
...
- Example: `tenant-dashboard-query` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `analytics` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `analytics` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `analytics` only through contracts `microservices/analytics/contracts/analytics.proto`, `microservices/analytics/contracts/asyncapi-v1.yaml`, `microservices/analytics/contracts/graphql-v1.sdl`, `microservices/analytics/contracts/openapi-v1.yaml`.
- If product: `analytics` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `tenant-dashboard-query`, `billing-rollup`, `audit-log-search`, `data-export`.
- Example: `tenant-dashboard-query` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `analytics` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/analytics/policy/audit-log-pii.cedar`, `microservices/analytics/policy/dashboard.cedar`, `microservices/analytics/policy/dual-context.cedar`, `microservices/analytics/policy/regulator-export.cedar`, `microservices/analytics/policy/residency.cedar`, `microservices/analytics/policy/tenant-bootstrap.cedar` with the full action/resource/context tuple.
...
- Example: `analytics::tenant-dashboard-query::execute` evaluates locally, then emits an audit event before the usecase writes `analytics.tenant_dashboard_query`.
...
## §cell-eligibility
This anchor is closed for `analytics` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `tenant-dashboard-query` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `analytics` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `analytics` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `analytics.tenant-dashboard-query` and is included in every AI/risk/model call audit event.
...
- Example: `tenant-dashboard-query` can request classification or summarization with `audience_tag=analytics.tenant-dashboard-query`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `analytics` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `analytics` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `tenant-dashboard-query` is read/written as an ontology node or edge projection when `analytics` needs cross-surface discovery.
- Bounded-context entity `billing-rollup` is read/written as an ontology node or edge projection when `analytics` needs cross-surface discovery.
- Bounded-context entity `audit-log-search` is read/written as an ontology node or edge projection when `analytics` needs cross-surface discovery.
- Bounded-context entity `data-export` is read/written as an ontology node or edge projection when `analytics` needs cross-surface discovery.
...
- Example: `tenant-dashboard-query` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `analytics` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/analytics/contracts/analytics.proto`, `microservices/analytics/contracts/asyncapi-v1.yaml`, `microservices/analytics/contracts/graphql-v1.sdl`, `microservices/analytics/contracts/openapi-v1.yaml`.
...
- IaC transport evidence: `microservices/analytics/iac/clickhouse/mv-templates/audit-log-table.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-day-billing-per-resource.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-hour-workflow-per-tenant.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-minute-error-burst-per-tenant.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-month-billing-per-resource.sql`, `microservices/analytics/iac/helm/clickhouse-analytics/Chart.yaml`; +6 more.
- Example: `analytics` `tenant-dashboard-query` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `analytics` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `tenant-dashboard-query`, `billing-rollup`, `audit-log-search`, `data-export` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/analytics/iac/clickhouse/mv-templates/audit-log-table.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-day-billing-per-resource.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-hour-workflow-per-tenant.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-minute-error-burst-per-tenant.sql`, `microservices/analytics/iac/clickhouse/mv-templates/mv-month-billing-per-resource.sql`, `microservices/analytics/iac/helm/clickhouse-analytics/Chart.yaml`; +6 more.
- Catalog crates/components: `microservices/analytics/catalog/README.md`, `microservices/analytics/catalog/capabilities.json`, `microservices/analytics/catalog/cedar-policies.json`, `microservices/analytics/catalog/clickhouse-analytics-helm.json`, `microservices/analytics/catalog/contracts.json`, `microservices/analytics/catalog/dashboards.json`; +6 more.
- Example: `tenant-dashboard-query` API runs as a Kata-isolated container; scheduled/background work for `analytics` uses a separate worker principal.
...
## §observability
This anchor is closed for `analytics` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/analytics/slos/audit-log-query-cold-latency.openslo.yaml`, `microservices/analytics/slos/audit-log-query-latency.openslo.yaml`, `microservices/analytics/slos/billing-reconciliation.openslo.yaml`, `microservices/analytics/slos/clickhouse-ingest-lag.openslo.yaml`, `microservices/analytics/slos/clickhouse-query-latency.openslo.yaml`, `microservices/analytics/slos/cluster-availability.openslo.yaml`; +7 more.
- Audit classes use prefix `oya.analytics.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `analytics_tenant_dashboard_query_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `analytics_tenant_dashboard_query_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/analytics/slos/audit-log-query-cold-latency.openslo.yaml`.
...
- Example trace: `analytics.tenant-dashboard-query` -> `policy.evaluate` -> `storage.tenant_dashboard_query` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `analytics` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `analytics` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `tenant-dashboard-query` action class.
...
- Cedar/IaC controls in scope: `microservices/analytics/policy/audit-log-pii.cedar`, `microservices/analytics/policy/dashboard.cedar`, `microservices/analytics/policy/dual-context.cedar`, `microservices/analytics/policy/regulator-export.cedar`, `microservices/analytics/policy/residency.cedar`, `microservices/analytics/policy/tenant-bootstrap.cedar`; +12 more.
- Canary payloads for `analytics` use fake `tenant-dashboard-query` identifiers and honey endpoints under `/.well-known/oya-canary/analytics`.
...
This anchor is closed for `analytics` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `analytics` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `analytics` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `analytics` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `tenant-dashboard-query` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `analytics` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `analytics` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `analytics` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/analytics/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/analytics/runbooks/capacity-rebalance.md`, `microservices/analytics/runbooks/clickhouse.md`, `microservices/analytics/runbooks/cold-tier-latency.md`, `microservices/analytics/runbooks/ingest-lag-burn.md`, `microservices/analytics/runbooks/keeper-quorum-recovery.md`, `microservices/analytics/runbooks/mv-lag-triage.md`; +2 more.
- Example: `tenant-dashboard-query` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
