use clap::Parser;
use oya_plant_maintenance_work_order_app::adapter::AdapterRegistry;
use oya_plant_maintenance_work_order_app::config::ServiceConfig;
use oya_plant_maintenance_work_order_app::error::Result;
use std::path::PathBuf;
use std::process::ExitCode;

#[derive(Debug, Parser)]
#[command(name = "oya-plant-maintenance-work-order")]
#[command(about = "Plant Maintenance service scaffold entrypoint")]
struct CliArgs {
    #[arg(long)]
...
fn run(args: CliArgs) -> Result<()> {
    ...
    tracing::info!(
        ...
        "plant maintenance scaffold validated"
    );
    Ok(())
}
