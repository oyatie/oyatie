...
## §principals
This anchor is closed for `docs` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.docs.runtime` owns normal `T0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.docs.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/docs/backfill-replay.md` when present.
- Platform principal `oyatie.docs.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.docs.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.docs.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `T0-suggest` is evaluated as `<tenant>.docs.t0-suggest` before any `docs` state mutation.
...
## §cedar-gates
This anchor is closed for `docs` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/docs/policy/auditor-scope.cedar`, `microservices/docs/policy/ci-scope.cedar`, `microservices/docs/policy/data-residency.md`, `microservices/docs/policy/editor-isolation.md`, `microservices/docs/policy/public-read.cedar`, `microservices/docs/policy/tenant-scope.cedar`.
- Action namespace uses `docs::<bounded_context>::<verb>`; the first protected action is `docs::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/docs/policy/auditor-scope.cedar`, `microservices/docs/policy/ci-scope.cedar`, `microservices/docs/policy/data-residency.md`, `microservices/docs/policy/editor-isolation.md`, `microservices/docs/policy/public-read.cedar`, `microservices/docs/policy/tenant-scope.cedar`.
- Example: `docs::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `docs` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `docs.block_types` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `docs.collab_crdt` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `docs.comments_and_suggestions` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `docs.document_store` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `docs.embed_resolver` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
This anchor is closed for `docs` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `docs` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `docs` only through contracts `microservices/docs/contracts/asyncapi/docs-events.yaml`, `microservices/docs/contracts/openapi/docs.yaml`, `microservices/docs/contracts/proto/docs.proto`.
- If product: `docs` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `block-types`, `collab-crdt`, `comments-and-suggestions`, `document-store`, `embed-resolver`, `export-import`; +2 more.
...
## §policy-evaluation
This anchor is closed for `docs` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/docs/policy/auditor-scope.cedar`, `microservices/docs/policy/ci-scope.cedar`, `microservices/docs/policy/data-residency.md`, `microservices/docs/policy/editor-isolation.md`, `microservices/docs/policy/public-read.cedar`, `microservices/docs/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `docs::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `docs.block_types`.
...
## §cell-eligibility
This anchor is closed for `docs` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `docs` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `docs` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `docs.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `T0-suggest` can request classification or summarization with `audience_tag=docs.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `docs` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `docs` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `block-types` is read/written as an ontology node or edge projection when `docs` needs cross-surface discovery.
- Bounded-context entity `collab-crdt` is read/written as an ontology node or edge projection when `docs` needs cross-surface discovery.
- Bounded-context entity `comments-and-suggestions` is read/written as an ontology node or edge projection when `docs` needs cross-surface discovery.
- Bounded-context entity `document-store` is read/written as an ontology node or edge projection when `docs` needs cross-surface discovery.
...
## §transport
This anchor is closed for `docs` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/docs/contracts/asyncapi/docs-events.yaml`, `microservices/docs/contracts/openapi/docs.yaml`, `microservices/docs/contracts/proto/docs.proto`.
...
- IaC transport evidence: `microservices/docs/iac/helm/Chart.yaml`, `microservices/docs/iac/helm/templates/deployment.yaml`, `microservices/docs/iac/helm/templates/hpa.yaml`, `microservices/docs/iac/helm/templates/networkpolicy.yaml`, `microservices/docs/iac/helm/templates/pdb.yaml`, `microservices/docs/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Example: `docs` `T0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `docs` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `block-types`, `collab-crdt`, `comments-and-suggestions`, `document-store`, `embed-resolver`, `export-import`; +2 more and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/docs/iac/helm/Chart.yaml`, `microservices/docs/iac/helm/templates/deployment.yaml`, `microservices/docs/iac/helm/templates/hpa.yaml`, `microservices/docs/iac/helm/templates/networkpolicy.yaml`, `microservices/docs/iac/helm/templates/pdb.yaml`, `microservices/docs/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/docs/catalog/oya-docs-block-types-kernel.yaml`, `microservices/docs/catalog/oya-docs-collab-crdt-adapter-valkey.yaml`, `microservices/docs/catalog/oya-docs-collab-crdt-adapter.yaml`, `microservices/docs/catalog/oya-docs-collab-crdt-kernel.yaml`, `microservices/docs/catalog/oya-docs-comments-and-suggestions-adapter-postgres.yaml`, `microservices/docs/catalog/oya-docs-comments-and-suggestions-kernel.yaml`; +18 more.
- Example: `T0-suggest` API runs as a Kata-isolated container; scheduled/background work for `docs` uses a separate worker principal.
...
## §observability
This anchor is closed for `docs` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/docs/slos/collab-cursor-sync-latency.openslo.yaml`, `microservices/docs/slos/crdt-merge-no-silent-loss.openslo.yaml`, `microservices/docs/slos/doc-list-latency.openslo.yaml`, `microservices/docs/slos/doc-open-latency.openslo.yaml`, `microservices/docs/slos/export-pdf-latency.openslo.yaml`, `microservices/docs/slos/pandoc-export-pipeline-availability.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.docs.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `docs_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `docs_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/docs/slos/collab-cursor-sync-latency.openslo.yaml`.
...
- Example trace: `docs.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `docs` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `docs` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/docs/policy/auditor-scope.cedar`, `microservices/docs/policy/ci-scope.cedar`, `microservices/docs/policy/data-residency.md`, `microservices/docs/policy/editor-isolation.md`, `microservices/docs/policy/public-read.cedar`, `microservices/docs/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `docs` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/docs`.
...
This anchor is closed for `docs` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `docs` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `docs` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `docs` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `docs` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `docs` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `docs` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/docs/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/docs/runbooks/attachment-restore.md`, `microservices/docs/runbooks/collab-conflict-resolution.md`, `microservices/docs/runbooks/doc-version-restore-corruption.md`, `microservices/docs/runbooks/editor-session-storm-throttle.md`, `microservices/docs/runbooks/embed-source-stale-detection.md`, `microservices/docs/runbooks/export-pipeline-failure-pandoc-rollback.md`; +1 more.
