...
This addendum converts the short implementation note into a buildable docs-service slice for Mermaid/C4 build; it does not rely on line count as proof.
...
For Mermaid/C4 build, the implementation must preserve dual-context separation, per-block ACL, legal hold, audit-chain records, and export/import safety described in `microservices/docs/PRD.md` and `microservices/docs/ARCHITECTURE.md`.

### Concrete mechanism
Primary artifact: `microservices/docs/ARCHITECTURE.md`.
Domain entities or operational surfaces: architecture diagrams generated from accepted docs.
...
1. Bind Mermaid/C4 build to the matching bounded context in `microservices/docs/manifest.json` and the catalog row named in this addendum.
...
|---|---|---|
| Google Docs / Microsoft Word Online | Native collaborative authoring with comments, sharing, history, and export. | `Mermaid/C4 build` must be first-party, tenant-scoped, auditable, and legal-hold aware. |
| Notion / Coda | Block-centric documents with embeds and structured workflows. | `Mermaid/C4 build` must preserve block ACL, embed policy checks, and cross-service refresh evidence instead of hidden workspace coupling. |
| Quip / GitHub | Team review and change history expectations. | `Mermaid/C4 build` must expose signed audit events, version/revert evidence, and contract-rendered developer docs. |
...
- Extra verification: rendered Mermaid and C4 output must be checked against `microservices/docs/ARCHITECTURE.md` before promotion.
