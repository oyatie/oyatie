...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that docs
  (CRDT collab + pandoc export pipeline + share ACL) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/docs.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/docs/iac/helm/templates/prometheusrule.yaml
---

# docs — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`docs`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-DOCS`)
...
- **How docs satisfies it**:
  - Capability calls in `microservices/docs/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_docs_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_docs_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/docs/iac/helm/templates/prometheusrule.yaml` (`docs-llm-capability-circuit-open`
    + `docs-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/docs/dashboards/` (per-capability panels).
  - Threat model: `microservices/docs/threat-model.md` §"Dependency
...
- **How docs satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_docs_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/docs/iac/helm/templates/prometheusrule.yaml` (`docs-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/docs/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/docs/capacity-model.md` (where
...
- **How docs satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/docs/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/docs/iac/helm/templates/prometheusrule.yaml` (`docs-saturation-cpu-over-70pct`,
    `docs-errors-5xx-rate-spike`, `docs-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/docs/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/docs/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How docs satisfies it**:
  - Every SLO in `microservices/docs/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/docs/iac/helm/templates/prometheusrule.yaml` (`docs-*-fast-burn-1h-14x` +
    `docs-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/docs/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/docs/hyperscaler-conformance.md
test -f microservices/docs/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/docs/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/docs/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/docs/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/docs/iac/helm/templates/prometheusrule.yaml || true
