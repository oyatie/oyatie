...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that slides
  (deck render + present mode + broadcast + export) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/slides.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/slides/iac/helm/templates/prometheusrule.yaml
---

# slides — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`slides`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-SLIDES`)
...
- **How slides satisfies it**:
  - Capability calls in `microservices/slides/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_slides_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_slides_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/slides/iac/helm/templates/prometheusrule.yaml` (`slides-llm-capability-circuit-open`
    + `slides-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/slides/dashboards/` (per-capability panels).
  - Threat model: `microservices/slides/threat-model.md` §"Dependency
...
- **How slides satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_slides_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/slides/iac/helm/templates/prometheusrule.yaml` (`slides-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/slides/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/slides/capacity-model.md` (where
...
- **How slides satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/slides/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/slides/iac/helm/templates/prometheusrule.yaml` (`slides-saturation-cpu-over-70pct`,
    `slides-errors-5xx-rate-spike`, `slides-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/slides/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/slides/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How slides satisfies it**:
  - Every SLO in `microservices/slides/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/slides/iac/helm/templates/prometheusrule.yaml` (`slides-*-fast-burn-1h-14x` +
    `slides-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/slides/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/slides/hyperscaler-conformance.md
test -f microservices/slides/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/slides/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/slides/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/slides/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/slides/iac/helm/templates/prometheusrule.yaml || true
