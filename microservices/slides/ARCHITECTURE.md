...
## §principals
This anchor is closed for `slides` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.slides.runtime` owns normal `T0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.slides.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/slides/backfill-replay.md` when present.
- Platform principal `oyatie.slides.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.slides.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.slides.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `T0-suggest` is evaluated as `<tenant>.slides.t0-suggest` before any `slides` state mutation.
...
## §cedar-gates
This anchor is closed for `slides` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/slides/policy/auditor-scope.cedar`, `microservices/slides/policy/ci-scope.cedar`, `microservices/slides/policy/data-residency.md`, `microservices/slides/policy/editor-isolation.md`, `microservices/slides/policy/public-read.cedar`, `microservices/slides/policy/tenant-scope.cedar`.
- Action namespace uses `slides::<bounded_context>::<verb>`; the first protected action is `slides::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/slides/policy/auditor-scope.cedar`, `microservices/slides/policy/ci-scope.cedar`, `microservices/slides/policy/data-residency.md`, `microservices/slides/policy/editor-isolation.md`, `microservices/slides/policy/public-read.cedar`, `microservices/slides/policy/tenant-scope.cedar`.
- Example: `slides::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `slides` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `slides.unknown` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- Planned table/event surface `slides.t0_suggest_2` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `slides.t0_suggest_3` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `slides.t0_suggest_4` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `slides.t0_suggest_5` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
...
This anchor is closed for `slides` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `slides` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `slides` only through contracts `microservices/slides/contracts/asyncapi/slides-events.yaml`, `microservices/slides/contracts/openapi/slides.yaml`, `microservices/slides/contracts/proto/slides.proto`.
- If product: `slides` may call substrate services but must not create product-to-product synchronous dependencies.
...
## §policy-evaluation
This anchor is closed for `slides` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/slides/policy/auditor-scope.cedar`, `microservices/slides/policy/ci-scope.cedar`, `microservices/slides/policy/data-residency.md`, `microservices/slides/policy/editor-isolation.md`, `microservices/slides/policy/public-read.cedar`, `microservices/slides/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `slides::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `slides.unknown`.
...
## §cell-eligibility
This anchor is closed for `slides` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `slides` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `slides` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `slides.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `T0-suggest` can request classification or summarization with `audience_tag=slides.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `slides` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `slides` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `unknown` is read/written as an ontology node or edge projection when `slides` needs cross-surface discovery.
...
## §transport
This anchor is closed for `slides` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/slides/contracts/asyncapi/slides-events.yaml`, `microservices/slides/contracts/openapi/slides.yaml`, `microservices/slides/contracts/proto/slides.proto`.
...
- IaC transport evidence: `microservices/slides/iac/helm/Chart.yaml`, `microservices/slides/iac/helm/templates/deployment.yaml`, `microservices/slides/iac/helm/templates/hpa.yaml`, `microservices/slides/iac/helm/templates/networkpolicy.yaml`, `microservices/slides/iac/helm/templates/pdb.yaml`, `microservices/slides/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Example: `slides` `T0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `slides` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.
...
- IaC manifests in scope: `microservices/slides/iac/helm/Chart.yaml`, `microservices/slides/iac/helm/templates/deployment.yaml`, `microservices/slides/iac/helm/templates/hpa.yaml`, `microservices/slides/iac/helm/templates/networkpolicy.yaml`, `microservices/slides/iac/helm/templates/pdb.yaml`, `microservices/slides/iac/helm/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/slides/catalog/oya-slides-acl-adapter-postgres.yaml`, `microservices/slides/catalog/oya-slides-acl-domain.yaml`, `microservices/slides/catalog/oya-slides-acl-kernel.yaml`, `microservices/slides/catalog/oya-slides-ai-content-generation-domain.yaml`, `microservices/slides/catalog/oya-slides-broadcast-mode-adapter-livekit.yaml`, `microservices/slides/catalog/oya-slides-image-adapter-clamav.yaml`; +18 more.
- Example: `T0-suggest` API runs as a Kata-isolated container; scheduled/background work for `slides` uses a separate worker principal.
...
## §observability
This anchor is closed for `slides` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/slides/slos/broadcast-mode-availability.openslo.yaml`, `microservices/slides/slos/collab-cursor-sync-latency.openslo.yaml`, `microservices/slides/slos/crdt-merge-no-silent-loss.openslo.yaml`, `microservices/slides/slos/deck-open-latency.openslo.yaml`, `microservices/slides/slos/export-mp4-latency.openslo.yaml`, `microservices/slides/slos/export-pdf-latency.openslo.yaml`; +7 more.
- Audit classes use prefix `oya.slides.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `slides_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `slides_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/slides/slos/broadcast-mode-availability.openslo.yaml`.
...
- Example trace: `slides.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `slides` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `slides` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/slides/policy/auditor-scope.cedar`, `microservices/slides/policy/ci-scope.cedar`, `microservices/slides/policy/data-residency.md`, `microservices/slides/policy/editor-isolation.md`, `microservices/slides/policy/public-read.cedar`, `microservices/slides/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `slides` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/slides`.
...
This anchor is closed for `slides` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `slides` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `slides` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `slides` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `slides` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `slides` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `slides` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/slides/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/slides/runbooks/animation-engine-rollback.md`, `microservices/slides/runbooks/attachment-restore.md`, `microservices/slides/runbooks/broadcast-mode-degraded.md`, `microservices/slides/runbooks/collab-conflict-resolution-crdt.md`, `microservices/slides/runbooks/export-pipeline-failure-pptx.md`, `microservices/slides/runbooks/share-acl-drift.md`; +1 more.
