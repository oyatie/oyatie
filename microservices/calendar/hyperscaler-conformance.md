...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that calendar
  (agenda render + CalDAV + RSVP fanout) satisfies four hyperscaler-grade invariants drawn from
...
prometheusrule_path: microservices/calendar/iac/helm/templates/prometheusrule.yaml
---

# calendar — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`calendar`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-CALENDAR`)
...
- **How calendar satisfies it**:
  - Capability calls in `microservices/calendar/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_calendar_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_calendar_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/calendar/iac/helm/templates/prometheusrule.yaml` (`calendar-llm-capability-circuit-open`
    + `calendar-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/calendar/dashboards/` (per-capability panels).
  - Threat model: `microservices/calendar/threat-model.md` §"Dependency
...
- **How calendar satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_calendar_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/calendar/iac/helm/templates/prometheusrule.yaml` (`calendar-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/calendar/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/calendar/capacity-model.md` (where
...
- **How calendar satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/calendar/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/calendar/iac/helm/templates/prometheusrule.yaml` (`calendar-saturation-cpu-over-70pct`,
    `calendar-errors-5xx-rate-spike`, `calendar-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/calendar/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/calendar/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How calendar satisfies it**:
  - Every SLO in `microservices/calendar/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/calendar/iac/helm/templates/prometheusrule.yaml` (`calendar-*-fast-burn-1h-14x` +
    `calendar-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/calendar/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/calendar/hyperscaler-conformance.md
test -f microservices/calendar/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/calendar/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/calendar/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/calendar/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/calendar/iac/helm/templates/prometheusrule.yaml || true
