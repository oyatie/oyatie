...
Audience: an Oyatie operator or migration owner preparing `cloud-iac` for the Wave 15-ZD doctrine surface before implementation waves author runtime code, manifests, Jenkinsfiles, ArgoCD applications, or sharding bodies.
...
1. Treat `oya verify --ci-required` as the local rehearsal for `cloud-iac` changes before push.
...
1. Search `cloud-iac` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `cloud-iac` manifest `fitness_lanes` array only in the manifest-owning slot.
...
1. Treat Jenkins LTS as the self-hostable CI substrate for `cloud-iac` when GitHub Actions runners are unavailable.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `cloud-iac`.
