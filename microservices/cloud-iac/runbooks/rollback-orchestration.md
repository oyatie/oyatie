...
1. Identify the rollback target apply: `cargo run -p oya-dev-cli -- iac history --microservice <ms> --env <env> --limit 5`.
...
| 3 | Invoke rollback: `cargo run -p oya-dev-cli -- iac rollback --microservice <ms> --env production --to-apply <apply-id> --reason "<rfc>"`. The CLI: <br> a. validates signature requirements; <br> b. signs the rollback payload with iac-rollback-worker Ed25519 key (or 2-person-rule manual sign if from operator console); <br> c. orchestrates the revert: ArgoCD re-syncs the prior apply's manifest set; <br> d. updates iac-state-index with the rollback row (signed); <br> e. emits `ApplyRolledBack` event consumed by audit-chain + observability + grafana-oncall | ≤ 1min |
| 4 | Verify cluster state reverts: ArgoCD app shows `Synced` at prior apply's content digest within ≤ 2min | ≤ 2min |
| 5 | Verify apply-state index reflects rollback: `cargo run -p oya-dev-cli -- iac status --microservice <ms> --env production` returns prior apply id + `rolled_back_at` timestamp | ≤ 1min |
...
| 2 | Identify the next-best apply in `cargo run -p oya-dev-cli -- iac history --limit 10` |
...
After completion:
- `cargo run -p oya-dev-cli -- iac status --microservice <ms> --env production` returns the rollback target apply-id.
