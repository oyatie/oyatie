...
1. Treat `oya verify --ci-required` as the local rehearsal for `cloud-kms` changes before push.
...
3. When `cloud-kms` changes touch Rust, contracts, manifests, generated docs, or governance lanes, run the verifier before handoff.
...
1. Search `cloud-kms` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `cloud-kms` manifest `fitness_lanes` array only in the manifest-owning slot.
...
1. Treat Jenkins LTS as the self-hostable CI substrate for `cloud-kms` when GitHub Actions runners are unavailable.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `cloud-kms`.
