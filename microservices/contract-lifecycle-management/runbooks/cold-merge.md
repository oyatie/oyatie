---
doc_class: Runbook
doc_id: RUNBOOK-CONTRACT_LIFECYCLE_MANAGEMENT-COLD_MERGE
...
# Runbook: Contract Lifecycle Management Cold Merge

## Operator Contract
- Runbook id: contract-lifecycle-management-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-contract-lifecycle-management-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=contract-lifecycle-management; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
...
3. Verify alerts: `oya observability alerts list --service contract-lifecycle-management --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service contract-lifecycle-management --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service contract-lifecycle-management --cell $CELL --window 30m`.
