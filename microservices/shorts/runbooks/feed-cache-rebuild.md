...
- `oya_shorts_feed_cache_inconsistency_total` > 0 (Redis split-brain or AOF corruption).
- Manual rebuild after Redis flush or schema migration or ranking-model version promotion (P03+).
...
| Step | Action | Time |
|---|---|---|
| 1 | Verify Redis cluster status: `kubectl -n shorts get pods -l app=shorts-redis` (replicas Ready) | ≤ 2 min |
...
| 3 | Replay from `shorts_videos` Postgres table for last 7 days; rank with current ranking heuristic / model; write to Redis | 30–60 min depending on tenant size |
...
- `microservices/shorts/failure-modes.md` FM-01, FM-03.
- `microservices/shorts/capacity-model.md` §"Redis".
- `microservices/shorts/multi-region.md`.
- `microservices/shorts/backfill-replay.md` BF-02.
