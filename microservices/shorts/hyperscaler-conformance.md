...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that shorts
  (video transcode + DRM + auto-caption + moderation) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/shorts.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml
---

# shorts — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`shorts`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-SHORTS`)
...
- **How shorts satisfies it**:
  - Capability calls in `microservices/shorts/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_shorts_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_shorts_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml` (`shorts-llm-capability-circuit-open`
    + `shorts-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/shorts/dashboards/` (per-capability panels).
  - Threat model: `microservices/shorts/threat-model.md` §"Dependency
...
- **How shorts satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_shorts_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml` (`shorts-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/shorts/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/shorts/capacity-model.md` (where
...
- **How shorts satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/shorts/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml` (`shorts-saturation-cpu-over-70pct`,
    `shorts-errors-5xx-rate-spike`, `shorts-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/shorts/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/shorts/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How shorts satisfies it**:
  - Every SLO in `microservices/shorts/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml` (`shorts-*-fast-burn-1h-14x` +
    `shorts-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/shorts/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/shorts/hyperscaler-conformance.md
test -f microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/shorts/iac/helm/shorts/templates/prometheusrule.yaml || true
