---
doc_class: Runbook
doc_id: RUNBOOK-DATA_WAREHOUSE-AUTO_REBALANCE
...
# Runbook: Data Warehouse Auto Rebalance

## Operator Contract
- Runbook id: data-warehouse-auto-rebalance.
- Primary service namespace: `data-warehouse`.
...
- Owning team: axis-data-warehouse + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-data-warehouse-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=data-warehouse; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n data-warehouse rollout status deploy/data-warehouse --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service data-warehouse --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service data-warehouse --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service data-warehouse --cell $CELL --window 30m`.
