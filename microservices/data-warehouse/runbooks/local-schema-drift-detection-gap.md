# Data Warehouse Runbook: Schema Drift Detection Gap

Service: data-warehouse  
Surface: local operational primitive suite  
Primary SLO: schema-drift-detection  
Domain focus: pipeline, sla-tier, freshness, completeness

## Trigger
- Alert `data-warehouse-schema-drift-detection` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_warehouse_schema_drift_detection_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when schema-drift-detection is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-warehouse-local-schema-drift-detection` and policy file name to the ticket.
