# Data Warehouse Runbook: Sla Tier Alert Lag

Service: data-warehouse  
Surface: local operational primitive suite  
Primary SLO: sla-tier-breach-alert  
Domain focus: pipeline, sla-tier, freshness, completeness

## Trigger
- Alert `data-warehouse-sla-tier-breach-alert` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_warehouse_sla_tier_breach_alert_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when sla-tier-breach-alert is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-warehouse-local-sla-tier-breach-alert` and policy file name to the ticket.
