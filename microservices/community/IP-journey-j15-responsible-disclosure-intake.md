...
ip_id: IP-journey-j15-responsible-disclosure-intake
journey_id: j15-bug-bounty-researcher-submission
microservice: community
role: responsible-disclosure-intake
status: draft
date: 2026-05-20
related_adrs:
  - ADR-0297
...
# IP - j15-bug-bounty-researcher-submission - community - responsible-disclosure-intake

## A. Intent

This implementation plan binds the `community` slice of `j15-bug-bounty-researcher-submission` to real community contracts, Cedar fragments, event channels, proto3 services, SLOs, and counterpart behavior. The previous generated row loop has been consolidated: only rows with a grounded community action remain, and speculative rows without a backing artifact are removed rather than expanded.
...
|---:|---|---|---|---|---|
| 1 | Accept a responsible-disclosure-intake evidence bundle only through the fully anonymous dropbox flow. | Trigger: encrypted submission; actor: Cedar `FullyAnonymousSubmitter::"public"` using `microservices/community/policy/anonymity-mode-fully-anonymous.cedar`. | Stores ciphertext hash and channel metadata without stable user id; rejects rate-limited resources. | AsyncAPI `community.flag.raised` in `microservices/community/contracts/asyncapi/community-events.yaml` plus capability `microservices/community/capabilities/whistleblower-submission.yaml` when whistleblower-specific. | Matches SecureDrop source submission: ciphertext routed, platform does not deanonymize. |
...
- Surface evidence: `microservices/community/IP-journey-j15-responsible-disclosure-intake.md` matched `openapi, asyncapi, .proto`; contract files `microservices/community/contracts/openapi/community.yaml, microservices/community/contracts/asyncapi/community-events.yaml, microservices/community/contracts/proto/community.proto`; type anchor `microservices/community/manifest.json`.
...
- Surface evidence: `microservices/community/IP-journey-j15-responsible-disclosure-intake.md` matched `SLO, payment`; anchors `microservices/community/manifest.json`; type anchor `microservices/community/manifest.json`.

## Sustainability emission (per ADR-0344)
- Per-call audit row emission: populate `cost_usd_minor_units`, `co2_grams`, and `watt_hours` with provider and region on every audit-chain row.
- Carbon-aware scheduling eligibility: opt-in only; do not defer Tier 0/1 workloads or realtime-mandated compliance-pack workloads (`eu-ai-act-annex-iii`, `hipaa-em-incident-response`, `pci-dss-realtime-fraud-detection`).
- finops-portal rollup axes affected: tenant / product / capability / provider / cell / compliance_pack.
- Surface evidence: `microservices/community/IP-journey-j15-responsible-disclosure-intake.md` matched `attribution`; anchors `microservices/community/manifest.json`; type anchor `microservices/community/manifest.json`.
