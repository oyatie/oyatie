...
parallel_work_compatibility: Independent of j130 community-whistleblower surface (different topic in community)
wave_15_journey_ip_substance: rewritten-2026-05-21
---

# IP - j129-court-warrant-pierces-personal-tenant-with-judicial-oversight - community - transparency-report

## A. Intent

This implementation plan binds the `community` slice of `j129-court-warrant-pierces-personal-tenant-with-judicial-oversight` to real community contracts, Cedar fragments, event channels, proto3 services, SLOs, and counterpart behavior. The previous generated row loop has been consolidated: only rows with a grounded community action remain, and speculative rows without a backing artifact are removed rather than expanded.
...
|---:|---|---|---|---|---|
| 1 | List or create the scoped community space for `transparency-report`. | Trigger: journey entry or tenant navigation; actor: `TenantMember` or `AnonymousPrincipal` for public spaces. | Reads `SpaceList` or creates postable space state under tenant_id. | OpenAPI `listSpaces` in `microservices/community/contracts/openapi/community.yaml` and `microservices/community/policy/tenant-scope.cedar`. | Matches Discourse category access scoped by group/tenant. |
...
- Surface evidence: `microservices/community/IP-journey-j129-transparency-report.md` matched `openapi, asyncapi, .proto`; contract files `microservices/community/contracts/openapi/community.yaml, microservices/community/contracts/asyncapi/community-events.yaml, microservices/community/contracts/proto/community.proto`; type anchor `microservices/community/manifest.json`.
...
- Surface evidence: `microservices/community/IP-journey-j129-transparency-report.md` matched `SLO, payment`; anchors `microservices/community/manifest.json`; type anchor `microservices/community/manifest.json`.
