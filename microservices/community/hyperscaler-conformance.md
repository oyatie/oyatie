...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that community
  (post create + thread fanout + voting + moderation) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/community.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/community/iac/helm/community/templates/prometheusrule.yaml
---

# community — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`community`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-COMMUNITY`)
...
- **How community satisfies it**:
  - Capability calls in `microservices/community/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_community_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_community_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/community/iac/helm/community/templates/prometheusrule.yaml` (`community-llm-capability-circuit-open`
    + `community-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/community/dashboards/` (per-capability panels).
  - Threat model: `microservices/community/threat-model.md` §"Dependency
...
- **How community satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_community_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/community/iac/helm/community/templates/prometheusrule.yaml` (`community-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/community/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/community/capacity-model.md` (where
...
- **How community satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/community/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/community/iac/helm/community/templates/prometheusrule.yaml` (`community-saturation-cpu-over-70pct`,
    `community-errors-5xx-rate-spike`, `community-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/community/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/community/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How community satisfies it**:
  - Every SLO in `microservices/community/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/community/iac/helm/community/templates/prometheusrule.yaml` (`community-*-fast-burn-1h-14x` +
    `community-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/community/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/community/hyperscaler-conformance.md
test -f microservices/community/iac/helm/community/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/community/iac/helm/community/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/community/iac/helm/community/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/community/iac/helm/community/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/community/iac/helm/community/templates/prometheusrule.yaml || true
