---
doc_class: Runbook
doc_id: RUNBOOK-COMMUNITY-COLD_MERGE
...
# Runbook: Community Cold Merge

## Operator Contract
- Runbook id: community-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-community-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=community; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n community rollout status deploy/community --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service community --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service community --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service community --cell $CELL --window 30m`.
