...
companion_docs:
  ...
  - microservices/community/README.md
...
`community` keeps its existing bounded context and flat `microservices/community/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `community`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `community` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_request` drives CPU `0.12`, RAM `256`, storage `12`, and connection pool variables.
