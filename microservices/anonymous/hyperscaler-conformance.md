...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that anonymous
  (pseudonymous identity + affinity attestation + hard delete) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/anonymous.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/anonymous/iac/helm/templates/prometheusrule.yaml
---

# anonymous — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`anonymous`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-ANONYMOUS`)
...
- **How anonymous satisfies it**:
  - Capability calls in `microservices/anonymous/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_anonymous_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_anonymous_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/anonymous/iac/helm/templates/prometheusrule.yaml` (`anonymous-llm-capability-circuit-open`
    + `anonymous-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/anonymous/dashboards/` (per-capability panels).
  - Threat model: `microservices/anonymous/threat-model.md` §"Dependency
...
- **How anonymous satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_anonymous_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/anonymous/iac/helm/templates/prometheusrule.yaml` (`anonymous-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/anonymous/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/anonymous/capacity-model.md` (where
...
- **How anonymous satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/anonymous/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/anonymous/iac/helm/templates/prometheusrule.yaml` (`anonymous-saturation-cpu-over-70pct`,
    `anonymous-errors-5xx-rate-spike`, `anonymous-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/anonymous/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/anonymous/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How anonymous satisfies it**:
  - Every SLO in `microservices/anonymous/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/anonymous/iac/helm/templates/prometheusrule.yaml` (`anonymous-*-fast-burn-1h-14x` +
    `anonymous-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/anonymous/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/anonymous/hyperscaler-conformance.md
test -f microservices/anonymous/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/anonymous/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/anonymous/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/anonymous/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/anonymous/iac/helm/templates/prometheusrule.yaml || true
