...
## §principals
This anchor is closed for `governance` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.governance.runtime` owns normal `audit-refresh` execution and never borrows a tenant principal.
- Platform principal `oyatie.governance.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/governance/backfill-replay.md` when present.
- Platform principal `oyatie.governance.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.governance.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.governance.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `audit-refresh` is evaluated as `<tenant>.governance.audit-refresh` before any `governance` state mutation.
...
## §cedar-gates
This anchor is closed for `governance` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/governance/policy/auditor-scope.cedar`, `microservices/governance/policy/cedar-canonical-imports.cedar`, `microservices/governance/policy/ci-scope.cedar`, `microservices/governance/policy/data-residency.md`, `microservices/governance/policy/lane-execution.md`, `microservices/governance/policy/public-read.cedar`; +1 more.
- Action namespace uses `governance::<bounded_context>::<verb>`; the first protected action is `governance::audit-refresh::execute`.
...
- Concrete fragments in scope: `microservices/governance/policy/auditor-scope.cedar`, `microservices/governance/policy/cedar-canonical-imports.cedar`, `microservices/governance/policy/ci-scope.cedar`, `microservices/governance/policy/data-residency.md`, `microservices/governance/policy/lane-execution.md`, `microservices/governance/policy/public-read.cedar`; +1 more.
- Example: `governance::audit-refresh::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `governance` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `governance.aggregation_indexer` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `governance.bundled_check_crate` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `governance.evidence_emitter` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `governance.lane_runtime` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `governance.policy_engine` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
- Example: `audit-refresh` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `governance` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `governance` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `governance` only through contracts `microservices/governance/contracts/asyncapi/governance-events.yaml`, `microservices/governance/contracts/openapi/governance.yaml`, `microservices/governance/contracts/proto/governance.proto`.
- If product: `governance` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `aggregation-indexer`, `bundled-check-crate`, `evidence-emitter`, `lane-runtime`, `policy-engine`.
- Example: `audit-refresh` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `governance` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/governance/policy/auditor-scope.cedar`, `microservices/governance/policy/cedar-canonical-imports.cedar`, `microservices/governance/policy/ci-scope.cedar`, `microservices/governance/policy/data-residency.md`, `microservices/governance/policy/lane-execution.md`, `microservices/governance/policy/public-read.cedar`; +1 more with the full action/resource/context tuple.
...
- Example: `governance::audit-refresh::execute` evaluates locally, then emits an audit event before the usecase writes `governance.aggregation_indexer`.
...
## §cell-eligibility
This anchor is closed for `governance` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `audit-refresh` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `governance` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `governance` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `governance.audit-refresh` and is included in every AI/risk/model call audit event.
...
- Example: `audit-refresh` can request classification or summarization with `audience_tag=governance.audit-refresh`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `governance` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `governance` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `aggregation-indexer` is read/written as an ontology node or edge projection when `governance` needs cross-surface discovery.
- Bounded-context entity `bundled-check-crate` is read/written as an ontology node or edge projection when `governance` needs cross-surface discovery.
- Bounded-context entity `evidence-emitter` is read/written as an ontology node or edge projection when `governance` needs cross-surface discovery.
- Bounded-context entity `lane-runtime` is read/written as an ontology node or edge projection when `governance` needs cross-surface discovery.
...
- Example: `audit-refresh` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `governance` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/governance/contracts/asyncapi/governance-events.yaml`, `microservices/governance/contracts/openapi/governance.yaml`, `microservices/governance/contracts/proto/governance.proto`.
...
- IaC transport evidence: `microservices/governance/iac/build/Dockerfile.distroless-rust`, `microservices/governance/iac/helm/_oya-helpers/Chart.yaml`, `microservices/governance/iac/helm/_oya-helpers/templates/_helpers.tpl`, `microservices/governance/iac/helm/apicurio-registry/Chart.yaml`, `microservices/governance/iac/helm/evidence-store/Chart.yaml`, `microservices/governance/iac/helm/evidence-store/values.yaml`; +6 more.
- Example: `governance` `audit-refresh` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `governance` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `aggregation-indexer`, `bundled-check-crate`, `evidence-emitter`, `lane-runtime`, `policy-engine` and deploy as separate app/worker/API pods where files exist.
...
## §observability
This anchor is closed for `governance` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/governance/slos/aspirational-enforcement-correctness.openslo.yaml`, `microservices/governance/slos/check-crate-availability.openslo.yaml`, `microservices/governance/slos/conformance-evidence-freshness.openslo.yaml`, `microservices/governance/slos/envoy-wasm-filter-latency-p99.openslo.yaml`, `microservices/governance/slos/gate-validate-latency.openslo.yaml`, `microservices/governance/slos/per-lane-runtime-budget.openslo.yaml`; +4 more.
- Audit classes use prefix `oya.governance.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `governance_audit_refresh_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `governance_audit_refresh_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/governance/slos/aspirational-enforcement-correctness.openslo.yaml`.
...
- Example trace: `governance.audit-refresh` -> `policy.evaluate` -> `storage.audit_refresh` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `governance` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `governance` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `audit-refresh` action class.
...
- Cedar/IaC controls in scope: `microservices/governance/policy/auditor-scope.cedar`, `microservices/governance/policy/cedar-canonical-imports.cedar`, `microservices/governance/policy/ci-scope.cedar`, `microservices/governance/policy/data-residency.md`, `microservices/governance/policy/lane-execution.md`, `microservices/governance/policy/public-read.cedar`; +13 more.
- Canary payloads for `governance` use fake `audit-refresh` identifiers and honey endpoints under `/.well-known/oya-canary/governance`.
...
This anchor is closed for `governance` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `governance` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `governance` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `governance` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `audit-refresh` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `governance` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `governance` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `governance` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/governance/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/governance/runbooks/aggregation-rebuild.md`, `microservices/governance/runbooks/envoy-wasm-filter-rollback.md`, `microservices/governance/runbooks/evidence-replay.md`, `microservices/governance/runbooks/industry-baseline-refresh.md`, `microservices/governance/runbooks/lane-bypass-emergency.md`, `microservices/governance/runbooks/lane-failure-triage.md`; +2 more.
- Example: `audit-refresh` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
