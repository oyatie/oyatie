...
| Migration | `microservices/messenger/migration-playbooks/from-slack.md` + `migration-from-connect.md` | D-4.8 outbound refs | Migration plays; Slack export ingestion + retired µservice unwind |
...
| Deprecation | `microservices/messenger/deprecation-notice.md` (6 KB) | D-4.8 outbound refs | µservice deprecation notice (references retired ADR-MSGR-0001 line predecessor) |
...
The audit records the §3.2 verdict as REVISE per §D-4.23 because three stale dependency edges (`connector`, `network`, `cell`) and the live ADR-0316 citation block phase-3 promotion until Wave 15J/15K/15L remediation removes them.
...
- Slack cross-org DM: COVERED via Matrix federation bridge per ADR-MSGR-0004.
...
| F-MSGR-004 | P1 | §3.2 outbound refs | outbound-cross-reference | manifest.json `depends_on_microservices[]` includes `connector`, `network`, `cell` | Three stale dependency edges: connect is absorbed (PRD §1), network retires (memory 2026-05-21), cell retires (memory 2026-05-21). | Remove `connector` (Wave 15I retirement), remove `network` (Wave 15K merge into community), remove `cell` (Wave 15L retirement); add `mail`, `social`, `community` per F-MSGR-003. | Wave 15K/15L |
...
4. **µservice absorption documentation.** PRD §1 says connect is absorbed into messenger via the bominal_source field referencing ADR-0208 connect-dual-context-unified-channel-hub + ADR-0215 retention-legal-hold-dual-context. But the manifest.json still names `connector` as a `depends_on_microservices` edge. Wave 15I retirement of `microservices/connector/` per ADR-0138 six-path-deprecation pattern needs to coordinate with messenger's removal of the dependency edge. Orchestrator: route this to Wave 15I (Foundry + connect retirement combined).
...
The Wave 4 directive narrows the counterpart anchor to Slack + Microsoft Teams (chat side) + Discord. The audit excludes Microsoft Teams Meetings from the parity matrix because meetings belong to the meet µservice per ADR-MSGR-0001 §scope-2. The audit excludes Discord Activities mini-games from the parity matrix because the games are out-of-scope intentional per the feature-parity-matrix.md §13 doctrine reason. The audit does NOT exclude Slack cross-org DMs from the parity matrix because Slack is a messenger feature (not a meet feature); it is covered via Matrix federation bridge per ADR-MSGR-0004.
