...
## §principals
This anchor is closed for `messenger` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.messenger.runtime` owns normal `smart-reply-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.messenger.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/messenger/backfill-replay.md` when present.
- Platform principal `oyatie.messenger.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.messenger.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.messenger.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `smart-reply-suggest` is evaluated as `<tenant>.messenger.smart-reply-suggest` before any `messenger` state mutation.
...
## §cedar-gates
This anchor is closed for `messenger` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/messenger/policy/attachment-malware-quarantine.md`, `microservices/messenger/policy/auditor-scope.cedar`, `microservices/messenger/policy/channel-scope.cedar`, `microservices/messenger/policy/ci-scope.cedar`, `microservices/messenger/policy/data-residency.md`, `microservices/messenger/policy/dual-context-isolation.md`; +4 more.
- Action namespace uses `messenger::<bounded_context>::<verb>`; the first protected action is `messenger::smart-reply-suggest::execute`.
...
- Concrete fragments in scope: `microservices/messenger/policy/attachment-malware-quarantine.md`, `microservices/messenger/policy/auditor-scope.cedar`, `microservices/messenger/policy/channel-scope.cedar`, `microservices/messenger/policy/ci-scope.cedar`, `microservices/messenger/policy/data-residency.md`, `microservices/messenger/policy/dual-context-isolation.md`; +4 more.
- Example: `messenger::smart-reply-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `messenger` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `messenger.messenger` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- Planned table/event surface `messenger.smart_reply_suggest_2` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `messenger.smart_reply_suggest_3` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `messenger.smart_reply_suggest_4` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `messenger.smart_reply_suggest_5` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
...
- Example: `smart-reply-suggest` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `messenger` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `messenger` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `messenger` only through contracts `microservices/messenger/contracts/asyncapi/messenger-events.yaml`, `microservices/messenger/contracts/openapi/messenger.yaml`, `microservices/messenger/contracts/proto/messenger.proto`.
- If product: `messenger` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `messenger`.
- Example: `smart-reply-suggest` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `messenger` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/messenger/policy/attachment-malware-quarantine.md`, `microservices/messenger/policy/auditor-scope.cedar`, `microservices/messenger/policy/channel-scope.cedar`, `microservices/messenger/policy/ci-scope.cedar`, `microservices/messenger/policy/data-residency.md`, `microservices/messenger/policy/dual-context-isolation.md`; +4 more with the full action/resource/context tuple.
...
- Example: `messenger::smart-reply-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `messenger.messenger`.
...
## §cell-eligibility
This anchor is closed for `messenger` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `smart-reply-suggest` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `messenger` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `messenger` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `messenger.smart-reply-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `smart-reply-suggest` can request classification or summarization with `audience_tag=messenger.smart-reply-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `messenger` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `messenger` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `messenger` is read/written as an ontology node or edge projection when `messenger` needs cross-surface discovery.
...
- Example: `smart-reply-suggest` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `messenger` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/messenger/contracts/asyncapi/messenger-events.yaml`, `microservices/messenger/contracts/openapi/messenger.yaml`, `microservices/messenger/contracts/proto/messenger.proto`.
...
- IaC transport evidence: `microservices/messenger/iac/helm/messenger/Chart.yaml`, `microservices/messenger/iac/helm/messenger/templates/deployment.yaml`, `microservices/messenger/iac/helm/messenger/templates/hpa.yaml`, `microservices/messenger/iac/helm/messenger/templates/networkpolicy.yaml`, `microservices/messenger/iac/helm/messenger/templates/pdb.yaml`, `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml`; +6 more.
- Example: `messenger` `smart-reply-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `messenger` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `messenger` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/messenger/iac/helm/messenger/Chart.yaml`, `microservices/messenger/iac/helm/messenger/templates/deployment.yaml`, `microservices/messenger/iac/helm/messenger/templates/hpa.yaml`, `microservices/messenger/iac/helm/messenger/templates/networkpolicy.yaml`, `microservices/messenger/iac/helm/messenger/templates/pdb.yaml`, `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/messenger/catalog/oya-messenger-app.yaml`, `microservices/messenger/catalog/oya-messenger-channel-store-adapter-postgres.yaml`, `microservices/messenger/catalog/oya-messenger-channel-store-domain.yaml`, `microservices/messenger/catalog/oya-messenger-channel-store-kernel.yaml`, `microservices/messenger/catalog/oya-messenger-channel-store-rest.yaml`, `microservices/messenger/catalog/oya-messenger-channel-store-usecase.yaml`; +18 more.
- Example: `smart-reply-suggest` API runs as a Kata-isolated container; scheduled/background work for `messenger` uses a separate worker principal.
...
## §observability
This anchor is closed for `messenger` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/messenger/slos/attachment-scan-freshness.openslo.yaml`, `microservices/messenger/slos/mention-fanout.openslo.yaml`, `microservices/messenger/slos/message-send-availability.openslo.yaml`, `microservices/messenger/slos/message-send-latency.openslo.yaml`, `microservices/messenger/slos/presence-propagation.openslo.yaml`, `microservices/messenger/slos/read-receipt-fanout.openslo.yaml`; +7 more.
- Audit classes use prefix `oya.messenger.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `messenger_smart_reply_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `messenger_smart_reply_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/messenger/slos/attachment-scan-freshness.openslo.yaml`.
...
- Example trace: `messenger.smart-reply-suggest` -> `policy.evaluate` -> `storage.smart_reply_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `messenger` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `messenger` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `smart-reply-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/messenger/policy/attachment-malware-quarantine.md`, `microservices/messenger/policy/auditor-scope.cedar`, `microservices/messenger/policy/channel-scope.cedar`, `microservices/messenger/policy/ci-scope.cedar`, `microservices/messenger/policy/data-residency.md`, `microservices/messenger/policy/dual-context-isolation.md`; +16 more.
- Canary payloads for `messenger` use fake `smart-reply-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/messenger`.
...
This anchor is closed for `messenger` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `messenger` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `messenger` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `messenger` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `smart-reply-suggest` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `messenger` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `messenger` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `messenger` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/messenger/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/messenger/runbooks/attachment-restore.md`, `microservices/messenger/runbooks/channel-acl-drift.md`, `microservices/messenger/runbooks/e2e-encryption-key-rotation.md`, `microservices/messenger/runbooks/ediscovery-export.md`, `microservices/messenger/runbooks/huddle-sfu-degraded.md`, `microservices/messenger/runbooks/mention-storm-throttle.md`; +4 more.
- Example: `smart-reply-suggest` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
