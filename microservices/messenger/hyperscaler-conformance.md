...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that messenger
  (message send + websocket fanout + MLS E2E) satisfies four hyperscaler-grade invariants drawn from
...
prometheusrule_path: microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml
---

# messenger — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`messenger`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-MESSENGER`)
...
- **How messenger satisfies it**:
  - Capability calls in `microservices/messenger/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_messenger_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_messenger_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml` (`messenger-llm-capability-circuit-open`
    + `messenger-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/messenger/dashboards/` (per-capability panels).
  - Threat model: `microservices/messenger/threat-model.md` §"Dependency
...
- **How messenger satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_messenger_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml` (`messenger-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/messenger/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/messenger/capacity-model.md` (where
...
- **How messenger satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/messenger/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml` (`messenger-saturation-cpu-over-70pct`,
    `messenger-errors-5xx-rate-spike`, `messenger-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/messenger/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/messenger/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How messenger satisfies it**:
  - Every SLO in `microservices/messenger/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml` (`messenger-*-fast-burn-1h-14x` +
    `messenger-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/messenger/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/messenger/hyperscaler-conformance.md
test -f microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/messenger/iac/helm/messenger/templates/prometheusrule.yaml || true
