use clap::Parser;
use oya_contact_center_voice_routing_app::adapter::AdapterRegistry;
use oya_contact_center_voice_routing_app::config::ServiceConfig;
use oya_contact_center_voice_routing_app::error::Result;
use std::path::PathBuf;
use std::process::ExitCode;

#[derive(Debug, Parser)]
#[command(name = "oya-contact-center-voice-routing")]
#[command(about = "Contact Center µservice scaffold entrypoint")]
struct CliArgs {
    #[arg(long)]
...
fn run(args: CliArgs) -> Result<()> {
    ...
    tracing::info!(
        ...
        "contact center scaffold validated"
    );
    Ok(())
}
