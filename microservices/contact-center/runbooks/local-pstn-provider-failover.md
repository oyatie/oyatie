...
Primary SLO: transfer-success  
Domain focus: omnichannel, routing, call, transfer

## Trigger
- Alert `contact-center-transfer-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_contact_center_transfer_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when transfer-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `contact-center-local-transfer-success` and policy file name to the ticket.
