---
doc_class: Runbook
doc_id: RUNBOOK-REAL_ESTATE-HOT_SPLIT
...
# Runbook: Real Estate Hot Split

## Operator Contract
- Runbook id: real-estate-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-real-estate-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=real-estate; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n real-estate rollout status deploy/real-estate --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service real-estate --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service real-estate --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service real-estate --cell $CELL --window 30m`.
