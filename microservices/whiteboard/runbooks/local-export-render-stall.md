# Whiteboard Runbook: Export Render Stall

Service: whiteboard  
Surface: local operational primitive suite  
Primary SLO: export-render-latency  
Domain focus: whiteboard, cursor, stroke, crdt

## Trigger
- Alert `whiteboard-export-render-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_whiteboard_export_render_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when export-render-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `whiteboard-local-export-render-latency` and policy file name to the ticket.
