# whiteboard DR failover runbook

## Purpose

Recover the whiteboard service inside the declared ADR-0343 DR envelope without changing tenant version pins or weakening Cedar admission.

## Recovery order

1. Freeze new mutating ingress for whiteboard at api-gateway while read-only routes stay available when data freshness permits.
...
3. admit boards read-only, replay canvas operation logs, rebuild history snapshots, and reopen collaboration admission by cell.
