# Whiteboard Runbook: Crdt Merge Conflict

Service: whiteboard  
Surface: local operational primitive suite  
Primary SLO: crdt-merge-success  
Domain focus: whiteboard, cursor, stroke, crdt

## Trigger
- Alert `whiteboard-crdt-merge-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_whiteboard_crdt_merge_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when crdt-merge-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `whiteboard-local-crdt-merge-success` and policy file name to the ticket.
