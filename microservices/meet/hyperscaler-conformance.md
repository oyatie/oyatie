...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that meet
  (room create + screen share + transcription + E2E) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/meet.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/meet/iac/helm/meet/templates/prometheusrule.yaml
---

# meet — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`meet`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-MEET`)
...
- **How meet satisfies it**:
  - Capability calls in `microservices/meet/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_meet_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_meet_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml` (`meet-llm-capability-circuit-open`
    + `meet-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/meet/dashboards/` (per-capability panels).
  - Threat model: `microservices/meet/threat-model.md` §"Dependency
...
- **How meet satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_meet_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml` (`meet-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/meet/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/meet/capacity-model.md` (where
...
- **How meet satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/meet/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml` (`meet-saturation-cpu-over-70pct`,
    `meet-errors-5xx-rate-spike`, `meet-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/meet/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/meet/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How meet satisfies it**:
  - Every SLO in `microservices/meet/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml` (`meet-*-fast-burn-1h-14x` +
    `meet-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/meet/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/meet/hyperscaler-conformance.md
test -f microservices/meet/iac/helm/meet/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/meet/iac/helm/meet/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/meet/iac/helm/meet/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/meet/iac/helm/meet/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/meet/iac/helm/meet/templates/prometheusrule.yaml || true
