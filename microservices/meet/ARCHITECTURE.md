...
## §principals
This anchor is closed for `meet` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.meet.runtime` owns normal `meeting-topic-and-participant-hint` execution and never borrows a tenant principal.
- Platform principal `oyatie.meet.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/meet/backfill-replay.md` when present.
- Platform principal `oyatie.meet.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.meet.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.meet.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `meeting-topic-and-participant-hint` is evaluated as `<tenant>.meet.meeting-topic-and-participant-hint` before any `meet` state mutation.
...
## §cedar-gates
This anchor is closed for `meet` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/meet/policy/auditor-scope.cedar`, `microservices/meet/policy/ci-scope.cedar`, `microservices/meet/policy/data-residency.md`, `microservices/meet/policy/meeting-scope.cedar`, `microservices/meet/policy/public-read.cedar`, `microservices/meet/policy/recording-consent.md`; +2 more.
- Action namespace uses `meet::<bounded_context>::<verb>`; the first protected action is `meet::meeting-topic-and-participant-hint::execute`.
...
- Concrete fragments in scope: `microservices/meet/policy/auditor-scope.cedar`, `microservices/meet/policy/ci-scope.cedar`, `microservices/meet/policy/data-residency.md`, `microservices/meet/policy/meeting-scope.cedar`, `microservices/meet/policy/public-read.cedar`, `microservices/meet/policy/recording-consent.md`; +2 more.
- Example: `meet::meeting-topic-and-participant-hint::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `meet` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `meet.meet` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- Planned table/event surface `meet.meeting_topic_and_participant_hint_2` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `meet.meeting_topic_and_participant_hint_3` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `meet.meeting_topic_and_participant_hint_4` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `meet.meeting_topic_and_participant_hint_5` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
...
- Example: `meeting-topic-and-participant-hint` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `meet` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `meet` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `meet` only through contracts `microservices/meet/contracts/asyncapi/meet-events.yaml`, `microservices/meet/contracts/openapi/meet.yaml`, `microservices/meet/contracts/proto/meet.proto`.
- If product: `meet` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `meet`.
- Example: `meeting-topic-and-participant-hint` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `meet` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/meet/policy/auditor-scope.cedar`, `microservices/meet/policy/ci-scope.cedar`, `microservices/meet/policy/data-residency.md`, `microservices/meet/policy/meeting-scope.cedar`, `microservices/meet/policy/public-read.cedar`, `microservices/meet/policy/recording-consent.md`; +2 more with the full action/resource/context tuple.
...
- Example: `meet::meeting-topic-and-participant-hint::execute` evaluates locally, then emits an audit event before the usecase writes `meet.meet`.
...
## §cell-eligibility
This anchor is closed for `meet` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `meeting-topic-and-participant-hint` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
This anchor is closed for `meet` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `meet` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `meet.meeting-topic-and-participant-hint` and is included in every AI/risk/model call audit event.
...
- Example: `meeting-topic-and-participant-hint` can request classification or summarization with `audience_tag=meet.meeting-topic-and-participant-hint`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `meet` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `meet` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `meet` is read/written as an ontology node or edge projection when `meet` needs cross-surface discovery.
...
- Example: `meeting-topic-and-participant-hint` resolves `tenant.compliance_packs`, `principal.audience_type`, and `resource.home_cell` before Cedar evaluation.
...
## §transport
This anchor is closed for `meet` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/meet/contracts/asyncapi/meet-events.yaml`, `microservices/meet/contracts/openapi/meet.yaml`, `microservices/meet/contracts/proto/meet.proto`.
...
- IaC transport evidence: `microservices/meet/iac/helm/meet/Chart.yaml`, `microservices/meet/iac/helm/meet/templates/deployment.yaml`, `microservices/meet/iac/helm/meet/templates/hpa.yaml`, `microservices/meet/iac/helm/meet/templates/networkpolicy.yaml`, `microservices/meet/iac/helm/meet/templates/pdb.yaml`, `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml`; +6 more.
- Example: `meet` `meeting-topic-and-participant-hint` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `meet` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `meet` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/meet/iac/helm/meet/Chart.yaml`, `microservices/meet/iac/helm/meet/templates/deployment.yaml`, `microservices/meet/iac/helm/meet/templates/hpa.yaml`, `microservices/meet/iac/helm/meet/templates/networkpolicy.yaml`, `microservices/meet/iac/helm/meet/templates/pdb.yaml`, `microservices/meet/iac/helm/meet/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/meet/catalog/oya-meet-audio-kernel.yaml`, `microservices/meet/catalog/oya-meet-e2e-encryption-kernel.yaml`, `microservices/meet/catalog/oya-meet-live-stream-egress-adapter-srs-rtmp.yaml`, `microservices/meet/catalog/oya-meet-meeting-instance-adapter-coturn.yaml`, `microservices/meet/catalog/oya-meet-meeting-instance-adapter-livekit.yaml`, `microservices/meet/catalog/oya-meet-meeting-room-adapter-postgres.yaml`; +18 more.
- Example: `meeting-topic-and-participant-hint` API runs as a Kata-isolated container; scheduled/background work for `meet` uses a separate worker principal.
...
## §observability
This anchor is closed for `meet` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/meet/slos/availability.openslo.yaml`, `microservices/meet/slos/e2e-mls-handshake-latency.openslo.yaml`, `microservices/meet/slos/live-caption-latency.openslo.yaml`, `microservices/meet/slos/media-glass-to-glass-latency.openslo.yaml`, `microservices/meet/slos/meeting-summary-post-end-latency.openslo.yaml`, `microservices/meet/slos/participant-join-latency.openslo.yaml`; +8 more.
- Audit classes use prefix `oya.meet.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `meet_meeting_topic_and_participant_hint_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `meet_meeting_topic_and_participant_hint_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/meet/slos/availability.openslo.yaml`.
...
- Example trace: `meet.meeting-topic-and-participant-hint` -> `policy.evaluate` -> `storage.meeting_topic_and_participant_hint` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `meet` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `meet` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `meeting-topic-and-participant-hint` action class.
...
- Cedar/IaC controls in scope: `microservices/meet/policy/auditor-scope.cedar`, `microservices/meet/policy/ci-scope.cedar`, `microservices/meet/policy/data-residency.md`, `microservices/meet/policy/meeting-scope.cedar`, `microservices/meet/policy/public-read.cedar`, `microservices/meet/policy/recording-consent.md`; +14 more.
- Canary payloads for `meet` use fake `meeting-topic-and-participant-hint` identifiers and honey endpoints under `/.well-known/oya-canary/meet`.
...
This anchor is closed for `meet` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `meet` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `meet` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `meet` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `meeting-topic-and-participant-hint` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `meet` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `meet` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `meet` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/meet/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/meet/runbooks/coturn-key-rotation.md`, `microservices/meet/runbooks/live-caption-stalled.md`, `microservices/meet/runbooks/lobby-bypass-incident.md`, `microservices/meet/runbooks/recording-storage-degraded.md`, `microservices/meet/runbooks/sfu-degraded.md`, `microservices/meet/runbooks/transcription-classifier-rollback.md`; +1 more.
- Example: `meeting-topic-and-participant-hint` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
