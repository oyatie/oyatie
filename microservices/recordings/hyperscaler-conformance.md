...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that recordings
  (recording pipeline + transcript search + redaction + legal hold) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/recordings.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml
---

# recordings — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`recordings`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-RECORDINGS`)
...
- **How recordings satisfies it**:
  - Capability calls in `microservices/recordings/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_recordings_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_recordings_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml` (`recordings-llm-capability-circuit-open`
    + `recordings-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/recordings/dashboards/` (per-capability panels).
  - Threat model: `microservices/recordings/threat-model.md` §"Dependency
...
- **How recordings satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_recordings_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml` (`recordings-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/recordings/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/recordings/capacity-model.md` (where
...
- **How recordings satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/recordings/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml` (`recordings-saturation-cpu-over-70pct`,
    `recordings-errors-5xx-rate-spike`, `recordings-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/recordings/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/recordings/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How recordings satisfies it**:
  - Every SLO in `microservices/recordings/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml` (`recordings-*-fast-burn-1h-14x` +
    `recordings-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/recordings/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/recordings/hyperscaler-conformance.md
test -f microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/recordings/iac/helm/recordings/templates/prometheusrule.yaml || true
