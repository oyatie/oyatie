...
exit_gate: |
  All 15 IPs merged; oya-governance-promotion-readiness CI lane green for release/application/*; HG-APP gate in /specs/hyperscaler-gates.json registers green; cargo nextest run --workspace exits 0; cargo run -p oya-dev-cli -- gate validate per-microservice-layout --microservice application exits 0; cargo run -p oya-dev-cli -- gate validate authority-cohesion --microservice application exits 0; pack-kr Application Shell deployed to staging with TTI p99 ≤2s under 10k synthetic concurrent users; CDN global purge drill completes ≤60s; module-loader rejects tampered bundle (SRI + signature drills).
depends_on:
...
cargo run -p oya-dev-cli -- gate validate lean-a2 --microservice application
cargo run -p oya-dev-cli -- gate validate port-location --microservice application
cargo run -p oya-dev-cli -- gate validate layer-correctness --microservice application
cargo run -p oya-dev-cli -- gate validate per-microservice-layout --microservice application
cargo run -p oya-dev-cli -- gate validate statelessness --microservice application
cargo run -p oya-dev-cli -- gate validate shardability --microservice application
cargo run -p oya-dev-cli -- gate validate data-class --microservice application
cargo run -p oya-dev-cli -- gate validate cedar-policy --microservice application
cargo run -p oya-dev-cli -- gate validate audit-chain --microservice application
