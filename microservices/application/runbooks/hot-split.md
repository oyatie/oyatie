---
doc_class: Runbook
doc_id: RUNBOOK-APPLICATION-HOT_SPLIT
...
# Runbook: Application Hot Split

## Operator Contract
- Runbook id: application-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-application-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=application; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n application rollout status deploy/application --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service application --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service application --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service application --cell $CELL --window 30m`.
