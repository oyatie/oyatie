...
companion_docs:
  ...
  - microservices/drive/README.md
  - microservices/drive/ARCHITECTURE.md
---
...
`drive` keeps its existing bounded context and flat `microservices/drive/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `drive`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `drive` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_request` drives CPU `0.4`, RAM `1024`, storage `51200`, and connection pool variables.
