...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that drive
  (object storage + DLP + share link + WORM immutability) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/drive.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/drive/iac/helm/templates/prometheusrule.yaml
---

# drive — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`drive`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-DRIVE`)
...
- **How drive satisfies it**:
  - Capability calls in `microservices/drive/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_drive_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_drive_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/drive/iac/helm/templates/prometheusrule.yaml` (`drive-llm-capability-circuit-open`
    + `drive-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/drive/dashboards/` (per-capability panels).
  - Threat model: `microservices/drive/threat-model.md` §"Dependency
...
- **How drive satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_drive_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/drive/iac/helm/templates/prometheusrule.yaml` (`drive-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/drive/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/drive/capacity-model.md` (where
...
- **How drive satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/drive/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/drive/iac/helm/templates/prometheusrule.yaml` (`drive-saturation-cpu-over-70pct`,
    `drive-errors-5xx-rate-spike`, `drive-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/drive/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/drive/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How drive satisfies it**:
  - Every SLO in `microservices/drive/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/drive/iac/helm/templates/prometheusrule.yaml` (`drive-*-fast-burn-1h-14x` +
    `drive-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/drive/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/drive/hyperscaler-conformance.md
test -f microservices/drive/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/drive/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/drive/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/drive/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/drive/iac/helm/templates/prometheusrule.yaml || true
