...
ip_id: IP-journey-j80-document-storage-boundary
...
# IP-journey-j80-document-storage-boundary - drive

## Goal

Implement the `document-storage-boundary` slice for `drive` so j80 can satisfy `KR-PIPA + KR-CSAP` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/drive/contracts/openapi/j80-document-storage-boundary-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/drive/contracts/asyncapi/j80-document-storage-boundary-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/drive/contracts/proto/j80-document-storage-boundary-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/drive/policy/j80-document-storage-boundary.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/drive/runbooks/j80-document-storage-boundary-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/drive/tests/j80_document_storage_boundary_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J80DriveDocumentStorageBoundaryStarted`.
- `J80DriveDocumentStorageBoundaryPermitted`.
- `J80DriveDocumentStorageBoundaryDenied`.
- `J80DriveDocumentStorageBoundaryCommitted`.
- `J80DriveDocumentStorageBoundaryRolledBack`.

## Implementation rows

### IP row 001 - drive document-storage-boundary
Scope: implement one drive behavior for j80, not a cross-service refactor.
