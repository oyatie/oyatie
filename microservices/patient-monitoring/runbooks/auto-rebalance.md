---
doc_class: Runbook
doc_id: RUNBOOK-PATIENT_MONITORING-AUTO_REBALANCE
microservice: patient-monitoring
status: wave-15-zf-scaffold
date: 2026-05-21
owner_team: council-architecture + ops-sre-reliability
...
# Runbook: Patient Monitoring Auto Rebalance

## Operator Contract
- Runbook id: patient-monitoring-auto-rebalance.
- Primary service namespace: `patient-monitoring`.
...
- Owning team: council-architecture + ops-sre-reliability.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-patient-monitoring-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=patient-monitoring; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n patient-monitoring rollout status deploy/patient-monitoring --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service patient-monitoring --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service patient-monitoring --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service patient-monitoring --cell $CELL --window 30m`.
