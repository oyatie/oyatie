# IP-016 Learning Management backfill and replay worker

Service: learning-management
ChangeSet scope: microservices/learning-management/IP-016-backfill-replay-worker.md
...
## A. Problem
This IP closes the backfill and replay worker gap for `learning-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `backfill-replay-worker` works for course catalogs, enrollment, learning paths, assessments, credential assertions, and regulated attestations need an auditable learning substrate.
...
The success condition is an implementation plan a cold engineer can trace from this IP to concrete files such as `microservices/learning-management/backfill-replay.md` without inventing a hidden service boundary.
...
## C. Deliverables
- D01: `microservices/learning-management/PRD.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D02: `microservices/learning-management/ARCHITECTURE.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D03: `microservices/learning-management/manifest.json` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D04: `microservices/learning-management/competitor-parity-matrix.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D05: `microservices/learning-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D06: `microservices/learning-management/backfill-replay.md` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D07: `microservices/learning-management/contracts/openapi-v1.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D08: `microservices/learning-management/contracts/asyncapi-v1.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D09: `microservices/learning-management/contracts/learning-management-v1.proto` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D10: `microservices/learning-management/src/domain/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D11: `microservices/learning-management/src/usecase/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D12: `microservices/learning-management/src/adapter/mod.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D13: `microservices/learning-management/src/config.rs` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D14: `microservices/learning-management/policies/local-course-publish-approval.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D15: `microservices/learning-management/policies/local-cohort-enrollment-scope.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D16: `microservices/learning-management/policies/local-certificate-issue-gate.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D17: `microservices/learning-management/policy/credential-training-authorization.cedar` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
- D18: `microservices/learning-management/capabilities/course-enroll.yaml` — modify or bind for `backfill-replay-worker` evidence and contract traceability.
...
5. Bind adapter behavior through `src/worker/ replay lane` and `microservices/learning-management/src/adapter/mod.rs`, using typed errors from `microservices/learning-management/src/error.rs` instead of stringly provider failures.
...
- `microservices/learning-management/backfill-replay.md`
...
|---|---|---|
| Workday Learning | Provides a native `backfill and replay worker` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `backfill-replay-worker` to tenant scope, Cedar deny evidence, audit-chain records, and flat `learning-management` ownership. |
...
- Do not move `backfill-replay-worker` into a sibling service such as `community` or `workflow-engine` unless a later ADR changes ownership.
