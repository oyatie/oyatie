# Learning Management Runbook: Assessment Attempt Replay

Service: learning-management  
Surface: local operational primitive suite  
Primary SLO: cohort-enrollment-latency  
Domain focus: course, cohort, assessment, content

## Trigger
- Alert `learning-management-cohort-enrollment-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_learning_management_cohort_enrollment_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when cohort-enrollment-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `learning-management-local-cohort-enrollment-latency` and policy file name to the ticket.
