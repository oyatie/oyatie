# Learning Management Runbook: Certificate Issue Stall

Service: learning-management  
Surface: local operational primitive suite  
Primary SLO: certificate-issue-latency  
Domain focus: course, cohort, assessment, content

## Trigger
- Alert `learning-management-certificate-issue-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_learning_management_certificate_issue_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when certificate-issue-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `learning-management-local-certificate-issue-latency` and policy file name to the ticket.
