# Financial Planning Runbook: Analytics Cost Overrun

Service: financial-planning  
Surface: local operational primitive suite  
Primary SLO: variance-explain-freshness  
Domain focus: budget, forecast, variance, close

## Trigger
- Alert `financial-planning-variance-explain-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_financial_planning_variance_explain_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when variance-explain-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `financial-planning-local-variance-explain-freshness` and policy file name to the ticket.
