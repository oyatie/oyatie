# Financial Planning Runbook: Scenario Version Conflict

Service: financial-planning  
Surface: local operational primitive suite  
Primary SLO: close-cycle-latency  
Domain focus: budget, forecast, variance, close

## Trigger
- Alert `financial-planning-close-cycle-latency` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_financial_planning_close_cycle_latency_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when close-cycle-latency is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `financial-planning-local-close-cycle-latency` and policy file name to the ticket.
