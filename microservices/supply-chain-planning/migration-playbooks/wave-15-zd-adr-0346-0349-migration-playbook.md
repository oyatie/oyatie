...
# Migration Playbook - Wave 15-ZD ADR-0346..0349 doctrine for `supply-chain-planning`

Audience: an Oyatie operator or migration owner preparing `supply-chain-planning` for the Wave 15-ZD doctrine surface before implementation waves author runtime code, manifests, Jenkinsfiles, ArgoCD applications, or sharding bodies.

Outcome: `supply-chain-planning` has a documented migration path for the four doctrine decisions, with no runtime mutation and no manifest mutation.
...
1. Confirm the `supply-chain-planning` manifest exists and is not marked with `status: "Retired"` and is not `doc_class: RetiredMicroserviceMarker`.
2. Capture the current migration-playbooks directory listing before authoring implementation follow-up.
3. Capture whether `supply-chain-planning` already has CI, CD, cellular, sharding, or governance lane references in PRD, ARCH, README, IPs, manifests, runbooks, contracts, Cedar, SLOs, and capabilities.
...
1. Treat `oya verify --ci-required` as the local rehearsal for `supply-chain-planning` changes before push.
...
3. When `supply-chain-planning` changes touch Rust, contracts, manifests, generated docs, or governance lanes, run the verifier before handoff.
...
1. Search `supply-chain-planning` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `supply-chain-planning` manifest `fitness_lanes` array only in the manifest-owning slot.
...
1. Prepare `supply-chain-planning` for a future `sharding_automation` block with autosharding, auto_rebalance, and dynamic_sharding sub-blocks.
...
1. Treat Jenkins LTS as the self-hostable CI substrate for `supply-chain-planning` when GitHub Actions runners are unavailable.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `supply-chain-planning`.
...
1. Freeze the current `supply-chain-planning` doctrine references before implementation starts.
2. Assign ownership for the Wave 15-ZA, 15-ZB, 15-ZD, and 15-ZE follow-up surfaces touching this microservice.
3. Confirm whether `supply-chain-planning` is stateful, tenant-facing, edge-facing, or substrate-facing; that classification drives sharding and CI/CD evidence.
...
5. This file is the `supply-chain-planning` migration scaffold that those implementation lanes can cite.
...
2. This playbook lists every enforced_by lane from the four ADRs that can affect `supply-chain-planning`.
...
- Local verifier transcript for `supply-chain-planning` once Wave 15-ZA lands.
- Rename inventory diff for `supply-chain-planning` once Wave 15-ZB lands.
...
Stop when `supply-chain-planning` has this scaffold committed under `migration-playbooks/`, ADR citation validation passes for the docs corpus, and no file outside this slot boundary is staged for the ZF-18 commit.
