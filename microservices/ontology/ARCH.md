...
- Integration state: PROPOSED for `ontology`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `ontology` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/postgresql-cluster@v1[paid], on-prem/clickhouse-iceberg-layer@v1[paid], colo/openbao-secret-binding@v1[paid], oyatie-as-cloud-provider/service-mesh-waypoint@v1[paid].
...
- Capacity: `per_query` drives CPU `0.24`, RAM `512`, storage `10`, and connection pool variables.
