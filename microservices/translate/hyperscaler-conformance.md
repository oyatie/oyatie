...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that translate
  (MT engine routing + TM leverage + QE + real-time stream) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/translate.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/translate/iac/helm/translate/templates/prometheusrule.yaml
---

# translate — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`translate`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-TRANSLATE`)
...
- **How translate satisfies it**:
  - Capability calls in `microservices/translate/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_translate_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_translate_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/translate/iac/helm/translate/templates/prometheusrule.yaml` (`translate-llm-capability-circuit-open`
    + `translate-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/translate/dashboards/` (per-capability panels).
  - Threat model: `microservices/translate/threat-model.md` §"Dependency
...
- **How translate satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_translate_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/translate/iac/helm/translate/templates/prometheusrule.yaml` (`translate-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/translate/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/translate/capacity-model.md` (where
...
- **How translate satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/translate/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/translate/iac/helm/translate/templates/prometheusrule.yaml` (`translate-saturation-cpu-over-70pct`,
    `translate-errors-5xx-rate-spike`, `translate-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/translate/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/translate/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How translate satisfies it**:
  - Every SLO in `microservices/translate/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/translate/iac/helm/translate/templates/prometheusrule.yaml` (`translate-*-fast-burn-1h-14x` +
    `translate-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/translate/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/translate/hyperscaler-conformance.md
test -f microservices/translate/iac/helm/translate/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/translate/iac/helm/translate/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/translate/iac/helm/translate/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/translate/iac/helm/translate/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/translate/iac/helm/translate/templates/prometheusrule.yaml || true
