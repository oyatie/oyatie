...
companion_docs:
  ...
  - microservices/translate/README.md
  - microservices/translate/ARCHITECTURE.md
---
...
`translate` keeps its existing bounded context and flat `microservices/translate/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `translate`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `translate` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_request` drives CPU `0.35`, RAM `768`, storage `16`, and connection pool variables.
- DR: RTO `2100` seconds and RPO `300` seconds constrain backup and failover primitives.
