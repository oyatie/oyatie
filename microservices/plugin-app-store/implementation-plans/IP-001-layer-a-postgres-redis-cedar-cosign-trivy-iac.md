...
impl_plan_id: IP-001-layer-a-postgres-redis-cedar-cosign-trivy-iac
status: pending
...
# IP-001-layer-a-postgres-redis-cedar-cosign-trivy-iac: Layer-A Postgres + Valkey + Cedar evaluator + Cosign + Trivy + Wasmtime IaC
...
Test fixtures live under `microservices/plugin-app-store/tests/fixtures/ip-001-layer-a-postgres-redis-cedar-cosign-trivy-iac/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.
...
- `ip_id`: `IP-001-layer-a-postgres-redis-cedar-cosign-trivy-iac`
