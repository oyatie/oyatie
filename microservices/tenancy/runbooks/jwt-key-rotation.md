...
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate tenancy-jwt-key-rotation --production-snapshot` exits non-zero against the latest production evidence bundle.
...
14. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate tenancy-jwt-key-rotation --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p oya-tenancy-domain jwt_key_rotation -- --nocapture`.
...
7. Pause promotion: `oya vcs hold --microservice tenancy --reason $INCIDENT_ID --runbook jwt-key-rotation`.
