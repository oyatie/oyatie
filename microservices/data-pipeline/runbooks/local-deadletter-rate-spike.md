# Data Pipeline Runbook: Deadletter Rate Spike

Service: data-pipeline  
Surface: local operational primitive suite  
Primary SLO: deadletter-rate  
Domain focus: ingest, transform, quality, freshness

## Trigger
- Alert `data-pipeline-deadletter-rate` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_pipeline_deadletter_rate_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when deadletter-rate is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-pipeline-local-deadletter-rate` and policy file name to the ticket.
