# Data Pipeline Runbook: Source Credential Expiry

Service: data-pipeline  
Surface: local operational primitive suite  
Primary SLO: ingest-freshness  
Domain focus: ingest, transform, quality, freshness

## Trigger
- Alert `data-pipeline-ingest-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_data_pipeline_ingest_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when ingest-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `data-pipeline-local-ingest-freshness` and policy file name to the ticket.
