...
**Phase:** PHASE-02-FOUNDRY-DATA-SUBSTRATE (new phase introduced 2026-05-18; addendum at `microservices/foundry/PHASE-02-FOUNDRY-DATA-SUBSTRATE-ADDENDUM.md`)
...
The cluster is foundry-namespace; the canonical Helm chart lives at `microservices/foundry/iac/helm/milvus/` and is already partially authored. This IP completes the chart, verifies all four planes (Access / Coordinator / Worker / Storage) reach steady state, and exercises a smoke round-trip.

## File targets (paths + line ranges)

| Path | Action | Line range | Owner |
|---|---|---|---|
| `microservices/foundry/iac/helm/milvus/Chart.yaml` | exists | 1-30 | infra |
| `microservices/foundry/iac/helm/milvus/values.yaml` | exists | 1-150 | infra |
| `microservices/foundry/iac/helm/milvus/templates/external-secret.yaml` | exists | 1-30 | infra |
| `microservices/foundry/iac/helm/milvus/templates/network-policy.yaml` | exists | 1-40 | infra |
| `microservices/foundry/iac/helm/milvus/templates/prometheus-rule.yaml` | exists | 1-70 | infra |
| `microservices/foundry/iac/helm/milvus/templates/service-monitor.yaml` | exists | 1-30 | infra |
| `microservices/foundry/iac/helm/milvus/templates/coord-statefulset.yaml.tpl` | reference | n/a — chart bundled | infra |
| `microservices/foundry/iac/helm/milvus/templates/worker-deployment.yaml.tpl` | reference | n/a — chart bundled | infra |
| `microservices/foundry/iac/kustomize/overlays/pack-kr/milvus/kustomization.yaml` | create | 1-25 | infra+pack-kr |
| `microservices/foundry/iac/kustomize/overlays/pack-eu/milvus/kustomization.yaml` | create | 1-25 | infra+pack-eu |
| `microservices/foundry/iac/kustomize/components/milvus-foundry-namespace/namespace.yaml` | create | 1-20 | infra |
| `microservices/foundry/iac/kustomize/components/milvus-foundry-namespace/rbac.yaml` | create | 1-60 | infra |
| `microservices/foundry/tests/integration/milvus_cluster_smoke.rs` | create | 1-200 | backend |
...
1. **Helm chart** — already authored at `microservices/foundry/iac/helm/milvus/`; this IP audits + completes any missing template.
...
4. **Cluster smoke test** — Rust integration test at `microservices/foundry/tests/integration/milvus_cluster_smoke.rs` verifying 4-plane health (see Test plan below).
...
|---|---|---|
| `test_helm_install_dev_cell` | `tests/integration/milvus_cluster_smoke.rs` | `helm install foundry-milvus microservices/foundry/iac/helm/milvus/` succeeds in dev cell |
...
- `helm install foundry-milvus microservices/foundry/iac/helm/milvus/` succeeds in dev cell within 5min.
...
- **Dashboard:** `microservices/foundry/dashboards/milvus-overview.json` (existing) wired to the per-cell Prometheus.
- **Evidence pack:** `evidence/foundry-milvus-cluster-bootstrap-<cell-id>.json` emitted to `microservices/foundry/evidence/` on every successful helm-install run.
...
Small cell scales the worker plane to 3 query + 2 data + 1 index. Large cell scales to 12 query + 8 data + 4 index. Per-cell sizing is read from `microservices/foundry/capacity-model-milvus.md`.
