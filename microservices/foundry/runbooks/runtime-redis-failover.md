---
doc_class: Runbook
title: Redis cluster failover (partition / ACL drift / shard fail)
...
date: 2026-05-17
related_artifacts:
  - microservices/foundry/failure-modes.md (FM-02, FM-03)
  - microservices/foundry/threat-model.md (T-T-02)
  - microservices/foundry/policy/runtime-isolation.md
  - microservices/foundry/multi-region.md
doc_status: published
---

# Runbook: Redis cluster failover

## Trigger

ONE of:
- Redis cluster partition (FM-02): `oya_foundry_runtime_redis_connection_failures_total > threshold`; majority loss in shards.
- Redis ACL drift (FM-03): `oya-check-session-prefix-isolation` lane fails OR Helm-state-validator alarms; OR `oya_foundry_runtime_unauthorized_attempt_total > 0` from Redis-side rejection.
...
| 2 | Verify replica promotion: for each shard with majority loss, replica auto-promotes via Redis Sentinel / Cluster | ≤5min |
| 3 | Check ingest queue depth in runtime executor pods: backpressure to clients via 503 (not infinite queue) | ≤2min |
| 4 | HPA may scale runtime-pool to absorb cold-restore latency hit (Redis miss → Postgres) | ≤5min |
...
| 3 | Compare with declared (Helm values): `diff <(redis-cli ACL LIST) microservices/foundry/iac/helm/redis/expected-acl.txt` | ≤2min |
...
| 6 | Audit who mutated ACL: OpenBao audit log + Redis audit log + Kubernetes audit log | – |
...
| 2 | Replica auto-promote (Redis Cluster); verify promotion: `cluster nodes` | ≤2min |
...
- For FM-03: harden control-plane access to Redis (Kubernetes RBAC + OpenBao JIT-only); audit which path allowed live mutation; tighten admission controllers.

## References

- `microservices/foundry/failure-modes.md` FM-02, FM-03.
- `microservices/foundry/threat-model.md` T-T-02.
- `microservices/foundry/policy/runtime-isolation.md` TI-01, TI-02.
- `microservices/foundry/multi-region.md` §"DR Failover".
