...
./bin/oya vcs claim \
  ...
  docs/architecture/notes microservices/foundry/scratch
```
...
The claim is now active. Other agents cannot touch `docs/architecture/notes/**` or `microservices/foundry/scratch/**` while you hold it.

## Step 2 — work (edit files)

```bash
mkdir -p docs/architecture/notes microservices/foundry/scratch
...
cat > microservices/foundry/scratch/$USER-tutorial.json <<EOF
{ "actor": "$USER",
...
```bash
git add docs/architecture/notes microservices/foundry/scratch
