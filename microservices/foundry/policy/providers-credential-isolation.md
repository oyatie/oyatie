...
related_artifacts:
  ...
  - microservices/foundry/runbooks/credential-rotation.md
  - microservices/foundry/runbooks/provider-credentials-revoke.md
...
**Statement.** Any new adapter crate publish (or version bump of an existing adapter) requires two-person review: one axis-foundry reviewer + one ops-security reviewer. The CODEOWNERS file at `microservices/foundry/CODEOWNERS` encodes the rule.
...
The OpenBao policy that issues tokens to foundry-providers pods is in `microservices/cloud-secrets/policy/foundry-providers.hcl`; the Cedar fragment that gates tenant-operator credential management is in `microservices/foundry/policy/openbao-credential.cedar`.
...
- `microservices/foundry/threat-model.md` T-01.
- `microservices/foundry/runbooks/credential-rotation.md`.
