...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that forms
  (form render + submission + AI builder + PII encryption) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/forms.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml
---

# forms — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`forms`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-FORMS`)
...
- **How forms satisfies it**:
  - Capability calls in `microservices/forms/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_forms_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_forms_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml` (`forms-llm-capability-circuit-open`
    + `forms-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/forms/dashboards/` (per-capability panels).
  - Threat model: `microservices/forms/threat-model.md` §"Dependency
...
- **How forms satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_forms_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml` (`forms-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/forms/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/forms/capacity-model.md` (where
...
- **How forms satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/forms/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml` (`forms-saturation-cpu-over-70pct`,
    `forms-errors-5xx-rate-spike`, `forms-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/forms/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/forms/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How forms satisfies it**:
  - Every SLO in `microservices/forms/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml` (`forms-*-fast-burn-1h-14x` +
    `forms-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/forms/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/forms/hyperscaler-conformance.md
test -f microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml || true
