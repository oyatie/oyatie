...
## §principals
This anchor is closed for `forms` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.forms.runtime` owns normal `t0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.forms.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/forms/backfill-replay.md` when present.
- Platform principal `oyatie.forms.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.forms.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.forms.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `t0-suggest` is evaluated as `<tenant>.forms.t0-suggest` before any `forms` state mutation.
...
## §cedar-gates
This anchor is closed for `forms` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/forms/policy/auditor-scope.cedar`, `microservices/forms/policy/ci-scope.cedar`, `microservices/forms/policy/data-residency.md`, `microservices/forms/policy/dual-context.md`, `microservices/forms/policy/public-read.cedar`, `microservices/forms/policy/tenant-scope.cedar`.
- Action namespace uses `forms::<bounded_context>::<verb>`; the first protected action is `forms::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/forms/policy/auditor-scope.cedar`, `microservices/forms/policy/ci-scope.cedar`, `microservices/forms/policy/data-residency.md`, `microservices/forms/policy/dual-context.md`, `microservices/forms/policy/public-read.cedar`, `microservices/forms/policy/tenant-scope.cedar`.
- Example: `forms::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `forms` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `forms.unknown` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- Planned table/event surface `forms.t0_suggest_2` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `forms.t0_suggest_3` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `forms.t0_suggest_4` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
- Planned table/event surface `forms.t0_suggest_5` inherits the same `tenant_id` and audit fields until a migration file supersedes this placeholder name.
...
This anchor is closed for `forms` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `forms` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `forms` only through contracts `microservices/forms/contracts/asyncapi/forms.asyncapi.yaml`, `microservices/forms/contracts/openapi/forms.openapi.yaml`, `microservices/forms/contracts/proto/forms.proto`.
- If product: `forms` may call substrate services but must not create product-to-product synchronous dependencies.
...
## §policy-evaluation
This anchor is closed for `forms` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/forms/policy/auditor-scope.cedar`, `microservices/forms/policy/ci-scope.cedar`, `microservices/forms/policy/data-residency.md`, `microservices/forms/policy/dual-context.md`, `microservices/forms/policy/public-read.cedar`, `microservices/forms/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `forms::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `forms.unknown`.
...
## §cell-eligibility
This anchor is closed for `forms` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `forms` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `forms` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `forms.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `t0-suggest` can request classification or summarization with `audience_tag=forms.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `forms` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `forms` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `unknown` is read/written as an ontology node or edge projection when `forms` needs cross-surface discovery.
...
## §transport
This anchor is closed for `forms` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/forms/contracts/asyncapi/forms.asyncapi.yaml`, `microservices/forms/contracts/openapi/forms.openapi.yaml`, `microservices/forms/contracts/proto/forms.proto`.
...
- IaC transport evidence: `microservices/forms/iac/helm/captcha-sidecar/Chart.yaml`, `microservices/forms/iac/helm/captcha-sidecar/values.yaml`, `microservices/forms/iac/helm/form-cdn/Chart.yaml`, `microservices/forms/iac/helm/form-cdn/values.yaml`, `microservices/forms/iac/helm/form-rest/Chart.yaml`, `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml`; +6 more.
- Example: `forms` `t0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `forms` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.
...
- IaC manifests in scope: `microservices/forms/iac/helm/captcha-sidecar/Chart.yaml`, `microservices/forms/iac/helm/captcha-sidecar/values.yaml`, `microservices/forms/iac/helm/form-cdn/Chart.yaml`, `microservices/forms/iac/helm/form-cdn/values.yaml`, `microservices/forms/iac/helm/form-rest/Chart.yaml`, `microservices/forms/iac/helm/form-rest/templates/prometheusrule.yaml`; +6 more.
- Catalog crates/components: `microservices/forms/catalog/oya-forms-bulk-distribute-worker.yaml`, `microservices/forms/catalog/oya-forms-captcha-adapter.yaml`, `microservices/forms/catalog/oya-forms-conditional-logic-domain.yaml`, `microservices/forms/catalog/oya-forms-crypto-domain.yaml`, `microservices/forms/catalog/oya-forms-domain.yaml`, `microservices/forms/catalog/oya-forms-export-worker.yaml`; +18 more.
- Example: `t0-suggest` API runs as a Kata-isolated container; scheduled/background work for `forms` uses a separate worker principal.
...
## §observability
This anchor is closed for `forms` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/forms/slos/accessibility-wcag-correctness.openslo.yaml`, `microservices/forms/slos/ai-form-build-latency.openslo.yaml`, `microservices/forms/slos/analytics-render-latency.openslo.yaml`, `microservices/forms/slos/bulk-distribute-latency.openslo.yaml`, `microservices/forms/slos/export-csv-latency.openslo.yaml`, `microservices/forms/slos/field-validate-latency.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.forms.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `forms_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `forms_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/forms/slos/accessibility-wcag-correctness.openslo.yaml`.
...
- Example trace: `forms.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `forms` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `forms` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/forms/policy/auditor-scope.cedar`, `microservices/forms/policy/ci-scope.cedar`, `microservices/forms/policy/data-residency.md`, `microservices/forms/policy/dual-context.md`, `microservices/forms/policy/public-read.cedar`, `microservices/forms/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `forms` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/forms`.
...
This anchor is closed for `forms` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `forms` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `forms` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `forms` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `forms` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `forms` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `forms` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/forms/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/forms/runbooks/ai-form-build-rollback.md`, `microservices/forms/runbooks/captcha-degraded.md`, `microservices/forms/runbooks/embed-iframe-csp-incident.md`, `microservices/forms/runbooks/export-pipeline-failure.md`, `microservices/forms/runbooks/pii-leak-incident-p0.md`, `microservices/forms/runbooks/response-store-corruption.md`; +1 more.
