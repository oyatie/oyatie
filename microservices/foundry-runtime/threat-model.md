...
- Mitigations: every invocation carries OIDC bearer + actor binding + audit-chain Ed25519 seal; per-changeset evidence at `microservices/foundry-runtime/evidence/multispectrum/*.json` git-committed; commit signed.
- Owner: axis-foundry-runtime + audit-chain
- Residual: L
...
- Mitigations: M01 disallows tenant-supplied code execution (PRD OQ#5 resolved disallow); when sandbox decided post-M01, gVisor / Firecracker / WASM isolate per per-tenant tier; today's pod has seccomp + AppArmor + non-root + read-only FS; egress network policy default-deny except sibling SPIFFE-validated endpoints.
- Owner: ops-security + axis-foundry-runtime
...
|---|---|---|
| SOC 2 Type 2 | CC1–CC9 covered inline per row above | `microservices/foundry-runtime/compliance.md` |
