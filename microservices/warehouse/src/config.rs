...
impl ServiceConfig {
    pub fn local_default(tenant_id: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            observability: ObservabilityConfig {
                service_namespace: "oyatie.warehouse".to_string(),
                audit_topic: "audit.warehouse".to_string(),
                metrics_prefix: "oya_warehouse".to_string(),
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.service_name != "warehouse" {
            return Err(ServiceError::validation(
                "service_name",
                "service_name must remain warehouse",
            ));
        }
