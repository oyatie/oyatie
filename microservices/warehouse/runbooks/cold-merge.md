---
doc_class: Runbook
doc_id: RUNBOOK-WAREHOUSE-COLD_MERGE
...
# Runbook: Warehouse Cold Merge

## Operator Contract
- Runbook id: warehouse-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-warehouse-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=warehouse; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n warehouse rollout status deploy/warehouse --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service warehouse --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service warehouse --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service warehouse --cell $CELL --window 30m`.
