...
Live Protected Health Information (PHI) under HIPAA never enters foundry-eval eval-set authoring. Eval-set golden inputs and case prompts use synthetic-PHI fixtures only. This policy is BLOCKER on all `microservices/foundry-eval/eval-sets/**` content under the `oya-check-synthetic-phi-only` LEAN lane.
