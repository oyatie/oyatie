...
`microservices/foundry-eval/iac/helm/{postgres,clickhouse,golden-store}/`: 3 charts × {Chart.yaml + values.yaml + templates/}.
...
for chart in postgres clickhouse golden-store; do
  helm lint microservices/foundry-eval/iac/helm/$chart/
  helm template microservices/foundry-eval/iac/helm/$chart/ | kubectl apply --dry-run=client -f -
done
