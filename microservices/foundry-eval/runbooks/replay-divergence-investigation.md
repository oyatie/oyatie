...
date: 2026-05-17
related_artifacts:
  - microservices/foundry-eval/failure-modes.md (FM-04 replay divergence)
...
| 4 | Categorise: (a) non-deterministic case (seed not present / seed value differs); (b) provider response drift (provider model changed); (c) tokeniser drift; (d) capability prompt template changed; (e) golden-output mismatch | ≤ 15 min |
...
| 9 | For (e) — golden mismatch: re-verify golden Cosign signature; if signature valid, re-evaluate golden (may need refresh) | per category |
...
- ADR-0024 §"Replay against past traces".
- `microservices/foundry-eval/failure-modes.md` FM-04.
- `microservices/foundry-eval/threat-model.md` T-E-01 (sandbox escape).
- `microservices/foundry-eval/PRD.md` §"Performance Targets" (replay determinism ≤ 100ms).
