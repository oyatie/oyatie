...
- Alert: `oya_llm_gateway_pool_exhausted{provider="..."} == 1` (pool has zero active keys).
- Metric: `oya_llm_gateway_active_keys{provider="..."}` gauge at 0 (mirrors `KeyPool::active_count`).
...
2. **Why blacklisted?** Inspect the dominant upstream failure on the metric
   `oya_llm_gateway_key_failures_total{provider,code}`:
...
- Reduce load: tighten per-tenant budgets for the noisiest tenant(s) (identify via
  `oya_llm_gateway_tokens_total{tenant}`); the offending tenant should be the one feeling 429
...
- If a fallback provider pool has capacity, confirm the failover ladder is routing to it
  (`oya_llm_gateway_fallback_total`); if not, check the fallback config.
...
- `oya_llm_gateway_active_keys{provider}` > 0 for every pool.
