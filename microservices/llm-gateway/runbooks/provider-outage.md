...
- Alert: `oya_llm_gateway_provider_breaker_open{provider="..."} == 1`.
- Metric: `oya_llm_gateway_upstream_failures_total{provider,code=~"5..|transport"}` spiking across
...
- Symptom: 503 `{"error":{"type":"gateway_provider_unavailable", ...}}` + `Retry-After`; or, if a
  fallback absorbed it, elevated `oya_llm_gateway_fallback_total{from,to}` with 200s.
...
1. **Confirm provider-level, not key-level.** `GET /admin/v1/pools` → if `active_keys > 0` but
   ...
   `oya_llm_gateway_upstream_failures_total` is uniform across all fingerprints.
2. **Is a fallback absorbing it?** Check `oya_llm_gateway_fallback_total{from="<down>",to="..."}`
...
- Provider status page reports resolved; `oya_llm_gateway_upstream_failures_total` for that
