...
The llm-gateway is a **stateless** reverse proxy. It holds no durable tenant data of its own:

- **Pooled provider keys** are ephemeral in-memory cache loaded from OpenBao at startup and on periodic refresh. Keys are not persisted by the gateway; they are re-fetched from OpenBao on restart or key rotation.
...
| RTO    | < 60 s | Process restart re-initializes the key pool from OpenBao. Recovery time is bounded by OpenBao key-fetch latency (typically < 5 s) plus Kubernetes liveness probe grace period. |
...
2. On startup the process fetches pooled keys from OpenBao (`BAO_TOKEN` injected from a k8s Secret).
