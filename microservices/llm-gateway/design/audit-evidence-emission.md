...
`contracts/llm-gateway.asyncapi.yaml#AuditPayload`) with the Bedrock-shaped field set:
...
`contracts/llm-gateway.asyncapi.yaml` (AsyncAPI 3.1.0) so a single `request_id` ties audit ↔ usage ↔
OTel trace.
...
- `contracts/llm-gateway.asyncapi.yaml` (`AuditPayload`, `UsagePayload`).
