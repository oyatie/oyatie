...
- **Detection:** `KeyPool::select` → `Exhausted`; `oya_llm_gateway_active_keys == 0`.
...
- **Detection:** High `oya_llm_gateway_key_blacklist_total` churn.
...
- **Detection:** `oya_llm_gateway_key_refresh_failures_total` > 0.
...
- **Detection:** `oya_llm_gateway_audit_emit_failures_total` > 0; alert-if-disabled.
...
- `crates/oya-llm-gateway-kernel/src/lib.rs` (state machine; unit tests cover 1.1–1.4 transitions).
