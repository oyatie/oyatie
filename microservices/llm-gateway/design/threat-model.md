...
llm-gateway is a **reverse proxy**, not a model host, so only the proxy-relevant subset of the
...
- **Mitigation (front-line control):** The jittered-cooldown key-pool state machine
  (`crates/oya-llm-gateway-kernel`) — failing keys are blacklisted and cooled down; **jitter
...
- **Mitigation:** Two distinct realms; Cedar default-deny on admin/audit actions unless the
  principal is in the corresponding realm (`policy/llm-gateway.cedar`); constant-time check per realm.
...
- `policy/llm-gateway.cedar`; `design/data-residency.md`; `design/audit-evidence-emission.md`.
