...
- Reading the audit stream / body store requires the `AuditReader` Cedar role (distinct from admin),
  and reads are SIEM-forwarded (brief §7). See `policy/llm-gateway.cedar`.
...
- `design/audit-evidence-emission.md`, `design/tenant-isolation.md`, `policy/llm-gateway.cedar`.
