...
**Research foundation:** `microservices/llm-gateway/design/hyperscaler-best-practice-brief.md`
**Status:** Planned (kernel implemented; rest crate + contracts spec'd to production)

## Scope

Take the llm-gateway µservice from its current code-backed local foundation (`CS-LLM-GATEWAY-AGENT-DISPATCH-001` — pure kernel + axum SSE-passthrough proxy) to a **production-grade multi-provider LLM gateway** with:
...
1. **PRD** — `microservices/llm-gateway/PRD.md` (research-grounded; Acceptance Criteria AC-1..AC-8).
2. **Contracts** (canonical versions mandatory):
   - `contracts/llm-gateway.openapi.yaml` — OpenAPI **3.2.0**.
   - `contracts/llm-gateway.asyncapi.yaml` — AsyncAPI **3.1.0**.
   - `contracts/llm-gateway.proto` — proto3 admin/internal gRPC.
3. **Capabilities** — `capabilities/llm-gateway.capabilities.yaml`.
4. **Policy** — `policy/llm-gateway.cedar` (ingress + admin realm authz).
...
Add the canonical routes to `crates/oya-llm-gateway-rest` alongside the existing transparent `/proxy/{group}/{*rest}` path:
...
**Acceptance:** matches PRD AC-1; `contracts/llm-gateway.openapi.yaml` validates as OpenAPI 3.2.0.
...
**Acceptance:** PRD AC-3, AC-4.1; kernel transitions already covered by `oya-llm-gateway-kernel` unit tests.
...
- Cedar realm policies at `policy/llm-gateway.cedar`.
...
**Acceptance:** PRD AC-6; contract `contracts/llm-gateway.asyncapi.yaml` (AsyncAPI 3.1.0).
...
|---|---|---|
| All keys in a pool blacklisted | `active_count == 0`; `oya_llm_gateway_pool_exhausted` | 503 `gateway_key_exhausted` + `Retry-After` = soonest restore; page per `runbooks/key-exhaustion.md` |
...
- `microservices/llm-gateway/PRD.md` (§4 recommended design, §6 acceptance criteria).
- `microservices/llm-gateway/design/hyperscaler-best-practice-brief.md` (cited per domain).
- ADR-0090, ADR-0105, ADR-0131, ADR-0373, ADR-0373, ADR-0373.
- Kernel: `crates/oya-llm-gateway-kernel/src/lib.rs` (implemented, unit-tested).
- Rest foundation: `crates/oya-llm-gateway-rest/src/{proxy,channel,keystore,auth,metrics,logging,state,config}.rs`.
