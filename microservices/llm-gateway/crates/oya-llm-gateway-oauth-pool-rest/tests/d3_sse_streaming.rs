...
struct NoopSink;
impl oya_llm_gateway_oauth_pool_kernel::EventSink for NoopSink {
    fn emit(&self, _: oya_llm_gateway_oauth_pool_kernel::LlmGatewayEvent) {}
}
...
async fn sse3_streaming_response_hop_by_hop_not_leaked() {
    ...
    let app = oya_llm_gateway_oauth_pool_rest::build_router(state);
...
async fn sse4_lease_held_during_stream_third_request_503() {
    ...
    let lease1 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now()).unwrap();
    let lease2 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now()).unwrap();

    // Third request should fail — both seats are in use.
    let result3 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
    assert!(
        result3.is_err(),
    ...
    assert!(
        matches!(
            result3.err().unwrap(),
            oya_llm_gateway_oauth_pool_kernel::SubscriptionPoolError::NoEligibleSeat
        ),
    ...
    let result4 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
    assert!(
        result4.is_ok(),
...
async fn sse5_mid_stream_error_seat_outcome_server_error() {
    ...
    let err_stream: oya_llm_gateway_oauth_pool_rest::BoxStream<Result<Bytes, RestAdapterError>> =
    ...
    let r1 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
    let r2 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
    let r3 = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
...
async fn sse6_client_drop_releases_lease_cleanly() {
    ...
    let infinite_stream: oya_llm_gateway_oauth_pool_rest::BoxStream<
    ...
    let result = SubscriptionPool::lease(&pool_ref, &agent, &gate, Instant::now());
    assert!(
        result.is_ok(),
