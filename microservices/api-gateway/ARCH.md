...
`api-gateway` keeps its existing bounded context and flat `microservices/api-gateway/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `api-gateway`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `api-gateway` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/edge-gateway@v1[paid], oci-guest/edge-gateway@v1[paid], on-prem/edge-gateway@v1[paid], colo/edge-gateway@v1[paid], oyatie-as-cloud-provider/edge-gateway@v1[paid].
...
- Capacity: `per_request` drives CPU `0.1`, RAM `128`, storage `0`, and connection pool variables.
