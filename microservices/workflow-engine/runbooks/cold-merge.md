---
doc_class: Runbook
doc_id: RUNBOOK-WORKFLOW_ENGINE-COLD_MERGE
...
# Runbook: Workflow Engine Cold Merge

## Operator Contract
- Runbook id: workflow-engine-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-workflow-engine-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=workflow-engine; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n workflow-engine rollout status deploy/workflow-engine --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service workflow-engine --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service workflow-engine --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service workflow-engine --cell $CELL --window 30m`.
