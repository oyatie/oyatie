---
doc_class: Runbook
title: Valkey Sentinel failover for lease coordinator
...
date: 2026-05-17
related_artifacts:
  - microservices/workflow-engine/failure-modes.md (FM-05 Valkey outage)
  - microservices/workflow-engine/PRD.md (execution-engine adapter-redis)
  - microservices/workflow-engine/multi-region.md
doc_status: published
---

# Runbook: Valkey Sentinel failover for lease coordinator

## Trigger

ONE of:

1. **Auto**: Valkey Sentinel quorum lost; `oya_workflow_engine_redis_sentinel_quorum_healthy == 0` for ≥ 30s.
...
- Single AZ Valkey pod failure with quorum preserved: Sev-3 (operational).
- Quorum lost; cluster-wide step dispatch halted: Sev-1.
- Auto-recovery successful within 5min: Sev-2 retroactively.

## Impact

When Valkey is unhealthy, the engine's lease coordinator is unavailable. New step dispatch fails; in-flight runs pause at next step boundary. Long-running workflows are unaffected (their state is in Postgres; resumption resumes once Valkey is back). Tenant impact: step latency spike + transient run-start refusal.

## Pre-checks

1. Verify Valkey Sentinel pod state: `kubectl -n workflow-engine get pods -l app=redis-sentinel`.
2. Verify Sentinel quorum: `redis-cli -p 26379 sentinel masters`.
3. Verify primary Valkey reachability: `redis-cli -h redis-primary.workflow-engine.svc PING`.
...
| 3 | Verify step dispatch resumes via fallback path; latency degraded (claim acquisition now ~50ms vs ~5ms via Valkey). |
| 4 | Investigate quorum loss: AZ outage? Network partition? Resource exhaustion? |
| 5 | Restore Valkey: scale up new Sentinel pods; verify quorum reached. |
| 6 | Deactivate Postgres fallback: `cargo run -p oya-dev-cli -- workflow-engine deactivate-postgres-lock-fallback`. Engine clients reconnect to Valkey. |
| 7 | Tenant notification + status page update. |

## Recovery Path C — Sentinel + Valkey both lost (extreme failure)

| Step | Action |
|---|---|
| 1 | Sev-1; engage director-level. |
| 2 | Postgres fallback (Path B step 2) active immediately. |
| 3 | Rebuild Valkey cluster from Helm chart: `kubectl rollout restart deployment/redis-sentinel deployment/redis-primary`. |
| 4 | Verify cluster healthy + Sentinel quorum re-established. |
| 5 | Lease state in Valkey is regenerable from Postgres (engine knows which runs are in-flight); engine workers re-claim leases automatically. |
| 6 | Deactivate Postgres fallback once Valkey is fully recovered. |

## Recovery Path D — DR failover triggered Valkey transition

When DR failover is in progress (per `multi-region.md`), Valkey Sentinel must be promoted in the DR pair:

| Step | Action |
|---|---|
| 1 | DR failover initiates (Sev-1 or Sev-2 from multi-region drill). |
| 2 | DR-pair Valkey Sentinel cluster pre-warmed; engine clients connect to DR-pair endpoints (DNS failover). |
| 3 | Lease state in DR-pair Valkey is empty (cross-pack replication forbidden); engine workers regenerate leases from DR-pair Postgres state. |
...
| 3 | Cycle Valkey pods (drain + drain-and-replace); Sentinel handles primary election transparently. |
...
- Action: review Valkey capacity if quorum was lost due to resource exhaustion.
