...
ip_id: IP-journey-j102-cross-tenant-orchestration
...
# IP - workflow-engine role in j102: Raw material purchase with quality attestation
...
workflow-engine owns only the cross-tenant-orchestration slice for j102. It does not absorb another service
responsibility, does not bypass Cedar, and does not write into another tenant-owned store without an explicit grant.

## Acceptance criteria

1. workflow-engine exposes or consumes the typed j102 contract without ad hoc string parsing.
...
|---|---|---|---|---|
| 01 purchase run start | marketplace purchase order is accepted for specialty steel. | `POST /runs` starts `raw-material-purchase-v1`; Cedar `start_workflow_run` binds buyer tenant, supplier tenant, `deal_set_id`, and idempotency key. | WorkflowStarted carries the purchase correlation id and active-run-cap evidence. | matches SAP Ariba supplier workflow start. |
| 02 quality attestation wait | supplier submits mill certificate and batch quality evidence. | `SignalWorkflowRun` accepts `quality_attestation_received`; spec signature is checked via `policy/spec-integrity.md` before the step advances. | StepCompleted records `quality_attestation_sha` and audit seal. | matches Camunda receive task for supplier certificate. |
| 03 lab exception branch | quality lab flags off-spec chemistry. | workflow-engine pauses the run through `/runs/{run_id}/pause` and emits StepFailed with retry exhaustion metadata. | WorkflowPaused plus `workflow-step-execute-latency` show the operator evidence. | matches Temporal activity failure with manual intervention. |
| 04 counterparty notification | AcmeRawMaterials must acknowledge the off-spec branch. | EventBus publishes StepStarted/StepCompleted to the tenant-hashed channel; tenant_id remains server-stamped. | notification step records permit id and correlation id. | matches ServiceNow workflow notification task. |
| 05 replacement batch | supplier proposes replacement batch under same deal set. | `SignalWorkflowRun` accepts replacement proposal only while run status is PAUSED or WAITING. | StepRetried references original failed step and replacement batch id. | matches Step Functions callback task token resume. |
| 06 audit close | buyer accepts conforming material. | WorkflowCompleted emitted only after quality and escrow steps are sealed. | completion SLO plus audit hash prove closeout. | matches Oracle Procurement receipt close. |
| 07 boundary denial | unrelated supplier attempts to view purchase quality state. | `read_step_execution` is denied by `tenant-scope.cedar` cross-tenant forbid. | unauthorized read counter and denial audit provide evidence. | matches Auth0 org-scoped read denial. |
| 08 debug replay | operator replays attestation sequence after a lab outage. | `ReplayDebuggerBackend.StreamReplaySteps` reads snapshots without executing supplier calls. | ReplaySession verifies deterministic state transition. | matches Temporal deterministic replay. |
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/workflow-engine/IP-journey-j102-cross-tenant-orchestration.md` matched `SLO, escrow, payment`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/workflow-engine/IP-journey-j102-cross-tenant-orchestration.md` matched `emission`.
