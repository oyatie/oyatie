...
ip_id: IP-journey-j86-cadence-orchestrator
...
# IP-journey-j86-cadence-orchestrator - workflow-engine

## Goal

Implement the `cadence-orchestrator` slice for `workflow-engine` so j86 can satisfy `PCI-DSS-L1-v4` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/workflow-engine/contracts/openapi/j86-cadence-orchestrator-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/workflow-engine/contracts/asyncapi/j86-cadence-orchestrator-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/workflow-engine/contracts/proto/j86-cadence-orchestrator-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/workflow-engine/policy/j86-cadence-orchestrator.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/workflow-engine/runbooks/j86-cadence-orchestrator-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/workflow-engine/tests/j86_cadence_orchestrator_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J86WorkflowEngineCadenceOrchestratorStarted`.
- `J86WorkflowEngineCadenceOrchestratorPermitted`.
- `J86WorkflowEngineCadenceOrchestratorDenied`.
- `J86WorkflowEngineCadenceOrchestratorCommitted`.
- `J86WorkflowEngineCadenceOrchestratorRolledBack`.
...
|---|---|---|---|---|
| 01 payment run start | tokenized payment workflow starts. | `POST /runs` / `ExecutionEngine.StartWorkflowRun`; Cedar uses `policy/tenant-scope.cedar` and spec reads pin `version_sha`. | WorkflowStarted with token reference, never PAN. | matches Stripe payment-intent workflow. |
| 02 3DS challenge | issuer challenge waits for callback. | `POST /runs/{run_id}/signal` / `ExecutionEngine.SignalWorkflowRun`; Cedar uses `policy/tenant-scope.cedar` and spec reads pin `version_sha`. | StepCompleted with challenge_result_hash. | matches Step Functions callback task. |
| 03 capture branch | payment capture is authorized. | `EventBus.PublishWorkflowEvent` and AsyncAPI lifecycle channel; Cedar uses `policy/tenant-scope.cedar` and spec reads pin `version_sha`. | StepStarted for downstream payment service. | matches Adyen capture workflow. |
| 04 fraud hold | fraud signal pauses capture. | `POST /runs/{run_id}/pause` / `ExecutionEngine.PauseWorkflowRun`; Cedar uses `policy/tenant-scope.cedar` and spec reads pin `version_sha`. | WorkflowPaused with fraud_hold reason. | matches Riskified review workflow. |
| 05 settlement close | settlement confirmation seals workflow. | Workflow completion through `WorkflowCompleted` AsyncAPI event; Cedar uses `policy/tenant-scope.cedar` and spec reads pin `version_sha`. | audit seal and completion SLO. | matches Temporal payment saga. |
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/workflow-engine/IP-journey-j86-cadence-orchestrator.md` matched `SLO, payment`.
