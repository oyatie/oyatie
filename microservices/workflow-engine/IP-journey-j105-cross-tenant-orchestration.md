...
ip_id: IP-journey-j105-cross-tenant-orchestration
...
# IP - workflow-engine role in j105: Dispute cross-tenant arbitration
...
workflow-engine owns only the cross-tenant-orchestration slice for j105. It does not absorb another service
responsibility, does not bypass Cedar, and does not write into another tenant-owned store without an explicit grant.

## Acceptance criteria

1. workflow-engine exposes or consumes the typed j105 contract without ad hoc string parsing.
...
|---|---|---|---|---|
| 01 dispute filed | buyer files off-spec material dispute. | `POST /runs` starts arbitration workflow with claim id, purchase id, and idempotency key. | WorkflowStarted links claimant/respondent tenant pair and audit id. | matches Stripe dispute workflow opening. |
| 02 evidence request | arbitrator requests batch evidence from both tenants. | `SignalWorkflowRun` advances evidence collection only for scoped tenant resources. | StepStarted records requested evidence class and permit decision. | matches Camunda multi-party evidence task. |
| 03 respondent answer | AcmeRawMaterials submits response before deadline. | EventBus emits StepCompleted to tenant-hashed channel with correlation id. | deadline slack gauge and audit seal prove timeliness. | matches ServiceNow case response SLA. |
| 04 expert review | quality expert marks evidence inconclusive. | `pauseWorkflowRun` captures manual review reason and prevents settlement execution. | WorkflowPaused evidence anchors reviewer escalation. | matches Temporal manual activity pause. |
| 05 award branch | arbitrator decides refund or replacement. | `signalWorkflowRun` records decision payload; payment or replacement remains downstream service work. | StepCompleted carries award id and no shared-table writes. | matches AWS Step Functions choice state. |
| 06 compensation branch | settlement downstream call fails after award. | StepRetried/StepFailed uses replay-safe idempotency key and compensation reason. | retry counter and audit seal latency show evidence. | matches Saga compensation in Temporal. |
| 07 boundary denial | respondent tries to view claimant-private notes. | `tenant-scope.cedar` denies read of step payload outside tenant scope. | CrossTenantBoundaryDenied audit evidence is required. | matches Auth0 tenant-isolated case notes denial. |
| 08 debug replay | arbitrator reviews disputed timeline. | `StreamReplaySteps` exposes redacted snapshots under auditor entitlement. | ReplaySession proves deterministic sequence. | matches Camunda Operate incident/history view. |
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/workflow-engine/IP-journey-j105-cross-tenant-orchestration.md` matched `payment`.
...
- Authority: ADR-0344.
- Trigger evidence: `microservices/workflow-engine/IP-journey-j105-cross-tenant-orchestration.md` matched `emission`.
