...
struct NoopSink;
impl oya_cloud_intelligence_kernel::EventSink for NoopSink {
    fn emit(&self, _: oya_cloud_intelligence_kernel::LlmGatewayEvent) {}
}
...
async fn sse3_streaming_response_hop_by_hop_not_leaked() {
    ...
    let app = oya_cloud_intelligence_rest::build_router(state);
...
async fn sse4_lease_held_during_stream_third_request_503() {
    ...
    assert!(
        matches!(
            result3.err().unwrap(),
            oya_cloud_intelligence_kernel::SubscriptionPoolError::NoEligibleSeat
        ),
...
async fn sse5_mid_stream_error_seat_outcome_server_error() {
    ...
    let err_stream: oya_cloud_intelligence_rest::BoxStream<Result<Bytes, RestAdapterError>> =
