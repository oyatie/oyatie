...
|---|---|---|
| `microservices/intelligence/iac/helm/postgres/Chart.yaml` | create | upstream dep on bitnami/postgresql-ha at pinned LTS |
| `microservices/intelligence/iac/helm/postgres/values.yaml` | create | HA primary + replica; INSERT-only role for `foundry_evidence_writer`; SELECT for `foundry_evidence_reader`; SELECT+UPDATE+DELETE for `foundry_evidence_retention_cascader` (Cedar-gated RPC only) |
| `microservices/intelligence/iac/helm/evidence-builder/Chart.yaml` | create | deploys pack-builder + recorder REST + bridge worker + regulator-export worker + archive-cascade worker |
| `microservices/intelligence/iac/helm/evidence-builder/values.yaml` | create | per-component replicas + resources; SPIFFE binding; PDB; HPA |
| `microservices/intelligence/iac/helm/evidence-blob-store/Chart.yaml` | create | wraps S3 bucket-policy chart for the audit-chain WORM bucket SUBSCRIPTION (read-side; foundry-evidence reads from substrate's bucket via cross-µservice IAM) |
| `microservices/intelligence/iac/helm/evidence-blob-store/values.yaml` | create | per-pack substrate-bucket reference; cross-µservice IAM principal |
| `microservices/intelligence/iac/kustomize/base/kustomization.yaml` | create | shared base |
| `microservices/intelligence/iac/kustomize/overlays/pack-kr/kustomization.yaml` | create | pack-kr overlay (3y retention reference; KR-pinned) |
| `microservices/intelligence/iac/terraform/oci-evidence-blob-store-iam.tf` | create | cross-µservice IAM grant to read substrate-owned WORM bucket |

## Acceptance Gates

```bash
helm lint microservices/intelligence/iac/helm/postgres
helm lint microservices/intelligence/iac/helm/evidence-builder
...
- `microservices/intelligence/policy/evidence-pack-integrity.md` §"EPI-04".
