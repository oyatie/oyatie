...
impl_plan_id: IP-002-layer-a-postgres-clickhouse-baseline-store-iac
status: pending
...
# IP-002: Layer-A Postgres + ClickHouse + Baseline-Output Store IaC

## Intent

Bundle: Helm charts for Postgres (eval-set metadata; CloudNativePG-backed), ClickHouse (parity-analytics MergeTree), and baseline-store (SeaweedFS + KMS integration). Each chart with HA configuration + per-pack region pinning.

## ChangeSet boundary

`microservices/intelligence/iac/helm/{postgres,clickhouse,baseline-store}/`: 3 charts × {Chart.yaml + values.yaml + templates/}.
...
| `iac/helm/baseline-store/Chart.yaml` | create |
| `iac/helm/baseline-store/values.yaml` | create |

## Acceptance Gates

```bash
for chart in postgres clickhouse baseline-store; do
  helm lint microservices/intelligence/iac/helm/$chart/
  helm template microservices/intelligence/iac/helm/$chart/ | kubectl apply --dry-run=client -f -
done
