...
Live Protected Health Information (PHI) under HIPAA never enters foundry-eval eval-set authoring. Eval-set baseline inputs and case prompts use synthetic-PHI fixtures only. This policy is BLOCKER on all `microservices/intelligence/eval-sets/**` content under the `oya-check-synthetic-phi-only` LEAN lane.
