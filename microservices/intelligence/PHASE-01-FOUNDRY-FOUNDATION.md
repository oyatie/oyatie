...
`microservices/intelligence/IP-001..IP-090-*.md`, partitioned by BC:
...
- The 90 consolidated IPs exist at `microservices/intelligence/IP-*.md`.
...
- All 4 OpenSLO manifests at `microservices/intelligence/slos/` validate.
- Cross-BC e2e tests (AC-X1..AC-X12 from `PRD.md`) green.
- ADR-0138 Phase 5 (six-path code removal) green for any external consumers
  of `microservices/intelligence-{runtime,supervisor,eval,evidence,guardrails,
...
| R4 | External consumers of old `microservices/intelligence-<bc>/` paths break | ADR-0138 Strangler migration + grep verification commands | axis-foundry |
...
- `microservices/intelligence/PRD.md` — consolidated PRD.
- `microservices/intelligence/bc-sources/` — per-BC archive (PRDs, PHASE-01s,
