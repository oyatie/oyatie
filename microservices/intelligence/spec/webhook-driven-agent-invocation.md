...
| B-004 | Principal::"oyatie.agent.codex" | Action::"foundry.webhook.route" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=webhook-driven-agent-invocation | permit when evidence_hash and changeset_id are present |
...
| B-016 | Principal::"oyatie.agent.executor" | Action::"foundry.webhook.receive" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=webhook-driven-agent-invocation | permit when evidence_hash and changeset_id are present |
...
| B-028 | Principal::"oyatie.service.webhook-receiver" | Action::"foundry.webhook.dedup" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=webhook-driven-agent-invocation | permit when evidence_hash and changeset_id are present |
...
| B-040 | Principal::"oyatie.service.admission-gate" | Action::"foundry.agent.invoke" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=webhook-driven-agent-invocation | permit when evidence_hash and changeset_id are present |
...
- Guard 05: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.agent.invoke on Resource::"repo:oyatie/microservices/intelligence" only while `agent_invoked` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
- Guard 17: Principal::"oyatie.service.merge-queue" may invoke foundry.webhook.receive on Resource::"repo:oyatie/microservices/intelligence" only while `delivery_quarantined` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
- Guard 29: Principal::"oyatie.service.completion-gate" may invoke foundry.agent.invoke on Resource::"repo:oyatie/microservices/intelligence" only while `hmac_verified` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
- Guard 41: Principal::"oyatie.agent.codex" may invoke foundry.webhook.receive on Resource::"repo:oyatie/microservices/intelligence" only while `agent_invoked` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
- Guard 53: Principal::"oyatie.agent.planner" may invoke foundry.agent.invoke on Resource::"repo:oyatie/microservices/intelligence" only while `delivery_quarantined` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
- Guard 65: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.webhook.receive on Resource::"repo:oyatie/microservices/intelligence" only while `hmac_verified` is current, the changeset id is stable, the event is signed, and the ADR-0112 invariant is cited.
...
| webhook-driven-agent-invocation-matrix-03 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-04 | cargo test -p oya-vcs-review-mergequeue-kernel --scope router_matched --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves router_matched cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-05 | cargo test -p oya-vcs-webhook-receiver-kernel --scope agent_invoked --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves agent_invoked cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-06 | cargo test -p oya-vcs-admission-gate-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-07 | cargo test -p oya-vcs-changebundle-kernel --scope retry_scheduled --adr ADR-0112 | oya-vcs-changebundle-kernel | proves retry_scheduled cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-08 | cargo test -p oya-vcs-promotion-controller-kernel --scope delivery_quarantined --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves delivery_quarantined cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-09 | cargo test -p oya-vcs-cli-ratchet-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-15 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-16 | cargo test -p oya-vcs-review-mergequeue-kernel --scope retry_scheduled --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves retry_scheduled cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-17 | cargo test -p oya-vcs-webhook-receiver-kernel --scope delivery_quarantined --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves delivery_quarantined cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-18 | cargo test -p oya-vcs-admission-gate-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-19 | cargo test -p oya-vcs-changebundle-kernel --scope delivery_received --adr ADR-0112 | oya-vcs-changebundle-kernel | proves delivery_received cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-20 | cargo test -p oya-vcs-promotion-controller-kernel --scope hmac_verified --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves hmac_verified cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-21 | cargo test -p oya-vcs-cli-ratchet-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-27 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-28 | cargo test -p oya-vcs-review-mergequeue-kernel --scope delivery_received --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves delivery_received cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-29 | cargo test -p oya-vcs-webhook-receiver-kernel --scope hmac_verified --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves hmac_verified cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-30 | cargo test -p oya-vcs-admission-gate-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-31 | cargo test -p oya-vcs-changebundle-kernel --scope router_matched --adr ADR-0112 | oya-vcs-changebundle-kernel | proves router_matched cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-32 | cargo test -p oya-vcs-promotion-controller-kernel --scope agent_invoked --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves agent_invoked cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-33 | cargo test -p oya-vcs-cli-ratchet-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-39 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-40 | cargo test -p oya-vcs-review-mergequeue-kernel --scope router_matched --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves router_matched cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-41 | cargo test -p oya-vcs-webhook-receiver-kernel --scope agent_invoked --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves agent_invoked cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-42 | cargo test -p oya-vcs-admission-gate-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-43 | cargo test -p oya-vcs-changebundle-kernel --scope retry_scheduled --adr ADR-0112 | oya-vcs-changebundle-kernel | proves retry_scheduled cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-44 | cargo test -p oya-vcs-promotion-controller-kernel --scope delivery_quarantined --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves delivery_quarantined cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-45 | cargo test -p oya-vcs-cli-ratchet-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-51 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-52 | cargo test -p oya-vcs-review-mergequeue-kernel --scope retry_scheduled --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves retry_scheduled cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-53 | cargo test -p oya-vcs-webhook-receiver-kernel --scope delivery_quarantined --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves delivery_quarantined cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-54 | cargo test -p oya-vcs-admission-gate-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-55 | cargo test -p oya-vcs-changebundle-kernel --scope delivery_received --adr ADR-0112 | oya-vcs-changebundle-kernel | proves delivery_received cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-56 | cargo test -p oya-vcs-promotion-controller-kernel --scope hmac_verified --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves hmac_verified cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-57 | cargo test -p oya-vcs-cli-ratchet-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-63 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope event_log_appended --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves event_log_appended cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-64 | cargo test -p oya-vcs-review-mergequeue-kernel --scope delivery_received --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves delivery_received cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-65 | cargo test -p oya-vcs-webhook-receiver-kernel --scope hmac_verified --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves hmac_verified cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-66 | cargo test -p oya-vcs-admission-gate-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-67 | cargo test -p oya-vcs-changebundle-kernel --scope router_matched --adr ADR-0112 | oya-vcs-changebundle-kernel | proves router_matched cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-68 | cargo test -p oya-vcs-promotion-controller-kernel --scope agent_invoked --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves agent_invoked cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-69 | cargo test -p oya-vcs-cli-ratchet-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-cli-ratchet-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
...
| webhook-driven-agent-invocation-matrix-75 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope delivery_deduped --adr ADR-0112 | oya-vcs-merge-queue-conflict-kernel | proves delivery_deduped cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-76 | cargo test -p oya-vcs-review-mergequeue-kernel --scope router_matched --adr ADR-0112 | oya-vcs-review-mergequeue-kernel | proves router_matched cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-77 | cargo test -p oya-vcs-webhook-receiver-kernel --scope agent_invoked --adr ADR-0112 | oya-vcs-webhook-receiver-kernel | proves agent_invoked cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-78 | cargo test -p oya-vcs-admission-gate-kernel --scope result_posted_back --adr ADR-0112 | oya-vcs-admission-gate-kernel | proves result_posted_back cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-79 | cargo test -p oya-vcs-changebundle-kernel --scope retry_scheduled --adr ADR-0112 | oya-vcs-changebundle-kernel | proves retry_scheduled cannot advance without policy, evidence, trace, and audit correlation |
| webhook-driven-agent-invocation-matrix-80 | cargo test -p oya-vcs-promotion-controller-kernel --scope delivery_quarantined --adr ADR-0112 | oya-vcs-promotion-controller-kernel | proves delivery_quarantined cannot advance without policy, evidence, trace, and audit correlation |
...
| Peer spec | microservices/intelligence/spec/changeset-state-machine.md | Cross-checks Foundry Changeset State Machine against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/merge-queue-projected-state.md | Cross-checks Foundry Merge Queue Projected State against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/webhook-driven-agent-invocation.md | Cross-checks Foundry Webhook Driven Agent Invocation against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/vcs-orchestrator-end-to-end.md | Cross-checks Foundry VCS Orchestrator End to End against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/agent-pipeline-isolation-worktree.md | Cross-checks Foundry Agent Pipeline Isolation Worktree against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/admission-gate-policy-and-evidence.md | Cross-checks Foundry Admission Gate Policy and Evidence against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/completion-gate-reviewer-and-ci.md | Cross-checks Foundry Completion Gate Reviewer and CI against Foundry Webhook Driven Agent Invocation. |
| Peer spec | microservices/intelligence/spec/agent-types-and-roles.md | Cross-checks Foundry Agent Types and Roles against Foundry Webhook Driven Agent Invocation. |
