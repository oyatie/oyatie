...
| B-004 | Principal::"oyatie.agent.codex" | Action::"foundry.queue.check_path_overlap" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=merge-queue-projected-state | permit when evidence_hash and changeset_id are present |
...
| B-016 | Principal::"oyatie.agent.executor" | Action::"foundry.queue.enqueue" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=merge-queue-projected-state | permit when evidence_hash and changeset_id are present |
...
| B-028 | Principal::"oyatie.service.webhook-receiver" | Action::"foundry.queue.check_conflict" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=merge-queue-projected-state | permit when evidence_hash and changeset_id are present |
...
| B-040 | Principal::"oyatie.service.admission-gate" | Action::"foundry.queue.revalidate" | Resource::"repo:oyatie/microservices/intelligence" | workflow=governance_pipeline; tenant_id=oyatie; intent=merge-queue-projected-state | permit when evidence_hash and changeset_id are present |
...
- Guard 05: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.queue.revalidate on Resource::"repo:oyatie/microservices/intelligence" only while `transient_ref_created` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 17: Principal::"oyatie.service.merge-queue" may invoke foundry.queue.enqueue on Resource::"repo:oyatie/microservices/intelligence" only while `admitted` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 29: Principal::"oyatie.service.completion-gate" may invoke foundry.queue.revalidate on Resource::"repo:oyatie/microservices/intelligence" only while `parked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 41: Principal::"oyatie.agent.codex" may invoke foundry.queue.enqueue on Resource::"repo:oyatie/microservices/intelligence" only while `queue_received` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 53: Principal::"oyatie.agent.planner" may invoke foundry.queue.revalidate on Resource::"repo:oyatie/microservices/intelligence" only while `merge_tree_checked` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
- Guard 65: Principal::"oyatie.service.vcs-orchestrator" may invoke foundry.queue.enqueue on Resource::"repo:oyatie/microservices/intelligence" only while `transient_ref_created` is current, the changeset id is stable, the event is signed, and the ADR-0111 invariant is cited.
...
| merge-queue-projected-state-matrix-03 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope merge_tree_checked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves merge_tree_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-04 | cargo test -p oya-vcs-review-mergequeue-kernel --scope path_overlap_checked --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves path_overlap_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-05 | cargo test -p oya-vcs-webhook-receiver-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-06 | cargo test -p oya-vcs-admission-gate-kernel --scope projected_ci_running --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves projected_ci_running cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-07 | cargo test -p oya-vcs-changebundle-kernel --scope admitted --adr ADR-0111 | oya-vcs-changebundle-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-08 | cargo test -p oya-vcs-promotion-controller-kernel --scope repositioned --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves repositioned cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-09 | cargo test -p oya-vcs-cli-ratchet-kernel --scope parked --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-15 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-16 | cargo test -p oya-vcs-review-mergequeue-kernel --scope projected_ci_running --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves projected_ci_running cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-17 | cargo test -p oya-vcs-webhook-receiver-kernel --scope admitted --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-18 | cargo test -p oya-vcs-admission-gate-kernel --scope repositioned --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves repositioned cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-19 | cargo test -p oya-vcs-changebundle-kernel --scope parked --adr ADR-0111 | oya-vcs-changebundle-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-20 | cargo test -p oya-vcs-promotion-controller-kernel --scope refused --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves refused cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-21 | cargo test -p oya-vcs-cli-ratchet-kernel --scope queue_received --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves queue_received cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-27 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope admitted --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-28 | cargo test -p oya-vcs-review-mergequeue-kernel --scope repositioned --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves repositioned cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-29 | cargo test -p oya-vcs-webhook-receiver-kernel --scope parked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-30 | cargo test -p oya-vcs-admission-gate-kernel --scope refused --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves refused cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-31 | cargo test -p oya-vcs-changebundle-kernel --scope queue_received --adr ADR-0111 | oya-vcs-changebundle-kernel | proves queue_received cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-32 | cargo test -p oya-vcs-promotion-controller-kernel --scope projected_base_computed --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves projected_base_computed cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-33 | cargo test -p oya-vcs-cli-ratchet-kernel --scope merge_tree_checked --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves merge_tree_checked cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-39 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope parked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-40 | cargo test -p oya-vcs-review-mergequeue-kernel --scope refused --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves refused cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-41 | cargo test -p oya-vcs-webhook-receiver-kernel --scope queue_received --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves queue_received cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-42 | cargo test -p oya-vcs-admission-gate-kernel --scope projected_base_computed --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves projected_base_computed cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-43 | cargo test -p oya-vcs-changebundle-kernel --scope merge_tree_checked --adr ADR-0111 | oya-vcs-changebundle-kernel | proves merge_tree_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-44 | cargo test -p oya-vcs-promotion-controller-kernel --scope path_overlap_checked --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves path_overlap_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-45 | cargo test -p oya-vcs-cli-ratchet-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-51 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope queue_received --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves queue_received cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-52 | cargo test -p oya-vcs-review-mergequeue-kernel --scope projected_base_computed --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves projected_base_computed cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-53 | cargo test -p oya-vcs-webhook-receiver-kernel --scope merge_tree_checked --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves merge_tree_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-54 | cargo test -p oya-vcs-admission-gate-kernel --scope path_overlap_checked --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves path_overlap_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-55 | cargo test -p oya-vcs-changebundle-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-changebundle-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-56 | cargo test -p oya-vcs-promotion-controller-kernel --scope projected_ci_running --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves projected_ci_running cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-57 | cargo test -p oya-vcs-cli-ratchet-kernel --scope admitted --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-63 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope merge_tree_checked --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves merge_tree_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-64 | cargo test -p oya-vcs-review-mergequeue-kernel --scope path_overlap_checked --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves path_overlap_checked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-65 | cargo test -p oya-vcs-webhook-receiver-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-66 | cargo test -p oya-vcs-admission-gate-kernel --scope projected_ci_running --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves projected_ci_running cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-67 | cargo test -p oya-vcs-changebundle-kernel --scope admitted --adr ADR-0111 | oya-vcs-changebundle-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-68 | cargo test -p oya-vcs-promotion-controller-kernel --scope repositioned --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves repositioned cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-69 | cargo test -p oya-vcs-cli-ratchet-kernel --scope parked --adr ADR-0111 | oya-vcs-cli-ratchet-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
...
| merge-queue-projected-state-matrix-75 | cargo test -p oya-vcs-merge-queue-conflict-kernel --scope transient_ref_created --adr ADR-0111 | oya-vcs-merge-queue-conflict-kernel | proves transient_ref_created cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-76 | cargo test -p oya-vcs-review-mergequeue-kernel --scope projected_ci_running --adr ADR-0111 | oya-vcs-review-mergequeue-kernel | proves projected_ci_running cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-77 | cargo test -p oya-vcs-webhook-receiver-kernel --scope admitted --adr ADR-0111 | oya-vcs-webhook-receiver-kernel | proves admitted cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-78 | cargo test -p oya-vcs-admission-gate-kernel --scope repositioned --adr ADR-0111 | oya-vcs-admission-gate-kernel | proves repositioned cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-79 | cargo test -p oya-vcs-changebundle-kernel --scope parked --adr ADR-0111 | oya-vcs-changebundle-kernel | proves parked cannot advance without policy, evidence, trace, and audit correlation |
| merge-queue-projected-state-matrix-80 | cargo test -p oya-vcs-promotion-controller-kernel --scope refused --adr ADR-0111 | oya-vcs-promotion-controller-kernel | proves refused cannot advance without policy, evidence, trace, and audit correlation |
...
| Peer spec | microservices/intelligence/spec/changeset-state-machine.md | Cross-checks Foundry Changeset State Machine against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/merge-queue-projected-state.md | Cross-checks Foundry Merge Queue Projected State against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/webhook-driven-agent-invocation.md | Cross-checks Foundry Webhook Driven Agent Invocation against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/vcs-orchestrator-end-to-end.md | Cross-checks Foundry VCS Orchestrator End to End against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/agent-pipeline-isolation-worktree.md | Cross-checks Foundry Agent Pipeline Isolation Worktree against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/admission-gate-policy-and-evidence.md | Cross-checks Foundry Admission Gate Policy and Evidence against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/completion-gate-reviewer-and-ci.md | Cross-checks Foundry Completion Gate Reviewer and CI against Foundry Merge Queue Projected State. |
| Peer spec | microservices/intelligence/spec/agent-types-and-roles.md | Cross-checks Foundry Agent Types and Roles against Foundry Merge Queue Projected State. |
