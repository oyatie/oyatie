//! M02-P01 — anthropic api-auth adapter.
...
use oya_intelligence_adapter_anthropic_api_kernel::{
...
impl AnthropicApiAdapter {
    ...
    fn synthesize_token_id(&self, sref: &SecretReference) -> String {
        ...
        let dbg = format!("{sref:?}");
        format!("mock-anthropic-api-{}", dbg.len())
    }
}

impl ProviderAuthPort for AnthropicApiAdapter {
...
mod tests {
    ...
    use oya_intelligence_adapter_anthropic_api_kernel::{AUTH_MODE, AuthMode};
    ...
    fn adapter_clock_override_respected() {
        let a = AnthropicApiAdapter::new().with_clock(5_000, 60);
