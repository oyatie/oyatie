---
doc_class: Runbook
doc_id: RUNBOOK-OBSERVABILITY-COLD_MERGE
...
# Runbook: Observability Cold Merge

## Operator Contract
- Runbook id: observability-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-observability-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=observability; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n observability rollout status deploy/observability --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service observability --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service observability --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service observability --cell $CELL --window 30m`.
