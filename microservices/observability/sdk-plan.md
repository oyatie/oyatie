...
OpenSLO manifest authoring is via git PR (CODEOWNERS + branch-protection enforced). The SDKs do NOT expose a "create/update manifest" method directly — that path is via the tenant operator's git workflow, validated at PR time. If future tenant feedback surfaces a need for programmatic manifest authoring, that's a per-tenant DPA-recorded entitlement granted via Cedar.
