...
companion_docs:
  ...
  - microservices/observability/README.md
...
`observability` keeps its existing bounded context and flat `microservices/observability/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `observability`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `observability` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oci-guest/clickhouse-iceberg-layer@v1[paid], on-prem/telemetry-backbone@v1[paid], colo/object-storage-versioned@v1[paid], oyatie-as-cloud-provider/service-mesh-waypoint@v1[paid].
...
- Capacity: `per_query` drives CPU `0.26`, RAM `768`, storage `20`, and connection pool variables.
