...
## §principals
This anchor is closed for `consent-graph` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.consent-graph.runtime` owns normal `consent-grant` execution and never borrows a tenant principal.
- Platform principal `oyatie.consent-graph.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/consent-graph/backfill-replay.md` when present.
- Platform principal `oyatie.consent-graph.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.consent-graph.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.consent-graph.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `consent-grant` is evaluated as `<tenant>.consent-graph.consent-grant` before any `consent-graph` state mutation.
...
## §cedar-gates
This anchor is closed for `consent-graph` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/consent-graph/policy/aggregate-k-anonymity.cedar`, `microservices/consent-graph/policy/break-glass-healthcare.cedar`, `microservices/consent-graph/policy/cross-tenant-projection.cedar`, `microservices/consent-graph/policy/deny-all-fallback.cedar`.
- Action namespace uses `consent-graph::<bounded_context>::<verb>`; the first protected action is `consent-graph::consent-grant::execute`.
...
- Concrete fragments in scope: `microservices/consent-graph/policy/aggregate-k-anonymity.cedar`, `microservices/consent-graph/policy/break-glass-healthcare.cedar`, `microservices/consent-graph/policy/cross-tenant-projection.cedar`, `microservices/consent-graph/policy/deny-all-fallback.cedar`.
- Example: `consent-graph::consent-grant::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `consent-graph` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `consent_graph.agreement` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `consent_graph.enforcement` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `consent_graph.revocation` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `consent_graph.projection_gateway` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `consent_graph.audit_bridge` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
- Example: `consent-grant` reads include `tenant_id` and `home_cell`; a stale `jurisdiction_code` forces most-restrictive-pack handling until tenancy refresh completes.
...
This anchor is closed for `consent-graph` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `consent-graph` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `consent-graph` only through contracts `microservices/consent-graph/contracts/asyncapi/consent-events.yaml`, `microservices/consent-graph/contracts/openapi/consent-graph.yaml`, `microservices/consent-graph/contracts/proto/consent-graph.proto`.
- If product: `consent-graph` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `agreement`, `enforcement`, `revocation`, `projection-gateway`, `audit-bridge`, `partner-directory`.
- Example: `consent-grant` may depend on `tenancy` for tenant state and `observability` for audit emission, but not on another product UI workflow.
...
## §policy-evaluation
This anchor is closed for `consent-graph` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/consent-graph/policy/aggregate-k-anonymity.cedar`, `microservices/consent-graph/policy/break-glass-healthcare.cedar`, `microservices/consent-graph/policy/cross-tenant-projection.cedar`, `microservices/consent-graph/policy/deny-all-fallback.cedar` with the full action/resource/context tuple.
...
- Example: `consent-graph::consent-grant::execute` evaluates locally, then emits an audit event before the usecase writes `consent_graph.agreement`.
...
## §cell-eligibility
This anchor is closed for `consent-graph` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
- Example: `consent-grant` traffic in a KR pack tenant stays in KR home cell; DR failover requires pack approval and emits a cell-failover audit event.
...
## §transport
This anchor is closed for `consent-graph` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/consent-graph/contracts/asyncapi/consent-events.yaml`, `microservices/consent-graph/contracts/openapi/consent-graph.yaml`, `microservices/consent-graph/contracts/proto/consent-graph.proto`.
...
- IaC transport evidence: `microservices/consent-graph/iac/helm/consent-graph/Chart.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/consent-graph-app-deployment.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/enforcement-app-deployment.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/hg-consent-registration.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/rbac.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/revocation-and-workers.yaml`; +6 more.
- Example: `consent-graph` `consent-grant` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `consent-graph` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `agreement`, `enforcement`, `revocation`, `projection-gateway`, `audit-bridge`, `partner-directory` and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/consent-graph/iac/helm/consent-graph/Chart.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/consent-graph-app-deployment.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/enforcement-app-deployment.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/hg-consent-registration.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/rbac.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/revocation-and-workers.yaml`; +6 more.
- Catalog crates/components: `microservices/consent-graph/catalog/oya-consent-graph-agreement-adapter.yaml`, `microservices/consent-graph/catalog/oya-consent-graph-agreement-domain.yaml`, `microservices/consent-graph/catalog/oya-consent-graph-agreement-kernel.yaml`, `microservices/consent-graph/catalog/oya-consent-graph-agreement-rest.yaml`, `microservices/consent-graph/catalog/oya-consent-graph-agreement-sdk.yaml`, `microservices/consent-graph/catalog/oya-consent-graph-agreement-usecase.yaml`; +18 more.
- Example: `consent-grant` API runs as a Kata-isolated container; scheduled/background work for `consent-graph` uses a separate worker principal.
...
## §observability
This anchor is closed for `consent-graph` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/consent-graph/slos/agreement-state-divergence-zero.openslo.yaml`, `microservices/consent-graph/slos/audit-chain-coverage-completeness.openslo.yaml`, `microservices/consent-graph/slos/bilateral-chain-link-integrity.openslo.yaml`, `microservices/consent-graph/slos/cedar-evaluation-latency.openslo.yaml`, `microservices/consent-graph/slos/consent-grant-latency.openslo.yaml`, `microservices/consent-graph/slos/cross-tenant-projection-freshness.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.consent.graph.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `consent_graph_consent_grant_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `consent_graph_consent_grant_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/consent-graph/slos/agreement-state-divergence-zero.openslo.yaml`.
...
- Example trace: `consent-graph.consent-grant` -> `policy.evaluate` -> `storage.consent_grant` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `consent-graph` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `consent-graph` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `consent-grant` action class.
...
- Cedar/IaC controls in scope: `microservices/consent-graph/policy/aggregate-k-anonymity.cedar`, `microservices/consent-graph/policy/break-glass-healthcare.cedar`, `microservices/consent-graph/policy/cross-tenant-projection.cedar`, `microservices/consent-graph/policy/deny-all-fallback.cedar`, `microservices/consent-graph/iac/helm/consent-graph/Chart.yaml`, `microservices/consent-graph/iac/helm/consent-graph/templates/consent-graph-app-deployment.yaml`; +10 more.
- Canary payloads for `consent-graph` use fake `consent-grant` identifiers and honey endpoints under `/.well-known/oya-canary/consent-graph`.
...
This anchor is closed for `consent-graph` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `consent-graph` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `consent-graph` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `consent-graph` audit evidence keep legitimate recovery from becoming an adversary shortcut.
- Mistaken mutation path: high-impact `consent-grant` mutations require idempotency, undo/cooldown where product semantics allow, and sealed evidence for later correction.
- Disaster surge: `consent-graph` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `consent-graph` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `consent-graph` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/consent-graph/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/consent-graph/runbooks/GDPR-DSAR-cross-tenant.md`, `microservices/consent-graph/runbooks/audit-chain-divergence-recovery.md`, `microservices/consent-graph/runbooks/consent-forgery-detected.md`, `microservices/consent-graph/runbooks/data-residency-enforcement.md`, `microservices/consent-graph/runbooks/partner-offboarding.md`, `microservices/consent-graph/runbooks/partner-onboarding.md`; +2 more.
- Example: `consent-grant` asks the sidecar for a scoped credential after Cedar allow; the sidecar returns a short-lived handle, not the underlying secret.
