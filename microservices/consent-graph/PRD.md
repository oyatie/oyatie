...
## ADR-0339 adoption
- Lifecycle: PROPOSED for `consent-graph` until service wrappers invoke signed shared OpenTofu modules and implementation evidence lands.
- ADR-0339 adoption keeps reusable HCL in `microservices/cloud-iac/modules/<context>/<primitive>/`; `consent-graph` owns primitive selection and tenant-scoped variables.
