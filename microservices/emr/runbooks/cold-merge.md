...
# Runbook: Emr Cold Merge

## Operator Contract
- Runbook id: emr-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-emr-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=emr; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n emr rollout status deploy/emr --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service emr --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service emr --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service emr --cell $CELL --window 30m`.
