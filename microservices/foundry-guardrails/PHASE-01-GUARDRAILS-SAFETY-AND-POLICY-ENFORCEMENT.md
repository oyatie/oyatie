...
- Nothing deferred (no FUTURE-marked stub; M01 ships all 6 BCs with at least one canonical implementation each).
...
Naming justifications are in `microservices/foundry-guardrails/PRD.md` §"Bounded Contexts".
...
- `foundry-providers` LLM-judge low-cost endpoint (Open Question 1; resolved in follow-up ADR).
- Tenant-facing Workflow Studio policy-author UX (deferred to Workflow Studio's own roadmap; this phase ships the engine + Cedar fragment authoring via git PR).
...
Every IP in this phase emits a ChangeSet per ADR-0110 (claimable + verifiable + bundleable + promotable). The minimum ChangeSet payload per IP, written at `microservices/foundry-guardrails/evidence/multispectrum/<change_id>-<unix_ts>.json`:
...
```json
{
  ...
  "milestone": "M01-foundation",
  "phase": "P01-guardrails-safety-and-policy-enforcement",
  "claim_paths": ["microservices/foundry-guardrails/src/crates/<crate>/**", "..."],
  "intent": "<one-line>",
  "spec_refs": ["microservices/foundry-guardrails/PRD.md§<section>", "ADR-0140§Cedar"],
...
cargo run -p oya-dev-cli -- vcs claim \
  ...
  --paths "microservices/foundry-guardrails/src/crates/<crate>/**"
...
Multispectrum evidence per docs/AGENTS.md §changeset: each IP emits `microservices/foundry-guardrails/evidence/multispectrum/<change_id>-<unix_ts>.json` per `/specs/multispectrum-review.json` v2.4.0; F13_safety facet added for this µservice's safety-bearing posture.
