...
Primary SLO: postmortem-seal-completeness  
Domain focus: war-room, escalation, page, acknowledge

## Trigger
- Alert `incident-management-postmortem-seal-completeness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_incident_management_postmortem_seal_completeness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when postmortem-seal-completeness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `incident-management-local-postmortem-seal-completeness` and policy file name to the ticket.
