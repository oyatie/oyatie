# Incident Management Runbook: Paging Storm Throttle

Service: incident-management  
Surface: local operational primitive suite  
Primary SLO: page-to-acknowledge  
Domain focus: war-room, escalation, page, acknowledge

## Trigger
- Alert `incident-management-page-to-acknowledge` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_incident_management_page_to_acknowledge_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when page-to-acknowledge is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `incident-management-local-page-to-acknowledge` and policy file name to the ticket.
