...
## B. Approach
Implement `ontology-projection` as a service-local slice, not as a shared platform facility. The technical mechanism is mapping incident resources to shared ontology terms bound to `ontology projection port` and checked before user-visible promotion.
