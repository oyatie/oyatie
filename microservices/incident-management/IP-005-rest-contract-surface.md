# IP-005 Incident Management REST contract surface

Service: incident-management
ChangeSet scope: microservices/incident-management/IP-005-rest-contract-surface.md
...
## A. Problem
This IP closes the REST contract surface gap for `incident-management`. The previous 55-line shell repeated the same objective/prerequisite/test/rollback labels without proving how `rest-contract-surface` works for paging, escalation, incident rooms, stakeholder updates, and postmortems are time-critical and must keep evidence when systems are degraded.
...
## B. Approach
Implement `rest-contract-surface` as a service-local slice, not as a shared platform facility. The technical mechanism is HTTP command/query API bound to `adapter/http.rs` and checked before user-visible promotion.
...
The approach explicitly covers idempotency, pagination, deny errors, status update approval, import/export; those are the failure modes that a stamped IP did not name.

## C. Deliverables
- D01: `microservices/incident-management/PRD.md` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D02: `microservices/incident-management/ARCHITECTURE.md` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D03: `microservices/incident-management/manifest.json` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D04: `microservices/incident-management/competitor-parity-matrix.md` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D05: `microservices/incident-management/feature-parity-matrix-2026-05-20.md` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D06: `microservices/incident-management/contracts/openapi-v1.yaml` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D07: `microservices/incident-management/contracts/asyncapi-v1.yaml` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D08: `microservices/incident-management/contracts/incident-management-v1.proto` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D09: `microservices/incident-management/src/domain/mod.rs` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D10: `microservices/incident-management/src/usecase/mod.rs` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D11: `microservices/incident-management/src/adapter/http.rs` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D12: `microservices/incident-management/src/adapter/asyncapi.rs` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D13: `microservices/incident-management/policies/local-page-dispatch-guard.cedar` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D14: `microservices/incident-management/policies/local-escalation-policy-control.cedar` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D15: `microservices/incident-management/policies/local-war-room-open-approval.cedar` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D16: `microservices/incident-management/policy/sre-incident-command-authorization.cedar` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D17: `microservices/incident-management/capabilities/page-dispatch.yaml` — modify or bind for `rest-contract-surface` evidence and contract traceability.
- D18: `microservices/incident-management/capabilities/escalation-evaluate.yaml` — modify or bind for `rest-contract-surface` evidence and contract traceability.
...
2. Add or update the `rest-contract-surface` contract shape in `contracts/openapi-v1.yaml` and keep request/event/proto field names aligned with ADR-0105 layer naming.
...
5. Bind adapter behavior through `adapter/http.rs` and `microservices/incident-management/src/adapter/mod.rs`, using typed errors from `microservices/incident-management/src/error.rs` instead of stringly provider failures.
...
8. Update dashboards/SLOs or the service operating bar to expose success, refusal, replay, latency, and audit completeness for `rest-contract-surface`.
...
## E. Acceptance
- A reviewer can trace `rest-contract-surface` from this IP to at least one real contract, one real policy file, one real source file, and one capability/catalog artifact.
...
- The observability evidence includes metric, trace, structured log, audit event id, policy decision id, and low-cardinality labels for `rest-contract-surface`.
...
|---|---|---|
| PagerDuty | Provides a native `REST contract surface` experience inside its product boundary. | Oyatie keeps the same operator fluency while binding `rest-contract-surface` to tenant scope, Cedar deny evidence, audit-chain records, and flat `incident-management` ownership. |
...
- Do not move `rest-contract-surface` into a sibling service such as `observability` or `messenger` unless a later ADR changes ownership.
