...
1. Treat `oya verify --ci-required` as the local rehearsal for `cell-rebalancer` changes before push.
...
3. When `cell-rebalancer` changes touch Rust, contracts, manifests, generated docs, or governance lanes, run the verifier before handoff.
...
1. Search `cell-rebalancer` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `cell-rebalancer` manifest `fitness_lanes` array only in the manifest-owning slot.
...
1. Treat Jenkins LTS as the self-hostable CI substrate for `cell-rebalancer` when GitHub Actions runners are unavailable.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `cell-rebalancer`.
