---
doc_class: Runbook
doc_id: RUNBOOK-HEALTHCARE_INTEGRATION-AUTO_REBALANCE
...
# Runbook: Healthcare Integration Auto Rebalance

## Operator Contract
- Runbook id: healthcare-integration-auto-rebalance.
- Primary service namespace: `healthcare-integration`.
...
- Owning team: axis-healthcare-integration + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-healthcare-integration-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=healthcare-integration; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n healthcare-integration rollout status deploy/healthcare-integration --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service healthcare-integration --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service healthcare-integration --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service healthcare-integration --cell $CELL --window 30m`.
