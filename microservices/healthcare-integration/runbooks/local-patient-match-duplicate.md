...
Primary SLO: fhir-bundle-success  
Domain focus: hl7, fhir, hipaa, phi

## Trigger
- Alert `healthcare-integration-fhir-bundle-success` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_healthcare_integration_fhir_bundle_success_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when fhir-bundle-success is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `healthcare-integration-local-fhir-bundle-success` and policy file name to the ticket.
