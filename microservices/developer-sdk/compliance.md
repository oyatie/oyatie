...
This anchor is closed for `developer-sdk` against ADR-0250 §D-1: certification-ready evidence roster and audit scope.

### Service-specific answer
- Certification scope for `developer-sdk` covers packs `kr`, `eu`, `us`, `us-healthcare`, `us-financial`, `us-public-sector`.
...
## §pack-overlay-roster
This anchor is closed for `developer-sdk` against ADR-0251 §D-2: pack activation, overlays and per-pack Cedar deltas.

### Service-specific answer
- Active/expected pack roster: `kr`, `eu`, `us`, `us-healthcare`, `us-financial`, `us-public-sector`.
- Pack overlays modify Cedar fragments `microservices/developer-sdk/policy/admin-scope.cedar`, `microservices/developer-sdk/policy/developer-scope.cedar`, `microservices/developer-sdk/policy/payout-scope.cedar`, `microservices/developer-sdk/policy/public-read.cedar` without changing domain code.
...
- Example: `developer-onboard` under KR pack uses KR cell routing, KR breach-notification timers, and pack-local audit retention.
...
This anchor is closed for `developer-sdk` against ADR-0284 §D-1: platform_owner indirection and hard-coded brand-string audit.

### Service-specific answer
- Runtime platform-owner string is configured as `platform_owner.display_name`; `developer-sdk` does not hard-code user-visible owner names in API or UI output.
...
- Example: `developer-onboard` error text says `platform owner` or config-resolved name, while audit event principal remains `oyatie.developer-sdk.runtime`.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.6.A: detection-event categories and routing topology.

### Service-specific answer
- `developer-sdk` emits detection signals through ADR-0263 audit pipeline, not an ungoverned side channel.
- Detection families applicable here: policy violation, insider risk, account-takeover, content/transaction abuse where `developer-onboard` touches those data classes.
- Signal sources: `microservices/developer-sdk/policy/admin-scope.cedar`, `microservices/developer-sdk/policy/developer-scope.cedar`, `microservices/developer-sdk/policy/payout-scope.cedar`, `microservices/developer-sdk/policy/public-read.cedar`, `microservices/developer-sdk/dashboards/onboarding-funnel.json`, `microservices/developer-sdk/dashboards/payout-health.json`; +10 more.
- Example event class: `oya.developer.sdk.developer.onboard.risk_signal_emitted` with risk score, reason code, and tenant-safe dimensions.
...
This anchor is closed for `developer-sdk` against ADR-0310 §D-1: detection-to-investigation evidence handoff and case binding.

### Service-specific answer
- Investigation handoff starts from a signed detection event emitted by `developer-sdk` and ends in a case record with immutable evidence pointers.
- Cedar permit `oyatie.developer-sdk.investigation.open` gates who may create, read, export, or close a case.
- Evidence pack includes audit event id, policy decision hash, affected `developer-onboard` resource ids, tenant id, data classes, and SLO/degraded-mode context.
...
- Example: a suspicious `developer-onboard` mutation opens a case only after risk threshold and Cedar permit both pass; low-confidence signals remain queued for aggregation.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 8: privileged access, break-glass and UEBA controls.

### Service-specific answer
- Operators of `developer-sdk` have no standing unredacted tenant-data access; JIT elevation uses identity step-up and Cedar approval.
- Break-glass access requires reason, scope, expiry, reviewer where required, and post-hoc audit review.
- Sensitive surfaces: `microservices/developer-sdk/contracts/asyncapi/developer-sdk-events.yaml`, `microservices/developer-sdk/contracts/openapi/developer-sdk.yaml`, `microservices/developer-sdk/contracts/openapi/oya-ecosystem.yaml`, `microservices/developer-sdk/contracts/proto/developer-sdk.proto`, `developer_sdk.developer_onboarding`, `developer_sdk.signing_key_issuance`; +6 more.
...
- Example: reading `developer_sdk.developer_onboarding` outside declared incident purpose creates a high-risk insider signal and routes to investigation.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 9: threat feed sources, freshness and degraded-mode policy.

### Service-specific answer
- `developer-sdk` consumes central threat intelligence for IP/domain reputation, credential stuffing, bot fingerprints, sanctions/abuse lists where applicable, and malicious package indicators.
...
- Enforcement points: `microservices/developer-sdk/policy/admin-scope.cedar`, `microservices/developer-sdk/policy/developer-scope.cedar`, `microservices/developer-sdk/policy/payout-scope.cedar`, `microservices/developer-sdk/policy/public-read.cedar`, `microservices/developer-sdk/iac/helm/backstage/Chart.yaml`, `microservices/developer-sdk/iac/helm/backstage/values.yaml`; +10 more.
- Example: `developer-onboard` with malicious IP reputation receives stricter quota/challenge; high-risk legal/financial flows can halt pending investigation.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 16: signing, encryption, ECH and PQC key rotation cadence.

### Service-specific answer
- Signing keys for `oya.developer-sdk` audit events rotate at ≤90 days or immediately on suspected compromise.
...
- Secret paths use `${openbao:secret/<tenant_id>/developer-sdk/<key-class>}` and never embed raw tenant ids in metrics labels.
- Runbook evidence: `microservices/developer-sdk/runbooks/codegen-pipeline-non-deterministic.md`, `microservices/developer-sdk/runbooks/dev-portal-down.md`, `microservices/developer-sdk/runbooks/developer-revocation-cascade-stuck.md`, `microservices/developer-sdk/runbooks/kyc-pipeline-stuck.md`, `microservices/developer-sdk/runbooks/payout-settlement-mismatch.md`, `microservices/developer-sdk/runbooks/sandbox-provision-slow.md`; +2 more.
- Example: `developer-onboard` credential rotation drains in-flight requests with old key id, validates new key id, then retires old leases after audit-chain seal.
...
## §crypto-agility-plan
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 20: algorithm roster, deprecation triggers and migration windows.

### Service-specific answer
- `developer-sdk` uses algorithm policy from sidecar/config; domain code never hard-codes cipher or signature choices.
...
- Affected surfaces: `microservices/developer-sdk/contracts/asyncapi/developer-sdk-events.yaml`, `microservices/developer-sdk/contracts/openapi/developer-sdk.yaml`, `microservices/developer-sdk/contracts/openapi/oya-ecosystem.yaml`, `microservices/developer-sdk/contracts/proto/developer-sdk.proto`, `microservices/developer-sdk/iac/helm/backstage/Chart.yaml`, `microservices/developer-sdk/iac/helm/backstage/values.yaml`; +10 more.
...
- Example: `developer-onboard` accepts classical TLS during PQC migration but prefers hybrid when both peers support it and records negotiated group in telemetry.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 12: pentest scope, bounty intake and remediation SLO.

### Service-specific answer
- `developer-sdk` is in annual full-scope pentest and every major `developer-onboard` launch adds targeted test scope before production promotion.
- In-scope assets: `microservices/developer-sdk/contracts/asyncapi/developer-sdk-events.yaml`, `microservices/developer-sdk/contracts/openapi/developer-sdk.yaml`, `microservices/developer-sdk/contracts/openapi/oya-ecosystem.yaml`, `microservices/developer-sdk/contracts/proto/developer-sdk.proto`, `microservices/developer-sdk/iac/helm/backstage/Chart.yaml`, `microservices/developer-sdk/iac/helm/backstage/values.yaml`; +14 more.
...
- Example: a researcher bypassing `developer-sdk` tenant scoping gets safe-harbor handling and an investigation case, not abuse-defence friction by default.
...
## §facility-controls
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 13: inherited data-center, cell and physical-access controls.

### Service-specific answer
- `developer-sdk` inherits facility controls from `cell`, `cloud-iac`, and the active provider cell; no direct human facility access is owned by this µservice.
...
- Example: `developer-onboard` in a regulated cell can only schedule onto node pools with matching facility attestation and residency tag.
...
- Example: `developer-onboard` image promotion requires cosign signature, SLSA provenance, vulnerability scan, license check, and matching manifest/catalog record.
...
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.5: applicable safety/security/policy edge cases and fallbacks.

### Service-specific answer
- Applicable rows for `developer-sdk` include account recovery, mistaken mutation, regional outage, regulator deadline, audit access, and delegated-agent authority where relevant.
- Safety invariant: `developer-onboard` never creates human harm through unnecessary friction, lost recovery path, or silent data loss.
...
- Example: `developer-onboard` during regional outage preserves audit evidence locally and blocks cross-border transfer if pack policy forbids DR failover.
...
## §data-classification
This anchor is closed for `developer-sdk` against documentation-rigor.md §3.2.4 Domain 14: data classes, retention, encryption and transfer restrictions.

### Service-specific answer
- Data classes processed: `INTERNAL_ONLY`, `AUDIT`, `PII_QUASI`.
- State/event surfaces carrying classification: `developer_sdk.developer_onboarding`, `developer_sdk.signing_key_issuance`, `developer_sdk.api_contracts_registry`, `developer_sdk.sdk_codegen`, `developer_sdk.sandbox_provisioner`, `developer_sdk.dev_portal`; +2 more.
...
- Example: `developer-onboard` labels identifiers as `PII_IDENTIFYING`, operational logs as `AUDIT`, and aggregate metrics as `INTERNAL_ONLY` unless manifest narrows them.
