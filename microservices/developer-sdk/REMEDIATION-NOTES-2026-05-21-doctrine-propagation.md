...
- Value: 0.3 vCPU, 768 MiB RAM, 5 GB storage, Valkey/PostgreSQL/outbound connections 2/3/10, scaling_dimension=per_workflow_run, cell_placement_class=Tier-2.
- ADR: ADR-0340 capacity declaration and ADR-0248 cell criticality numbering.
- Why: 0.3 vCPU and 768 MiB per tenant reserve headroom for codegen, sandbox provisioning, payout, and OpenBao-backed signing workflows.
...
### Block 2: dr
- Value: RTO 900s, RPO 60s, active-active multi-region=true, backup_substrate=postgres_wal_g, object_storage_versioned, openbao_seal_unseal, audit_chain_merkle_seal, valkey, failover_runbook=runbooks/dr-failover.md, dr_tier=T1.
- ADR: ADR-0343 DR matrix and compliance-pack floor overlay.
- Why: 900s RTO and 60s RPO are tighter than HIPAA floor because signing keys and payout workflow checkpoints are control-plane records.
...
- Why: Developer SDK is a first-party ecosystem control-plane product. It issues signing keys and provisions sandbox tenants through Oyatie-owned workflows, while plugin runtime execution is explicitly owned by plugin-app-store, so Tier 2 runtime with Tier-2 capability cell placement is the correct co-variant declaration.
...
- Why: Developer SDK exposes public codegen, portal, signing, and sandbox APIs; tenant pinning is mandatory for SDK consumers.
...
- Value: cedar, postgresql, valkey, opentelemetry, opentofu, openbao, cosign; oss_stewardship_class_overrides=[].
- ADR: ADR-0345 OSS stewardship registry and class vocabulary.
- Why: OpenBao and Cosign are direct dependencies for signing-key and artifact trust workflows; registry defaults remain authoritative.
