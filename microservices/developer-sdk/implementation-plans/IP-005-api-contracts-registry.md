...
impl_plan_id: IP-005-api-contracts-registry
status: pending
...
# IP-005-api-contracts-registry: OpenAPI 3.2 + AsyncAPI 3.1 + proto3 canonical contracts registry

## Intent

Authoritative API contracts registry: every oyatie public API consumed by external developers has an OpenAPI 3.2 spec; every async event has an AsyncAPI 3.1 spec; every gRPC service has a proto3 spec. Version-locked + git-managed; consumed by codegen.
...
New / modified crates:
- `oya-developer-sdk-api-contracts-registry-kernel`
- `oya-developer-sdk-api-contracts-registry-domain`
- `oya-developer-sdk-api-contracts-registry-usecase`
- `oya-developer-sdk-api-contracts-registry-adapter`
...
|---|---|---|
| `microservices/developer-sdk/contracts/openapi/oya-ecosystem.yaml` | create | canonical OpenAPI 3.2 aggregated spec |
| `microservices/developer-sdk/contracts/asyncapi/oya-ecosystem-events.yaml` | create | AsyncAPI 3.1 event channels |
| `microservices/developer-sdk/contracts/proto/oya-ecosystem.proto` | create | proto3 service definitions |
...
```bash
cargo check -p oya-developer-sdk-api-contracts-registry-kernel --all-features
cargo build -p oya-developer-sdk-api-contracts-registry-kernel --all-features
cargo clippy -p oya-developer-sdk-api-contracts-registry-kernel --all-features -- -D warnings
cargo nextest run -p oya-developer-sdk-api-contracts-registry-kernel --all-features
cargo deny check --hide-inclusion-graph
cargo doc -p oya-developer-sdk-api-contracts-registry-kernel --no-deps
...
| Test | Verifies |
|---|---|
| `openapi-spec-validates` | Spectral lint passes |
| `asyncapi-spec-validates` | AsyncAPI Studio CLI lint passes |
| `proto-spec-validates` | buf lint passes |
| `contract-breaking-change-rejected` | Removing a path triggers breaking-change CI fail |
...
Test fixtures live under `microservices/developer-sdk/tests/fixtures/ip-005-api-contracts-registry/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.
...
- Spec lint fails.
- Breaking change merged without ADR + sunset window.
...
- `ip_id`: `IP-005-api-contracts-registry`
...
[`IP-006-sdk-codegen-ts-rust-swift-kotlin-csharp-python`](IP-006-sdk-codegen-ts-rust-swift-kotlin-csharp-python.md)
