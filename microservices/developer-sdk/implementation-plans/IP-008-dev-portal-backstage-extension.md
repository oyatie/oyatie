...
impl_plan_id: IP-008-dev-portal-backstage-extension
status: pending
...
# IP-008-dev-portal-backstage-extension: Backstage extension developer portal (API browser + try-in-sandbox)

## Intent

Backstage extension serving the developer portal: API browser using Backstage TechDocs + Spectral-rendered OpenAPI; try-in-sandbox interactive widget; per-developer dashboard.
...
New / modified crates:
- `oya-developer-sdk-dev-portal-kernel`
- `oya-developer-sdk-dev-portal-domain`
...
|---|---|---|
| `microservices/developer-sdk/iac/backstage-extension/package.json` | create | Backstage plugin manifest |
| `microservices/developer-sdk/iac/backstage-extension/src/index.ts` | create | plugin entry |
| `microservices/developer-sdk/iac/backstage-extension/src/components/ApiBrowser.tsx` | create | API browser |
| `microservices/developer-sdk/iac/backstage-extension/src/components/TryInSandbox.tsx` | create | Try-in-sandbox interactive widget |
...
```rust
export const developerSdkPlugin = createPlugin({
    id: 'oya-developer-sdk',
    routes: { root: rootRouteRef },
});
export const ApiBrowserPage = developerSdkPlugin.provide(
    createRoutableExtension({
        name: 'ApiBrowserPage',
        component: () => import('./components/ApiBrowser').then(m => m.ApiBrowser),
...
```bash
cargo check -p oya-developer-sdk-dev-portal-kernel --all-features
cargo build -p oya-developer-sdk-dev-portal-kernel --all-features
cargo clippy -p oya-developer-sdk-dev-portal-kernel --all-features -- -D warnings
cargo nextest run -p oya-developer-sdk-dev-portal-kernel --all-features
cargo deny check --hide-inclusion-graph
cargo doc -p oya-developer-sdk-dev-portal-kernel --no-deps
...
| Test | Verifies |
|---|---|
| `test_extension_loads_in_backstage` | Plugin registers without error |
| `test_api_browser_renders_openapi` | OpenAPI spec rendered |
| `test_try_in_sandbox_invokes_dev_tenant` | Request lands on dev sandbox tenant |
...
Test fixtures live under `microservices/developer-sdk/tests/fixtures/ip-008-dev-portal-backstage-extension/`. Determinism rule: every test that produces an artifact must produce byte-identical output on two consecutive runs (`diff -q` exits 0); enforced by the `deterministic-output` CI lane.
...
- Try-in-sandbox bypasses dev tenant isolation.
...
- `ip_id`: `IP-008-dev-portal-backstage-extension`
...
[`IP-009-dev-portal-app-submission-flow`](IP-009-dev-portal-app-submission-flow.md)
