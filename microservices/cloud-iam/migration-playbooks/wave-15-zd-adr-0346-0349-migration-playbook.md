...
Audience: an Oyatie operator or migration owner preparing `cloud-iam` for the Wave 15-ZD doctrine surface before implementation waves author runtime code, manifests, Jenkinsfiles, ArgoCD applications, or sharding bodies.
...
1. Search `cloud-iam` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `cloud-iam` manifest `fitness_lanes` array only in the manifest-owning slot.
...
- Local verifier transcript for `cloud-iam` once Wave 15-ZA lands.
