...
impl_plan_id: IP-001-layer-a-cdn-waf-postgres-redis-ws-gateway-iac
status: pending
...
helm lint microservices/workflow-studio/iac/helm/studio-redis
...
- `tests/iac/{studio-postgres,studio-redis,visual-canvas-rest,collab-crdt-worker,node-library-registry-rest,license-gate-cedar,studio-cdn,studio-waf}.bats` running `helm install --dry-run` + `helm test`.
