...
Author the kernel + domain layers of the paired BCs `dsl-emitter` (visual → canonical workflow_spec.v1.json) and `dsl-loader` (workflow_spec.v1.json → visual). The load(emit(visual)) == visual round-trip byte-equality is the load-bearing invariant per AC-02; property-tested over a 100-spec golden corpus.
...
| `microservices/workflow-studio/capabilities/eval/round-trip-golden-corpus.jsonl` | create | 100 golden spec corpus per FR-06 |
...
fn test_load_emit_byte_equal_over_corpus() {
    let corpus_path = "microservices/workflow-studio/capabilities/eval/round-trip-golden-corpus.jsonl";
...
cargo run -p oya-dev-cli -- gate validate workflow-spec-roundtrip --microservice workflow-studio \
  --spec-corpus microservices/workflow-studio/capabilities/eval/round-trip-golden-corpus.jsonl
```
...
- Any spec in the golden corpus fails round-trip — BLOCKER per AC-02; do not merge.
