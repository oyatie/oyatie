...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that workflow-studio
  (editor REST + collab CRDT) satisfies four hyperscaler-grade invariants drawn from
...
prometheusrule_path: microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml
---

# workflow-studio — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`workflow-studio`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-WORKFLOW_STUDIO`)
...
- **How workflow-studio satisfies it**:
  - Capability calls in `microservices/workflow-studio/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_workflow_studio_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_workflow_studio_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml` (`workflow-studio-llm-capability-circuit-open`
    + `workflow-studio-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/workflow-studio/dashboards/` (per-capability panels).
  - Threat model: `microservices/workflow-studio/threat-model.md` §"Dependency
...
- **How workflow-studio satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_workflow_studio_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml` (`workflow-studio-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/workflow-studio/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/workflow-studio/capacity-model.md` (where
...
- **How workflow-studio satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/workflow-studio/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml` (`workflow-studio-saturation-cpu-over-70pct`,
    `workflow-studio-errors-5xx-rate-spike`, `workflow-studio-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/workflow-studio/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/workflow-studio/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How workflow-studio satisfies it**:
  - Every SLO in `microservices/workflow-studio/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml` (`workflow-studio-*-fast-burn-1h-14x` +
    `workflow-studio-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/workflow-studio/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/workflow-studio/hyperscaler-conformance.md
test -f microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/workflow-studio/iac/helm/visual-canvas-rest/templates/prometheusrule.yaml || true
