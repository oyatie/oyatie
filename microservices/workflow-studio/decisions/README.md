...
Service-scoped ADRs are numbered `ADR-WS-NNNN` (four-digit, sequential within this directory). The `WS` prefix prevents collision with the repo-wide `ADR-NNNN` series and matches the convention adopted by sibling µservices migrating to ADR-0131 per-microservice flat layout.
