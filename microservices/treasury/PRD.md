...
The target is not a monolithic ERP platform; the target is SAP Treasury and Risk Management parity through a flat, tenant-scoped microservice that composes with shared Oyatie substrates.
...
## ADR-0339 adoption
- Lifecycle: PROPOSED for `treasury` until service wrappers invoke signed shared OpenTofu modules and implementation evidence lands.
- ADR-0339 adoption keeps reusable HCL in `microservices/cloud-iac/modules/<context>/<primitive>/`; `treasury` owns primitive selection and tenant-scoped variables.
