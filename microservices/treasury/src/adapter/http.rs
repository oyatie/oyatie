...
impl HttpHandler {
    pub fn routes() -> Vec<HttpRoute> {
        vec![
            HttpRoute { method: "POST", path: "/v1/treasury/cash-positions:rollup", capability: "cash-position", idempotent: true },
            HttpRoute { method: "POST", path: "/v1/treasury/bank-accounts:bind", capability: "bank-account", idempotent: true },
            HttpRoute { method: "POST", path: "/v1/treasury/fx-exposures:measure", capability: "fx-exposure", idempotent: true },
            HttpRoute { method: "POST", path: "/v1/treasury/hedge-designations:approve", capability: "hedge-designation", idempotent: true },
            HttpRoute { method: "POST", path: "/v1/treasury/payments:execute", capability: "payment-execution", idempotent: true },
        ]
    }
