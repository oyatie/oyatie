---
doc_class: Runbook
doc_id: RUNBOOK-OPS_DASHBOARD_CONTROL_CENTER-COLD_MERGE
...
# Runbook: Ops Dashboard Control Center Cold Merge

## Operator Contract
- Runbook id: ops-dashboard-control-center-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ops-dashboard-control-center-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ops-dashboard-control-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
...
3. Verify alerts: `oya observability alerts list --service ops-dashboard-control-center --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service ops-dashboard-control-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service ops-dashboard-control-center --cell $CELL --window 30m`.
