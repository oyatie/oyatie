...
ip_id: IP-012-cell-operator-panel
status: planned
...
Technical target set: cell-operator.json, cluster-health-query.yaml, cluster-health-freshness.openslo.yaml, prod-spiffe-kill-switch.yaml. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `cell health panel` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: cell-operator.json, cluster-health-query.yaml, cluster-health-freshness.openslo.yaml, prod-spiffe-kill-switch.yaml.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `cell health panel`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from cell-operator.json, cluster-health-query.yaml, cluster-health-freshness.openslo.yaml, prod-spiffe-kill-switch.yaml; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: cell-operator.json, cluster-health-query.yaml, cluster-health-freshness.openslo.yaml, prod-spiffe-kill-switch.yaml.
...
|---|---|---|
| AWS console cell/region health and Backstage service health | Mature external control surface to compare against. | AWS console cell/region health and Backstage service health are counterparts; Oyatie adds SPIFFE and Cilium drop visibility. |
...
1. Aggregate Kubernetes node pressure, SPIFFE SVID TTL, Cilium drops, and recovery workflow state per cell.
2. Keep the panel read-focused; recovery mutation is a separate approved action path.
3. AWS console and Backstage service health are counterparts; Oyatie adds cell-tier and SVID evidence.
4. Dashboard panels must resolve to `dashboards/cell-operator.json`.
5. Tests cover stale poll cycle, SVID TTL paging threshold, and Cilium prod drop alert.
...
- Hook 19.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 19.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 19.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 19.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 19.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 19.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 19.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-012-cell-operator-panel.md` matched [`SLO`].
