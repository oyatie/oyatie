...
ip_id: IP-013-adr-promotion-triage-panel
status: planned
...
Technical target set: adr-promotion-triage catalog row, governance gRPC queue, audit emission contract. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `ADR evidence queue` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: adr-promotion-triage catalog row, governance gRPC queue, audit emission contract.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `ADR evidence queue`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from adr-promotion-triage catalog row, governance gRPC queue, audit emission contract; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: adr-promotion-triage catalog row, governance gRPC queue, audit emission contract.
...
|---|---|---|
| GitHub pull-request review queues and Backstage TechDocs governance | Mature external control surface to compare against. | GitHub pull-request review queues and Backstage TechDocs governance are counterparts; Oyatie separates recommendation from approval authority. |
...
1. Read ADR queue from governance and allow recommend-only actions from triage operators.
2. Preserve whistleblower anonymity by redacting submitter_id outside governance principals.
3. GitHub review queues and Backstage TechDocs governance are counterparts.
4. Recommendation is not approval; approval remains outside this panel.
5. Tests cover redaction, stale queue, T2 recommend, and audit event emission.
...
- Hook 20.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 20.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 20.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 20.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 20.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 20.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 20.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-013-adr-promotion-triage-panel.md` matched [`SLO`].
...
- metering_trigger_evidence: `microservices/ops-dashboard-control-center/IP-013-adr-promotion-triage-panel.md` matched [`emission`].
