...
arch_id: ARCH-ops-dashboard-control-center
...
- Integration state: PROPOSED for `ops-dashboard-control-center`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `ops-dashboard-control-center` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/tenant-namespace@v1[both], aws-guest/postgres-wal-g@v1[both], aws-guest/valkey-cluster@v1[both], colo/audit-chain-merkle-seal@v1[both], oyatie-as-cloud-provider/openbao-seal-unseal@v1[both].
...
- Capacity: `per_request` drives CPU `0.18`, RAM `384`, storage `2`, and connection pool variables.
