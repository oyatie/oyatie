...
ip_id: IP-002-incident-command-workflows
status: planned
...
Technical target set: incident-declare.yaml, incident-remediation-approve.yaml, incident-command catalog/API, incident-command.md runbook. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `incident command and remediation approval` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: incident-declare.yaml, incident-remediation-approve.yaml, incident-command catalog/API, incident-command.md runbook.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `incident command and remediation approval`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from incident-declare.yaml, incident-remediation-approve.yaml, incident-command catalog/API, incident-command.md runbook; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: incident-declare.yaml, incident-remediation-approve.yaml, incident-command catalog/API, incident-command.md runbook.
...
|---|---|---|
| PagerDuty and incident.io | Mature external control surface to compare against. | PagerDuty and incident.io are paging/workflow counterparts, but this IP keeps Ops Dashboard as the audited decision surface; Stripe Internal Admin is the audit-log bar. |
...
1. Model incident declaration separately from remediation approval and post-incident evidence export.
2. Bind severity, tenant/cell scope, actor, rationale, and audit seal into the incident-command aggregate.
3. PagerDuty remains paging; Ops Dashboard owns the governed command decision and evidence.
4. Tests cover declare, approve remediation, deny without Cedar, duplicate idempotency key, and evidence export replay.
5. Runbook linkage must resolve to `runbooks/incident-command.md`.
...
- Hook 9.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 9.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 9.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 9.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 9.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 9.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 9.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-002-incident-command-workflows.md` matched [`SLO`].
...
- metering_trigger_evidence: `microservices/ops-dashboard-control-center/IP-002-incident-command-workflows.md` matched [`emission`].
