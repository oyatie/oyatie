...
ip_id: IP-015-observability-pivot
status: planned
...
The previous slice for `IP-015-observability-pivot` was too close to a design-anchor shell: it named the intended control but did not bind the work to ops-dashboard-control-center's actual contracts, policy files, SLOs, and runbooks. This IP closes the `trace/log/metric/audit drill-down` gap for the ops-dashboard-control-center µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
Technical target set: admin-action-audit-stream.json, observability-pivot catalog rows, admin-action-audit-seal-completeness.openslo.yaml. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `trace/log/metric/audit drill-down` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: admin-action-audit-stream.json, observability-pivot catalog rows, admin-action-audit-seal-completeness.openslo.yaml.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `trace/log/metric/audit drill-down`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from admin-action-audit-stream.json, observability-pivot catalog rows, admin-action-audit-seal-completeness.openslo.yaml; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: admin-action-audit-stream.json, observability-pivot catalog rows, admin-action-audit-seal-completeness.openslo.yaml.
...
|---|---|---|
| Datadog, Grafana, and Honeycomb | Mature external control surface to compare against. | Datadog, Grafana, and Honeycomb are observability counterparts; Oyatie keeps pivots tenant-scoped and audit-sealed. |
...
1. Open trace/log/metric/audit pivots from focused dashboard cells with tenant scope preserved.
2. Verify Merkle audit chain locally before showing audit stream health.
3. Datadog, Grafana, and Honeycomb are counterparts; Oyatie adds Cedar-scoped audit pivots.
4. Keyboard affordances are acceptance criteria because operators use this during incidents.
5. Tests cover audit scope denial, trace not found, high-cardinality tenant label refusal, and p99 pivot latency.
...
- Hook 22.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 22.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 22.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 22.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 22.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 22.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 22.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-015-observability-pivot.md` matched [`SLO`, `p99`].
