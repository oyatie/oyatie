...
ip_id: IP-010-cedar-admin-console-surface
status: planned
...
# Cedar admin console surface

## A. Problem

The previous slice for `IP-010-cedar-admin-console-surface` was too close to a design-anchor shell: it named the intended control but did not bind the work to ops-dashboard-control-center's actual contracts, policy files, SLOs, and runbooks. This IP closes the `Cedar fragment lifecycle` gap for the ops-dashboard-control-center µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
5. Wire dashboard/SLO/runbook evidence from admin-action-authorization.cedar, pack-author.json, pack-author-quarantine.md, cedar admin console catalog row; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: admin-action-authorization.cedar, pack-author.json, pack-author-quarantine.md, cedar admin console catalog row.
...
|---|---|---|
| AWS Verified Permissions is the Cedar-adjacent counterpart; GitHub branch protection is the soak/quorum governance analogy for policy promotion. | Mature external control surface to compare against. | AWS Verified Permissions is the Cedar-adjacent counterpart; GitHub branch protection is the soak/quorum governance analogy for policy promotion. |
...
1. Track Cedar fragment states Proposed, Soaking, Active, Sunset, and Quarantined.
2. Require quorum-2 hardware-key publish for T3 fragment activation.
3. Expose eval-error rate and quarantine status in `dashboards/pack-author.json`.
4. AWS Verified Permissions is the policy counterpart; Oyatie adds soak/quarantine workflow.
5. Tests cover wildcard permit lint failure, insufficient quorum, soak too short, and quarantine rollback.
...
- Hook 17.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 17.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 17.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 17.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 17.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 17.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 17.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-010-cedar-admin-console-surface.md` matched [`SLO`].
