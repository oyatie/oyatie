...
ip_id: IP-016-on-call-handoff-bc
status: planned
...
# On-call handoff BC implementation

## A. Problem

The previous slice for `IP-016-on-call-handoff-bc` was too close to a design-anchor shell: it named the intended control but did not bind the work to ops-dashboard-control-center's actual contracts, policy files, SLOs, and runbooks. This IP closes the `on-call handoff aggregate` gap for the ops-dashboard-control-center µservice, not for a generic operations or governance product. The implementation must be reviewable as a single Oya VCS changeset and must not claim runtime maturity until the named artifacts exist and validate.
...
Technical target set: on-call-handoff.md, oncall-handoff-failure.md, command SLOs, PagerDuty adapter. If one of these paths is absent when implementation starts, create that exact missing artifact or record an explicit IaC/catalog gap in this IP's evidence; do not cite fake Terraform, fake Cedar entity types, or unavailable endpoints.

## C. Deliverables

- Contract updates in the relevant OpenAPI, AsyncAPI, or Proto file named by the service manifest.
- Domain/kernel value object for `on-call handoff aggregate` with tenant, principal, cell, HLC timestamp, Cedar decision, audit event, and idempotency fields.
...
- Dashboard, SLO, and runbook linkage using the concrete artifact set: on-call-handoff.md, oncall-handoff-failure.md, command SLOs, PagerDuty adapter.
- Catalog/capability row update when the IP exposes or changes an operator/governance capability.

## D. Implementation Steps

1. Read `microservices/ops-dashboard-control-center/manifest.json`, `PRD.md`, `ARCHITECTURE.md`, `contracts/openapi/ops-dashboard-control-center.yaml`, `contracts/asyncapi/ops-dashboard-control-center-events.yaml`, and `contracts/proto/ops_dashboard_control_center.proto` and confirm the bounded context that owns `on-call handoff aggregate`; update the manifest or catalog only if that owner is missing.
...
5. Wire dashboard/SLO/runbook evidence from on-call-handoff.md, oncall-handoff-failure.md, command SLOs, PagerDuty adapter; dashboard panels must point to real metric/event names and runbook links must resolve.
...
- Operational evidence: on-call-handoff.md, oncall-handoff-failure.md, command SLOs, PagerDuty adapter.
...
|---|---|---|
| PagerDuty and incident.io | Mature external control surface to compare against. | PagerDuty and incident.io are handoff counterparts; Oyatie adds Ed25519 acknowledgement and SPIFFE-attested delivery. |
...
1. Create and acknowledge signed handoff records with active incidents, SLO state, and freeze-window snapshot.
2. PagerDuty integration reads incident state but Ops Dashboard owns signed handoff evidence.
3. PagerDuty and incident.io are handoff counterparts; Oyatie adds Ed25519 ack and SPIFFE delivery.
4. Missed ack emits a PagerDuty page to both outgoing and incoming engineers.
5. Tests cover no-ack timeout, signed ack verification, emergency-services bypass, and SLO snapshot freshness.
...
- Hook 23.1: changed-file evidence must include this IP path and the concrete service artifacts named above.
- Hook 23.2: contract parsing must run after any OpenAPI, AsyncAPI, or Proto edit for this slice.
- Hook 23.3: Cedar permit and forbid cases must cite the real policy file and action name used by this slice.
- Hook 23.4: audit evidence must include event class, seal reference, actor, tenant/cell scope, and idempotency key.
- Hook 23.5: rollback evidence must name the runbook or explain why the slice is read-only.
- Hook 23.6: counterpart closure must be reviewed against the named GitHub/Stripe/Snowflake/etc. row, not inferred from line count.
- Hook 23.7: promotion is blocked if any cited dashboard, SLO, catalog, capability, or runbook path is absent.
...
- ha_trigger_evidence: `microservices/ops-dashboard-control-center/IP-016-on-call-handoff-bc.md` matched [`SLO`].
