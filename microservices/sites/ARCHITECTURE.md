...
## §principals
This anchor is closed for `sites` against ADR-0242 §D-1: principal roster and tenant-scoped caller model.

### Service-specific answer
- Platform principal `oyatie.sites.runtime` owns normal `T0-suggest` execution and never borrows a tenant principal.
- Platform principal `oyatie.sites.worker` owns async jobs, retry queues, and backfill replay listed in `microservices/sites/backfill-replay.md` when present.
- Platform principal `oyatie.sites.auditor` has read-only evidence access through auditor Cedar fragments, not direct database credentials.
- Platform principal `oyatie.sites.ci` is limited to synthetic tenants and non-production cells by the CI-scope Cedar fragment.
- Tenant callers are represented as `<tenant>.sites.caller` and must provide `tenant_id`, `principal_id`, `audience_type`, and workload SVID context.
...
- Example: a tenant principal invoking `T0-suggest` is evaluated as `<tenant>.sites.t0-suggest` before any `sites` state mutation.
...
## §cedar-gates
This anchor is closed for `sites` against ADR-0243 §D-2: Cedar fragment roster, default-deny and action taxonomy.

### Service-specific answer
- Default-deny is represented by the first matching Cedar fragment in `microservices/sites/policy/auditor-scope.cedar`, `microservices/sites/policy/ci-scope.cedar`, `microservices/sites/policy/data-residency.md`, `microservices/sites/policy/editor-isolation.md`, `microservices/sites/policy/public-read.cedar`, `microservices/sites/policy/tenant-scope.cedar`.
- Action namespace uses `sites::<bounded_context>::<verb>`; the first protected action is `sites::t0-suggest::execute`.
...
- Concrete fragments in scope: `microservices/sites/policy/auditor-scope.cedar`, `microservices/sites/policy/ci-scope.cedar`, `microservices/sites/policy/data-residency.md`, `microservices/sites/policy/editor-isolation.md`, `microservices/sites/policy/public-read.cedar`, `microservices/sites/policy/tenant-scope.cedar`.
- Example: `sites::t0-suggest::execute` denies if `resource.tenant_id != principal.tenant_id` or if the compliance pack adds a stricter overlay.
...
## §tenant-scoping
This anchor is closed for `sites` against ADR-0244 §D-3: tenant_id, audience_type, provider_credential_mode and row/event boundaries.
...
- State surface `sites.block` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `sites.cdn_delivery` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `sites.cms_collection` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `sites.domain_binding` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
- State surface `sites.page` is documented as tenant-scoped with `tenant_id`, `principal_id`, `audit_event_class`, `home_cell`, and `lifecycle_state` columns/events.
...
This anchor is closed for `sites` against ADR-0245 §D-1: substrate/product classification and dependency direction.

### Service-specific answer
- Manifest classifies `sites` as `product`, so this section treats it as a product consumer.
- Declared substrate/product dependencies: `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- If substrate: products consume `sites` only through contracts `microservices/sites/contracts/asyncapi/sites-events.yaml`, `microservices/sites/contracts/openapi/sites.yaml`, `microservices/sites/contracts/proto/sites.proto`.
- If product: `sites` may call substrate services but must not create product-to-product synchronous dependencies.
...
- Primary bounded contexts bound to this classification: `block`, `cdn-delivery`, `cms-collection`, `domain-binding`, `page`, `search`; +1 more.
...
## §policy-evaluation
This anchor is closed for `sites` against ADR-0246 §D-4: library-first policy evaluation mode and fallback limits.
...
- The caller-side library evaluates fragments from `microservices/sites/policy/auditor-scope.cedar`, `microservices/sites/policy/ci-scope.cedar`, `microservices/sites/policy/data-residency.md`, `microservices/sites/policy/editor-isolation.md`, `microservices/sites/policy/public-read.cedar`, `microservices/sites/policy/tenant-scope.cedar` with the full action/resource/context tuple.
...
- Example: `sites::t0-suggest::execute` evaluates locally, then emits an audit event before the usecase writes `sites.block`.
...
## §cell-eligibility
This anchor is closed for `sites` against ADR-0248 §D-1: cell tier, shard width, DR pair and shuffle-shard behavior.
...
This anchor is closed for `sites` against ADR-0255 §D-2: Intelligence calls, library-first dispatch and audience tags.

### Service-specific answer
- `sites` uses Intelligence only through a declared dispatch boundary; default is library-first and network-opt-in must name a latency budget.
- Audience tag format is `sites.t0-suggest` and is included in every AI/risk/model call audit event.
...
- Example: `T0-suggest` can request classification or summarization with `audience_tag=sites.t0-suggest`; the response is logged with model version and policy decision.
...
## §ontology-read-path
This anchor is closed for `sites` against ADR-0257 §D-2: Ontology entity reads, projection cache and freshness floor.

### Service-specific answer
- Ontology read mode is library-first projection cache for `sites` unless this section documents a network-only exception.
- Entity types read by this µservice: `Tenant`, `Principal`, `PolicyGrant`, `CompliancePack`, plus bounded-context entities named below.
- Bounded-context entity `block` is read/written as an ontology node or edge projection when `sites` needs cross-surface discovery.
- Bounded-context entity `cdn-delivery` is read/written as an ontology node or edge projection when `sites` needs cross-surface discovery.
- Bounded-context entity `cms-collection` is read/written as an ontology node or edge projection when `sites` needs cross-surface discovery.
- Bounded-context entity `domain-binding` is read/written as an ontology node or edge projection when `sites` needs cross-surface discovery.
...
## §transport
This anchor is closed for `sites` against ADR-0253 §D-1: HTTP/3 fallback chain, strict TLS, ECH and PQC posture.

### Service-specific answer
- Public/API contracts in scope: `microservices/sites/contracts/asyncapi/sites-events.yaml`, `microservices/sites/contracts/openapi/sites.yaml`, `microservices/sites/contracts/proto/sites.proto`.
...
- IaC transport evidence: `microservices/sites/iac/helm/Chart.yaml`, `microservices/sites/iac/helm/templates/cronjob.yaml`, `microservices/sites/iac/helm/templates/deployment.yaml`, `microservices/sites/iac/helm/templates/hpa.yaml`, `microservices/sites/iac/helm/templates/networkpolicy.yaml`, `microservices/sites/iac/helm/templates/pdb.yaml`; +6 more.
- Example: `sites` `T0-suggest` calls use HTTP/3 on normal networks and HTTP/2 when UDP/QUIC is blocked by enterprise firewalls.
...
## §deployment-shape
This anchor is closed for `sites` against ADR-0254 §D-2: Kubernetes, Cloud Hypervisor, Kata, Wasm/container/VM split.

### Service-specific answer
- Runtime components derive from bounded contexts `block`, `cdn-delivery`, `cms-collection`, `domain-binding`, `page`, `search`; +1 more and deploy as separate app/worker/API pods where files exist.
...
- IaC manifests in scope: `microservices/sites/iac/helm/Chart.yaml`, `microservices/sites/iac/helm/templates/cronjob.yaml`, `microservices/sites/iac/helm/templates/deployment.yaml`, `microservices/sites/iac/helm/templates/hpa.yaml`, `microservices/sites/iac/helm/templates/networkpolicy.yaml`, `microservices/sites/iac/helm/templates/pdb.yaml`; +6 more.
- Catalog crates/components: `microservices/sites/catalog/oya-sites-block-adapter-loro.yaml`, `microservices/sites/catalog/oya-sites-cdn-delivery-adapter-cloudflare-cdn-stub.yaml`, `microservices/sites/catalog/oya-sites-cdn-delivery-adapter-libvips.yaml`, `microservices/sites/catalog/oya-sites-cdn-delivery-adapter-pandoc.yaml`, `microservices/sites/catalog/oya-sites-cdn-delivery-adapter-s3.yaml`, `microservices/sites/catalog/oya-sites-cdn-delivery-app.yaml`; +18 more.
- Example: `T0-suggest` API runs as a Kata-isolated container; scheduled/background work for `sites` uses a separate worker principal.
...
## §observability
This anchor is closed for `sites` against ADR-0263 §D-1: audit events, metrics, logs and trace span shape.

### Service-specific answer
- SLO/dashboard sources in scope: `microservices/sites/slos/accessibility-wcag-correctness.openslo.yaml`, `microservices/sites/slos/acme-renew-latency.openslo.yaml`, `microservices/sites/slos/cms-query-latency.openslo.yaml`, `microservices/sites/slos/image-optimize-latency.openslo.yaml`, `microservices/sites/slos/page-render-latency.openslo.yaml`, `microservices/sites/slos/publish-latency.openslo.yaml`; +6 more.
- Audit classes use prefix `oya.sites.<context>.<outcome>` and must be registered centrally per ADR-0263.
- Core counter metric: `sites_t0_suggest_total` with dimensions `outcome`, `route_class`, `tenant_id_class`, and `cell_tier`.
- Core latency metric: `sites_t0_suggest_latency_ms` with bounded cardinality and no raw `tenant_id` label.
- Reference evidence artifact: `microservices/sites/slos/accessibility-wcag-correctness.openslo.yaml`.
...
- Example trace: `sites.t0-suggest` -> `policy.evaluate` -> `storage.t0_suggest` -> `audit.emit`.
...
## §abuse-defence
This anchor is closed for `sites` against ADR-0297 §D-3: anti-bot, anti-spoof, anti-scrape controls plus UX floor.

### Service-specific answer
- Internet-facing `sites` routes use edge rate limits by IP, JA4 fingerprint, tenant, route class, and `t0-suggest` action class.
...
- Cedar/IaC controls in scope: `microservices/sites/policy/auditor-scope.cedar`, `microservices/sites/policy/ci-scope.cedar`, `microservices/sites/policy/data-residency.md`, `microservices/sites/policy/editor-isolation.md`, `microservices/sites/policy/public-read.cedar`, `microservices/sites/policy/tenant-scope.cedar`; +12 more.
- Canary payloads for `sites` use fake `t0-suggest` identifiers and honey endpoints under `/.well-known/oya-canary/sites`.
...
This anchor is closed for `sites` against documentation-rigor.md §3.2.5: applicable human-safety and platform edge-case handling.

### Service-specific answer
- Network partition: `sites` keeps tenant-local reads when safe, stops cross-cell writes that would violate residency, and emits degraded-mode audit events.
...
- Regional outage: home-cell failover follows `multi-region.md`; if a pack forbids cross-border DR, `sites` preserves local queue state instead of failing open.
...
- Account recovery/hijack path: identity step-up and `sites` audit evidence keep legitimate recovery from becoming an adversary shortcut.
...
- Disaster surge: `sites` enforces per-tenant isolation so one hot tenant or emergency mode cannot starve unrelated cells.
...
This anchor is closed for `sites` against ADR-0296 §D-1: credential sidecar, OpenBao TTLs and secret-reference path.

### Service-specific answer
- Credential scopes for `sites` include `identity`, `tenancy`, `policy-engine`, `observability`, `audit-chain`, `cloud-secrets`.
- SecretReference pattern is `${openbao:secret/<tenant_id>/sites/<credential-name>}`; platform-owned credentials use tenant `oyatie` only for oyatie-internal calls.
...
- Rotation cadence is linked from `microservices/sites/runbooks/acme-cert-renewal-failure.md`, `microservices/sites/runbooks/ai-page-build-rollback.md`, `microservices/sites/runbooks/asset-optimization-degraded.md`, `microservices/sites/runbooks/cdn-cache-purge-cascade.md`, `microservices/sites/runbooks/custom-domain-dns-drift.md`, `microservices/sites/runbooks/page-export-corruption.md`; +1 more.
