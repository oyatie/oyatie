...
`cloud-k8s` keeps its existing bounded context and flat `microservices/cloud-k8s/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `cloud-k8s`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `cloud-k8s` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: on-prem/kubeadm-cluster@v1[both], on-prem/cilium-cni@v1[both], on-prem/istio-ambient@v1[both], on-prem/envoy-gateway@v1[both], aws-guest/eks-cluster@v1[paid], oci-guest/oke-cluster@v1[paid], oyatie-as-cloud-provider/tenant-namespace@v1[both], oyatie-as-cloud-provider/per-cell-nodepool-kata@v1[both].
...
- Capacity: `per_capability` drives CPU `0.22`, RAM `512`, storage `4`, and connection pool variables.
