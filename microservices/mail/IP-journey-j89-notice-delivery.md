...
ip_id: IP-journey-j89-notice-delivery
...
# IP-journey-j89-notice-delivery - mail

## Goal

Implement the `notice-delivery` slice for `mail` so j89 can satisfy `UK-AADC + UK-Online-Safety-Act` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/mail/contracts/openapi/j89-notice-delivery-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/mail/contracts/asyncapi/j89-notice-delivery-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/mail/contracts/proto/j89-notice-delivery-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/mail/policy/j89-notice-delivery.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/mail/runbooks/j89-notice-delivery-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/mail/tests/j89_notice_delivery_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J89MailNoticeDeliveryStarted`.
- `J89MailNoticeDeliveryPermitted`.
- `J89MailNoticeDeliveryDenied`.
- `J89MailNoticeDeliveryCommitted`.
- `J89MailNoticeDeliveryRolledBack`.

## Implementation rows

### IP row 001 - mail notice-delivery
Scope: implement one mail behavior for j89, not a cross-service refactor.
...
- Surface evidence: `microservices/mail/IP-journey-j89-notice-delivery.md` matched `openapi, asyncapi, .proto`; contract files `microservices/mail/contracts/openapi/mail.yaml, microservices/mail/contracts/asyncapi/mail-events.yaml, microservices/mail/contracts/proto/mail.proto`; type anchor `crates/oya-shared-email-comms-kernel/src/lib.rs::OutboundMessage`.
