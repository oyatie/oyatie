...
status: Active
entry_gate: |
  ADR-0135 (full social network super-app dissolution) + ADR-0132 (no-grouping forward policy) accepted;
  /specs/microservices/mail.json published; existing crates oya-mail-* legacy-prefix migration recognised as a working
...
- `/specs/microservices/mail.json` — promote to `mail.json` reference; retain legacy communications pointer for migration window.
- `registry/artifact-capabilities-registry.json` — add mail-send, mailbox-search, ediscovery-export capabilities.

### Out-of-scope

- Migration of existing `oya-mail-* legacy-prefix migration` callers — owned by IP-M03-CONNECT-MIGR-* phase running in parallel. This phase introduces `oya-mail-*` crates without removing the legacy communications stubs (deprecation cycle).
...
Coverage check vs. PRD §"Bounded Contexts": all 8 BCs covered. Coverage check vs. ADR-0135 communications service separation: mail's slice of the parallel-session dissolution is fully scoped.
...
- ADR-0135: full social network super-app (parallel-session dissolution authority).
