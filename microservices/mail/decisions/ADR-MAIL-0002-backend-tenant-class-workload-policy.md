...
purpose: Close PRD-mail Open Question 4 (gap-fill) — which SMTP+IMAP/JMAP backend ships per tenant_class and workload profile, and how the µservice keeps its BCs backend-neutral.
---

# ADR-MAIL-0002: Mail-server backend per tenant_class and workload profile — Postfix+Dovecot for high-volume paid workloads; Stalwart for demo_trial and standard paid workloads; both behind oya-mail-* port traits
...
- **Stalwart Mail Server** — modern Rust-native single-binary SMTP + IMAP + JMAP + Sieve + WebDAV, JMAP-first design (RFC 8620 / 8621 native), single-config-file deployment, integrated full-text search (Tantivy), simpler ops surface. Stalwart 0.10.x LTS; smaller operator pool; less battle-tested at hyperscale volume.

Naïvely picking one for all tenants forces a trade. Postfix-only over-engineers the `demo_trial` and standard paid workload operator surface (multiple daemons, multiple config languages, hand-tuned LMTP between Postfix and Dovecot). Stalwart-only under-serves high-volume paid tenants who need (a) reputation-pool sophistication, (b) proven multi-million-mailbox horizontal scale, (c) DKIM/DMARC tooling depth that Postfix has spent twenty years accumulating, (d) operator familiarity from the enterprise mail-admin labor market.

The tenant_class model segments oyatie tenants as `demo_trial` or `paid`. Backend selection follows tenant_class plus workload, compliance, and deployment context because the ops complexity vs. scale-headroom curve is workload-shaped.
...
oyatie mail ships **two backend stacks** behind the µservice's port traits, selected per tenant by tenant_class plus workload profile:

1. **Postfix 3.8.x LTS + Dovecot 2.3.21 LTS** for `tenant_class=paid` high-volume or regulated workload profiles. Adapters: `oya-mail-inbound-smtp-adapter-postfix`, `oya-mail-outbound-smtp-adapter-postfix`, `oya-mail-imap-frontend-adapter-dovecot`. LMTP between Postfix and the `mailbox-store` Postgres+S3 backend; Dovecot reads from the same `mailbox-store` via the `dovecot-rust-lda` LDA adapter. Rspamd for spam classification. Per-tenant SMTP IP pool with warm-up automation (M3AAWG BCP v3).

2. **Stalwart 0.10.x LTS** for `tenant_class=demo_trial` and `tenant_class=paid` standard workload profiles. Adapter: `oya-mail-{inbound-smtp,outbound-smtp,imap-frontend}-adapter-stalwart`. Stalwart's internal SMTP queue + JMAP server + IMAP server unify under one process per cell; full-text search routed through the µservice's own `oya-mail-search-index-adapter-search-index` (Tantivy) rather than Stalwart's internal index to keep the search posture uniform across tenant classes and workload profiles.

3. **Both stacks share the same `oya-mail-*` BCs.** No business logic lives in adapters; ports in `-kernel`, math in `-domain`, orchestration in `-usecase`. Workload-profile-specific behaviour (reputation pool size, warmup cadence, queue depth limits) lives in tenant configuration consumed by the usecase layer, not in adapter code. Cross-product rule LEAN-A2 unaffected.

4. **Workload-profile migration path.** A paid tenant moving from standard to high-volume or regulated workload profile migrates from Stalwart to Postfix+Dovecot with the `mail-backend-migration` IP (zero-downtime: dual-publish DKIM keys + parallel-deliver during cutover, then drain Stalwart). The migration is operator-driven, not automatic.
...
### A. Postfix + Dovecot only (all tenant classes)
- Pros: one ops surface, one operator skill set, deepest battle-testing, broadest deliverability tooling.
- Cons: ops complexity (multiple daemons, multiple config languages, separate auth/quota/sieve paths) is excessive for `demo_trial` and standard paid tenants whose volume rarely justifies it; operator cost for small tenants effectively makes mail unprofitable for standard workloads.
...
### B. Stalwart only (all tenant classes)
...
- Rejected: under-served high-volume paid workload kills the hyperscaler-grade mandate for paid tenants that need hyperscaler grade.
...
- Cons: doubles operational surface per tenant (two daemons per tenant regardless of tenant_class or workload profile); breaks the per-tenant logical-mail-server boundary; doesn't actually solve the workload-fit problem.
...
- Tenant_class plus workload-profile-shaped operator surface: `demo_trial` and standard paid tenants get a single-binary stack their volume justifies; high-volume paid tenants get the proven hyperscale stack their volume demands.
...
- Stalwart's JMAP-native posture aligns with ADR-MAIL-0003 (JMAP-first SDK launch) for `demo_trial` and standard paid tenants — JMAP is already the native protocol, not a bolt-on.
- High-volume paid tenants inherit Postfix's twenty-year deliverability tooling (sender-rep heuristics, DKIM rotation tooling, DMARC report aggregation, ARC chain support, M3AAWG BCP-conformant warmup automation).
...
- Backend migration (Stalwart → Postfix+Dovecot on workload-profile change) is a non-trivial operator-driven IP; can't be automated end-to-end because DKIM key rotation + reputation pool warmup + DNS propagation are inherently slow.
- Backend reversion (Postfix+Dovecot → Stalwart) is not supported because Stalwart cannot match Postfix's reputation-pool sophistication. Documented as a "no path back" in the tenant SLA so paid tenants can plan accordingly.
...
- Per-tenant config records which backend the tenant runs; tenant-provisioning workflow consumes tenant_class + region + workload profile to pick the backend; migration record retained in `microservices/mail/evidence/backend-migration/<tenant>-<ts>.json`.
- Dashboards: per-backend SLO panels + cross-backend deliverability comparison panel so the µservice operator can detect "standard-profile Stalwart deliverability score lagging high-volume Postfix" and triage at the backend level.
...
- **RFC 8620 / 8621** (JMAP Core / Mail): Stalwart native; Postfix+Dovecot via the `httpie-jmap-gateway` shim adapter `oya-mail-imap-frontend-adapter-jmap-bridge` (high-volume paid workload profile).
- KR-pack: both backends supportable under KR-resident KMS + 5y retention floor; pack onboarding chooses based on tenant_class and workload profile as above.
- HIPAA-pack: paid tenant_class by definition (BAA required); Postfix+Dovecot stack inherits when the workload profile requires it.
