...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that mail
  (SMTP inbound + outbound delivery + spam/DLP) satisfies four hyperscaler-grade invariants drawn from
...
prometheusrule_path: microservices/mail/iac/helm/templates/prometheusrule.yaml
---

# mail — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`mail`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-MAIL`)
...
- **How mail satisfies it**:
  - Capability calls in `microservices/mail/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_mail_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_mail_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/mail/iac/helm/templates/prometheusrule.yaml` (`mail-llm-capability-circuit-open`
    + `mail-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/mail/dashboards/` (per-capability panels).
  - Threat model: `microservices/mail/threat-model.md` §"Dependency
...
- **How mail satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_mail_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/mail/iac/helm/templates/prometheusrule.yaml` (`mail-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/mail/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/mail/capacity-model.md` (where
...
- **How mail satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/mail/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/mail/iac/helm/templates/prometheusrule.yaml` (`mail-saturation-cpu-over-70pct`,
    `mail-errors-5xx-rate-spike`, `mail-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/mail/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/mail/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How mail satisfies it**:
  - Every SLO in `microservices/mail/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/mail/iac/helm/templates/prometheusrule.yaml` (`mail-*-fast-burn-1h-14x` +
    `mail-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/mail/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/mail/hyperscaler-conformance.md
test -f microservices/mail/iac/helm/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/mail/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/mail/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/mail/iac/helm/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/mail/iac/helm/templates/prometheusrule.yaml || true
