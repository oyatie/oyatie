...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/emergency/manifest.json
  - microservices/emergency/PRD.md
  - microservices/emergency/README.md
...
`emergency` keeps its existing bounded context and flat `microservices/emergency/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `emergency`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `emergency` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/tenant-namespace@v1[both], aws-guest/postgres-wal-g@v1[both], aws-guest/valkey-cluster@v1[both], oci-guest/tenant-namespace@v1[both], oci-guest/valkey-cluster@v1[both], oci-guest/always-free/tenant-namespace@v1[demo_trial], on-prem/tenant-namespace@v1[both], colo/tenant-namespace@v1[both].
...
- Capacity: `per_request` drives CPU `0.45`, RAM `1024`, storage `12`, and connection pool variables.
- DR: RTO `1800` seconds and RPO `120` seconds constrain backup and failover primitives.
