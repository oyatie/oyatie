...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/marketplace/manifest.json
  - microservices/marketplace/PRD.md
  - microservices/marketplace/README.md
  - microservices/marketplace/ARCHITECTURE.md
---
...
`marketplace` keeps its existing bounded context and flat `microservices/marketplace/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `marketplace`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `marketplace` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oyatie-as-cloud-provider/tenant-namespace@v1[both], oyatie-as-cloud-provider/per-cell-nodepool-runc@v1[both], oyatie-as-cloud-provider/audit-chain-sink@v1[paid], on-prem/postgres-service-database@v1[paid], on-prem/valkey-cluster@v1[both], aws-guest/edge-waf@v1[both].
...
- Capacity: `per_request` drives CPU `0.18`, RAM `640`, storage `18`, and connection pool variables.
