...
- Service-specific pattern: `oya_marketplace_seller_onboarding_deny_total` rises while `oya_marketplace_seller_onboarding_deny_spike_dependency_error_ratio` is flat; inspect local state before escalating Stripe Connect support.
...
7. Pause promotion: `oya vcs hold --microservice marketplace --reason $INCIDENT_ID --runbook seller-onboarding-deny-spike`.
...
17. Open external vendor ticket: `oya vendor ticket open --vendor "Stripe Connect support" --incident $INCIDENT_ID --summary marketplace-seller-onboarding-deny-spike`.
