...
- Service-specific pattern: `oya_marketplace_settlement_ledger_replay_lag_seconds` rises while `oya_marketplace_settlement_ledger_replay_dependency_error_ratio` is flat; inspect local state before escalating Stripe Connect support.
...
7. Pause promotion: `oya vcs hold --microservice marketplace --reason $INCIDENT_ID --runbook settlement-ledger-replay`.
...
17. Open external vendor ticket: `oya vendor ticket open --vendor "Stripe Connect support" --incident $INCIDENT_ID --summary marketplace-settlement-ledger-replay`.
