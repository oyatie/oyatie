---
doc_class: Runbook
doc_id: RUNBOOK-CRM-AUTO_REBALANCE
microservice: crm
...
# Runbook: Crm Auto Rebalance

## Operator Contract
- Runbook id: crm-auto-rebalance.
- Primary service namespace: `crm`.
...
- Owning team: axis-crm + axis-erp-parity.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-crm-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=crm; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n crm rollout status deploy/crm --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service crm --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service crm --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service crm --cell $CELL --window 30m`.
