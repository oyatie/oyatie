...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/payments/manifest.json
...
`payments` keeps its existing bounded context and flat `microservices/payments/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `payments`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `payments` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: oyatie-as-cloud-provider/tenant-namespace@v1[both], oyatie-as-cloud-provider/per-cell-nodepool-kata@v1[paid], on-prem/postgres-service-database@v1[paid], on-prem/valkey-cluster@v1[paid], on-prem/openbao-secret-binding@v1[paid], aws-guest/edge-waf@v1[paid].
...
- Capacity: `per_request` drives CPU `0.2`, RAM `768`, storage `24`, and connection pool variables.
