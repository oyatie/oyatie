...
ip_id: IP-journey-j82-regulated-money-movement
...
# IP-journey-j82-regulated-money-movement - payments

## Goal

Implement the `regulated-money-movement` slice for `payments` so j82 can satisfy `KR-FSS` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/payments/contracts/openapi/j82-regulated-money-movement-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/payments/contracts/asyncapi/j82-regulated-money-movement-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/payments/contracts/proto/j82-regulated-money-movement-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/payments/policy/j82-regulated-money-movement.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/payments/runbooks/j82-regulated-money-movement-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/payments/tests/j82_regulated_money_movement_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J82PaymentsRegulatedMoneyMovementStarted`.
- `J82PaymentsRegulatedMoneyMovementPermitted`.
- `J82PaymentsRegulatedMoneyMovementDenied`.
- `J82PaymentsRegulatedMoneyMovementCommitted`.
- `J82PaymentsRegulatedMoneyMovementRolledBack`.

## Implementation rows

### IP row 001 - payments regulated-money-movement
Scope: implement one payments behavior for j82, not a cross-service refactor.
...
- Authority: ADR-0343.
- Trigger evidence: `microservices/payments/IP-journey-j82-regulated-money-movement.md` matched `financial, payment`.
