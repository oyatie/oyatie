...
# Migration Playbook - Wave 15-ZD ADR-0346..0349 doctrine for `comms-email`

Audience: an Oyatie operator or migration owner preparing `comms-email` for the Wave 15-ZD doctrine surface before implementation waves author runtime code, manifests, Jenkinsfiles, ArgoCD applications, or sharding bodies.

Outcome: `comms-email` has a documented migration path for the four doctrine decisions, with no runtime mutation and no manifest mutation.
...
1. Confirm the `comms-email` manifest exists and is not marked with `status: "Retired"` and is not `doc_class: RetiredMicroserviceMarker`.
2. Capture the current migration-playbooks directory listing before authoring implementation follow-up.
3. Capture whether `comms-email` already has CI, CD, cellular, sharding, or governance lane references in PRD, ARCH, README, IPs, manifests, runbooks, contracts, Cedar, SLOs, and capabilities.
...
1. Treat `oya verify --ci-required` as the local rehearsal for `comms-email` changes before push.
...
3. When `comms-email` changes touch Rust, contracts, manifests, generated docs, or governance lanes, run the verifier before handoff.
...
1. Search `comms-email` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `comms-email` manifest `fitness_lanes` array only in the manifest-owning slot.
...
1. Prepare `comms-email` for a future `sharding_automation` block with autosharding, auto_rebalance, and dynamic_sharding sub-blocks.
...
1. Treat Jenkins LTS as the self-hostable CI substrate for `comms-email` when GitHub Actions runners are unavailable.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `comms-email`.
...
1. Freeze the current `comms-email` doctrine references before implementation starts.
2. Assign ownership for the Wave 15-ZA, 15-ZB, 15-ZD, and 15-ZE follow-up surfaces touching this microservice.
3. Confirm whether `comms-email` is stateful, tenant-facing, edge-facing, or substrate-facing; that classification drives sharding and CI/CD evidence.
...
5. This file is the `comms-email` migration scaffold that those implementation lanes can cite.
...
2. This playbook lists every enforced_by lane from the four ADRs that can affect `comms-email`.
...
- Local verifier transcript for `comms-email` once Wave 15-ZA lands.
- Rename inventory diff for `comms-email` once Wave 15-ZB lands.
...
Stop when `comms-email` has this scaffold committed under `migration-playbooks/`, ADR citation validation passes for the docs corpus, and no file outside this slot boundary is staged for the ZF-18 commit.
