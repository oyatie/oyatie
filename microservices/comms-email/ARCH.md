...
related_adrs:
  ...
  - ADR-0349
companion_docs:
  - microservices/comms-email/manifest.json
  - microservices/comms-email/PRD.md
  - microservices/comms-email/README.md
  - microservices/comms-email/ARCHITECTURE.md
---

# Architecture: Comms Email

## Architecture Boundary

`comms-email` keeps its existing bounded context and flat `microservices/comms-email/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `comms-email`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `comms-email` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
...
- Capacity: `per_message` drives CPU `0.08`, RAM `192`, storage `2`, and connection pool variables.
