...
- **Tenant Outcome 2 — Professional-only, never federates to Personal.** Per parallel ADR-0238, `network` is Professional-tier-only; the `context_kind: Professional` discriminator is compile-time-only; no Personal posts, no Personal followers, no Personal-tier audit scope ever surfaces. The sibling `social` µservice owns the Personal/General microblog surface.
