...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that network
  (professional graph + connection + InMail bridge + recommender) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/network.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/network/iac/helm/network/templates/prometheusrule.yaml
---

# network — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`network`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-NETWORK`)
...
- **How network satisfies it**:
  - Capability calls in `microservices/network/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_network_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_network_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/network/iac/helm/network/templates/prometheusrule.yaml` (`network-llm-capability-circuit-open`
    + `network-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/network/dashboards/` (per-capability panels).
  - Threat model: `microservices/network/threat-model.md` §"Dependency
...
- **How network satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_network_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/network/iac/helm/network/templates/prometheusrule.yaml` (`network-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/network/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/network/capacity-model.md` (where
...
- **How network satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/network/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/network/iac/helm/network/templates/prometheusrule.yaml` (`network-saturation-cpu-over-70pct`,
    `network-errors-5xx-rate-spike`, `network-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/network/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/network/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How network satisfies it**:
  - Every SLO in `microservices/network/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/network/iac/helm/network/templates/prometheusrule.yaml` (`network-*-fast-burn-1h-14x` +
    `network-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/network/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/network/hyperscaler-conformance.md
test -f microservices/network/iac/helm/network/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/network/iac/helm/network/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/network/iac/helm/network/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/network/iac/helm/network/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/network/iac/helm/network/templates/prometheusrule.yaml || true
