---
doc_class: Runbook
doc_id: RUNBOOK-DIAGNOSTICS-AUTO_REBALANCE
...
# Runbook: Diagnostics Auto Rebalance

## Operator Contract
- Runbook id: diagnostics-auto-rebalance.
- Primary service namespace: `diagnostics`.
...
- Owning team: axis-diagnostics + council-product.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-diagnostics-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=diagnostics; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n diagnostics rollout status deploy/diagnostics --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service diagnostics --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service diagnostics --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service diagnostics --cell $CELL --window 30m`.
