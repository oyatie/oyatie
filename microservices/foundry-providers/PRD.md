...
Per ADR-0105 (13-value canonical layer enum) and ADR-0106 (`application` → `usecase` rename), the µservice exposes layers: `kernel`, `domain`, `usecase`, `api`, `adapter` (split per-vendor), `rest`, `worker`, `sdk`, `app`. ADR-0131 §"per-microservice flat layout" prescribes that all crates live under `microservices/foundry-providers/src/crates/`.
...
JUSTIFICATION:
- microservice = foundry-providers (microservices/foundry-providers/)
...
4. **Aggregator-billing tenants.** Tenants who bring an OpenRouter or LiteLLM key (rather than per-vendor keys) are not in scope for M01. Tracked separately for M03+ if tenant demand surfaces.
