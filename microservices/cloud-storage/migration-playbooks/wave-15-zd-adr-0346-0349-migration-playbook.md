...
1. Treat `oya verify --ci-required` as the local rehearsal for `cloud-storage` changes before push.
...
3. When `cloud-storage` changes touch Rust, contracts, manifests, generated docs, or governance lanes, run the verifier before handoff.
...
1. Search `cloud-storage` artifact surfaces for `oya-foundry-fitness-*` references.
...
4. Update any future `cloud-storage` manifest `fitness_lanes` array only in the manifest-owning slot.
...
5. Future Jenkinsfile parity must mirror the GitHub Actions CI steps for `cloud-storage`.
