...
| #2 AI tag-suggest semantics (multi-label classification vs retrieval) | Open | follow-on capability ADR |
| #3 web-clipper extension distribution (all browsers day-1 vs staged) | Open | follow-on IP |
| #4 daily-note timezone authority (user-local vs tenant-default) | Open | UX research follow-on |
| #5 public-share-link OG metadata leakage policy | Open | follow-on policy decision |
| #6 mobile-platform offline-edit conflict semantics (LWW vs CRDT-merge) | Open | follow-on ADR |

All PRD Open Questions are either closed by an ADR in this directory or scheduled for follow-on. Future ADRs land here with sequential `ADR-NOTES-XXXX` IDs.
