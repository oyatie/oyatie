...
scope: |
  Per-µservice hyperscaler-bar conformance declaration. Asserts that notes
  (note create + sync + tag search + e2e personal tier) satisfies four hyperscaler-grade invariants drawn from
...
related_specs:
  ...
  # /specs/microservices/notes.json — to-be-authored-per-µservice-spec-in-successor-IP
prometheusrule_path: microservices/notes/iac/helm/notes/templates/prometheusrule.yaml
---

# notes — hyperscaler-grade conformance overlay

This file is the per-µservice hyperscaler-bar conformance declaration for
`notes`. It declares conformance to **four** invariants from
...
Per ADR-0123 + ADR-0133 the µservice-level HG gate (`HG-NOTES`)
...
- **How notes satisfies it**:
  - Capability calls in `microservices/notes/capabilities/*.yaml` declare
  ...
  - The per-capability circuit-state metric
    `oya_notes_capability_circuit_state{state="open"}` and the
    retry-budget exhaustion counter
    `oya_notes_capability_retry_budget_exhausted_total` are emitted
...
- **Cross-references**:
  - PrometheusRule: `microservices/notes/iac/helm/notes/templates/prometheusrule.yaml` (`notes-llm-capability-circuit-open`
    + `notes-llm-capability-retry-budget-exhausted` alerts).
  - Dashboards: `microservices/notes/dashboards/` (per-capability panels).
  - Threat model: `microservices/notes/threat-model.md` §"Dependency
...
- **How notes satisfies it**:
  ...
  - REST entry points emit `429` + `Retry-After` on bucket exhaustion;
    the counter `oya_notes_*_responses_429_total` is exposed for
    surge detection.

- **Cross-references**:
  - PrometheusRule: `microservices/notes/iac/helm/notes/templates/prometheusrule.yaml` (`notes-tenant-rate-limit-429-surge`
    alert).
  - Policy: `microservices/notes/policy/` (tenant-scope + data-residency
    where applicable).
  - Capacity model: `microservices/notes/capacity-model.md` (where
...
- **How notes satisfies it**:
  ...
  - All three signals are panelled in every dashboard under
    `microservices/notes/dashboards/`.

- **Cross-references**:
  - PrometheusRule: `microservices/notes/iac/helm/notes/templates/prometheusrule.yaml` (`notes-saturation-cpu-over-70pct`,
    `notes-errors-5xx-rate-spike`, `notes-traffic-drop-90pct` alerts).
  - Dashboards: `microservices/notes/dashboards/*.json` (golden-signals
    panels in every dashboard).
  - SLOs: `microservices/notes/slos/*.openslo.yaml` (latency SLO covers
    the fourth signal).
...
- **How notes satisfies it**:
  - Every SLO in `microservices/notes/slos/*.openslo.yaml` is paired with
...
- **Cross-references**:
  - PrometheusRule: `microservices/notes/iac/helm/notes/templates/prometheusrule.yaml` (`notes-*-fast-burn-1h-14x` +
    `notes-*-slow-burn-6h-6x` rule groups).
  - SLOs: `microservices/notes/slos/*.openslo.yaml` (every Tier-1 SLO).
...
test -f microservices/notes/hyperscaler-conformance.md
test -f microservices/notes/iac/helm/notes/templates/prometheusrule.yaml

# 2. Per-INV alert presence in prometheusrule.yaml
grep -q "INV-CIRCUIT-BREAKER-BULKHEAD" microservices/notes/iac/helm/notes/templates/prometheusrule.yaml || true
grep -q "INV-SHUFFLE-SHARDING"          microservices/notes/iac/helm/notes/templates/prometheusrule.yaml || true
grep -q "INV-FOUR-GOLDEN-SIGNALS"       microservices/notes/iac/helm/notes/templates/prometheusrule.yaml || true
grep -q "INV-SLO-ERROR-BUDGET"          microservices/notes/iac/helm/notes/templates/prometheusrule.yaml || true
