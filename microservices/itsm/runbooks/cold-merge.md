---
doc_class: Runbook
doc_id: RUNBOOK-ITSM-COLD_MERGE
microservice: itsm
...
# Runbook: Itsm Cold Merge

## Operator Contract
- Runbook id: itsm-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-itsm-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=itsm; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n itsm rollout status deploy/itsm --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service itsm --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service itsm --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service itsm --cell $CELL --window 30m`.
