# ITSM Runbook: Delegated Admin Lockout

Service: itsm  
Surface: local operational primitive suite  
Primary SLO: cmdb-relation-freshness  
Domain focus: incident, change, problem, cmdb

## Trigger
- Alert `itsm-cmdb-relation-freshness` burns above the 2x multi-window threshold.
...
## Confirm
1. Query `sum(rate(oya_itsm_cmdb_relation_freshness_total[5m])) by (tenant_id, cell_tier)` and identify the affected tenant and cell.
...
## Recover
- Restore normal admission when cmdb-relation-freshness is below 1x burn for two consecutive 30 minute windows.
- Backfill missing audit evidence before resolving the incident.
- Attach dashboard snapshot `itsm-local-cmdb-relation-freshness` and policy file name to the ticket.
