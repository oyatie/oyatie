...
entry_gate: |
  ...
  microservices/foundry-supervisor/src/crates/; foundry-runtime + foundry-providers
...
exit_gate: |
  ...
  exits 0; supervisor's own OpenSLO manifest at microservices/foundry-supervisor/slos/
...
|---|---|---|
| `foundry-supervisor` | all 5 BCs | All under `microservices/foundry-supervisor/` per ADR-0131 |

Plus cross-cutting:
- `microservices/foundry-supervisor/slos/{kill-switch-engage.openslo.yaml, deployment-admit.openslo.yaml, supervision-event-lag.openslo.yaml, autonomy-policy-eval.openslo.yaml}` — supervisor's own SLO manifests
...
- Cross-region (multi-pack) federation of fleet state — deferred to a post-M01 ADR; M01 ships per-pack pinning per `policy/data-residency.md`.
...
| [`IP-012-supervisor-self-slos.md`](IP-012-supervisor-self-slos.md) | Author OpenSLO manifests at `microservices/foundry-supervisor/slos/`; observability gate observes | pending | axis-foundry-control-plane + axis-observability | IP-011 |
...
Each IP emits a ChangeSet per ADR-0110 at `microservices/foundry-supervisor/evidence/multispectrum/<change_id>-<unix_ts>.json` validated by `oya-governance-multispectrum-evidence` lane.
...
cargo run -p oya-dev-cli -- vcs claim \
  ...
  --paths "microservices/foundry-supervisor/src/crates/<crate>/**"
