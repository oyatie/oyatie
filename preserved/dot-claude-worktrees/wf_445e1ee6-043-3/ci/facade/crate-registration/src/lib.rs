...
/// The READ-ONLY half of [`register_crate`]: load the live SSOT snapshot and run the pure kernel
/// planner, stopping BEFORE dispatch. Nothing on disk is touched.
///
/// This is the gate-facing invocation surface. [`register_crate`] itself MUTATES the hand-authored
/// SSOTs (OWNERS, the workspace member globs, the capability registry, the owning ADR's governed
/// paths, the catalog, the reachability registry), so it can never run inside presubmit and is not
/// a materializer producer either — the materializer only re-derives generated faces and must not
/// rewrite human merge surfaces. A gate consumes THIS function instead: an EMPTY plan for an
/// already-registered crate is the born-accounting completeness assertion, and it exercises every
/// loader (capability registry, OWNERS resolution, member globs, ADR corpus, catalog, reachability,
/// `git ls-files`) against the live corpus.
///
/// # Errors
/// [`RegisterError`] on a loader IO failure or a kernel plan refusal — the same typed refusals
/// [`register_crate`] surfaces, because it is the same code path.
pub fn plan_registration(
    repo_root: &Path,
    req: &RegisterCrateRequest,
) -> Result<RegistrationPlan, RegisterError> {
    load_and_plan(repo_root, req).map(|(_, _, plan)| plan)
}

/// Load the live SSOT snapshot and compute the plan, returning the loader outputs the dispatch
/// half also needs (the resolved config + the tracked-path universe).
fn load_and_plan(
    repo_root: &Path,
    req: &RegisterCrateRequest,
) -> Result<(OyaCiConfig, Vec<String>, RegistrationPlan), RegisterError> {
    ...
    let capabilities = load_capability_set(repo_root)?;
    let tracked_paths = list_tracked_paths(repo_root)?;
    let current = load_current_state(repo_root, &cfg, req, &tracked_paths)?;

    // --- PLAN: the pure kernel computes the ordered upsert diff ---
    let plan: RegistrationPlan = plan_register_crate(req, &current, &capabilities)?;
    Ok((cfg, tracked_paths, plan))
}
...
pub fn register_crate_detailed(repo_root: &Path, req: &RegisterCrateRequest) -> RegisterOutcome {
    let (cfg, tracked_paths, plan) = match load_and_plan(repo_root, req) {
        Ok(loaded) => loaded,
        Err(error) => {
...
pub fn list_tracked_paths(repo_root: &Path) -> Result<Vec<String>, RegisterError> {
