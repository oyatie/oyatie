...
// ─────────────────────────────────────────────────────────────────────────────────────────────
// Merge-driver registration liveness (live corpus).
//
// The same "declared automation must resolve to a real command" predicate applied to
// `.gitattributes`. Producer-independent: it reads the tracked files off the working tree, the
// way the sibling root-workspace-hygiene gate does, because both inputs ARE tracked source.
//
// This does NOT probe `tools/hooks/bin/*` for executability. That directory is gitignored, so on
// any clean checkout the probe would be red for a reason that is not a defect — it would measure
// build state, not wiring state. What is asserted is the static chain that a fresh clone can
// still get wrong: declared attribute -> versioned registration -> stable non-buck-out path ->
// a rust_binary target that produces that name -> a command guarded against an absent binary.
// ─────────────────────────────────────────────────────────────────────────────────────────────

use ci_hook_wiring::merge_driver_registration::{
    self, REGISTRATION_FILE, declared_drivers, registered_drivers,
};

fn read_tracked(root: &Path, rel: &str) -> String {
    let path = root.join(rel);
    std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()))
}

/// Every `rust_binary` target name declared under `tools/`, i.e. the binaries a
/// `buck2 build <target> --out tools/hooks/bin/<name>` can actually produce.
///
/// Scans `tools/*/BUCK` only: all merge drivers live there, and a driver registered against a
/// binary outside `tools/` would be a separate reviewed change. A miss here is fail-CLOSED — the
/// gate reports `merge_driver_missing_build_target` rather than passing vacuously.
fn tools_rust_binary_names(root: &Path) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    let entries = std::fs::read_dir(root.join("tools")).expect("read tools/");
    for entry in entries {
        let buck = entry.expect("tools/ dir entry").path().join("BUCK");
        let Ok(text) = std::fs::read_to_string(&buck) else {
            continue;
        };
        for block in text.split("rust_binary(").skip(1) {
            let Some((_, after_name)) = block.split_once("name") else {
                continue;
            };
            let Some((_, quoted)) = after_name.split_once('"') else {
                continue;
            };
...
fn codes(findings: BTreeSet<ci_hook_wiring::Finding>) -> BTreeSet<String> {
    findings.into_iter().map(|finding| finding.code).collect()
}

#[test]
fn merge_driver_registration_is_green_on_the_live_corpus() {
    let root = repo_root();
    let gitattributes = read_tracked(&root, ".gitattributes");
    let gitconfig = read_tracked(&root, REGISTRATION_FILE);
    let binaries = tools_rust_binary_names(&root);

    // Validate the instrument before trusting its verdict: a scanner that found nothing would
    // make every driver look unbuildable, and an empty attribute parse would make the GREEN
    // direction vacuous.
    assert!(
        binaries.len() > 3,
        "rust_binary scan under tools/ returned {} names — the scanner is broken, not the repo",
        binaries.len()
    );
    let declared = declared_drivers(&gitattributes);
    assert!(
        !declared.is_empty(),
        ".gitattributes declares custom merge drivers; the parser found none"
    );

    eprintln!(
        "MERGE-DRIVER live corpus: declared={declared:?} registered={:?} tools_binaries={}",
        registered_drivers(&gitconfig).keys().collect::<Vec<_>>(),
        binaries.len()
    );

    let findings = merge_driver_registration::evaluate_keyed(&gitattributes, &gitconfig, &binaries);
    assert!(
        findings.is_empty(),
        "every custom merge driver in .gitattributes must resolve to a buildable, guarded, \
         stable-path command registered in {REGISTRATION_FILE}: {findings:#?}"
    );
}

#[test]
fn merge_driver_registration_is_red_on_the_measured_regressions() {
    let root = repo_root();
    let gitattributes = read_tracked(&root, ".gitattributes");
    let gitconfig = read_tracked(&root, REGISTRATION_FILE);
    let binaries = tools_rust_binary_names(&root);

    // 1. The state this gate was written for: the registration payload absent entirely.
    let dropped = merge_driver_registration::evaluate_keyed(&gitattributes, "", &binaries);
    assert!(
        codes(dropped).contains("merge_driver_unregistered"),
        "an unregistered declared driver must be RED"
    );

    // 2. The literal shape found in git config on 2026-07-31: an absolute buck-out path inside a
    //    worktree that no longer exists.
    let dead = gitconfig.replace(
        "./tools/hooks/bin/",
        "/Users/x/wt/buck-out/v2/art/root/abc/tools/",
    );
    assert!(
        codes(merge_driver_registration::evaluate_keyed(
            &gitattributes,
            &dead,
            &binaries
        ))
        .contains("merge_driver_unstable_binary_path"),
        "a buck-out / absolute driver path must be RED"
    );

    // 3. A new driver attribute landing with no registration behind it.
    let extra = format!("{gitattributes}\nsome/new/ledger.jsonl merge=not-registered\n");
    assert!(
        codes(merge_driver_registration::evaluate_keyed(
            &extra, &gitconfig, &binaries
        ))
        .contains("merge_driver_unregistered"),
        "adding a merge= attribute without registering the driver must be RED"
    );

    // 4. Dropping the absent-binary guard: the one-sided-%A data-loss shape.
    let unguarded = gitconfig
        .replace("test -x ./tools/hooks/bin/", "true ./tools/hooks/bin/")
        .replace("git merge-file", "true");
    assert!(
        codes(merge_driver_registration::evaluate_keyed(
            &gitattributes,
            &unguarded,
        ...
        .contains("merge_driver_absent_binary_unguarded"),
        "an unguarded driver command must be RED"
    );
}
