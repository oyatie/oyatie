...
`git config merge.friction-ledger.driver` line pointing at a build output: the only registration
this driver ever had named a `buck-out/` artifact inside a worktree that was later deleted, so
git fell back to the plain text merge with no error for as long as that path was dead.
...
buck2 build //tools/oya-friction-ledger-merge-driver-app:oya-friction-ledger-merge-driver \
  --out tools/hooks/bin/oya-friction-ledger-merge-driver
```

`ci/facade/hook-wiring` fails closed if the `.gitattributes` declaration, the versioned
registration and the build target ever disagree — including a re-introduced `buck-out/` path.
