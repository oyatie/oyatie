...
purpose: |
  ...
  hand-back signal.
lift_target: oyatie/docs/agents/HUMAN-OPERATOR-GUIDE.md
...
> Read this only when an agent has halted with `BLOCKED_ON_HUMAN_ORCHESTRATOR` (per [`ESCALATION-MATRIX.md`](ESCALATION-MATRIX.md)). Steady-state autonomy is the default; human intervention is a rare exception.
...
2. If approved: author the Cedar policy diff per [`docs/standards/autonomy-ceiling.md`](../../docs/standards/autonomy-ceiling.md); attach to a fresh PR.
3. Emit hand-back:
   ```
...
2. If grit 0.3.0 bug recurrence (RM-05): apply known workaround per [`docs/standards/claude-code-harness.md`](../../docs/standards/claude-code-harness.md); update `MFL-NNNN` if novel shape.
...
Full RACI: [`docs/RACI-OWNERSHIP.md`](../../docs/RACI-OWNERSHIP.md).
