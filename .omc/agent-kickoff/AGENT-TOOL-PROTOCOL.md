...
lift_target: oyatie/docs/agents/AGENT-TOOL-PROTOCOL.md
...
related:
  ...
  - .omc/standards/claude-code-harness.md
  - .omc/standards/agent-instructions-discipline.md
  - .omc/standards/git-workflow.md
---
...
`grit` is the only sanctioned merge primitive. Never invoke `git rebase`, `git merge`, `git push`, `gh pr merge` to land work — let `grit done` orchestrate the merge queue (per [`docs/DESIGN.md`](../../docs/DESIGN.md) §3.0.5.2).
