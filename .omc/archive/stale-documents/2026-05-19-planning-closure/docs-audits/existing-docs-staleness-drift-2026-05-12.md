...
**Hyperscaler-best-practices adoption gaps:** Three top-rank items from `docs/standards/hyperscaler-best-practices.md` are undocumented: build-artifact signing/SBOM/SLSA (missing `oya-governance-supply-chain` lane); progressive-delivery rail (no feature-flag substrate); cargo-vet audit trail (missing from standards). Update TOOLCHAIN.md systematically.
