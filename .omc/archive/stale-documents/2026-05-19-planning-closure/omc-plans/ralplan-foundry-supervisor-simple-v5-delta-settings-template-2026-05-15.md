...
| `SecretStorePort` impl (in-memory ref) on `OpenBaoAdapter` | `crates/oya-intelligence-account-adapter-openbao/src/lib.rs` | 26 (impl) + 88-164 (10 unit tests) | Spawn-time env-var injection resolves `sref://...` through this port at supervisor-app run-time; renderer never calls it. |
