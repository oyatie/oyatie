...
| Unit (serde round-trip — **v3 diff #10**) | `tests/provider_family_round_trip.rs::toml_string_OpenAIOrCodex_decodes_to_enum_variant_OpenAiOrCodex` | `cargo test -p oya-intelligence-supervisor-conformance --test provider_family_round_trip` |
...
| **`oya-intelligence-account-adapter-openbao` `SecretStorePort`** (**v4 patch #4**) | `crates/oya-intelligence-account-adapter-openbao/src/lib.rs` | 26 impl + 88–164 tests | Adapter present (verified `OpenBaoAdapter: SecretStorePort` in-memory ref impl, 10 unit tests at `crates/oya-intelligence-account-adapter-openbao/src/lib.rs:26+88-164`). Network-OpenBao upgrade tracked separately. |
...
| `oya-governance-banned-primitives` binary | `tools/oya-governance-banned-primitives/src/main.rs` | L15 (existing) | Real binary; pattern model for new gate bins (v3 diff #8). |
...
| `oya-governance-predictable-naming-kernel` binary | (**no `main.rs`** — library only) | — | **Plan must NOT invoke as `cargo run`.** See §B.11 row 18 substitution (v3 diff #8). New binary `tools/oya-governance-predictable-naming/src/main.rs` is declared by this plan in §B.1 (directory does not yet exist; created as scaffold artifact). |
...
| **`tools/oya-governance-predictable-naming/`** (NEW binary scaffold, v4 patch #1) | Directory does not yet exist (verified `ls tools/`); declared as scaffold artifact. Binary `tools/oya-governance-predictable-naming/src/main.rs` wraps `oya-governance-predictable-naming-kernel::check()`. Pattern model: `tools/oya-governance-banned-primitives/src/main.rs:15`. |
...
**v4 patch #7 — `AccountSnapshotProvider` production wiring:** Production impl: composite over `oya-intelligence-account-adapter-{claude,codex,gemini}` reading `registry/accounts/*.toml`. Test impl: in-memory `Vec<ProviderAccount>` shim. The `registry/accounts/` directory is a new scaffold artifact (§B.10 row).
...
| **`tools/oya-governance-predictable-naming/` binary scaffold (v4 patch #1)** | `tools/oya-governance-predictable-naming/src/main.rs` — new binary scaffold; directory does not yet exist. Wraps `oya-governance-predictable-naming-kernel::check()`. Pattern model: `tools/oya-governance-banned-primitives/src/main.rs:15`. |
...
| 7.7 | **ProviderFamily round-trip (v3 diff #10)** | `cargo test -p oya-intelligence-supervisor-conformance --test provider_family_round_trip` |
...
| **18 — v4 patch #1** | Predictable-naming (new `[[bin]]` target `oya-governance-predictable-naming` — grit unit #49 in §B.12; pattern model `tools/oya-governance-banned-primitives/src/main.rs:15`) | `cargo run -p oya-governance-predictable-naming -- --check crates/oya-intelligence-supervisor-{kernel,app,conformance} crates/oya-intelligence-jsonl-supervisor-adapter` |
...
| 38 | `crates/oya-intelligence-supervisor-conformance/build.rs::main` | M02-P06 | Emits T-tier into seed (v3 diff #7) |
...
| 40 | `crates/oya-intelligence-supervisor-conformance/tests/provider_family_round_trip.rs::toml_string_round_trips_to_enum_variant_for_all_four_families` | M02-P06 | v3 diff #10 |
...
| 49 | `tools/oya-governance-predictable-naming/src/main.rs::main` | M02-P06 | **v4 patch #1** — new `[[bin]]` target wrapping `oya-governance-predictable-naming-kernel::check()`. Unconditional grit claim. Pattern model: `tools/oya-governance-banned-primitives/src/main.rs:15`. |
...
| 14 | §B.0 row "OpenBao adapter" + §B.10 row "OpenBao default" + open-questions | OpenBao constraint implicit | Explicit row: secrets via `oya-intelligence-account-adapter-openbao` `SecretStorePort`; supervisor + drivers never in-line secrets |

### §E.v4 — Iteration 4 change log (7 narrow patches, addendum to §E.v3)

| Patch # | Section touched | Before (v3) | After (v4) |
|---------|----------------|-------------|-----------|
| 1 | §B.11 row 18 + §B.12 unit #49 + §B.1 + §B.10 | Row 18 carried `Option A (preferred)` + `Option B (fallback): cargo test …`; §B.12 unit #49 marked "ONLY claimed if Option A path is taken … drops to grit unit #49(unused) … if Option B fallback is taken" | Row 18 collapsed to single Option A command: `cargo run -p oya-governance-predictable-naming -- --check crates/oya-intelligence-supervisor-{kernel,app,conformance} crates/oya-intelligence-jsonl-supervisor-adapter`. §B.12 unit #49 unconditional (Option B language deleted). New scaffold artifact `tools/oya-governance-predictable-naming/` declared in §B.1 + §B.10 (verified directory absent via `ls tools/`; pattern model `tools/oya-governance-banned-primitives/src/main.rs:15`) |
...
| 7 | §B.2.5 + §B.10 | `AccountSnapshotProvider` had no production-wiring note; `registry/accounts/` undeclared as scaffold | §B.2.5: "Production impl: composite over `oya-intelligence-account-adapter-{claude,codex,gemini}` reading `registry/accounts/*.toml`. Test impl: in-memory `Vec<ProviderAccount>` shim." §B.10: new `registry/accounts/` scaffold-artifact row (verified directory absent via `ls registry/`) |
...
- **New binaries:** 1 (`tools/oya-governance-predictable-naming` — v4 patch #1).
...
- **3 critical RISKs declared with mitigation lanes:** torn-write, hung-CLI, poison-message. OpenBao adapter RISK retired in v4 (adapter verified present, `crates/oya-intelligence-account-adapter-openbao/src/lib.rs:26+88-164`).
