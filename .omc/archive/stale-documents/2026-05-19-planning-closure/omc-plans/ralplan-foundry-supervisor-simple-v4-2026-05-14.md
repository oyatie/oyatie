...
| **18 — v4 patch #1** | Predictable-naming (new `[[bin]]` target `oya-governance-predictable-naming` — grit unit #49 in §B.12; pattern model `tools/oya-governance-banned-primitives/src/main.rs:15`) | `cargo run -p oya-governance-predictable-naming -- --check crates/oya-foundry-supervisor-{kernel,app,conformance} crates/oya-intelligence-jsonl-supervisor-adapter` |
