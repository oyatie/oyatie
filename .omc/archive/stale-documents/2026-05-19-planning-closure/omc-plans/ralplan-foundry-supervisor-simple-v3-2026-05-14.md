...
| `oya-foundry-fitness-banned-primitives` binary | `tools/oya-foundry-fitness-banned-primitives/src/main.rs` | L15 (existing) | Real binary; pattern model for new gate bins (**diff #8**). |
...
| **18 — diff #8** | Predictable-naming (substituted: `oya-governance-predictable-naming-kernel` is library-only; pattern model `oya-foundry-fitness-banned-primitives` exists at `tools/oya-foundry-fitness-banned-primitives/src/main.rs:15`) | Option A (preferred): declare new `[[bin]]` target `oya-foundry-fitness-predictable-naming` (grit unit #46 in §B.12) wrapping the lib `fn check()`; gate becomes `cargo run -p oya-foundry-fitness-predictable-naming -- --check crates/oya-foundry-supervisor-{kernel,app,conformance} crates/oya-intelligence-jsonl-supervisor-adapter`. Option B (fallback): `cargo test -p oya-governance-predictable-naming-kernel -- --test-threads=1` and assert exit 0. |
