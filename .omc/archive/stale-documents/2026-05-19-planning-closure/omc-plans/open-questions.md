...
- [ ] **Conformance-crate location** — Stand up `oya-foundry-supervisor-conformance` as a fourth (test-only) crate or fold it into `supervisor-app/tests/` to honor the "three new crates" cap? — Touches §B.1 and CI lane wiring.
...
- [ ] **`oya-intelligence-api` future** — Plan §2.1 surfaces ambiguity: rename to a distinct feature (e.g. `oya-foundry-meta-api`) or merge into the existing `oya-intelligence-policy-api`. Architect call. — Blocks Sub-plan B step 1.
...
- [ ] OpenBao adapter sync/async surface — Confirm `oya-foundry-account-adapter-openbao` exposes sync `dereference` (kernel-compatible) or whether the supervisor runtime must wrap async — affects worker-spawn latency budget in IP-007/008/009.
- [ ] Conformance crate placement — Decide if `oya-foundry-supervisor-conformance` lives under `crates/` (regular crate, counted in `cargo deny`) or under `tools/` (runner, dev-only) — impacts external-dep accounting.
...
- [ ] Cron-parser ownership — Decide whether the hand-rolled cron parser lives in `oya-foundry-supervisor-adapter-cron-tokio` (inlined) or gets promoted to `oya-foundry-cron-kernel` for cross-Foundry reuse.
...
- [ ] Driver-crate rename ADR ordering — Rename ADR (`oya-foundry-account-adapter-{claude-code,codex-cli,gemini-cli}` → `oya-foundry-{claude,codex,gemini}-account-adapter`) is prerequisite to this plan; must it merge before any supervisor work begins, or land in the same PR train? Recommendation: separate ADR merges first.
...
- [ ] Conformance crate placement — Should `oya-foundry-supervisor-conformance` live under `crates/` (regular crate, counted in `cargo deny`) or under `tools/` (dev-only runner)? Plan currently places it under `crates/`. Impacts external-dep accounting and CI lane coverage.

## ralplan-foundry-supervisor-simple-v3 — 2026-05-14

**Carried forward from v2 (still open after iteration 3):**

- [ ] Driver-crate rename ADR ordering — Rename ADR (`oya-foundry-account-adapter-{claude-code,codex-cli,gemini-cli}` → `oya-foundry-{claude,codex,gemini}-account-adapter`) is still prerequisite. Must it merge before any supervisor work begins, or land in the same PR train? Recommendation: separate ADR merges first. (Unchanged by v3.)
...
- [ ] Diff #14 OpenBao adapter readiness — Plan declares `oya-foundry-account-adapter-openbao` `SecretStorePort` as the canonical secret backend. If this crate is not yet real at execution time (RISK: not verified in v3 due to time-boxed inventory), what is the bootstrap order? Recommendation: executor stops, opens an ADR to bootstrap `oya-foundry-account-adapter-openbao`, then resumes; supervisor MUST NOT in-line secret material as a workaround.
