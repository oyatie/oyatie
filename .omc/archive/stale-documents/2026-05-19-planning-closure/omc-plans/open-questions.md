...
- [ ] **Codex CLI stop-hook surface** — Does the Codex CLI ship a native stop-hook today, or do we ship the stdout-sentinel + exit-code fallback and downgrade `autonomy_tier` (T3→T2)? — Blocks IP-D claim-done and ADR-00ZZ ratification.
- [ ] **Gemini CLI stop-hook surface** — Same question for Gemini CLI; need verified citation to release notes before claiming parity. — Blocks IP-D claim-done.
- [ ] **`projected_tokens` source of truth at reservation time** — Do we estimate from the inbox `body_ref` length, from a per-`SessionTicket` running EMA, or accept a producer-supplied hint? Affects `usage_window_kernel.try_reserve_for` precision and false-deny rate. — Blocks IP-C surface freeze.
- [ ] **Watchdog default (90 s) vs per-provider override** — Should `HeartbeatPolicy.watchdog_timeout_seconds` be a single global default or per-`ProviderFamily` (Anthropic streams differently from Gemini)? — Blocks IP-A trait freeze.
- [ ] **Dead-letter retention + replay policy** — Auto-purge after N days, manual operator replay through dashboard, or both? Affects `dashboard-kernel` row schema. — Blocks IP-E projection schema lock.
- [ ] **`SessionTicket` issuance for cron idle-tick** — Idle ticks have no inbox row; should we mint a synthetic `MessageId` or use a reserved sentinel value that bypasses reservation? Either way the audit chain must show it. — Blocks IP-E daemon impl.
- [ ] **Conformance-crate location** — Stand up `oya-intelligence-supervisor-conformance` as a fourth (test-only) crate or fold it into `supervisor-app/tests/` to honor the "three new crates" cap? — Touches §B.1 and CI lane wiring.
- [ ] **Per-account vs per-ticket cost-ceiling** — `oya-cloud-billing-kernel` reservations are usually per-account; for cross-account fan-out we may need per-ticket sub-reservation. Confirm whether existing kernel API supports it or needs a third surface expansion. — Risk to "zero net-new surface beyond 2 expansions" claim.
...
- [ ] **Skeleton state of `oya-intelligence-account-*` crates** — are `Cargo.toml` deps already pointing in the clean-arch direction, or do they need re-wiring? — Affects estimate for P00-01..P00-08.
...
- [ ] **`oya-intelligence-api` future** — Plan §2.1 surfaces ambiguity: rename to a distinct feature (e.g. `oya-intelligence-meta-api`) or merge into the existing `oya-intelligence-policy-api`. Architect call. — Blocks Sub-plan B step 1.
...
- [x] **EDIT 4 — cargo-semver-checks pinning** — v3 §8.1 Semver-checks row pinned to `cargo-semver-checks 0.46.0` + `--format json > /tmp/semver-output.json` + new fitness lane `oya-governance-rename-baseline-reset` defined as the BASELINE-RESET classifier (scaffolded in Shard 0 step 7b). — Status: CLOSED.
- [x] **EDIT 5 — Freeze-enforcement primitive** — v3 §6 R2 rewrites with new fitness lane `oya-governance-rename-freeze-window` (scaffolded in Shard 0 step 7a). Config = freeze_active + freeze_end_ts + expedite_override_token (single-use rotation). Merge-queue calls lane on dequeue. Architect residual #1 closed as byproduct. — Status: CLOSED.
...
3. The `oya-governance-rename-baseline-reset` lane's classifier — does it correctly distinguish a name-only delta from a real semver delta? Needs the lane's algorithm spec.
...
- [x] **RESIDUAL 1 — HIGH: BNF grammar violation in new lane crate names** — The two new fitness-lane crates introduced in the iter-2 fold (`oya-governance-rename-freeze-window` and `oya-governance-rename-baseline-reset`) violated `docs/standards/crate-naming-convention.md §2` BNF: missing role token and 3-token capability tail (cap is 2). Renamed both per Architect-recommended fix, dropping the `rename-` prefix and appending `-kernel` role token: `oya-governance-freeze-window-kernel` and `oya-governance-baseline-reset-kernel`. Both parse as feature=`fitness-<*>` (3 tokens at feature cap), role=`kernel`, no capability (forbidden for kernels per constraint 4). Updated all 7 occurrence sites in v3 (§1 inventory, §5.0 ADR-exception, §5.1 Shard 0 steps 7a/7b, §6 R2, §8.1 semver gate, §10 open-questions excerpt, §11 ADR-0055 protocol). Also added 3 new feature-compound entries to `[workspace.metadata.oya].compound_features` in §3.2 to satisfy `crate-naming-convention.md §6` rule 1: `fitness-architecture-conventions`, `fitness-freeze-window`, `fitness-baseline-reset`. — Status: CLOSED.

- [x] **RESIDUAL 2 — MODERATE: `oya-tooling-agent-read lane-config set` write-through-read** — §6 R2 had used the READ-named sanctioned-primitive (`oya-tooling-agent-read`, triad READ slot per `git-workflow.md §2-3`) for a write operation (lane-config token mint). Chose OPTION A per Architect recommendation: routed the token-mint through `icm store` (already a sanctioned primitive triad member; the same operation auto-satisfies the Directive 12 rationale-row requirement). Token mint via `icm store -t lane-config-oyatie -c "freeze_window:expedite_token=$(uuidgen)" -i critical -k "lane=oya-governance-freeze-window-kernel"`; lane runtime reads via `icm recall ... | jq -r '.[] | select(.content | contains("expedite_token=")) | .content | sub(".*expedite_token=";"")'`; rotation via tombstone row `expedite_token=REVOKED` (latest-row-wins). No §5.0 sanctioned-primitive triad table needed editing (the table is in `git-workflow.md`, not v3). — Status: CLOSED.
...
2. **`oya-governance-freeze-window-kernel` lane** + ICM
...
- [ ] **Canonical-decision-tree per-crate `src/`-inspection audit (top
  ...
  - **(1b) Per-`*-api` protocol classification**: ~22 crates need
    ...
    for OTLP; `oya-intelligence-rag-api` for streaming retrieval;
...
## ralplan-foundry-subscription-autonomy-supervisor — 2026-05-14

- [ ] Codex CLI `--json-output` behavior — Confirm current OSS Codex CLI emits `{"type":"session_end"}` on `--json-output`; if not, the sentinel-only path becomes primary (changes driver risk class from MEDIUM to HIGH; affects IP-008 acceptance bar).
- [ ] OpenBao adapter sync/async surface — Confirm `oya-intelligence-account-adapter-openbao` exposes sync `dereference` (kernel-compatible) or whether the supervisor runtime must wrap async — affects worker-spawn latency budget in IP-007/008/009.
- [ ] Conformance crate placement — Decide if `oya-intelligence-supervisor-conformance` lives under `crates/` (regular crate, counted in `cargo deny`) or under `tools/` (runner, dev-only) — impacts external-dep accounting.
- [ ] Dashboard-e2e write-rejection inheritance — Confirm `tools/oya-dashboard-e2e` can host the supervisor’s P02 negative-test (POST/PUT/DELETE → 405) without framework changes.
- [ ] Cron-parser ownership — Decide whether the hand-rolled cron parser lives in `oya-intelligence-supervisor-adapter-cron-tokio` (inlined) or gets promoted to `oya-intelligence-cron-kernel` for cross-Foundry reuse.
- [ ] M02 acceptance-gate clarification — Confirm whether the "3 × 2 = 6 cells live-smoke" gate counts subscription-mode-via-supervisor as the subscription cell, or whether the supervisor is purely additive to an already-passing gate.
- [ ] `nix` dependency for PTY fallback — Confirm ADR-0099 (`nix` exception, gated behind `pty` feature) is acceptable to the Engineering-Excellence-Council, or if Gemini-CLI v0.3 minimum-floor should be the *only* supported path (which would let us drop the ADR and the feature flag).

## ralplan-foundry-supervisor-simple-v2 — 2026-05-14

- [ ] Cedar policy bootstrap — Zero `.cedar` files exist in the workspace today; supervisor capability rows carry `Cedar policy path = TBD`. Decide: block this plan on `ADR-cedar-policy-bootstrap`, or proceed with autonomy-ceiling-only enforcement + follow-up ADR. Recommendation: proceed; track risk.
- [ ] Driver-crate rename ADR ordering — Rename ADR (`oya-intelligence-account-adapter-{claude-code,codex-cli,gemini-cli}` → `oya-foundry-{claude,codex,gemini}-account-adapter`) is prerequisite to this plan; must it merge before any supervisor work begins, or land in the same PR train? Recommendation: separate ADR merges first.
- [ ] `api-rest-adapter` stub future — By side-stepping `oya-intelligence-api-rest-adapter` (currently 14-line stub) via direct hyper, do we commit it to remain a stub until M02-P04 lands its own real router? Recommendation: stays stub until M02-P04.
- [ ] Gemini idempotency drift — If a future Gemini SDK release ships an idempotency key, who is responsible for re-running the conformance crate to re-measure the gemini-driver tier? Recommendation: lean-a5-doc-coverage row in the gemini-driver crate.
- [ ] Live smoke matrix gating — Canonical way to gate `$OYA_LIVE_SMOKE=1` in CI without burning provider credit on every PR. Recommendation: nightly-only lane.
- [ ] `cargo public-api` tooling availability — Lean-a10 snapshot uses `cargo public-api`, not in workspace tooling. Either install it CI-side (counts as tooling dep, not workspace dep — does NOT violate Branch Y) or substitute `rustdoc --output-format json` + `jq`. Recommendation: ADR to install `cargo public-api`.
- [ ] `projected_tokens_p95` cold-start window — Plan says fallback = `cost_ceiling_tokens` when n<10 samples exist. Is 10 samples the right floor, or should it be higher (e.g. n<50)? Affects cold-start safety margin for fresh installs.
- [ ] Supervisor degrade authority — Plan calls `ProviderAccount::degrade(reason)` directly on 3 consecutive `ReserveBreached` verdicts. Is that authority appropriately T3PropAct, or does autonomous account-state mutation require T4Actuate ceiling? Recommendation: T3PropAct (reversible via `recover()`); confirm with capability-registry council.
- [ ] Conformance crate placement — Should `oya-intelligence-supervisor-conformance` live under `crates/` (regular crate, counted in `cargo deny`) or under `tools/` (dev-only runner)? Plan currently places it under `crates/`. Impacts external-dep accounting and CI lane coverage.

## ralplan-foundry-supervisor-simple-v3 — 2026-05-14

**Carried forward from v2 (still open after iteration 3):**

- [ ] Driver-crate rename ADR ordering — Rename ADR (`oya-intelligence-account-adapter-{claude-code,codex-cli,gemini-cli}` → `oya-foundry-{claude,codex,gemini}-account-adapter`) is still prerequisite. Must it merge before any supervisor work begins, or land in the same PR train? Recommendation: separate ADR merges first. (Unchanged by v3.)
- [ ] `api-rest-adapter` stub future — Side-stepping the 14-line stub via direct hyper persists in v3 §B.7. Does this commit `oya-intelligence-api-rest-adapter` to remain a stub until M02-P04 lands a real router? Recommendation: stays stub until M02-P04. (Unchanged by v3.)
- [ ] Gemini idempotency drift — If a future Gemini SDK release ships an idempotency key, who re-runs the conformance crate? Recommendation: lean-a5 doc-coverage row in the gemini-driver crate. (Unchanged by v3.)
- [ ] Live smoke matrix gating — Canonical way to gate `$OYA_LIVE_SMOKE=1` in CI without burning provider credit on every PR. Recommendation: nightly-only lane. (Unchanged by v3.)
- [ ] `cargo public-api` tooling availability — Lean-a10 snapshot tool not in workspace. Either install CI-side (tooling dep, not workspace dep) or substitute `rustdoc --output-format json` + `jq`. Recommendation: ADR to install `cargo public-api`. (Unchanged by v3.)
- [ ] `projected_tokens_p95` cold-start window — Plan still says fallback = `cost_ceiling_tokens` when n<10. Confirm 10 vs 50 sample floor for cold-start safety on fresh installs. (Unchanged by v3.)
- [ ] Supervisor degrade authority — Plan still calls `ProviderAccount::degrade(reason)` on 3 consecutive `ReserveBreached`. T3PropAct (reversible via `recover()`) vs T4Actuate decision still pending capability-registry council confirmation. (Unchanged by v3.)
- [ ] Conformance crate placement — `crates/` vs `tools/` decision still open in v3 §B.1 (currently under `crates/`). Impacts `cargo deny` accounting + CI lane coverage. (Unchanged by v3.)

**Resolved by v3 (no longer open):**

- [x] Cedar policy bootstrap — RESOLVED. v3 diff #1 verified `docs/policies/autonomy-ceiling.cedar` exists (3-line M02-P05-IP-002 seed). ADR refocused as `ADR-cedar-policy-extend-supervisor-capabilities`; new file `docs/policies/foundry-supervisor.cedar` extends the seed. Cedar fragment coverage gate (B.11 row 21) added.

**New in v3 (iteration 3 specific):**

- [ ] Diff #6 phase-slot collision — Plan carves `M02-P06-foundry-supervisor` after verifying P00..P05 occupy the M02 phases directory and P06 is free at scan time. If a parallel claim races and lands P06 first, executor falls back to P07. Decide: should this fallback rule be promoted to plan-wide policy (collision detection at claim time) or stay local to this plan? Recommendation: codify as a project-memory directive `phase-slot-collision-fallback-rule`.
- [ ] Diff #3 `AccountSnapshotProvider` port location — Plan places the port inside `oya-intelligence-supervisor-kernel` (private to this plan's surface, no existing-kernel lean-a10 fire). Should it instead live in `oya-intelligence-account-domain` next to `ProviderAccount` for reuse by other apps (route-policy-app, multi-subscription-pool)? Recommendation: keep in supervisor-kernel for v3; promote to account-domain only if a 2nd consumer surfaces.
- [ ] Diff #8 Option A vs B — Plan offers either declaring new `[[bin]]` targets for the doc-catalog + predictable-naming gates (Option A, grit unit #49 in §B.12) OR substituting `cargo test` (Option B). Decide at execution: which path. Option A is cleaner long-term but adds 2 binary surfaces; Option B is pragmatic but couples gate to test framework. Recommendation: Option A for predictable-naming (matches `banned-primitives` pattern at `tools/oya-governance-banned-primitives/src/main.rs:15`); Option B fallback for doc-catalog if `oya-dev-cli gate validate doc-catalog` subcommand is not yet registered.
- [ ] Diff #14 OpenBao adapter readiness — Plan declares `oya-intelligence-account-adapter-openbao` `SecretStorePort` as the canonical secret backend. If this crate is not yet real at execution time (RISK: not verified in v3 due to time-boxed inventory), what is the bootstrap order? Recommendation: executor stops, opens an ADR to bootstrap `oya-intelligence-account-adapter-openbao`, then resumes; supervisor MUST NOT in-line secret material as a workaround.
- [ ] Diff #9 RSS sampling on non-Linux — C.13c reads `/proc/self/status` `VmRSS`, which is Linux-only. CI runs on Linux today, but local devs on macOS will see this row fail. Decide: skip-on-non-Linux annotation in the bench harness, or substitute `mach_task_basic_info` (would add a tooling dep — not workspace dep, but still surface area). Recommendation: skip-on-non-Linux + log a notice; CI authoritative.
- [ ] Diff #13 wildcard sub-claim expansion timing — Audit_chain + matrix_3x2x2 retain wildcard grit patterns (`test_audit_*`, 12-combo matrix) to be expanded at scaffold time. Decide: must the expansion be committed BEFORE the wildcard claim resolves, or is the pattern itself sufficient for parent grit DAG to merge? Recommendation: expansion before resolution; carry a §B.12 footnote in executor IP-001-supervisor.md confirming the rule.

## ralplan-foundry-supervisor-simple-v4 — 2026-05-14

- [ ] v4 patch #6 — `InboxStore::dead_letter()` atomic semantics authority — Plan chose Option β: `dead_letter()` consumes the lock atomically in a single Locked→DeadLettered transition; calling on an unlocked message returns `StoreError::InvalidTransition`. The contract lives in the trait doc-comment in `crates/oya-intelligence-supervisor-kernel/src/lib.rs::InboxStore`. Decide: should this contract also be codified in `crates/oya-intelligence-jsonl-supervisor-adapter`'s `JsonlInbox` impl-level doc-comment (defense-in-depth) or rely solely on the trait? Recommendation: codify in both (trait = contract, adapter = enforcement); add a unit-test row in §B.12 unit #25 asserting `InvalidTransition` is returned when `dead_letter` is called on an unlocked message id.
- [ ] v4 patch #3 — macOS RSS sampling path — C.13c is CI-only (Linux runners, `/proc/self/status` `VmRSS`). macOS dev runs skip via `cfg(target_os="linux")`. Decide whether to add a `cfg(target_os="macos")` sampling path using `mach_task_basic_info` (no workspace-dep impact; pure libc syscall) so macOS dev runs can still report RSS, or leave it Linux-only with explicit skip-and-log. Recommendation: leave Linux-only for v4; reopen if foundry developers on macOS need RSS regression visibility locally.
- [ ] v4 patch #7 — `registry/accounts/` directory ownership — New scaffold artifact declared but the plan does not specify which crate "owns" the directory layout (i.e., who writes the canonical `registry/accounts/*.toml` schema documentation). Decide: should the schema live alongside the production `AccountSnapshotProvider` impl in `oya-intelligence-supervisor-app`, or in `oya-intelligence-account-domain` as a domain-level artifact? Recommendation: schema doc in `docs/foundry/supervisor/operations.md` (already in §B.10 doc-coverage suite); impl in app.
- [ ] Carry-forward (unresolved from v3): Diff #9 RSS sampling on non-Linux — superseded by v4 patch #3 entry above; close upon merging open-questions v4 review.

## ralplan-foundry-supervisor-simple-v5-delta — 2026-05-15

- [ ] verify-codex-and-gemini-config-paths-at-CI-time — `~/.codex/config.toml` + `~/.codex/hooks.json` and `~/.gemini/settings.json` are verified present on the planner's host (real, not assumed). The `lean-settings-drift` CI lane (§B.5) must enforce these paths on every CI runner: if a runner has `CODEX_HOME` or `GEMINI_HOME` set, the renderer's `canonical_paths_for_provider` must match the env-derived path; otherwise fall back to documented defaults. Decide: should the lane HARD-FAIL on mismatch or emit a CI warning + record the divergence into `.omc/state/settings-drift-report.json` for triage? Recommendation: hard-fail on first mismatch; we cannot ship a settings-drift lane that silently tolerates path drift.
- [ ] capture-codex-gemini-hook-event-vocabulary — §B.6 R1/R2 require fixtures captured from `~/.codex/hooks.json` (9.2K, verified present) and `gemini --help` (binary verified at `/opt/homebrew/bin/gemini`). Decide: is capturing fixtures from the planner's host acceptable as "ground truth," or must capture re-run on a fresh CI runner with a pinned CLI version to bind the fixture to a version? Recommendation: capture at planner-host time (Wave 4d) AND require version-pinning via `codex --version` + `gemini --version` recorded in the fixture file header; CI lane re-captures and diffs on version drift.
- [ ] gemini-hook-support-state — v5 ships GeminiRenderer with hooks no-op + capability-seed `hooks_supported = false`. This is the honest-degrade default until Gemini's hook surface is verified. Decide: should v5 also gate Gemini onboarding behind an explicit `--allow-no-hooks` flag in `oya foundry settings render`, or is the capability-seed row sufficient signal? Recommendation: capability-seed row only for v5; subsequent phase decides Gemini's tier after R2 fixture capture.
- [ ] settings-template-versioning-policy — `SettingsTemplate::version: u32` is monotonic. Decide: when the renderer encounters a higher-version template than it knows how to render (forward-compat), should it (a) refuse with `SettingsRendererError::UnsupportedFormat`, (b) render the subset it understands and emit a `DriftEntry::Extra` for unknown fields, or (c) refuse + emit a capability-seed downgrade row? Recommendation: option (a) — refuse explicitly; templates are workspace-owned, so a renderer-template version skew is a CI-blockable workspace bug, not runtime polymorphism.
- [ ] auto-reconcile-mode-default — `VerifyMode::FailOnDrift` vs `VerifyMode::AutoReconcile` (§B.4 wiring). Decide: which is the production default? Recommendation: `FailOnDrift` in CI (so settings drift surfaces as a red lane); `AutoReconcile` in supervisor-app runtime (so a new account row in `registry/accounts/` is rendered on first `tick_once` without manual intervention). Document the split in the ADR consequences section.
