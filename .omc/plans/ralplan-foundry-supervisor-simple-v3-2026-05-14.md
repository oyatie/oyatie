...
| Unit (saturation — **diff #4**) | `tests/saturation.rs::tick_once_returns_saturated_when_in_flight_at_capacity` | `cargo test -p oya-foundry-supervisor-app --test saturation` |
...
| Perf budget | `benches/heartbeat.rs` — 4 metrics split per **diff #9**: C.13a-d | `cargo bench -p oya-foundry-supervisor-app --bench heartbeat -- --save-baseline supervisor-v3` |
...
| `oya-foundry-fitness-predictable-naming-kernel` binary | (**no `main.rs`** — library only) | — | **Plan must NOT invoke as `cargo run`.** See §B.11 row 18 substitution (**diff #8**). |
...
| 7.5 | **Cost-ceiling enforcement (diff #12)** | `cargo test -p oya-foundry-supervisor-app --test lifecycle cost_ceiling_blocks_when_projected_exceeds_ceiling` |
| 7.6 | **Saturation outcome (diff #4)** | `cargo test -p oya-foundry-supervisor-app --test saturation` |
...
| **18 — diff #8** | Predictable-naming (substituted: `oya-foundry-fitness-predictable-naming-kernel` is library-only; pattern model `oya-foundry-fitness-banned-primitives` exists at `tools/oya-foundry-fitness-banned-primitives/src/main.rs:15`) | Option A (preferred): declare new `[[bin]]` target `oya-foundry-fitness-predictable-naming` (grit unit #46 in §B.12) wrapping the lib `fn check()`; gate becomes `cargo run -p oya-foundry-fitness-predictable-naming -- --check crates/oya-foundry-supervisor-{kernel,app,conformance} crates/oya-foundry-jsonl-supervisor-adapter`. Option B (fallback): `cargo test -p oya-foundry-fitness-predictable-naming-kernel -- --test-threads=1` and assert exit 0. |
...
| 10 | `crates/oya-foundry-supervisor-kernel/src/lib.rs::UsageWindowSnapshot` | M02-P06 | Audit-only (diff #5) |
...
| 16 | `crates/oya-foundry-supervisor-kernel/src/lib.rs::SupervisorConfig` | M02-P06 | Diff #4 |
| 17 | `crates/oya-foundry-supervisor-kernel/src/lib.rs::TickOutcome` | M02-P06 | Diff #4 |
...
| 23 | `crates/oya-foundry-supervisor-kernel/src/lib.rs::InboxStore` | M02-P06 | Sync port |
| 24 | `crates/oya-foundry-supervisor-kernel/src/lib.rs::AccountSnapshotProvider` | M02-P06 | **Diff #3 port** |
...
| 33f | `crates/oya-foundry-supervisor-app/tests/lifecycle.rs::cost_ceiling_blocks_when_projected_exceeds_ceiling` | M02-P06 | Diff #12 |
...
| 37 | `crates/oya-foundry-supervisor-app/tests/saturation.rs::tick_once_returns_saturated_when_in_flight_at_capacity` | M02-P06 | Diff #4 |
