...
Phase-0 input: `/Users/jasonlee/oyatie/.omc/specs/deep-dive-oyatie-sst-consolidation.md`
Trace synthesis: `/Users/jasonlee/oyatie/.omc/specs/deep-dive-trace-oyatie-sst-consolidation.md`
...
- **Inputs/preconditions:** Architect verdict ITERATE consumed; Critic findings consumed; lift-source `.omc/specs/adr-draft-grit-icm-sanctioned-primitives.md` exists; `.omc/specs/pre-cutover-drafts-2026-05-12.md §Draft 2` exists; `oyatie/docs/RACI-OWNERSHIP.md` exists (70 rows, verified).
...
- **Symbols to claim:** **Two-track**. Track 1 (doc edits): `icm-lock-p3-docs` — two PRD files in different repos, can proceed concurrently. Track 2 (new fitness-lane crate `oya-foundry-fitness-portfolio-citation-kernel`): **scaffold-claim per ADR-0054** (same sequence as P2 — icm-coordination-lock under `scaffold-locks-oyatie`, scaffold `crates/oya-foundry-fitness-portfolio-citation-kernel/{Cargo.toml, src/lib.rs}` and lane runner at `tools/oya-foundry-fitness-portfolio-citation/`, append to workspace, `grit init`, close window). After window closes, normal grit claims against `crates/oya-foundry-fitness-portfolio-citation-kernel/src/lib.rs::verify` etc.
...
- **Outputs / acceptance evidence:** Both PRD files cross-cite; `cargo run -p oya-foundry-fitness-portfolio-citation-kernel` (or lane runner) green; lane wired into the foundry fitness manifest. Scaffold-lock window closed in icm.
- **Maps to spec criterion:** A1. Citation: ADR-0054 (P0.5).

### P3.5 — Cross-cite ultragoal foundry corpus into canonical foundry SPEC
- **Inputs/preconditions:** P1 merged (foundry corpus rows in inventory classified as KEEP; phantom-path row for `oyatie/.omx/ultragoal/` correctly marked); foundry-salvage output exists at `.omc/specs/foundry-salvage-from-ultragoal-2026-05-12.md` (22KB, verified). `oyatie/docs/products/foundry/PRD.md` exists (75.5KB, verified). `oyatie/docs/products/foundry/PHASE-00-SPEC.md` does NOT exist (verified — new file target).
...
- **Symbols to claim:** **Scaffold-claim per ADR-0054** for new fitness-lane crate `crates/oya-foundry-fitness-authoritative-tracked-kernel/`: lead agent `icm store -t scaffold-locks-oyatie -c "agent=<id> path=crates/oya-foundry-fitness-authoritative-tracked-kernel window=open" -i critical`; scaffold; append to workspace; `grit init`; close window. After window closes, normal grit claims against `crates/oya-foundry-fitness-authoritative-tracked-kernel/src/lib.rs::check` etc. Plus `icm-lock-p10-docs` for any companion doc updates.
...
- **Outputs / acceptance evidence:** `oya-foundry-fitness-authoritative-tracked-kernel` lane green; any `.gitignored` paths that held authoritative state are either committed or demoted (with the demotion logged in the inventory ledger). Scaffold-lock window closed in icm.
...
- **ADR-0052 (inventory)** ← `.omc/plans/deep-dive-inventory-draft.md` (inventory enumeration; lands as `oyatie/docs/decisions/ADR-0052-grit-cutover-inventory.md` at P1).
- **ADR-0053 (sanctioned primitives + cutover direction)** ← `.omc/specs/adr-draft-grit-icm-sanctioned-primitives.md` (lands as `oyatie/docs/decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md` at P5 alongside banned-primitives lane activation; iter-2 edits to lift-source: add bootstrap-window clause to §Decision, add human-orchestrator definition to §Decision Drivers/§Glossary, add cutover-is-one-time-not-retroactive clause to §Consequences §Neutral, unify §Follow-ups with this plan's prior §3 follow-ups).
- **ADR-0054 (scaffold-claim pattern)** ← `.omc/specs/pre-cutover-drafts-2026-05-12.md §Draft 2` (lands as `oyatie/docs/decisions/ADR-0054-grit-scaffold-claim-pattern.md` at P0.5; iter-2 edit: document `Cargo.toml::workspace_members` as NOT viable per verified `grit symbols` output, adopt icm-coordination-lock fallback as canonical).
