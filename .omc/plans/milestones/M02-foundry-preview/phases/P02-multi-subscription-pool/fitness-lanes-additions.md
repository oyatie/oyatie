...
purpose: |
  ...
  authoritative lane definitions land under `.omc/governance-lanes/<lane>.md` once approved
...
## 1. `oya-governance-pool-routing-honor` (BLOCKER)
...
## 2. `oya-governance-tos-acknowledgment` (BLOCKER)
...
- **Implementation:** `tools/oya-governance-tos-acknowledgment/src/main.rs`.
- **Acceptance:** `oya gate validate oya-governance-tos-acknowledgment` exit code 0.

## 3. `oya-governance-upstream-api-drift` (HIGH)
...
- **Implementation:** `tools/oya-governance-upstream-api-drift/src/main.rs` + `.github/workflows/upstream-api-drift.yml`.
- **Acceptance:** lane runs nightly; `oya gate validate oya-governance-upstream-api-drift --dry-run` exit code 0.

## 4. `oya-governance-compat-api-shape-binding` (BLOCKER)
...
When approved, each row above lands as a file under `.omc/governance-lanes/<lane>.md` following the canonical lane-file shape; M01-P08 owns the lift. This delta-spec is the proposal anchor only.
