...
parent: ./INDEX.md
id: M02-P01-IP-001
title: Anthropic Claude adapter (API + subscription)
status: complete
...
# M02-P01-IP-001 — Anthropic Claude adapter (API + subscription)

## Purpose
Ship Anthropic adapter in both API and subscription auth modes.
...
## Icm-store-payload
```
icm store -t context-oyatie -c 'M02-P01-IP-001 Anthropic Claude adapter (API + subscription) shipped; acceptance commands green' -i high -k 'M02-P01-IP-001,complete'
```
