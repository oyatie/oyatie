---
purpose: Auto-backfilled purpose for IP-002-openai-adapter.md
---
...
parent: ./INDEX.md
id: M02-P01-IP-002
title: OpenAI adapter (API + subscription)
status: complete
...
purpose: Ship OpenAI adapter in both API and subscription auth modes.
---

# M02-P01-IP-002 — OpenAI adapter (API + subscription)

## Purpose
Ship OpenAI adapter in both API and subscription auth modes.
...
## Icm-store-payload
```
icm store -t context-oyatie -c 'M02-P01-IP-002 OpenAI adapter (API + subscription) shipped; acceptance commands green' -i high -k 'M02-P01-IP-002,complete'
```
