...
parent: ./INDEX.md
id: M02-P01-IP-004
title: Usage-window kernel + account-route policy
status: complete
...
# M02-P01-IP-004 — Usage-window kernel + account-route policy

## Purpose
Ship usage windows (5h/1wk/project) and account-route policy (budget + reserve + no-silent-switch + privacy + residency + failover).
...
## Icm-store-payload
```
icm store -t context-oyatie -c 'M02-P01-IP-004 Usage-window kernel + account-route policy shipped; acceptance commands green' -i high -k 'M02-P01-IP-004,complete'
```
