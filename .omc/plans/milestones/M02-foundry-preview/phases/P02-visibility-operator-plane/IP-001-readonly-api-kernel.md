...
# M02-P02-IP-001 — Read-only REST + WS API kernel

## Purpose
Ship read-only REST + WebSocket APIs for account/session/usage/routing dashboards.
...
## Icm-store-payload
```
icm store -t context-oyatie -c 'M02-P02-IP-001 Read-only REST + WS API kernel shipped; acceptance commands green' -i high -k 'M02-P02-IP-001,complete'
```
