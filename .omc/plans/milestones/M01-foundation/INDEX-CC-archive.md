...
## Status
**open.** M01-P07 (Oya VCS / GitOps-capable grit successor) is first-prerequisite planned from approved ralplan v5. M01-P08 remains the compatibility cutover lane already in flight; M01-P09..P09 fan out after the relevant P00/P01 gates are green.
...
## Dependencies
M01-P07 (Oya VCS / GitOps-capable grit successor) is the first prerequisite before broad multi-agent fan-out. M01-P08 (agentic-pipeline cutover) remains the compatibility foundation while P00 is built; every other M-CC phase and every main-spine milestone phase depends on P00 accepted or an explicit P00 compatibility waiver plus P01 ≥ P5 merged. M01-P09..P09 then run in parallel with M01 onward.

## Acceptance gate
- All ten phases' CI lanes green on `main`.
- M01-P07 acceptance requires Oya VCS ChangeSet scheduling, grit-authority projection, GitOps promotion, review/fix/rebase/merge-queue loop, issue-tracker integration, affected-build closure, and ops.oyatie.com evidence views green or explicitly compatibility-waived.
- M01-P13 acceptance also requires the dependency-seam discipline lane, tech-debt ledger, trigger DSL, replacement parity lane, and ADR-0091..ADR-0094 plan hooks from `ralplan-dep-seam-phaseout-round-5.md`.
- No main-spine milestone can pass its acceptance gate without inheriting the relevant M-CC lanes (e.g., M01 acceptance requires M01-P10 orphan-detection green; M02 acceptance requires M01-P12 provider-agnosticism green).
...
First-claim seed for M-CC overall: start [`phases/P00-gitops-vcs-replacement/INDEX.md`](phases/P00-gitops-vcs-replacement/INDEX.md) IP-001 + IP-009, while existing [`../../ralplan-oyatie-sst-consolidation.md`](../../ralplan-oyatie-sst-consolidation.md) compatibility cutover remains M01-P08. After P00 accepted and P01 P5 merged, the M01-P09..P09 fan-out begins. Each phase has its own first-claim seed in its INDEX.
