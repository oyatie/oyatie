...
- `oya-foundry-rag-api` (row 74): streaming retrieval → likely `grpc` (server-streaming) OR `rest` with SSE; if multi-protocol, split into `oya-foundry-rag-rest` + `oya-foundry-rag-worker`.
