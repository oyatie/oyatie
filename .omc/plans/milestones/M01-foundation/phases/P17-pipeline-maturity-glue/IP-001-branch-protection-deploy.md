...
`.github/branch-protection.yaml` now declares 15 required checks for `dev`: the cargo quartet, Oya VCS admission/provider execution, supply-chain, cohesion, api-semver, honest-claims, aspirational-enforcement, banned-primitives, protection-context-match, governance dependency-seam, and `oya-pr-review`. The 2026-05-19 live audit found GitHub `dev` protection requiring only 10 contexts, leaving six canonical checks unenforced plus one stale retired context required even while PR #148 reported `CLEAN/MERGEABLE`.

This IP deploys the `dev` ruleset, keeps `infra/branch-protection/dev.json` aligned with `.github/branch-protection.yaml`, and extends the existing `oya-governance-protection-context-match` lane so it re-verifies live GitHub required contexts against the on-disk policy on every PR. The live-check step fails closed unless a read token with GitHub Administration read permission is available.
...
- `oya-governance-protection-context-match` blocks if either YAML→workflow names drift or live GitHub required contexts drift from YAML.
...
- `.github/workflows/oya-governance-protection-context-match.yml::*` (required existing workflow fails closed on live drift)
