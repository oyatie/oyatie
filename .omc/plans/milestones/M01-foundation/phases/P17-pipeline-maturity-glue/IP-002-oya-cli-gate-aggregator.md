...
M01-P07 cutover that sunsets `grit` + `oya-tooling-agent-read` (per
...
The canonical sequencing is: M01-P07 lands Oya VCS → claim/work/done flow
...
- `oya claim → work → verify → done` end-to-end against a fake provider —
  **NOT satisfied**; deferred pending M01-P07 sunset (see above).
- Evidence at `/evidence/pipeline-maturity-glue/ip-002-oya-cli.json` —
