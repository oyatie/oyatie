...
execution_variant_note: "Delta 1: DistanceMetric enum + SearchHit value-object backported from P10 similarity-kernel spec into existing oya-search-index-vector-domain. No new crate scaffold per F-M02B-PLAN-LIVE-CRATE-RECONCILIATION. Cites impl-plan oya-vector-similarity-kernel/src/types.rs DistanceMetric+SearchHit spec."
---
...
Record completion via the Oya VCS pipeline (external coordination tooling retired per AGENTS.md):

```bash
# State is captured automatically by `oya vcs promote` above.
# Additional context note for the pipeline record:
# "IP-P10-vector merged; 12 crates; pgvector HNSW day-1 (m=16 ef_construction=128);
#  per-tenant RLS isolation; EmbeddingStorePort/SimilaritySearchPort;
#  cosine+dot+L2 metric selection; p99<=200ms at 1k RPS;
#  next: P11-finance-library/impl-plan"
```
