...
| IP-003 | Cloud Network VPC + LB + DNS + CDN + interconnect API | vpc-lb-oci-request-contract-green; dns-cdn-interconnect-second-provider-live-smoke pending | [`IP-003-network-api-adapters.md`](IP-003-network-api-adapters.md) |
...
| Network | `oya-cloud-network-{dns,lb,vpc}-api` partial plus `oya-cloud-network-adapter-oci` VPC/VCN and Load Balancer request contracts green; DNS/CDN/interconnect adapters and live smoke pending — IP-003 | OCI nonprod VCN (10.0.0.0/16), IGW + NAT GW + Service GW + 3 NSGs + public/private subnets |
