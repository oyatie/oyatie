...
status: in-progress (IP-001 adapter-port green; IP-002 OCI object+block request-contract green 2026-05-20)
...
| IP-002 | Cloud Storage object + block API + adapter set | object-and-block-oci-request-contract-green; second-provider pending | [`IP-002-storage-api-adapters.md`](IP-002-storage-api-adapters.md) |
...
| Storage | `oya-cloud-storage-domain` object + block provider ports and `oya-cloud-storage-adapter-oci` object/block request contracts green; second provider pending | OCI Object Storage namespace `axdotp9iv3ua` + bucket `oyatie-audit-cold-backup` (Archive tier); OCI Block Volume request-contract scoped to cloud compartment |
