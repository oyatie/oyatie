...
execution_variant_note: >
  Delta-1 ships MessageDeliveryClass + MessengerChannelKind into the existing
  oya-connect-messenger-domain crate via a new delivery_class.rs module and
  re-export. No new crate scaffolds, no new workspace deps, std-only.
blocked_by:
...
<!-- grit and icm are retired external coordination primitives (2026-05-16).
     Symbol-lock and context-store steps that previously used those tools
     are now handled by the Oya VCS foundry pipeline (M-CC-P11 substrate).
     Use `cargo run -p oya-dev-cli -- vcs claim` for symbol locks.
     See docs/decisions/ADR-0054-grit-scaffold-claim-pattern.md. -->
