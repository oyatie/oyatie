...
- [ ] `api-rest-adapter` stub future — By side-stepping `oya-foundry-api-rest-adapter` (currently 14-line stub) via direct hyper, do we commit it to remain a stub until M02-P04 lands its own real router? Recommendation: stays stub until M02-P04.
...
- [ ] `api-rest-adapter` stub future — Side-stepping the 14-line stub via direct hyper persists in v3 §B.7. Does this commit `oya-foundry-api-rest-adapter` to remain a stub until M02-P04 lands a real router? Recommendation: stays stub until M02-P04. (Unchanged by v3.)
...
- [ ] Diff #3 `AccountSnapshotProvider` port location — Plan places the port inside `oya-foundry-supervisor-kernel` (private to this plan's surface, no existing-kernel lean-a10 fire). Should it instead live in `oya-foundry-account-domain` next to `ProviderAccount` for reuse by other apps (route-policy-app, multi-subscription-pool)? Recommendation: keep in supervisor-kernel for v3; promote to account-domain only if a 2nd consumer surfaces.
...
- [ ] v4 patch #6 — `InboxStore::dead_letter()` atomic semantics authority — Plan chose Option β: `dead_letter()` consumes the lock atomically in a single Locked→DeadLettered transition; calling on an unlocked message returns `StoreError::InvalidTransition`. The contract lives in the trait doc-comment in `crates/oya-foundry-supervisor-kernel/src/lib.rs::InboxStore`. Decide: should this contract also be codified in `crates/oya-foundry-jsonl-supervisor-adapter`'s `JsonlInbox` impl-level doc-comment (defense-in-depth) or rely solely on the trait? Recommendation: codify in both (trait = contract, adapter = enforcement); add a unit-test row in §B.12 unit #25 asserting `InvalidTransition` is returned when `dead_letter` is called on an unlocked message id.
