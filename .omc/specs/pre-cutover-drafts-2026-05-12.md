...
This pattern lands as an ADR alongside the inventory ADR: `ADR-NNNN-grit-scaffold-claim-pattern.md`.

---

## Draft 3 — Parallel-claim demo script (A7 deliverable scaffold)

Target location for the recorded runbook: `oyatie/docs/runbooks/grit-parallel-claim-demo.md`.
...
| `goals.json` (goal manifest) | `icm store -t goals-oyatie` + plan files under `.omc/plans/` |
...
See ADR-NNNN-grit-icm-as-sanctioned-primitives.md and the spec at
`.omc/specs/deep-dive-oyatie-sst-consolidation.md`.
...
- All six drafts above are .omc/specs/ scratch and pre-approval. None of them are committed; none of them are authoritative until the Planner→Architect→Critic consensus lands and the user approves execution.
