...
|---|---|---|
| 1 | Wire `oya-dev-cli gate validate active-artifact-contract` | subcommand callable; loads `.omc/registries/artifact-capabilities-registry.json`; invokes kernel validator |
...
| 5 | Emit evidence/status bundle for lane result (per spec/status pattern amendment #5) | artifact at `.omc/evidence/lane-run-${RUN_ID}.json`; conforming to evidence-bundle-template |
| 6 | Materialize first graph edges from validated row (kinetic CreateArtifact action) | edge written to `.omc/graph/edges-2026-05-13.json` OR audit-chain (foundation-blocked alternative) |
...
Generated outputs from canonical registries: `nodes`, `edges`, `reverse_indexes`, `unresolved_refs`, `owners`, `freshness`, `impact_queries`. Output paths under `.omc/graph/materialized/`. Generator crate `crates/oya-gen-graph-materialize` (planned; post-VL slice).
...
NOT normal operating mode. Add to `.omc/specs/master-plan-sequencing.json`:
...
Add to `.omc/specs/control-plane-slos.json` (NEW; allowed under narrowing as it's the operationalization-spec for VL):
