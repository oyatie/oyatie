...
- 7 canonical specs at `.omc/specs/` (active-artifact-contract v3.0.0 + 5 inherited + plan-schema + markdown-retirement-policy + artifact-profile-defaults + root-hub-pointers)
- 5 registries at `.omc/registries/` (artifact-capabilities + reusable-building-blocks + knowledge-graph-{semantic/kinetic/dynamic})
- 2 ledgers/attestations at `.omc/ledger/` + `.omc/evidence/`
...
**14 kinetic action types** (per `.omc/registries/knowledge-graph-kinetic.json`):
...
**Markdown-retirement 8 phases** + retention rules (only README + CLAUDE + AGENTS survive; .omc/audits/consensus archives kept).
...
| 2 | Schema validation | ✅ JSON Schema 2020-12 in .omc/specs/active-machine-readable-artifact-contract.json | Validator parses + checks instance |
