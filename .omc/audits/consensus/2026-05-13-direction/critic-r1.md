...
- (ii)  Linus-grade:             FAIL — There is silent-regression risk already visible: `.omc/specs/root-hub-pointers.json` still points `knowledge_graph_catalog.current_path` at `.omc/registries/knowledge-graph-catalog.json`, while tracked HEAD has `.omc/registries/knowledge-graph-semantic.json`, `knowledge-graph-kinetic.json`, and `knowledge-graph-dynamic.json` instead. That is exactly the kind of stale pointer a load-bearing validator should catch.
- (iii) verified-claims:         FAIL — `git ls-files` confirms these cited files exist: `.omc/specs/active-machine-readable-artifact-contract.json`, `.omc/registries/artifact-capabilities-registry.json`, `.omc/specs/artifact-profile-defaults.json`, `.omc/specs/markdown-retirement-policy.json`, `.omc/ledger/markdown-retirement-ledger.json`, `.omc/specs/root-hub-pointers.json`, `.omc/specs/master-plan-sequencing.json`, `.omc/specs/hyperscaler-gates.json`, `.omc/specs/evidence-taxonomy.json`, `.omc/specs/stop-conditions.json`, `.omc/specs/final-report-schema.json`, `.omc/specs/plan-schema.json`, `docs/decisions/ADR-0069-active-machine-readable-artifact-contract.md`, and `crates/oya-check-active-artifact-contract/{Cargo.toml,src/lib.rs}`. But `git ls-files` does NOT return `.omc/audits/consensus/2026-05-13-direction/planner-v1.md`, `.omc/registries/knowledge-graph-catalog.json`, or the architect-named `semantic-knowledge-graph-catalog.json` / `kinetic-knowledge-graph-catalog.json` / `dynamic-knowledge-graph-catalog.json` paths. Actual tracked split paths are `.omc/registries/knowledge-graph-semantic.json`, `.omc/registries/knowledge-graph-kinetic.json`, and `.omc/registries/knowledge-graph-dynamic.json`.
...
3. Correct all cited paths to tracked HEAD names and fix `.omc/specs/root-hub-pointers.json` knowledge-graph pointer before claiming verified-claims.
