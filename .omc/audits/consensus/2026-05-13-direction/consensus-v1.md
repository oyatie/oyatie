...
- **Stale-pointer repair**: e.g., the `.omc/specs/root-hub-pointers.json` stale `knowledge-graph-catalog.json` pointer (FIXED inline this batch per critic r1 finding #3).
...
1. **Tracked registry row** in `.omc/registries/artifact-capabilities-registry.json` (already exists; ≥1 row)
...
- **#3 (stale pointer)**: `.omc/specs/root-hub-pointers.json` `knowledge_graph_catalog` row replaced with 3-layer split rows (`knowledge_graph_semantic` + `knowledge_graph_kinetic` + `knowledge_graph_dynamic`).
...
1. Planner v1 had verified-claims FAIL (cited `.omc/audits/consensus/2026-05-13-direction/planner-v1.md` which was not yet HEAD-tracked at architect r1 time + cited deleted catalog path). Both fixed by this commit.
