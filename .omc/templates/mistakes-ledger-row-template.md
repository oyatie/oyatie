...
lift_target: oyatie/docs/templates/mistakes-ledger-row-template.md
enforcing_fitness_lane: oya-foundry-fitness-mistakes-ledger-cite
...
- id: MFL-NNNN
  ...
  mechanical_prevention:
    ...
    name: oya-foundry-fitness-<lane> | scripts/hooks/<hook>.mjs | <validator-name>
...
```
| MFL-0010 | 2026-05-09 | RUNBOOKS-INDEX referenced 49 P0 runbook files that did not exist on disk after cleanup | No CI gate verifying RUNBOOKS-INDEX entries resolve to real files | `oya-foundry-fitness-runbook-index-resolves` lane + per-runbook stub authoring at index-update time | shipped 2026-05-09 | per Codex Round 2 verdict + RUNBOOKS-INDEX §1 |
```
...
3. **Mechanical prevention is mandatory** — if you cannot name a CI lane / hook / validator / runtime gate, file an escalation (`.omc/templates/checklists/escalation-checklist.md`).
