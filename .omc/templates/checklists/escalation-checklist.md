...
**Trigger:** `grit session start` bug (or analogous) blocks the sanctioned pipeline. **Resolution:** Halt sanctioned flow; switch to documented workaround per `.omc/specs/pre-cutover-drafts-2026-05-12.md §Draft 1`; emit `icm store -t upstream-tool-bugs -c "<one line>" -i high`; human orchestrator files upstream issue. *Target collapse: yes (when upstream ships fix).*
