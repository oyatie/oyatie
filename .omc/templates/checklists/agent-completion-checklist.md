...
If any command fails, **do not** proceed. Re-walk the work, or halt and emit `BLOCKED_ON_HUMAN_ORCHESTRATOR` per `.omc/templates/checklists/escalation-checklist.md`.
...
Per `.omc/templates/checklists/inventory-update-checklist.md` — append a row to the active inventory ledger naming: source path, target path, archive path (if any), tombstone (if delete), audit emission ID, agent or human orchestrator. *Lane:* `oya-foundry-fitness-inventory-tracker`.
