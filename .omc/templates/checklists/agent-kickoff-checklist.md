...
If no `open` IP exists at the milestone-active layer, halt and emit `BLOCKED_ON_HUMAN_ORCHESTRATOR` per `.omc/templates/checklists/escalation-checklist.md`.
...
- **No `git`, no `gh`** outside the documented carve-out (see `.omc/specs/adr-draft-grit-icm-sanctioned-primitives.md §Cutover bootstrap window` + `§Glossary §Human orchestrator`). If a workflow appears to require `git`/`gh`, emit `BLOCKED_ON_HUMAN_ORCHESTRATOR`.
