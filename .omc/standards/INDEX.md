...
purpose: |
  ...
  authority position.
lift_target: oyatie/docs/standards/INDEX.md
...
1. Carries `status: pending approval` and a `lift_target: docs/standards/<file>.md` in
   frontmatter.
...
Status: **pending approval** (working draft under `.omc/standards/`). Lift target on
approval: `oyatie/docs/standards/`. Each file lifts independently per its own
`lift_target` frontmatter field. After lift, `docs/STANDARDS-AND-TEMPLATES.md` is
updated to reference the lifted paths, and the `<!-- forward-reference: wave-1 -->`
and `<!-- forward-reference: wave-2 -->` sentinels in `docs/AGENTS.md`,
`docs/README.md`, and `docs/CONSTITUTION.md` that point at `standards/*` rows above
are removed in the corresponding wave PR (not in this batch — the user lifts).
...
- [`.omc/plans/MASTERPLAN.md`](../plans/MASTERPLAN.md) §2 Compound principles
  (Directives 1-12).
