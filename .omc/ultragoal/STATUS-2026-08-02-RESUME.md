...
## Hard blockers
- G028 declaration MERGED (#1529 → tip `0c1014b87`); the 2026-08-03 read-only live inventory confirms ARS/ERS still **20Gi** because GitOps reconciliation is absent. Prior founder ruling fixes **class B / KEEP_CURRENT_LAB=true**; v1 independent review returned **REQUEST_CHANGES**. V2 now has target context/API/certificate identity, actual Helm release names, metadata-only Secret facts, and live ownership drift, but remains unreviewable until content digests, named principals/owners, Secret restoration authority, and the three kube-mode objects' disposition are frozen. No implementation or helm/CRS/render.sh apply authorized.
...
- Repository facts fix the 18-Application blast radius, existing protected `ci/facade` Buck2 fan-in, ARC chart declarations, and absent hermetic pins
- Read-only live inventory (`G028-LIVE-ARC-ADOPTION-INVENTORY-2026-08-03.md`) fixes target `admin@oya-talos` / API endpoint / public certificate fingerprint, Helm release names `arc` and `oya-arm64`, live 20Gi values, ownership classes, and metadata-only Secret key names without reading credential bytes
- Blocking fields remain: Argo/ARC content digests; bootstrap principal/RBAC/audit/rollback owner; Secret durable provisioner/restoration/lifetime/revocation authority; exact rendered inventory; disposition of three `HELM_LIVE_NOT_RENDERED` kube-mode objects
- Artifacts: `.omc/ultragoal/G028-CLASS-B-INDEPENDENT-REVIEW-2026-08-02.md`, `.omc/ultragoal/G028-CLASS-B-V2-STAGED-ARC-ADOPTION-DESIGN-2026-08-02.md`, `.omc/ultragoal/G028-BOOTSTRAP-REPOSITORY-FACTS-2026-08-02.md`, `.omc/ultragoal/G028-ARC-ADOPTION-REPOSITORY-FACTS-2026-08-02.md`, `.omc/ultragoal/G028-LIVE-ARC-ADOPTION-INVENTORY-2026-08-03.md`
...
### Portable agent lifecycle protocol / `.omc`–`.omx` unification (2026-08-03)
- User-fixed goal: one process portable across sessions, models, repositories, Claude Code, Codex, Grok Build, and Antigravity; centrally managed protocol/bootstrap boundary at `~/.agents`
- Authority correction: `~/.agents` owns portable vocabulary and user-local shared-harness artifact identities, but does not replace repository governance or become a cross-machine database; Oyatie still resolves `docs/AGENTS.md` and `specs/masterplan.json#masterplan_v2` through `specs/root-hub-pointers.json`
- Live oracle: `.omx/ultragoal` had 35 files, all identical to `.omc/ultragoal`; `.omc` had 44 additional; `.omx` had zero unique — dual copies are a transitional defect, not two authorities
- Smallest slice: shared ultragoal artifacts only; native sessions, caches, locks, logs, and mode state remain native; whole-tree symlinking is forbidden
- Bun discipline: freeze oracle → minimal reducer/store → cross-adapter vertical slices → real-workload shadow parity → separately admitted read/write cutovers → delete duplicate shared projections after measured retirement criteria
- Draft: `.omc/ultragoal/PORTABLE-AGENT-LIFECYCLE-PROTOCOL-DESIGN-2026-08-03.md`; preliminary adversarial review pending; not frozen, implementable, or cutover-authorized
- Current `.omc`/`.omx` parity copies remain transitional continuity only; the draft itself is intentionally not copied into `.omx`, avoiding expansion of the dual-authority surface
