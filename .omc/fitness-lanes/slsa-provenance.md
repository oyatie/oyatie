...
- kernel_crate: `oya-foundry-fitness-slsa-provenance-kernel` — `ProvenanceRecord { artifact, builder_id, slsa_level, attestation_path }`, verdict `SlsaProvenanceFitnessReport { records_checked }`.
- runner_path: `tools/oya-foundry-fitness-slsa-provenance`
- inputs: in-toto attestation JSONs, registered builder list.
- failure_modes:
  - missing attestation file
  - SLSA level < 2
  - builder id not in registered list
- ci_invocation: `cargo run -p oya-foundry-fitness-slsa-provenance`
