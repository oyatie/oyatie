...
- kernel_crate: `oya-foundry-fitness-pre-flight-checklist-kernel` — `PreFlightChecklist { pr_id, items }`, `ChecklistItem { id, state }`, verdict `PreFlightChecklistFitnessReport { pr_count }`.
- runner_path: `tools/oya-foundry-fitness-pre-flight-checklist`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-pre-flight-checklist`
