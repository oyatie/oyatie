# ADR-DRAFT — grit + icm + oya-tooling-agent-read as sole sanctioned primitives for agentic work

Pre-approval scratch. Lifts to `oyatie/docs/decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md` once the ralplan consensus plan lands and execution approval is captured.

---

## Status
Proposed — pending ralplan consensus approval at `.omc/plans/ralplan-oyatie-sst-consolidation.md`.
...
## Notes for the eventual ADR landing
- Lift this draft to `oyatie/docs/decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md`.
- Confirm ADR-0053 is still the next free slot at landing time by reading `oyatie/docs/ADR-INDEX.md` (highest existing was ADR-0051 as of 2026-05-12; slots 0052/0053/0054 are reserved for inventory + this ADR + scaffold-claim).
- Add to `ADR-INDEX.md` under the "Cross-cutting / Tooling" section or equivalent.
- Cite this ADR from the rewritten `oyatie/docs/AGENTS.md` §"Sanctioned primitives" section.
- Cite this ADR from the inventory ADR (ADR-0052).
- Cite this ADR from the deprecation notice at `bominal/agents/ultragoal/DEPRECATED.md`.
- Update `oyatie/docs/CHANGELOG.md` per `DOC-CATALOG.md` lifecycle protocol.
- Add an `MFL-NNNN` row to `MISTAKES-LEDGER.md` for the omx-mismatch-state-machine failure (`G004-reconciliation-blocker.md` is the candidate evidence row), mechanically prevented by this ADR's grit-claim==intent reshape.
