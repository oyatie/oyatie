...
3. New-crate scaffolding: if your phase creates a new crate (e.g., tools/oya-agent-read/), use the scaffold-claim pattern from `.omc/scratch/pre-cutover-drafts-2026-05-12.md §Draft 2`. Try Cargo.toml::workspace_members first; if not indexed, fall back to icm-as-coordination-lock under topic `scaffold-locks-oyatie`.
