...
pub async fn accept_cloud_compute_k8s_cluster_from_api_with_authorization_verifier(
    ...
    let snapshot = repository
        .accept_create_intent(CloudComputeK8sAcceptCreateIntentCommand {
            operation_key: operation_key.clone(),
            intent: request.body,
            request_id: request.boundary.request_id,
        })
        .await
        .map_err(acceptance_repository_error)?;
    validate_cloud_compute_k8s_operation_snapshot(
        &snapshot,
        &operation_key,
        &request.path_cluster_id,
        Some(&expected_intent),
    )
    .map_err(acceptance_repository_error)?;
    Ok(CloudComputeK8sCreateAcceptanceResponse {
        operation: snapshot,
    })
}
...
pub async fn get_cloud_compute_k8s_operation_from_api_with_authorization_verifier(
    ...
    let lookup = repository
        .get_create_operation(CloudComputeK8sReadCreateOperationQuery {
            operation_key: operation_key.clone(),
            resource_id: request.path_cluster_id.clone(),
        })
        .await
        .map_err(acceptance_repository_error)?;
