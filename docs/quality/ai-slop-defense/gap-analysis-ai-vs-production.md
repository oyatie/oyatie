...
| AIS-041 Provider-specific in kernel | D04 | `oya-foundry-fitness-provider-coupling` | CLOSED |
...
| AIS-073 AGPL/GPL trap | D27 | `oya-foundry-fitness-license` | CLOSED |
...
| AIS-112 Architecture-map drift | D36 | `oya-foundry-fitness-architecture-map-freshness` | CLOSED |
...
| AIS-132 Missing audit emit | D21 | `oya-foundry-fitness-audit-emission` | CLOSED |
| AIS-140 Irreversible schema change | D22 + D23 | `oya-foundry-fitness-schema-migration` | CLOSED (extend to rollback) |
| AIS-141 Global rollout | D24 | no per-tenant lane today | **OPEN (C)** |
| AIS-150 PII in logs | D20 | `oya-foundry-fitness-data-class` | CLOSED |
| AIS-151 Missing consent check | D16 | `oya-foundry-fitness-capability-publish` | CLOSED |
...
**Fix**: Adopt `oya-foundry-fitness-no-unwrap` lane immediately
...
`clippy::unreachable = "deny"`) + `oya-foundry-fitness-error-fan-in`
...
**Fix**: Adopt `cargo-vet` + dep-allowlist (`oya-foundry-fitness-dep-allowlist`)
+ Unicode-discipline lane (`oya-foundry-fitness-unicode-discipline`,
...
**Fix**: Adopt `loom` model-check lane (`oya-foundry-fitness-cancel-safety`)
