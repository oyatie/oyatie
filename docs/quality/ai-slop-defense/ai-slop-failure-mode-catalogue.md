...
| **AIS-002** | Hallucinated stdlib / framework function (`std::sync::AsyncMutex`, `tokio::spawn_blocking_local`). | LLM emits `tokio::join_all` thinking it exists in Tokio (it lives in `futures`). | H | `cargo check` is mandatory pre-commit; planned advisory lane `oya-governance-compile-clean` records compile-clean gaps before IP merge once implemented. |
