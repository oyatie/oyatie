...
Per-cell. A bad release reverts in one cell without disturbing healthy cells. Per-cell rollback emits D14 audit-chain evidence ([ADR-0003](../../../docs/decisions/ADR-0709-general-live-apex.md)) and is tracked by planned advisory lane `governance-rollback-evidence`.
...
- `pipeline-slo-coverage` (existing; extended) — requires burn-rate alert per service.
