...
## Wave 15-ZF surviving doctrine refs — ADR-0346..ADR-0348 + ADR-0515

Every agent MUST treat [ADR-0346](decisions/ADR-0346-oya-verify-must-run-full-ci-mirror.md), [ADR-0347](decisions/ADR-0347-governance-fitness-bulk-rename.md), and [ADR-0348](decisions/ADR-0348-autosharding-auto-rebalance-dynamic-sharding.md) as active operating-contract doctrine for their surviving non-CI obligations until superseded or amended by a newer ADR. For CI/CD enforcement, [ADR-0515](decisions/ADR-0515-phase0-firewall-one-canonical-ci-cloud-native-posture.md) is the current single-truth authority: GitHub Actions + branch protection are the live runner/authority until explicit owned-runner cutover, the cloud-ci Rust gate apps produce the one protected `oya-ci-required` context, and superseded Prow/Jenkins/legacy `oya` CLI governance wording exists only as Git-history provenance or local-feedback evidence. No GitHub Actions status/check outside `oya-ci-required` may be promoted as protected-branch authority.
...
| ADR-0515 | GitHub Actions plus branch protection are the sole live CI authority until an explicit owned-runner cutover; `oya-ci-required` is the single protected context and pipeline gates are Rust binaries. The seven superseded CI/CD ADR source files exist only in Git history. | Do not restore a superseded source file or expose it through ADR projections, root-hub current paths, or active masterplan bindings. Do not add Jenkins/Groovy, Prow, legacy `oya` CLI CI authority, or any GitHub Actions status/check outside `oya-ci-required` as protected-branch authority. |
