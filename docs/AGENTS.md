...
**REVIEW-ADMISSION-GAP-LIVE-BOUNDARY (F-PR5-06):** F-PR5-06 remains open: formal GitHub `reviewDecision`, reviewer-author separation, and branch-protection drift reconciliation remain tracked by `registry/fixuptasks.jsonl#F-PR5-06`. The retired PR metadata preflight does not close it (ADR-0716). This is **not a cloud-enforced review admission gate**: PR #964 merged on green `oya-ci-required` alone while `reviewDecision` was empty and the only review state was COMMENTED by the owner — closure rests on the review thread plus the protected context, not on server-side review state, until a trusted producer closes F-PR5-06.
