...
Active hooks — SSOT is [`.claude/settings.json`](../.claude/settings.json), which the `enforcement-liveness` face resolves against `tools/hooks/`; this list is a mirror, not an authority. PreToolUse/Bash: `main-checkout-guard.sh`, `local-authority-enforcer.sh`, `stale-tool-suggester.sh`. (The `no-cargo-enforcer.sh` hook is retired with ADR-0716: cargo is the merge path.) PreToolUse/Task: `pre-dispatch-guide.sh`. PostToolUse/Edit|MultiEdit|Write: `spec-version-pin-suggester.sh`, `adr-orphan-detect.sh`, `vacuous-green-gate-detect.sh`. PostToolUse/Bash|WebFetch|WebSearch: `injection-content-scanner.sh`. Stop: `stop-did-you-forget-suggester.sh`. There is no SessionStart hook, and no agent-runtime merge-review, telemetry, loop-cancellation, or memory-bootstrap hook — the prior text named five behaviours and one file (`scripts/hooks/guard-pr-merge-review.mjs`), none of which existed in-tree. The git-level pre-push hook is the owned Rust verifier `oya-pre-push-verify` (`//ci/facade/generated-artifact-freshness:oya-pre-push-verify-bin`), installed OUTSIDE the checked-out tree so a contribution can never replace the hook git executes: `buck2 run //ci/facade/generated-artifact-freshness:oya-pre-push-verify-bin -- reconcile` converges the installed hook state toward the DECLARED wiring state in `tools/hooks/pre-push-verifier.wiring.json` (failing closed when the binary disagrees with the declaration) and copies the pinned binary and its PINNED generator tools into the git hooks dir (built once from the reconcile-time checkout, never from the active checkout's Buck graph — the hook executes only those prebuilt tools against the repo tree as DATA, so a malicious branch cannot make the hook run checkout-controlled code), preserves a LOCAL or worktree-scoped `core.hooksPath` (refusing a global/system-scoped shared hooks dir and an in-tree path), and writes a protocol manifest bound to the pinned tool builds, the installed hook's own identity, and the COMPLETE generator build inputs (crate sources, `Cargo.toml`/Buck rules, workspace `Cargo.lock`, transitive path dependencies); linked worktrees keep per-worktree pinned-tool generations under worktree-keyed subdirectories so worktrees at different generator sources never clobber each other's installation; the hook fails closed with an explicit reinstall requirement when the manifest, the pinned tools, the generator source, the installed hook's identity, or the repository's generated-face protocol is stale, and its verify path runs the same ADR-0595 determinism canary as the canonical freshness gate when non-PR-owned faces are present and re-asserts committed-tree cleanliness AFTER the generators run (see `docs/oya-ci/gate-catalog.md` face-settle protocol). It runs the local `oya-cloud-ci-face-settle --verify` check on every branch push AND every non-deletion tag push (tag objects peeled to their commit) whose pushed commit equals HEAD (fail-closed otherwise); it is an ADVISORY local layer that `git push --no-verify` can bypass, so the cloud-ci freshness gate behind `oya-ci-required` is the enforcement backstop; it is a git hook, not a Claude Code hook, and is not part of the `.claude/settings.json` mirror list above.
