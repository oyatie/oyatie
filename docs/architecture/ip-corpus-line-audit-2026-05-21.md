...
### §2.2 `oya vcs` (now `oya git`) — superseded by [[oya-git-canonical-2026-05-18]]

The audit found **63 line-anchored references** to `oya vcs`, `oya-vcs-promotion-readiness`, `oya vcs done`, or `crates/oya-dev-cli/src/commands/vcs/*` across IP files. The verb-surface rename per PR-159B is incomplete in the IP corpus.
...
Action: rename lane `oya-vcs-promotion-readiness` → `oya-governance-promotion-readiness` (matching the foundry-fitness→governance rename pattern) **OR** retain the `oya-vcs-` prefix per the corrected [[oya-git-canonical-2026-05-18]] doctrine which says raw-git is canonical and `oya git` is the agent VCS primitive. The decision belongs to the governance lane owner; **the audit's job is to flag**.
...
This is **30 IPs × 1 line each = 30 mechanical rewrites**. Per [[oya-git-canonical-2026-05-18]], the canonical primitive is `oya git done` (drop-in for raw git + ledger layer).
...
| `microservices/observability/IP-014-automated-rollback-primitive.md` | body | `crates/oya-dev-cli/src/commands/vcs/rollback.rs` (path uses `vcs/`, not `git/`) |
...
**0 IP-body hits.** The IP corpus is clean of grit/rtk/icm/vox references per [[deprecate-external-agent-coord-tooling]]. The `oya vcs` references in §2.2 are the residual: they came from the same era but were independently retired by the [[oya-git-canonical-2026-05-18]] sub-doctrine.
...
```
oya-cell-boundary                          (used by cell/IP-006 — verify against ADR-0248 cellular tier doctrine)
oya-check-openslo-conformance              (used by cell/IP-013 — verify against canonical openslo lane name)
...
per-microservice-layout                    (short form of oya-governance-per-microservice-layout — needs canonicalisation)
authority-cohesion                         (short form of oya-governance-authority-cohesion)
hyperscaler-maturity-claims                (short form — verify canonical name)
layer-correctness                          (short form — verify against oya-governance-layer-correctness or similar)
...
Without `depends_on:` declarations a cycle check is not possible. Wave-3-E must populate the dependency front-matter, then a `cargo run -p oya-dev-cli -- ip-dag-lint` (which does not exist yet — see §10) can detect cycles.
...
| IP file | Action |
|---|---|
| `microservices/observability/IP-012-oya-vcs-promotion-readiness-lane.md` | **Rename file** to `IP-012-oya-git-promotion-readiness-lane.md`; update body to use `oya git` verb surface; do NOT supersede — the lane itself is canonical |
| `microservices/community/IP-013-oya-vcs-promotion-readiness.md` | Rename to `IP-013-oya-git-promotion-readiness.md` |
| `microservices/observability/IP-014-automated-rollback-primitive.md` | Update `crates/oya-dev-cli/src/commands/vcs/rollback.rs` path to `crates/oya-dev-cli/src/commands/git/rollback.rs` |
...
| Rename `oya-vcs-promotion-readiness` lane → `oya-governance-promotion-readiness` (if governance-lane decision says so) **OR** retain (if [[oya-git-canonical-2026-05-18]] says lane stays vcs-prefixed) | 15 IP file fronts | low — depends on lane-roster decision |
| Rename `crates/oya-dev-cli/src/commands/vcs/*` → `git/*` in IP body refs | 1 IP (observability/IP-014) | low |
...
- 154 internet-facing IPs need ADR-0297 + the matching `oya-governance-edge-abuse-defence` lane
- ~30-50 narrow auth IPs (identity 17 + governance authz + cloud-secrets + consent-graph) need ADR-0298 + ADR-0299 + auth lanes
- 61 financial IPs need ADR-0307 + `oya-governance-fraud-detect-evidence` lane
...
18. **`microservices/observability/IP-014-automated-rollback-primitive.md` references `crates/oya-dev-cli/src/commands/vcs/rollback.rs`; per [[oya-git-canonical-2026-05-18]] the path is `…/commands/git/rollback.rs`.** Update path ref. P1-RENAME.
...
30. **The acceptance-lane roster has at least 25 candidate lane names that may not appear in the canonical roster** (oya-cell-boundary, perf-canvas-60fps, a11y-* lanes, etc.). Cross-check against `documentation-rigor.md` + ADR-0297..0310 §E. P2-LANE-ROSTER.
...
1. **`oya-vcs-promotion-readiness` lane name** — does the [[oya-git-canonical-2026-05-18]] doctrine retain the `oya-vcs-` lane prefix (since the lane predates the verb rename) or rename to `oya-governance-promotion-readiness`?
...
- ICM memory recalls: [[oyatie-is-a-tenant-doctrine]], [[oya-git-canonical-2026-05-18]], [[layer-enum-adr-0105-13-canonical]], [[cedar-as-universal-gate]], [[amazon-shape-cellular-architecture]], [[build-ahead-of-certification]], [[bominal-inheritance-precedence]], [[no-silent-regression]].
