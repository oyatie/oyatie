...
2. **Per-crate migration** (28 crates, in Phase 1–4 order):
   ...
   # Edit all use declarations: oya_foundry_fitness_<X>_kernel → oya_governance_<X>_kernel
...
Note: `crates/oya-dev-cli/src/retired_vocabulary_gate.rs` line 48 explicitly excludes `docs/plans` from the retired-vocabulary corpus scan, consistent with treating plans as forward-looking documents.
