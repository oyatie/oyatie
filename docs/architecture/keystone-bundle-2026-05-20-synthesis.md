...
**Note:** The audit doc itself has stale `planned_enforcement_ref: oya-foundry-fitness-doc-rigor` — pre-rename. Phase-2 fixes this in audit's own front-matter too (`oya-governance-doc-rigor`).
...
**Headline:** the IP corpus is structurally large + pre-rename clean (0 stale `oya-foundry-fitness-*` references in acceptance_lanes), but doctrine-binding is severely weak (14% of IPs cite any keystone-bundle ADR; 0% cite any amendment ADR).
...
| Post-rename `oya-foundry-fitness-*` in acceptance_lanes = 0 hits | **PASS** — the foundry → governance rename cleaned this corpus thoroughly. |
