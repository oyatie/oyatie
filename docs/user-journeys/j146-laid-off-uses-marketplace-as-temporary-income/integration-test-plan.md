...
date: 2026-05-20
authority_tier: 2
adr_invariants_tested: [ADR-0145, ADR-0244, ADR-0249, ADR-0252, ADR-0292, ADR-0311]
---

# j146 — Integration test plan

## A. Fixtures

| Fixture | Setup |
|---|---|
| `<chris-personal-tenant>` | B2C_JOB_SEEKER_ACTIVE; will sub-tier B2C_MARKETPLACE_SELLER |
| `<carlos-personal-tenant>` | FL buyer |
| `<devika-personal-tenant>` | NY freelance buyer |
| `<marcin-business-tenant>` | Berlin VAT-registered business |
| Marketplace operator tenant | Live with FX adapter, FedEx adapter, OFAC adapter |
| Sales-tax pack | US-MI nexus + 50-state facilitator-collection |
| EU-VAT pack | OSS rules; reverse-charge logic |

## B. Test cases

### B.1 — Hardware sale happy path (3 items, mixed US states)

Assert: Each sale settles T+2bd; facilitator collects state sales tax; finops categorizes correctly.

### B.2 — Service listing + escrow happy path (Devika)

Assert: Contract signed by both passkeys; escrow funded; sessions complete; deliverable sent; escrow released; settlement.

### B.3 — EU cross-border B2B service (Marcin) — reverse-charge

Assert: VAT pack determines reverse-charge; invoice generated net of VAT; FX-settled USD; settlement category `marketplace_consulting_2026` with foreign-derived sub-tag.

### B.4 — Sanctions screen blocks unauthorized buyer

Steps: simulate OFAC-listed entity attempting purchase.
Assert: contract-signing step fails Cedar; emits `SanctionsScreenFailed`; refund triggered.

### B.5 — DSA disclosure limits

Steps: EU buyer requests seller info via DSA flow.
Assert: response includes last-4 of tax ID only, not full ID.

### B.6 — Year-end 1099-NEC

Steps: Chris total receivables from US sources cross $600 in 2026.
Assert: 1099-NEC issued by marketplace in January 2027; finops-portal renders the form.

### B.7 — FX rate transparency

Steps: EUR-to-USD settlement.
Assert: settlement document shows `mid_rate` + `spread_pct` separately.

### B.8 — Personal-data isolation

Steps: buyer attempts to query Chris's personal-Mail.
Assert: Cedar denies; `CrossTenantAccessDenied`.

### B.9 — Audience sub-tier activate/deactivate cycle

Steps: activate B2C_MARKETPLACE_SELLER; deactivate; reactivate.
Assert: listings pause on deactivate; reactivate via re-tap; no data loss.

### B.10 — Concurrent operation with j144 pipeline

Steps: run j146 listings concurrently with j144 job-search pipeline.
Assert: no interference; both audit-trails distinct.
...
|---|---|
| Listing publish | ≤ 1s |
| Settlement (T+2bd) | per ACH cycle |
| FX quote | ≤ 500ms |
| Sanctions screen | ≤ 2s |

## D. Chaos

- D.1: FedEx adapter down; listing publish blocked on shipping-class items; service-class continues.
- D.2: OFAC list service slow; sanctions screen times out; contract-signing pauses.
- D.3: FX adapter spread spikes; settlement holds with notice to seller.
- D.4: Buyer disputes; escrow holds funds; arbitration kicked off.

## E. ADR coverage

| ADR | Test |
|---|---|
| ADR-0249 multi-category marketplace | B.1, B.2, B.3 |
| ADR-0292 marketplace primitive | B.1 |
| ADR-0252 HLC settlement | B.1, B.2 |
| ADR-0311 personal-tenant data isolation | B.8 |
| ADR-0244 audience_type sub-tier | B.9 |
| ADR-0145 cross-tenant gRPC | B.1, B.3 |
| EU DSA Article 30 | B.5 |
| EU VAT OSS reverse-charge | B.3 |
| OFAC sanctions | B.4 |

## Completion expansion — j146 integration rigor pass
...
Test case 005: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 017: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 025: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 037: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 041: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 045: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 049: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 057: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 065: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 077: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 085: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 089: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 097: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 105: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 117: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 125: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 137: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 145: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 157: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 161: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 165: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 169: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 177: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 185: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 197: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 201: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 205: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 217: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 225: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 229: audit-chain seal verification for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 237: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 241: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 245: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 249: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 257: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 261: audit-chain seal verification for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 265: default-deny refusal for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 269: audit-chain seal verification for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 277: audit-chain seal verification for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 281: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 285: audit-chain seal verification for marketplace seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 289: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 297: default-deny refusal for finops-portal seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
