...
| 3 | `j29-screen-03` | connector | Continue `workflow-studio-personal-automation` step 3. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 7 | `j29-screen-07` | connector | Continue `workflow-studio-personal-automation` step 7. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 11 | `j29-screen-11` | connector | Continue `workflow-studio-personal-automation` step 11. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 15 | `j29-screen-15` | connector | Continue `workflow-studio-personal-automation` step 15. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 19 | `j29-screen-19` | connector | Continue `workflow-studio-personal-automation` step 19. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 23 | `j29-screen-23` | connector | Continue `workflow-studio-personal-automation` step 23. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 27 | `j29-screen-27` | connector | Continue `workflow-studio-personal-automation` step 27. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 31 | `j29-screen-31` | connector | Continue `workflow-studio-personal-automation` step 31. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 35 | `j29-screen-35` | connector | Continue `workflow-studio-personal-automation` step 35. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 39 | `j29-screen-39` | connector | Continue `workflow-studio-personal-automation` step 39. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 43 | `j29-screen-43` | connector | Continue `workflow-studio-personal-automation` step 43. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 47 | `j29-screen-47` | connector | Continue `workflow-studio-personal-automation` step 47. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 51 | `j29-screen-51` | connector | Continue `workflow-studio-personal-automation` step 51. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 55 | `j29-screen-55` | connector | Continue `workflow-studio-personal-automation` step 55. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 59 | `j29-screen-59` | connector | Continue `workflow-studio-personal-automation` step 59. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 63 | `j29-screen-63` | connector | Continue `workflow-studio-personal-automation` step 63. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 67 | `j29-screen-67` | connector | Continue `workflow-studio-personal-automation` step 67. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 71 | `j29-screen-71` | connector | Continue `workflow-studio-personal-automation` step 71. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 75 | `j29-screen-75` | connector | Continue `workflow-studio-personal-automation` step 75. | Runs `shipping-label-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 3 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 7 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 11 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 15 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 19 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 23 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 27 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 31 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 35 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 39 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 43 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| 47 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j29.connect.ux_recovery`. |
...
| UX-A001 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A005 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A009 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A013 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A017 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A021 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A025 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A029 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A033 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A037 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A041 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A045 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A049 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A053 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A057 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A061 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A065 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A069 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A073 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A077 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A081 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A085 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A089 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A093 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A097 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A101 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A105 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A109 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A113 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A117 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A121 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A125 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A129 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A133 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A137 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A141 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A145 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A149 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A153 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A157 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A161 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A165 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A169 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A173 | `connector` `shipping-label-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
