...
date: 2026-05-20
authority_tier: 2
adr_invariants_tested: [ADR-0145, ADR-0244, ADR-0299, ADR-0311]
---

# j145 — Integration test plan
...
### B.1 — Happy-path application → onboarding (full E2E)

Steps: submit application → screen → phone-screen → onsite-loop → offer → counter → accept → provision → background check → start.
Assert: 24-day workflow completes; all 18+ cross-tenant emissions audit-sealed both sides.

### B.2 — Cross-tenant principal provisioning requires user approval

Steps: KrampusCorp emits CTPP request. Verify Chris's personal-tenant Cedar status is `pending_user_approval`.
Steps: Chris taps Approve.
Assert: principal provisioned only after step.

### B.3 — Personal-tenant can refuse provisioning

Steps: configure Chris's personal-tenant policy default-deny for provisioning. Trigger request.
Assert: `refused_consent`; offer remains valid; alternate-onboarding path enabled.

### B.4 — Background check requires FCRA consent

Steps: trigger background check without `fcra_consent_ref`.
Assert: Cedar denies at step 6.1; emits `BackgroundCheckMissingConsent`.

### B.5 — Reference attestation verification

Steps: KrampusCorp validates j143 receipt against `<former-employer-tenant>` public endpoint.
Assert: `valid: true`; key_id matches Karim's registered public key.

### B.6 — Forged reference rejected

Steps: present a fabricated receipt with bogus signature.
Assert: verify returns `valid: false`; KrampusCorp's HR queue surfaces `ReferenceAttestationFailed`.

### B.7 — Anti-leak: KrampusCorp cannot read personal-Mail

Steps: KrampusCorp HR principal attempts to query Chris's personal-Mail.
Assert: Cedar default-deny; audit emits `CrossTenantAccessDenied`.

### B.8 — Same passkey authenticates both tenants

Steps: Chris uses the same WebAuthn credential_id to authenticate to personal and KrampusCorp tenants.
Assert: both succeed; credential_id identical in both LoginSuccess audit records.

### B.9 — Audience-type transition on start-day

Steps: trigger employment-start event.
Assert: Chris's personal-tenant audience_type transitions to B2C_CONSUMER; krampuscorp principal set to B2B_TENANT_EMPLOYEE.

### B.10 — Pay-transparency disclosure invariant

Steps: KrampusCorp posts a role on Community at a US-NY worksite without comp band.
Assert: posting validation rejects (NY Pay-Transparency Law); requires comp band.

### B.11 — Counter-offer flow audit-sealed both sides

Steps: counter, revised offer, accept.
Assert: 3 cross-tenant events, all double-sealed with HLC merge anchors.

### B.12 — Identity-bind on interview Meet

Steps: Chris joins Linda's interview room.
Assert: Linda's UI shows Chris's Community LinkedIn-mode verified profile badge.

## C. Performance SLAs

| Operation | SLA |
|---|---|
| Cross-tenant application submit | ≤ 5s end-to-end |
| AI screening (KrampusCorp side) | ≤ 30s |
| Offer letter delivery cross-tenant | ≤ 60s |
| Principal provisioning request → approval round-trip | ≤ 5m end-to-end (user-mediated) |
| Background check end-to-end | per vendor (typically 1-3 business days) |

## D. Chaos

- D.1: Background check vendor down 4h; workflow holds; resumes on recovery.
- D.2: KrampusCorp tenant pauses hiring mid-loop; offer pending state retained; expires per policy.
- D.3: Chris's personal-tenant identity service brief outage during provisioning; request queues + retries.
- D.4: Interview canvas state lost; consent retention failed; KrampusCorp HR surfaces issue.

## E. ADR coverage

| ADR | Test cases |
|---|---|
| ADR-0145 §A1 no-shared-DB | B.1, B.7 |
| ADR-0145 §A3 explicit-tenant-id | B.1, B.2 |
| ADR-0244 audience_type | B.9 |
| ADR-0299 passkey survives | B.8 |
| ADR-0311 personal-tenant autonomy | B.2, B.3 |
| FCRA fairness | B.4 |
| US-NY Pay-Transparency | B.10 |

## Completion expansion — j145 integration rigor pass
...
Test case 005: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 025: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 033: default-deny refusal for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 061: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 065: default-deny refusal for workflow-engine seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 089: default-deny refusal for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 117: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 137: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 145: default-deny refusal for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 153: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 173: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 193: default-deny refusal for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 201: default-deny refusal for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 209: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 221: audit-chain seal verification for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 229: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 233: default-deny refusal for workflow-engine seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 237: audit-chain seal verification for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 241: default-deny refusal for tenancy seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 245: audit-chain seal verification for community seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 257: default-deny refusal for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 265: default-deny refusal for payments seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 269: audit-chain seal verification for tenancy seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 273: default-deny refusal for community seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 277: audit-chain seal verification for mail seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
...
Test case 285: audit-chain seal verification for meet seeds tenant fixtures, executes the journey action, and asserts no unrelated tenant row is read.
