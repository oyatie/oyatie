...
critical_path_rows_satisfied:
  - "§3.2.5 row 21 — Pseudonymous + privacy-by-default (partial)"
pack_overlays_activated:
  - pack-us-irs-2024-tax
  - pack-us-state-va-tax
  - pack-us-state-ca-tax
  - pack-us-state-va-cdpa-2023
...
# j128 — Diana's personal-tenant family-tax filing via Workflow Studio

## At a glance

Diana Reyes spends two hours on her family-room couch on Sunday
afternoon 2026-04-12 to file her family's federal + Virginia + California
joint tax return using Workflow Studio in her PERSONAL tenant. She
pulls Stripe vinyl-sales transactions, Vanguard dividends, her
GAO W-2 (via NFC cross-tenant write, not via GAO directly),
Jennifer's Smithsonian W-2 (via spouse-tax-collaboration cross-tenant
permit), and runs a 15-step workflow that produces sealed final PDFs
+ IRS MeF + VA DOR + CA FTB submissions + a $3,127 Stripe payment.
Her GAO agency tenant has ZERO visibility into any of this. Tenant
boundary preserved.
...
|---|---|---:|
| [`story.md`](story.md) | Diana's Sunday afternoon | ≥800 |
...
- §3.2.5 row 21 — partial (personal-tenant pseudonymity in the broader
  context: GAO does not see Diana's tax filings)

## Cross-references

### Sibling dual-tenant journeys

- [j126](../j126-government-auditor-3pao-conducts-fedramp-audit/) — Foundation (work side)
- [j127](../j127-dual-tenant-identity-employee-resigns-and-keeps-personal/) — Transition
- [j129](../j129-court-warrant-pierces-personal-tenant-with-judicial-oversight/) — Judicial piercing
...
- [j66](../j66-tax-quarterly-filing-multi-jurisdiction/) — Multi-jurisdiction tax
- [j48](../j48-sidebusiness-stripe-tax-and-invoicing/) — Side business tax

### Binding ADRs

- ADR-0311 (dual-tenant)
- ADR-0245 (substrate-vs-product)
- ADR-0246 amendment (library-first dispatch)

## Hyperscaler precedents

- Apple Shortcuts on Personal Apple ID (workflow primitive)
- Google Workspace vs Personal Google Drive (productive personal tenant)
...
j128 demonstrates that the personal tenant has the full platform
substrate, scoped per-tenant. The architecture's invariants:

1. Substrate = shared (Workflow Engine, Connect, Intelligence run
   for any tenant).
2. Product instance = tenant-scoped (Diana's "family-tax-2025"
   workflow lives in `diana-reyes-personal-92381` only).
3. Cross-tenant permits = explicit (NFC, Smithsonian, IRS — all
   explicitly granted by counterparty tenants).
4. GAO tenant = zero visibility (verified by integration test B.1,
   B.2).

## Completion expansion — j128 readme rigor pass
...
Coverage row 001: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0299.
...
Coverage row 005: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 009: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0317.
...
Coverage row 013: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0314.
...
Coverage row 017: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0311.
...
Coverage row 021: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0299.
...
Coverage row 025: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0244.
...
Coverage row 029: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 033: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0314.
...
Coverage row 037: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0311.
...
Coverage row 041: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 045: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0244.
...
Coverage row 049: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0317.
...
Coverage row 053: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0314.
...
Coverage row 057: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0311.
...
Coverage row 061: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0299.
...
Coverage row 065: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 069: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0317.
...
Coverage row 073: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0314.
...
Coverage row 077: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0311.
...
Coverage row 081: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0299.
...
Coverage row 085: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0244.
...
Coverage row 089: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 093: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0314.
...
Coverage row 097: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0311.
...
Coverage row 101: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 105: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0244.
...
Coverage row 109: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0317.
...
Coverage row 113: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0314.
...
Coverage row 121: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0299.
...
Coverage row 125: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 129: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0317.
...
Coverage row 133: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0314.
...
Coverage row 141: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0299.
...
Coverage row 145: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0244.
...
Coverage row 149: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 153: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0314.
...
Coverage row 157: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0311.
...
Coverage row 161: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 165: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0244.
...
Coverage row 169: workflow-engine owns durable orchestration, state-machine replay, and idempotent cross-service compensation and cites ADR-0317.
...
Coverage row 177: payments owns settlement, payout, deduction, escrow, tax, and marketplace-facilitator ledgering and cites ADR-0311.
