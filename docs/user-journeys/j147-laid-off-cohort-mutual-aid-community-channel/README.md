...
# j147 — The 200-laid-off cohort mutual-aid Community channel

## What this journey is

200 RIF colleagues from Marcus's multinational opt into a private cohort Community channel. The cohort is a Cedar-gated sub-tenant under the former-employer; members verify their employment cryptographically; share leads, resources, mutual aid; do cross-tenant referrals (one cohort member, Reza, refers Olu to Coda); and at T+12mo elect to transition to a member-owned community-co-op tenant.

## Why this journey exists

j147 demonstrates the **alumni cohort as a long-lived, cryptographically-attested community** — a primitive missing from current industry tools (LinkedIn alumni groups are honor-system; TeamBlind verifies employer but not separation; this is the gap oyatie fills).

## µservices touched (4)

| µservice | Role |
|---|---|
| community | Sub-tenant cohort channel; multi-channel namespace; moderation; structured referrals; governance vote |
| identity | Verified-former-employer attestation issuance + public verification |
| messenger | DM between members; anti-fraud detection at cohort scope |
| workflow-engine | Cross-tenant referral application routing; governance-vote workflow; long-lived governance transition |

## Key ADRs

- **ADR-0299** account-recovery + attestation primitive
- **ADR-0249** multi-category community (cohorts as a class)
- **ADR-0292** marketplace primitive (referrals + bonuses)
- **ADR-0307** detection-substrate (HRRP at cohort scope)
- **ADR-0311** sub-tenant cohort ownership
- **ADR-0244** audience_type alumni overlay

## Labor-law anchors

- **US NLRA 1935** — protected concerted activity (alumni-cohort discussion of employment is protected)
- **US FCRA 1970** — fairness on background-check signals from peer referrals
- **EU GDPR Article 89** — public-interest archive exception (cohort archival evidence)
- **US WARN Act** — cohort-class-action context (≥200 layoffs is the threshold)

## Files in this bundle

| File | Purpose |
|---|---|
| `story.md` | Cohort-formation through 12-month transition arc |
| `ux-flow.md` | Screen-by-screen |
| `handshake.md` | Wire-level cohort + cross-tenant referral + governance |
| `schemas/VerifiedFormerEmployerAttestation.json` | The cryptographic primitive |
| `integration-test-plan.md` | B.1-B.10 + chaos |

Per-µservice IPs:
- `microservices/community/IP-journey-j147-cohort-sub-tenant-and-referrals.md`
- `microservices/identity/IP-journey-j147-verified-former-employer-attestation.md`
- `microservices/messenger/IP-journey-j147-cohort-dm-anti-fraud.md`
- `microservices/workflow-engine/IP-journey-j147-cohort-governance-and-cross-tenant-referrals.md`

## Status

Draft. Authored 2026-05-20 as part of Wave-3-F.

## Completion expansion — j147 readme rigor pass
...
Coverage row 005: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 009: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0297.
...
Coverage row 013: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 033: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 041: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 045: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 061: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 065: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0297.
...
Coverage row 069: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 105: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 129: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 145: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 149: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0297.
...
Coverage row 153: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 161: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 165: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0311.
...
Coverage row 173: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 181: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 185: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0299.
...
Coverage row 189: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 201: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
...
Coverage row 209: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0320.
...
Coverage row 217: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0244.
...
Coverage row 221: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0311.
...
Coverage row 225: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0292.
Reader path 226: start at this README, inspect the story, follow ux-flow, verify handshake, parse schemas, then run integration-test-plan before touching implementation.
Done signal 227: artifact line floors pass, all schema files include _meta, all service IPs are >=400 lines, and the personal/work tenant boundary remains explicit.
Risk note 228: any future edit that weakens tenant labels, Cedar purpose binding, or audit-chain emissions must update ADR references and tests in the same change.
Coverage row 229: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0317.
Reader path 230: start at this README, inspect the story, follow ux-flow, verify handshake, parse schemas, then run integration-test-plan before touching implementation.
Done signal 231: artifact line floors pass, all schema files include _meta, all service IPs are >=400 lines, and the personal/work tenant boundary remains explicit.
Risk note 232: any future edit that weakens tenant labels, Cedar purpose binding, or audit-chain emissions must update ADR references and tests in the same change.
Coverage row 233: identity owns principal resolver, audience-type classifier, passkey continuity, and tenant-membership boundary and cites ADR-0297.
Reader path 234: start at this README, inspect the story, follow ux-flow, verify handshake, parse schemas, then run integration-test-plan before touching implementation.
