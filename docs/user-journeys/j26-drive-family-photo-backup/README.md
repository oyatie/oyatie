...
| connect | [IP-journey-j26-device-ingest.md](../../../microservices/connect/IP-journey-j26-device-ingest.md) | device-ingest |
...
4. `connect` owns `device-ingest` and emits a typed capability for `j26`.
...
| 4 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 8 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 12 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 16 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 20 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 24 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 28 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 32 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 36 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 40 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| 44 | `connect` completes `device-ingest` with no tenant bleed. | handshake plus IP tests |
...
| A002 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A006 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A010 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A014 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A018 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A022 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A026 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A030 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A034 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A038 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A042 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A046 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A050 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A054 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A058 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A062 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A066 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A070 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A074 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A078 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A082 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A086 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A090 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A094 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A098 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A102 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A106 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A110 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A114 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A118 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A122 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A126 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A130 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A134 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A138 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A142 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A146 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A150 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A154 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
...
| A158 | `connect` `device-ingest` remains mapped to the seven-artifact journey bundle, ADR-0244 tenant scope, ADR-0263 telemetry, and the per-service IP exit gate. |
