...
| 4 | `j26-screen-04` | connector | Continue `drive-family-photo-backup` step 4. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 8 | `j26-screen-08` | connector | Continue `drive-family-photo-backup` step 8. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 12 | `j26-screen-12` | connector | Continue `drive-family-photo-backup` step 12. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 16 | `j26-screen-16` | connector | Continue `drive-family-photo-backup` step 16. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 20 | `j26-screen-20` | connector | Continue `drive-family-photo-backup` step 20. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 24 | `j26-screen-24` | connector | Continue `drive-family-photo-backup` step 24. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 28 | `j26-screen-28` | connector | Continue `drive-family-photo-backup` step 28. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 32 | `j26-screen-32` | connector | Continue `drive-family-photo-backup` step 32. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 36 | `j26-screen-36` | connector | Continue `drive-family-photo-backup` step 36. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 40 | `j26-screen-40` | connector | Continue `drive-family-photo-backup` step 40. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 44 | `j26-screen-44` | connector | Continue `drive-family-photo-backup` step 44. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 48 | `j26-screen-48` | connector | Continue `drive-family-photo-backup` step 48. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 52 | `j26-screen-52` | connector | Continue `drive-family-photo-backup` step 52. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 56 | `j26-screen-56` | connector | Continue `drive-family-photo-backup` step 56. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 60 | `j26-screen-60` | connector | Continue `drive-family-photo-backup` step 60. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 64 | `j26-screen-64` | connector | Continue `drive-family-photo-backup` step 64. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 68 | `j26-screen-68` | connector | Continue `drive-family-photo-backup` step 68. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 72 | `j26-screen-72` | connector | Continue `drive-family-photo-backup` step 72. | Runs `device-ingest`, preserves tenant context, and shows localized recovery if needed. |
...
| 4 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 8 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 12 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 16 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 20 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 24 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 28 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 32 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 36 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 40 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 44 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| 48 | `connector` rejects stale, replayed, or over-scoped request. | Safe retry or appeal path; event `j26.connect.ux_recovery`. |
...
| UX-A002 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A006 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A010 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A014 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A018 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A022 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A026 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A030 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A034 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A038 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A042 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A046 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A050 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A054 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A058 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A062 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A066 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A070 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A074 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A078 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A082 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A086 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A090 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A094 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A098 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A102 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A106 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A110 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A114 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A118 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A122 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A126 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A130 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A134 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A138 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A142 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A146 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A150 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A154 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A158 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A162 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A166 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A170 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
...
| UX-A174 | `connector` `device-ingest` has stable layout, keyboard path, screen-reader label, localized recovery copy, and no default abuse-defence friction. |
