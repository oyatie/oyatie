...
Aiko + Joshua coordinate the outreach. For each Band A supplier, MFI sends a structured request via the `connector` µservice's cross-tenant data exchange channel. Each request carries:
...
In early January Aiko returns to work + opens the `connector` µservice's supplier-data ingest panel. The ingest workflow is structured per-supplier:
...
- **Day 2 (Tue Feb 24)**: Scope-3 reconciliation. The E&Y team uses `connector` µservice's per-supplier Merkle proof to verify each Band A supplier's submission. They sample 5 of 47 Band A submissions. They request the cross-tenant audit-chain replay for each — the dual-seal in MFI tenant + supplier tenant must match byte-for-byte. All 5 match.
