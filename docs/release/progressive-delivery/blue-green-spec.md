...
Traffic-shift is driven by Argo Rollouts BlueGreen strategy or by Flagger's pre-promotion gates, both invoking the mesh (Istio Ambient per [ADR-0044](../../decisions/ADR-0044-service-mesh-istio-ambient-and-envoy-gateway.md)) via `oya-platform-traffic-shift-kernel` (NEW; adapter pattern).
