...
| `staging` | `staging` (Layer 3) | dev-tier + staging-tier consumers + canary cohort | `evidence: pass` (full replay; eval-harness green per [ADR-0024](../../decisions/ADR-0024-foundry-eval-harness-and-replay.md)) | `staging-promoter` agent (autonomous) |
| `prod` | `prod` (Layer 4) | all consumers honouring autonomy ceiling ([ADR-0022](../../decisions/ADR-0022-autonomy-ceiling-runtime-enforcement.md)) | `evidence: pass` + canary-100% + SLO-clean + comments-resolved + (per change class) reviewer re-affirm | `prod-promoter` agent |
...
Per [ADR-0024](../../decisions/ADR-0024-foundry-eval-harness-and-replay.md), every capability has an eval-set. The eval-harness runs:
...
- Capability registry implementation — owned by [ADR-0021](../../decisions/ADR-0021-foundry-capability-registry-and-mcp-gateway.md).
- Eval-harness — owned by [ADR-0024](../../decisions/ADR-0024-foundry-eval-harness-and-replay.md).
- Autonomy-ceiling runtime — owned by [ADR-0022](../../decisions/ADR-0022-autonomy-ceiling-runtime-enforcement.md).
...
- [ADR-0055](../../decisions/ADR-0055-four-layer-branch-pipeline.md) — capability stages mirror the four branch layers defined in ADR-0055; `oya-foundry-fitness-capability-stage-binding` is a branch-pipeline lane.
