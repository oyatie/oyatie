...
**Canary cohort.** Per [`../progressive-delivery/canary-rail-spec.md`](../progressive-delivery/canary-rail-spec.md) stage progression (1% → 5% → 25% → 50% → 100%). Stable cohort (regulated tenants) sees `false` at every canary stage.
...
When a single change crosses ≥ 2 axes (per [ADR-0011](../../decisions/ADR-0011-cross-axis-contract-registry.md)):
...
- Cross-axis contract registry — owned by [ADR-0011](../../decisions/ADR-0011-cross-axis-contract-registry.md).
- Vertical pack regulatory bindings — owned by [ADR-0034](../../decisions/ADR-0034-per-vertical-data-class-overrides.md).
