...
Hyperscaler default (Google, Microsoft) is **trunk-based development** — one long-lived branch, short-lived feature branches, every merge gated by reviewer + green CI. DORA research identifies TBD as an elite-engineering-org capability.
...
- **Feature flags hide incomplete work.** Per [`feature-flag-architecture.md`](../progressive-delivery/feature-flag-architecture.md).
...
The model **does not** tolerate brokenness on `origin/dev` — the 3-gate refuses red. The model **does** accept that **canary regressions can happen on staging**, because canaries are real-traffic experiments and regression is the data the experiment is designed to detect. The `staging-fixer` agent has a **4-hour SLA** on canary-regression events; the regression is bounded in time + blast radius (canary cohort only, not all-tenant) via the progressive-delivery rails ([ADR-0040](../../decisions/ADR-0040-progressive-delivery-canary-blue-green-metric-gated-rollback.md)).
...
| Metric | Target | Lane |
|---|---|---|
| `local_dev_pr_open_to_merge_p95_minutes` (local-dev PR open → origin/dev merge) | ≤ 30 min | `governance-dev-promotion-cadence` (MED) |
