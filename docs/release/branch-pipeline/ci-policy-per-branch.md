...
Every CI lane is invoked through `intelligence-ci-runner-kernel` (NEW) + per-provider adapter, per [Directive 4](../../../docs/MASTERPLAN.md) and the existing Foundry adapter posture ([ADR-0020](../../decisions/ADR-0020-intelligence-multi-provider-adapter-model.md)):

- `intelligence-ci-runner-adapter-github-actions` — GitHub Actions workflow_dispatch.
- `intelligence-ci-runner-adapter-buildkite` — Buildkite pipeline trigger.
- `intelligence-ci-runner-adapter-circleci` — CircleCI v2 API.
- `intelligence-ci-runner-adapter-1es` — Microsoft 1ES templated pipelines.
- `intelligence-ci-runner-adapter-gitlab-ci` — GitLab pipeline trigger.
...
|---|---|---|---|
| `governance-cohesion` | BLOCKER | every PR / commit | [ADR-0001](../../decisions/ADR-0001-cohesion-thesis-one-product-seven-axes.md) |
| `governance-supply-chain` | BLOCKER | every PR / commit | [ADR-0039](../../decisions/ADR-0039-supply-chain-security-trivy-cosign-sbom-signed-commits.md) |
| `governance-api-semver` | BLOCKER | every PR touching `contracts/` | [ADR-0037](../../decisions/ADR-0037-public-api-stability-tiers-and-deprecation.md) |
| `governance-pr-shape` | BLOCKER (PR-level) | every PR | repo PR template |
| `governance-pr-review-verdict-present` | BLOCKER (local-dev → origin/dev) | every PR | this composer; NEW |
| `governance-promotion-gate-local-dev-to-origin-dev` | BLOCKER (gate-class) | local-dev → origin/dev | this composer; NEW |
| `governance-image-discipline` | BLOCKER | every PR touching `Dockerfile`/`Containerfile` | [Directive 5](../../../docs/MASTERPLAN.md) |
| `governance-canary-regression-sla` | HIGH | staging | this composer; NEW |
| `governance-pr-comment-resolution` | BLOCKER (staging → prod) | staging-landed change sets | this composer; NEW |
| `governance-promotion-gate-staging-to-prod` | BLOCKER (gate-class) | staging → prod | this composer; NEW |
| `governance-canary-required` | BLOCKER | prod | [`../progressive-delivery/progressive-delivery-strategy.md`](../progressive-delivery/progressive-delivery-strategy.md) |
| `governance-rollback-evidence` | BLOCKER | prod | [`../progressive-delivery/blue-green-spec.md`](../progressive-delivery/blue-green-spec.md) |
| `pipeline-slo-coverage` | HIGH | every service | [ADR-0042](../../decisions/ADR-0042-observability-stack-otel-and-in-house-ui.md) |
...
**Re-run policy.** Lane re-runs on every commit pushed to the PR HEAD (the agent may have addressed a `REQUEST_CHANGES` verdict). Lane outcomes are stored in `intelligence-ci-state-store` keyed by commit SHA.
...
**Gate semantics on `origin/dev` → `staging` promotion:** **none.** `staging-promoter` does not consult CI. The re-run on `origin/dev` HEAD is observational — outcomes recorded; red lanes here indicate a `dev-promoter` orchestration bug (squash produced something different from the PR HEAD) which is a `governance-cohesion`-class incident.

**Mutator constraint.** Only `dev-promoter` agent may merge to `origin/dev` (via `gh pr merge --squash`). Planned advisory lane: `governance-no-direct-origin-dev-commit` (planned blocker).
...
**Red-lane handling on staging.** If a lane goes red on `staging` HEAD (despite being green at dev entry), this signals integration-level breakage (e.g., a flake, an environmental dependency, or a regression that the PR-time runner didn't catch). The `staging-fixer` agent picks up via `EVT-CI-RED-<job>` and fixes through the standard PR flow (worktree → local dev → origin/dev → staging). **Cannot commit directly to staging** — planned advisory lane: `governance-no-direct-staging-commit` (planned blocker).
...
- `governance-slo-burn-rate-fast` (zero open alerts; freshness ≤ 5 min)
