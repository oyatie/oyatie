...
**Constraints encoded.** No force-push (NEVER); commits come only from `prod-promoter`; fast-forward only; Cosign-signed commits + SLSA L2+ provenance + SBOM 2.3 attestations all mandatory per [ADR-0039](../../decisions/ADR-0039-supply-chain-security-trivy-cosign-sbom-signed-commits.md); the 5-gate verification lanes are required status checks.
...
Each promoter agent ships with a Cosign-keyless identity (per [ADR-0039](../../decisions/ADR-0039-supply-chain-security-trivy-cosign-sbom-signed-commits.md)). The git-server's `restrictions.apps` allowlist accepts only commits signed by those identities. The identity binding is itself audit-chain-emitted at agent boot; rotation requires a Cosign-signed rotation record + a council-architecture approval.
...
- Cosign identity rotation — owned by [ADR-0039](../../decisions/ADR-0039-supply-chain-security-trivy-cosign-sbom-signed-commits.md).
