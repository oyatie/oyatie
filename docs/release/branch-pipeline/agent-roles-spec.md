...
2. Reviewer-agent dispatch — by file-glob change class (per §6 of this file + `docs/AGENTS.md`), invokes each applicable reviewer agent via the agent-runtime Skill API; waits for `APPROVE` verdict (or `REQUEST_CHANGES`); records via `icm store -t pr-review-verdicts`.
3. CI clearance — polls fitness-lane outcomes via `oya-foundry-ci-state-store`; requires every lane GREEN on the PR HEAD.

When all three gates green simultaneously, invokes `gh pr merge --squash --auto` (under [Directive 12](../../../docs/MASTERPLAN.md) with `icm store -t direct-tool-invocations -c '<rationale>' -i high -k 'gh,dev-promoter'`).
...
**Authority.** May claim symbols, modify file contents, add tests, revert specific origin/dev commits via the standard PR path. **Cannot** commit directly to `staging` or `prod` or `origin/dev`. **Cannot** change architectural shape — requires ADR + human review per [Directive 2](../../../docs/MASTERPLAN.md).
...
| Migrations / SQL (`migrations/**/*.sql`) | `database-reviewer` | Approve / Request-Changes (BLOCKER class — [ADR-0045](../../decisions/ADR-0045-database-tier-strategy.md)) | **yes** |
...
Verdict recorded via `icm store -t pr-review-verdicts` per [ADR-0053](../../decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md); without the record, `oya-foundry-fitness-pr-review-verdict-present` blocks promotion.
...
- [ADR-0053](../../decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md) — all promoter agents use `grit` + `icm` + `oya-tooling-agent-read`; direct tool invocations logged via `icm store -t direct-tool-invocations`.
- [ADR-0055](../../decisions/ADR-0055-four-layer-branch-pipeline.md) — this file specifies the agent roles that operate the four-layer pipeline defined in ADR-0055.
