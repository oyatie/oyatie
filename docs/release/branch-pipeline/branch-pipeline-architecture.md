...
2. **Reviewer-agent verdict = `APPROVE`.** Per-change-class dispatch (per `docs/AGENTS.md`): `rust-reviewer`, `typescript-reviewer`, `python-reviewer`, `database-reviewer`, `security-reviewer`, `privacy-reviewer`, `tdd-guide`, `silent-failure-hunter`, `doc-updater`, `doc-style-reviewer`, `capability-reviewer`, `perf-reviewer`. Multi-class PRs invoke multiple reviewers in parallel; all must `APPROVE`. Verdict recorded via `icm store -t pr-review-verdicts -c '<verdict + items>' -i high -k 'pr-<id>,<reviewer>'` per [ADR-0053](../../decisions/ADR-0053-grit-icm-as-sanctioned-primitives.md). Enforced by `oya-foundry-fitness-pr-review-verdict-present` (BLOCKER, scoped to this transition).
