...
**Positions.**
- **LINUX ADR-0014** (`docs/decisions/ADR-0014-container-runtime-unified-multi-isolation.md`, Accepted): ONE OCI/CRI frontend + pluggable `IsolationBackend` port spanning `Native|Sandbox|Microvm|Confidential`; **owned Rust VMM** behind a `Vmm` port with firecracker-minimal / cloud-hypervisor-rich profiles; guest kernel = the framekernel. Reuse rust-vmm / Firecracker / Cloud-Hypervisor **now** (Stage 2), own the VMM **later** (Stage 3) only on a benchmark gain.
