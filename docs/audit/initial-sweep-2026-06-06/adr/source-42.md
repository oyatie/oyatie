...
- **truth_flag:** PARTIAL. Crypto/protocol substance is TRUE and current. BUT the §B-5 per-µservice protocol table still lists a `foundry` row ("retired external agent harness pipeline; internal") — **both "foundry" the µservice AND "retired external agent harness" are RETIRED** (ADR-0335 foundry→intelligence; GLOSSARY L241/L1042 retired external agent harness retired). That row is stale-vocab and should read `intelligence` (or be dropped). The table also lists `governance` and `policy-engine` as separate rows — consistent with 0335/0347 (foundry split into intelligence + governance), so the framework is right; only the literal `foundry` row is wrong.
...
- **tensions:** (1) Retired `foundry`/`retired external agent harness` row (above). (2) ECH cites "RFC 9460" for Encrypted Client Hello — ECH is actually `draft-ietf-tls-esni` (the ADR's own §D-3 correctly says "draft-ietf-tls-esni-18 (RFC 9460 when published)"); RFC 9460 is SVCB/HTTPS records. Minor technical-citation imprecision, not load-bearing. (3) References ADR-0121 (onprem kubeadm/containerd/istio) in `related` — that stack is superseded by ADR-0375 (Talos); the network amendment should reference the Talos/Cilium substrate, not the retired istio stack.
- **hyperscaler_challenge:** ALIGNED. Cloudflare/Google/AWS are exactly the deployers of HTTP/3 + ECH + X25519MLKEM768 hybrid (this is current 2024–2026 industry frontier). A hyperscaler would make this decision. Implication: KEEP; amend the foundry/retired external agent harness row + istio reference.
- **ai_slop:** LOW. Dense but genuinely technical; code-points, OIDs, and SLO thresholds are specific and checkable. Not padding.
- **refinement:** Replace §B-5 `foundry`/"retired external agent harness" row with `intelligence`; swap ADR-0121 reference for ADR-0375; tighten the RFC-9460-vs-ECH-draft citation.
...
- ADR-0354 §B-5: a literal `foundry` µservice row labeled "**retired external agent harness** pipeline" (→ intelligence; both terms retired, ADR-0335/0347, GLOSSARY).
