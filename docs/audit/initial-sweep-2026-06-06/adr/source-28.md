...
- **tensions:** (1) **Retired-brand drift** — uses "Foundry" and "retired external agent harness" as live internal labels; per map §1.3 foundry-dissolution chain (0136→0239→0247/0255→0335) the brand is dead. This is the same M-04 (Foundry/intelligence audience conflation) mistake that 0221 codified a CI gate against — ironically 0220 *is* the audience-split ADR yet still spells the internal side "Foundry." (2) `axis-foundry` listed as a Decider — a retired axis label. (3) ADR-0136 cited as "Foundry internal only, Accepted" — but map §1.3 flags 0136 as stale-front-matter (declared superseded by 0335/0247 yet still reads Accepted on disk); trust the superseding ADR. (4) Shared Milvus aligns with map §3 data posture (Milvus canonical, ADR-0192) — no conflict.
