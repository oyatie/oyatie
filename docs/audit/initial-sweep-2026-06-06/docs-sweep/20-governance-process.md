...
- **Brand-check checklists guard the wrong brand:** `checklists/pre-push.md:30` and `pre-merge.md:32` only police the `Oyatie` string (per ADR-0017); **neither mentions `foundry`** — so the retired brand is explicitly *outside* enforced brand hygiene. The rename was never operationalized as policy.
