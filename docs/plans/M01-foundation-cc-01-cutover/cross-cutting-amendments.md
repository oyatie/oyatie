...
Captured 2026-05-12 by the orchestrator. **Status**: Accepted — pending user execution approval (deferred until masterplan + hyperscaler research finish).

This document codifies the cross-cutting constraints that landed AFTER the iter-2 Critic APPROVE on the cutover plan (see `INDEX.md`). The consensus plan is unchanged; this amendment is read alongside the plan by every execution agent. Constraints here inherit into every phase (P0.5 through P10) of the cutover.
...
Per the LTS-lookup agent's findings at `docs/standards/lts-versions-verified.md` (ADR-0055), the cutover-deliverable phases (P2 helper crate + P3/P10 fitness lanes + P6/P7 archive-orphan lane) MUST adopt these current-LTS pins:

| Dependency | Cutover-required pin | Why |
|---|---|---|
| Rust toolchain | `>= 1.85` (workspace `rust-version`); current stable 1.95.0 | cargo-deny 0.19.5+ requires 1.85 MSRV |
...
Per the hyperscaler-research agent's output at `docs/standards/hyperscaler-best-practices.md` (ADR-0055), these practices apply to the cutover phases:

- **Working Backwards / PRFAQ**: not applicable to internal cutover; reserved for product launches.
- **Design Doc** (Google): each cutover phase already has a Design-Doc-equivalent in the spec tree (deep-dive spec + ADR drafts).
...
When the masterplan composer's output lands at `docs/MASTERPLAN.md`:

- The cutover plan should be moved under `docs/plans/milestones/M-CC-01-agentic-pipeline/` (or under M01-Foundation if composer placed it there).
...
- This document (`cross-cutting-amendments.md`) lifts into the milestone's INDEX.md as the §Inherited-constraints section.
...
Cross-cutting amendments: **this document** (Accepted 2026-05-12).
...
1. Read `docs/MASTERPLAN.md` to confirm cutover's placement in the 4-tier hierarchy.
2. Read `docs/standards/hyperscaler-best-practices.md` to update §4 above with concrete practice citations.
3. Surface the consolidated picture to user via AskUserQuestion.
