...
## Symptom
EDI counterparty (carrier, customs, broker) is unreachable; outbound EDI documents queued

## Detection
- Source signal: EDI ack-rate dashboard; per-counterparty health probe
...
## Containment
Notify the affected counterparty; switch to backup interchange method if SLA-critical (e.g. customs filing)

## Diagnosis
Identify outage scope (counterparty-side, network, or our adapter); check for protocol drift (X12 vs EDIFACT version)

## Recovery
Resume EDI; replay queued documents; verify counterparty ack; emit MFL row if structural
