...
## Symptom
KMS DEK or KEK suspected compromise; emergency rotation required

## Detection
- Source signal: Security-review trip; per-tenant access anomaly; vendor security advisory
...
## Containment
Quarantine the suspected DEK/KEK scope; pause writes on affected tenant cells; preserve KMS audit log

## Diagnosis
Verify scope of compromise (DEK / KEK / HSM); check for cross-tenant exposure; coordinate with regulators if PHI/PCI/PII was at rest under the affected key

## Recovery
Rotate KEK; re-wrap DEKs; re-encrypt at-rest data per affected store; revoke old keys; emit incident notification per COMPLIANCE-MATRIX SLA
