...
# Oyatie Runbook — Cloud IAM Key Rotation
...
## Symptom
scheduled or emergency IAM key rotation across cloud control + data plane

## Detection
- Source signal: Key-age dashboard; KMS rotation policy alert; emergency-rotation page
...
## Containment
Pause writes that depend on the rotating key class; ensure rolling rotation does not break in-flight workloads

## Diagnosis
Identify which workloads bind to the rotating key; verify dual-key window suffices; check secret-store sync cadence

## Recovery
Issue new key; dual-key window; cut over readers; revoke old key; verify with synthetic transactions; emit EVT-KEY-ROTATED
