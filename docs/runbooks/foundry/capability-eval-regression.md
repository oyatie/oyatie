...
# Oyatie Runbook — Capability Eval Regression
...
## Symptom
a per-capability eval run regressed below pass-threshold after a model upgrade or capability change

## Detection
- Source signal: Per-capability eval pass-rate trend; nightly eval lane block; ADR-0024 replay-as-eval anomaly
...
## Containment
Hold the model rollout (or capability change) at the affected percentile; preserve eval traces

## Diagnosis
Compare the failing eval cohort against golden + adversarial + linguistic cohorts; identify the regression class (correctness / safety / cost / latency)

## Recovery
Either rollback the model upgrade, patch the capability, or update the eval cohort if the regression was a false-positive; emit MFL row if the gate failed open
