...
> **Owner:** `vertical-healthcare + ops-sre-reliability`
...
## Symptom
clinical-safety SLO breach (e.g. lab-result delivery latency, alert-fatigue spike, DDI-check miss)

## Detection
- Source signal: Per-clinical-SLO dashboard; per-tenant clinical-safety alert
...
## Containment
Notify clinical-safety officer; pause the affected clinical capability if patient harm risk

## Diagnosis
Identify whether anomaly is workflow, integration (HL7/FHIR), AI-capability, or data-quality

## Recovery
Restore SLO; if patient-impacting, file regulator notification; emit per-incident postmortem within Sev 1 SLA
