...
## Symptom
a marketplace listing must be taken down (security advisory, license violation, regulator order, vendor request)

## Detection
- Source signal: Vendor-security advisory; per-listing takedown ticket; regulator notification
...
## Containment
Hide listing from marketplace surface; freeze new installs; notify existing tenants

## Diagnosis
Verify takedown reason; check legal scope; coordinate with affected tenants for migration path

## Recovery
Take down listing; emit per-tenant migration plan; if security-driven, file MFL row
