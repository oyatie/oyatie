# Oyatie Runbook — Auction Engine Overload
...
## Symptom
ads auction engine throughput collapsed; auction win-rate degraded; per-publisher fill rate dropping

## Detection
- Source signal: Auction engine SLO dashboard; per-publisher fill-rate alert
...
## Containment
Throttle low-revenue publishers; failover to backup auction node; preserve auction-tier metrics

## Diagnosis
Identify overload: per-publisher spike, per-bidder loop, or backend (BigQuery / ClickHouse) saturation

## Recovery
Drain overload; restore SLO; ship per-bidder rate-limit if structural; emit MFL row
