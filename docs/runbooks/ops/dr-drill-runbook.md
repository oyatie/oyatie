# Oyatie Runbook — Disaster Recovery Drill
...
## Symptom
scheduled DR drill (per quarter per region per ADR-0049)

## Detection
- Source signal: DR-drill schedule
...
## Containment
Notify all affected teams; freeze production changes during drill window

## Diagnosis
Execute per-drill scenario (region-loss, cell-loss, AZ-loss, KMS-loss); measure RTO/RPO

## Recovery
Restore production; emit per-drill report; file DR gaps as MFL rows + tracking issues
