...
# Oyatie Runbook — Regulator Notification Procedure
...
## Symptom
an incident requires regulator notification (KISA / PIPC / FSC / KCC / NIS / MFDS / per-jurisdiction equivalent)

## Detection
- Source signal: Per-regulator notification SLA breach; per-incident regulator-impact assessment
...
## Containment
Notify Founder + council-privacy + Legal; preserve full incident evidence pack

## Diagnosis
Identify per-regulator SLA; coordinate per-region pack maintainer for jurisdiction-specific SLA + form

## Recovery
File per-regulator notification; emit EVT-REGULATOR-NOTIFIED; coordinate ongoing regulator inquiry
