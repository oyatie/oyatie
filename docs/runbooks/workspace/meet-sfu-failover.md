# Oyatie Runbook — Meet SFU Failover
...
## Symptom
Selective Forwarding Unit failure; meet sessions degraded or dropped

## Detection
- Source signal: Per-region SFU health dashboard; per-meeting drop-rate SLO
...
## Containment
Failover affected meetings to neighbor SFU; throttle new-meeting creation in affected region

## Diagnosis
Identify failure: SFU process crash, network, or cascading thermal

## Recovery
Restore SFU; verify per-meeting quality recovers; schedule capacity review if peak-load triggered
