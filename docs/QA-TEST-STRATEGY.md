...
`oya verify` (the persona-CLI per [TOOLCHAIN.md](TOOLCHAIN.md)) bundles `cargo fmt --check + cargo clippy + cargo nextest run + cargo machete + scripts/check-architecture-boundaries.sh + oya gate validate` into one pre-push gate.
