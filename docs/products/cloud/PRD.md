...
- ADR-0346 is amended by ADR-0515 for Cloud Provider acceptance: legacy `./bin/oya verify --ci-required` output is optional local-feedback/provenance only, while protected merge authority is `oya-ci-required`. Historical enforced-by cross-reference: `oya-governance-oya-verify-*` lane family retained as provenance only.
...
- ADR-0349 binds Cloud deployment surfaces to Jenkins CI wording is historical/provenance after ADR-0515; GitHub Actions produces `oya-ci-required` until explicit owned-runner cutover, and ArgoCD remains separately authorized CD evidence where applicable.
