...
- ADR-0513/ADR-0346 amendment binds Cloud Provider acceptance to Buck2-backed local verification and Prow/Kubernetes-native `oya-ci-required`; retired `oya verify` CLI surfaces are historical/local migration inputs only. Enforced-by cross-reference: Rust/Buck2/Prow governance mirror, step-exit, and skip-policy lanes.
