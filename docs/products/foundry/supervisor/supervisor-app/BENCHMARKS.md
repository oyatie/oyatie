...
cargo bench -p oya-foundry-supervisor-app -- heartbeat
...
cargo flamegraph -p oya-foundry-supervisor-app --bench heartbeat -- --profile-time 30
...
The CI lane `lean-settings-drift` (part of the larger `oya-foundry-fitness-*` suite) runs benchmarks on every PR:
