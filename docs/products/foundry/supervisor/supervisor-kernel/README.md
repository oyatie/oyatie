...
**Crate:** `oya-foundry-supervisor-kernel`  
...
#### `RoutePolicy`
Wraps `oya-foundry-route-policy-kernel`. Method: `select(eligible_accounts: &[ProviderAccount]) -> Result<AccountId, NoEligibleAccount>`.

#### `UsageEnforcement`
Wraps `oya-foundry-usage-window-kernel`. Enforces Cedar autonomy-tier ceilings via `check_limit(&ticket, &spend_record) -> Result<(), OverLimit>`.
