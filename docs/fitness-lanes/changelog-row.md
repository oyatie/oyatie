...
- kernel_crate: `oya-foundry-fitness-changelog-row-kernel` — `ChangelogRow { pr_id, touches_canonical, row_present, row_classification }`, verdict `ChangelogRowFitnessReport { prs_checked }`.
- runner_path: `tools/oya-foundry-fitness-changelog-row`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-changelog-row`
