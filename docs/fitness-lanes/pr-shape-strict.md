...
- kernel_crate: `oya-foundry-fitness-pr-shape-strict-kernel` — `PrBody { pr_id, h2_sections }`, verdict `PrShapeStrictFitnessReport { prs_checked }`.
- runner_path: `tools/oya-foundry-fitness-pr-shape-strict`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-pr-shape-strict`
