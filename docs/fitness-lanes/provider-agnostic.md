...
- kernel_crate: `oya-foundry-fitness-provider-agnostic-kernel` — `ProviderToken { path, line, token, in_adapter_crate }`, verdict `ProviderAgnosticFitnessReport { tokens_checked }`.
- runner_path: `tools/oya-foundry-fitness-provider-agnostic`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-provider-agnostic`
