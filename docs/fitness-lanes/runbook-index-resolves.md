...
- enforces: STANDARD/runbook-index; AGENTS.md fitness-lane `oya-foundry-fitness-runbook-index-resolves`.
- kernel_crate: `oya-foundry-fitness-runbook-index-kernel` — `RunbookEntry { runbook_id, path }`, `RunbookFile { path }`, verdict `RunbookIndexFitnessReport { entries_checked, files_checked }`.
- runner_path: `tools/oya-foundry-fitness-runbook-index-resolves`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-runbook-index-resolves`
