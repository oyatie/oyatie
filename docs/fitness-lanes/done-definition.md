...
- kernel_crate: `oya-foundry-fitness-done-definition-kernel` — `DoneDefinition { pr_id, items }`, `DItem { id, state, deferred_issue }`, verdict `DoneDefinitionFitnessReport { pr_count }`.
- runner_path: `tools/oya-foundry-fitness-done-definition`
...
- ci_invocation: `cargo run -p oya-foundry-fitness-done-definition`
