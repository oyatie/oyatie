...
- enforces: STANDARD/perf-evidence; AGENTS.md fitness-lane `oya-foundry-fitness-perf-evidence`.
- kernel_crate: `oya-foundry-fitness-perf-evidence-kernel` — `PerfEvidence { crate_id, p50_ms, p95_ms, p99_ms, source }`, verdict `PerfEvidenceFitnessReport { evidences_checked }`.
- runner_path: `tools/oya-foundry-fitness-perf-evidence`
...
- failure_modes:
  ...
  - p99 < p50 (impossible)
- ci_invocation: `cargo run -p oya-foundry-fitness-perf-evidence`
