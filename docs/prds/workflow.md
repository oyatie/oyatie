...
|---|---|---|---|
| `studio` | `oya-workflow-studio-{rest,graphql,sdk}` | Visual editor authoring surface; version history; template library | `WorkflowDefinition`, `WorkflowVersion` |
...
```
{studio-rest, studio-graphql, triggers-adapter, integrations-adapter, worker}
        ↑ depends on
...
**Active-active compatibility**: `stateless-compatible` for Studio REST/GraphQL
...
- Studio REST/GraphQL: stateless HPA on CPU >70%; min 2; max 50 pods.
