...
3. **`toolchains//:rust` is pinned to the workspace Rust + linker parity with `.cargo/config.toml`.** The Buck2 Rust toolchain target pins the same Rust channel as `rust-toolchain.toml` (currently 1.97.1) and reproduces the workspace's existing `.cargo/config.toml` rustflags / linker selection (mold + clang where configured) so Buck2 and `cargo` produce link-parity artifacts. Cargo stays usable for local dev; Buck2 is the canonical graph for hermetic/cache/affected-target builds.
...
## Historical residual from ADR-358 (E3 fold 2026-08-06)

**Title:** ADR-0358-ideal-production-roadmap-strangler-bazel-oya-overlay

**Preserved decision gist:** 1. **Migration posture = strangler-fig.** Spec the ideal target architecture and contracts first; build the ideal alongside the current tree; migrate service-by-service behind stable contracts (Workflow/Ontology adapter layer, OpenAPI 3.2.0 / AsyncAPI 3.1.0 / proto3). Trunk stays shippable; lanes run in parallel; a service cuts over only when green. No big-bang rewrite, no indefinite evolve-in-place. 2. **Toolchain overhaul = Bazel `rules_rust` build graph + `oya` governance overlay.** Bazel provides the build/test DAG, hermetic remote cache, remote build execution, and affected-target selecti
