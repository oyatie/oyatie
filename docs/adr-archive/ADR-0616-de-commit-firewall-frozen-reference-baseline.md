...
Before this decision, `gate-baseline.generated.json` was the firewall ratchet's FROZEN reference and
the last committed generated merge surface after ADR-0604/0613 de-committed every other face. The
emitter read that face as a committed blob at the merge base. ADR-0596 had forbidden de-committing it
because the ratchet read history, while ADR-0613's candidate-tree materialization pattern did not yet
cover a merge-base history read.

That committed posture caused recurring fragility: a capability MOVE had to relocate the committed
face, and the #828→#830 deadlock plus the 2026-07-09 keystone break both stemmed from committed-face
staleness/relocation (byte-identical relocation deferred content re-keying to an unwired postsubmit
controller).

The current implementation keeps the immutable merge-base source as the authority while deriving the
face rather than committing it. The materializer creates a detached worktree at the merge base and
regenerates the baseline twice from the immutable merge-base source. The emitter consumes the resulting
`--regen-baseline-face`; `resolve_merge_base_baseline_snapshot` in
`ci/facade/scm-facts-snapshot/src/lib.rs` requires that regenerated input and provides no committed-face
`git show` fallback. The producer (`ci/facade/artifact-inventory-registry`) remains `--repo-root`-pure
and ambient-git-free, so immutable history is preserved without treating generated bytes as authority.
...
## Historical residual from ADR-596 (E3 fold 2026-08-06)

**Title:** Forbid de-committing firewall frozen-reference artifacts — make the #828 ratchet-baseline deadlock class impossible

**Preserved decision gist:** The generated-artifact control-plane gate gains a new rule: > **`frozen_reference_artifact_must_stay_committed`** — RED when a declared artifact whose > `path` is a firewall FROZEN-REFERENCE is declared with a de-commit `materialization_mode` > (`not-tracked-in-git`, or any future mode whose semantics is "absent from the committed tree > / derive-on-demand"). The frozen-reference set is derived **universally from DATA, not hardcoded paths**: - **Primary signal (authoritative, repo-agnostic):** the path appears as `frozen_reference.face_path` in the repo's `ratchet-policy.json` (the firewall ra
