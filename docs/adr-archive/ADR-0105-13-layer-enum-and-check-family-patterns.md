...
## Historical residual from ADR-107 (E3 fold 2026-08-06)

**Title:** `tools/` directory canonical-suffix binding (was: implicit `app` layer — SUPERSEDED by ADR-0105)

**Preserved decision gist:** > **SUPERSEDED — historical content preserved for traceability. See §"Amendment 2026-05-15 — no-exception canonical naming" and §"Amendment 2026-05-15 — Superseded" above for the current (post-supersession) state.** ~~**Any crate under `tools/` is implicitly the `app` layer.**~~ Superseded 2026-05-15. The canonical rule is: every `tools/` crate ends in a canonical layer suffix; binaries use `-app`. The original text is preserved below for traceability only. ~~Any crate under `tools/` is implicitly the `app` layer.~~ No explicit `*-app` suffix is required. The directory location is the layer de
