...
2. CI lane `cloud-ci/Rust gate packet tenant-environment-tier` enforces (a) every µservice that performs outbound side effects checks env_tier, (b) every API-key issuance flow validates Cedar tier-grant, (c) every prod-tier destructive op carries the acknowledgment header.
