...
| `eventing.outbox.publish` | data | event | exactly-once via outbox + Kafka per ADR-0046; AsyncAPI source `contracts/asyncapi/platform/eventing-outbox-v1.yaml`; Protobuf source `contracts/proto/platform/eventing/v1/eventing-outbox-v1.proto` | stable | `oya-eventing-application` |
...
| `regulatory-pack.bind` | control | REST | per-tenant pack binding; multi-pack supported (rare); residency immutable post-bind; OpenAPI source `contracts/openapi/platform/platform-regulatory-pack-v1.yaml` | stable | `oya-platform-regulatory-pack-api` |

## 3. Axis 1 — SaaS surfaces

| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `workflow.definition.publish` | control | REST | versioned; semver per ADR-0035; jurisdiction overlay per regional pack | stable | `oya-saas-workflow-api` |
| `workflow.run.start` | data | REST | per-tenant cell-routed; idempotent | stable | `oya-saas-workflow-app` |
| `workflow.run.event` | data | event | per-step audit emission per ADR-0003 | stable | (per workflow runtime) |
| `plugin.manifest.register` | control | REST | per ADR-0036 schema; Cosign-signed per ADR-0039 | stable | `oya-saas-plugin-marketplace-api` |
| `plugin.invocation` | data | REST + WASM | Wasmtime-sandboxed per ADR-0023; capability-gated PluginContext | stable | `oya-saas-plugin-runtime-app` |
| `marketplace.listing.publish` | control | REST | per-vertical / per-region filterable; trust-tier per ADR-0036 | stable | `oya-saas-marketplace-api` |
| `bench.surface.render` | data | REST + websocket | per-tenant shell renderer per ADR-0017 | stable | `oya-saas-bench-app` |

## 4. Axis 2 — Workspace surfaces

| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `workspace.mail.smtp.receive` | data | SMTP | RFC 5321; per-tenant routing; phishing+DLP+classify before store | stable | `oya-workspace-mail-api` |
| `workspace.mail.imap` | data | IMAP | RFC 3501; per-tenant; per-folder access | stable | `oya-workspace-mail-api` |
| `workspace.mail.jmap` | data | JMAP | RFC 8620; per-tenant | stable | `oya-workspace-mail-api` |
| `workspace.calendar.caldav` | data | CalDAV | RFC 4791; per-tenant | stable | `oya-workspace-calendar-api` |
| `workspace.docs.crdt.connect` | data | websocket | Yrs CRDT; per-doc state-vector compatibility ≥ 2 versions | stable | `oya-workspace-docs-api` |
| `workspace.drive.put` | data | REST | per-object KMS-shred (per record DEK); per-permission ACL; OpenAPI source `contracts/openapi/workspace/workspace-drive-v1.yaml` | stable | `oya-workspace-drive-api` |
| `workspace.drive.get` | data | REST | per-tenant cell-routed; signed-URL OK; audit-emit per access; OpenAPI source `contracts/openapi/workspace/workspace-drive-v1.yaml` | stable | `oya-workspace-drive-api` |
...
| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `foundry.capability.invoke` | data | REST + MCP | autonomy-tier-gated; data-class-class-allowlisted; evidence-emitted | preview | `oya-foundry-api` |
| `foundry.capability.publish` | control | REST | per [`templates/capability-record-template.yaml`](templates/capability-record-template.yaml); eval-set pass required; OpenAPI source `contracts/openapi/foundry/registry-v1.yaml` | stable | `oya-foundry-registry-api` |
...
| `foundry.fitness.run` | control | CI lane | per Maturity Move #8 + cross-axis cohesion check per Foundry-improvements top-20 #13 | stable | `oya-foundry-fitness-*` |
...
| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `cloud.iam.role.create` | control | REST | per-tenant per-region; Cedar policy attached; OpenAPI source `contracts/openapi/cloud/cloud-iam-v1.yaml` | stable | `oya-cloud-iam-api` |
| `cloud.iam.sts.token` | control | REST | short-lived (≤ 1h); per-purpose bound; OpenAPI source `contracts/openapi/cloud/cloud-iam-v1.yaml` | stable | `oya-cloud-iam-api` |
...
| `cloud.cell.bind` | control | REST | per-tenant cell-routing assignment; OpenAPI source `contracts/openapi/cloud/cloud-cell-bind-v1.yaml` | stable | `oya-cloud-cell-application` |
...
| `cloud.storage.object.put` / `.get` | data | REST | per-object KMS-shred; per-bucket lifecycle; OpenAPI source `contracts/openapi/cloud/cloud-storage-object-v1.yaml` | stable | `oya-cloud-storage-object-api` |
| `cloud.storage.block.create` | control | REST | per-region per-AZ; per-tenant quota; OpenAPI source `contracts/openapi/cloud/cloud-storage-block-v1.yaml` | stable | `oya-cloud-storage-block-api` |
| `cloud.network.vpc.create` | control | REST | per-tenant; per-region; OpenAPI source `contracts/openapi/cloud/cloud-network-vpc-v1.yaml` | stable | `oya-cloud-network-vpc-api` |
...
| `cloud.billing.invoice.generate` | analytics | REST | per-region tax-invoice format (KR 전자세금계산서, JP 適格請求書, EU per-country, IN GST, BR NF-e, KSA FATOORA, UAE); OpenAPI source `contracts/openapi/cloud/cloud-billing-invoice-v1.yaml` | stable | `oya-cloud-billing-tax-application` |
...
| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `search.crawler.schedule` | control | REST | per-host politeness; per-corpus rights ledger | preview | `oya-search-crawler-api` |
| `search.crawler.frontier` | data | event | per-URL deduplication; per-host budget | preview | `oya-search-crawler-frontier-app` |
| `search.parser.process` | data | REST + event | HTML / PDF / DOCX / OCR / Korean morphology | preview | `oya-search-parser-app` |
| `search.index.inverted.upsert` | data | REST + event | engine-enforced isolation per ADR-0006; per-tenant + cross-tenant per consent | preview | `oya-search-index-inverted-app` |
| `search.index.vector.upsert` | data | REST | pgvector day-1 per ADR-0050/0177; per-tenant | preview | `oya-search-index-vector-app` |
| `search.query.execute` | data | REST | per-tenant + per-pack consent-tier filter | preview | `oya-search-query-api` |
| `search.serp.render` | data | REST | per-locale + per-region | preview | `oya-search-serp-app` |
| `search.rag.retrieve` (Foundry-internal) | data | REST | per-tenant boundary; per-class allowlist | stable | `oya-search-rag-api` |
| `search.dsr.cascade.execute` | data | REST | per-tenant + per-record cascade-purge with proof-of-erasure | stable | `oya-search-dsr-app` |

## 9. Axis 7 — Ads + Analytics surfaces

| id | plane | kind | invariant | tier | crate |
|---|---|---|---|---|---|
| `ads.campaign.publish` | control | REST | per-advertiser; per-tenant inventory | preview | `oya-ads-campaign-api` |
| `ads.audience.publish` | control | REST | DECLARED_PREFERENCE only; per-class allowlist | preview | `oya-ads-audience-api` |
| `ads.auction.bid` | data | REST | sub-100ms; per-cell-routed; per-policy gate | preview | `oya-ads-auction-api` |
| `ads.serve` | data | REST | per-policy gate; brand-safety + IVT filter | preview | `oya-ads-serve-app` |
| `ads.impression.record` | data | event | per-impression audit-emit | stable | `oya-ads-impression-app` |
| `ads.click.record` | data | event | per-click audit-emit + DSR cascade | stable | `oya-ads-click-app` |
| `ads.conversion.record` | data | event | server-API + privacy-preserving aggregation | stable | `oya-ads-conversion-app` |
| `ads.attribution.batch` | analytics | batch | per-conversion cross-touch attribution; per-DP-budget | preview | `oya-ads-attribution-app` |
| `ads.advertiser-console.report` | analytics | REST | per-advertiser; per-cohort | preview | `oya-ads-console-api` |
| `analytics.event.ingest` | data | event | consent-tier-bound; data-class annotated | preview | `oya-analytics-event-app` |
| `analytics.dashboard.query` | analytics | REST | per-tenant + per-purpose; DP-aggregated | preview | `oya-analytics-warehouse-api` |
| `analytics.dp-budget.consume` | analytics | internal | per-tenant per-class ε-budget enforcement | stable | `oya-analytics-dp-app` |
| `analytics.dsr.cascade.execute` | data | REST | per-tenant + per-record purge | stable | `oya-analytics-dsr-app` |
