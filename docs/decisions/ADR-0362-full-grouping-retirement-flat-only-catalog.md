...
related_specs: [/specs/per-microservice-flat-layout.json, /specs/microservices/connect-suite.json, /specs/microservices/enterprise-suite.json]
session_context:
  authored: 2026-05-25
...
ADR-0132 established a no-suite **forward-policy** but (a) explicitly **grandfathered** existing suite wrappers ("Connect Suite … Enterprise Suite … Foundry … remain as authored until their owning teams refactor them"), (b) explicitly **rejected** retiring grouping as a brand-layer concept, and (c) **specified** a `no-new-suite-bundles` BLOCKER lane that was **never implemented** (no check crate; not a registered lane — an aspirational-enforcement gap).
...
The previously-aspirational `no-new-suite-bundles` lane is **implemented for real** as the `no-grouping` gate (`oya-check-no-grouping`, registered in `oya gate run-all`). It fails on:
...
- **Full dissolution now** (author all 11 flat PRDs this pass) — rejected: would duplicate the in-flight ADR-0238/0237 Connect strangler, usurp `enterprise-product-council`'s authority over the Enterprise split, and manufacture paper µservices (honest-claims violation).
...
- **Leave ADR-0132's grandfather in place** — rejected; it left two live suite wrappers binding shared concerns indefinitely.
...
- ADR-0132: No-suite forward-policy (amended by this ADR — grandfather + brand carve-out removed; aspirational gate now real).
- ADR-0131: Per-microservice flat layout.
- ADR-0238 / ADR-0237: Connect dissolution + strangler migration (owns the real decomposition).
