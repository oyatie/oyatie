...
session_context:
  ...
  parallel_session_caveat: |
    Authored in oyatie 2026-05-17 to fill the parallel-session ADR-0126 slot that
    ADR-0132 cross-references but does not re-author. Verify inheritance from
    Bominal post-merge; if a Bominal-side ADR-0126 exists with a different
    decision body, the oyatie-side decision (this ADR) takes precedence per
    feedback_bominal_inheritance_precedence ("oyatie session decisions override
    Bominal").
bominal_source: |
  Authored in oyatie 2026-05-17; verify inheritance from Bominal post-merge.
  Bominal ADR-0126 is a different decision (Employment classification — 8-class
  enum) per docs/decisions/ADR-0060 §inheritance table; the Bominal number is
  inherited only into the HR/payroll product line, NOT into Connect. This ADR
  reuses the 0126 number inside the Connect dissolution lane because parallel
  ADR-0132 already claims it as the cross-reference target. The two 0126 IDs
  coexist by namespace (HR/payroll vs. Connect-dissolution) and never collide
...
`/specs/products/connect/*.json` reference pointers) **retires** when **all 8**
...
- **Pros**:
  - One terminal-state ADR; no follow-up dissolution churn.
...
- **5 net-new µservices to stand up.** `community` (PRD authored,
  pack-fill TBD), `social` (TBD), `shorts` (TBD), `network` (TBD), `anonymous`
  (TBD). Each needs PRD + threat-model + dpia + compliance + capacity-model
  + cost-budget + IaC + SLO + 15-IP phase plan + HG gate.
...
- `/specs/per-microservice-flat-layout.json`.
- `/specs/products/connect/{mail,messenger,calendar}.json` (legacy reference pointers retained for migration window).
