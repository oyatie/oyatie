...
- **Prevention**: M02-P22 phase-spec explicitly lists `cargo run -p oya-check-doc-coverage -- --workspace` (no `--report-only`) in BLOCKER command list (per `.omc/plans/milestones/M02-substrate/phases/P22-m02-exit-gate/phase-spec.md`). M02-P22 cannot pass its exit gate without doc-coverage going green or being explicitly waived per ADR.
