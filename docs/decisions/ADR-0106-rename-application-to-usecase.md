...
- **Documentation rewrite**: `docs/standards/clean-architecture.md` + `ADR-0056` §Layer semantics + ADR-0105 should be updated to use `usecase`. This commit edits Cargo.toml comments inline; the canonical doc updates are tracked as follow-up so the rename ships atomically.
