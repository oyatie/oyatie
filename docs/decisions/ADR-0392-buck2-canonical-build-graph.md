...
3. **`toolchains//:rust` is pinned to the workspace Rust + linker parity with `.cargo/config.toml`.** The Buck2 Rust toolchain target pins the same Rust channel as `rust-toolchain.toml` (currently 1.95.0) and reproduces the workspace's existing `.cargo/config.toml` rustflags / linker selection (mold + clang where configured) so Buck2 and `cargo` produce link-parity artifacts. Cargo stays usable for local dev; Buck2 is the canonical graph for hermetic/cache/affected-target builds.
