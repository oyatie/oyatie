...
Concrete tech (Tauri 2.x desktop / mobile, Dioxus, Compose Multiplatform, SwiftUI / Jetpack Compose native, KMM) is **deferred to the per-product PRD** that authors the native shell. The decision is a per-product trade-off (binary size, native-feature parity, team skill, store policy), not a cross-axis architecture decision. Each per-product PRD must:
