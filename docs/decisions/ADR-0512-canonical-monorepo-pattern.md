...
3. **Granularity: crate = bounded context.** A single-concern service is ONE crate (`microservices/<ms>/` with `src/`, ADR-0509's form). A multi-bounded-context service is one crate per bounded context under `microservices/<ms>/crates/` (e.g. `itsm` = on-call-schedule / escalation-policy / incident-room / status-update / postmortem, plus an optional umbrella). **No layer-per-crate** (ADR-0357's over-engineering: clean-arch layers are not crates) and **no whole-service mega-crate** (a coarse Buck2 target whose any-edit rebuilds everything).
