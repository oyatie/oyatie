...
Implementation note for Phase 2: the `oya-search-server` Phase-2 build would consume Tantivy as a library crate. Tantivy is what Quickwit (a CNCF Sandbox log-search engine) is built on; it's the established Rust open-standard. Building on Tantivy is not "writing a search engine from scratch"; it's "wrapping a KEEP-classified library in an oyatie-native server shell" — exactly the AWS/Google pattern.
