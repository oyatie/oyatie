...
session_context:
  authored: 2026-05-29
  basis: "Founder decision 2026-05-29: the CI/CD destination stack = Buck2 (build/RBE, ADR-0392/0408 in-flight) + Argo Workflows (k8s-native CI orchestration, REPLACES transitory Jenkins) + ArgoCD/Argo Rollouts (CD). ADR-0359 (Jenkins-sole-CI) is only status=Proposed (never Accepted) — supersede it. The oya gate engine stays the bespoke governance overlay Argo Workflows invokes; Forgejo Commit Status remains the gate-result sink until the SCM cutover (ADR-0510). This composes with the Buck2 reversal PR — it does not re-decide it."
purpose: Name Argo Workflows as the destination CI orchestrator (CNCF, k8s-native, self-hostable — passes the hyperscaler-lens), supersede the Proposed Jenkins-sole-CI ADR-0359, and frame Jenkins as the transitory bootstrap orchestrator. Keep the oya gate engine as the bespoke governance overlay and Forgejo Commit Status as the gate-result sink. Compose around the in-flight Buck2 reversal (ADR-0392/0408) without re-deciding it.
---
...
ADR-0349 made Jenkins + ArgoCD the self-hostable CI/CD substrate. ADR-0359 (Proposed) then pushed Jenkins to be the *sole* CI orchestrator, removing GitHub Actions after PR #180's entire 37-job matrix was blocked by a GitHub Actions budget cap — a metered third-party single point of failure. ADR-0361 specced the Jenkins-native execution; ADR-0387/0374 built the Forgejo→Jenkins→commit-status webhook bridge. ADR-0363 confirmed Jenkins as the external-CI that posts Forgejo Commit Statuses.

Jenkins **works today** and is genuine self-hostable substrate — but it is not the cloud/k8s-native destination:

- **Not k8s-native by design.** Jenkins is a controller+agent model bolted onto Kubernetes via the k8s plugin; pipelines are Groovy (`oyaCiLane.groovy`) on a stateful controller. The platform substrate is Talos/k8s + ArgoCD GitOps (ADR-0370/0375/0378); a CI orchestrator that is itself a k8s-native, declarative, GitOps-managed CRD workload is the architecturally-coherent destination.
- **Agentic-development-optimized feedback.** The N-lane parallel agent swarm (ADR-0391, parallel-swarm model) needs fast affected-gating feedback with per-lane isolation and CAS-shared results. A DAG-native engine that models each gate as a container step with explicit inputs/artifacts fits the affected-target + verdict-cache model (ADR-0366) more directly than Groovy stages.
- **Pipeline-optimized.** The destination pipeline is Buck2 RBE + remote cache + speculative merge-train (ADR-0111/0369). Argo Workflows composes cleanly with Buck2 container steps, Argo Events (Forgejo webhook → workflow trigger), and ArgoCD/Argo Rollouts CD — one CNCF-aligned, self-hostable family.
...
### 1. Destination CI orchestrator = Argo Workflows; Jenkins = transitory bootstrap

The CI/CD destination stack is:

- **Build / RBE:** Buck2 + remote build execution (ADR-0392/0408, in-flight — *not re-decided here*).
- **CI orchestration:** **Argo Workflows** (k8s-native DAG engine) + **Argo Events** (Forgejo webhook → workflow trigger), **replacing** Jenkins.
- **CD:** **ArgoCD** (GitOps) + **Argo Rollouts** (progressive delivery, ADR-0366 D5/D6).

**Jenkins is the transitory bootstrap orchestrator** — it stays the working CI surface (ADR-0361/0387) until the Argo Workflows lanes reach parity, then is retired. This mirrors the SCM transitory pattern (ADR-0510: Forgejo transitory → bespoke VCS): keep what works, name the destination, migrate when the replacement is green.

### 2. ADR-0359 is superseded; its anti-GitHub-Actions verdict is retained

ADR-0359's removal of GitHub Actions as a metered third-party SPOF **stands** — the destination remains fully self-hosted, covering air-gap/on-prem/colo/oyatie-as-cloud contexts (ADR-0215/0164). What changes: the self-hosted orchestrator destination is **Argo Workflows on our own k8s**, not Jenkins-sole. No regression to GitHub Actions is implied or permitted.

### 3. The oya gate engine stays the bespoke governance overlay Argo Workflows invokes

Argo Workflows orchestrates; it does **not** absorb governance. Each workflow step invokes the bespoke `oya gate` checks (the ~20 oyatie-specific governance gates with no off-the-shelf equivalent, per ADR-0363 §4) and standard tools (cargo/nextest, cargo-deny, Trivy/cosign/Syft) as container steps. The orchestrator is OSS-adopted; the **differentiator (governance-as-code / AI-slop-defense) stays bespoke** — exactly the bespoke-over-OSS split ADR-0363 drew for Jenkins, now carried to Argo Workflows.

### 4. Forgejo Commit Status remains the gate-result sink (until the SCM cutover)

Argo Workflows posts per-context and the single trustless-gateway signed status (ADR-0367) to **Forgejo's Commit-Status API**, which gates merges via required status checks (ADR-0363). This is unchanged from the Jenkins path — only the *poster* changes (Argo Workflows replaces Jenkins as the status producer). At the SCM cutover (ADR-0510 §3) the sink moves to the bespoke VCS status surface; the trust model (ADR-0367) is host-independent.
...
- **Keep Jenkins as the sole/destination orchestrator (ratify ADR-0359 as-is)** — rejected: not k8s-native (stateful Groovy controller bolted onto k8s), weaker fit for the affected-gating + CAS-sharing + Buck2-RBE destination pipeline and the GitOps/Talos substrate. Jenkins is kept as the transitory bootstrap, not ratified as destination.
...
- Removes the Jenkins-as-destination lock-in while retaining ADR-0359's correct anti-GitHub-Actions SPOF removal.
...
### Negative / risk
- A migration period runs two orchestrators (Jenkins transitory + Argo Workflows building parity); mitigated by the local `oya verify` mirror as the gate of record during transition (ADR-0346) and by retiring Jenkins lanes only when the Argo equivalent is green.
...
## References
- ADR-0359 — Jenkins replaces GitHub Actions (Proposed; **superseded here**; anti-GHA-SPOF verdict retained).
...
- ADR-0349 / 0361 — Jenkins + ArgoCD substrate; Jenkins-native execution (transitory bootstrap).
- ADR-0387 / 0374 — Forgejo→Jenkins→commit-status webhook bridge (the poster Argo Workflows replaces).
- ADR-0363 — git + Jenkins + Forgejo; oya = governance-gate engine; Forgejo Commit-Status sink.
