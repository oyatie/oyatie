...
| W3 | Anthropic API phase-out begins when foundry-runtime achieves the readiness gate | axis-foundry | Signal: foundry-runtime parity demonstrated per `registry/vendor-lockin-phaseout/index.json`, then first release train after the quarterly phase-out review |
