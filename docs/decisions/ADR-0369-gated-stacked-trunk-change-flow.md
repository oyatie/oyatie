...
deliverables:
  ...
  - id: ADR-0369-D2
    description: "Stacked-diff authoring on plain git: agents author small (<~200 LOC) dependent units as a ghstack-style chain of Forgejo PRs (base = parent's branch). NOT jj."
  ...
  - id: ADR-0369-D3
    description: "The trustless-gateway as a single required status check (the binding gate, NOT Forgejo's merge button — it has merge-despite-failed-checks races): green only when trusted-runner-signed evidence (ADR-0367) AND independent reviewer attestation both verify against the PR head SHA."
...
purpose: Choose the change-flow MECHANISM that maximizes throughput for a concurrent agent fleet under the ADR-0367 trustless model — "gated stacked-trunk with a speculative train" on plain git + Forgejo, evaluated against all options (PR, change-centric/Gerrit, stacked, jj, trunk+flags, pre-receive, merge-train).
---
...
gate, speculative merge-train). Forgejo ground-truth (2026): no native merge-queue (forgejo#5102 open),
required-status-checks + Commit-Status-API + auto-merge + CODEOWNERS present, pre-receive hooks exist
but run as the Forgejo OS user (RCE-grade), and known merge-despite-failed-checks races (#11224/#8189)
mean **the binding gate must be the required check itself, not the merge button.**

## Decision
**Gated stacked-trunk with a speculative train**, on plain git (ADR-0363) + Forgejo PRs as the cheap
...
3. **The trustless gateway — one required check (now, D3).** A required `trustless-gateway` context is
   ...
   verification *is* the check — robust to Forgejo's merge-button races.
...
- **Gerrit (the server)** — rejected: wrong substrate (contradicts ADR-0363 Forgejo); borrow its
...
- **Pre-receive hook as the primary path** — rejected as primary: loses the PR audit object and runs
  RCE-grade code as the Forgejo OS user; keep only as an optional defense-in-depth backstop.
...
- Positive: max throughput (small units + speculation + auto-merge, no human bottleneck) with conflicts
  prevented (sharding) and resolved (train); rides plain git + Forgejo (no new VCS).
- Negative/cost: stacked-PR tooling on Forgejo (ghstack-style) + the merge-train bot are build/adopt
...
Forgejo #5102/#11224/#8189, agentjj jj→git reversion, GitHub stacked-PRs-for-agents (2026), bors/TAP.
