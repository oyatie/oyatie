...
supersedes: []
superseded_by: []
related: [ADR-0056, ADR-0105, ADR-0110, ADR-0115, ADR-0119, ADR-0122, ADR-0126, ADR-0127, ADR-0130, ADR-0131]
related_specs: [/specs/per-microservice-flat-layout.json]
session_context:
  authored: 2026-05-17
  parallel_session_caveat: "Parallel session merges before this ADR. ADR numbers 0125–0129 are claimed by parallel work (OP-11 audit, Connect super-app, hyperscaler patterns, hyperscaler invariants, plan-schema). This ADR was renumbered 0126→0132 on 2026-05-17 to clear the collision. Connect-specific µservice topology is delegated to parallel ADR-0126; this ADR carries only the universal forward-policy."
...
Connect's specific dissolution into seven flat sub-products (mail / messenger / calendar / social / shorts / network / anonymous) is owned by parallel ADR-0126 (`connect-full-social-network-super-app`); this ADR cross-references that decision but does not re-author it.
...
- Authoring a new `/specs/products/<bundle>/suite.json` collection-spec wrapper that binds multiple flat µservices.
...
- **Dissolve all existing suites in this ADR** — rejected per user directive 2026-05-17 ("we won't worry about suites yet"); out of scope. Refactor of existing suite µservices is deferred to per-team decisions.
...
- **New CI lane: `oya-governance-no-new-suite-bundles`** (BLOCKER on `dev`). Refuses:
  ...
  - New `/specs/products/<name>/suite.json` files.
...
- Existing suite files (`/specs/products/connect/suite.json`, `/specs/products/enterprise/suite.json`, ADR-0029, etc.) are explicitly exempted by allowlist in the lane until their owning team refactors them.
...
- ADR-0126 (parallel session 2026-05-17): Connect-specific dissolution — full social-network super-app expansion; this ADR cross-references but does not re-author.
