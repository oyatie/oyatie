...
The GH #983 metadata packet implementation is the Rust binary `libs/oya-check-pr-traceability/src/bin/pr-traceability-admission.rs`, owned through `libs/oya-check-pr-traceability/OWNERS`; it is a PR title/body hygiene preflight and not the trusted review-producer closure for F-PR5-06.
