...
The run-observability packet status contract is part of the same single-required-context substrate: `specs/cloud-ci-run-observability-packet.schema.json` defines the machine-readable packet, `cloud/cloud-ci/gates/oya-cloud-ci-firewall-app/src/run_observability_packet.rs` implements the pure validator, `cloud/cloud-ci/gates/oya-cloud-ci-firewall-app/tests/run_observability_packet.rs` locks the regression contract, and `specs/fixtures/cloud-ci-run-observability/tc-1003-bad-actions-log-scrape-only.json`, `specs/fixtures/cloud-ci-run-observability/tc-1003-bad-context-mismatch.json`, `specs/fixtures/cloud-ci-run-observability/tc-1003-bad-failed-without-diagnostics.json`, `specs/fixtures/cloud-ci-run-observability/tc-1003-bad-unstable-nested-ids.json`, `specs/fixtures/cloud-ci-run-observability/tc-1003-bad-unstable-packet-id.json`, `specs/fixtures/cloud-ci-run-observability/tc-1003-good-failed-gate-diagnosable.json`, and `specs/fixtures/cloud-ci-run-observability/tc-1003-good-passed-no-failure-diagnostics.json` are the born-red/born-green fixture corpus. The contract closes the false-green class where a failed `oya-ci-required` run is only diagnosable by scraping GitHub Actions logs instead of reading typed cloud-ci/oya-ci status artifacts.
