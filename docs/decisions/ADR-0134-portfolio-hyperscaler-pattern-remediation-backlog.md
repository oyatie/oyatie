...
| Structured failure summary (P0 + P1) | `oya-governance-structured-failure-summary` | Controller harvests buck2 event-log JSON; crier posts `{target, error_type, first_stderr}[]` summary to Forgejo; logs persisted to S3 with direct URL; replaces fragile `grep 'Action failed:'` and eliminates `kubectl exec` diagnosis. Proposed in ADR-0514. |
...
| oya-ci deck SolidJS UI (LATER / Phase 3) | `oya-governance-oya-ci-deck` | SolidJS-based CI-visibility surface for founder + agents; introspectable job status, logs, failure taxonomy. Deferred, proposed in ADR-0514 Phase 3. |
