...
1. **Sanctioned primitives are `grit` + `icm` + `oya-agent-read` only.** Any new agent-callable tool must justify itself against "why doesn't `grit` or `icm` already do this." Agent-side `git`/`gh` is banned.
...
3. **Inventory before deletion.** No file or script is deleted until it appears in the inventory ledger at `oyatie/docs/decisions/ADR-NNNN-grit-cutover-inventory.md` (or equivalent) with classification: `KEEP` / `REPLACE-WITH-GRIT` / `REPLACE-WITH-ICM` / `REPLACE-WITH-HELPER` / `ARCHIVE` / `DELETE`. The inventory pass covers `/Users/jasonlee/oyatie/**` (excluding `target/`, `node_modules/`, `.git/`) and the bominal surfaces the cutover touches (`/Users/jasonlee/bominal/agents/`, `/Users/jasonlee/bominal/docs/`).
...
1. **A1 — Bidirectional PRD citation.** `oyatie/docs/PRD.md` cites `bominal/docs/consolidated/PRD.md` as portfolio parent; bominal cites oyatie as canonical implementation home. *Test*: new fitness lane `oya-foundry-fitness-portfolio-citation` passes on both sides.
2. **A2 — Inventory ledger committed.** `oyatie/docs/decisions/ADR-NNNN-grit-cutover-inventory.md` (or equivalent) exists, classifies every file/dir/script under the inventory scope, and is referenced from `ADR-INDEX.md`. *Test*: ADR shape lane + every entry has a classification value from the closed set.
...
10. **The PR-merge flow under `grit done` produces something semantically equivalent to a GitHub PR.** If `grit done` only does local merge today, the helper layer extends to `oya-agent-write pr-finalize <session>` as a one-call atom that creates the PR record without exposing `gh` to agents.
...
3. agent reads via: grit symbols | grit status | grit watch | icm recall-context | oya-agent-read log/diff/pr-view
...
| **oya-agent-read** | NEW thin sanctioned helper. Read-only wrapper over `git`/`gh` for operations grit doesn't cover. Audit-emitting. | `tools/oya-agent-read/` (to be created). |
| **Sanctioned primitive** | A tool agents may invoke. The set is exactly `{grit, icm, oya-agent-read}`. Adding to the set requires an ADR. | `oyatie/docs/AGENTS.md` |
...
| **Inventory ledger** | The ADR + tracked table that classifies every file/dir/script in scope before any deletion happens. | `oyatie/docs/decisions/ADR-NNNN-grit-cutover-inventory.md` |
...
- Lane 3 (what is the direction shift): RESOLVED — agentic-pipeline mechanism, sanctioned primitives `{grit, icm, oya-agent-read}`, agent-side `git`/`gh` banned.
