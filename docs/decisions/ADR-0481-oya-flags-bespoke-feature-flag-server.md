...
supersedes: []
superseded_by: []
related: [ADR-0083, ADR-0476, ADR-0408, ADR-0509]
deliverables:
  - id: D1
    description: "New µservice microservices/oya-flags/ — Rust workspace, axum + Connect-RPC. Speaks OpenFeature flag-evaluation protocol (OpenFeature/flagd provider plan SDK compat retained). PostgreSQL backend for flag definitions; in-memory hot cache for evaluation hot-path."
  ...
  - id: D3
    description: "Flag-as-code: definitions in oya-vcs (oya-vcs roadmap plan) GitOps repo; Buck2 CI (ADR-0408) validates schema; flag changes flow through PR review. Audit trail = git history."
  ...
  - id: D4
    description: "OTel (OTel observability plan) integration: every evaluation emits span with flag_key + variation + tenant. Cloud-intelligence routing (ADR-0384) uses oya-flags for Cluster I/II/III selection."
  ...
  - id: D5
    description: "Tenant flag plane via Crossplane (Crossplane XR plan) TenantApplication XR. Tenants set per-tenant flags via Self-Service UI (Rust-native portal plan). oya-identity (ADR-0476) human auth + SPIFFE service auth."
...
Accepted — 2026-05-28 (founder-locked). Amends OpenFeature/flagd provider plan (server provider only; OpenFeature SDK
protocol unchanged).

## Context

OpenFeature/flagd provider plan adopted the OpenFeature SDK + flagd DaemonSet as the feature flag substrate. The OpenFeature
...
speaking the OpenFeature flag-evaluation protocol. OpenFeature/flagd provider plan SDK adoption is preserved; only the
server provider changes.

- **Server**: `microservices/oya-flags/` — Rust, axum, Connect-RPC. Implements the OpenFeature
  flag-evaluation gRPC protocol so existing SDK wiring (OpenFeature/flagd provider plan Phase 2) continues unchanged.
- **Storage**: PostgreSQL (PostgreSQL storage plan) for flag definitions. In-memory hot cache for evaluation
...
- **Flag-as-code**: flag bundles in oya-vcs (oya-vcs roadmap plan) GitOps repo. Buck2 CI (ADR-0408) runs
  schema validation as a BLOCKER gate. Audit trail = git history.
- **Observability**: every evaluation emits OTel (OTel observability plan) span with `feature_flag.key`,
...
- **Tenant plane**: Crossplane (Crossplane XR plan) `TenantApplication` XR provisions a per-tenant flag
  namespace. Tenants toggle flags via Self-Service UI (Rust-native portal plan).
...
|---|---|
| flagd (OpenFeature/flagd provider plan Phase-1) | Go binary; no Cedar integration; flagd DSL limits targeting expressiveness; external roadmap dependency. |
...
| OTel (OTel observability plan) | `feature_flag.key`/`value`/`tenant_id` span attributes on every evaluation. |
...
| Crossplane (Crossplane XR plan) | `TenantApplication` XR provisions per-tenant flag namespace. |
| oya-identity (ADR-0476) | Human + SPIFFE auth for flag API. |
| Self-Service UI (Rust-native portal plan) | Tenant flag panel reads/writes via oya-flags API. |
