...
3. **Document `oya-tooling-agent-read` audit-chain topic** in `docs/security-program.json`. Each read invocation should emit `EVT-AGENT-READ-<verb>` with agent id, args, timestamp.
