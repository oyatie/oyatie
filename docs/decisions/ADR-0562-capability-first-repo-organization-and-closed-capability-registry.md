...
The entries are therefore left pointing at `cloud/cloud-os/`, where they are now inert, and the 41 relocated crates are **scanned** by the policy at their new home — verified green (45 tests pass). The exemption turned out to be unnecessary: nothing in the node-OS tree trips the interpreter-command or CLI-package dimensions. Retiring the two dead entries is a shrink, which the gate permits, and belongs in its own change where the shrink is the subject rather than a side effect.
