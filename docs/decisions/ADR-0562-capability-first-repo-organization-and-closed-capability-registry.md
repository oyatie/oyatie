...
#### §10.28 De-brand strangler MOVE-2: `messaging` capability EVENT-BUS boundary (libs/oya-bus-boundary-kernel → messaging/core/bus-boundary) — single-crate follow-on to §10.27 that homes the first of the three boundary kernels left in the junk-drawer

Continuing §10.27 (which homed the substrate FLOOR at `messaging/core/substrate-kernel`), this move homes the messaging capability's EVENT-BUS boundary surface out of the `libs/` junk-drawer. The crate is a pure-types + traits domain surface (`EventBus<'a, S: MessagingSubstrate>`, `SubscriberGroup`, `Event`, `ChannelName`) generic over the ONE substrate port — no I/O, no adapters — so it lands at FACE `core` per ADR-0570, mirroring its siblings `messaging/core/substrate-kernel` (§10.27) and `messaging/core/domain` (§10.5). The de-brand drops the `oya-` vendor prefix to the capability slug per ADR-0532/0533 (cargo name = de-branded path tail): `oya-bus-boundary-kernel` → `messaging-bus-boundary`.

**Orphan-leaf move (ZERO reverse-deps; nothing above needs rewiring):** unlike §10.27's substrate-kernel (a sink whose three consumers had to be rewired), this crate is an ORPHAN LEAF — nothing in the repo depends on it, so NO consumer manifests/BUCKs/sources change. Its ONLY forward dep is `messaging-substrate-kernel` (homed by §10.27 at `messaging/core/substrate-kernel`); the codemod recomputes that cargo path-dep from `../../messaging/core/substrate-kernel` to the sibling `../substrate-kernel`, while the BUCK label `//messaging/core/substrate-kernel:messaging-substrate-kernel` is workspace-absolute and unchanged. The resulting intra-`messaging/core/` edge (`bus-boundary` → `substrate-kernel`) is ACYCLIC and introduces no layer inversion.

The move was performed by `oya-reorg-codemod-app` (NOT by hand), gated on the buck2-full-tree dry-run (`cargo metadata` + `buck2 targets //...` both resolved post-move on a shadow tree, `buck_ok=true` not null, `clean=true`). The capability registry RETIRES the now-stale `libs/oya-bus-boundary-kernel` membership glob from the closed `messaging` membership entry (the crate is now self-owned by the `messaging/` capability dir via `messaging.absorbs_current_dirs`, exactly as `messaging/core/substrate-kernel` and `messaging/core/domain` are); the two remaining boundary-kernel globs (`libs/oya-queue-boundary-kernel`, `libs/oya-stream-boundary-kernel`) stay for their own later moves. No membership/acyclicity POLICY data changes (the `messaging` root, `messaging/*/*` crate_root_glob, and root workspace `messaging/*/*` member glob were all seeded by §10.5, so `root_workspace_changed=false` — the glob already covers `messaging/core/bus-boundary`). The move's tracked, born-accounted artifact paths are `messaging/core/bus-boundary/Cargo.toml`, `messaging/core/bus-boundary/BUCK`, and `messaging/core/bus-boundary/src/lib.rs` (the module-doc `//! # …` header + the `# Naming justification` block are de-branded in-place to the capability-path grammar, folded into the move commit — the codemod rewrites cargo/buck idents but not free-text doc comments); the newly-authored catalog record `registry/catalog/messaging-bus-boundary.yaml` (`role: kernel`, mirroring `registry/catalog/messaging-substrate-kernel.yaml`; reached by the existing `registry/catalog/` reachability prefix — the crate becomes catalog-governed under `messaging/`, so `ci/facade/service-catalog-parity` requires the row); and the committed move-plan `specs/reorg/messaging-bus-boundary-move-plan.json` (reached by the existing ADR-0563 `specs/reorg/` reachability prefix). Like §10.27, this PR removes NO prior spent plan: the three committed plans that remain on `dev` — `specs/reorg/ci-move-plan.json` (#1216), `specs/reorg/iam-pdp-cedar-move-plan.json` (#1184), and `specs/reorg/messaging-substrate-kernel-move-plan.json` (§10.27 / #1225) — are all LOAD-BEARING and/or already-landed and are excluded by the codemod's `plan_is_landed` merge-base carve-out (every old crate-dir of all three is absent at the merge-base `dev`), so exactly ONE active move-plan (this one, whose `libs/oya-bus-boundary-kernel` old dir is still present at the merge-base) remains and the single-active-plan invariant holds.
