...
| 3 | storage drive/recordings | `oya/drive`, `oya/recordings` → **`app/` consumer products** (compose storage+iam+comms+billing). `storage/facade` = the S3-analog CAS we sell as infra. | §3#5/§6; storage `boundary_note` |
...
| 10 | marketplace dev-cli | `dev-cli` (generic gate binary, 122 src, zero marketplace domain logic) → **`ci/`** and is **de-CLI'd** (CLI-retirement: it becomes a gate app/API, not a CLI surface). | §3 rule #3 (CI engines → ci/build); ci charter |
...
**Net effect:** thirteen of fourteen require zero or scaffold-only relocation; the only real crate
move is dev-cli → `ci` (Q10). The five app-bound surfaces (emergency, imaging, diagnostics, drive,
recordings) hold **zero crates today** — reclassifying them from a substrate `absorbs_current_dirs`
entry to `app/` is a phantom-absorb correction that *reduces* registry drift, not a code move.
