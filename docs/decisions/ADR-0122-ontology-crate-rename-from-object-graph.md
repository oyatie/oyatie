...
The crate `oya-platform-object-graph-kernel` was scaffolded before that rename. It is now drift: a stale token in canonical naming surface that contradicts the doctrine + `/specs/products/ontology.json` (PRD authored 2026-05-16, this same PR sequence).

The multispectrum-review wave that landed as PR #20 (2026-05-16) also surfaced this drift via the A1 naming-adherence facet, but the fix was filed as follow-up. Per `/specs/agent-durable-goal.json#operating_principles/OP-11` (no_stubs_no_defer_100_production_quality, accepted 2026-05-16), the rename is now executed in-PR rather than deferred.
...
Plus the planned-but-not-yet-scaffolded crates per `/specs/products/ontology.json#identity.crate_refs_planned`:
...
- **Keep "object-graph" naming**: rejected — contradicts the ratified glossary rename + Bominal-ADR-0106 + `/specs/products/ontology.json`. Would force every consumer to maintain two mental models.
...
- **Defer the rename to a follow-up PR**: rejected per `/specs/agent-durable-goal.json#operating_principles/OP-11` — drift between code and doctrine is in-scope and must be fixed in-PR.

## Consequences

### Positive
- Crate names match canonical doctrine + Palantir-aligned Ontology terminology.
- `/specs/products/ontology.json` becomes self-consistent (no "but the crate is still called object-graph" caveat).
...
2. Re-amend `/specs/products/ontology.json` to use the legacy token.
...
- `/specs/products/ontology.json#identity.crate_refs_current` updated to reference the renamed crate.
