...
We adopt a single **append-only, hash-chained audit-event log** as the tamper-evident record-keeping surface for every regulated event in every axis. The kernel is `crates/oya-platform-audit-chain-kernel`; the application layer is `crates/oya-platform-audit-chain-app`; per-tenant shards live behind `crates/oya-platform-audit-chain-adapter-postgres-*` with optional cold-tier mirror.

### Chain structure

```rust
// crates/oya-platform-audit-chain-kernel
...
Every capability declared under `product-control/capabilities/<id>.yaml` carries an explicit `evidence_emission_topic` field. The Foundry runtime (ADR-0007) refuses to invoke a capability whose declared emissions are not satisfied by the chain at run time. The contract is enforced at three points:
...
3. **Daily integrity check** — `oya-platform-audit-chain-app` walks each tenant shard, verifies hash continuity, recomputes per-day root, compares to the trust-portal anchor, and emits `EVT-AUDIT-INTEGRITY-OK` or pages a P1 bridge on mismatch.
...
- Applies to: every regulated capability invocation, every cross-axis data flow, every consent decision, every break-glass invocation, every regulatory-pack adoption, every model training-data ingestion under `model_training_oya` purpose.
- Does not apply to: ephemeral debug logs, per-axis local telemetry that touches no regulated data, build-time stdout (which lives in CI logs).
...
- CI: `oya-foundry-fitness-evidence-contract` runs on every PR touching `product-control/capabilities/` or `crates/oya-*-app/` with a capability dispatch.
...
- **Pros:** lower per-event hot-path cost; per-axis ownership is clearer.
- **Cons:** rollup window is a regulator-visible gap; cross-axis correlation requires log-stitch joins; cohesion claim fails at the substrate level.
...
2. **Q2.** Cold-tier mirror — Iceberg/Parquet on object store, or a dedicated cold-chain store? Default: Iceberg + per-tenant key-shred. → owner: `axis-cloud`.
...
4. **Q4.** Cross-tenant root anchor publication cadence — minute? 5-minute? Trust portal UX implication. Default: 1 min. → owner: `axis-foundry` (trust portal).

---

## References

- `docs/DESIGN.md` §7 (audit chain), §11 (cross-axis contradiction audit)
