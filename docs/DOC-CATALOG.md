...
| `doc.spec_founder_product_intent` | `/specs/founder-product-intent.json` | `founder` | direct founder-intent change or council-architecture admission review; qualified product authority remains unresolved | per event | root-hub-pointers.json, masterplan.json, planning-closure-contract.json | `authority-cohesion`, `spec-contract-mirror` | NO |
