...
| `doc.spec_agent_durable_goal` | `/specs/agent-durable-goal.json` | `council-architecture + axis-foundry` | autonomous-Foundry contract change (review tiering, score cards, source/doubt-driven, autonomy ceiling) | quarterly | AGENTS.md, DESIGN.md, DOC-CATALOG.md, multispectrum-review.json | `authority-cohesion`, `spec-contract-mirror` | NO |
...
| `doc.spec_multispectrum_review` | `/specs/multispectrum-review.json` | `council-architecture` | review-doctrine change (facets, executor topology, consensus protocol) | per event | agent-durable-goal.json, AGENTS.md | `spec-contract-mirror`, `authority-cohesion` | NO |
...
| `doc.audit_kg_robustness` | `/registry/kg-audit/index.json` | `council-architecture + ontology-team` | KG audit re-run | quarterly | knowledge-graph specs, ontology PRD | `spec-contract-mirror` | NO |
| `doc.audit_prow_k8s_native_hygiene_2026_06_04` | `audits/prow-k8s-native-hygiene-audit-2026-06-04.md` | `platform-scm-ci-cd + council-architecture` | native SCM/CI/CD lane-unlocker direction or retired CLI authority changes | per event | specs/repo-hygiene-automation.json, specs/oya-ci-prow-capability-parity.json, ADR-0513, ADR-0516 | `repo-hygiene-automation-check` | YES — agents may update with fresh verification evidence |
...
| `doc.agent_security_model` | `security.md` | `axis-foundry` + `ops-security` | agent sandbox or permission contract change | quarterly | AGENT-INSTRUCTION-SOURCES.md, SECURITY-PROGRAM.md, TOOLCHAIN.md | `doc-catalog-self-coverage`, `readme-doc-coverage` | NO |
| `doc.buck2_native_ffi_investigation` | `buck2-native-ffi-investigation.md` | `axis-foundry` | Buck2 native-FFI investigation or cutover evidence changes | per Buck2 cutover change + quarterly | TOOLCHAIN.md, RELEASE-MANAGEMENT.md | `doc-catalog-self-coverage`, `readme-doc-coverage` | YES |
