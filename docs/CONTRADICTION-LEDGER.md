...
| `LEDG-014` | `gap-docs-project.md` J-006 | Missing PIPA / DPIA binders — cannot evidence Korea launch readiness | Compliance + per-vertical | Author KR-pack regulatory binder per [COMPLIANCE-MATRIX §3.1](COMPLIANCE-MATRIX.md); per-vertical DPIA template | OPEN | `docs/regional-packs/oya-pack-kr` + `council-privacy` | W-Vertical-Pilot |
...
| `LEDG-024` | Codex §10 | Korea posture omits 본인확인서비스 (identity verification), MyData / Open Banking, location law, telecom secrecy | Vertical-fintech + Cross-cutting + KR-pack | `DESIGN.md §12` expansion + KR-pack regulatory deepening | OPEN | `docs/regional-packs/oya-pack-kr` + `vertical-fintech` |
