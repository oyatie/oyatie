# oya-ci firewall / ratchet mental model
...
The gate-baseline `_provenance` also carries a `config_digest`, so a change to `oya-ci.toml` is
visible in the diff.

## Why a fan-in job, not a reusable workflow

The required-context name is `oya-ci-required` — a single fan-in job that `needs:` every gate lane.
