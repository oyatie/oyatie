# oya-ci — the config-driven conformance FLOOR

oya-ci is a portable, config-driven conformance engine: a deterministic **producer** builds a set
...
- **Config kernel** — `libs/oya-ci-config` (crate `oya-ci-config-kernel`): the closed-schema,
  I/O-free TOML loader + the bundled default.
- **Producer** — `cloud/cloud-ci/gates/oya-cloud-ci-accounting-registry-app`: the `rust_binary`
  that reads `oya-ci.toml`, runs git plumbing, and emits the generated faces (including
  `gate-baseline.generated.json`). NOT a CLI subcommand.
- **Firewall** — `cloud/cloud-ci/gates/oya-cloud-ci-firewall-app`: the shrink-only ratchet runner.
- **registry-drift** — `cloud/cloud-ci/gates/registry-drift`: the committed==regenerated byte-diff.
- **Gates** — the per-gate `oya-cloud-ci-*-app` crates (each a pure `evaluate_keyed`) + the
  raw-corpus census in `libs/oya-check-brand-residue`.
