...
|---|---|---|---|---|
| Rust toolchain | 1.96.0 | 2026-05-28 | https://blog.rust-lang.org/2026/05/28/Rust-1.96.0/ | latest stable channel; no formal LTS (six-week cadence). Contains Cargo fixes for CVE-2026-5223 and CVE-2026-5222. |
...
| Buck2 | 2026-06-01 | 2026-06-01 | https://github.com/facebook/buck2/releases/tag/2026-06-01 | Buck2 uses date-tagged releases; Oyatie pins the latest verified date tag in the GitHub bridge bootstrap while Buck2 remains build/test/check authority. |
...
| cargo-deny | 0.19.5 | 2026-05-09 | https://github.com/EmbarkStudios/cargo-deny/blob/main/CHANGELOG.md | MSRV 1.85.0, edition 2024. Adds SARIF output; compatible with workspace Rust 1.96.0. |
...
Sources read: `/Users/jasonlee/Developer/source/rust-toolchain.toml` (`channel = "1.96.0"`); `/Users/jasonlee/Developer/source/Cargo.toml` (workspace `rust-version = "1.96.0"`, edition 2024); `/Users/jasonlee/Developer/source/rustfmt.toml` (`edition = "2024"`, `style_edition = "2024"`); `/Users/jasonlee/Developer/source/.github/workflows/github-lane-unlocker-ci-cd.yml` (`FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`, `actions/checkout@v6`, `BUCK2_RELEASE: "2026-06-01"`); `/Users/jasonlee/Developer/source/scripts/ci/github-actions-lane-unlocker-bootstrap.sh` (`BUCK2_RELEASE:=2026-06-01`); `/Users/jasonlee/Developer/source/specs/github-lane-unlocker-bridge.json` (Node 24 action-runtime decision, Node 26 non-use rationale, Buck2 release pin); `/Users/jasonlee/Developer/source/deny.toml` (license allow-list: 0BSD, Apache-2.0, BSD-2/3, ISC, MIT, MPL-2.0, Unicode-3.0).

| Component | Oyatie pinned | Current LTS / stable | Behind? | Severity |
|---|---|---|---|---|
| Rust toolchain | 1.96.0 (`rust-toolchain.toml`; workspace `rust-version`) | 1.96.0 (stable) | No | OK — cargo-deny 0.19 MSRV 1.85 is below the workspace pin. |
...
| Buck2 | 2026-06-01 (`BUCK2_RELEASE`) | 2026-06-01 date tag | No | OK — latest verified upstream date tag as of 2026-06-04. |
...
| cargo-deny | invoked via `cargo deny check` (D11); version not pinned | 0.19.5 | n/a | MED — MSRV is compatible with Rust 1.96.0, but the tool version still needs a durable pin. |
...
1. **Rust drift is closed for this snapshot.** The workspace now pins `rust-toolchain.toml` `channel = "1.96.0"`, `rust-version = "1.96.0"`, `edition = "2024"`, and repo-root rustfmt `edition = "2024"` / `style_edition = "2024"`; cargo-deny 0.19.5 MSRV 1.85 is below the workspace pin.
...
As of this update, oyatie's Rust LTS drift is closed for the current 1.96.0 stable snapshot: the workspace pins `rust-toolchain.toml` `channel = "1.96.0"`, Cargo `rust-version = "1.96.0"`, all Cargo workspace members report edition 2024, and repo-root `rustfmt.toml` pins both `edition = "2024"` and `style_edition = "2024"`. cargo-deny 0.19.5 (current, 2026-05-09) requires Rust 1.85 MSRV, so it is compatible with the current workspace pin. The remaining supply-chain task is to pin the cargo-deny tool version durably.
