...
- **`cargo-nextest`** (Apache-2 / MIT) — fast, parallel test runner with sharding + flaky-quarantine integration. Canonical (per project memory + ADR-0024). NEVER use bare `cargo test`. Per [`oya dev check`](../TOOLCHAIN.md) bundle.
