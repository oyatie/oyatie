...
- [ ] **RR25** Every Foundry capability shipping in this wave has: record + eval-set ≥ min-pass-rate + autonomy tier + audit topic + Cosign. *Lane:* `oya-foundry-fitness-capability-publish`. Grit symbols scaffolded per ADR-0054.
...
- [ ] **RR31** `EVT-WAVE-GATE-PASSED` emitted with milestone ID + gate name + sign-off list. *Lane:* `oya-foundry-fitness-audit-emission`. Inventory entry per ADR-0052.
