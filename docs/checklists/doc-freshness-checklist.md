...
| `ImplementationPlan` | per-PR | Author-emitted; agent re-emits on `grit done` (ADR-0053) | `oya-governance-plan-hierarchy` |
...
| `InventoryLedger` | per cutover/migration event (ADR-0052) | Agent append; manual for retire/tombstone | `oya-governance-inventory-tracker` |
