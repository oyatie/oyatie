...
enforcing_fitness_lane: oya-foundry-fitness-doc-freshness
...
|---|---|---|---|
| `Constitution` | 1 quarter | Manual; council-architecture only | `oya-foundry-fitness-authority-cohesion` |
| `Operating-Contract` (AGENTS.md) | 1 quarter | Manual; axis-foundry + council-architecture | `oya-foundry-fitness-authority-cohesion` |
| `MasterPlan` | 1 quarter | Manual; council-architecture | `oya-foundry-fitness-plan-hierarchy` |
| `MilestoneIndex` | 2 weeks | Status auto-recomputed from phase INDEX rollup | `oya-foundry-fitness-plan-hierarchy` |
| `PhaseIndex` | 1 week | Status auto-recomputed from IP rollup | `oya-foundry-fitness-plan-hierarchy` |
| `ImplementationPlan` | per-PR | Author-emitted; agent re-emits on `grit done` (ADR-0053) | `oya-foundry-fitness-plan-hierarchy` |
...
| `MistakesLedger` | monthly + per-mistake-event | Manual append; quarterly council review | `oya-foundry-fitness-mistakes-ledger-cite` |
...
| `DesignDoc` | per-phase | Manual | `oya-foundry-fitness-design-doc-shape` (advisory) |
| `InventoryLedger` | per cutover/migration event (ADR-0052) | Agent append; manual for retire/tombstone | `oya-foundry-fitness-inventory-tracker` |

## Walk procedure

- [ ] **F1** For each row above, compute `now - last_updated_at`. If `> staleness budget`, the doc is **stale**.
- [ ] **F2** For each stale doc, the auto-update path emits a draft (or a `BLOCKED_ON_HUMAN_ORCHESTRATOR` event for council-only docs). *Lane:* `oya-foundry-fitness-doc-freshness`.
- [ ] **F3** No `<!-- forward-reference: wave-1 -->` markers point at artifacts that should have existed by `now`. *Lane:* `oya-foundry-fitness-forward-reference`.
- [ ] **F4** `docs/DOC-CATALOG.md` itself walked: every row's `update_trigger` events for the past period accounted for. *Lane:* `oya-foundry-fitness-doc-catalog`.
- [ ] **F5** `docs/CHANGELOG.md` rows present for every doc touched since last walk. *Lane:* `oya-foundry-fitness-changelog-row`.
- [ ] **F6** Orphans: every file under `docs/` has either an `update_trigger` row in `DOC-CATALOG.md` OR an explicit `excludes:` entry in upstream contract. *Lane:* `oya-foundry-fitness-orphan-detection`.

## Anti-patterns

- Bumping `last_updated_at` without a substantive change. *(forbidden — `oya-foundry-fitness-doc-freshness` detects via diff size.)*
