...
- [ ] **P1** Identify the change class on a single line in the eventual PR `## Issue`. *Class:* `feature | bugfix | refactor | migration | docs | chore | capability | plugin | runbook | ADR | pack-update`. *Lane:* Buck2 PR-shape/traceability target.
...
- [ ] **P5** Confirm license posture. *Command:* current Buck2 dependency/license policy target or registry-backed lane.
...
- [ ] **P7** Identify the per-change-class reviewer agent (per `docs/AGENTS.md §Per-change-class reviewer agents`). *Lane:* Buck2 PR-review evidence target.
...
- [ ] **P9** For hook / harness / CLI changes: run the relevant Buck2/Rust harness self-test first; do not add Node/package-manager tooling unless the lane has an explicit strict-TypeScript tooling exception.
...
### refactor
- [ ] No behavior change asserted; public API surface unchanged. *Command:* lane-owned Buck2 API compatibility target or documented equivalent.
