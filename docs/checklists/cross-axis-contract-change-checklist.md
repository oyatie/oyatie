...
enforcing_fitness_lane: oya-foundry-fitness-cross-axis-notify
...
- [ ] **X2** Identify which axes are **consumers** of the contract (depend on the surface). *Lane:* `oya-foundry-fitness-cross-axis-deps` (graph from `Cargo.toml [dependencies]` + `contracts/<surface>` references).
...
- [ ] **X4** Apply PR label `cross-axis-contract-change` AND sub-label `breaking | non-breaking-additive | non-breaking-renaming | non-breaking-clarification`. *Lane:* `oya-foundry-fitness-cross-axis-notify`.

## Notification

- [ ] **X5** Notify every consumer-axis team via PR review request. *Lane:* `oya-foundry-fitness-cross-axis-notify`. (Auto-emit to per-axis Slack channels via Foundry capability `pr.cross-axis.notify`, per ADR-0053 sanctioned primitive path.)
- [ ] **X6** Update `docs/DESIGN.md §10` registry row in the same PR. *Lane:* `oya-foundry-fitness-design-contracts-mirror`.
- [ ] **X7** If breaking: author an ADR per `docs/templates/adr-template-v2.md` documenting the migration path. *Lane:* `oya-foundry-fitness-adr-shape`.
- [ ] **X7-INV** If contract file renamed/moved: append inventory ledger row per ADR-0052. *Lane:* `oya-foundry-fitness-inventory-tracker`.
...
- [ ] **X13** Contract file (`contracts/<surface>.<openapi|proto|asyncapi>.yaml`) updated; lints clean. *Lane:* `oya-foundry-fitness-contracts-lint`.
- [ ] **X14** Provider-neutral interface preserved: provider-specific code remains in `oya-<context>-adapter-<provider>-*` crates only. *Lane:* `oya-foundry-fitness-provider-coupling`.
...
- [ ] **X17** Schema migration (if state) ships up + down + dry-run + per-tenant + per-cell rollback. *Lane:* `oya-foundry-fitness-schema-migration`.
- [ ] **X18** Feature-flag wired for breaking changes; canary rollout planned per `docs/RELEASE-MANAGEMENT.md`. *Lane:* `oya-foundry-fitness-release-readiness`.

## Audit + comms

- [ ] **X19** Audit-chain `EVT-CROSS-AXIS-CONTRACT-CHANGED` emitted with producer-axis, consumer-axis list, change-class, old/new schema hashes. *Lane:* `oya-foundry-fitness-audit-emission`.
- [ ] **X20** `docs/CHANGELOG.md` row appended.
- [ ] **X21** `docs/MISTAKES-LEDGER.md` row added if this change exists because a prior cross-axis change broke a consumer silently. *Lane:* `oya-foundry-fitness-mistakes-ledger-cite`.
...
- Skipping `docs/DESIGN.md §10` update — `oya-foundry-fitness-design-contracts-mirror` refuses the merge.
