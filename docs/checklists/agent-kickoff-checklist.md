...
Read every returned memory. If empty, that's a signal — your work has no prior context; proceed but emit a context-store at completion (per `agent-completion-checklist.md`). Uses ADR-0053 sanctioned primitive `icm`.
...
Every symbol in the IP `§Symbols to grit-claim` **MUST** show `unclaimed` (ADR-0054 scaffold-claim pattern). If any shows `claimed-by-<agent>`, pick a different IP (per phase INDEX `§Parallelism strategy`).
...
- IP `§Symbols to grit-claim` is empty or contains non-`file::Identifier` placeholders — refuse to claim; the IP is not in final shape (ADR-0054).
