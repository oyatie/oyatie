...
- kernel_crate: `oya-governance-cohesion-fitness-kernel` (EXISTING) — `CrossAxisContract`, verdict `CohesionFitnessReport`.
...
- severity: BLOCKER
- kernel_sketch: SEE existing `crates/oya-governance-cohesion-fitness-kernel/src/lib.rs` (treated as the canonical reference; copy `CrossAxisContract` + `validate_cohesion_fitness` shape verbatim for new lanes).
