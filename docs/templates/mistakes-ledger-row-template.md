...
3. **Mechanical prevention is mandatory** — if you cannot name authoritative CI / validator / runtime enforcement, file an escalation (`templates/checklists/escalation-checklist.md`); an optional hook alone never qualifies. ADR-0053 sanctioned primitives are valid only when paired with the authoritative backstop.
