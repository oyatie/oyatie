...
entry_gate: |
  ...
  Example: "M03/P00 complete; tenancy kernel ships; Buck2 affected checks green."
exit_gate: |
...
```bash
buck2 build //:<phase>-affected-build //:<phase>-policy-check --show-output     # exit 0
buck2 test //:<phase>-affected-tests                                             # exit 0; 0 failures
buck2 build //:<phase>-dependency-policy-check //:<phase>-docs-check --show-output  # exit 0
```

### Fitness lane gates

```bash
buck2 build //:<phase>-lean-a1-check //:<phase>-lean-a2-check --show-output  # layer ordering + cross-vertical refusal
buck2 build //:<phase>-lean-a3-check //:<phase>-lean-a4-check --show-output  # BC boundary + naming conformance
```
...
buck2 build //:<phase>-workflow-event-registry-check --show-output
# Verify Ontology object types registered
buck2 build //:<phase>-ontology-type-registry-check --show-output
```
...
|---|---|---|
| `dependency-direction` | Buck2 `lean-a1` target for `P0Y-<slug>` | exit 0 |
| `cross-product-refusal` | Buck2 `lean-a2` target for `P0Y-<slug>` | exit 0 |
| `port-location` | Buck2 `port-location` target for `P0Y-<slug>` | exit 0 |
| `layer-correctness` | Buck2 `layer-correctness` target for `P0Y-<slug>` | exit 0 |
| `statelessness` | Buck2 `statelessness` target for `P0Y-<slug>` | exit 0 |
| `shardability` | Buck2 `shardability` target for `P0Y-<slug>` | exit 0 |
