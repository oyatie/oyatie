...
How might we build an OAuth-subscription-pool LLM gateway in Oyatie that gives parallel agents transparent access to Claude Code / OpenAI Codex / Gemini CLI subscriptions, with gpt-load's reliability patterns and one-api's provider breadth, while being productizable for Oyatie tenants per oyatie-dogfood-tenancy?
