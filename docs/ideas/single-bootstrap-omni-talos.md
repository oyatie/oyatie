...
| Apps | Argo CD + Kustomize | app-of-apps GitOps (all in-cluster): **Forgejo** `infra/forge/` (git forge + merge gate), **Jenkins** + signed agents + SeaweedFS + cargo mirror `infra/ci/` (CI), **OpenBao** `infra/kms/` (KMS), **observability** `microservices/observability/`, **Kyverno** (admission + cosign verify), **Istio Ambient** (L7, with worker pools), workloads |

**Substrate trio (ADR-0363: git + Jenkins + Forgejo)** lives entirely inside the Omni-managed cluster
as Argo CD apps. Jenkins = CI (build/test → posts 14 commit-status contexts to Forgejo's merge gate,
`oyaCiLane.groovy`); Forgejo = git forge + merge gate; distinct from the Argo CD CD loop. The single
bootstrap only stands up **Omni → cluster → Cilium → Argo CD**; Argo CD pulls everything else.

**GitOps source of truth:** GitHub (`github.com/jason931225/oyatie`) **at bootstrap**; Forgejo
(in-cluster, Argo CD-managed workload per `infra/forge/forgejo-argocd-app.yaml`) becomes primary only
after a deliberate post-bootstrap cutover (ADR-0247). This avoids the "Argo CD needs Forgejo which
needs Argo CD" deadlock — Forgejo is just another managed app; GitHub bootstraps the loop. Forgejo's
primary role is the CI merge-gate substrate (Jenkins commit-status → merge gate, ADR-0363), not the
CD loop itself.
...
| **worker `system`** (trusted) | vanilla, runc | Argo CD, Forgejo, Jenkins controller, OpenBao, observability backends, Kyverno, cert-manager, ingress/Gateway, istiod |
| **worker `ci`** (semi-trusted) | vanilla (Kata for untrusted PR exec) | Jenkins agents (ephemeral) |
...
Forgejo/Jenkins/OpenBao/ArgoCD). The **topology** is faithful; **capacity** is not. On one host run a
