...
| Apps | Argo CD + Kustomize | app-of-apps GitOps (all in-cluster): **GitHub** `infra/forge/` (git forge + merge gate), **Jenkins** + signed agents + SeaweedFS + cargo mirror `infra/ci/` (CI), **OpenBao** `infra/kms/` (KMS), **observability** `microservices/observability/`, **Kyverno** (admission + cosign verify), **Istio Ambient** (L7, with worker pools), workloads |

**Substrate trio (ADR-0363: git + Jenkins + GitHub)** lives entirely inside the Omni-managed cluster
as Argo CD apps. Jenkins = CI (build/test → posts 14 commit-status contexts to GitHub's merge gate,
`oyaCiLane.groovy`); GitHub = git forge + merge gate; distinct from the Argo CD CD loop. The single
...
| **worker `system`** (trusted) | vanilla, runc | Argo CD, GitHub, Jenkins controller, OpenBao, observability backends, Kyverno, cert-manager, ingress/Gateway, istiod |
...
GitHub/Jenkins/OpenBao/ArgoCD). The **topology** is faithful; **capacity** is not. On one host run a
