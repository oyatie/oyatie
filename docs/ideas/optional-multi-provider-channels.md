...
> never default implement/scout drivers, never merge authority, never CursorBench-ranked.

## How Might We

**HMW** keep `omx` / `omc` / `gjc` / `grok` available as **optional second-opinion / multi-provider**
channels without making them default drivers, without mapping [CursorBench](https://cursor.com/cursorbench)
onto them, and with **accurate flag semantics**.
...
(`.cursor/rules/model-routing.mdc`, `.grok/harness/model-routing.v1.json`).
...
| Side panel | Cursor UI | advisory; human-driven |
...
**CursorBench does not transfer** to these channels. Pick omx/omc/gjc/grok by provider need and
independence — not by CursorBench rank.
