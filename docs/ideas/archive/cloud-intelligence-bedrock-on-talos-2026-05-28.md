...
The naive answer ("just proxy all providers") conflates two different concerns: (a) the routing + pooling + cost-control layer (cloud-intelligence v1) and (b) the capability-port abstraction that insulates callers from provider lock-in. This idea-pager argues for the Bedrock-on-Talos pattern: treat `oya-invoke` as the capability port, run AWS Bedrock-compatible endpoints via the cloud-intelligence gateway on Talos, and defer cross-provider transparent failover to v2.
...
- **v2**: Bedrock Converse / InvokeModel compat surface on Talos. `oya-invoke` capability port. Cross-provider transparent failover via Application Inference Profiles (AWS pattern, self-hosted equivalent).
...
- **`oya-invoke` call convention**: should it be a Rust trait (`InvokeGate`) or a plain HTTP client pointed at the gateway? Lean HTTP client (simpler, no trait coupling, gateway is the right abstraction boundary).
