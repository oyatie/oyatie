...
`oya-ci` collapses several separately-planned things into one substrate: the **CI gate**
...
- **Phase 1 — Core gate (plank+crier+job):** `oya-ci-controller` spawns a K8s Job per PR running
  the TRUNK gate script vs the PR ref; posts a terminal `oya-ci-gate` status + summary. Cut over;
...
## Open Questions
- `oya-ci` as one multi-binary crate (controller/merge/deck/plugins as modes) vs a workspace of crates? (lean: workspace, crate-per-component, shared core.)
