...
**Recommendation:** Monthly IAM sync between `axis-cloud` and `platform-tenancy-identity` is already in both charters. Additionally, add a fitness function `oya-foundry-fitness-iam-lockstep` that verifies the two IAM kernel shape versions are compatible after any IAM change. This is better than a process-only mitigation.
