...
`buck2 run //cloud/cloud-ci/gates/cloud-ci-firewall-app:cloud-ci-firewall-signoff-fixer
...
| `cloud-ci-automation-ratchet` | core | producer-face (`automation_ratchet`) | `advisory_claiming_enforced`, `blocking_invariant_mapped_to_cli`, `ratchet_regression` (frozen-empty), `enforceable_or_automatable_marked_human_judgment`, `duplicate_row_id` (frozen-empty), `unknown_classification`, `missing_or_empty_required_field` |
...
| `cloud-ci-bnf-layer-suffix` | rust-cargo | producer-face (`bnf_layer_suffix`) | `bnf_unknown_role`, `bnf_role_mismatch`, `bnf_missing_prefix`, `bnf_empty_after_prefix`, `bnf_undeclared_role`, `bnf_undeclared_context`, `bnf_name_uppercase` |
...
`cloud-ci-face-settle --verify` is the REQUIRED last step before EVERY push, explicitly
...
(`cloud-ci-face-settle --settle --commit` for faces, `cargo metadata >/dev/null` plus a
...
as its `cloud-ci-friction-accounting-app-gate` buck2 `rust_test` under the binding
...
`cloud-ci-canonical-json-app-gate` buck2 `rust_test` under the binding `buck2 test
...
//cloud/cloud-ci/gates/cloud-ci-embedded-asset-hermeticity-app:cloud-ci-embedded-asset-hermeticity-fixer
...
rustls, the AWS SDKs, etcd). It runs as its `cloud-ci-kernel-purity-app-gate` buck2 `rust_test`
...
prefix denies the kube-* family without matching the owned `kuberos`/`cloud-kernel-*` crates.
Automation-default (founder directive 2026-06-11): the `cloud-ci-kernel-purity-app-bin` binary
...
`buck-syntax-kernel` sound parser + write-through fixer harness (ADR-0549 closed the
...
`cloud-ci-no-graphql-without-adr-app-gate` buck2 `rust_test` (plus a labeled matrix check).
