...
| R-009 | License drift (an external dep changes from Apache-2 to AGPL/SSPL/BUSL) | 4 | 3 | 3 | **36** | `ops-security` + `axis-foundry` (foundry) | License policy ADR (planned P0); `oya-foundry-fitness-license` CI lane; per-quarter license audit; Vendor-Partner-Ledger | cargo-deny exit; license-watch alerts |
...
| R-019 | M3/MVP vocab leakage from legacy docs into new docs | 2 | 4 | 2 | 16 | `council-architecture` | `oya-foundry-fitness-glossary` checks for retired terms; banner in PRD §3.1; ADR-0050/0185/0191 explicitly amended | Grep for retired terms in PR diffs |
...
| R-026 | Glossary term drift across docs | 2 | 3 | 1 | 6 | `council-architecture` | `oya-foundry-fitness-glossary` |
