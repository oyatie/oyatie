...
| MFL-0019 | 2026-07-26 | Abandoned buck2 daemons accumulated to 59 processes holding 22.8 GB with zero active builds, starving later agent lanes of RAM and file descriptors | The daemon idle timeout was 4h, so a daemon minted per verification-step `--isolation-dir` outlived by most of a working day the lane that created it | `.buckconfig` `[buck2] daemon_idle_timeout_s` lowered 14400 -> 7200, floored above the FULL-tier merge-base-worktree idle gap so no warm main-tree graph is reaped mid-lane | (target with protected `oya-ci-required` admission) | per [GH #1377](https://github.com/jason931225/oyatie/issues/1377); peak concurrency is NOT addressed by this row and remains open there |
