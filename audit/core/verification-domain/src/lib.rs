...
//! `Verified` (the redaction claim, like the first-period claim, is
//! independently confirmed via a port rather than taken on the caller's
//! word). See [`verify`]'s own doc for the full breakdown and
...
//! through the four ports it is given. It is a pure function to a
//! [`VerificationVerdict`].
//!
...
//! traits shaped just like three of the four in [`ports`] — because that
//! dependency edge is out of scope here. [`ports::RootRegistry`],
...
//! `audit_verification_kernel` counterpart. The fourth,
//! [`ports::RedactionRegistry`], has no `audit_verification_kernel`
//! counterpart at all — it is this crate's own addition, for the same
//! reason [`ports::RootRegistry::is_first_period`] is: see its doc.
//!
//! Key resolution, prior-root lookup, and redaction confirmation all need a
//! real read path this pure crate does not have, so [`ports::KeyResolver`],
//! [`ports::RootRegistry`], and [`ports::RedactionRegistry`] stay
//! caller-implemented ports with no concrete adapter here — exactly the
//! same shape `audit_sealing_domain::seal_record::PriorPeriodLookup` uses,
//! and for the same reason (see [`ports::RootRegistry::is_first_period`]'s
//! doc). Merkle-proof verification, by contrast, is pure math with no I/O,
//! so this crate DOES supply a real, concrete [`ports::MerkleVerifier`]
