---
doc_class: Runbook
doc_id: RUNBOOK-AUDIT_CHAIN-AUTO_REBALANCE
...
# Runbook: Audit Chain Auto Rebalance

## Operator Contract
- Runbook id: audit-chain-auto-rebalance.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-audit-chain-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=audit-chain; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n audit-chain rollout status deploy/audit-chain --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service audit-chain --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service audit-chain --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service audit-chain --cell $CELL --window 30m`.
