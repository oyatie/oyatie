...
## Trigger Conditions
- Page on alert `AuditChainSignatureVerificationFailureCritical` when `audit_chain_signature_verification_failure_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `AuditChainSignatureVerificationFailureSloBurn` when `audit_chain_signature_verification_failure_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev1 if `audit_chain_signature_verification_failure_correctness_ratio < 0.9999` and the affected label set includes `tenant_id`, `cell_id`, or `principal_id`.
- Open a sev1 if `audit_chain_signature_verification_failure_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
...
- Trigger from CI when `cargo run -p dev-cli -- gate validate audit-chain-signature-verification-failure --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Service-specific metric `audit_chain_signature_verification_failure_merkle_gap_total` exceeds the threshold documented in `audit/observability/slos/chain-of-custody-integrity-correctness.openslo.yaml`.
...
- Metric pattern: `audit_chain_signature_verification_failure_error_ratio` rises before `audit_chain_signature_verification_failure_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `audit_chain_signature_verification_failure_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal, replay backlog, or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `audit_chain_signature_verification_failure_queue_depth`; isolate tenant before fleet mitigation.
...
- Service-specific metric pattern: `audit_chain_signature_verification_failure_seal_latency_seconds` rises while `audit_chain_signature_verification_failure_evidence_export_age_seconds` is flat; inspect local worker health before escalating vendors.
- Service-specific metric pattern: `audit_chain_signature_verification_failure_evidence_export_age_seconds` rises while `audit_chain_signature_verification_failure_error_ratio` is flat; suspect stale export, stale recommendation, stale projection, or vendor dependency lag.
...
7. Check Prometheus error ratio: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=audit_chain_signature_verification_failure_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=audit_chain_signature_verification_failure_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=audit_chain_signature_verification_failure_queue_depth{cell="prod-us-east-1"}'`.
10. Check service-specific signal: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=audit_chain_signature_verification_failure_merkle_gap_total{cell="prod-us-east-1"}'`.
...
15. Run production snapshot gate: `cargo run -p dev-cli -- gate validate audit-chain-signature-verification-failure --production-snapshot --cell $CELL`.
16. Run crate smoke test: `cargo test -p audit-chain-domain signature_verification_failure -- --nocapture`.
17. Check API contract smoke: `curl -s https://audit-chain.internal.oyatie.dev/v1/audit-chain/signature-verification-failure/incident-handoff -H "x-tenant: $TENANT"`.
...
23. Check SLO files: `ls audit/observability/slos/*.openslo.yaml | sort | rg "chain|signature"`.
24. Check catalog components: `find audit/catalog -maxdepth 1 -type f | sort | rg "emission|sealing|verification|query|retention"`.
25. Run targeted SQL state query: `psql $OYATIE_PROD_DSN -c "select incident_id, tenant_id, cell_id, state, updated_at from audit_chain_signature_verification_failure_incidents where updated_at > now() - interval '30 minutes' order by updated_at desc limit 20;"`.
26. Confirm no cross-cell spread: `oya ops cells query --metric audit_chain_signature_verification_failure_error_ratio --window 30m --threshold 0.02`.
...
2. Does audit_chain_signature_verification_failure_queue_depth grow while audit_chain_signature_verification_failure_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths audit/runbooks/signature-verification-failure.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://audit-chain.internal.oyatie.dev/v1/audit-chain/signature-verification-failure/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: confirmed chain integrity risk.
  - Required action: keep `audit-chain-signature-verification-failure-circuit-breaker` open until `audit_chain_signature_verification_failure_error_ratio` is below 0.005 for 3 evaluator windows.
...
- Branch B: export-only degradation.
  - Required action: keep `audit-chain-signature-verification-failure-circuit-breaker` open until `audit_chain_signature_verification_failure_error_ratio` is below 0.01 for 3 evaluator windows.
...
## Resolution Steps
1. Identify code owner path: `rg "signature_verification_failure|AuditChainSignatureVerificationFailureCritical|audit_chain.signature_verification_failure.incident_state" crates audit -g "!audit/runbooks/**"`.
2. Patch domain invariant: `edit audit-chain-domain where signature_verification_failure state transition is validated`.
...
6. Add regression test: `cargo test -p audit-chain-domain signature_verification_failure_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate audit-chain-signature-verification-failure --fixture incident-signature-verification-failure.json`.
8. Add SLO assertion: `update audit/observability/slos/chain-of-custody-integrity-correctness.openslo.yaml with alert AuditChainSignatureVerificationFailureCritical when this was a missing alert`.
9. Add dashboard panel: `update audit/dashboards/emission-rate.json with audit_chain_signature_verification_failure_error_ratio, audit_chain_signature_verification_failure_lag_seconds, and audit_chain_signature_verification_failure_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric audit_chain_signature_verification_failure_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `audit-chain-domain`: inspect for `signature_verification_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `audit-chain-usecase`: inspect for `signature_verification_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `audit-chain-file-adapter`: inspect for `signature_verification_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
- `shared-audit-chain-client-kernel`: inspect for `signature_verification_failure` invariants, alert emission, ADR-0263 evidence fields, and tenant/cell scoping before touching adjacent code.
...
- `audit/dashboards/emission-rate.json`: verify panel coverage for `audit_chain_signature_verification_failure_error_ratio`, `audit_chain_signature_verification_failure_lag_seconds`, and `audit_chain_signature_verification_failure_merkle_gap_total`.
...
- `audit_chain_signature_verification_failure_error_ratio < 0.005` for 3 consecutive 10 minute windows.
- `audit_chain_signature_verification_failure_lag_seconds < 120` for all production cells.
- `audit_chain_signature_verification_failure_queue_depth` is draining and not growing for the affected tenant.
- Service-specific signal `audit_chain_signature_verification_failure_merkle_gap_total` is below the threshold documented in `audit/observability/slos/chain-of-custody-integrity-correctness.openslo.yaml`.
...
## Handoff Notes
- Do not hand off with only the alert name; include `audit_chain_signature_verification_failure_error_ratio`, `audit_chain_signature_verification_failure_lag_seconds`, `audit_chain_signature_verification_failure_queue_depth`, `audit_chain_signature_verification_failure_merkle_gap_total`, current breaker state, and audit seal status.
...
- `audit/catalog/` for component and owner vocabulary; existing runbook topic `signature-verification-failure` was preserved as the scenario anchor.
