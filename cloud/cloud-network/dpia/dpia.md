...
# Data Protection Impact Assessment: cloud-network
...
- Event surface: automation events for this microservice include Buck2/Prow required-context outcomes, governance lane evidence, sharding automation decisions, and native promotion transitions.
...
- Cedar decision id, audit-chain event id, Buck2/Prow run id, native promotion run id, cosign verification result, and governance lane id.
...
### ADR-Bound Controls
- ADR-0346: historical local verifier doctrine only; active evidence is Buck2 target output plus trusted Rust/Prow `oya-ci-required` controller context.
...
- ADR-0349: historical Jenkins/ArgoCD doctrine only; active CI/CD authority is ADR-0513 oya-ci/Prow plus native release-conveyor-like promotion, with GitHub Actions as a temporary adapter.
...
- Access control: service principals, Prow principals, native promotion principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
### Residual Risk
- Residual risk is medium until Wave 15-ZA/ZB/ZD/ZE implementation PRs land the Buck2/Prow evidence, governance rename, sharding automation bodies, and native promotion substrate files.
