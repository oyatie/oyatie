...
slot_owner: ZF-9
sharding_role: network-topology-owner
---
# IP-WAVE-15-ZD-sharding-automation: Cloud Network Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/cloud-network/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: cloud-network must interpret ADR-0348 through its own bounded context: cloud-network.
SCOPE-004: cloud-network uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: cloud-network uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: cloud-network uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Buck2/Prow, CUE/KRM, and native promotion surfaces.
...
STANCE-001: Microservice: cloud-network.
STANCE-002: Owner team: axis-cloud-network.
STANCE-003: Sharding role: network-topology-owner.
STANCE-004: Role statement: keeps cell and shard traffic policy aligned with Cilium, mesh, and region boundaries.
STANCE-005: Bounded context anchor: cloud-network.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_capability.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for cloud-network.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for cloud-network because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; cloud-network must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where cloud-network participates.
...
STANCE-018: CI stance is Buck2 target evidence plus ADR-0513 oya-ci/Prow required context before push for downstream code, schema, or workflow changes.
STANCE-019: CI/CD substrate stance is ADR-0513 Buck2/Prow plus native promotion; ADR-0349 Jenkins/ArgoCD wording is historical provenance only.
STANCE-020: Governance naming stance is ADR-0347; this IP uses governance lane identifiers consistently.

## 3. ADR-0346 Historical Local-Verifier Provenance
ADR346-PURPOSE-001: ADR-0346 local verifier wording is historical provenance only.
ADR346-PURPOSE-002: Active verification evidence is Buck2 target output plus ADR-0513 oya-ci/Prow `oya-ci-required` required context.
ADR346-PURPOSE-003: Retired local CLI skip flags, exit-code contracts, and verifier source hooks must not be reintroduced as merge authority.
ADR346-ENFORCED-BY-001: buck2-target-evidence (active lane; refuses changes without the relevant Buck2 build/test/check evidence).
ADR346-ENFORCED-BY-002: oya-ci-required-controller-context (active lane; refuses merge readiness when the trusted Prow context is missing).
ADR346-ENFORCED-BY-003: retired-local-verifier-provenance (documentation lane; keeps historical ADR text out of active runbooks, PRDs, and substrate docs).
...
## 6. ADR-0349 Historical CI/CD Provenance
ADR349-PURPOSE-001: ADR-0349 Jenkins/ArgoCD wording is historical provenance only and is not active or interim authority.
ADR349-PURPOSE-002: GitHub/GitHub Actions are temporary PR/publication and shadow-evidence adapters.
ADR349-PURPOSE-003: ADR-0513 oya-ci/Prow owns required CI context; native release-conveyor-like promotion owns durable CD.
ADR349-PURPOSE-004: First-party Kubernetes desired state is CUE/KRM. Helm is external chart compatibility only after CUE/KRM normalization.
ADR349-PURPOSE-005: Cosign verification, tenant namespace isolation, declarative Prow state, and audit-chain deploy emission are enforced by native governance lanes.
ADR349-ENFORCED-BY-001: oya-governance-buck2-prow-required-context (new lane; refuses Buck2 target / Prow job drift such that a CI step exists in one surface but not the other across the per-microservice CI-parity contract enumerated in D-3 below; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
ADR349-ENFORCED-BY-002: oya-governance-native-promotion-cosign-verified (new lane; refuses native promotion desired-state sources that reference an image without a cosign-verify policy attached per D-6 + ADR-0181; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
ADR349-ENFORCED-BY-003: oya-governance-native-promotion-tenant-namespace-isolation (new lane; refuses native promotion desired-state authoring that crosses tenant namespaces without a Cedar policy gate granting cross-tenant access per D-11 + ADR-0243; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
ADR349-ENFORCED-BY-004: oya-governance-prow-controller-declarative-config-only (new lane; refuses Prow controller state declared via the UI; every Prow controller state file is authored under cloud/cloud-iac/modules/<context>/oya-ci/prow/ with declarative JCasC YAML per D-1; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
ADR349-ENFORCED-BY-005: oya-governance-native-promotion-audit-chain-emit (new lane; refuses native promotion transitions that do not emit an audit-chain row per ADR-0263 D.4 deploy-event class; promoted to BLOCKER 30 days post Wave 15-ZE-completion)
...
PLAN-026: Use ADR-0513 oya-ci/Prow required-context evidence when CI surfaces are introduced.
PLAN-027: Use native promotion cosign verification when deployment manifests are introduced.
...
ROLLBACK-002: For cloud-network, rollback starts from the audit-chain row that recorded the automation event.
...
VERIFY-024: Downstream implementation must prove native promotion emits deploy audit rows when deployment surfaces land.
VERIFY-025: Downstream implementation must prove Buck2/Prow required-context evidence when CI lands.
...
ACCEPT-015: The file treats ADR-0349 Jenkins/ArgoCD substrate expectations as historical provenance and cites ADR-0513 Buck2/Prow active authority.
