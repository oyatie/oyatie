...
companion_docs:
  ...
  - microservices/cloud-network/PRD.md
  - microservices/cloud-network/README.md
---

# Architecture: Cloud Network

## Architecture Boundary

`cloud-network` keeps its existing bounded context and flat `microservices/cloud-network/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0349; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
- Integration state: PROPOSED for `cloud-network`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `cloud-network` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: aws-guest/vpc@v1[both], oci-guest/vcn@v1[both], oci-guest/drg@v1[paid], oyatie-as-cloud-provider/cell-zone@v1[both].
...
- Capacity: `per_capability` drives CPU `0.14`, RAM `256`, storage `2`, and connection pool variables.
- DR: RTO `600` seconds and RPO `300` seconds constrain backup and failover primitives.
