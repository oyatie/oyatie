---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_NETWORK-AUTO_REBALANCE
...
# Runbook: Cloud Network Auto Rebalance

## Operator Contract
- Runbook id: cloud-network-auto-rebalance.
- Primary service namespace: `cloud-network`.
...
- Owning team: axis-cloud-network.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-network-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-network; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-network rollout status deploy/cloud-network --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-network --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-network --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-network --cell $CELL --window 30m`.
