---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_NETWORK-HOT_SPLIT
...
# Runbook: Cloud Network Hot Split

## Operator Contract
- Runbook id: cloud-network-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-network-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-network; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-network rollout status deploy/cloud-network --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-network --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-network --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-network --cell $CELL --window 30m`.
