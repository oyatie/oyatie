---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_DATA-HOT_SPLIT
...
# Runbook: Cloud Data Hot Split

## Operator Contract
- Runbook id: cloud-data-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-data-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-data; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-data rollout status deploy/cloud-data --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-data --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-data --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-data --cell $CELL --window 30m`.
