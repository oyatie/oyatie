...
1. Confirm the `cloud-network-dns` manifest exists and is not marked with `status: "Retired"` and is not `doc_class: RetiredMicroserviceMarker`.
2. Capture the current migration-playbooks directory listing before authoring implementation follow-up.
3. Capture whether `cloud-network-dns` already has CI, CD, cellular, sharding, or governance lane references in PRD, ARCH, README, IPs, manifests, runbooks, contracts, Cedar, SLOs, and capabilities.
...
1. Treat legacy `oya verify --ci-required` as optional local-feedback/provenance for `cloud-network-dns` changes; push/merge readiness depends on branch-protected `oya-ci-required` evidence.
...
3. When `cloud-network-dns` changes touch Rust, contracts, manifests, generated docs, or governance lanes, record branch-protected `oya-ci-required` evidence before handoff; a legacy verifier run may be attached only as local-feedback/provenance.
...
1. Search `cloud-network-dns` artifact surfaces for `oya-governance-*` references.
...
4. Update any future `cloud-network-dns` manifest `governance_lanes` array only in the manifest-owning slot.
...
1. Prepare `cloud-network-dns` for a future `sharding_automation` block with autosharding, auto_rebalance, and dynamic_sharding sub-blocks.
...
1. Treat Jenkins CI wording as historical for `cloud-network-dns`; any self-hosted/owned CI cutover must preserve the ADR-0515 `oya-ci-required` contract.
...
5. Future owned-runner parity for `cloud-network-dns` must preserve the same cloud-ci Rust gate packets and `oya-ci-required` context.
...
1. Freeze the current `cloud-network-dns` doctrine references before implementation starts.
2. Assign ownership for the Wave 15-ZA, 15-ZB, 15-ZD, and 15-ZE follow-up surfaces touching this microservice.
3. Confirm whether `cloud-network-dns` is stateful, tenant-facing, edge-facing, or substrate-facing; that classification drives sharding and CI/CD evidence.
...
2. This playbook lists every enforced_by lane from the four ADRs that can affect `cloud-network-dns`.
...
Stop when `cloud-network-dns` has this scaffold committed under `migration-playbooks/`, ADR citation validation passes for the docs corpus, and no file outside this slot boundary is staged for the ZF-18 commit.
