...
# Data Protection Impact Assessment: cloud-network-dns
...
- Event surface: automation events for this microservice include Buck2/Prow verifier evidence, governance lane rename evidence, sharding automation decisions, and release-conveyor/controller transitions.
...
- Cedar decision id, audit-chain event id, Buck2/Prow evidence id, `oya-ci-required` context id, release-conveyor/controller sync id, cosign/provenance verification result, and governance lane id.
...
### ADR-Bound Controls
- ADR-0346: historical local verifier/gate CLI doctrine; active CI/merge authority is superseded by ADR-0513 Buck2 evidence plus Rust/Prow `oya-ci-required`. The historical governance lane names remain provenance only.
...
- ADR-0349: historical Jenkins/ArgoCD doctrine; active direction is Kubernetes-native oya-ci/Prow jobs and release-conveyor-like native CD seams, with GitHub/GitHub Actions as temporary adapter/shadow only. Historical lane names remain provenance only.
...
- Access control: service principals, oya-ci/Prow principals, release-conveyor/controller principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
### Residual Risk
- Residual risk is medium until implementation PRs land Buck2/Prow evidence, governance rename, sharding automation bodies, and native oya-ci/release-conveyor substrate files.
- Residual risk becomes low when the ADR-0346, ADR-0347, ADR-0348, ADR-0349 historical-provenance lanes and ADR-0513 active authority checks pass for this microservice.
