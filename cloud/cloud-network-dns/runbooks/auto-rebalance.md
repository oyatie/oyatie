---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_NETWORK_DNS-AUTO_REBALANCE
...
# Runbook: Cloud Network Dns Auto Rebalance

## Operator Contract
- Runbook id: cloud-network-dns-auto-rebalance.
- Primary service namespace: `cloud-network-dns`.
...
## Doctrine Anchors
- ADR-0513 authority update: Buck2 evidence plus Rust/Prow `oya-ci-required` replaces retired local verifier/gate CLI authority.
...
- Trigger 7: Buck2/Prow `oya-ci-required` evidence drift blocks the release train for the sharding automation lane.
- Trigger 8: the native release-conveyor/controller reports pending reconciliation tied to this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-network-dns-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-network-dns; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-network-dns rollout status deploy/cloud-network-dns --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-network-dns --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-network-dns --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-network-dns --cell $CELL --window 30m`.
...
9. Verify release-conveyor/controller health through the owning Buck2/Prow evidence target.
10. Verify Buck2/Prow `oya-ci-required` evidence exists for this service before declaring the runbook complete.
...
5. If controller reconciliation is pending, pause execution until release-conveyor confirms the service declaration is current.
6. If Prow/GitHub shadow evidence is unknown, keep the change in report-only state and run Buck2 verification before push.
...
9. Keep native release-conveyor/controller reconciliation in evidence-only mode; do not run manual `kubectl apply` or manual Helm CLI deploys.
...
- Evidence 6: release-conveyor/controller sync id and cosign/provenance policy result.
- Evidence 7: `oya-ci-required` context id or GitHub Actions shadow run id proving CI/CD evidence for this service.
- Evidence 8: Buck2 local verification result before any push related to this runbook.
...
6. Re-run release-conveyor/controller reconciliation evidence and confirm no manual drift remains.
...
7. Confirm release-conveyor/controller did not sync unsigned images and did not cross tenant namespaces.
