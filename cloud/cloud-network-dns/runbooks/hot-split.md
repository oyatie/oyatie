---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_NETWORK_DNS-HOT_SPLIT
...
# Runbook: Cloud Network Dns Hot Split

## Operator Contract
- Runbook id: cloud-network-dns-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-network-dns-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-network-dns; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-network-dns rollout status deploy/cloud-network-dns --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-network-dns --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-network-dns --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-network-dns --cell $CELL --window 30m`.
