---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_NETWORK_DNS-COLD_MERGE
...
# Runbook: Cloud Network Dns Cold Merge

## Operator Contract
- Runbook id: cloud-network-dns-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-network-dns-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-network-dns; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-network-dns rollout status deploy/cloud-network-dns --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-network-dns --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-network-dns --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-network-dns --cell $CELL --window 30m`.
