...
Audience: an Oyatie operator or migration owner reading historical Wave 15-ZD doctrine propagation for `cell-lifecycle`. The CI/CD authority portions are superseded by ADR-0513 and must not be used to author Jenkinsfiles, ArgoCD applications, retired `bin/oya` verifier flows, or gate CLI surfaces.
...
1. Treat Buck2 verification as the local rehearsal for `cell-lifecycle` changes before push.
...
1. Treat this phase as superseded historical context; do not author Jenkins or ArgoCD interim surfaces for `cell-lifecycle`.
