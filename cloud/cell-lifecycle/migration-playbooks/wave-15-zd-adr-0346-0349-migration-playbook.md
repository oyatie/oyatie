...
# Migration Playbook - Wave 15-ZD ADR-0346..0349 doctrine for `cell-lifecycle`

Audience: an Oyatie operator or migration owner preparing `cell-lifecycle` for the Wave 15-ZD doctrine surface before implementation waves author runtime code, manifests, Jenkinsfiles, ArgoCD applications, or sharding bodies.

Outcome: `cell-lifecycle` has a documented migration path for the four doctrine decisions, with no runtime mutation and no manifest mutation.
...
1. Confirm the `cell-lifecycle` manifest exists and is not marked with `status: "Retired"` and is not `doc_class: RetiredMicroserviceMarker`.
2. Capture the current migration-playbooks directory listing before authoring implementation follow-up.
3. Capture whether `cell-lifecycle` already has CI, CD, cellular, sharding, or governance lane references in PRD, ARCH, README, IPs, manifests, runbooks, contracts, Cedar, SLOs, and capabilities.
...
1. Treat legacy `oya verify --ci-required` as optional local-feedback/provenance for `cell-lifecycle` changes; push/merge readiness depends on branch-protected `oya-ci-required` evidence.
...
3. When `cell-lifecycle` changes touch Rust, contracts, manifests, generated docs, or governance lanes, record branch-protected `oya-ci-required` evidence before handoff; a legacy verifier run may be attached only as local-feedback/provenance.
...
1. Search `cell-lifecycle` artifact surfaces for `oya-governance-*` references.
...
4. Update any future `cell-lifecycle` manifest `governance_lanes` array only in the manifest-owning slot.
...
1. Prepare `cell-lifecycle` for a future `sharding_automation` block with autosharding, auto_rebalance, and dynamic_sharding sub-blocks.
...
1. Treat Jenkins CI wording as historical for `cell-lifecycle`; any self-hosted/owned CI cutover must preserve the ADR-0515 `oya-ci-required` contract.
...
5. Future owned-runner parity for `cell-lifecycle` must preserve the same cloud-ci Rust gate packets and `oya-ci-required` context.
...
1. Freeze the current `cell-lifecycle` doctrine references before implementation starts.
2. Assign ownership for the Wave 15-ZA, 15-ZB, 15-ZD, and 15-ZE follow-up surfaces touching this microservice.
3. Confirm whether `cell-lifecycle` is stateful, tenant-facing, edge-facing, or substrate-facing; that classification drives sharding and CI/CD evidence.
...
5. This file is the `cell-lifecycle` migration scaffold that those implementation lanes can cite.
...
2. This playbook lists every enforced_by lane from the four ADRs that can affect `cell-lifecycle`.
...
- Local verifier transcript for `cell-lifecycle` once Wave 15-ZA lands.
- Rename inventory diff for `cell-lifecycle` once Wave 15-ZB lands.
...
Stop when `cell-lifecycle` has this scaffold committed under `migration-playbooks/`, ADR citation validation passes for the docs corpus, and no file outside this slot boundary is staged for the ZF-18 commit.
