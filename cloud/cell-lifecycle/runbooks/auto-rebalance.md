---
doc_class: Runbook
doc_id: RUNBOOK-CELL_LIFECYCLE-AUTO_REBALANCE
...
# Runbook: Cell Lifecycle Auto Rebalance

## Operator Contract
- Runbook id: cell-lifecycle-auto-rebalance.
- Primary service namespace: `cell-lifecycle`.
...
- Trigger 7: Prow/GitHub shadow-evidence drift blocks the release train for the sharding automation lane.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-lifecycle-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-lifecycle; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-lifecycle rollout status deploy/cell-lifecycle --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-lifecycle --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cell-lifecycle --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cell-lifecycle --cell $CELL --window 30m`.
...
- Evidence 7: Prow job id plus GitHub Actions shadow run id when present for this service.
...
6. Re-run native controller refresh and confirm no manual drift remains.
