---
doc_class: Runbook
doc_id: RUNBOOK-CELL_LIFECYCLE-COLD_MERGE
...
# Runbook: Cell Lifecycle Cold Merge

## Operator Contract
- Runbook id: cell-lifecycle-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-lifecycle-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-lifecycle; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-lifecycle rollout status deploy/cell-lifecycle --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-lifecycle --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cell-lifecycle --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cell-lifecycle --cell $CELL --window 30m`.
