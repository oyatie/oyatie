---
doc_class: Runbook
doc_id: RUNBOOK-CELL_LIFECYCLE-HOT_SPLIT
...
# Runbook: Cell Lifecycle Hot Split

## Operator Contract
- Runbook id: cell-lifecycle-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-lifecycle-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-lifecycle; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-lifecycle rollout status deploy/cell-lifecycle --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-lifecycle --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cell-lifecycle --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cell-lifecycle --cell $CELL --window 30m`.
