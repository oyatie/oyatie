---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_BILLING-AUTO_REBALANCE
...
# Runbook: Cloud Billing Auto Rebalance

## Operator Contract
- Runbook id: cloud-billing-auto-rebalance.
- Primary service namespace: `cloud-billing`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-billing-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-billing; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-billing rollout status deploy/cloud-billing --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-billing --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-billing --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-billing --cell $CELL --window 30m`.
