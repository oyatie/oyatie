---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_BILLING-HOT_SPLIT
...
# Runbook: Cloud Billing Hot Split

## Operator Contract
- Runbook id: cloud-billing-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-billing-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-billing; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-billing rollout status deploy/cloud-billing --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-billing --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-billing --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-billing --cell $CELL --window 30m`.
