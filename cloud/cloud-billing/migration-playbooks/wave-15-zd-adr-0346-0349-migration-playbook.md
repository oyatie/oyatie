# cloud-billing Wave 15-ZD active substrate migration playbook

Audience: an Oyatie operator or migration owner preparing `cloud-billing` for parallel lane work without reviving retired local CLI, Jenkins, ArgoCD, or first-party Helm authority.
...
2. Keep cloud-billing edits inside `cloud/cloud-billing/**`, the Rust hygiene guard/test, and lane-specific evidence.
...
- Rust hygiene test rejects retired Cargo/local-CLI/Jenkins/ArgoCD/Helm authority for `cloud-billing`.
- Targeted scans under `cloud/cloud-billing` show no active retired authority phrases.
- `cloud/cloud-billing/iac/k8s/helm` is absent.
