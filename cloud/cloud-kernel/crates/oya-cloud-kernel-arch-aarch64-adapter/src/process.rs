...
static mut VFS: Option<crate::user_layout::vfs::Vfs> = None;
...
pub fn with_vfs<R>(f: impl FnOnce(&mut crate::user_layout::vfs::Vfs) -> R) -> R {
    // SAFETY: single-core, non-reentrant access to the Frame-owned VFS.
    let vfs = unsafe { &mut *core::ptr::addr_of_mut!(VFS) };
    f(vfs.get_or_insert_with(crate::user_layout::vfs::Vfs::new))
}
