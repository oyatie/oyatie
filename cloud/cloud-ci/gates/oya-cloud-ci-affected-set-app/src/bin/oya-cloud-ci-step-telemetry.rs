...
use oya_cloud_ci_affected_set_app::long_step_telemetry_line;
...
fn main() -> ExitCode {
    let args = match parse_args(std::env::args()) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "{LOG}: usage: oya-cloud-ci-step-telemetry --phase <name> [--interval-seconds N] -- <command> [args...]"
            );
...
mod tests {
    ...
    fn argv(args: &[&str]) -> impl Iterator<Item = String> {
        std::iter::once("oya-cloud-ci-step-telemetry".to_owned())
