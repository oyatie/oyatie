...
impl Report {
    fn from_findings(findings: BTreeSet<Finding>) -> Self {
        let violations = findings.into_iter().map(|finding| finding.code).collect();
...
pub fn evaluate_keyed(spec: &Value) -> BTreeSet<Finding> {
    ...
    require_eq_str(spec, "status", "Proposed-target", "status_not_proposed_target", &mut findings);
    require_array_superset(spec, "controls", REQUIRED_CONTROLS, "missing_required_control", &mut findings);
    forbid_array_overlap(spec, "controls", FORBIDDEN_CONTROL_MARKERS, "forbidden_control_marker", &mut findings);
    require_bool_path(spec, &["strict_separation_constraints", "no_external_saas_ci"], true, "strict_separation_allows_external_saas_ci", &mut findings);
    require_bool_path(spec, &["strict_separation_constraints", "no_live_provider_apply"], true, "strict_separation_allows_live_provider_apply", &mut findings);
    require_bool_path(spec, &["strict_separation_constraints", "no_github_actions_fallback"], true, "strict_separation_allows_github_actions_fallback", &mut findings);
    require_bool_path(spec, &["strict_separation_constraints", "no_public_cloud_runtime_dependency"], true, "strict_separation_allows_public_cloud_runtime_dependency", &mut findings);
    forbid_text_markers(
    ...
    require_bool_path(spec, &["pure_dogfood_constraints", "self_hosted_github_kubernetes_ci_lane"], true, "missing_self_hosted_github_kubernetes_ci_lane", &mut findings);
    require_bool_path(spec, &["pure_dogfood_constraints", "dogfood_resource_substrate_required_before_external_provider_apply"], true, "missing_dogfood_resource_substrate_constraint", &mut findings);
    require_bool_path(spec, &["pure_dogfood_constraints", "vfkit_linux_or_kubernetes_cluster_tests_must_be_recorded_before_kubernetes_readiness_claim"], true, "missing_kubernetes_readiness_evidence_constraint", &mut findings);
    require_bool_path(spec, &["pure_dogfood_constraints", "g007_must_reconcile_historical_jenkins_wording"], true, "missing_g007_reconciliation_constraint", &mut findings);
    require_array_superset_at(spec, &["evidence_vocabulary", "required_category_evidence_classes"], REQUIRED_EVIDENCE_CLASSES, "missing_required_evidence_class", &mut findings);
...
fn validate_sources(spec: &Value, findings: &mut BTreeSet<Finding>) -> BTreeMap<String, BTreeSet<String>> {
    let mut source_providers = BTreeSet::new();
    let mut coverage_by_provider: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
    let Some(sources) = spec.get("official_source_evidence").and_then(Value::as_array) else {
        findings.insert(Finding::new("official_source_evidence_not_array", "official_source_evidence"));
        return coverage_by_provider;
    };

    for source in sources {
        let provider = source.get("provider").and_then(Value::as_str).unwrap_or_default();
        let url = source.get("url").and_then(Value::as_str).unwrap_or_default();
        if provider.is_empty() {
            findings.insert(Finding::new("source_missing_provider", "<missing-provider>"));
            continue;
        }
        ...
        if source.get("evidence_use").and_then(Value::as_str).unwrap_or_default().trim().is_empty() {
        ...
        coverage_by_provider.entry(provider.to_owned()).or_default().extend(coverage);
    }
...
fn official_url(provider: &str, url: &str) -> bool {
    match provider {
        "aws" => url.starts_with("https://aws.amazon.com/") || url.starts_with("https://docs.aws.amazon.com/"),
        "google_cloud" => url.starts_with("https://cloud.google.com/") || url.starts_with("https://docs.cloud.google.com/"),
        "azure" => url.starts_with("https://azure.microsoft.com/") || url.starts_with("https://learn.microsoft.com/"),
        "oci" => url.starts_with("https://www.oracle.com/") || url.starts_with("https://docs.oracle.com/"),
        "kubernetes" => url.starts_with("https://kubernetes.io/"),
        "cncf" => url.starts_with("https://github.com/cncf/") || url.starts_with("https://www.cncf.io/"),
        _ => false,
    }
}
...
fn validate_categories(
    ...
    let Some(categories) = spec.get("category_taxonomy").and_then(Value::as_array) else {
        findings.insert(Finding::new("category_taxonomy_not_array", "category_taxonomy"));
        return;
    };
    if categories.is_empty() {
        findings.insert(Finding::new("category_taxonomy_empty", "category_taxonomy"));
    }
    let category_ids: BTreeSet<String> = categories.iter().filter_map(|c| c.get("id").and_then(Value::as_str).map(str::to_owned)).collect();
    ...
    for category in categories {
        let category_id = category.get("id").and_then(Value::as_str).unwrap_or("<missing-category-id>");
        if category.get("target_capability").and_then(Value::as_str).unwrap_or_default().trim().is_empty() {
            findings.insert(Finding::new("category_missing_target_capability", category_id));
        }
        ...
        if category_id != "cloud_native_platform_contract" {
            ...
            for provider in PROVIDERS {
                if !example_providers.contains(*provider) {
                    findings.insert(Finding::new("category_missing_provider_example", &format!("{category_id}:{provider}")));
                }
            }
        }
        let evidence = string_set(category.get("required_evidence"));
        if evidence.is_empty() {
            findings.insert(Finding::new("category_missing_required_evidence", category_id));
        }
        for required in REQUIRED_EVIDENCE_CLASSES {
            if !evidence.contains(*required) {
                findings.insert(Finding::new("category_missing_required_evidence_class", &format!("{category_id}:{required}")));
            }
        }
        let normalized_evidence = normalized_claim_text(category.get("required_evidence").unwrap_or(&Value::Null));
        for marker in VAGUE_EVIDENCE_MARKERS {
            if normalized_evidence.split_whitespace().any(|token| token == *marker) {
                findings.insert(Finding::new("category_vague_evidence_marker", &format!("{category_id}:{marker}")));
            }
        }
        let gates = string_set(category.get("hyperscaler_gates"));
        if gates.is_empty() {
            findings.insert(Finding::new("category_missing_hyperscaler_gates", category_id));
        }
        if !gates.iter().any(|gate| gate.starts_with("HG-")) {
            findings.insert(Finding::new("category_hyperscaler_gate_id_not_hg", category_id));
        }
        let source_providers_for_category: BTreeSet<&str> = coverage_by_provider
            .iter()
            .filter_map(|(provider, covered)| covered.contains(category_id).then_some(provider.as_str()))
            .collect();
        ...
        for provider in needed {
            if !source_providers_for_category.contains(provider) {
                findings.insert(Finding::new("category_missing_official_source_coverage", &format!("{category_id}:{provider}")));
            }
        }
    }
}
...
fn validate_mapping(spec: &Value, findings: &mut BTreeSet<Finding>) {
    let Some(mappings) = spec.get("local_oyatie_mapping").and_then(Value::as_array) else {
        findings.insert(Finding::new("local_oyatie_mapping_not_array", "local_oyatie_mapping"));
        return;
    };
    let mapping_ids: BTreeSet<String> = mappings.iter().filter_map(|m| m.get("category_id").and_then(Value::as_str).map(str::to_owned)).collect();
    for required in REQUIRED_CATEGORIES {
    ...
    for mapping in mappings {
        let category_id = mapping.get("category_id").and_then(Value::as_str).unwrap_or("<missing-category-id>");
        match mapping.get("claim_status").and_then(Value::as_str) {
            Some("target_spec_only" | "metadata_foundation" | "evidence_required") => {}
            _ => { findings.insert(Finding::new("mapping_invalid_claim_status", category_id)); }
        }
        if mapping.get("honest_claim").and_then(Value::as_str).unwrap_or_default().trim().is_empty()
            || mapping.get("cannot_claim_yet").and_then(Value::as_array).is_none_or(Vec::is_empty)
        {
            findings.insert(Finding::new("mapping_missing_claim_or_nonclaim_text", category_id));
        }
        let blocked = string_set(mapping.get("blocked_claim_families"));
        for required in REQUIRED_NONCLAIMS {
            if !blocked.contains(*required) {
                findings.insert(Finding::new("mapping_missing_blocked_claim_family", &format!("{category_id}:{required}")));
            }
        }
        if contains_forbidden_claim(mapping.get("honest_claim").unwrap_or(&Value::Null)) {
            findings.insert(Finding::new("mapping_honest_claim_forbidden_overclaim", category_id));
        }
        let cannot_claim_text = flattened_text(mapping.get("cannot_claim_yet").unwrap_or(&Value::Null));
        if !(cannot_claim_text.contains("feature parity") && cannot_claim_text.contains("production")) {
            findings.insert(Finding::new("mapping_nonclaims_missing_parity_or_production", category_id));
        }
    }
}
...
fn validate_claim_matrix(spec: &Value, findings: &mut BTreeSet<Finding>) {
    ...
    for field in ["can_claim_now", "cannot_claim_yet", "evidence_required_before_claim"] {
        if matrix.get(field).and_then(Value::as_array).is_none_or(Vec::is_empty) {
    ...
    if contains_forbidden_claim(matrix.get("can_claim_now").unwrap_or(&Value::Null)) {
        findings.insert(Finding::new("claim_matrix_can_claim_now_forbidden_overclaim", "claim_matrix.can_claim_now"));
    }
    let Some(requirements) = matrix.get("evidence_required_before_claim").and_then(Value::as_array) else {
        return;
    };
    ...
    for item in requirements {
        if let Some(family) = item.get("claim_family").and_then(Value::as_str) {
            claim_families.insert(family.to_owned(), flattened_text(item.get("requires").unwrap_or(&Value::Null)));
        }
    }
    let Some(strict_ci_text) = claim_families.get("strict_dogfood_ci") else {
        findings.insert(Finding::new("missing_strict_dogfood_ci_requirement", "claim_matrix.evidence_required_before_claim"));
        return;
    };
    if !strict_ci_text.contains("no external saas fallback") {
        findings.insert(Finding::new("strict_dogfood_ci_allows_external_saas_fallback", "strict_dogfood_ci"));
    }
    if !(strict_ci_text.contains("self-hosted") && strict_ci_text.contains("g007")) {
        findings.insert(Finding::new("strict_dogfood_ci_missing_self_hosted_g007", "strict_dogfood_ci"));
    }
}
...
fn validate_nonclaims(spec: &Value, findings: &mut BTreeSet<Finding>) {
    let nonclaims: BTreeSet<String> = spec
        ...
        .map(|items| items.iter().filter_map(|item| item.get("id").and_then(Value::as_str).map(str::to_owned)).collect())
        .unwrap_or_default();
    for required in REQUIRED_NONCLAIMS {
...
fn validate_next_goal_mapping(spec: &Value, findings: &mut BTreeSet<Finding>) {
    let Some(mapping) = spec.get("next_goal_mapping").and_then(Value::as_object) else {
        findings.insert(Finding::new("next_goal_mapping_not_object", "next_goal_mapping"));
        return;
    };
    let values: BTreeSet<String> = mapping.values().filter_map(Value::as_str).map(str::to_owned).collect();
    ...
    if mapping.get("dogfood_ci_claim_path").and_then(Value::as_str) != Some("G007") {
        findings.insert(Finding::new("dogfood_ci_claim_path_not_g007", "dogfood_ci_claim_path"));
    }
}

fn require_eq_str(spec: &Value, field: &str, expected: &str, code: &str, findings: &mut BTreeSet<Finding>) {
    if spec.get(field).and_then(Value::as_str) != Some(expected) {
        findings.insert(Finding::new(code, field));
    }
}

fn require_bool_path(spec: &Value, path: &[&str], expected: bool, code: &str, findings: &mut BTreeSet<Finding>) {
    if value_at(spec, path).and_then(Value::as_bool) != Some(expected) {
        findings.insert(Finding::new(code, &path.join(".")));
    }
}

fn require_array_superset(spec: &Value, field: &str, required: &[&str], code: &str, findings: &mut BTreeSet<Finding>) {
    require_array_superset_at(spec, &[field], required, code, findings);
}

fn require_array_superset_at(spec: &Value, path: &[&str], required: &[&str], code: &str, findings: &mut BTreeSet<Finding>) {
...
fn forbid_array_overlap(spec: &Value, field: &str, forbidden: &[&str], code: &str, findings: &mut BTreeSet<Finding>) {
...
fn forbid_text_markers(value: Option<&Value>, markers: &[&str], code: &str, key: &str, findings: &mut BTreeSet<Finding>) {
...
fn flattened_text(value: &Value) -> String {
    match value {
        Value::Object(map) => map.values().map(flattened_text).collect::<Vec<_>>().join(" ").to_lowercase(),
        Value::Array(items) => items.iter().map(flattened_text).collect::<Vec<_>>().join(" ").to_lowercase(),
...
fn contains_forbidden_claim(value: &Value) -> bool {
    ...
    FORBIDDEN_CAN_CLAIM_PHRASES.iter().any(|phrase| {
        let phrase = format!(" {} ", normalized_claim_text(&Value::String((*phrase).to_owned())));
...
mod tests {
    ...
    fn rejects_forbidden_claim_variants() {
        let mut spec = minimal_green();
        spec["claim_matrix"]["can_claim_now"][0]["claim"] = json!("Oyatie Cloud has production-readiness and feature-parity.");
        assert!(evaluate(&spec).violations.contains("claim_matrix_can_claim_now_forbidden_overclaim"));
    }
    ...
    fn rejects_external_saas_evidence_lane() {
        ...
        assert!(evaluate(&spec).violations.contains("forbidden_allowed_evidence_lane"));
    }
    ...
    fn rejects_missing_category_and_required_evidence() {
        ...
        assert!(evaluate(&spec).violations.contains("missing_required_category"));
