...
//!
//! A PURE function of declared inputs: it reads `git-facts.generated.json` (the committed,
//! registry-drift-protected snapshot of the four git outputs emitted by the out-of-graph
//! `oya-cloud-ci-git-facts-emitter-app`) + the real reachability/owner/justification sources, then
...
//! `<out-dir>` (default `<repo-root>/cloud/cloud-ci/gates`). `--git-facts` defaults to the
//! committed `git-facts.generated.json` beside the faces.
...
/// The committed snapshot of the four git outputs (emitted by `oya-cloud-ci-git-facts-emitter-app`).
...
/// Read + parse the git-facts face. A missing/malformed face is a hard error (it must be
/// regenerated by the emitter), so a degraded checkout fails LOUDLY rather than silently
/// producing false-green faces.
fn load_git_facts(path: &Path) -> Result<GitFacts, CliError> {
    let text = std::fs::read_to_string(path).map_err(|e| {
        CliError::Io(format!(
            "{}: {e} (run oya-cloud-ci-git-facts-emitter-app to regenerate the git-facts face)",
            path.display()
        ))
    })?;
    let value: Value = serde_json::from_str(&text)
        .map_err(|e| CliError::Io(format!("{}: parse git-facts: {e}", path.display())))?;
...
fn run() -> Result<(), CliError> {
    ...
    let mut git_facts_path: Option<PathBuf> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--git-facts" => {
                i += 1;
                git_facts_path = args.get(i).map(PathBuf::from);
            }
            "--face" => {
                i += 1;
    ...
    // The committed git-facts face (the declared input that replaces ambient git). Defaults to
    ...
    let git_facts_path = git_facts_path.unwrap_or_else(|| {
        repo_root.join("cloud/cloud-ci/gates/oya-cloud-ci-accounting-registry-app/git-facts.generated.json")
    });
    let git_facts = load_git_facts(&git_facts_path)?;
    ...
    let inputs = collect_repo_inputs(&repo_root, &cfg, &git_facts);
    let registry = build_registry(&inputs, &policy)?;
    let crosswalk = build_decision_crosswalk(&collect_crosswalk_inputs(&repo_root, &cfg))?;
    let enforcement =
        build_enforcement_inventory(&collect_enforcement_inputs(&repo_root, &cfg, &git_facts))?;
    ...
    let staleness_input = build_staleness_input(&git_facts, &registry);
...
/// wall-clock). Reads HEAD time + commit timestamps from the declared git-facts face (no
...
fn build_staleness_input(git_facts: &GitFacts, registry: &Value) -> Value {
    let now_secs = git_facts.head_time_secs;
    let commit_ts = &git_facts.commit_author_ts_secs;
...
fn collect_enforcement_inputs(
    ...
    for cargo in tracked_paths_matching(git_facts, |p| {
...
/// Tracked paths matching a predicate (filtered from the declared git-facts tracked-paths).
fn tracked_paths_matching(git_facts: &GitFacts, pred: impl Fn(&str) -> bool) -> Vec<String> {
    git_facts
        .tracked_paths
        .iter()
        .filter(|p| pred(p))
...
/// git-facts face (no ambient git); the owner/justification/reachability maps are derived from
...
fn collect_repo_inputs(
    ...
    let tracked_paths = git_facts.tracked_paths.clone();
    let last_touch = git_facts.last_touch.clone();
