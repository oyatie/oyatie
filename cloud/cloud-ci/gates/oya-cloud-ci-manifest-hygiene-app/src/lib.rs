...
pub fn evaluate_keyed(input: &Value) -> BTreeSet<Finding> {
    ...
    for row in &rows {
        ...
        if !flag(row, "has_rust_version_workspace") {
            findings.insert(Finding::new("manifest_missing_rust_version_workspace", name));
        }
        ...
        if flag(row, "has_lib") && !flag(row, "has_lib_doctest_false") {
...
mod tests {
    ...
    fn each_missing_field_fires_its_code() {
        ...
        assert!(evaluate_keyed(&input).iter().all(|f| f.key == "oya-bad-domain"));
    }
    ...
    fn missing_lib_doctest_fires_when_lib_present() {
        ...
        assert_eq!(findings.iter().next().unwrap().code, "manifest_missing_lib_doctest_false");
    }
