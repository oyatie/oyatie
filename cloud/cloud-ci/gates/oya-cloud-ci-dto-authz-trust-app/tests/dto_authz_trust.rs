...
fn baseline_is_nonempty_and_covers_a_known_instance() {
    ...
    assert!(
        baseline.len() >= 40,
        "expected the frozen baseline to enumerate the ~90 known instances the review found; got {}",
        baseline.len()
    );
    // v2 keys have the format `<file>#<fn>:<body_hash>` — match by `<file>#<fn>:` prefix so the
    // check is stable across body edits that change the hash but not the identity.
