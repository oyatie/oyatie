...
/// last-touch commit metadata for every tracked NON-generated-class path, so ANY commit
/// touching such a path changes the regenerated bytes, while faces-only settle commits and
/// Cargo.lock-only refresh commits are fixpoints. The other producer faces are stable bytes
/// (content-derived, unchanged by these fixtures).
