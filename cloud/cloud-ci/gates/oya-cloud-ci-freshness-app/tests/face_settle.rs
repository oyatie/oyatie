...
use oya_cloud_ci_freshness_app::{
    ...
    FaceSettleMode, assert_committed_tree_clean, assert_non_face_tree_clean,
    generated_face_paths, parse_face_settle_args, settle_regenerated_faces,
    verify_committed_faces,
};
...
/// Stand-in for the scm-facts emitter CONTRACT (the property the verify mode exists for):
/// `scm-facts.generated.json` is a pure function of the committed tree that records per-path
/// last-touch commit metadata for every tracked NON-generated path, so ANY commit touching any
/// non-face path changes the regenerated bytes, while a faces-only settle commit is a fixpoint.
/// The other producer faces are stable bytes (content-derived, unchanged by these fixtures).
fn metadata_bearing_regenerated_faces(root: &Path) -> Vec<(String, String)> {
    let face_paths: BTreeSet<String> = generated_face_paths().into_iter().collect();
    let mut scm_facts =
        String::from("scm-facts stand-in: per-path last_touch_commit over tracked non-face paths\n");
    for path in git_output(root, &["ls-files"]).lines() {
        if face_paths.contains(path) {
            // Emitter fixpoint contract: generated-class paths are excluded from last_touch,
            // so the faces-only settle commit does not churn scm-facts.
            continue;
        }
...
fn verify_passes_on_properly_settled_tree() {
    ...
    let report = verify_committed_faces(&root, metadata_bearing_regenerated_faces(&root))
...
fn verify_never_mutates_the_tree() {
    ...
    let green = verify_committed_faces(&root, metadata_bearing_regenerated_faces(&root))
        .expect("green verify");
    ...
    let red = verify_committed_faces(&root, metadata_bearing_regenerated_faces(&root))
        .expect("red verify");
