...
//! `//cloud/cloud-ci/gates:accounting-registry-producer` (register #20 — NOT an `oya` CLI).
//!
//! Usage:
//!   accounting-registry-producer [--repo-root <path>] [--out-dir <path>] [--stdout]
//!
...
fn main() {
    if let Err(error) = run() {
        eprintln!("accounting-registry-producer: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), CliError> {
    ...
    let out_dir = out_dir
        .unwrap_or_else(|| repo_root.join("cloud/cloud-ci/gates/accounting-registry-producer"));
