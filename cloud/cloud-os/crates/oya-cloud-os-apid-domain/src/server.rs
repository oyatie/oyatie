...
//! connection), reads length-prefixed [`Request`](oya_cloud_os_proto_api::Request)
//! frames using the [`oya_cloud_os_proto_api`] wire codec, dispatches them to a
//! [`Backend`], and writes the framed [`Response`](oya_cloud_os_proto_api::Response).
//!
//! The wire format and framing are owned entirely by [`oya_cloud_os_proto_api`]; this
...
//! use oya_cloud_os_proto_api::{write_message, read_message, Request, Response};
//!
...
fn is_timeout(err: &oya_cloud_os_proto_api::WireError) -> bool {
    matches!(
        err,
        oya_cloud_os_proto_api::WireError::Io(e)
...
mod tests {
    ...
    use oya_cloud_os_proto_api::{read_message, write_frame, write_message};
