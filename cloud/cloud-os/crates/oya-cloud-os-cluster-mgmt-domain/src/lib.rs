...
mod tests {
    ...
    ///
    /// Sibling of the same guard in `oya-cloud-os-secrets-domain` and `oya-cloud-os-trustd-domain`.
    ...
    /// `oya-cloud-os-secrets-domain`, and the reason it has to exist here too: that crate
    ...
    ///
    /// Zero, not one: unlike `oya-cloud-os-secrets-domain` there is no modeled *target*
    ...
    /// the `oya-cloud-os-cluster-mgmt-domain` `rust_library` gives
    ///
    /// ```text
    ...
    fn no_buck_target_enables_the_model() {
        ...
        assert!(
            buck.contains("oya-cloud-os-cluster-mgmt-domain"),
    ...
    //
    //   buck2 build //cloud/cloud-os/crates/oya-cloud-os-cluster-mgmt-domain:oya-cloud-os-cluster-mgmt-domain
    ...
    fn modeled_crypto_constructors_stay_behind_the_gate() {
        ...
        // not. Same shape as `oya_cloud_os_secrets_domain::tests::crate_root_gate_is_present`
        // and `oya_cloud_os_trustd_domain::token::tests::gated`.
        //
