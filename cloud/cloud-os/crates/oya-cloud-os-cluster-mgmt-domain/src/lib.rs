...
/// crates beyond `talos-core`, and `oya_cloud_os_kernel::Error` is `no_std`/`alloc`
...
impl From<oya_cloud_os_kernel::Error> for ClusterError {
    fn from(e: oya_cloud_os_kernel::Error) -> Self {
        match e {
            oya_cloud_os_kernel::Error::Invalid(m)
            | oya_cloud_os_kernel::Error::Parse(m)
            | oya_cloud_os_kernel::Error::Unsupported(m) => ClusterError::Invalid(m),
            oya_cloud_os_kernel::Error::NotFound(m) => ClusterError::NotFound(m),
            oya_cloud_os_kernel::Error::InvalidState(m) => ClusterError::InvalidState(m),
            oya_cloud_os_kernel::Error::PermissionDenied(m) | oya_cloud_os_kernel::Error::Other(m) => {
                ClusterError::Invalid(m)
            }
            oya_cloud_os_kernel::Error::Timeout => ClusterError::Invalid("operation timed out".into()),
        }
    }
}
...
mod tests {
    ...
    use oya_cloud_os_kernel::version::Version;
    ...
    fn error_conversion_from_core() {
        let core = oya_cloud_os_kernel::Error::not_found("x");
