...
//!
//! Unlike `oya-cloud-os-trustd-domain`, there is no real backend beside the model to
...
//! an implementation.
//!
//! Measured on this tree: the production `oya-cloud-os-secrets-domain` library has five
...
mod tests {
    ...
    /// hole in the sibling guards in `oya-cloud-os-trustd-domain` and
    /// `oya-cloud-os-cluster-mgmt-domain`. Both now carry this same shape, and those two
    ...
    /// consumer of `oya-cloud-os-secrets-domain` without touching the modeled target at
    ...
    /// modeled-crypto (`oya-cloud-os-secrets-domain-modeled`)
    ///   left: 2
    ///  right: 1
    /// ```
    ///
    ...
    fn no_production_buck_target_enables_the_model() {
        ...
        assert_eq!(
            ...
            "exactly one BUCK target may enable modeled-crypto \
             (`oya-cloud-os-secrets-domain-modeled`); a second occurrence means another \
