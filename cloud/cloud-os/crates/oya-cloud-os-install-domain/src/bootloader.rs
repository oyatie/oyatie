...
/// Map a bootloader error into the workspace-wide [`oya_cloud_os_kernel::Error`].
impl From<BootloaderError> for oya_cloud_os_kernel::Error {
    fn from(e: BootloaderError) -> Self {
        match e {
            BootloaderError::InvalidConfig(m) => oya_cloud_os_kernel::Error::Invalid(m),
            BootloaderError::SlotNotPopulated(_) => oya_cloud_os_kernel::Error::InvalidState(e_display(&e)),
            BootloaderError::NoPreviousEntry => {
                oya_cloud_os_kernel::Error::InvalidState("no previous boot entry".to_string())
            }
            BootloaderError::UnsupportedLayout(m) => oya_cloud_os_kernel::Error::Unsupported(m),
            BootloaderError::SecureBootViolation(m) => oya_cloud_os_kernel::Error::PermissionDenied(m),
            BootloaderError::Io(m) => oya_cloud_os_kernel::Error::Other(m),
        }
    }
}
...
mod tests {
    ...
    fn converts_into_core_error() {
        let core: oya_cloud_os_kernel::Error = BootloaderError::UnsupportedLayout("no ESP".into()).into();
        assert_eq!(core.kind(), "unsupported");
        let core2: oya_cloud_os_kernel::Error =
