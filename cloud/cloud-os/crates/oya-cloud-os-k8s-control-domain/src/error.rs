...
//! cleanly into the workspace-wide [`oya_cloud_os_kernel::Error`] at the crate boundary.
...
impl From<ControlError> for oya_cloud_os_kernel::Error {
    fn from(e: ControlError) -> Self {
        match e {
            ControlError::InvalidConfig(m) | ControlError::Policy(m) => {
                oya_cloud_os_kernel::Error::Invalid(m)
            }
            ControlError::MissingSecret(m) => oya_cloud_os_kernel::Error::NotFound(m),
            ControlError::Render(m) | ControlError::Manifest(m) => oya_cloud_os_kernel::Error::Other(m),
            ControlError::Reconcile(m) => oya_cloud_os_kernel::Error::InvalidState(m),
            ControlError::KubeAccess(m) => oya_cloud_os_kernel::Error::PermissionDenied(m),
        }
    }
}
...
mod tests {
    ...
    fn converts_into_core_error() {
        let core: oya_cloud_os_kernel::Error = ControlError::Reconcile("bad".into()).into();
        assert_eq!(core.kind(), "invalid_state");
        let core: oya_cloud_os_kernel::Error = ControlError::MissingSecret("ca".into()).into();
        assert_eq!(core.kind(), "not_found");
        let core: oya_cloud_os_kernel::Error = ControlError::KubeAccess("denied".into()).into();
