...
impl Code {
    ...
    /// Map a [`oya_cloud_os_kernel::Error`] kind onto a gRPC status code, matching the
    /// way Talos's apid translates internal errors at the wire boundary.
    pub fn from_core_error(err: &oya_cloud_os_kernel::Error) -> Code {
        match err {
            oya_cloud_os_kernel::Error::Invalid(_) | oya_cloud_os_kernel::Error::Parse(_) => Code::InvalidArgument,
            oya_cloud_os_kernel::Error::NotFound(_) => Code::NotFound,
            oya_cloud_os_kernel::Error::PermissionDenied(_) => Code::PermissionDenied,
            oya_cloud_os_kernel::Error::InvalidState(_) => Code::FailedPrecondition,
            oya_cloud_os_kernel::Error::Timeout => Code::DeadlineExceeded,
            oya_cloud_os_kernel::Error::Unsupported(_) => Code::Unimplemented,
            oya_cloud_os_kernel::Error::Other(_) => Code::Unknown,
        }
    }
}
...
impl From<oya_cloud_os_kernel::Error> for ApiError {
    fn from(err: oya_cloud_os_kernel::Error) -> Self {
...
mod tests {
    ...
    fn code_grpc_values_and_mapping() {
        ...
        assert_eq!(
            Code::from_core_error(&oya_cloud_os_kernel::Error::not_found("x")),
            Code::NotFound
        );
        assert_eq!(
            Code::from_core_error(&oya_cloud_os_kernel::Error::permission_denied("x")),
            Code::PermissionDenied
        );
        assert_eq!(
            Code::from_core_error(&oya_cloud_os_kernel::Error::Timeout),
            Code::DeadlineExceeded
        );
        assert_eq!(
            Code::from_core_error(&oya_cloud_os_kernel::Error::invalid_state("x")),
    ...
    fn api_error_from_core() {
        let e: ApiError = oya_cloud_os_kernel::Error::not_found("missing").into();
