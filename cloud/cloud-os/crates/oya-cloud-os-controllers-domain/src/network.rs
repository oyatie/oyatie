...
use oya_cloud_os_kernel::Result as CoreResult;
use oya_cloud_os_kernel::{NodeAddress, ResourceId};
use oya_cloud_os_cosi_domain::resource::ResourceKind;
use oya_cloud_os_cosi_domain::{Metadata, Resource};
use oya_cloud_os_machine_config_domain::yaml::Yaml;
...
use oya_cloud_os_machine_config_domain::{KindSpec, Registry, decode_documents};
...
impl LinkStatusSourceController {
    ...
    pub fn new() -> Self {
        #[cfg(target_os = "linux")]
        {
            Self::new_with_source(oya_cloud_os_network_domain::list_link_statuses)
        }
...
fn load_network_config(contents: &str) -> CoreResult<oya_cloud_os_machine_config_domain::Config> {
...
fn push_config_doc_vip_operator_specs(
    out: &mut Vec<OperatorSpec>,
    container: &oya_cloud_os_machine_config_domain::Config,
) -> CoreResult<()> {
    for doc in container.documents() {
        if doc.meta.kind != LAYER2_VIP_CONFIG_KIND {
            continue;
        }
        let root = oya_cloud_os_machine_config_domain::yaml::parse(&doc.body)
            .map_err(|e| oya_cloud_os_kernel::Error::parse(e.to_string()))?;
...
fn vip_operator_spec(link_name: impl Into<String>) -> OperatorSpec {
    OperatorSpec {
        ...
        require_up: true,
        route_metric: oya_cloud_os_network_domain::DEFAULT_ROUTE_METRIC,
...
fn yaml_required_str<'a>(root: &'a Yaml, key: &str) -> CoreResult<&'a str> {
    root.get_str(key)
        .ok_or_else(|| oya_cloud_os_kernel::Error::invalid(format!("{key} must be specified")))
}
...
fn parse_yaml_u16(value: Option<&str>, field: &str) -> CoreResult<Option<u16>> {
    ...
    value
        .parse::<u16>()
        .map(Some)
        .map_err(|_| oya_cloud_os_kernel::Error::invalid(format!("{field} must be a u16")))
}
...
fn parse_cidr_core(cidr: &str) -> CoreResult<(NodeAddress, u8)> {
    let (address, prefix) = cidr
        .trim()
        .split_once('/')
        .ok_or_else(|| oya_cloud_os_kernel::Error::invalid("CIDR value is missing '/'"))?;
    let address = NodeAddress::parse(address.trim())?;
    let prefix_len = prefix
        .trim()
        .parse::<u8>()
        .map_err(|_| oya_cloud_os_kernel::Error::invalid(format!("invalid CIDR prefix '{prefix}'")))?;
...
fn parse_config_hex_bytes(s: &str) -> CoreResult<Vec<u8>> {
    parse_hex_bytes(s).map_err(|e| oya_cloud_os_kernel::Error::invalid(e.to_string()))
}
...
fn parse_oper_state(s: &str) -> ReconcileResult<oya_cloud_os_network_domain::OperState> {
    match s {
        "up" => Ok(oya_cloud_os_network_domain::OperState::Up),
        "down" => Ok(oya_cloud_os_network_domain::OperState::Down),
        "unknown" => Ok(oya_cloud_os_network_domain::OperState::Unknown),
...
fn client_identifier_as_str(spec: &ClientIdentifierSpec) -> &'static str {
    match spec.client_identifier {
        oya_cloud_os_network_domain::nethelpers::ClientIdentifier::None => "none",
        oya_cloud_os_network_domain::nethelpers::ClientIdentifier::Mac => "mac",
        oya_cloud_os_network_domain::nethelpers::ClientIdentifier::Duid => "duid",
    }
}
...
fn oper_state_as_str(state: oya_cloud_os_network_domain::OperState) -> &'static str {
    match state {
        oya_cloud_os_network_domain::OperState::Up => "up",
        oya_cloud_os_network_domain::OperState::Down => "down",
        oya_cloud_os_network_domain::OperState::Unknown => "unknown",
    }
}
...
"
    ...
    fn physical_status(name: &str) -> LinkStatus {
        LinkStatus {
            ...
            aliases: Vec::new(),
            admin_up: false,
            oper_state: oya_cloud_os_network_domain::OperState::Down,
            carrier: false,
    ...
    fn link_status_publication_controller_publishes_live_kernel_status_resources() {
        ...
        status.oper_state = oya_cloud_os_network_domain::OperState::Up;
        status.carrier = true;
    ...
    fn link_status_publication_controller_surfaces_malformed_live_status_without_silent_drop() {
        ...
        rt.register(Box::new(LinkStatusSourceController::new_with_source(
            || {
                Err(oya_cloud_os_kernel::Error::invalid(
...
";
    ...
    fn default_dhcp4_operator_spec_matches_source_defaults() {
        ...
        assert_eq!(op.route_metric, oya_cloud_os_network_domain::DEFAULT_ROUTE_METRIC);
