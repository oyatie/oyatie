...
use oya_cloud_os_block_domain::{VolumeConfigController, VolumeConfigRuntimeMode};
use oya_cloud_os_kernel::MachineType;
use oya_cloud_os_runtime_cri_domain::RegistriesConfigController;
...
impl Init {
    ...
    pub fn register_block_volume_config_controller(&mut self) -> Result<()> {
        if self
            ...
            .get(&crate::controllers::ControllerId::from(
                oya_cloud_os_block_domain::VOLUME_CONFIG_CONTROLLER_NAME,
            ))
            .is_some()
        {
            return Ok(());
    ...
    pub fn register_cri_registries_config_controller(&mut self) -> Result<()> {
        if self
            .controllers
            .get(&crate::controllers::ControllerId::from(
                oya_cloud_os_runtime_cri_domain::REGISTRIES_CONFIG_CONTROLLER_NAME,
            ))
            .is_some()
        {
            return Ok(());
...
mod tests {
    ...
    fn volume_config_startup_registers_block_controller_during_boot() {
        ...
        let block_controller_id =
            crate::controllers::ControllerId::from(oya_cloud_os_block_domain::VOLUME_CONFIG_CONTROLLER_NAME);
    ...
    fn cri_registries_config_startup_registers_controller_during_boot() {
        ...
        assert_eq!(
            controller.outputs(),
            &[oya_cloud_os_runtime_cri_domain::RegistriesConfigResource::kind().to_string()]
        );
    }
}
