...
//! converts cleanly into the workspace-wide [`oya_cloud_os_kernel::Error`] at the crate
//! boundary.
...
impl From<K8sError> for oya_cloud_os_kernel::Error {
    fn from(e: K8sError) -> Self {
        match e {
            K8sError::InvalidConfig(m)
            | K8sError::InvalidNodeName(m)
            | K8sError::MissingSecret(m) => oya_cloud_os_kernel::Error::Invalid(m),
            K8sError::Render(m) | K8sError::Bootstrap(m) => oya_cloud_os_kernel::Error::Other(m),
            K8sError::EtcdState(m) => oya_cloud_os_kernel::Error::InvalidState(m),
            K8sError::UnknownComponent(m) => oya_cloud_os_kernel::Error::NotFound(m),
            K8sError::NotBootstrapped => {
                oya_cloud_os_kernel::Error::InvalidState("control plane not bootstrapped".into())
            }
        }
    }
}
...
mod tests {
    ...
    fn converts_into_core_error() {
        let core: oya_cloud_os_kernel::Error = K8sError::EtcdState("not joined".into()).into();
        assert_eq!(core.kind(), "invalid_state");
        let core: oya_cloud_os_kernel::Error = K8sError::UnknownComponent("foo".into()).into();
