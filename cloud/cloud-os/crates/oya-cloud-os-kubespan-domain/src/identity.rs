...
fn derive_address(cluster_id: &str, public_key: &WireguardKey) -> NodeAddress {
    let net_hash = oya_cloud_os_kernel::id::Fingerprint::of_str(cluster_id).value();
    let host_hash = oya_cloud_os_kernel::id::Fingerprint::of_str(public_key.as_str()).value();
