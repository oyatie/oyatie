...
pub const VOLUME_CONFIG_TYPE: &str = oya_cloud_os_block_domain::VOLUME_CONFIG_TYPE;
...
pub fn volume_config_kind() -> ResourceKind {
    ResourceKind::new(oya_cloud_os_block_domain::mount::BLOCK_NAMESPACE, VOLUME_CONFIG_TYPE)
}
...
impl V1Alpha1ServiceResource {
    ...
    pub fn new(id: impl Into<String>, spec: V1Alpha1ServiceSpec) -> oya_cloud_os_kernel::Result<Self> {
    ...
    pub fn registryd(spec: V1Alpha1ServiceSpec) -> oya_cloud_os_kernel::Result<Self> {
...
pub type SourceBlockVolumeType = oya_cloud_os_block_domain::VolumeType;

/// Source-shaped partition provisioning subset used by the image-cache volumes.
pub type ImageCacheVolumeProvisioningSpec = oya_cloud_os_block_domain::VolumeConfigProvisioningSpec;

/// Source `block.VolumeConfig.Mount` subset written by Image Cache.
pub type ImageCacheVolumeMountSpec = oya_cloud_os_block_domain::VolumeConfigMountSpec;

/// Source-shaped `block.VolumeConfig` spec for Image Cache COSI output.
pub type ImageCacheVolumeConfigSpec = oya_cloud_os_block_domain::VolumeConfigSpec;

/// Generic COSI resource wrapper for source block `VolumeConfig` output.
pub type ImageCacheVolumeConfigResource = oya_cloud_os_block_domain::VolumeConfigResource;
...
pub fn image_cache_local_enabled_from_machine_config_contents(
    ...
    let doc = oya_cloud_os_machine_config_domain::yaml::parse(&body)
...
fn image_cache_v1alpha1_body(contents: &str) -> Cow<'_, str> {
    match oya_cloud_os_machine_config_domain::decode_documents(contents) {
...
pub fn image_cache_config_key() -> oya_cloud_os_kernel::Result<String> {
    Ok(oya_cloud_os_cosi_domain::Metadata::new(
...
pub fn image_cache_copy_state_key() -> oya_cloud_os_kernel::Result<String> {
    Ok(oya_cloud_os_cosi_domain::Metadata::new(
        IMAGE_CACHE_NAMESPACE,
...
pub fn registryd_service_key() -> oya_cloud_os_kernel::Result<String> {
    Ok(oya_cloud_os_cosi_domain::Metadata::new(
        V1ALPHA1_NAMESPACE,
        V1ALPHA1_SERVICE_TYPE,
...
pub fn image_cache_volume_config_key(id: &str) -> oya_cloud_os_kernel::Result<String> {
    Ok(oya_cloud_os_block_domain::volume_config_key(id)?)
}
...
fn image_cache_disk_volume_config_resource(
    ...
    if !contents.trim().is_empty() {
        let container =
            oya_cloud_os_machine_config_domain::load_from_bytes(contents).map_err(|err| err.to_string())?;
        let volume_configs =
            oya_cloud_os_machine_config_domain::volume_configs(&container).map_err(|err| err.to_string())?;
...
fn apply_image_cache_disk_volume_config_override(
    ...
    provisioning.wave = oya_cloud_os_block_domain::WAVE_SYSTEM_DISK;
    ...
    provisioning.type_uuid = Some(oya_cloud_os_block_domain::layout::type_guid::LINUX_FILESYSTEM.to_string());
    provisioning.filesystem = Some(oya_cloud_os_block_domain::FilesystemType::Ext4);
...
fn block_key_to_store<T>(result: oya_cloud_os_block_domain::Result<T>) -> StoreResult<T> {
...
mod tests {
    ...
    use oya_cloud_os_cosi_domain::Phase as CosiPhase;
...
"#,
    ...
    fn registryd_test_set_file_times(path: &Path, accessed: SystemTime, modified: SystemTime) {
        std::fs::OpenOptions::new()
            .write(true)
            .open(path)
            .expect("open test file to set times")
            .set_times(
                std::fs::FileTimes::new()
                    .set_accessed(accessed)
                    .set_modified(modified),
            )
            .expect("set test file times with Rust std");
    }
...
"#;
    ...
    fn image_cache_bridge_writes_config_mount_request_and_finalizer_to_cosi_state() {
        ...
        let mut state = oya_cloud_os_cosi_domain::State::new();
    ...
    fn image_cache_cosi_controller_runs_under_runtime_loop() {
        let mut runtime = oya_cloud_os_cosi_domain::Runtime::new();
        let disk_volume =
    ...
    fn image_cache_cosi_controller_spec_matches_source_declarations() {
        ...
        let expected_outputs = vec![
            ...
            Output::shared(ResourceKind::new(
                oya_cloud_os_block_domain::mount::BLOCK_NAMESPACE,
                VOLUME_CONFIG_TYPE,
            )),
