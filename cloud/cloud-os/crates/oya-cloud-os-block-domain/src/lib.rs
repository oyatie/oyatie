...
//! primitives such as [`oya_cloud_os_kernel`] and [`oya_cloud_os_cosi_domain`].
...
///
/// Distinct from [`oya_cloud_os_kernel::Error`] so callers can match on block-specific
...
impl From<BlockError> for oya_cloud_os_kernel::Error {
    fn from(e: BlockError) -> Self {
        match e {
            BlockError::InvalidDevice(m) | BlockError::Geometry(m) => oya_cloud_os_kernel::Error::Invalid(m),
            BlockError::BadTable(m) => oya_cloud_os_kernel::Error::Parse(m),
            BlockError::Unrecognized => {
                oya_cloud_os_kernel::Error::NotFound("no recognized signature".to_string())
            }
            BlockError::BadTransition(m) => oya_cloud_os_kernel::Error::InvalidState(m),
            BlockError::KeyFailure(m) => oya_cloud_os_kernel::Error::PermissionDenied(m),
            BlockError::NotFound(m) => oya_cloud_os_kernel::Error::NotFound(m),
        }
    }
}
...
mod tests {
    ...
    fn error_converts_to_core_error() {
        let e: oya_cloud_os_kernel::Error = BlockError::InvalidDevice("/dev/null".to_string()).into();
        assert_eq!(e.kind(), "invalid");
        let e: oya_cloud_os_kernel::Error = BlockError::BadTransition("x".to_string()).into();
        assert_eq!(e.kind(), "invalid_state");
        let e: oya_cloud_os_kernel::Error = BlockError::Unrecognized.into();
