...
impl Condition for Any<'_> {
    fn poll(&self) -> Poll {
        if self.members.is_empty() {
            ...
            // don't spin forever.
            return Poll::Failed(oya_cloud_os_kernel::Error::invalid("any-of has no members"));
        }
        let mut pending: Vec<String> = Vec::new();
        ...
        if all_failed {
            Poll::Failed(oya_cloud_os_kernel::Error::invalid_state(
