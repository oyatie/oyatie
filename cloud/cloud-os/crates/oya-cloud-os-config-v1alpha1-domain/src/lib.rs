...
//!
//! The crate builds on [`oya_cloud_os_machine_config_domain`] for the shared [`Validator`] /
//! [`ValidationMode`] / [`ValidationReport`] machinery and on `talos-core` for
//! the cross-cutting [`oya_cloud_os_kernel::Error`] type, pulling in no external crates so
...
pub use oya_cloud_os_kernel::machine_type::MachineType;
pub use oya_cloud_os_kernel::{Error, Result};
