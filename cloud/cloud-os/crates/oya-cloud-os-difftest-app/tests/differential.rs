...
use oya_cloud_os_machine_config_domain::corpus::{CorpusConfig, Validity, load_record};

use oya_cloud_os_kernel::NodeAddress;
use oya_cloud_os_kernel::machine_type::MachineType;
use oya_cloud_os_kernel::role::{Role, RoleSet};
use oya_cloud_os_kernel::version::build as version_build;
use oya_cloud_os_kubernetes_domain::secrets::REQUIRED_SECRETS;
use oya_cloud_os_kubernetes_domain::{
...
use oya_cloud_os_network_domain::nethelpers::{
...
use oya_cloud_os_platform_domain::metal_oauth::{OAuthConfigError, new_config as new_metal_oauth_config};
use oya_cloud_os_platform_domain::metal_url::{UrlVariableValues, populate_url, required_variables};
use oya_cloud_os_runtime_cri_domain::{CriContainerState, CriRuntime, PodSandboxState, RuntimeService};
use oya_cloud_os_secrets_domain::certsans::CertSans;
use oya_cloud_os_secrets_domain::etcd::{EtcdCert, EtcdController};
use oya_cloud_os_secrets_domain::kubernetes::{K8sCert as SecretK8sCert, KubernetesController};
...
fn dhcpv4_config_doc_operator_tsv() {
    ...
    for (lineno, line) in records(&body).iter().enumerate() {
        ...
        let operators = oya_cloud_os_init_app::config::machine_config_dhcp_operators(&config).unwrap();
        assert_eq!(
            operators.len(),
            1,
...
fn resolver_config_doc_operator_tsv() {
    ...
    for (lineno, line) in records(&body).iter().enumerate() {
        ...
        let specs = oya_cloud_os_init_app::config::machine_config_resolver_specs(&config).unwrap();
        assert_eq!(
            specs.len(),
            1,
...
fn link_fields_fingerprint(fields: &oya_cloud_os_machine_config_domain::LinkFields) -> String {
    let up = fields
        .up
...
fn link_config_parse_fingerprint(config: &str, kind: &str) -> String {
    let container = oya_cloud_os_machine_config_domain::load_from_bytes(config).unwrap();
    match kind {
        "LinkConfig" => {
            let links = oya_cloud_os_machine_config_domain::link_configs(&container).unwrap();
            assert_eq!(
                links.len(),
                1,
        ...
        "VLANConfig" => {
            let vlans = oya_cloud_os_machine_config_domain::vlan_configs(&container).unwrap();
            assert_eq!(
                vlans.len(),
                1,
...
fn link_config_projection_fingerprint(config: &str, _kind: &str) -> String {
    match oya_cloud_os_controllers_domain::machine_config_link_projection_fingerprint(config) {
...
fn size_limit_fingerprint(value: Option<oya_cloud_os_machine_config_domain::SizeLimit>) -> String {
    match value {
        Some(oya_cloud_os_machine_config_domain::SizeLimit::Absolute(bytes)) => bytes.to_string(),
        Some(oya_cloud_os_machine_config_domain::SizeLimit::RelativePercent(percent)) => format!("{percent}%"),
        Some(oya_cloud_os_machine_config_domain::SizeLimit::NegativeBytes(bytes)) => format!("-{bytes}"),
        Some(oya_cloud_os_machine_config_domain::SizeLimit::NegativeRelativePercent(percent)) => {
            format!("-{percent}%")
        }
        None => "-".to_string(),
    }
...
fn volume_config_parse_fingerprint(config: &str, kind: &str) -> String {
    let container = match oya_cloud_os_machine_config_domain::load_from_bytes(config) {
        Ok(container) => container,
        Err(error) => return format!("error={error}"),
    };
    match kind {
        "VolumeConfig" => {
            let docs = match oya_cloud_os_machine_config_domain::volume_configs(&container) {
                Ok(docs) => docs,
                Err(error) => return format!("error={error}"),
            };
        ...
        "UserVolumeConfig" => {
            let docs = match oya_cloud_os_machine_config_domain::user_volume_configs(&container) {
                Ok(docs) => docs,
                Err(error) => return format!("error={error}"),
            };
        ...
        "RawVolumeConfig" => {
            let docs = match oya_cloud_os_machine_config_domain::raw_volume_configs(&container) {
                Ok(docs) => docs,
                Err(error) => return format!("error={error}"),
            };
...
fn volume_config_projection_fingerprint(config: &str) -> String {
    let manager = match oya_cloud_os_init_app::config::machine_config_volume_manager(config) {
        Ok(manager) => manager,
        Err(error) => return format!("error={error}"),
    };
    manager
...
fn vip_operator_config_tsv() {
    ...
    for (lineno, line) in records(&body).iter().enumerate() {
        ...
        let actual = oya_cloud_os_controllers_domain::network::machine_config_operator_specs_fingerprint(&config)
...
fn default_dhcp_operator_synthesis_tsv() {
    ...
    for (lineno, line) in records(&body).iter().enumerate() {
        ...
        let actual = oya_cloud_os_controllers_domain::default_dhcp_link_status_projection_fingerprint(
            config.as_deref(),
            &[status],
        )
        .unwrap_or_else(|error| format!("error={error}"));
...
fn control_plane_pki_file_closure_tsv() {
    ...
    let rendered: BTreeMap<&str, &oya_cloud_os_kubernetes_domain::RenderedFile> = output
        .files()
        .iter()
