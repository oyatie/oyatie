...
/// Convert this crate's error into the workspace-wide [`oya_cloud_os_kernel::Error`].
impl From<KernelParamError> for oya_cloud_os_kernel::Error {
    fn from(e: KernelParamError) -> Self {
        match e {
            ...
            KernelParamError::NotFound(m) => oya_cloud_os_kernel::Error::NotFound(m),
            KernelParamError::WriteRejected(m) => oya_cloud_os_kernel::Error::InvalidState(m),
            KernelParamError::Parse(m) => oya_cloud_os_kernel::Error::Parse(m),
            KernelParamError::Conflict(m) => oya_cloud_os_kernel::Error::Invalid(m),
        }
    }
}
...
mod tests {
    ...
    fn error_kind_and_conversion() {
        ...
        let e: oya_cloud_os_kernel::Error = KernelParamError::NotFound("k".into()).into();
