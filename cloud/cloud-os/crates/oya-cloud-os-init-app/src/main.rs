...
use oya_cloud_os_cosi_domain::State;
use oya_cloud_os_machined_domain::error::{MachinedError, Result as MachinedResult};
use oya_cloud_os_machined_domain::{ServiceLauncher, Supervisor, SupervisorRegistrydServiceManager};
use oya_cloud_os_runtime_cri_domain::{
...
/// resolver semantics.
pub fn pre_config_resolv_conf_body(resolver: &oya_cloud_os_network_domain::ResolverSpec) -> String {
...
/// Extract the errno from a `oya_cloud_os_network_domain` error's Display string. Pure /
/// host-testable.
///
...
) -> oya_cloud_os_kernel::Result<ImageCacheRuntimePlan> {
...
pub fn project_image_cache_runtime_plan_with_copy_done(
    ...
    let local_enabled = match raw_config {
        Some(raw) => oya_cloud_os_init_app::config::machine_config_image_cache_local_enabled(raw)?,
        None => false,
    };
...
pub fn apply_image_cache_runtime_observation_to_state(
    ...
    registryd: RegistrydState,
) -> oya_cloud_os_cosi_domain::StoreResult<ImageCacheRuntimePlan> {
...
/// [`oya_cloud_os_runtime_cri_domain::RegistrydRuntimeService`] payload for host-safe request
...
pub fn hydrate_declared_image_cache_block_state(
    raw_config: Option<&str>,
    state: &mut State,
) -> oya_cloud_os_kernel::Result<bool> {
    let Some(raw_config) = raw_config.filter(|raw| !raw.trim().is_empty()) else {
        return Ok(false);
    };
    if !oya_cloud_os_init_app::config::machine_config_image_cache_local_enabled(raw_config)? {
        return Ok(false);
    }

    let manager = oya_cloud_os_init_app::config::machine_config_volume_manager(raw_config)?;
    ...
    let resource = VolumeStatusResource::new(VolumeStatus::new(volume.config.clone()))
        .map_err(oya_cloud_os_kernel::Error::from)?;
    ...
    state.create(Box::new(resource)).map_err(|err| {
        oya_cloud_os_kernel::Error::invalid_state(format!(
...
pub fn hydrate_bootstrap_image_cache_mount_state(
    raw_config: Option<&str>,
    state: &mut State,
) -> oya_cloud_os_kernel::Result<usize> {
...
fn upsert_image_cache_bootstrap_volume_status(
    state: &mut State,
    volume_id: &str,
) -> oya_cloud_os_kernel::Result<()> {
    let mut status = VolumeStatus::new(VolumeConfig::partition(volume_id, volume_id, 0));
    status.phase = VolumePhase::Ready;
    let mut desired = VolumeStatusResource::new(status).map_err(oya_cloud_os_kernel::Error::from)?;
...
/// answers `/healthz`.
pub fn pid1_registryd_launch_result(id: &str) -> oya_cloud_os_kernel::Result<bool> {
...
pub fn pid1_registryd_launch_result_at(
    ...
    if id != REGISTRYD_SERVICE_ID {
        return Err(oya_cloud_os_kernel::Error::unsupported(format!(
...
pub fn pid1_registryd_request_result(
    ...
    method: &str,
    target: &str,
) -> oya_cloud_os_kernel::Result<Option<RegistrydContentResponse>> {
    if id != REGISTRYD_SERVICE_ID {
        return Err(oya_cloud_os_kernel::Error::unsupported(format!(
            "unsupported registryd service id {id}"
        )));
...
pub fn pid1_registryd_runtime_service_snapshot(
    supervisor: &Supervisor,
    id: &str,
) -> oya_cloud_os_kernel::Result<Option<RegistrydRuntimeService>> {
    if id != REGISTRYD_SERVICE_ID {
        return Err(oya_cloud_os_kernel::Error::unsupported(format!(
            "unsupported registryd service id {id}"
        )));
...
pub fn pid1_registryd_response_bytes(
    ...
    method: &str,
    target: &str,
) -> oya_cloud_os_kernel::Result<Option<Vec<u8>>> {
    pid1_registryd_response_bytes_for_request_headers(
        supervisor,
        id,
        method,
        target,
        oya_cloud_os_runtime_cri_domain::RegistrydSourceRequestHeaders::default(),
    )
}
...
pub fn pid1_registryd_response_bytes_for_range(
    ...
    pid1_registryd_response_bytes_for_request_headers(
        ...
        method,
        target,
        oya_cloud_os_runtime_cri_domain::RegistrydSourceRequestHeaders {
            range: range_header,
            ..oya_cloud_os_runtime_cri_domain::RegistrydSourceRequestHeaders::default()
        },
    )
...
pub fn pid1_registryd_response_bytes_for_request_headers(
    ...
    method: &str,
    target: &str,
    headers: oya_cloud_os_runtime_cri_domain::RegistrydSourceRequestHeaders<'_>,
) -> oya_cloud_os_kernel::Result<Option<Vec<u8>>> {
    Ok(
...
fn registryd_read_http_request<S>(stream: &mut S) -> oya_cloud_os_kernel::Result<Vec<u8>>
where
    S: Read,
{
    ...
    loop {
        let read = stream
            .read(&mut buffer)
            .map_err(|err| oya_cloud_os_kernel::Error::Other(format!("registryd request read: {err}")))?;
        if read == 0 {
            break;
        }
        ...
        if request.len() > 8192 {
            return Err(oya_cloud_os_kernel::Error::invalid(
    ...
    S: Read + Write,
{
    ...
    stream
        .write_all(&response)
        .map_err(|err| oya_cloud_os_kernel::Error::Other(format!("registryd response write: {err}")))?;
    stream
        .flush()
        .map_err(|err| oya_cloud_os_kernel::Error::Other(format!("registryd response flush: {err}")))?;
    Ok(true)
}
...
fn pid1_registryd_runtime_service_accept_http_once_with_stop(
    ...
    listener
        ...
        .map_err(|err| oya_cloud_os_kernel::Error::Other(format!("registryd listener configure: {err}")))?;
    ...
    let result = loop {
        ...
        match listener.accept() {
            ...
            Err(err) => {
                break Err(oya_cloud_os_kernel::Error::Other(format!(
...
pub struct Pid1RegistrydRuntimeServer {
    ...
    join: Option<std::thread::JoinHandle<oya_cloud_os_kernel::Result<usize>>>,
}

impl Pid1RegistrydRuntimeServer {
    /// Ask the registryd server loop to stop and return the number of served requests.
    pub fn stop(mut self) -> oya_cloud_os_kernel::Result<usize> {
        ...
        join.join()
            .map_err(|_| oya_cloud_os_kernel::Error::Other("registryd server thread panicked".into()))?
    }
}
...
pub fn pid1_registryd_runtime_service_spawn_http_server_at(
    ...
    listener: std::net::TcpListener,
    timeout: Duration,
) -> oya_cloud_os_kernel::Result<Pid1RegistrydRuntimeServer> {
    if id != REGISTRYD_SERVICE_ID {
        return Err(oya_cloud_os_kernel::Error::unsupported(format!(
            "unsupported registryd service id {id}"
        )));
    ...
    let wake_address = listener.local_addr().map_err(|err| {
        oya_cloud_os_kernel::Error::Other(format!("registryd listener local address: {err}"))
    })?;
...
pub fn pid1_registryd_runtime_service_launch_result_at(
    ...
    health_address: &str,
    timeout: Duration,
) -> oya_cloud_os_kernel::Result<(bool, Pid1RegistrydRuntimeServer)> {
...
impl Pid1RegistrydRuntimeServiceLauncher {
    ...
    pub fn bind_at(listen_address: &str, timeout: Duration) -> oya_cloud_os_kernel::Result<Self> {
        let listener = std::net::TcpListener::bind(listen_address).map_err(|err| {
            oya_cloud_os_kernel::Error::Other(format!("registryd listen {listen_address}: {err}"))
        })?;
        let health_address = listener.local_addr().map_err(|err| {
            oya_cloud_os_kernel::Error::Other(format!("registryd local address {listen_address}: {err}"))
        })?;
    ...
    pub fn stop_registryd(mut self) -> oya_cloud_os_kernel::Result<usize> {
...
fn registryd_http_request_method_target(request: &[u8]) -> oya_cloud_os_kernel::Result<(&str, &str)> {
    let line_end = request
        .windows(2)
        .position(|window| window == b"\r\n")
        .or_else(|| request.iter().position(|byte| *byte == b'\n'))
        .ok_or_else(|| oya_cloud_os_kernel::Error::parse("registryd HTTP request missing request line"))?;
    let line = &request[..line_end];
    let line = std::str::from_utf8(line).map_err(|err| {
        oya_cloud_os_kernel::Error::parse(format!("registryd HTTP request line is not UTF-8: {err}"))
    })?;

    let mut parts = line.split_whitespace();
    let method = parts
        .next()
        .ok_or_else(|| oya_cloud_os_kernel::Error::parse("registryd HTTP request missing method"))?;
    let target = parts
        .next()
        .ok_or_else(|| oya_cloud_os_kernel::Error::parse("registryd HTTP request missing target"))?;
    let version = parts
        .next()
        .ok_or_else(|| oya_cloud_os_kernel::Error::parse("registryd HTTP request missing version"))?;
    if parts.next().is_some() || !version.starts_with("HTTP/") {
        return Err(oya_cloud_os_kernel::Error::parse(format!(
...
pub fn registryd_http_health_status_at(
    ...
    path: &str,
    timeout: Duration,
) -> oya_cloud_os_kernel::Result<Option<u16>> {
    let socket_addr: SocketAddr = address.parse().map_err(|err| {
        oya_cloud_os_kernel::Error::parse(format!("registryd health address {address}: {err}"))
    })?;
...
mod init {
    ...
    use oya_cloud_os_controllers_domain::network::LinkStatusResource;
    use oya_cloud_os_controllers_domain::{Domain, MachineConfigDocument, RegistryBuilder};
    use oya_cloud_os_cosi_domain::State;
    use oya_cloud_os_init_app::config::{StaticAddress, try_early_config};
    use oya_cloud_os_init_app::platform_config::{
        FileConfigStore, pre_config_network_bootstrap, resolve_config,
    };
    use oya_cloud_os_init_app::{DEFAULT_HOSTNAME, MACHINE_CONFIG_PATH, PROC_CMDLINE_PATH};

    use oya_cloud_os_kernel::MachineType;
    use oya_cloud_os_machined_domain::boot::{
    ...
    use oya_cloud_os_machined_domain::error::{MachinedError, Result as MResult};
    use oya_cloud_os_machined_domain::{ServiceLauncher, Supervisor};
    use oya_cloud_os_runtime_cri_domain::{
    ...
    impl LibcRuntime {
        ...
        fn configure_eth0_address(&mut self) {
            ...
            match oya_cloud_os_network_domain::linux_net::add_ipv4("eth0", &addr.addr, addr.prefix) {
            ...
            match oya_cloud_os_network_domain::linux_net::query_addrs("eth0") {
                Ok(addrs) => {
            ...
            match oya_cloud_os_network_domain::linux_net::get_operstate("eth0") {
        ...
        fn bootstrap_config_network(
            &mut self,
            bootstrap: &oya_cloud_os_platform_domain::aws::BootstrapNetworkConfig,
        ) {
            ...
            match oya_cloud_os_network_domain::linux_net::set_link_up(iface) {
        ...
        fn bootstrap_config_network_dhcp4(
            ...
            let config = oya_cloud_os_network_domain::Dhcp4ClientConfig::new(xid, mac);
            let mut transaction =
                oya_cloud_os_network_domain::Dhcp4WireTransaction::new(config).with_retry_schedule(2, 3);
                ...
                loop {
                    match socket.recv_from(&mut buf) {
                        Ok((n, _peer)) => match transaction.handle_packet(&buf[..n]) {
                            Ok(oya_cloud_os_network_domain::Dhcp4WireAction::Send(next)) => {
                                outbound = next;
                                break;
                            }
                            Ok(oya_cloud_os_network_domain::Dhcp4WireAction::Bound(lease)) => {
                                println!(
                            ...
                            Ok(oya_cloud_os_network_domain::Dhcp4WireAction::Ignored(reason)) => {
                            ...
                            ) =>
                        {
                            match transaction.handle_timeout() {
                                ...
                                Err(oya_cloud_os_kernel::Error::Timeout) => {
                                    println!(
        ...
        fn apply_pre_config_dhcp4_lease(
            &self,
            bootstrap: &oya_cloud_os_platform_domain::aws::BootstrapNetworkConfig,
            lease: &oya_cloud_os_network_domain::Dhcp4Lease,
        ) {
            let iface = bootstrap.interface;
            let mut op = oya_cloud_os_network_domain::OperatorSpec::dhcp4(iface);
            ...
            let oya_cloud_os_network_domain::OperatorOutput {
                addresses,
                routes,
                resolver,
                ..
            ...
            for address in addresses {
                ...
                match oya_cloud_os_network_domain::linux_net::add_ipv4(
                    iface,
            ...
            for route in routes {
                let destination = route
                    .destination
                    .map(|addr| oya_cloud_os_network_domain::linux_net::parse_ipv4(&addr.to_string()))
                    .transpose();
                let gateway = route
                    .gateway
                    .map(|addr| oya_cloud_os_network_domain::linux_net::parse_ipv4(&addr.to_string()))
                ...
                match oya_cloud_os_network_domain::linux_net::add_ipv4_route(
                    iface,
                    destination,
                    route.prefix_len,
                    gateway,
        ...
        fn bootstrap_config_network_dhcp6(
            ...
            let salt = time_mix();
            let config = oya_cloud_os_network_domain::Dhcp6ClientConfig::new(
                dhcp6_transaction_id(mac, salt),
                dhcp6_iaid_from_mac(mac),
                oya_cloud_os_network_domain::Dhcp6ClientIdentifier::DuidLlt {
                    mac,
                    seconds_since_2000: dhcp6_duid_llt_time(),
                },
            )
            .with_request_transaction_id(dhcp6_transaction_id(mac, salt ^ 0x0052_4551));
            let mut transaction = oya_cloud_os_network_domain::Dhcp6WireTransaction::new(config);
            let mut outbound = match transaction.start() {
                Ok(outbound) => outbound,
                Err(e) => {
            ...
            loop {
                ...
                loop {
                    match socket.recv_from(&mut buf) {
                        Ok((n, _peer)) => match transaction.handle_packet(&buf[..n]) {
                            Ok(oya_cloud_os_network_domain::Dhcp6WireAction::Send(next)) => {
                                outbound = next;
                                attempts = 1;
                                timeout = Duration::from_secs(5);
                                break;
                            }
                            Ok(oya_cloud_os_network_domain::Dhcp6WireAction::Bound(lease)) => {
                            ...
                            Ok(oya_cloud_os_network_domain::Dhcp6WireAction::Ignored(reason)) => {
        ...
        fn apply_pre_config_dhcp6_lease(
            &self,
            bootstrap: &oya_cloud_os_platform_domain::aws::BootstrapNetworkConfig,
            lease: &oya_cloud_os_network_domain::Dhcp6Lease,
        ) {
            let iface = bootstrap.interface;
            let output =
                match oya_cloud_os_network_domain::OperatorSpec::dhcp6(iface).apply_dhcp6_lease(lease, false) {
                    Ok(output) => output,
                    Err(e) => {
            ...
            for address in addresses {
                match address.address {
                    oya_cloud_os_kernel::address::NodeAddress::V6(_) => {
                        println!(
                            "[net] {iface}: DHCPv6 assigning {}/{}",
                            address.address, address.prefix_len
                        );
                        match oya_cloud_os_network_domain::linux_net::add_ipv6(
                            iface,
                            &address.address.to_string(),
                            address.prefix_len,
                        ) {
                    ...
                    oya_cloud_os_kernel::address::NodeAddress::V4(_) => {
        ...
        fn write_pre_config_resolver(
            ...
            iface: &str,
            family: &str,
            resolver: &oya_cloud_os_network_domain::ResolverSpec,
        ) {
    ...
    impl Runtime for LibcRuntime {
        ...
        fn load_config(&mut self) -> MResult<String> {
            ...
            let cmdline = oya_cloud_os_init_app::cmdline::CmdLine::parse(&cmdline_raw);
    ...
    fn send_dhcp4_packet(
        socket: &UdpSocket,
        outbound: &oya_cloud_os_network_domain::Dhcp4Outbound,
    ) -> std::io::Result<()> {
        let target: SocketAddr = match outbound.target {
            oya_cloud_os_network_domain::Dhcp4SendTarget::Broadcast => "255.255.255.255:67".parse().unwrap(),
            oya_cloud_os_network_domain::Dhcp4SendTarget::Unicast(addr) => {
    ...
    fn send_dhcp6_packet(
        socket: &UdpSocket,
        ifindex: u32,
        outbound: &oya_cloud_os_network_domain::Dhcp6Outbound,
    ) -> std::io::Result<()> {
        let target = match outbound.target {
            oya_cloud_os_network_domain::Dhcp6SendTarget::AllDhcpRelayAgentsAndServers => {
...
mod tests {
    ...
    fn registryd_test_last_modified(path: &std::path::Path) -> String {
        oya_cloud_os_runtime_cri_domain::registryd_http_last_modified_value(
    ...
    fn pid1_registryd_launcher_accepts_only_source_registryd_without_claiming_health() {
        assert!(!pid1_registryd_launch_result(oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID).unwrap());
    ...
    fn pid1_registryd_launcher_uses_loopback_health_probe_when_endpoint_answers() {
        ...
        assert!(
            pid1_registryd_launch_result_at(
                oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID,
                &address,
                oya_cloud_os_runtime_cri_domain::REGISTRYD_HEALTH_PATH,
...
fe8000000000000002005efffe102031 03 40 20 08 eth1\n";
    ...
    fn pre_config_resolver_materializes_resolv_conf_body() {
        let resolver = oya_cloud_os_network_domain::ResolverSpec::new_with_search(
            vec![
                oya_cloud_os_kernel::address::NodeAddress::parse_v4("8.8.8.8").unwrap(),
                oya_cloud_os_kernel::address::NodeAddress::parse_v4("8.8.4.4").unwrap(),
            ],
            vec!["corp.example.com".to_string(), "example.com".to_string()],
            oya_cloud_os_network_domain::ConfigLayer::Operator,
        )
        .unwrap();
...
";
    ...
    fn image_cache_runtime_adapters_retain_loaded_registryd_service_on_boot_supervisor() {
        ...
        impl oya_cloud_os_machined_domain::ServiceLauncher for HealthyRegistrydLauncher {
            fn launch(&mut self, id: &str) -> oya_cloud_os_machined_domain::error::Result<bool> {
                assert_eq!(id, oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID);
                Ok(true)
            }
        }
        ...
        let mut state = State::new();
        let plan = ImageCacheRuntimePlan {
            config: oya_cloud_os_runtime_cri_domain::ImageCacheConfig {
                status: oya_cloud_os_runtime_cri_domain::ImageCacheStatus::Preparing,
                copy_status: oya_cloud_os_runtime_cri_domain::ImageCacheCopyStatus::Skipped,
                roots: vec![root.display().to_string()],
            },
            registryd_action: oya_cloud_os_runtime_cri_domain::RegistrydAction::Start,
            ..ImageCacheRuntimePlan::default()
        };
        let mut supervisor = oya_cloud_os_machined_domain::Supervisor::new(oya_cloud_os_kernel::MachineType::Worker);
        ...
        assert_eq!(
            ...
            oya_cloud_os_runtime_cri_domain::RegistrydServiceExecutionStatus::LoadedAndStarted
        );
        assert!(outcome.registryd_state.running);
        assert!(outcome.registryd_state.healthy);
        assert_eq!(
            outcome.observed_plan.registryd_action,
            oya_cloud_os_runtime_cri_domain::RegistrydAction::None
        );
    ...
    fn image_cache_copy_ready_plan(
        ...
        ImageCacheRuntimePlan {
            config: oya_cloud_os_runtime_cri_domain::ImageCacheConfig {
                status: oya_cloud_os_runtime_cri_domain::ImageCacheStatus::Preparing,
                copy_status: oya_cloud_os_runtime_cri_domain::ImageCacheCopyStatus::Ready,
                roots: vec![target.display().to_string()],
            },
            copy_plan: Some(oya_cloud_os_runtime_cri_domain::ImageCacheCopyPlan {
                source: source.display().to_string(),
                target: target.display().to_string(),
            }),
            registryd_action: oya_cloud_os_runtime_cri_domain::RegistrydAction::None,
            ..ImageCacheRuntimePlan::default()
        }
    }
    ...
    impl oya_cloud_os_machined_domain::ServiceLauncher for NoopRegistrydLauncher {
        fn launch(&mut self, id: &str) -> oya_cloud_os_machined_domain::error::Result<bool> {
    ...
    fn image_cache_copy_adapter_for_boot_privilege_maps_host_and_vm_gates() {
        ...
        assert_eq!(
            host.environment(),
            oya_cloud_os_runtime_cri_domain::ImageCacheCopyRuntimeEnvironment::HostSafe
        );
        assert_eq!(host.gate(), oya_cloud_os_runtime_cri_domain::ImageCacheCopyGate::Disabled);

        let vm = image_cache_copy_adapter_for_boot_privilege(true);
        assert_eq!(
            vm.environment(),
            oya_cloud_os_runtime_cri_domain::ImageCacheCopyRuntimeEnvironment::VmPrivileged
        );
        assert_eq!(vm.gate(), oya_cloud_os_runtime_cri_domain::ImageCacheCopyGate::Enabled);
    }
    ...
    fn image_cache_runtime_adapters_leave_copy_effect_host_safe_by_default() {
        ...
        assert_eq!(
            ...
            oya_cloud_os_runtime_cri_domain::ImageCacheCopyExecutionStatus::DisabledByEnvironment
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            oya_cloud_os_runtime_cri_domain::ImageCacheCopyStatus::Ready
        );
    ...
    fn image_cache_runtime_adapters_execute_copy_effect_for_vm_adapter_before_registryd() {
        ...
        assert_eq!(
            outcome.copy_report.status,
            oya_cloud_os_runtime_cri_domain::ImageCacheCopyExecutionStatus::Copied
        );
        assert_eq!(
            image_cache_copy_execution_status_label(outcome.copy_report.status),
            "copied"
...
";
        let mut state = State::new();
        let mut disk_status = VolumeStatus::new(VolumeConfig::partition(
            oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_DISK_VOLUME_ID,
            oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_DISK_VOLUME_ID,
            0,
        ));
        disk_status.phase = VolumePhase::Ready;
        let mut iso_status = VolumeStatus::new(VolumeConfig::partition(
            oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_ISO_VOLUME_ID,
            oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_ISO_VOLUME_ID,
            0,
        ));
        ...
        state
            .create(Box::new(
                VolumeMountStatusResource::new(
                    image_cache_mount_status_id(oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_DISK_VOLUME_ID),
                    VolumeMountStatusSpec::new(
                        oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_DISK_VOLUME_ID,
                        oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_CONTROLLER_NAME,
        ...
        state
            .create(Box::new(
                VolumeMountStatusResource::new(
                    image_cache_mount_status_id(oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_ISO_VOLUME_ID),
                    VolumeMountStatusSpec::new(
                        oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_ISO_VOLUME_ID,
                        oya_cloud_os_runtime_cri_domain::IMAGE_CACHE_CONTROLLER_NAME,
    ...
    fn pid1_registryd_request_bridge_serves_retained_supervisor_runtime_service() {
        ...
        let unloaded = oya_cloud_os_machined_domain::Supervisor::new(oya_cloud_os_kernel::MachineType::Worker);
        assert_eq!(
    ...
    fn pid1_registryd_http_once_stream_preserves_eot_blob_content_type() {
        ...
        let request = format!(
            ...
            oya_cloud_os_runtime_cri_domain::REGISTRYD_LISTEN_ADDRESS
        );
    ...
    fn pid1_registryd_runtime_service_snapshot_serves_boot_retained_payload() {
        ...
        let empty_supervisor = oya_cloud_os_machined_domain::Supervisor::new(oya_cloud_os_kernel::MachineType::Worker);
        assert!(
    ...
    fn pid1_registryd_runtime_service_http_once_stream_serves_cloned_runtime_payload() {
        ...
        let service = oya_cloud_os_runtime_cri_domain::RegistrydRuntimeService::from_runtime_plan(&plan);
        ...
        let empty_service =
            oya_cloud_os_runtime_cri_domain::RegistrydRuntimeService::from_runtime_plan(&empty_plan);
        let mut stream = MemoryStream::new(
            format!(
                "GET {} HTTP/1.1\r\nHost: {}\r\n\r\n",
                oya_cloud_os_runtime_cri_domain::REGISTRYD_HEALTH_PATH,
    ...
    fn pid1_registryd_runtime_service_serve_http_bounded_processes_loopback_requests() {
        ...
        let service = oya_cloud_os_runtime_cri_domain::RegistrydRuntimeService::from_runtime_plan(&plan).clone();
    ...
    fn pid1_registryd_runtime_service_launcher_bind_at_serves_bound_payload() {
        ...
        assert!(
            launcher
                .launch_registryd_runtime_service(oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID, &service)
                .unwrap()
        );
    ...
    fn pid1_registryd_runtime_service_launch_result_preserves_if_range_over_loopback() {
        ...
        let blob_last_modified =
            oya_cloud_os_runtime_cri_domain::registryd_http_last_modified_value(source_time).unwrap();
    ...
    fn pid1_registryd_runtime_service_launcher_stop_tears_down_bound_server() {
        ...
        oya_cloud_os_machined_domain::ServiceLauncher::stop(
            &mut launcher,
            oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID,
        )
        .unwrap();
    ...
    fn pid1_registryd_runtime_service_launcher_drives_runtime_adapters_to_loopback_payload() {
        ...
        assert_eq!(
            ...
            oya_cloud_os_runtime_cri_domain::ImageCacheStatus::Ready
        );
        assert_eq!(
            outcome.observed_plan.registryd_action,
            oya_cloud_os_runtime_cri_domain::RegistrydAction::None
    ...
    fn pid1_registryd_runtime_service_launcher_stop_all_tears_down_adapter_server() {
        ...
        assert!(outcome.registryd_state.healthy);
        assert_eq!(
            supervisor.state_of(oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID),
            Some(oya_cloud_os_machined_domain::ServiceState::Healthy)
        );
        ...
        assert_eq!(
            supervisor.state_of(oya_cloud_os_runtime_cri_domain::REGISTRYD_SERVICE_ID),
            Some(oya_cloud_os_machined_domain::ServiceState::Finished)
        );
        assert_eq!(
    ...
    fn image_cache_runtime_plan_projection_stays_disabled_without_feature_flag() {
        use oya_cloud_os_block_domain::{VolumeConfig, VolumePhase, VolumeStatus};
        use oya_cloud_os_runtime_cri_domain::{
...
";
    ...
    fn image_cache_declared_volume_status_hydrates_boot_cosi_state_without_starting_registryd() {
        use oya_cloud_os_block_domain::{IMAGE_CACHE_VOLUME_ID, VolumePhase, VolumeStatusResource};
        use oya_cloud_os_cosi_domain::State;
        use oya_cloud_os_runtime_cri_domain::{
...
";
    ...
    fn image_cache_bootstrap_mount_state_seeds_ready_roots_when_canonical_paths_exist() {
        use oya_cloud_os_block_domain::{IMAGE_CACHE_VOLUME_ID, VolumeMountStatusResource, VolumeStatusResource};
        use oya_cloud_os_cosi_domain::State;
        use oya_cloud_os_runtime_cri_domain::{
...
";
        ...
        assert_eq!(disk.status.phase, oya_cloud_os_block_domain::VolumePhase::Ready);
    ...
    fn image_cache_bootstrap_mount_state_does_not_synthesize_missing_roots() {
        use oya_cloud_os_cosi_domain::State;
        use oya_cloud_os_runtime_cri_domain::{ImageCacheStatus, RegistrydAction, RegistrydState};
