...
use oya_cloud_os_machine_config_domain::{
...
use oya_cloud_os_network_domain::{
...
pub fn try_early_config(contents: &str) -> oya_cloud_os_kernel::Result<EarlyConfig> {
    let body = v1alpha1_body(contents);
    Ok(EarlyConfig {
        hostname: extract_hostname(&body),
        machine_type: machine_scalar(&body, "type").map(|s| MachineType::parse(&s)),
...
pub fn machine_config_dhcp_operators(contents: &str) -> oya_cloud_os_kernel::Result<Vec<OperatorSpec>> {
    ...
    for config in dhcpv4_configs(&container)? {
        ...
        op.route_metric = config.route_metric_or(oya_cloud_os_network_domain::DEFAULT_ROUTE_METRIC);
...
pub fn machine_config_resolver_specs(contents: &str) -> oya_cloud_os_kernel::Result<Vec<ResolverSpec>> {
    if contents.trim().is_empty() {
        return Ok(Vec::new());
    }
    ...
    for ns in &config.nameservers {
        ...
        servers.push(oya_cloud_os_kernel::NodeAddress::parse(&ns.address)?);
    }
...
pub fn machine_config_volume_manager(contents: &str) -> oya_cloud_os_kernel::Result<VolumeManager> {
    if contents.trim().is_empty() {
...
pub fn machine_config_image_cache_local_enabled(contents: &str) -> oya_cloud_os_kernel::Result<bool> {
    if contents.trim().is_empty() {
        return Ok(false);
    }

    let body = v1alpha1_body(contents);
    let doc = oya_cloud_os_machine_config_domain::yaml::parse(&body).map_err(|err| {
        oya_cloud_os_kernel::Error::parse(format!("machine.features.imageCache.localEnabled: {err}"))
    })?;
    ...
    local_enabled.as_bool().ok_or_else(|| {
        oya_cloud_os_kernel::Error::invalid("machine.features.imageCache.localEnabled must be boolean")
    })
}

fn project_system_volume_config(
    doc: &VolumeConfigDoc,
) -> oya_cloud_os_kernel::Result<Option<BlockVolumeConfig>> {
    ...
    match doc.name.as_str() {
        ...
        "STATE" => Err(oya_cloud_os_kernel::Error::invalid(
        ...
        other => Err(oya_cloud_os_kernel::Error::invalid(format!(
            "unknown system VolumeConfig {other:?}"
        ))),
    }
}

fn project_user_volume_config(doc: &UserVolumeConfigDoc) -> oya_cloud_os_kernel::Result<BlockVolumeConfig> {
    ...
    match doc.volume_type {
        ...
        UserVolumeType::Disk => {
            let selector = doc.provisioning.disk_selector.clone().ok_or_else(|| {
                oya_cloud_os_kernel::Error::invalid(format!(
...
fn project_raw_volume_config(doc: &RawVolumeConfigDoc) -> oya_cloud_os_kernel::Result<BlockVolumeConfig> {
    if doc.encryption_configured {
        return Err(oya_cloud_os_kernel::Error::unsupported(format!(
...
fn push_dhcp_operator_specs(
    ...
    link_name: &str,
    options: &oya_cloud_os_machine_config_domain::DhcpOptions,
) -> oya_cloud_os_kernel::Result<()> {
    if options.ipv4() {
        let mut op = OperatorSpec::dhcp4(link_name);
        op.route_metric = options.route_metric_or(oya_cloud_os_network_domain::DEFAULT_ROUTE_METRIC);
        out.push(op);
    }
...
fn hex_bytes(s: &str) -> oya_cloud_os_kernel::Result<Vec<u8>> {
    ...
    for idx in (0..s.len()).step_by(2) {
        let byte = u8::from_str_radix(&s[idx..idx + 2], 16)
            .map_err(|_| oya_cloud_os_kernel::Error::invalid("duidv6 must be hexadecimal"))?;
        out.push(byte);
    }
    Ok(out)
}
...
fn first_iface_address_raw(body: &str) -> Option<String> {
    let doc = oya_cloud_os_machine_config_domain::yaml::parse(body).ok()?;
    let first = doc
        .get("machine")?
        .get("network")?
        .get("interfaces")?
...
";
        ...
        assert_eq!(operators[0].kind, oya_cloud_os_network_domain::OperatorKind::Dhcp6);
        ...
        assert_eq!(operators[1].kind, oya_cloud_os_network_domain::OperatorKind::Dhcp4);
        assert_eq!(operators[1].link_name, "eth1");
        assert_eq!(
            operators[1].route_metric,
            oya_cloud_os_network_domain::DEFAULT_ROUTE_METRIC
        );
...
";
        ...
        assert_eq!(operators[0].kind, oya_cloud_os_network_domain::OperatorKind::Dhcp4);
        assert_eq!(
...
";
        ...
        assert_eq!(ephemeral.class, oya_cloud_os_block_domain::VolumeClass::System);
        ...
        assert_eq!(user.class, oya_cloud_os_block_domain::VolumeClass::User);
...
";
        ...
        assert_eq!(user.class, oya_cloud_os_block_domain::VolumeClass::User);
        assert_eq!(user.priority, 100);
        assert_eq!(user.config.volume_type, oya_cloud_os_block_domain::VolumeType::Disk);
...
";
        ...
        assert_eq!(raw.class, oya_cloud_os_block_domain::VolumeClass::User);
        assert_eq!(raw.priority, 100);
        assert_eq!(raw.config.volume_type, oya_cloud_os_block_domain::VolumeType::Partition);
        ...
        assert_eq!(
            ...
            oya_cloud_os_block_domain::PartitionMatchPolicy::FirstMatch
        );
    }
...
";
        ...
        assert_eq!(image_cache.class, oya_cloud_os_block_domain::VolumeClass::System);
