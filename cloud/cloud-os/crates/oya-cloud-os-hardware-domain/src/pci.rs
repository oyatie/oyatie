...
impl From<PciError> for oya_cloud_os_kernel::Error {
    fn from(e: PciError) -> Self {
        match e {
            PciError::InvalidAddress(s) => oya_cloud_os_kernel::Error::parse(s),
            PciError::ScanFailed(s) => oya_cloud_os_kernel::Error::Other(s),
        }
    }
}
