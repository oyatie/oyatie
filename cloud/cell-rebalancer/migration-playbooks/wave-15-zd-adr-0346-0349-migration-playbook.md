# cell-rebalancer Wave 15-ZD active substrate migration playbook

Audience: an Oyatie operator or migration owner preparing `cell-rebalancer` for parallel lane work without reviving retired local CLI, Jenkins, ArgoCD, or first-party Helm authority.
...
2. Keep cell-rebalancer edits inside `cloud/cell-rebalancer/**`, the Rust hygiene guard/test, and lane-specific evidence.
...
- Rust hygiene test rejects retired Cargo/local-CLI/Jenkins/ArgoCD/Helm authority for `cell-rebalancer`.
- Targeted scans under `cloud/cell-rebalancer` show no active retired authority phrases.
- `cloud/cell-rebalancer/iac/k8s/helm` is absent.
- OpenAPI and manifest artifacts parse.
