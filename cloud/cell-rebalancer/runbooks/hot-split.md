---
doc_class: Runbook
doc_id: RUNBOOK-CELL_REBALANCER-HOT_SPLIT
...
# Runbook: Cell Rebalancer Hot Split

## Operator Contract
- Runbook id: cell-rebalancer-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-rebalancer-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-rebalancer; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-rebalancer rollout status deploy/cell-rebalancer --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-rebalancer --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cell-rebalancer --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cell-rebalancer --cell $CELL --window 30m`.
