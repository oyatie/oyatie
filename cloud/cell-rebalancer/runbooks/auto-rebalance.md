---
doc_class: Runbook
doc_id: RUNBOOK-CELL_REBALANCER-AUTO_REBALANCE
...
# Runbook: Cell Rebalancer Auto Rebalance

## Operator Contract
- Runbook id: cell-rebalancer-auto-rebalance.
- Primary service namespace: `cell-rebalancer`.
...
- Owning team: axis-platform-reliability + axis-tenancy + axis-governance.
...
## Doctrine Anchors
- ADR-0346 purpose wording: historical local verifier doctrine only; active evidence is Buck2 output plus oya-ci-required.
...
- ADR-0349 purpose wording: historical Jenkins/ArgoCD provenance only; active CI/CD authority is ADR-0513 oya-ci/Prow plus native promotion seams.
- ADR-0349 active replacement lanes: `oya-governance-buck2-prow-required-context`; `oya-governance-native-promotion-cosign-verified`; `oya-governance-native-promotion-tenant-namespace-isolation`; `oya-governance-native-promotion-audit-chain-emit`.
...
- Trigger 7: Buck2/Prow required-context drift blocks the release train for the sharding automation lane.
- Trigger 8: native promotion reports a pending reconcile tied to this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-rebalancer-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-rebalancer; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-rebalancer rollout status deploy/cell-rebalancer --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-rebalancer --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cell-rebalancer --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cell-rebalancer --cell $CELL --window 30m`.
...
9. Verify native promotion reconcile health through the Prow-backed promotion evidence for `$SERVICE`.
10. Verify Buck2/Prow required-context evidence exists for this service before declaring the runbook complete.
...
5. If native reconcile is pending, pause execution until promotion evidence confirms the service declaration is current.
6. If Buck2/Prow evidence is unknown, keep the change in report-only state and collect Buck2 target evidence before push.
...
9. Keep native promotion in reconcile-only mode; do not run manual `kubectl apply` or manual Helm CLI deploys.
...
- Evidence 6: native promotion run id and cosign verification policy result.
- Evidence 7: Prow job id plus temporary GitHub Actions run id proving adapter parity for this service.
- Evidence 8: Buck2 target output plus oya-ci-required context before any push related to this runbook.
...
6. Re-run native promotion reconciliation and confirm no manual drift remains.
...
6. Confirm Buck2/Prow evidence is attached per ADR-0513.
7. Confirm native promotion did not accept unsigned images and did not cross tenant namespaces.
