---
doc_class: Runbook
doc_id: RUNBOOK-CELL_REBALANCER-COLD_MERGE
...
# Runbook: Cell Rebalancer Cold Merge

## Operator Contract
- Runbook id: cell-rebalancer-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cell-rebalancer-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cell-rebalancer; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cell-rebalancer rollout status deploy/cell-rebalancer --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cell-rebalancer --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cell-rebalancer --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cell-rebalancer --cell $CELL --window 30m`.
