...
use oya_tenancy_tenant_lifecycle_domain::DesiredTenantState;
use oya_tenancy_tenant_lifecycle_kernel::{
    AppliedWriteRecord, OperationRecord, StoreError, TenantLifecycleStore,
};
use oya_tenancy_tenant_lifecycle_usecase::TenantLifecycleProvider;
use oya_tenancy_tenant_lifecycle_usecase::reconcile::{
...
fn retired_id_is_never_reused_end_to_end() {
    ...
    let key = oya_tenancy_tenant_lifecycle_domain::derive_step_key("other", 9, "create").unwrap();
    assert!(matches!(
        provider.create(
            &name(),
            Tenant {
