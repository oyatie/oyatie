---
doc_class: Runbook
doc_id: RUNBOOK-TENANCY-AUTO_REBALANCE
...
# Runbook: Tenancy Auto Rebalance

## Operator Contract
- Runbook id: tenancy-auto-rebalance.
...
- Safety invariant: never bypass residency, compliance pack, cosign, controller-state, GitOps, or audit-chain controls to speed up mitigation.
- Safety invariant: prefer refusal with evidence over a partially observed automation event.

## Doctrine Anchors
- ADR-0346 purpose wording: ADR-0346 is historical local-verifier doctrine; active evidence is Buck2 target output plus trusted Rust/Prow `oya-ci-required`.
- ADR-0346 enforced_by lanes: `buck2-prow-required-matrix-coverage`; `buck2-prow-required-step-exit-semantics`; `buck2-prow-required-skip-policy`; `trusted-pr-submission-requires-oya-ci-required`; `buck2-prow-required-exit-code-contract`.
...
- ADR-0349 purpose wording: native oya-ci and native release conveyor are the two canonical self-hostable CI/CD substrates for the Oyatie corpus.
- ADR-0349 enforced_by lanes: `oya-governance-github-shadow-parity`; `oya-governance-native-release-conveyor-application-cosign-verified`; `oya-governance-native-release-conveyor-tenant-namespace-isolation`; `oya-governance-oci-required-controller-state`; `oya-governance-deploy-audit-chain-emit`.
...
- Trigger 7: oya-ci-required or temporary GitHub Actions shadow parity drift blocks the release train for the sharding automation lane.
- Trigger 8: native release conveyor reports a pending sync tied to this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tenancy-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tenancy; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tenancy rollout status deploy/tenancy --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tenancy --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service tenancy --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service tenancy --cell $CELL --window 30m`.
...
9. Verify native release-conveyor reconciliation health: `native-release-conveyor app get $SERVICE --refresh`.
10. Verify oya-ci-required plus temporary GitHub Actions shadow parity evidence exists for this service before declaring the runbook complete.
...
5. If GitOps sync is pending, pause execution until native release conveyor confirms the service declaration is current.
6. If oya-ci-required plus temporary GitHub Actions shadow parity is unknown, keep the change in report-only state and run local `oya-ci-required` Prow context before push.
...
9. Keep native release conveyor in sync-only mode; do not run manual `kubectl apply` or manual package CLI deploys.
...
- Evidence 6: native release-conveyor package sync id and cosign verification policy result.
- Evidence 7: Prow run id plus optional GitHub Actions shadow run id proving CI/CD parity for this service.
- Evidence 8: `oya-ci-required` Prow context local mirror result before any push related to this runbook.
...
6. Re-run native release-conveyor refresh and confirm no manual drift remains.
...
6. Confirm native oya-ci and GitHub Actions parity evidence is attached per ADR-0349.
7. Confirm native release conveyor did not sync unsigned images and did not cross tenant namespaces.
