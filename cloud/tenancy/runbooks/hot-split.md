---
doc_class: Runbook
doc_id: RUNBOOK-TENANCY-HOT_SPLIT
...
# Runbook: Tenancy Hot Split

## Operator Contract
- Runbook id: tenancy-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tenancy-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tenancy; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tenancy rollout status deploy/tenancy --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tenancy --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service tenancy --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service tenancy --cell $CELL --window 30m`.
