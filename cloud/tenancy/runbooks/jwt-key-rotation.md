...
## Trigger Conditions
- Page on alert `TenancyJwtKeyRotationCritical` when `oya_tenancy_jwt_key_rotation_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `TenancyJwtKeyRotationSloBurn` when `oya_tenancy_jwt_key_rotation_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `oya_tenancy_jwt_key_rotation_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `oya_tenancy_jwt_key_rotation_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `tenancy.jwt-key-rotation.customer_visible` in Zendesk.
- Trigger from Prow when `Buck2/Prow native gate evidence for tenancy-jwt-key-rotation --production-snapshot` is non-green against the latest production evidence bundle.
...
- Metric pattern: `oya_tenancy_jwt_key_rotation_error_ratio` rises before `oya_tenancy_jwt_key_rotation_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_tenancy_jwt_key_rotation_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_tenancy_jwt_key_rotation_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_tenancy_jwt_key_rotation_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_tenancy_jwt_key_rotation_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_tenancy_jwt_key_rotation_queue_depth{cell="prod-us-east-1"}'`.
...
14. Verify production snapshot evidence through Prow/Buck2: `Buck2/Prow native gate evidence for tenancy-jwt-key-rotation --production-snapshot --cell $CELL`.
15. Check Buck2 owner target: `Buck2/Prow targeted test evidence for oya-tenancy-domain jwt_key_rotation -- --nocapture`.
16. Check API contract smoke: `curl -s https://tenancy.internal.oyatie.dev/v1/tenancy/jwt-key-rotation/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
2. Does oya_tenancy_jwt_key_rotation_queue_depth grow while oya_tenancy_jwt_key_rotation_error_ratio is flat?
...
19. Confirm user impact reduced: `curl -s https://tenancy.internal.oyatie.dev/v1/tenancy/jwt-key-rotation/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: policy or key mismatch.
  - Required action: keep `tenancy-jwt-key-rotation-circuit-breaker` open until `oya_tenancy_jwt_key_rotation_error_ratio` is below 0.005 for 3 windows.
...
6. Add regression test: `Buck2/Prow targeted test evidence for oya-tenancy-domain jwt_key_rotation_incident_regression -- --nocapture`.
7. Attach Buck2/Prow fixture evidence: `Buck2/Prow native gate evidence for tenancy-jwt-key-rotation --fixture incident-jwt-key-rotation.json`.
...
9. Add dashboard panel: `update microservices/tenancy/dashboards/tenant-lifecycle-ops.json with oya_tenancy_jwt_key_rotation_error_ratio, oya_tenancy_jwt_key_rotation_lag_seconds, and oya_tenancy_jwt_key_rotation_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_tenancy_jwt_key_rotation_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- oya_tenancy_jwt_key_rotation_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- oya_tenancy_jwt_key_rotation_lag_seconds < 120 for all production cells.
- oya_tenancy_jwt_key_rotation_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include oya_tenancy_jwt_key_rotation_error_ratio, oya_tenancy_jwt_key_rotation_lag_seconds, oya_tenancy_jwt_key_rotation_queue_depth, current breaker state, and audit seal status.
