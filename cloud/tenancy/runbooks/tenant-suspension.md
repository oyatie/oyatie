...
- Trigger from Prow when `Buck2/Prow native gate evidence for tenancy-tenant-suspension --production-snapshot` is non-green against the latest production evidence bundle.
...
14. Verify production snapshot evidence through Prow/Buck2: `Buck2/Prow native gate evidence for tenancy-tenant-suspension --production-snapshot --cell $CELL`.
15. Check Buck2 owner target: `Buck2/Prow targeted test evidence for oya-tenancy-domain tenant_suspension -- --nocapture`.
...
6. Add regression test: `Buck2/Prow targeted test evidence for oya-tenancy-domain tenant_suspension_incident_regression -- --nocapture`.
7. Attach Buck2/Prow fixture evidence: `Buck2/Prow native gate evidence for tenancy-tenant-suspension --fixture incident-tenant-suspension.json`.
...
9. Add dashboard panel: `update microservices/tenancy/dashboards/dr-pairing-state.json with oya_tenancy_tenant_suspension_error_ratio, oya_tenancy_tenant_suspension_lag_seconds, and oya_tenancy_tenant_suspension_queue_depth`.
