---
doc_class: Runbook
doc_id: RUNBOOK-TENANCY-COLD_MERGE
...
# Runbook: Tenancy Cold Merge

## Operator Contract
- Runbook id: tenancy-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-tenancy-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=tenancy; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n tenancy rollout status deploy/tenancy --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service tenancy --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service tenancy --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service tenancy --cell $CELL --window 30m`.
