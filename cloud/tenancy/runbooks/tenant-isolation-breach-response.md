---
doc_class: Runbook
title: Tenant Isolation Breach Response
status: Accepted
date: 2026-05-20
...
# Runbook: Tenant Isolation Breach Response
...
- Audit event class: `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT` with ADR-0263 fields `incident_id`, `tenant_id`, `cell_id`, `microservice`, `runbook_id`, `decision_id`, `evidence_hash`, `operator_id`.
- Stop condition: mitigation has held for 30 minutes, TenancyTenantIsolationBreachResponseCritical is green, and all handoff APIs in Cross-µservice Coordination return `202 accepted`.
...
- Trigger from Prow when `Buck2/Prow native gate evidence for tenancy-tenant-isolation-breach-response --production-snapshot` is non-green against the latest production evidence bundle.
...
- Alertmanager route: `oyatie-tenancy-tenant-isolation-breach-response-critical`; silence only with incident commander approval and `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT` evidence.
- Synthetic probe: `oya ops probe tenancy tenant-isolation-breach-response --cell prod-us-east-1 --tenant synthetic-canary` returns `healthy=true`.
- Drift detector: `registry/tenancy/tenant-isolation-breach-response/expected-state.json` hash differs from live `https://tenancy.internal.oyatie.dev/v1/admin/state-hash`.

## Symptoms
- User-facing impact: tenant isolation breach response blocks or corrupts the tenancy control path for affected tenants.
- Operators see Grafana panel `dr-pairing-state / Tenant Isolation Breach Response burn rate` turn red before the primary alert resolves.
...
- Log signature `audit_emit_status=stalled event_class=EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT` means mitigation cannot be closed until replay succeeds.

## Diagnostic Steps
1. Set incident variables: `export INCIDENT_ID=INC-tenancy-tenant-isolation-breach-response-$(date -u +%Y%m%dT%H%M%SZ); export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Confirm active alerts: `curl -s https://tenancy.internal.oyatie.dev/v1/alerts?runbook=tenant-isolation-breach-response | jq .alerts`.
3. Check Kubernetes rollout: `kubectl -n tenancy rollout status deploy/tenancy-tenant-isolation-breach-response-worker --timeout=60s`.
...
14. Verify production snapshot evidence through Prow/Buck2: `Buck2/Prow native gate evidence for tenancy-tenant-isolation-breach-response --production-snapshot --cell $CELL`.
15. Check Buck2 owner target: `Buck2/Prow targeted test evidence for oya-tenancy-domain tenant_isolation_breach_response -- --nocapture`.
...
```
- Branch A (confirmed security boundary failure): use the matching mitigation block below and record `decision_branch=A` in `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT`.
- Branch B (suspected false positive): use the matching mitigation block below and record `decision_branch=B` in `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT`.
- Branch C (forensic evidence unavailable): use the matching mitigation block below and record `decision_branch=C` in `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT`.
- Branch D (customer or regulator visible impact): use the matching mitigation block below and record `decision_branch=D` in `EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT`.
...
3. Freeze risky automation: `oya flags set oya.tenancy.tenant_isolation_breach_response.incident_hold=true --cell $CELL --tenant $TENANT --reason $INCIDENT_ID`.
4. Enable circuit breaker: `oya ops breaker open tenancy-tenant-isolation-breach-response-circuit-breaker --cell $CELL --tenant $TENANT --ttl 30m --reason $INCIDENT_ID`.
5. Reduce blast radius: `kubectl -n tenancy scale deploy/tenancy-tenant-isolation-breach-response-worker --replicas=1`.
6. Protect tenant boundary: `oya tenancy quarantine --tenant $TENANT --reason tenancy-tenant-isolation-breach-response --ttl 60m`.
7. Pause promotion: incident hold PR against `dev` (plain `git`; Buck2/Prow `oya-ci-required` evidence required).
8. Drain queue safely: `oya ops tenancy tenant-isolation-breach-response drain --cell $CELL --tenant $TENANT --max-items 500 --dry-run`.
9. Execute bounded drain: `oya ops tenancy tenant-isolation-breach-response drain --cell $CELL --tenant $TENANT --max-items 500 --confirm $INCIDENT_ID`.
10. Replay missing audit events: `oya audit-chain replay --event-class EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT --incident $INCIDENT_ID --from evidence/incidents/$INCIDENT_ID.json`.
11. Rollback last deploy if causal: `kubectl -n tenancy rollout undo deploy/tenancy-tenant-isolation-breach-response-worker`.
...
6. Add regression test: `Buck2/Prow targeted test evidence for oya-tenancy-domain tenant_isolation_breach_response_incident_regression -- --nocapture`.
7. Attach Buck2/Prow fixture evidence: `Buck2/Prow native gate evidence for tenancy-tenant-isolation-breach-response --fixture incident-tenant-isolation-breach-response.json`.
...
18. Seal resolution audit: `oya audit-chain emit --event-class EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT --incident $INCIDENT_ID --field resolution=complete --field runbook=tenant-isolation-breach-response`.
19. Verify seal: `oya audit-chain verify --event-class EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT --incident $INCIDENT_ID`.
...
- feature flag oya.tenancy.tenant_isolation_breach_response.incident_hold is false for the affected tenant unless long-term hold is approved.
...
runbook_id: tenancy-tenant-isolation-breach-response
microservice: tenancy
event_class: EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT
incident_id: <INC-...>
severity: sev0
status: draft
detected_at: <UTC>
...
# Tenant Isolation Breach Response postmortem

## Summary
- What happened in tenancy/tenant-isolation-breach-response.
...
- Emit EVT-TENANCY-TENANT_ISOLATION_BREACH_RESPONSE-INCIDENT with incident_id, tenant_id, cell_id, principal_id, decision_id, evidence_hash, operator_id, runbook_id.
...
- Customer communications: use status page component `oyatie-tenancy-tenant-isolation-breach-response` and keep private details in the incident channel.
...
- Notify `identity`: `oya incident handoff --target identity --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID --severity sev0 --branch A`; expect `202 accepted`.
- Require `identity` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `compliance`: `oya incident handoff --target compliance --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID --severity sev0 --branch B`; expect `202 accepted`.
- Require `compliance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `governance`: `oya incident handoff --target governance --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID --severity sev0 --branch C`; expect `202 accepted`.
- Require `governance` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Notify `observability`: `oya incident handoff --target observability --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID --severity sev0 --branch D`; expect `202 accepted`.
- Require `observability` to return `handoff_id` and `owner_rotation`; paste both into the incident timeline.
- Observability handoff API: `oya incident handoff --target observability --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID`.
- Governance handoff API: `oya incident handoff --target governance --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID`.
- Compliance handoff API: `oya incident handoff --target compliance --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID`.
- Identity handoff API: `oya incident handoff --target identity --source tenancy --runbook tenant-isolation-breach-response --incident $INCIDENT_ID`.
...
- Existing thin runbook topic `tenant-isolation-breach-response` was preserved as the scenario anchor while replacing generic steps with concrete commands.
