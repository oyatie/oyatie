...
- Trigger from Prow when `Buck2/Prow native gate evidence for tenancy-kyb-kyc-pipeline-stalled --production-snapshot` is non-green against the latest production evidence bundle.
...
14. Verify production snapshot evidence through Prow/Buck2: `Buck2/Prow native gate evidence for tenancy-kyb-kyc-pipeline-stalled --production-snapshot --cell $CELL`.
15. Check Buck2 owner target: `Buck2/Prow targeted test evidence for oya-tenancy-domain kyb_kyc_pipeline_stalled -- --nocapture`.
...
6. Add regression test: `Buck2/Prow targeted test evidence for oya-tenancy-domain kyb_kyc_pipeline_stalled_incident_regression -- --nocapture`.
7. Attach Buck2/Prow fixture evidence: `Buck2/Prow native gate evidence for tenancy-kyb-kyc-pipeline-stalled --fixture incident-kyb-kyc-pipeline-stalled.json`.
