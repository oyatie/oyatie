...
- [ADR-0349](../../docs/adr-archive/ADR-0349-jenkins-argocd-self-hostable-ci-cd-substrate.md): GitHub Actions `oya-ci-required` is the live CI authority until owned oya-ci runner cutover, and ArgoCD is the canonical GitOps CD orchestrator that replaces manual `kubectl apply` and Helm CLI deploys. Enforced by `oya-governance-github-actions-oya-ci-required-continuity`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-governance-github-actions-oya-ci-jcasc-only`, and `oya-governance-deploy-audit-chain-emit`.
