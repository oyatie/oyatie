...
- ADR-0515 — GitHub Actions `oya-ci-required` is the live CI authority until owned oya-ci runner cutover; ArgoCD replaces manual `kubectl apply` and Helm CLI deploys, with parity, cosign, tenant namespace, JCasC, and audit-chain enforcement by `oya-governance-github-actions-oya-ci-required-continuity`, `oya-governance-argocd-application-cosign-verified`, `oya-governance-argocd-tenant-namespace-isolation`, `oya-governance-github-actions-oya-ci-jcasc-only`, and `oya-governance-deploy-audit-chain-emit`.
