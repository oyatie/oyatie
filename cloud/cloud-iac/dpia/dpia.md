...
- Event surface: automation events for this microservice include required cloud-ci/oya-ci gate outcomes, governance lane rename evidence, sharding automation decisions, and ArgoCD GitOps CD transitions.
...
- Cedar decision id, audit-chain event id, `oya-ci-required` run id, ArgoCD sync id, cosign verification result, and governance lane id.
...
- Access control: service principals, cloud-ci/oya-ci principals, ArgoCD principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
...
- Residual risk becomes low when the ADR-0347, ADR-0348, and current cloud-ci/oya-ci enforcement lanes promote from advisory/report-only to BLOCKER and pass for this microservice through `oya-ci-required`.
