...
This first IAC-001 slice is deliberately metadata-only: it emits a normalized deny-by-default ingress and explicit egress allow-list policy shape for consuming wrappers, plus tenant/cell/compliance labels and a cosign attestation metadata digest. It does not create AWS security groups, call provider APIs, configure credentials, run OpenTofu plan/apply, or claim production cloud provisioning.

## Version

`v0.1.0`, release identifier `cloud-iac/modules/aws-guest/sg-baseline/v0.1.0`.
