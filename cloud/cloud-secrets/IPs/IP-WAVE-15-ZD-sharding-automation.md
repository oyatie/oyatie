...
SCOPE-005: cloud-secrets uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: cloud-secrets uses ADR-0515 for the single protected `oya-ci-required` context and separately authorized bridge/reference CD surfaces.
