...
`cloud-secrets` keeps its existing bounded context and flat `microservices/cloud-secrets/src/` ownership under ADR-0131 and ADR-0132. This `ARCH.md` is the Wave 15-ZF architecture propagation surface for ADR-0346, ADR-0347, ADR-0348, and ADR-0515; service-specific deep architecture remains in `ARCHITECTURE.md` when that artifact exists.
...
### ADR-0515 current CI/CD authority
- ADR-0515 is retained as self-hostable CI/CD control intent, not as a parallel
...
- Integration state: PROPOSED for `cloud-secrets`; ACCEPTED waits for service wrapper implementation and signed module evidence.
- Ownership split: cloud-iac owns reusable OpenTofu primitive bodies; `cloud-secrets` owns wrapper selection, variables, SLO-driven sizing, and blast-radius review.
- Current manifest pins: on-prem/pg-cluster@v1[both], on-prem/valkey-cluster@v1[both], oci-guest/vault@v1[paid], aws-guest/kms-key@v1[paid], oyatie-as-cloud-provider/per-cell-nodepool-kata@v1[both], oyatie-as-cloud-provider/cell-audit-chain-bridge@v1[both].
...
- Capacity: `per_request` drives CPU `0.16`, RAM `512`, storage `4`, and connection pool variables.
- DR: RTO `120` seconds and RPO `1` seconds constrain backup and failover primitives.
