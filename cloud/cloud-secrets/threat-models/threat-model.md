...
doc_id: TM-CLOUD-SECRETS-WAVE-15-ZF-11
...
# STRIDE Threat Model: cloud-secrets
...
- ADR-0346: full-matrix parity must be proven by branch-protected `oya-ci-required` / owned cloud-ci evidence; local `oya verify --ci-required` or Cargo rehearsals are non-authoritative and must not be cited as merge authority.
...
- ADR-0515: ArgoCD/GitOps cosign/audit-chain lanes preserve declarative CD controls; Jenkins mirrors are disconnected/self-hosted support only after cloud-ci re-homes shared Rust gate logic and are not parallel merge authority.
