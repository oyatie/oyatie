...
| `cloud/cloud-os/` | `os/` | **DONE** — residual deleted after absorb | tree removed (manifest + 4 dual-home crate roots) | Absorb tip `integ/os` @ `9ffc6496f` (`os/manifest.json` + OWNERS + receipt); founder ACK lifted park; findings `oyatie-45t0`/#1925 still open |

## Order (do not invert)

1. **Kernel absorb** on owning integ → then delete `cloud/cloud-kernel/**` on `integ/cloud`. ✅
2. **OS absorb** after `#1643` / `integ/os` forever residual (`os/manifest.json` + dual-home OWNERS) → then delete `cloud/cloud-os/**` on `integ/cloud`. ✅ synced to `origin/dev@9a56538c7`, then burned full residual.
...
| `cloud/cloud-os/**` on `integ/cloud` | **burned** (0 files) | this tip (post-`origin/dev` sync) |
| `os/manifest.json` on `integ/os` | **present** | `9ffc6496f` |
| Dual-home crate bodies clobbered on forever? | **NO** | absorb copied OWNERS + manifest only; #1643 forever sources retained |
...
- `integ/cloud` synced through `origin/dev` tip before burn so PR deletes tip dual-home crates, not only a stale residual.
