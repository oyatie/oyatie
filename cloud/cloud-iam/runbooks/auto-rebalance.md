---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_IAM-AUTO_REBALANCE
...
# Runbook: Cloud Iam Auto Rebalance

## Operator Contract
- Runbook id: cloud-iam-auto-rebalance.
- Primary service namespace: `cloud-iam`.
...
- Safety invariant: never bypass residency, compliance pack, cosign, native promotion, tenant-isolation, or audit-chain controls to speed up mitigation.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-iam-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-iam; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-iam rollout status deploy/cloud-iam --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-iam --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-iam --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-iam --cell $CELL --window 30m`.
...
6. Re-check native promotion reconciliation and confirm no manual drift remains.
