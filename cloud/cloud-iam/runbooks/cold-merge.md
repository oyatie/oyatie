---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_IAM-COLD_MERGE
...
# Runbook: Cloud Iam Cold Merge

## Operator Contract
- Runbook id: cloud-iam-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-iam-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-iam; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-iam rollout status deploy/cloud-iam --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-iam --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-iam --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-iam --cell $CELL --window 30m`.
