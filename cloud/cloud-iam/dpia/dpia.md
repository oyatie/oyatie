...
# Data Protection Impact Assessment: cloud-iam
...
- Event surface: automation events for this microservice include Buck2/Prow required-context outcomes, governance lane rename evidence, sharding automation decisions, and native promotion transitions.
...
- Cedar decision id, audit-chain event id, oya-ci required-context id, native promotion event id, cosign verification result, and governance lane id.
...
- ADR-0349 — historical Jenkins/ArgoCD doctrine only; active CI/CD authority is ADR-0513 Kubernetes-native oya-ci/Prow plus release-conveyor-like native promotion seams, with GitHub/GitHub Actions as temporary adapters.

### DPIA Assessment
- Necessity: automation evidence is necessary to prove Buck2/Prow coverage, governance lane vocabulary, tenant placement, rebalance reversibility, dynamic shard threshold decisions, and CI/CD deployment accountability.
...
- Access control: service principals, oya-ci/Prow principals, native promotion principals, and cell-orchestrator principals are least-privilege actors governed by Cedar and tenant namespace isolation.
