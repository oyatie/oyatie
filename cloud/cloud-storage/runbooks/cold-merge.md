---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_STORAGE-COLD_MERGE
...
# Runbook: Cloud Storage Cold Merge

## Operator Contract
- Runbook id: cloud-storage-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-storage-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-storage; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-storage rollout status deploy/cloud-storage --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-storage --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-storage --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-storage --cell $CELL --window 30m`.
