---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_BILLING_TAX-COLD_MERGE
...
# Runbook: Cloud Billing Tax Cold Merge

## Operator Contract
- Runbook id: cloud-billing-tax-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-billing-tax-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-billing-tax; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-billing-tax rollout status deploy/cloud-billing-tax --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-billing-tax --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-billing-tax --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-billing-tax --cell $CELL --window 30m`.
