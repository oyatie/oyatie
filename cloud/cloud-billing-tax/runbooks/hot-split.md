---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_BILLING_TAX-HOT_SPLIT
...
# Runbook: Cloud Billing Tax Hot Split

## Operator Contract
- Runbook id: cloud-billing-tax-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-billing-tax-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-billing-tax; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-billing-tax rollout status deploy/cloud-billing-tax --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-billing-tax --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-billing-tax --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-billing-tax --cell $CELL --window 30m`.
