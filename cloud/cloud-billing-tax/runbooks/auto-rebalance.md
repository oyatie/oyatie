---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_BILLING_TAX-AUTO_REBALANCE
...
# Runbook: Cloud Billing Tax Auto Rebalance

## Operator Contract
- Runbook id: cloud-billing-tax-auto-rebalance.
- Primary service namespace: `cloud-billing-tax`.
...
- Owning team: axis-cloud-billing.
...
## Doctrine Anchors
- ADR-0513 authority update: Buck2 evidence plus the trusted Rust/Prow `oya-ci-required` context supersedes retired local verifier/gate CLI authority.
- Historical ADR-0346 lane names are provenance only; do not implement retired local verifier/gate authority.
...
- ADR-0513 authority update: Jenkins/ArgoCD are historical only; native oya-ci/Prow and release-conveyor seams own durable CI/CD.
- Historical ADR-0349 lane names are provenance only; do not implement Jenkins/ArgoCD interim authority.
...
- Trigger 7: oya-ci/Prow required-context drift blocks the release train for the sharding automation lane.
- Trigger 8: native promotion reconcile is pending for this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-billing-tax-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-billing-tax; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-billing-tax rollout status deploy/cloud-billing-tax --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-billing-tax --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-billing-tax --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-billing-tax --cell $CELL --window 30m`.
...
9. Verify native promotion controller state for `$SERVICE` and confirm the required context is current.
10. Verify oya-ci/Prow required-context evidence exists for this service before declaring the runbook complete.
...
5. If native promotion reconcile is pending, pause execution until the service declaration is current.
6. If oya-ci/Prow required-context evidence is unknown, keep the change in report-only state and collect Buck2 evidence before merge.
...
9. Keep the native controller in reconciliation-only mode; do not run manual `kubectl apply` or manual Helm CLI deploys.
...
- Evidence 6: native promotion event id and cosign verification policy result.
- Evidence 7: oya-ci/Prow required-context id proving CI coverage for this service.
- Evidence 8: Buck2 evidence bundle and `oya-ci-required` status before any merge related to this runbook.
...
6. Re-run native promotion state refresh and confirm no manual drift remains.
...
6. Confirm oya-ci/Prow required-context evidence is attached per ADR-0513.
7. Confirm native promotion did not sync unsigned images and did not cross tenant namespaces.
