...
Audience: an Oyatie operator or migration owner reading historical Wave 15-ZD doctrine propagation for `cloud-billing-tax`. The CI/CD authority portions are superseded by ADR-0513 and must not be used to author Jenkinsfiles, ArgoCD applications, retired `bin/oya` verifier flows, or gate CLI surfaces.
...
1. Treat this phase as superseded historical context; use Buck2 evidence plus the trusted Rust/Prow `oya-ci-required` controller path for `cloud-billing-tax` changes before merge.
...
1. Treat this phase as superseded historical context; do not author Jenkins or ArgoCD interim surfaces for `cloud-billing-tax`.
...
- Buck2/Prow required-context transcript for `cloud-billing-tax` once the native oya-ci lane lands.
