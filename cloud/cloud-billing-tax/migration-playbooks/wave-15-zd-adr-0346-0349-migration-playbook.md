...
1. Treat legacy `oya verify --ci-required` as optional local-feedback/provenance for `cloud-billing-tax` changes; push/merge readiness depends on branch-protected `oya-ci-required` evidence.
...
3. When `cloud-billing-tax` changes touch Rust, contracts, manifests, generated docs, or governance lanes, record branch-protected `oya-ci-required` evidence before handoff; a legacy verifier run may be attached only as local-feedback/provenance.
...
1. Treat Jenkins CI wording as historical for `cloud-billing-tax`; any self-hosted/owned CI cutover must preserve the ADR-0515 `oya-ci-required` contract.
...
5. Future owned-runner parity for `cloud-billing-tax` must preserve the same cloud-ci Rust gate packets and `oya-ci-required` context.
