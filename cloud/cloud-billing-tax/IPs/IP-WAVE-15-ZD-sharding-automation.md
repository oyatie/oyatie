...
SCOPE-005: cloud-billing-tax uses ADR-0347 lane vocabulary, so governance-owned checks cite `oya-governance-*` and not the pre-rename fitness prefix.
SCOPE-006: cloud-billing-tax keeps ADR-0349 only as historical rollout provenance; active CI/CD rollout expectations come from ADR-0513 Buck2/Prow oya-ci and release-conveyor-like native seams.
...
STANCE-019: CI/CD substrate stance is ADR-0513 Kubernetes-native oya-ci/Prow plus release-conveyor-like native promotion; GitHub/GitHub Actions are temporary adapters and ADR-0349 is historical provenance only.
...
ADR346-PURPOSE-001: Historical local verifier doctrine is superseded for active authority by Buck2 target evidence plus the trusted Rust/Prow `oya-ci-required` controller context.
ADR346-PURPOSE-002: Active evidence MUST be produced by Buck2 targets and the required oya-ci controller context before merge.
ADR346-PURPOSE-003: Retired local skip-flag doctrine is not an active merge path; Buck2/Prow lane configuration owns scoped checks.
ADR346-PURPOSE-004: Retired local exit-code doctrine is provenance only; active required-context status comes from oya-ci/Prow.
ADR346-ENFORCED-BY-001: superseded local verifier lane provenance; preserve useful kernels as Rust/Buck2 checks and oya-ci/Prow jobs.
ADR346-ENFORCED-BY-002: superseded local verifier exit-semantics provenance; active failure semantics are required-context results.
ADR346-ENFORCED-BY-003: superseded local skip-flag provenance; active scoping belongs to Buck2 targets and Prow job config.
...
ADR349-PURPOSE-002: GitHub/GitHub Actions remain temporary PR/publication and shadow-evidence adapters while oya-ci owns the required context.
...
ADR349-PURPOSE-005: Cosign verification, tenant namespace isolation, and audit-chain deploy emission remain requirements, but enforcement belongs in Rust/Buck2/Prow and native promotion seams.
...
PLAN-024: Use Buck2 evidence plus trusted Rust/Prow `oya-ci-required` before any downstream merge.
...
ACCEPT-015: The file cites ADR-0349 only as superseded provenance and records ADR-0513 as active CI/CD authority.
