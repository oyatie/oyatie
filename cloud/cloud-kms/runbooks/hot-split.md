---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_KMS-HOT_SPLIT
...
# Runbook: Cloud Kms Hot Split

## Operator Contract
- Runbook id: cloud-kms-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-kms-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-kms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-kms rollout status deploy/cloud-kms --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-kms --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-kms --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-kms --cell $CELL --window 30m`.
