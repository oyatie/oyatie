...
#[test]
fn resolve_settings_merges_sources_and_records_provenance() -> oya_cloud_intelligence_claude_agent_sdk::Result<()> {
...
#[test]
fn resolve_settings_can_skip_filesystem_sources_but_keep_managed() -> oya_cloud_intelligence_claude_agent_sdk::Result<()> {
    let _lock = ENV_LOCK.lock().unwrap();
    let config = tempdir().unwrap();
    let _env = ClaudeConfigEnvGuard::set(config.path());
    let project = tempdir().unwrap();
...
#[test]
fn resolve_settings_skips_malformed_files_and_reports_parse_errors() -> oya_cloud_intelligence_claude_agent_sdk::Result<()>
{
    let _lock = ENV_LOCK.lock().unwrap();
    let config = tempdir().unwrap();
    let _env = ClaudeConfigEnvGuard::set(config.path());
