...
async fn handle_gemini_adapter_proxy(
    ...
    if openai_stream_requested(&headers, &body) {
        let stream_result = match request_shape {
            GeminiRequestShape::OpenAiChat => {
                adapter
                    .proxy_openai_chat_stream(&api_key, body.to_vec(), headers_to_btree(&headers))
                    .await
            }
            GeminiRequestShape::AnthropicMessages => {
                adapter
                    .proxy_anthropic_messages_stream(
                        &api_key,
                        body.to_vec(),
                        headers_to_btree(&headers),
                    )
                    .await
            }
        };
        match stream_result {
            ...
            Err(GeminiAdapterError::RateLimited { retry_after_secs }) => {
                let _ = lease_context
                    .lease
                    .complete(SeatOutcome::RateLimited429, Instant::now());
                return gemini_adapter_error_to_response(GeminiAdapterError::RateLimited {
                    retry_after_secs,
                });
            }
            Err(error) => {
                let _ = lease_context
                    .lease
                    .complete(SeatOutcome::ServerError5xx, Instant::now());
                return gemini_adapter_error_to_response(error);
            }
        }
    }
...
# TYPE oya_cloud_intelligence_default_pool_ready gauge\n",
    ...
    body.push_str(
        "# HELP oya_cloud_intelligence_policy_engine_ready Policy-engine readiness probe result\n\
# TYPE oya_cloud_intelligence_policy_engine_ready gauge\n",
    );
    body.push_str(&format!(
        "oya_cloud_intelligence_policy_engine_ready {}\n",
        u8::from(policy_engine_ready)
    ));
    body.push_str(
