...
- ADR-0349: Jenkins (LTS) is the sole CI orchestrator and ArgoCD is the canonical self-hostable GitOps CD substrate; GitHub Actions is retired, and ArgoCD REPLACES manual `kubectl apply` and Helm CLI deploys. Enforced by `governance-jenkins-canonical-no-gha-residue`, `governance-argocd-application-cosign-verified`, `governance-argocd-tenant-namespace-isolation`, `governance-jenkins-jcasc-only`, and `governance-deploy-audit-chain-emit`.
