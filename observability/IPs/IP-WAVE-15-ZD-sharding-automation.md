...
slot_owner: ZF-9
sharding_role: trigger-source
---
# IP-WAVE-15-ZD-sharding-automation: Observability Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/observability/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: observability must interpret ADR-0348 through its own bounded context: observability.
SCOPE-004: observability uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: observability uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: observability uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: observability.
STANCE-002: Owner team: axis-observability.
STANCE-003: Sharding role: trigger-source.
STANCE-004: Role statement: detects load skew, shard latency, utilization, quiet hours, and automation refusal signals.
STANCE-005: Bounded context anchor: observability.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_query.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, SOC 2 Type 2, iso27001, gdpr, hipaa, kr-isms-p, NIS2, lgpd, soc2, cn-pipl-2021, fedramp, il5, DORA.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for observability.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for observability because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; observability must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where observability participates.
...
ADR347-ENFORCED-BY-001: governance-no-foundry-fitness-residue (new lane; greps the corpus and refuses any non-historical reference to `governance-*`; historical references inside ADR-0335 + ADR-0347 retirement-context paragraphs are exempted via an allowlist of file paths declared in the lane's config)
...
ADR349-PURPOSE-002: Jenkins is the sole CI orchestrator; GitHub Actions is retired as a hosted PR review CI surface.
...
ADR349-ENFORCED-BY-001: governance-jenkins-canonical-no-gha-residue (new lane; refuses newly introduced GitHub Actions CI definitions and requires Jenkinsfile/JCasC coverage for the per-microservice CI contract; promoted to BLOCKER per ADR-0359/ADR-0361 Jenkins-sole-CI doctrine)
ADR349-ENFORCED-BY-002: governance-argocd-application-cosign-verified (new lane; refuses ArgoCD Application CRD sources that reference an image without a cosign-verify policy attached per D-6 + ADR-0181; promoted to BLOCKER per ADR-0359/ADR-0361 Jenkins-sole-CI doctrine)
ADR349-ENFORCED-BY-003: governance-argocd-tenant-namespace-isolation (new lane; refuses ArgoCD Application authoring that crosses tenant namespaces without a Cedar policy gate granting cross-tenant access per D-11 + ADR-0243; promoted to BLOCKER per ADR-0359/ADR-0361 Jenkins-sole-CI doctrine)
ADR349-ENFORCED-BY-004: governance-jenkins-jcasc-only (new lane; refuses Jenkins controller state declared via the UI; every Jenkins controller state file is authored under microservices/cloud-iac/modules/<context>/jenkins/jcasc/ with declarative JCasC YAML per D-1; promoted to BLOCKER per ADR-0359/ADR-0361 Jenkins-sole-CI doctrine)
ADR349-ENFORCED-BY-005: governance-deploy-audit-chain-emit (new lane; refuses ArgoCD sync transitions that do not emit an audit-chain row per ADR-0263 D.4 deploy-event class; promoted to BLOCKER per ADR-0359/ADR-0361 Jenkins-sole-CI doctrine)
...
ROLLBACK-002: For observability, rollback starts from the audit-chain row that recorded the automation event.
