...
23. Check catalog components: `find observability/catalog -maxdepth 1 -type f | sort | rg "observability|oncall"`.
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths observability/runbooks/oncall-rotation.md,evidence/incidents/$INCIDENT_ID.json`.
...
## Resolution Steps
1. Identify code owner path: `rg "oncall_rotation|ObservabilityOncallRotationCritical|observability.oncall_rotation.incident_state" crates microservices/observability -g "!observability/runbooks/**"`.
...
8. Add SLO assertion: `update observability/observability/slos/* with alert ObservabilityOncallRotationCritical when this was a missing alert`.
9. Add dashboard panel: `update observability/dashboards/gate-eligibility.json with oya_observability_oncall_rotation_error_ratio, oya_observability_oncall_rotation_lag_seconds, and oya_observability_oncall_rotation_queue_depth`.
