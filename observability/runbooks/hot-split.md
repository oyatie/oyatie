---
doc_class: Runbook
doc_id: RUNBOOK-OBSERVABILITY-HOT_SPLIT
...
# Runbook: Observability Hot Split

## Operator Contract
- Runbook id: observability-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-observability-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=observability; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n observability rollout status deploy/observability --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service observability --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service observability --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service observability --cell $CELL --window 30m`.
