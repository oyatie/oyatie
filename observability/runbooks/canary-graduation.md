...
## Trigger Conditions
- Page on alert `ObservabilityCanaryGraduationCritical` when `oya_observability_canary_graduation_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `ObservabilityCanaryGraduationSloBurn` when `oya_observability_canary_graduation_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `oya_observability_canary_graduation_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `oya_observability_canary_graduation_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `observability.canary-graduation.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p oya-dev-cli -- gate validate observability-canary-graduation --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `oya_observability_canary_graduation_error_ratio` rises before `oya_observability_canary_graduation_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `oya_observability_canary_graduation_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `oya_observability_canary_graduation_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_observability_canary_graduation_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_observability_canary_graduation_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=oya_observability_canary_graduation_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p oya-dev-cli -- gate validate observability-canary-graduation --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p oya-observability-domain canary_graduation -- --nocapture`.
16. Check API contract smoke: `curl -s https://observability.internal.oyatie.dev/v1/observability/canary-graduation/incident-handoff -H "x-oya-tenant: $TENANT"`.
...
23. Check catalog components: `find observability/catalog -maxdepth 1 -type f | sort | rg "observability|canary"`.
24. Confirm no cross-cell spread: `oya ops cells query --metric oya_observability_canary_graduation_error_ratio --window 30m --threshold 0.02`.
...
2. Does oya_observability_canary_graduation_queue_depth grow while oya_observability_canary_graduation_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths observability/runbooks/canary-graduation.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://observability.internal.oyatie.dev/v1/observability/canary-graduation/incident-handoff/health -H "x-oya-tenant: $TENANT"`.
...
- Branch A: single tenant isolated.
  - Required action: keep `observability-canary-graduation-circuit-breaker` open until `oya_observability_canary_graduation_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "canary_graduation|ObservabilityCanaryGraduationCritical|observability.canary_graduation.incident_state" crates microservices/observability -g "!observability/runbooks/**"`.
2. Patch domain invariant: `edit oya-observability-domain where canary_graduation state transition is validated`.
...
6. Add regression test: `cargo test -p oya-observability-domain canary_graduation_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p oya-dev-cli -- gate validate observability-canary-graduation --fixture incident-canary-graduation.json`.
8. Add SLO assertion: `update observability/observability/slos/* with alert ObservabilityCanaryGraduationCritical when this was a missing alert`.
9. Add dashboard panel: `update observability/dashboards/tenant-slo-overview.json with oya_observability_canary_graduation_error_ratio, oya_observability_canary_graduation_lag_seconds, and oya_observability_canary_graduation_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric oya_observability_canary_graduation_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `oya-observability-domain`: inspect for canary_graduation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `oya-cloud-observability-api`: inspect for canary_graduation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `oya-dev-cli`: inspect for canary_graduation invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- oya_observability_canary_graduation_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- oya_observability_canary_graduation_lag_seconds < 120 for all production cells.
- oya_observability_canary_graduation_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include oya_observability_canary_graduation_error_ratio, oya_observability_canary_graduation_lag_seconds, oya_observability_canary_graduation_queue_depth, current breaker state, and audit seal status.
