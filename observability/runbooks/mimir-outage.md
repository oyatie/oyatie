...
## Trigger Conditions
- Page on alert `ObservabilityMimirOutageCritical` when `observability_mimir_outage_error_ratio > 0.02` for 10 minutes in any production cell.
- Page on alert `ObservabilityMimirOutageSloBurn` when `observability_mimir_outage_lag_seconds > 300` for 2 consecutive evaluator windows.
- Open a sev0 if `observability_mimir_outage_correctness_ratio < 0.9999` and the affected label set includes `tenant_id` or `principal_id`.
- Open a sev1 if `observability_mimir_outage_queue_depth > 5000` for 15 minutes or retry backlog grows by more than 20 percent in one 5 minute window.
- Trigger from customer report when Support tags the case `observability.mimir-outage.customer_visible` in Zendesk.
- Trigger from CI when `cargo run -p dev-cli -- gate validate observability-mimir-outage --production-snapshot` exits non-zero against the latest production evidence bundle.
...
- Metric pattern: `observability_mimir_outage_error_ratio` rises before `observability_mimir_outage_lag_seconds`; if lag rises first, suspect dependency saturation rather than local regression.
- Metric pattern: `observability_mimir_outage_queue_depth` increases while pod CPU stays below 40 percent; suspect downstream refusal or feature flag deadlock.
- Tenant-specific shape: one `tenant_id` dominates labels in `observability_mimir_outage_queue_depth`; isolate before fleet mitigation.
...
7. Check Prometheus fast burn: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=observability_mimir_outage_error_ratio{cell="prod-us-east-1"}'`.
8. Check lag: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=observability_mimir_outage_lag_seconds{cell="prod-us-east-1"}'`.
9. Check queue: `curl -G https://mimir.dev.oyatie.internal/prometheus/api/v1/query --data-urlencode 'query=observability_mimir_outage_queue_depth{cell="prod-us-east-1"}'`.
...
14. Run production snapshot gate: `cargo run -p dev-cli -- gate validate observability-mimir-outage --production-snapshot --cell $CELL`.
15. Check Cargo owner crate: `cargo test -p observability-domain mimir_outage -- --nocapture`.
16. Check API contract smoke: `curl -s https://observability.internal.oyatie.dev/v1/observability/mimir-outage/incident-handoff -H "x-tenant: $TENANT"`.
...
23. Check catalog components: `find microservices/observability/catalog -maxdepth 1 -type f | sort | rg "observability|mimir"`.
24. Confirm no cross-cell spread: `oya ops cells query --metric observability_mimir_outage_error_ratio --window 30m --threshold 0.02`.
...
2. Does observability_mimir_outage_queue_depth grow while observability_mimir_outage_error_ratio is flat?
...
15. Protect evidence: `oya evidence freeze --incident $INCIDENT_ID --paths microservices/observability/runbooks/mimir-outage.md,evidence/incidents/$INCIDENT_ID.json`.
...
19. Confirm user impact reduced: `curl -s https://observability.internal.oyatie.dev/v1/observability/mimir-outage/incident-handoff/health -H "x-tenant: $TENANT"`.
...
- Branch A: storage or quorum pressure.
  - Required action: keep `observability-mimir-outage-circuit-breaker` open until `observability_mimir_outage_error_ratio` is below 0.005 for 3 windows.
...
## Resolution Steps
1. Identify code owner path: `rg "mimir_outage|ObservabilityMimirOutageCritical|observability.mimir_outage.incident_state" crates microservices/observability -g "!microservices/observability/runbooks/**"`.
2. Patch domain invariant: `edit observability-domain where mimir_outage state transition is validated`.
...
6. Add regression test: `cargo test -p observability-domain mimir_outage_incident_regression -- --nocapture`.
7. Add gate evidence: `cargo run -p dev-cli -- gate validate observability-mimir-outage --fixture incident-mimir-outage.json`.
8. Add SLO assertion: `update microservices/observability/slos/* with alert ObservabilityMimirOutageCritical when this was a missing alert`.
9. Add dashboard panel: `update microservices/observability/dashboards/tenant-slo-overview.json with observability_mimir_outage_error_ratio, observability_mimir_outage_lag_seconds, and observability_mimir_outage_queue_depth`.
...
14. Watch burn rate: `oya ops watch --metric observability_mimir_outage_error_ratio --threshold 0.005 --window 30m --cell $CELL`.
...
- `observability-domain`: inspect for mimir_outage invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 1.
- `cloud-observability-api`: inspect for mimir_outage invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 2.
- `dev-cli`: inspect for mimir_outage invariants, alert emission, and ADR-0263 evidence fields before touching adjacent code path 3.
...
- observability_mimir_outage_error_ratio < 0.005 for 3 consecutive 10 minute windows.
- observability_mimir_outage_lag_seconds < 120 for all production cells.
- observability_mimir_outage_queue_depth is draining and not growing for the affected tenant.
...
## Handoff Notes
- Do not hand off with only the alert name; include observability_mimir_outage_error_ratio, observability_mimir_outage_lag_seconds, observability_mimir_outage_queue_depth, current breaker state, and audit seal status.
