---
doc_class: Runbook
doc_id: RUNBOOK-OBSERVABILITY-AUTO_REBALANCE
...
# Runbook: Observability Auto Rebalance

## Operator Contract
- Runbook id: observability-auto-rebalance.
...
- Owning team: axis-observability.
...
- ADR-0349 enforced_by lanes: `governance-jenkins-canonical-no-gha-residue`; `governance-argocd-application-cosign-verified`; `governance-argocd-tenant-namespace-isolation`; `governance-jenkins-jcasc-only`; `governance-deploy-audit-chain-emit`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-observability-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=observability; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n observability rollout status deploy/observability --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service observability --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service observability --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service observability --cell $CELL --window 30m`.
...
10. Verify Jenkins required-check evidence exists for this service before declaring the runbook complete.
...
6. If Jenkins required-check is unknown, keep the change in report-only state and run local `oya verify --ci-required` before push.
...
- Evidence 7: Jenkins build id proving required-check coverage for this service.
