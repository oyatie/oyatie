...
- ADR-0349 enforced_by lanes: `oya-governance-jenkins-canonical-no-gha-residue`; `oya-governance-argocd-application-cosign-verified`; `oya-governance-argocd-tenant-namespace-isolation`; `oya-governance-jenkins-jcasc-only`; `oya-governance-deploy-audit-chain-emit`.
...
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service observability --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service observability --cell $CELL --window 30m`.
