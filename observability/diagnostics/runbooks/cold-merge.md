---
doc_class: Runbook
doc_id: RUNBOOK-DIAGNOSTICS-COLD_MERGE
...
# Runbook: Diagnostics Cold Merge

## Operator Contract
- Runbook id: diagnostics-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-diagnostics-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=diagnostics; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n diagnostics rollout status deploy/diagnostics --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service diagnostics --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service diagnostics --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service diagnostics --cell $CELL --window 30m`.
