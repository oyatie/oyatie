---
doc_class: Runbook
doc_id: RUNBOOK-DETECTION-AUTO_REBALANCE
...
# Runbook: Detection Auto Rebalance

## Operator Contract
- Runbook id: detection-auto-rebalance.
- Primary service namespace: `detection`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-detection-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=detection; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n detection rollout status deploy/detection --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service detection --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service detection --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service detection --cell $CELL --window 30m`.
