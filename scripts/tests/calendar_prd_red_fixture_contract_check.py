...
calendar inventory provenance, then remains RED until future Build cards create
...
REQUIRED_TOP_LEVEL_SOURCES = {
    ...
    "kanban:t_9aca64a9#calendar-source-map-and-backlog",
}
...
def validate_manifest_index_source_lock(index: dict[str, Any]) -> None:
    ...
    row = calendar_rows[0]
    require(row.get("manifest") == "oya/calendar/manifest.json", "calendar manifest-index row must point to oya/calendar/manifest.json")
    require(row.get("prd") == "specs/microservices/calendar.json", "calendar manifest-index row must point to specs/microservices/calendar.json")
    require(row.get("authority_status") == "source-authority-reconciled-by-t_ff8bab02", "calendar authority_status drifted")
    require("inventory/provenance only" in str(row.get("authority_boundary", "")).lower(), "calendar authority boundary must state inventory/provenance only")
...
def validate_contract_source_files() -> None:
    for family, raw_path in CONTRACT_SOURCE_FILES.items():
        ...
        require(path.exists(), f"calendar {family} source contract must exist at {raw_path}")
        require(path.is_file(), f"calendar {family} source contract must be a file: {raw_path}")
        require(not raw_path.endswith(GENERATED_SUFFIX), f"calendar {family} source contract must not be a generated face: {raw_path}")
        require(
            raw_path.startswith("oya/calendar/contracts/"),
            f"calendar {family} source contract must live under oya/calendar/contracts/: {raw_path}",
        )
