...
#[test]
fn checked_in_standards_docs_do_not_reintroduce_retired_local_commands() {
    let failures = gate::standards_retired_local_command_failures(Path::new(&repo_root()));
    assert!(failures.is_empty(), "{failures:?}");
}

#[test]
fn standards_retired_local_command_scan_rejects_local_gate_doc_vcs_examples() {
    let failures = gate::standards_retired_local_command_text_failures(
        "docs/standards/example.md",
        "Primary command:\n\
         oya gate validate proto3-authoring --scope microservices\n\
         oya doc openapi\n\
         Safer replacement: oya vcs for lifecycle\n\
         cargo fmt --check\n",
    );
    for expected in [
        "oya gate validate",
        ...
        "oya vcs",
        "cargo fmt --check",
        "Buck2 plus Prow/oya-ci-required",
    ] {
        assert!(
            failures.iter().any(|failure| failure.contains(expected)),
            "missing {expected:?} in {failures:?}"
...
#[test]
fn cargo_buck2_authority_boundary_scan_rejects_raw_cargo_authority() {
    let failures = gate::cargo_buck2_authority_boundary_text_failures(
        "docs/standards/multispectrum-review.md",
        "Run cargo run -p oya-check-dependency-seam, then cargo check -p <crate>, cargo nextest -p <crate>, and cargo nextest --workspace. Buck2 exists but Cargo workspace is sufficient.",
    );
    for expected in [
...
#[test]
fn cloud_billing_tax_authority_scan_rejects_retired_cli_and_helm_authority() {
    let root = temp_dir("cloud-billing-tax-authority");
    let ip_dir = root.join("cloud/cloud-billing-tax/IPs");
    let helm_dir = root.join("cloud/cloud-billing-tax/iac/k8s/helm/templates");
    std::fs::create_dir_all(&ip_dir).unwrap();
    std::fs::create_dir_all(&helm_dir).unwrap();
    std::fs::write(
        ip_dir.join("stale.md"),
    ...
    let failures = gate::cloud_billing_tax_authority_failures(&root);
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("cargo run -q -p oya-dev-cli -- gate validate")),
...
fn cloud_iam_authority_scan_rejects_retired_cli_cargo_and_helm_authority() {
    ...
    let helm_dir = root.join("cloud/cloud-iam/iac/k8s/helm/templates");
    std::fs::create_dir_all(&runbook_dir).unwrap();
    std::fs::create_dir_all(&helm_dir).unwrap();
    std::fs::write(
        runbook_dir.join("stale.md"),
        "Production validation still says cargo run -p oya-dev-cli and cargo test --features hermetic.\n",
    )
    .unwrap();
    std::fs::write(
        helm_dir.join("deployment.yaml"),
    ...
    let failures = gate::cloud_iam_authority_failures(&root);
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("cargo run -p oya-dev-cli")),
    ...
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("cargo test --features hermetic")),
        "{failures:?}"
    );
    assert!(
        failures
