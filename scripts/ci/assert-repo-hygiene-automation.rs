...
const STANDARDS_EXTERNAL_SUBSTRATE_CLEAN_FILES: &[&str] = &[
    "docs/standards/hyperscaler-best-practices.md",
...
const DESIGN_SYSTEM_SPEC_AUTHORITY_CLEAN_FILES: &[&str] = &[
    "specs/design-spec-maturity-claims.json",
    "specs/design-system/audit-evidence-timeline.json",
    "specs/design-system/catalog.json",
    "specs/design-system/cloud-cell-topology-map.json",
    "specs/design-system/communication-thread-list.json",
    "specs/design-system/entity-action-policy-preview.json",
    "specs/design-system/foundry-agent-run-timeline.json",
    "specs/design-system/ontology-graph-explorer.json",
    "specs/design-system/ops-deployment-status-panel.json",
    "specs/design-system/policy-disclosure-banner.json",
    "specs/design-system/score-card-result-table.json",
    "specs/design-system/spec-diff-viewer.json",
    "specs/design-system/tenant-context-switcher.json",
...
const SCHEMA_REGISTRY_SPEC_AUTHORITY_CLEAN_FILES: &[&str] = &[
...
const IDENTITY_VENDOR_NATIVE_BOUNDARY_FORBIDDEN_PHRASES: &[&str] = &[
...
const DESIGN_SYSTEM_SPEC_RETIRED_AUTHORITY_PHRASES: &[&str] = &[
    "target/debug/oya",
    "oya gate",
    "oya-dev-cli",
    "crates/oya-dev-cli",
...
const SCHEMA_REGISTRY_SPEC_RETIRED_AUTHORITY_PHRASES: &[&str] = &[
    "target/debug/oya",
    "oya gate",
    "oya verify",
    "oya-dev-cli",
...
const DEPLOYMENT_OPS_CONTRACT_RETIRED_AUTHORITY_PHRASES: &[&str] = &[
    "OpenTofu",
    "opentofu",
    "tofu",
    "Argo CD",
...
const CLOUD_BILLING_STALE_AUTHORITY_PHRASES: &[&str] = &[
    "canonical local pre-push verifier",
    "Jenkins (LTS) and ArgoCD are the canonical self-hostable CI/CD substrates",
    "Jenkins (LTS) and ArgoCD are the two canonical self-hostable CI/CD substrates",
    "ArgoCD is the canonical GitOps CD orchestrator",
    ...
    "cargo run --release",
    "cargo test --features hermetic",
    "cargo test -p oya-cloud-billing",
    "cargo check -p oya-cloud-billing",
    "cargo fmt",
    "cargo check",
    "cargo clippy",
    "cargo nextest",
...
const STANDARDS_EXTERNAL_SUBSTRATE_PHRASES: &[&str] = &[
    "ArgoCD",
    "Argo CD",
    "Argo Workflows/Rollouts",
    "Argo Rollouts",
...
pub fn cloud_billing_authority_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();
    let mut files = Vec::new();

    if let Err(error) =
        collect_markdown_yaml_json_files(root, Path::new("cloud/cloud-billing"), &mut files)
    {
        failures.push(format!("cloud-billing authority scan failed: {error}"));
    }
    files.sort();
    files.dedup();
    ...
    for rel in files {
        ...
        let Ok(text) = fs::read_to_string(&path) else {
            failures.push(format!(
                "{rel}: read failed during cloud-billing authority scan"
            ));
            continue;
        };
        for phrase in CLOUD_BILLING_STALE_AUTHORITY_PHRASES {
            if text.contains(phrase) {
                failures.push(format!(
                    "{rel}: stale cloud-billing active authority phrase present: {phrase:?}; use ADR-0513 Buck2/Prow `oya-ci-required`, CUE/KRM desired state, and native release-conveyor wording"
                ));
            }
        }
    }

    let helm_dir = root.join("cloud/cloud-billing/iac/k8s/helm");
    if helm_dir.exists() {
        failures.push(
...
pub fn spec_failures(spec: &str) -> Vec<String> {
    ...
    for (needle, message) in [
        ...
        (
            "\"Claude Code harness Buck2 evidence posture scan\"",
            "documentation sprawl automation targets must include the Claude Code harness Buck2 evidence posture scan",
        ),
        (
            "\"claude_code_harness_buck2_evidence_scan\"",
        ...
        (
            "\"identity vendor native desired-state boundary scan\"",
            "documentation sprawl automation targets must include the identity vendor native boundary scan",
        ),
        (
            "\"identity_vendor_native_boundary_scan\"",
        ...
        (
            "\"standards_retired_external_substrate_scan\"",
            "documentation sprawl policy must record the standards retired external substrate guard",
        ),
        (
            "\"microservice spec retired CI/CD and local gate authority scan\"",
            "documentation sprawl automation targets must include the microservice spec retired authority scan",
        ),
        (
            "\"microservice_spec_authority_scan\"",
            "documentation sprawl policy must record the microservice spec Buck2/Prow authority guard",
        ),
        (
            "\"design-system spec retired local validator authority scan\"",
            "documentation sprawl automation targets must include the design-system spec retired local validator scan",
        ),
        (
            "\"design_system_spec_authority_scan\"",
            "documentation sprawl policy must record the design-system spec Buck2/Prow authority guard",
        ),
        (
            "\"schema/registry spec retired local validator authority scan\"",
            "documentation sprawl automation targets must include the schema/registry retired local validator scan",
        ),
        (
            "\"schema_registry_spec_authority_scan\"",
            "documentation sprawl policy must record the schema/registry Buck2/Prow authority guard",
        ),
        (
            "\"deployment-ops contract retired external substrate authority scan\"",
            "documentation sprawl automation targets must include the deployment-ops retired external substrate scan",
        ),
        (
            "\"deployment_ops_contract_authority_scan\"",
...
pub fn claude_code_harness_buck2_evidence_text_failures(text: &str) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in CLAUDE_CODE_HARNESS_RAW_CARGO_EVIDENCE_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{CLAUDE_CODE_HARNESS_STANDARD_PATH}: Claude Code harness evidence posture contains raw Cargo/local-green phrase {phrase:?}; use Buck2 target evidence plus Prow/oya-ci-required wording"
            ));
        }
    }
    ...
    for required in [
        ...
        require_contains(
            ...
            required,
            &mut failures,
            CLAUDE_CODE_HARNESS_STANDARD_PATH,
        );
    }
...
pub fn claude_code_harness_buck2_evidence_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();
    let text = match fs::read_to_string(root.join(CLAUDE_CODE_HARNESS_STANDARD_PATH)) {
        Ok(text) => text,
        Err(error) => {
            failures.push(format!(
                "{CLAUDE_CODE_HARNESS_STANDARD_PATH}: read failed: {error}"
            ));
            return failures;
        }
    };

    failures.extend(claude_code_harness_buck2_evidence_text_failures(&text));
    failures
}
...
pub fn identity_vendor_native_boundary_text_failures(text: &str) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in IDENTITY_VENDOR_NATIVE_BOUNDARY_FORBIDDEN_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{IDENTITY_VENDOR_ISOLATION_STANDARD_PATH}: identity vendor isolation standard contains stale shared-path or chart authority phrase {phrase:?}; use identity-owned native desired-state package wording plus Buck2/Prow evidence"
            ));
        }
    }
    for required in [
        "identity-owned native desired-state packages",
        "service-owned KRM/CUE package",
        "Buck2/Prow IaC reference scan",
        "native controller operation ledger",
        "legacy shared-path chart checks are\nhistorical provenance",
    ] {
        require_contains(
            text,
            required,
            &mut failures,
            IDENTITY_VENDOR_ISOLATION_STANDARD_PATH,
        );
    }
...
pub fn identity_vendor_native_boundary_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();
    let text = match fs::read_to_string(root.join(IDENTITY_VENDOR_ISOLATION_STANDARD_PATH)) {
        Ok(text) => text,
        Err(error) => {
            failures.push(format!(
                "{IDENTITY_VENDOR_ISOLATION_STANDARD_PATH}: read failed: {error}"
            ));
            return failures;
        }
    };

    failures.extend(identity_vendor_native_boundary_text_failures(&text));
    failures
}
...
pub fn standards_retired_external_substrate_text_failures(
    clean_file: &str,
    text: &str,
) -> Vec<String> {
    let mut failures = Vec::new();
    for phrase in STANDARDS_EXTERNAL_SUBSTRATE_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: standards doc contains retired external substrate phrase {phrase:?}; use native release-conveyor/KRM, Buck2/Prow, or compatibility-adapter wording"
            ));
        }
    }
    failures
...
pub fn standards_retired_external_substrate_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();

    for clean_file in STANDARDS_EXTERNAL_SUBSTRATE_CLEAN_FILES {
        let text = match fs::read_to_string(root.join(clean_file)) {
            Ok(text) => text,
            Err(error) => {
                failures.push(format!("{clean_file}: read failed: {error}"));
        ...
        failures.extend(standards_retired_external_substrate_text_failures(
            clean_file, &text,
        ));
    }
...
pub fn microservice_spec_authority_text_failures(clean_file: &str, text: &str) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in MICROSERVICE_SPEC_RETIRED_AUTHORITY_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: microservice spec contains retired CI/CD or local gate authority phrase {phrase:?}; use Buck2/Prow oya-ci-required and native release-conveyor/KRM wording"
            ));
        }
    }
    for phrase in MICROSERVICE_SPEC_REQUIRED_AUTHORITY_PHRASES {
        if !text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: microservice spec missing required Buck2/Prow authority phrase {phrase:?}"
            ));
        }
    }
...
pub fn microservice_spec_authority_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();

    for clean_file in MICROSERVICE_SPEC_AUTHORITY_CLEAN_FILES {
        let text = match fs::read_to_string(root.join(clean_file)) {
            Ok(text) => text,
            Err(error) => {
                failures.push(format!("{clean_file}: read failed: {error}"));
        ...
        failures.extend(microservice_spec_authority_text_failures(clean_file, &text));
    }

    failures
}

pub fn design_system_spec_authority_text_failures(clean_file: &str, text: &str) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in DESIGN_SYSTEM_SPEC_RETIRED_AUTHORITY_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: design-system/spec file contains retired local validator or ops authority phrase {phrase:?}; use Buck2/Prow and native release-conveyor/KRM wording"
            ));
        }
    }
    if !text.contains("buck2 build //:") {
        failures.push(format!(
            "{clean_file}: design-system/spec file missing Buck2 authority command wording"
        ));
    }
...
pub fn design_system_spec_authority_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();

    for clean_file in DESIGN_SYSTEM_SPEC_AUTHORITY_CLEAN_FILES {
        let text = match fs::read_to_string(root.join(clean_file)) {
            Ok(text) => text,
            Err(error) => {
                failures.push(format!("{clean_file}: read failed: {error}"));
        ...
        failures.extend(design_system_spec_authority_text_failures(
            clean_file, &text,
        ));
    }
...
pub fn schema_registry_spec_authority_text_failures(clean_file: &str, text: &str) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in SCHEMA_REGISTRY_SPEC_RETIRED_AUTHORITY_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: schema/registry spec contains retired command or substrate authority phrase {phrase:?}; use Buck2/Prow command wording and native Rust/Prow/Kubernetes seams"
            ));
        }
    }
    if !text.contains("buck2 build //:") {
        failures.push(format!(
            "{clean_file}: schema/registry spec missing Buck2 authority command wording"
        ));
    }
...
pub fn schema_registry_spec_authority_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();

    for clean_file in SCHEMA_REGISTRY_SPEC_AUTHORITY_CLEAN_FILES {
        let text = match fs::read_to_string(root.join(clean_file)) {
            Ok(text) => text,
            Err(error) => {
                failures.push(format!("{clean_file}: read failed: {error}"));
        ...
        failures.extend(schema_registry_spec_authority_text_failures(
            clean_file, &text,
        ));
    }
...
pub fn deployment_ops_contract_authority_text_failures(
    clean_file: &str,
    text: &str,
) -> Vec<String> {
    let mut failures = Vec::new();

    for phrase in DEPLOYMENT_OPS_CONTRACT_RETIRED_AUTHORITY_PHRASES {
        if text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: deployment-ops contract contains retired external substrate or local entrypoint phrase {phrase:?}; use native release-conveyor/KRM, Buck2/Prow, and controller-reconciliation wording"
            ));
        }
    }
    for phrase in [
        "native release-conveyor",
        "KRM/CUE desired state",
        "Buck2/Prow",
        "oya-ci-required",
        ...
        if !text.contains(phrase) {
            failures.push(format!(
                "{clean_file}: deployment-ops contract missing required native authority phrase {phrase:?}"
            ));
        }
    }
...
pub fn deployment_ops_contract_authority_failures(root: &Path) -> Vec<String> {
    let mut failures = Vec::new();

    for clean_file in DEPLOYMENT_OPS_CONTRACT_AUTHORITY_CLEAN_FILES {
        let text = match fs::read_to_string(root.join(clean_file)) {
            Ok(text) => text,
            Err(error) => {
                failures.push(format!("{clean_file}: read failed: {error}"));
        ...
        failures.extend(deployment_ops_contract_authority_text_failures(
            clean_file, &text,
        ));
    }
...
pub fn evaluate(root: &Path) -> Evaluation {
    ...
    Evaluation {
        ...
        design_system_spec_authority_clean_files: DESIGN_SYSTEM_SPEC_AUTHORITY_CLEAN_FILES.len(),
        schema_registry_spec_authority_clean_files: SCHEMA_REGISTRY_SPEC_AUTHORITY_CLEAN_FILES
            .len(),
