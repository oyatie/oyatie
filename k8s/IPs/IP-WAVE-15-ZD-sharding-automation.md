...
slot_owner: ZF-9
sharding_role: runtime-substrate-owner
---
# IP-WAVE-15-ZD-sharding-automation: Cloud K8s Sharding Automation Stance

## 1. Scope
SCOPE-001: This IP is scoped only to `microservices/cloud-k8s/IPs/IP-WAVE-15-ZD-sharding-automation.md` for the ZF-9 artifact lane.
...
SCOPE-003: cloud-k8s must interpret ADR-0348 through its own bounded context: cloud-k8s.
SCOPE-004: cloud-k8s uses ADR-0346 as the local verifier contract for any downstream implementation PR that turns this plan into code.
SCOPE-005: cloud-k8s uses ADR-0347 lane vocabulary, so governance-owned checks cite `governance-*` and not the pre-rename fitness prefix.
SCOPE-006: cloud-k8s uses ADR-0349 for self-hostable CI/CD rollout expectations once Wave 15-ZE authors Jenkinsfile, Helm, and ArgoCD surfaces.
...
STANCE-001: Microservice: cloud-k8s.
STANCE-002: Owner team: axis-cloud-k8s.
STANCE-003: Sharding role: runtime-substrate-owner.
STANCE-004: Role statement: keeps Kubernetes cell runtime, namespace, and workload placement constraints aligned.
STANCE-005: Bounded context anchor: cloud-k8s.
STANCE-006: Cell placement class input: Tier-1.
STANCE-007: Capacity scaling dimension input: per_capability.
STANCE-008: Compliance pack input: kr, eu, us, us-healthcare, jp, sg, au, in, br, ae, ksa, hipaa, SOC 2 Type 2, iso27001, gdpr, kr-isms-p, NIS2, DORA, lgpd, soc2, cn-pipl-2021, fedramp, il5.
...
STANCE-010: Canonical autosharding stance is control_plane_driven; operator-picked placement is not the default for cloud-k8s.
...
STANCE-012: Dynamic sharding stance is explicit-threshold only; default-fill is rejected for cloud-k8s because load characteristics are service-specific.
STANCE-013: Audit stance is emit-on-every-event; cloud-k8s must not create silent tenant, cell, or shard transitions.
...
STANCE-015: Observability stance is metric-triggered; p99, utilization, skew, refusal, and rollback labels must be visible where cloud-k8s participates.
...
ROLLBACK-002: For cloud-k8s, rollback starts from the audit-chain row that recorded the automation event.
