...
//! `k8s-cluster-lifecycle-api` (the cluster-CRUD API) and
//! `k8s-sla-observability-api` (control-plane uptime / provisioning
//! latency) — will depend on THIS port, never on a concrete adapter. The
//! adapters ([`k8s-control-plane-host-adapter-capi`] for the live
