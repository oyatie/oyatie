...
ip_id: IP-journey-j87-workload-runtime
...
# IP-journey-j87-workload-runtime - cloud-k8s

## Goal

Implement the `workload-runtime` slice for `cloud-k8s` so j87 can satisfy `FedRAMP-High + DoD-IL5/IL6` without leaking tenant data, collapsing provider-BYOK and encryption-BYOK meanings, or bypassing Cedar.
...
| File | Purpose | Notes |
|---|---|---|
| `microservices/cloud-k8s/contracts/openapi/j87-workload-runtime-v1.yaml` | OpenAPI 3.2.0 REST surface | External read/write or admin action |
| `microservices/cloud-k8s/contracts/asyncapi/j87-workload-runtime-events-v1.yaml` | AsyncAPI 3.1.0 event surface | Emits ADR-0263 events |
| `microservices/cloud-k8s/contracts/proto/j87-workload-runtime-v1.proto` | proto3 internal RPC | Service-to-service call path |
| `microservices/cloud-k8s/policy/j87-workload-runtime.cedar` | Cedar permit/forbid bundle | Deny-wins gate |
| `microservices/cloud-k8s/runbooks/j87-workload-runtime-rollback.md` | Rollback and incident path | Includes regulator deadline handling |
| `microservices/cloud-k8s/tests/j87_workload_runtime_test.rs` | Integration tests | Positive, negative, rollback, audit |
...
- `J87CloudK8SWorkloadRuntimeStarted`.
- `J87CloudK8SWorkloadRuntimePermitted`.
- `J87CloudK8SWorkloadRuntimeDenied`.
- `J87CloudK8SWorkloadRuntimeCommitted`.
- `J87CloudK8SWorkloadRuntimeRolledBack`.

## Implementation rows

### IP row 001 - cloud-k8s workload-runtime
Scope: implement one cloud-k8s behavior for j87, not a cross-service refactor.
