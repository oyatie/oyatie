...
acceptance_lanes: [yaml-lint, branch-protection-conformance, oya-governance-authority-cohesion, oya-governance-hyperscaler-maturity-claims]
---
...
Add cloud-k8s-specific BLOCKER lanes to `.github/branch-protection.yaml` (`oya-cloud-k8s-iac-smoke`, `oya-check-cis-k8s-benchmark`, `oya-check-cosign-admission`, `oya-check-etcd-encryption`, `oya-check-istio-strict-mtls`) and register `HG-CLOUD-K8S` in `/specs/hyperscaler-gates.json` per ADR-0123.
...
branches:
  dev:
    required_status_checks:
      # ADDED by IP-014:
      - oya-cloud-k8s-iac-smoke
      - oya-check-cis-k8s-benchmark
      - oya-check-cosign-admission
      - oya-check-etcd-encryption
      - oya-check-istio-strict-mtls
  staging:
    required_status_checks:
      # ADDED:
      - oya-cloud-k8s-iac-smoke
  ...
  "gates": [
    {
      ...
      "criteria": [
        ...
        {"id": "HG-CLOUD-K8S-03", "name": "nsa-cisa-k8s-hardening-v1.2", "verifier": "oya-check-nsa-k8s-hardening"},
        {"id": "HG-CLOUD-K8S-04", "name": "istio-strict-mtls-mesh-wide", "verifier": "oya-check-istio-strict-mtls"},
        {"id": "HG-CLOUD-K8S-05", "name": "etcd-kms-envelope-encryption", "verifier": "oya-check-etcd-encryption"},
        {"id": "HG-CLOUD-K8S-06", "name": "cosign-admission-enforced", "verifier": "oya-check-cosign-admission"},
        {"id": "HG-CLOUD-K8S-07", "name": "kubernetes-api-proxy-only-path", "verifier": "oya-check-kubernetes-api-proxy-only-path"},
        {"id": "HG-CLOUD-K8S-08", "name": "per-pack-cluster-boundary", "verifier": "oya-check-data-residency-conformance"},
...
- HG-CLOUD-K8S registers green: `cargo run -p oya-dev-cli -- gate list --owner axis-cloud` shows HG-CLOUD-K8S
