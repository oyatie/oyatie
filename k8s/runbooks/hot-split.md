---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_K8S-HOT_SPLIT
...
# Runbook: Cloud K8S Hot Split

## Operator Contract
- Runbook id: cloud-k8s-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-k8s-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-k8s; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-k8s rollout status deploy/cloud-k8s --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-k8s --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service cloud-k8s --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service cloud-k8s --cell $CELL --window 30m`.
