---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_KMS-AUTO_REBALANCE
...
# Runbook: Cloud Kms Auto Rebalance

## Operator Contract
- Runbook id: cloud-kms-auto-rebalance.
- Primary service namespace: `cloud-kms`.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-kms-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-kms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-kms rollout status deploy/cloud-kms --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-kms --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service cloud-kms --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service cloud-kms --cell $CELL --window 30m`.
