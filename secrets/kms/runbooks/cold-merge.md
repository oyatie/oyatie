---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_KMS-COLD_MERGE
...
# Runbook: Cloud Kms Cold Merge

## Operator Contract
- Runbook id: cloud-kms-cold-merge.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-kms-cold-merge-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-kms; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-kms rollout status deploy/cloud-kms --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-kms --runbook cold-merge --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_cold_merge_candidate_count --service cloud-kms --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_cold_merge_quiet_hours --service cloud-kms --cell $CELL --window 30m`.
