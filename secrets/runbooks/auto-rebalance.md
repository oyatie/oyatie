...
## Doctrine Anchors
- ADR-0346 historical control intent: full-matrix parity is required, but local `oya verify --ci-required` is rehearsal only; branch-protected GitHub Actions `oya-ci-required` is live merge authority until owned `oya-ci` cutover.
...
- Trigger 7: branch-protected `oya-ci-required` drift or cloud-ci/GitOps evidence drift blocks the release train for the sharding automation lane.
...
4. Verify primary metric: `oya metrics query oya_sharding_auto_rebalance_candidate_count --service cloud-secrets --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_auto_rebalance_migration_seconds_p99 --service cloud-secrets --cell $CELL --window 30m`.
...
10. Verify `oya-ci-required` evidence, and any cloud-ci/GitOps mirror evidence after cutover, exists for this service before declaring the runbook complete.
...
6. If `oya-ci-required` evidence is unknown, keep the change in report-only state; local `oya verify --ci-required` may be used only as rehearsal and does not authorize push or merge.
...
- Evidence 7: GitHub Actions `oya-ci-required` run id, or owned `oya-ci` run id after cutover, proving the branch-protected gate for this service.
- Evidence 8: optional local rehearsal result if run; it must not substitute for branch-protected `oya-ci-required` evidence.
...
6. Confirm `oya-ci-required` evidence, and any cloud-ci/GitOps mirror evidence after cutover, is attached per the current authority chain.
