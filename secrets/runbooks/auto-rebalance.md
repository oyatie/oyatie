---
doc_class: Runbook
doc_id: RUNBOOK-CLOUD_SECRETS-AUTO_REBALANCE
...
# Runbook: Cloud Secrets Auto Rebalance

## Operator Contract
- Runbook id: cloud-secrets-auto-rebalance.
- Primary service namespace: `cloud-secrets`.
...
- Owning team: axis-cloud-secrets.
...
## Doctrine Anchors
- ADR-0346 historical control intent: full-matrix parity is required, but local `oya verify --ci-required` is rehearsal only; branch-protected GitHub Actions `presubmit` is live merge authority until owned `ci` cutover.
...
- ADR-0349 historical control intent: ArgoCD/GitOps remains the declarative CD direction; Jenkins mirrors are not parallel merge authority and may only support disconnected/self-hosted contexts after cloud-ci re-homes shared Rust gate logic.
...
- Trigger 4: `sharding_auto_rebalance_candidate_count` crosses the declared threshold for two evaluator windows.
- Trigger 5: `sharding_auto_rebalance_migration_seconds_p99` threatens the service SLO budget or promotion-gate quiet window.
- Trigger 6: governance reports missing sharding automation coverage for this service.
- Trigger 7: branch-protected `presubmit` drift or cloud-ci/GitOps evidence drift blocks the release train for the sharding automation lane.
- Trigger 8: ArgoCD reports a pending sync tied to this service after a sharding automation manifest change.

## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-cloud-secrets-auto-rebalance-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=cloud-secrets; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
2. Verify service deployment: `kubectl -n cloud-secrets rollout status deploy/cloud-secrets --timeout=60s`.
3. Verify alerts: `oya observability alerts list --service cloud-secrets --runbook auto-rebalance --since 30m`.
4. Verify primary metric: `oya metrics query sharding_auto_rebalance_candidate_count --service cloud-secrets --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query sharding_auto_rebalance_migration_seconds_p99 --service cloud-secrets --cell $CELL --window 30m`.
...
10. Verify `presubmit` evidence, and any cloud-ci/GitOps mirror evidence after cutover, exists for this service before declaring the runbook complete.
...
6. If `presubmit` evidence is unknown, keep the change in report-only state; local `oya verify --ci-required` may be used only as rehearsal and does not authorize push or merge.
...
- Evidence 3: audit-chain event `autosharding.auto_rebalance.validated` with metric snapshots for `sharding_auto_rebalance_candidate_count` and `sharding_auto_rebalance_migration_seconds_p99`.
...
- Evidence 7: GitHub Actions `presubmit` run id, or owned `ci` run id after cutover, proving the branch-protected gate for this service.
- Evidence 8: optional local rehearsal result if run; it must not substitute for branch-protected `presubmit` evidence.
...
6. Confirm `presubmit` evidence, and any cloud-ci/GitOps mirror evidence after cutover, is attached per the current authority chain.
