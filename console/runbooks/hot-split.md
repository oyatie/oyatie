---
doc_class: Runbook
doc_id: RUNBOOK-OPS_DASHBOARD_CONTROL_CENTER-HOT_SPLIT
...
# Runbook: Ops Dashboard Control Center Hot Split

## Operator Contract
- Runbook id: ops-dashboard-control-center-hot-split.
...
## Preflight Checklist
1. Set incident context: `export INCIDENT_ID=INC-ops-dashboard-control-center-hot-split-$(date -u +%Y%m%dT%H%M%SZ); export SERVICE=ops-dashboard-control-center; export CELL=prod-us-east-1; export TENANT=synthetic-canary`.
...
3. Verify alerts: `oya observability alerts list --service ops-dashboard-control-center --runbook hot-split --since 30m`.
4. Verify primary metric: `oya metrics query oya_sharding_hot_split_threshold_breach_total --service ops-dashboard-control-center --cell $CELL --window 30m`.
5. Verify secondary metric: `oya metrics query oya_sharding_hot_split_duration_seconds_p99 --service ops-dashboard-control-center --cell $CELL --window 30m`.
